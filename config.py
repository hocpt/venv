...
class Config:
    ...
    APP_MODULE_DIR = os.path.join(basedir, 'app')
    STATIC_DIR = os.path.join(APP_MODULE_DIR, 'static')
    SCREENSHOTS_SUBDIR_NAME_CONFIG = 'screenshots' # Tên thư mục con trong static
    
    # Đường dẫn vật lý đến thư mục cha chứa tất cả các ảnh screenshots
    # Ví dụ: /path/to/your/project/hpt10/app/static/screenshots/
    SCREENSHOT_STORAGE_PATH = os.path.join(STATIC_DIR, SCREENSHOTS_SUBDIR_NAME_CONFIG)
    UPLOAD_FOLDER = SCREENSHOT_STORAGE_PATH 

    MAX_CONTENT_LENGTH = 16 * 1024 * 1024 
    ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg'}

    #BASE_DIR = os.path.abspath(os.path.dirname(__file__))
    #SCREENSHOT_STORAGE_PATH = os.path.join(BASE_DIR, 'app_screenshots')
    ...
    SCHEDULER_JOBSTORES = {
            ...
            # 'default': {'type': 'sqlalchemy', 'url': SQLALCHEMY_DATABASE_URI} # Cách cấu hình qua dict
             'default': {'type': 'sqlalchemy', 'url': 'sqlite:///scheduler_test.db'}
        }
    SCHEDULER_EXECUTORS = {
    ...
    NEO4J_PASSWORD = os.environ.get("NEO4J_PASSWORD") # Nên đặt trong .env
    NEO4J_DATABASE = "neo4j"

    if not NEO4J_PASSWORD:
         print("CRITICAL WARNING: Missing Neo4j configuration (NEO4J_PASSWORD) in .env or environment variables!")
