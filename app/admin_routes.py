...
def view_history():
    try:
        ...
        # Giả sử hàm này trả về list of dicts và tổng số items
        # history_data, total_items = db.get_interaction_history(page=page, per_page=per_page)
        # **Tạm thời dùng hàm lấy hết (không phân trang)**
        history_data = db.get_all_interaction_history() # Cần tạo hàm này
        ...
        print(f"Lỗi nghiêm trọng load history: {e}")
        flash("Lỗi không mong muốn khi tải lịch sử tương tác.", "error")
        history_data = []
        # total_items = 0

    # Logic phân trang sẽ cần thêm thư viện như Flask-SQLAlchemy hoặc tính toán offset/limit thủ công
    return render_template('admin_history.html',
                           title="Lịch sử Tương tác",
                           history=history_data)
                           # pagination=pagination_object # Nếu dùng thư viện phân trang
