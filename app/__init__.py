...
def create_app(config_class=config.Config):
    ...
    def serve_screenshot_for_app(app_name, filename):
        ...
        logger.info(f"--- SERVE SCREENSHOT START ---")
        logger.info(f"Raw Request - App Name: '{app_name}', Filename: '{filename}'")
        logger.info(f"Configured SCREENSHOT_STORAGE_PATH: '{screenshots_base_dir}'")
        ...
        logger.info(f"Sanitized - Safe App Name: '{safe_app_name}', Safe Filename: '{safe_filename}'")


        if not safe_app_name or not safe_filename:
            logger.error("Invalid app_name or filename after sanitization.")
            return "App name hoặc filename không hợp lệ.", 400
            
        app_specific_dir_relative_to_storage = safe_app_name # Tên thư mục con
        full_dir_to_serve_from = os.path.join(screenshots_base_dir, app_specific_dir_relative_to_storage)
        
        logger.info(f"Attempting to serve '{safe_filename}' from directory '{full_dir_to_serve_from}'")

        # Kiểm tra sự tồn tại của thư mục và file một cách rõ ràng
        if not os.path.isdir(full_dir_to_serve_from):
            logger.error(f"Directory DOES NOT EXIST: '{full_dir_to_serve_from}'")
            raise NotFound(f"Thư mục ứng dụng '{safe_app_name}' không tìm thấy.")

        path_to_file_abs = os.path.abspath(os.path.join(full_dir_to_serve_from, safe_filename))
        logger.info(f"Absolute path to file for isfile check: '{path_to_file_abs}'")

        if not os.path.isfile(path_to_file_abs):
            logger.error(f"File IS NOT FOUND at (checked by os.path.isfile): '{path_to_file_abs}'")
            
            # Log nội dung thư mục để debug
            try:
                if os.path.exists(full_dir_to_serve_from):
                    logger.debug(f"Contents of directory '{full_dir_to_serve_from}': {os.listdir(full_dir_to_serve_from)}")
                else:
                    logger.warning(f"Directory '{full_dir_to_serve_from}' does not exist for listing.")
            except Exception as e_list:
                logger.error(f"Error listing directory '{full_dir_to_serve_from}': {e_list}")
            
            raise NotFound(f"File '{safe_filename}' không tồn tại trong thư mục của ứng dụng.")
        else:
            logger.info(f"File FOUND at (checked by os.path.isfile): '{path_to_file_abs}'")
            
        try:
            # send_from_directory nhận thư mục và tên file (tương đối với thư mục đó)
            return send_from_directory(full_dir_to_serve_from, safe_filename, as_attachment=False)
        except NotFound: # Lỗi này sẽ được ném bởi send_from_directory nếu nó cũng không tìm thấy
            logger.error(f"send_from_directory could not find '{safe_filename}' in '{full_dir_to_serve_from}'")
            response = make_response("File ảnh không tìm thấy trên server (send_from_directory).", 404)
            response.headers["Content-Type"] = "text/plain"
            return response
        except Exception as e:
            logger.error(f"Error during send_from_directory for '{safe_filename}': {e}", exc_info=True)
