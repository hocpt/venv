...
# Trong app/database.py
def get_screen_definitions_for_app(app_name: str, activity_name: str | None = None) -> list[dict] | None:
    """
    Lấy các cấu hình PIE cho một app_name.
    - Nếu activity_name được cung cấp (không phải None), chỉ lấy các định nghĩa khớp cả app_name và activity_name.
    - Nếu activity_name là None, lấy TẤT CẢ các định nghĩa cho app_name đó, 
      bất kể giá trị activity_name trong DB (cụ thể hoặc NULL).
    """
    # Sử dụng logger của Flask app nếu có, nếu không dùng print
    logger = current_app.logger if current_app else print
    
    conn = get_db_connection()
    if not conn:
        logger.error(f"get_screen_definitions_for_app: Không thể lấy kết nối DB cho app {app_name}")
        return None # Trả về None nếu không có kết nối
    
    cur = None
    screen_defs = []
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        params_list = [app_name] # Tham số đầu tiên luôn là app_name
        
        if activity_name is not None: # Trường hợp 1: activity_name được chỉ định cụ thể
            sql = """
            ...
            """
            params_list.append(activity_name)
            logger.debug(f"DB Querying screen_defs for '{app_name}' AND activity '{activity_name}'")
        else: # Trường hợp 2: activity_name là None (lấy tất cả cho app_name)
            sql = """
                SELECT definition_id, app_name, activity_name, logical_screen_name, 
                       defined_screen_id, identifying_elements_json, description, 
                       created_at, updated_at
                FROM screen_definitions 
                WHERE app_name = %s 
                ORDER BY activity_name NULLS FIRST, logical_screen_name; 
            """
            # params_list đã chứa app_name
            logger.debug(f"DB Querying ALL screen_defs for '{app_name}' (activity_name filter is None)")
            
        cur.execute(sql, tuple(params_list)) # Chuyển params_list thành tuple
        rows = cur.fetchall()
        
        for row_data in rows:
            def_dict = dict(row_data) # Chuyển DictRow thành dict Python thuần túy
            
            # Đảm bảo các key quan trọng luôn tồn tại, dù giá trị có thể là None
            if 'description' not in def_dict:
                def_dict['description'] = None 
            if 'identifying_elements_json' not in def_dict:
                # Mặc định là list rỗng nếu key thiếu hoặc giá trị là None từ DB,
                # vì template sẽ dùng |length hoặc lặp qua nó.
                def_dict['identifying_elements_json'] = [] 
            elif def_dict['identifying_elements_json'] is None: # Nếu DB trả về NULL cho JSONB
                def_dict['identifying_elements_json'] = []
            elif not isinstance(def_dict['identifying_elements_json'], list):
                # Trường hợp hiếm: JSONB trong DB không phải là array, cố gắng parse lại
                logger.warning(f"identifying_elements_json for def_id {def_dict.get('definition_id')} is not a list ({type(def_dict['identifying_elements_json'])}). Value: {str(def_dict['identifying_elements_json'])[:100]}")
                # Bạn có thể thử json.loads nếu nó là chuỗi, hoặc để là list rỗng
                def_dict['identifying_elements_json'] = []


            # Xử lý chuyển đổi datetime object sang ISO string
            for dt_field in ['created_at', 'updated_at']:
                if def_dict.get(dt_field) and isinstance(def_dict[dt_field], datetime):
                    try:
                        def_dict[dt_field] = def_dict[dt_field].isoformat()
                    except Exception as e_iso:
                        logger.warning(f"Could not convert datetime field '{dt_field}' to ISO string: {e_iso}")
                        def_dict[dt_field] = str(def_dict[dt_field]) # Fallback to string
            
            screen_defs.append(def_dict)
        
        logger.debug(f"DB: Found {len(screen_defs)} definitions for app '{app_name}' (activity filter: '{activity_name}')")
        return screen_defs
        
    except psycopg2.Error as db_err: # Bắt lỗi psycopg2 cụ thể
        logger.error(f"Lỗi CSDL psycopg2 khi lấy screen_definitions cho app {app_name}: {db_err}", exc_info=True)
        # Không rollback ở đây vì đây là SELECT
        return None # Trả về None khi có lỗi CSDL
    except Exception as e:
        logger.error(f"Lỗi không mong muốn khi lấy screen_definitions cho app {app_name}: {e}", exc_info=True)
...
def add_screen_definition(app_name: str, activity_name: str | None, 
    ...
    try:
        ...
        if cur: cur.close()
        if conn: conn.close()
def get_screen_definition_by_id(definition_id: int) -> dict | None:
    """Lấy chi tiết một screen_definition bằng definition_id."""
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        ...
        sql = """
            SELECT definition_id, app_name, activity_name, logical_screen_name, 
                   defined_screen_id, identifying_elements_json, description, 
                   created_at, updated_at
            FROM screen_definitions WHERE definition_id = %s;
        """
        cur.execute(sql, (definition_id,))
        row = cur.fetchone()
        if row:
                def_dict = dict(row)
                # Chuyển đổi datetime sang ISO string nếu cần, không cần xử lý đặc biệt cho description
                # vì None là giá trị Python hợp lệ.
                if def_dict.get('created_at') and isinstance(def_dict['created_at'], datetime): 
                    def_dict['created_at'] = def_dict['created_at'].isoformat()
                if def_dict.get('updated_at') and isinstance(def_dict['updated_at'], datetime): 
                    def_dict['updated_at'] = def_dict['updated_at'].isoformat()
                return def_dict
        return None
        
    except Exception as e:
        # current_app.logger.error(f"Lỗi khi lấy screen_definition by ID {definition_id}: {e}", exc_info=True)
        print(f"Lỗi khi lấy screen_definition by ID {definition_id}: {e}") # Fallback logging
        return None
    finally:
        if cur: cur.close()
        if conn: conn.close()

def update_screen_definition(definition_id: int, app_name: str, activity_name: str | None, 
                             logical_screen_name: str, defined_screen_id: str, 
                             identifying_elements_json: list, # Nhận list Python
                             description: str | None) -> tuple[bool, str | None]:
    """Cập nhật một screen_definition."""
    conn = get_db_connection()
    if not conn: return False, "Không thể kết nối CSDL."
    cur = None
    if not isinstance(identifying_elements_json, list) or \
       not all(isinstance(item, dict) for item in identifying_elements_json):
        return False, "identifying_elements_json phải là một list các dictionary."
    ...
    try:
        ...
        sql = """
            UPDATE screen_definitions SET
                app_name = %s,
                activity_name = %s,
                logical_screen_name = %s,
                defined_screen_id = %s,
                identifying_elements_json = %s,
                description = %s
                -- updated_at sẽ tự động cập nhật bởi trigger
        ...
        conn.commit()
        if cur.rowcount == 0:
            return False, "Không tìm thấy definition_id để cập nhật."
        # current_app.logger.info(f"Đã cập nhật screen_definition ID: {definition_id}")
        return True, None
    except psycopg2.IntegrityError as e:
        # current_app.logger.error(f"Lỗi IntegrityError khi cập nhật screen_definition {definition_id}: {e}")
        conn.rollback()
        error_msg = f"Lỗi CSDL: {e.diag.message_detail if e.diag else str(e)}. Có thể logical_screen_name hoặc defined_screen_id bị trùng."
        return False, error_msg
    except Exception as e:
        # current_app.logger.error(f"Lỗi khi cập nhật screen_definition {definition_id}: {e}", exc_info=True)
        conn.rollback()
...
def delete_screen_definition(definition_id: int) -> tuple[bool, str | None]:
    """Xóa một screen_definition."""
    conn = get_db_connection()
    if not conn: return False, "Không thể kết nối CSDL."
    cur = None
    try:
        cur = conn.cursor()
        sql = "DELETE FROM screen_definitions WHERE definition_id = %s;"
        cur.execute(sql, (definition_id,))
        conn.commit()
        if cur.rowcount == 0:
            return False, "Không tìm thấy definition_id để xóa."
        # current_app.logger.info(f"Đã xóa screen_definition ID: {definition_id}")
        return True, None
    except Exception as e:
        # current_app.logger.error(f"Lỗi khi xóa screen_definition {definition_id}: {e}", exc_info=True)
        conn.rollback()
        return False, f"Lỗi không xác định: {str(e)}"
    finally:
        if cur: cur.close()
...
def get_all_screen_definitions(page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    """
    Lấy tất cả các screen_definitions với phân trang.
    Trả về (list_of_definitions, total_items) hoặc (None, None) nếu lỗi.
    """
    conn = get_db_connection()
    if not conn:
        print("get_all_screen_definitions: Không thể lấy kết nối DB.")
        return None, None
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM screen_definitions;")
        ...
        if total_items > 0:
            ...
            sql = """
                SELECT definition_id, app_name, activity_name, logical_screen_name, 
                       ...
                       created_at, updated_at
                FROM screen_definitions 
                ORDER BY app_name, logical_screen_name
                LIMIT %s OFFSET %s;
            """
            # print(f"DEBUG DB (get_all_screen_definitions): SQL: {sql.strip()} with LIMIT {per_page} OFFSET {offset}")
            cur.execute(sql, (per_page, offset))
            rows = cur.fetchall()
            for row in rows:
                row_dict = dict(row)
                ...
                defs_page.append(row_dict)
        
        # print(f"DEBUG DB (get_all_screen_definitions): Fetched {len(defs_page)} defs for page {page}. Total: {total_items}")
        return defs_page, total_items
    except Exception as e:
        print(f"Lỗi khi lấy tất cả screen_definitions: {e}")
        import traceback
        traceback.print_exc()
        return None, None
    finally:
...
def get_screen_definition_by_defined_id(defined_screen_id: str, app_name: str) -> dict | None:
    """Lấy chi tiết một screen_definition bằng defined_screen_id và app_name."""
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        ...
        sql = """
            SELECT definition_id, app_name, activity_name, logical_screen_name, 
                   defined_screen_id, identifying_elements_json, description 
            FROM screen_definitions 
            WHERE defined_screen_id = %s AND app_name = %s;
        """
        cur.execute(sql, (defined_screen_id, app_name))
        row = cur.fetchone()
        return dict(row) if row else None
    except Exception as e:
        # current_app.logger.error(f"Lỗi khi lấy screen_definition by defined_id '{defined_screen_id}' for app '{app_name}': {e}", exc_info=True)
        print(f"Lỗi khi lấy screen_definition by defined_id '{defined_screen_id}' for app '{app_name}': {e}")
        return None
    finally:
        if cur: cur.close()
        if conn: conn.close()

def get_pie_definition_by_screen_id(app_name: str, defined_screen_id: str) -> dict | None:
    """
    Fetches a PIE definition record from the screen_definitions table.
    Returns the full record if found, otherwise None.
    """
    db = None
    try:
        # Assuming get_db() provides a valid database connection
        # If your project uses a different way to get db connection, adapt it.
        if 'get_db' in current_app.extensions: # Common pattern if using Flask-SQLAlchemy or similar
            db = current_app.extensions['get_db']()
        else: # Fallback to a simpler direct connection for this example
            db_uri = current_app.config.get('DATABASE_URI_PSYCOPG2') or current_app.config.get('SQLALCHEMY_DATABASE_URI')
            if not db_uri:
                current_app.logger.error("DATABASE_URI_PSYCOPG2 or SQLALCHEMY_DATABASE_URI not configured.")
                return None
            db = psycopg2.connect(db_uri)

        with db.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            sql = """
                SELECT id, app_name, activity_name, logical_name, screen_id, conditions, created_at, updated_at
                FROM screen_definitions
                WHERE app_name = %s AND screen_id = %s;
            """
            cur.execute(sql, (app_name, defined_screen_id))
            record = cur.fetchone()
            if record:
        ...
        current_app.logger.error(f"Database error in get_pie_definition_by_screen_id: {e}")
        return None
    finally:
        if db and not ('get_db' in current_app.extensions): # Close if we opened it directly
            db.close()

def get_pie_conditions_from_db(app_name: str, defined_screen_id: str) -> list[dict] | None:
    """
    Lấy danh sách các điều kiện (identifying elements) cho một PIE definition cụ thể.
    Trả về list các dict điều kiện, hoặc list rỗng nếu không có điều kiện,
    hoặc None nếu PIE definition không tìm thấy.
    """
    logger = current_app.logger
    conn = None
    cur = None
    conditions_list = None # Khởi tạo là None để phân biệt không tìm thấy PIE và PIE không có conditions

    logger.debug(f"DATABASE: Lấy PIE conditions cho app='{app_name}', defined_screen_id='{defined_screen_id}'")

    try:
        conn = get_db_connection()
        if not conn:
            logger.error("DATABASE: get_pie_conditions_from_db - Không thể kết nối CSDL.")
            return None # Lỗi kết nối

        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        
        # Tên cột chứa JSON conditions là 'identifying_elements_json'
        sql = """
            SELECT identifying_elements_json 
            FROM screen_definitions
            WHERE app_name = %s AND defined_screen_id = %s;
        """
        cur.execute(sql, (app_name, defined_screen_id))
        row = cur.fetchone()

        if row:
            # Tìm thấy PIE definition
            json_data_from_db = row['identifying_elements_json']
            if json_data_from_db:
                if isinstance(json_data_from_db, str): # Nếu DB trả về string
                    conditions_list = json.loads(json_data_from_db)
                elif isinstance(json_data_from_db, list): # Nếu DB đã trả về list (ví dụ JSONB)
                    conditions_list = json_data_from_db
                else:
                    logger.warning(f"DATABASE: Kiểu dữ liệu không mong đợi cho identifying_elements_json: {type(json_data_from_db)}")
                    conditions_list = [] # Coi như không có conditions hợp lệ
            else:
                # PIE definition có, nhưng không có conditions nào được lưu (cột là NULL hoặc JSON rỗng)
                conditions_list = [] 
            logger.debug(f"DATABASE: Tìm thấy {len(conditions_list)} conditions cho PIE {app_name}/{defined_screen_id}")
        else:
            # Không tìm thấy PIE definition -> conditions_list vẫn là None
            logger.warning(f"DATABASE: Không tìm thấy PIE definition cho app='{app_name}', defined_screen_id='{defined_screen_id}'")
            
    except json.JSONDecodeError as je:
        logger.error(f"DATABASE: Lỗi giải mã JSON conditions cho PIE {app_name}/{defined_screen_id}: {je}", exc_info=True)
        conditions_list = [] # Coi như không có conditions hợp lệ nếu JSON lỗi
    except psycopg2.Error as db_err:
        logger.error(f"DATABASE: Lỗi CSDL khi lấy PIE conditions cho {app_name}/{defined_screen_id}: {db_err}", exc_info=True)
        conditions_list = None # Lỗi DB, không thể xác định
    except Exception as e:
        logger.error(f"DATABASE: Lỗi không mong muốn khi lấy PIE conditions: {e}", exc_info=True)
        conditions_list = None # Lỗi không xác định
    finally:
        if cur:
            cur.close()
        if conn:
...
def update_pie_conditions_in_db(app_name: str, defined_screen_id: str, new_conditions_list: list[dict]) -> tuple[bool, str | None]:
    """
    Cập nhật (ghi đè) toàn bộ danh sách điều kiện cho một PIE definition đã tồn tại.
    """
    logger = current_app.logger
    conn = None
    cur = None
    
    logger.info(f"DATABASE: Cập nhật PIE conditions cho app='{app_name}', defined_screen_id='{defined_screen_id}' với {len(new_conditions_list)} điều kiện mới.")

    try:
        conn = get_db_connection()
        if not conn:
            logger.error("DATABASE: update_pie_conditions_in_db - Không thể kết nối CSDL.")
            return False, "Lỗi kết nối cơ sở dữ liệu."

        cur = conn.cursor()

        # Bước 1: Kiểm tra xem PIE definition có tồn tại không (SỬA TÊN CỘT Ở ĐÂY)
        check_sql = "SELECT definition_id FROM screen_definitions WHERE app_name = %s AND defined_screen_id = %s;"
        cur.execute(check_sql, (app_name, defined_screen_id))
        definition_record = cur.fetchone()

        if not definition_record:
            logger.warning(f"DATABASE: Không tìm thấy PIE Definition cho app='{app_name}', defined_screen_id='{defined_screen_id}' để cập nhật conditions.")
            return False, f"Không tìm thấy Định nghĩa PIE cho ứng dụng '{app_name}' và ID màn hình '{defined_screen_id}'."
        
        # definition_id_db = definition_record[0] # ID nội bộ của bản ghi screen_definitions

        # Bước 2: Cập nhật cột identifying_elements_json (hoặc tên cột JSON của bạn)
        # của bản ghi screen_definitions tương ứng.
        
        new_conditions_json_str = None
        if isinstance(new_conditions_list, list): # new_conditions_list có thể rỗng
            try:
                new_conditions_json_str = json.dumps(new_conditions_list)
            except TypeError as te:
                logger.error(f"DATABASE: Lỗi chuyển đổi new_conditions_list sang JSON cho PIE {app_name}/{defined_screen_id}: {te}", exc_info=True)
                return False, "Dữ liệu điều kiện mới không hợp lệ để chuyển sang JSON."
        else: # Nếu new_conditions_list không phải là list (ví dụ None)
            logger.warning(f"DATABASE: new_conditions_list không phải là list, sẽ lưu là NULL hoặc JSON array rỗng.")
            new_conditions_json_str = json.dumps([]) # Lưu là JSON array rỗng


        # SỬA TÊN CỘT TRONG MỆNH ĐỀ WHERE
        update_sql = """
            UPDATE screen_definitions
            SET identifying_elements_json = %s, 
                updated_at = NOW()
            WHERE app_name = %s AND defined_screen_id = %s; 
        """
        #                       ^^^^^^^^^^^^^^^^^ Đảm bảo đây là cột đúng
        
        cur.execute(update_sql, (new_conditions_json_str, app_name, defined_screen_id))
        
        if cur.rowcount == 0:
            # Điều này không nên xảy ra nếu bước kiểm tra ở trên đã thành công,
            # nhưng để phòng trường hợp race condition hoặc lỗi logic.
            conn.rollback()
            logger.warning(f"DATABASE: Không có dòng nào được cập nhật cho PIE {app_name}/{defined_screen_id} dù đã tìm thấy trước đó. Rollback.")
            return False, "Không cập nhật được dòng nào trong CSDL (có thể ID đã bị thay đổi?)."

        conn.commit()
        logger.info(f"DATABASE: Đã cập nhật thành công PIE conditions cho app='{app_name}', defined_screen_id='{defined_screen_id}'.")
        return True, None

    except psycopg2.Error as db_err:
        if conn:
            conn.rollback()
        logger.error(f"DATABASE: Lỗi CSDL khi cập nhật PIE conditions cho {app_name}/{defined_screen_id}: {db_err}", exc_info=True)
        # Lỗi gốc từ DB: column "screen_id" does not exist
        error_message_from_db = str(db_err) 
        if "column \"screen_id\" does not exist" in error_message_from_db:
             return False, f"Lỗi cơ sở dữ liệu: Cột 'screen_id' không tồn tại trong truy vấn cập nhật. Cần dùng 'defined_screen_id'."
        return False, f"Lỗi cơ sở dữ liệu: {error_message_from_db}"
    except Exception as e:
        if conn:
            conn.rollback()
        logger.error(f"DATABASE: Lỗi không mong muốn khi cập nhật PIE conditions: {e}", exc_info=True)
        return False, f"Lỗi không mong muốn của hệ thống: {e}"
    finally:
        if cur:
            cur.close()
        if conn:
...
def check_defined_screen_id_exists(app_name: str, defined_screen_id: str) -> bool:
    """
    Kiểm tra xem một defined_screen_id đã tồn tại cho một app_name cụ thể chưa.
    """
    logger = current_app.logger
    conn = None
    cur = None
    ...
    try:
        conn = get_db_connection() 
        if not conn:
            logger.error("DB_CHECK_PIE_ID: Không thể kết nối CSDL.")
            return True # Giả sử tồn tại nếu không thể kiểm tra để tránh ghi đè

        cur = conn.cursor()
        sql = "SELECT 1 FROM screen_definitions WHERE app_name = %s AND defined_screen_id = %s LIMIT 1;"
        cur.execute(sql, (app_name, defined_screen_id))
        exists = cur.fetchone() is not None
        logger.debug(f"DB_CHECK_PIE_ID: Kết quả kiểm tra cho app '{app_name}', defined_id '{defined_screen_id}': {exists}")
    except psycopg2.Error as e:
        logger.error(f"DB_CHECK_PIE_ID: Lỗi CSDL khi kiểm tra defined_screen_id '{defined_screen_id}' cho app '{app_name}': {e}", exc_info=True)
        return True # Giả sử tồn tại nếu có lỗi DB để tránh ghi đè
    except Exception as e:
        logger.error(f"DB_CHECK_PIE_ID: Lỗi không mong muốn khi kiểm tra defined_screen_id: {e}", exc_info=True)
        return True # Giả sử tồn tại nếu có lỗi không mong muốn
    finally:
        if cur:
            cur.close()
        if conn:
...
def create_new_pie_definition_from_node(
    app_name: str,
    ...
    """
    Tạo một định nghĩa PIE (Screen Definition) mới trong cơ sở dữ liệu PostgreSQL.
    Bao gồm cả việc lưu các điều kiện nhận diện.
    """
    logger = current_app.logger
    conn = None
    cur = None
    new_definition_id_from_db = None
    
    # Bước 0: Kiểm tra xem new_defined_screen_id đã tồn tại cho app_name này chưa
    if check_defined_screen_id_exists(app_name, new_defined_screen_id):
        error_msg = f"Defined Screen ID '{new_defined_screen_id}' đã tồn tại cho ứng dụng '{app_name}'. Vui lòng chọn ID khác."
        logger.warning(f"DATABASE: {error_msg}")
        return False, error_msg, None

    try:
        conn = get_db_connection() 
        if not conn:
            logger.error("DATABASE: Không thể kết nối CSDL để tạo PIE definition.")
            return False, "Lỗi kết nối cơ sở dữ liệu.", None
        
        cur = conn.cursor()

        identifying_elements_json_str = None
        if conditions and isinstance(conditions, list):
            try:
                identifying_elements_json_str = json.dumps(conditions)
            except TypeError as te:
                logger.error(f"DATABASE: Lỗi khi chuyển đổi conditions sang JSON: {te}", exc_info=True)
                return False, "Dữ liệu điều kiện không hợp lệ để chuyển sang JSON.", None
        
        # QUAN TRỌNG: Thay thế 'logical_screen_name' và 'pie_conditions_data' 
        # bằng tên cột chính xác trong bảng 'screen_definitions' của bạn.
        # Giả sử:
        # - Cột cho tên logic là 'logical_screen_name'
        # - Cột cho JSON điều kiện là 'pie_conditions_data' (BẠN CẦN THAY THẾ NẾU KHÁC)
        # - Cột ID tự tăng là 'definition_id'
        sql = """
            INSERT INTO screen_definitions 
                (app_name, activity_name, logical_screen_name, defined_screen_id, 
                 identifying_elements_json, description, created_at, updated_at) 
            VALUES (%s, %s, %s, %s, %s, %s, NOW(), NOW())
            RETURNING definition_id; 
        """
        ...
        params = (
            ...
            identifying_elements_json_str, # Đây là giá trị cho cột 'pie_conditions_data' (hoặc tên cột đúng của bạn)
            description    
        )
        
        logger.debug(f"DATABASE: Chuẩn bị thực thi SQL cho create_new_pie_definition: {sql} với params (phần tử đầu): {params[:4]}")
        cur.execute(sql, params)
        
        result_row = cur.fetchone()
        if result_row and result_row[0] is not None:
            new_definition_id_from_db = int(result_row[0])
            conn.commit()
            logger.info(f"DATABASE: Đã tạo PIE definition thành công cho app '{app_name}', defined_id '{new_defined_screen_id}' với DB ID: {new_definition_id_from_db}")
            return True, None, new_definition_id_from_db
        else:
            conn.rollback() 
            logger.error(f"DATABASE: Không thể lấy new_definition_id sau khi INSERT cho app '{app_name}', defined_id '{new_defined_screen_id}'.")
            return False, "Lỗi khi tạo PIE definition (không nhận được ID mới).", None

    except psycopg2.errors.UniqueViolation as uve:
        conn.rollback()
        error_msg = f"Defined Screen ID '{new_defined_screen_id}' đã tồn tại cho ứng dụng '{app_name}'. Vui lòng chọn ID khác (Lỗi UNIQUE)."
        logger.warning(f"DATABASE: Lỗi UNIQUE constraint khi tạo PIE cho {app_name}/{new_defined_screen_id}: {uve}")
        return False, error_msg, None
    except psycopg2.Error as db_err:
        if conn:
            conn.rollback()
        logger.error(f"DATABASE: Lỗi CSDL trong create_new_pie_definition_from_node cho {app_name}/{new_defined_screen_id}: {db_err}", exc_info=True)
        error_msg = f"Lỗi cơ sở dữ liệu khi tạo PIE: {str(db_err)}" 
        return False, error_msg, None
    except Exception as e:
        if conn:
            conn.rollback()
        logger.error(f"DATABASE: Lỗi không mong muốn trong create_new_pie_definition_from_node: {e}", exc_info=True)
        return False, f"Lỗi không mong muốn của hệ thống: {e}", None
    finally:
        if cur:
            cur.close()
        if conn:
