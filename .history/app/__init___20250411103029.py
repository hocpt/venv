...
def load_scheduled_jobs(app):
    with app.app_context():
        try:
            ...
            # <<< DEBUG: In ra cấu hình lấy từ DB >>>
            ...
            for job_config in job_configs:
                ...
                # <<< DEBUG: Log thông tin job sắp được thêm >>>
                ...
                try:
                    ...
                    print(f"DEBUG: Finished processing config for job '{job_id}'.") # Log khi kết thúc xử lý 1 job config
...
def create_app(config_class=config.Config):
    ...
    if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        ...
        load_scheduled_jobs(app)
         # <<< Thêm kiểm tra tạm thời ngay sau khi load >>>
        # with app.app_context(): # Có thể không cần context cho get_job
        #     temp_job = scheduler.get_job('suggestion_job')
        #     print(f"DEBUG [IMMEDIATE CHECK]: Job 'suggestion_job' in scheduler right after load: {temp_job}")
    else:
