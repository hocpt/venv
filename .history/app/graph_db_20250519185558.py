...
def get_screen_nodes_for_management(app_name: str | None,
    ...
    logger = current_app.logger 
    driver = get_driver() # Hàm lấy Neo4j driver của bạn
    if not driver:
    ...
    if filter_status and filter_status != 'all':
        ...
        if actual_filter_status == 'unknown': # Nếu người dùng chọn 'unknown', bao gồm cả các biến thể
    ...
    # QUAN TRỌNG: Đảm bảo data_query lấy elementId(s)
    data_query = f"""
        MATCH (s:Screen)
        {where_statement}
        RETURN
            elementId(s) AS neo4j_db_id, // <<< SỬ DỤNG ALIAS NÀY: neo4j_db_id
        ...
        with driver.session(database=db_name_neo4j) as session:
            ...
            if total_items > 0:
                ...
                for record in data_result:
                    ...
                    # Log giá trị của neo4j_db_id ngay tại đây
                    logger.info(f"GRAPH_DB_FETCHED_NODE: screen_id='{record_dict.get('screen_id')}', neo4j_db_id='{record_dict.get('neo4j_db_id')}', type='{type(record_dict.get('neo4j_db_id'))}'")
                    if not record_dict.get('neo4j_db_id'): # Kiểm tra nếu nó rỗng/None
                         logger.error(f"GRAPH_DB LỖI NGHIÊM TRỌNG: Node {record_dict.get('screen_id')} có neo4j_db_id là None hoặc rỗng từ Neo4j!")
                    raw_nodes_from_neo4j.append(record_dict)
        
        # Phần xử lý PIE và datetime có thể vẫn giữ nguyên hoặc được thực hiện ở admin_routes.py
        # Quan trọng là raw_nodes_from_neo4j chứa các dict với key 'neo4j_db_id' có giá trị.
