...
def merge_screen(screen_id, app_name, activity_name, extracted_elements, log_id): # <<< Thêm app_name vào tham số
    """
    Tạo hoặc cập nhật node Screen, lưu app_name và danh sách elements của nó.
    """
    driver = get_driver()
    with driver.session() as session:
        result = session.run(
        ...
        # --- Phần xử lý elements giữ nguyên ---
        existing_elements_map = {el.get('element_id'): el for el in existing_elements_data if isinstance(el, dict) and el.get('element_id')} # Thêm kiểm tra an toàn
        ...
        for new_el_data in extracted_elements:
             ...
             if existing_el:
                # Cập nhật timestamp và các thuộc tính khác nếu cần
                if existing_el.get('last_seen_timestamp') != current_time:
                      existing_el['last_seen_timestamp'] = current_time; updated = True
                # ... (các cập nhật khác cho element đã tồn tại) ...
             else:
                # Element mới
                new_element_entry = {
                    'element_id': el_id, 'identifier_type': new_el_data.get('identifier_type'), # Giữ lại identifier_type
                    'element_type': new_el_data.get('element_type'), 'text_content': new_el_data.get('text_content'),
                    'is_clickable_observed': False, 'is_editable_observed': False,
                    'classification': 'unclassified', 'attempt_count': 0, 'success_count': 0,
        ...
        if updated:
             ...
             session.run(
                 "MATCH (s:Screen) WHERE id(s) = $node_id SET s.elements = $elements_list",
                 node_id=node_id, elements_list=final_elements_list
             )
