...
def create_app(config_class=config.Config):
    ...
    screenshot_storage_dir = app.config.get('SCREENSHOT_STORAGE_PATH')
    if screenshot_storage_dir:
        if not os.path.exists(screenshot_storage_dir):
            try:
                os.makedirs(screenshot_storage_dir, exist_ok=True) # exist_ok=True để không lỗi nếu thư mục đã tồn tại
                app.logger.info(f"Ensured screenshot storage folder exists: {screenshot_storage_dir}")
            except OSError as e:
                app.logger.error(f"Error creating screenshot storage folder {screenshot_storage_dir}: {e}")
    else:
        app.logger.warning("SCREENSHOT_STORAGE_PATH is not defined in the config.")
