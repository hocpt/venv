...
PER_PAGE_RULES = 30
PER_PAGE_TEMPLATES = 3030 # <<< Định nghĩa số mục mỗi trang cho Rules
try:
...
def view_templates():
    ...
    if not db:
        ...
        try:
            ...
            templates_data, total_items = db.get_filtered_templates_with_details(
                ...
                page=page,
                per_page=PER_PAGE_TEMPLATES # <<< Sử dụng hằng số
            )
            ...
            # Lấy danh sách category duy nhất cho dropdown lọc (như cũ)
    ...
    # Truyền dữ liệu đã lọc, phân trang, categories, và filter vào template
    return render_template('admin_templates.html',
                           title=title,
                           templates=templates_data,              # <<< Dữ liệu trang hiện tại
                           distinct_categories=distinct_categories,
                           filters=active_filters,
