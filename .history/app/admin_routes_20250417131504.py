...
def add_transition():
    ...
    # Lấy dữ liệu cho dropdowns (cần cho cả GET và POST lỗi)
    strategy_stages = []
    all_stages = []
    all_macros = [] # <<< Danh sách Macro Codes
    try:
        strategy_stages = db.get_stages_for_strategy(strategy_id) or []
        all_stages = db.get_all_stages() or []
        # <<< Lấy danh sách macro definitions >>>
        all_macros = db.get_all_macro_definitions(include_generic=True) or []
    except Exception as e:
    ...
    if request.method == 'POST':
        ...
        priority_str = request.form.get('priority', '0')
        # <<< Lấy macro code và params_str từ form >>>
        action_macro_code = request.form.get('action_macro_code')
        action_params_str = request.form.get('action_params_str', '{}') # Default là JSON rỗng nếu không nhập

        # Validate cơ bản
        if not current_stage_id or not user_intent:
             flash("Current Stage và User Intent là bắt buộc.", "warning")
             return render_template('admin_add_transition.html', title="Thêm Transition (Lỗi)",
        ...
        try:
            priority = int(priority_str)
             ...
             return render_template('admin_add_transition.html', title="Thêm Transition (Lỗi)",
                                    strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                    strategy_stages=strategy_stages, all_stages=all_stages,
                                    all_macros=all_macros, valid_intents=VALID_INTENTS_FOR_TRANSITION,
                                    current_data=request.form), 400
        # <<< Validate JSON trong params nếu có nhập >>>
        if action_params_str and action_params_str.strip() and action_params_str != '{}':
             try:
                  json.loads(action_params_str)
             except json.JSONDecodeError:
                  flash("Action Params không phải là định dạng JSON hợp lệ.", "warning")
                  return render_template('admin_add_transition.html', title="Thêm Transition (Lỗi)",
                                         strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                         strategy_stages=strategy_stages, all_stages=all_stages,
                                         all_macros=all_macros, valid_intents=VALID_INTENTS_FOR_TRANSITION,
        ...
        # Gọi hàm db.add_new_transition đã sửa đổi
        # <<< Truyền action_macro_code và action_params_str >>>
        success, error_msg = db.add_new_transition(
            current_stage_id, user_intent, condition_logic, next_stage_id, priority,
            action_macro_code if action_macro_code else None, # Chuyển rỗng thành None
            action_params_str if (action_params_str and action_params_str.strip() and action_params_str != '{}') else None # Chỉ truyền nếu có nội dung và hợp lệ
        )

        if success:
            flash('Thêm transition thành công!', 'success')
            return redirect(url_for('admin.view_strategy_stages', strategy_id=strategy_id))
        else:
            flash(f'Thêm transition thất bại: {error_msg or "Lỗi không xác định."}', 'error')
            # Render lại form với lỗi, truyền đủ dữ liệu dropdown
            return render_template('admin_add_transition.html', title="Thêm Transition (Lỗi DB)",
                                   strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                   strategy_stages=strategy_stages, all_stages=all_stages,
                                   all_macros=all_macros, valid_intents=VALID_INTENTS_FOR_TRANSITION,
                                   current_data=request.form)
...
@admin_bp.route('/transitions/add', methods=['GET', 'POST'])
def add_transition():
    # ... (Phần lấy strategy_id, current_stage_id_prefill giữ nguyên) ...
    strategy_id = request.args.get('strategy_id')
    current_stage_id_prefill = request.args.get('current_stage_id')
    
    if not strategy_id: flash("Cần strategy_id.", "error"); return redirect(url_for('admin.view_strategies'))
    ...
    try:
        strategy_stages = db.get_stages_for_strategy(strategy_id) or []
        all_stages = db.get_all_stages() or []
        all_macros = db.get_all_macro_definitions() or [] # Lấy danh sách macro
    except Exception as e:
        flash(f"Lỗi tải dữ liệu form: {e}", "error")
        # Vẫn render form nhưng dropdown có thể trống
    ...
    if request.method == 'POST':
        ...
        # <<< Gọi hàm db.add_new_transition đã sửa >>>
        success, error_msg = db.add_new_transition(
            current_stage_id, user_intent, condition_logic, next_stage_id, priority,
        ...
        if success:
            ...
            flash(f'Thêm transition thất bại: {error_msg}', 'error')
            # <<< Render lại form với lỗi, truyền đủ dữ liệu dropdown >>>
            return render_template('admin_add_transition.html', title="Thêm Transition (Lỗi DB)",
                                   strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                   strategy_stages=strategy_stages, all_stages=all_stages,
