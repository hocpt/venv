...
def create_app(config_class=config.Config):
    ...
    # ======================================
    # === THÊM ROUTE ĐỂ PHỤC VỤ FILE TỪ THƯ MỤC SCREENSHOTS MỚI ===
    # URL sẽ có dạng /project_screenshots/ten_file.png
    @app.route('/project_screenshots/<path:filename>')
    def serve_project_screenshot(filename):
        # app.config['UPLOAD_FOLDER'] là đường dẫn tuyệt đối đến htp6/screenshots/
        # send_from_directory cần đường dẫn tuyệt đối đến thư mục và tên file
        screenshots_dir = app.config['UPLOAD_FOLDER']
        app.logger.debug(f"Đang phục vụ screenshot: {filename} từ thư mục: {screenshots_dir}")
