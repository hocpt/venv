...
def edit_template_details(template_ref):
    ...
    if request.method == 'POST':
        ...
        category = request.form.get('category')
        try:
            success = db.update_template_details(template_ref, description, category)
            if success:
                flash(f"Cập nhật chi tiết cho template '{template_ref}' thành công!", 'success')
            else:
                flash(f"Cập nhật chi tiết cho template '{template_ref}' thất bại (ID không tồn tại?).", 'warning')
        except Exception as e:
             print(f"Lỗi nghiêm trọng khi cập nhật template details {template_ref}: {e}")
             flash(f"Lỗi không mong muốn khi cập nhật template: {e}", "error")
        # Luôn redirect về trang list sau khi xử lý POST
        return redirect(url_for('admin.view_templates'))

    # GET request
    template_details = db.get_template_ref_details(template_ref) # Hàm này đã có
    if not template_details:
