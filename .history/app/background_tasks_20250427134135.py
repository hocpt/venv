...
def build_app_map_task(job_data: dict):
    ...
    with temp_app.app_context():
        ...
        structure_hash_str = "default_screen_id" # Fallback
        try:
            # Cố gắng tạo hash dựa trên activity và cấu trúc element
            if activity_name or elements:
                element_repr_list = sorted([
                    f"id={el.get('resource_id','_')};txt={el.get('text','_')};cls={el.get('class_name','_')}"
                    for el in elements if el # Lấy các thuộc tính quan trọng
                ])
                structure_string = f"{app_name}|{activity_name}|{'|'.join(element_repr_list)}"
                structure_hash_str = hashlib.sha256(structure_string.encode('utf-8')).hexdigest()[:24] # Hash dài hơn chút
            target_screen_id = structure_hash_str
        except Exception as hash_err:
             log.error(f"Error generating screen hash for {app_name}: {hash_err}", exc_info=True)
             target_screen_id = f"error_hash_{int(time.time())}" # ID tạm thời nếu lỗi hash

        log.debug(f"Target screen ID generated/determined: {target_screen_id}")
