...
@admin_bp.route('/strategies/<strategy_id>/edit', methods=['GET', 'POST'])
def edit_strategy(strategy_id):
    # ... (Lấy strategy details và stages như cũ) ...
    strategy = db.get_strategy_details(strategy_id) # Hàm này đã lấy cả strategy_type
    if not strategy: flash(f"Không tìm thấy strategy ID {strategy_id}.", "error"); return redirect(url_for('admin.view_strategies')) # Tạm về trang chung
    stages = []
    ...
    if request.method == 'POST':
        ...
        description = request.form.get('description')
        initial_stage_id = request.form.get('initial_stage_id')
        strategy_type = request.form.get('strategy_type') # <<< Lấy strategy_type

        # <<< Thêm strategy_type vào validate >>>
        if not name or not initial_stage_id or not strategy_type:
             flash("Name, Initial Stage ID, và Strategy Type là bắt buộc.", "warning")
             # Render lại với lỗi, giữ giá trị form lỗi
             return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                                    strategy=strategy, stages=stages,
                                    valid_types=VALID_STRATEGY_TYPES, current_data=request.form), 400
        if strategy_type not in VALID_STRATEGY_TYPES:
             flash("Strategy Type không hợp lệ.", "warning")
             return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                                    strategy=strategy, stages=stages,
                                    valid_types=VALID_STRATEGY_TYPES, current_data=request.form), 400

        # <<< Truyền strategy_type vào hàm DB >>>
        ...
        if success:
            ...
            if strategy_type == 'control':
                 ...
                 return redirect(url_for('admin.view_strategies_language')) # Cần tạo route này sau
        else:
            flash(f'Cập nhật chiến lược thất bại: {error_msg or "Lỗi không xác định."}', 'error')
            # Render lại với lỗi, giữ giá trị form lỗi
            return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                                   strategy=strategy, stages=stages,
                                   valid_types=VALID_STRATEGY_TYPES, current_data=request.form)

    # GET request: Truyền thêm valid_types
    return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                           strategy=strategy, stages=stages,
...
def add_strategy():
    ...
    if request.method == 'POST':
        ...
        if not strategy_id or not name or not initial_stage_id or not strategy_type:
             flash("Strategy ID, Name, Initial Stage ID, và Strategy Type là bắt buộc.", "warning")
             return render_template('admin_add_strategy.html', title="Thêm Chiến lược Mới", stages=stages,
                                    valid_types=VALID_STRATEGY_TYPES, current_data=request.form), 400
        if strategy_type not in VALID_STRATEGY_TYPES:
             flash("Strategy Type không hợp lệ.", "warning")
        ...
        if success:
            flash('Thêm chiến lược thành công!', 'success')
            # Tùy chọn: Redirect đến trang danh sách tương ứng với type vừa tạo
            if strategy_type == 'control':
                 return redirect(url_for('admin.view_strategies_control'))
            else:
                 return redirect(url_for('admin.view_strategies_language')) # Cần tạo route này sau
            # Hoặc redirect về trang chung nếu chưa tách
            # return redirect(url_for('admin.view_strategies'))
        else:
