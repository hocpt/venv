...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    ...
    Đã sửa lỗi không tồn tại cột updated_at.
    """
    logger = current_app.logger if current_app else print
    ...
    try:
        ...
        # === BƯỚC 1: INSERT vào response_templates (Bỏ cột updated_at) ===
        template_exists = False
        try:
            # <<< BỎ updated_at KHỎI CÂU LỆNH SQL VÀ PARAMS >>>
            sql_template = """
            ...
            conn.autocommit = True # Đảm bảo không còn trong transaction ngầm
            logger.warning(f"Template ref '{template_ref}' already exists. Proceeding to add variation.")
            template_exists = True
            cur = conn.cursor() # Mở lại cursor nếu cần
        ...
        # === BƯỚC 2: INSERT vào template_variations (Giữ nguyên) ===
        try:
            sql_variation = """
            ...
            conn.commit() # Commit sau khi cả hai bước (có thể) thành công
            returned_ref = template_ref
         ...
         logger.error(f"LỖI (database.py - add_new_template): INSERT thất bại: {db_err_outer}", exc_info=False)
         returned_ref = None
