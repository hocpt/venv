...
def api_get_managed_nodes():
    ...
    try:
        ...
        filter_status_from_req = request.args.get('filter_status', 'unknown') # Mặc định là 'unknown'
        page = request.args.get('page', 1, type=int)
        if page < 1: 
            page = 1
        
        # Lấy PER_PAGE từ config hoặc đặt giá trị mặc định
        # Giả sử bạn có hằng số PER_PAGE_NODE_ISSUES định nghĩa ở đâu đó hoặc lấy từ config
        # Ví dụ: PER_PAGE_NODE_ISSUES = current_app.config.get('PER_PAGE_NODE_ISSUES', 15)
        # Nếu không, bạn có thể đặt trực tiếp ở đây:
        ...
        logger.debug(f"API get_managed_nodes: app_filter='{app_name_for_db_query}', status_filter='{filter_status_from_req}', page={page}, per_page={per_page_val}")

        # Gọi hàm từ graph_db để lấy dữ liệu node và tổng số item
        # Hàm get_screen_nodes_for_management đã bao gồm việc lấy logical_pie_name
        ...
        if nodes_page_data: # nodes_page_data là list các dict
            for node_data_from_neo4j in nodes_page_data:
                # Tạo bản sao để không thay đổi đối tượng gốc nếu nó được cache hoặc dùng ở nơi khác
                node_render_data = dict(node_data_from_neo4j)

                raw_screenshot_filename = node_render_data.get('screenshot_path') # Đây LÀ CHỈ TÊN FILE
                node_app_name = node_render_data.get('app_name')
                
                logger.debug(f"Node processing for API: ID={node_render_data.get('screen_id')}, App={node_app_name}, Neo4jPath={raw_screenshot_filename}")

                if raw_screenshot_filename and node_app_name:
                    # Xây dựng path tương đối bao gồm app_name cho tham số filename của url_for
                    # để khớp với cấu trúc lưu trữ app/static/screenshots/APP_NAME/TEN_FILE.png
                    # và cách route `serve_app_specific_screenshot` hoạt động.
                    filename_for_url = f"{node_app_name}/{raw_screenshot_filename}"
                    
                    try:
                        # Tên endpoint 'serve_app_specific_screenshot' phải khớp với tên hàm route trong app/__init__.py
                        node_render_data['screenshot_full_url'] = url_for(
                            'serve_app_specific_screenshot', 
                            filename=filename_for_url,    # Truyền "APP_NAME/TEN_FILE.png"
                            _external=False # False để tạo URL tương đối trong cùng ứng dụng
                        )
                        logger.debug(f"Generated screenshot_full_url for '{filename_for_url}': {node_render_data['screenshot_full_url']}")
                    except Exception as url_exc:
                        logger.error(f"Lỗi tạo URL ảnh cho node {node_render_data.get('screen_id')} với file '{filename_for_url}': {url_exc}", exc_info=True)
                        node_render_data['screenshot_full_url'] = None
                else:
                    node_render_data['screenshot_full_url'] = None
                    if not raw_screenshot_filename:
                        logger.info(f"Node {node_render_data.get('screen_id')} (App: {node_app_name}) không có screenshot_path.")
                    if not node_app_name:
                         logger.warning(f"Node {node_render_data.get('screen_id')} không có app_name, không thể tạo URL ảnh an toàn.")
        ...
        pagination_details = {
            ...
            'has_prev': page > 1 and total_pages_val > 0, # Sửa điều kiện hợp lý hơn
            'has_next': page < total_pages_val,
            'prev_num': page - 1 if page > 1 else None,
            'next_num': page + 1 if page < total_pages_val else None
            # Thêm 'page_param' và 'extra_params' nếu helper phân trang của bạn cần
        }
        
        logger.info(f"API get_managed_nodes: Trả về {len(final_nodes_list_for_response)} nodes cho page {page}. Total items: {current_total_items}")
        return jsonify({
        ...
        return jsonify({"nodes": [], "pagination": {}, "error": "Đã xảy ra lỗi máy chủ nội bộ khi xử lý yêu cầu."}), 500
