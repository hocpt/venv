...
def edit_stage(stage_id):
    ...
    if db:
        try:
             ...
             print(f"Lỗi khi lấy stage details trong edit_stage: {e}")
             flash("Lỗi khi đọc thông tin stage.", "error")
             # <<< SỬA REDIRECT NÀY >>>
             return redirect(url_for('admin.index')) # Về Dashboard nếu lỗi DB
    ...
    # ... (Logic lấy strategy_id, type, redirect_endpoint, cancel_url như cũ) ...
    strategy_id_redirect = stage.get('strategy_id')
    # ... (code còn lại giữ nguyên) ...

    if request.method == 'POST':
        # ... (Logic xử lý POST giữ nguyên như cũ, redirect sau success dùng cancel_url) ...
        # Ví dụ:
        # if success:
        #     return redirect(cancel_url)
        # else:
        #     return render_template('admin_edit_stage.html', ..., cancel_url=cancel_url)
        # --- Bỏ qua phần logic POST ở đây để giữ ngắn gọn ---
        pass # Thay bằng logic POST của bạn

    # GET request (truyền cancel_url đúng)
    return render_template('admin_edit_stage.html',
