...
@# Dán TOÀN BỘ hàm này vào file app/admin_routes.py,
# THAY THẾ HOÀN TOÀN hàm add_strategy cũ.
...
def add_strategy():
    ...
    if request.method == 'POST':
        ...
        description = request.form.get('description', '').strip()
        initial_stage_id = request.form.get('initial_stage_id') # Giá trị có thể là ""
        # strategy_type đã được lấy và giải quyết ở trên (strategy_type_resolved)

        # Validate dữ liệu
        errors = []
        # *** DÒNG KIỂM TRA QUAN TRỌNG: BỎ 'initial_stage_id' khỏi check bắt buộc ***
        ...
        # Kiểm tra lại strategy_type_resolved (dù đã cố gắng giải quyết ở trên)
        if strategy_type_resolved not in ['language', 'control', 'mainloop']:
            errors.append("Strategy Type không hợp lệ.")
        # Có thể thêm các validate khác nếu cần (vd: độ dài ID, name)
        ...
        if errors:
            ...
            return render_template('admin_add_strategy.html',
                                   title=title + " (Lỗi)",
                                   stages=stages,
                                   strategy_type=strategy_type_resolved, # Dùng type đã giải quyết
                                   current_data=request.form), 400

        # Nếu không có lỗi validation, gọi hàm DB
        try:
            # Gọi hàm db.add_new_strategy
            # Chuyển initial_stage_id rỗng thành None khi gọi hàm DB
            success, error_msg = db.add_new_strategy(
                strategy_id=strategy_id,
                name=name,
                description=description or None, # Chuyển chuỗi rỗng thành None
                initial_stage_id=initial_stage_id if initial_stage_id else None, # <<< QUAN TRỌNG: Chuyển "" thành None
            ...
            if success:
                ...
                if strategy_type_resolved == 'control':
                    redirect_url = url_for('admin.view_strategies_control')
                elif strategy_type_resolved == 'mainloop':
                ...
                # Lỗi từ DB (ví dụ: ID trùng, FK constraint nếu stage chưa tồn tại VÀ bạn chưa sửa workflow)
