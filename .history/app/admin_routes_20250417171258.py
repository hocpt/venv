...
def add_stage():
    ...
    if not strategy_details:
        ...
        return redirect(url_for('admin.view_strategies_language')) # Tạm về language list
    ...
    if request.method == 'POST':
        ...
        try:
            # Giả sử add_new_stage trả về tuple (success, error_msg) hoặc bool
            # Nếu chỉ trả về bool:
            # success = db.add_new_stage(stage_id, form_strategy_id, description, order)
            # error_msg = "ID đã tồn tại?" if not success else None # Tự suy đoán lỗi
            # Nếu trả về tuple:
            success, error_msg = db.add_new_stage(stage_id, form_strategy_id, description, order) # <<< Giả sử hàm DB trả về tuple

            if success:
                flash(f"Thêm stage '{stage_id}' thành công!", 'success')
                # <<< SỬA LẠI REDIRECT Ở ĐÂY >>>
