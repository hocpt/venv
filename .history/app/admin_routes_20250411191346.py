...
def view_scheduled_jobs():
    """Hiển thị danh sách các Job đã cấu hình trong DB và trạng thái live."""
    db_jobs_config = []
    live_jobs_status = {}

    # 1. Lấy cấu hình jobs từ Database
    if db: # Kiểm tra db có được import thành công không
        try:
            ...
            print(f"Lỗi nghiêm trọng khi load job configs từ DB: {e}")
            flash("Lỗi không mong muốn khi tải cấu hình jobs từ DB.", "error")
            db_jobs_config = []
    else:
        flash("Lỗi kết nối database module.", "error")

    # 2. Lấy trạng thái jobs thực tế từ Scheduler đang chạy
    if scheduler: # Kiểm tra scheduler có được import/khởi tạo thành công không
        try:
        ...
        print("WARNING: Biến 'scheduler' là None, không thể lấy trạng thái live jobs.")
        flash("Scheduler chưa sẵn sàng, không thể hiển thị trạng thái đang chạy.", "warning")
...
def run_suggestion_job_now():
    ...
    job_id_to_run = 'suggestion_job' # ID của job chúng ta muốn chạy
    print(f"\n--- Bắt đầu chạy thủ công tác vụ: {job_id_to_run} ---")
    ...
    try:
        ...
        max_processed_id_in_batch = last_processed_id # Khởi tạo bằng ID cũ
        print(f"DEBUG: (Manual Run) Lấy last_processed_id = {last_processed_id}")

        # 2. Lấy các tương tác MỚI HƠN ID cuối cùng đã xử lý
        # Lấy một số lượng nhỏ hơn để test nhanh? Ví dụ limit=10
        limit = current_app.config.get('MANUAL_SUGGEST_LIMIT', 10) # Có thể thêm config này
        status_to_analyze = current_app.config.get('STATUS_TO_ANALYZE_SUGGEST', ['success_ai']) # Có thể thêm config này
        print(f"DEBUG: (Manual Run) Lấy tối đa {limit} tương tác với status {status_to_analyze} sau ID {last_processed_id}...")
        ...
        if interactions is None:
             error_message = "Lỗi khi lấy tương tác từ DB."
             raise Exception(error_message) # Nhảy tới khối except cuối

        interactions_found = len(interactions)
        if not interactions:
             flash(f"Không tìm thấy tương tác mới nào (sau ID {last_processed_id}) có status '{','.join(status_to_analyze)}' để phân tích.", "info")
             return redirect(url_for('admin.view_scheduled_jobs'))

        print(f"INFO: (Manual Run) Tìm thấy {interactions_found} tương tác để phân tích.")
        ...
        # 3. Lặp qua từng tương tác và xử lý
            ...
            if not interaction_data['received_text'] or not interaction_data['sent_text']:
                print(f"WARNING: (Manual Run) Bỏ qua interaction {history_id} do thiếu text.")
                max_processed_id_in_batch = max(max_processed_id_in_batch, history_id) # Vẫn cập nhật ID đã xem
                continue
            ...
            if suggested_keywords and suggested_template:
                print(f"INFO: (Manual Run) AI đề xuất cho ID {history_id}: keywords='{suggested_keywords[:50]}...', template='{suggested_template[:50]}...'")
                ...
                if added:
                    ...
                    print(f"LỖI: (Manual Run) Lưu đề xuất từ interaction {history_id} thất bại.")
                    # Nếu lưu lỗi, có thể không cập nhật max_id? Tạm thời vẫn cập nhật
            else:
                 print(f"DEBUG: (Manual Run) AI không tạo ra đề xuất hợp lệ cho interaction {history_id}.")

            # Luôn cập nhật ID lớn nhất đã xử lý (dù thành công hay không) để không bị lặp lại
            max_processed_id_in_batch = max(max_processed_id_in_batch, history_id)
        for interaction in interactions:
                        processed_count += 1
                        history_id = interaction.get('history_id')
                        if history_id is None:
                            # Cập nhật max_processed_id ngay cả khi bỏ qua
                            # để không bị kẹt nếu có history_id là NULL
                            if 'history_id' in interaction and interaction['history_id'] is not None:
                                 max_processed_id_in_batch = max(max_processed_id_in_batch, interaction['history_id'])
                            print("WARNING: (Manual Run) Found interaction record with no history_id, skipping.")
                            continue
        
                        # Luôn cập nhật ID cao nhất đã gặp
                        max_processed_id_in_batch = max(max_processed_id_in_batch, history_id)
                        ...
                        if not interaction_data['received_text'] or not interaction_data['sent_text']:
                            print(f"WARNING: (Manual Run) Bỏ qua interaction {history_id} do thiếu text.")
                            continue # Đã cập nhật max_id, đi tiếp
        
                        # <<< SỬA LỖI 1: NHẬN ĐỦ 4 GIÁ TRỊ TRẢ VỀ >>>
                        try:
                            suggested_keywords, suggested_category, suggested_template_ref, suggested_template = ai_service.suggest_rule_from_interaction(
                             ...
                             print(f"ERROR: (Manual Run) Exception calling ai_service for ID {history_id}: {ai_call_err}")
                             print(traceback.format_exc())
                             continue # Bỏ qua interaction này nếu AI lỗi
        
                        # 4. Lưu đề xuất vào DB
                        # Chỉ lưu nếu có keywords hoặc template text
                        if suggested_keywords or suggested_template:
                            ...
                            # <<< SỬA LỖI 2: TRUYỀN ĐỦ 4 GIÁ TRỊ VÀO DB >>>
                            try:
                                ...
                                if added:
                                 ...
                                 print(traceback.format_exc())
                                 # Có thể dừng lại hoặc bỏ qua tùy chiến lược
        ...
        if max_processed_id_in_batch > last_processed_id:
             print(f"DEBUG: (Manual Run) Cập nhật last_processed_id thành {max_processed_id_in_batch} cho task {job_id_to_run}")
             ...
             if not update_success:
                  ...
                  print(f"LỖI: (Manual Run) {error_message}")
        ...
        if error_message:
             ...
             flash(f"Chạy thủ công hoàn tất. Đã xử lý {processed_count}/{interactions_found} tương tác tìm thấy. Thêm được {suggestions_added} đề xuất mới. ID xử lý cuối cùng được cập nhật thành {max_processed_id_in_batch}.", "success")
    ...
    # Luôn redirect về trang quản lý jobs
    return redirect(url_for('admin.view_scheduled_jobs'))