...
def add_new_transition(current_stage_id: str, user_intent: str,
                       ...
                       condition_type: str | None = None, # <<< Thêm
                       condition_value: str | None = None  # <<< Thêm
                       ) -> tuple[bool, str | None]:
    ...
    try:
        cur = conn.cursor()
        # <<< Thêm condition_type, condition_value vào INSERT >>>
        sql = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s);
        """
        params = (current_stage_id, user_intent,
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str,
    ...
    try:
        cur = conn.cursor()
        # <<< Thêm condition_type, condition_value vào UPDATE >>>
        sql = """
            UPDATE stage_transitions
            SET current_stage_id = %s, user_intent = %s, condition_type = %s, condition_value = %s,
                next_stage_id = %s, priority = %s, action_to_suggest = %s
            WHERE transition_id = %s;
        """
        params = (current_stage_id, user_intent,
...
def get_transition_details(transition_id: int) -> dict | None:
    """Lấy chi tiết transition, bao gồm cả thông tin điều kiện."""
    if not transition_id: return None
    details = None
    conn = get_db_connection()
    # ... (try/except/finally) ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # <<< Thêm condition_type, condition_value vào SELECT >>>
        sql = """
            SELECT t.transition_id, t.current_stage_id, t.user_intent, t.condition_type, t.condition_value,
                   t.next_stage_id, t.action_to_suggest, t.priority, ss.strategy_id
            FROM stage_transitions t
        ...
        if row:
            details = dict(row)
            # Xử lý action_to_suggest (như cũ)
            ...
            # Condition type/value đã có sẵn trong details
    # ... (xử lý lỗi như cũ) ...
...
# --- Sửa hàm get_strategy_action_sequence ---
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    """Lấy chuỗi các bước hành động, bao gồm cả thông tin điều kiện."""
    # Hàm này sẽ lấy dữ liệu thô từ transitions, việc biên dịch thành cấu trúc IF/ELSE
    # sẽ được thực hiện trong compile_strategy_package
    if not strategy_id: return None
    transitions_list = None
    conn = get_db_connection()
    # ... (try/except/finally) ...
    try:
        ...
        sql = """
             SELECT
                 ...
                 t.next_stage_id, t.action_to_suggest, t.priority
             FROM stage_transitions t
             JOIN strategy_stages ss ON t.current_stage_id = ss.stage_id
             WHERE ss.strategy_id = %s
             ORDER BY t.current_stage_id, t.priority DESC, t.user_intent; -- Sắp xếp để xử lý logic dễ hơn
         """
        cur.execute(sql, (strategy_id,))
        rows = cur.fetchall()
        # Trả về list các dict transition thô, hàm compile sẽ xử lý tiếp
        transitions_list = [dict(row) for row in rows] if rows else []
    # ... (xử lý lỗi như cũ) ...
    except psycopg2.Error as db_err: print(f"LỖI (database.py - get_transitions_for_strategy): Truy vấn thất bại: {db_err}"); transitions_list = None
    except Exception as e: print(f"LỖI (database.py - get_transitions_for_strategy): Lỗi không xác định: {e}"); transitions_list = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
    # Lưu ý: Hàm này bây giờ trả về list các transition thô, chưa phải action_sequence cuối cùng
    # compile_strategy_package sẽ dùng list này để tạo action_sequence
    return transitions_list
