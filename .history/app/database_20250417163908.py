...
def add_new_transition(current_stage_id: str, user_intent: str,
    ...
    # Xử lý action chỉ khi có macro_code
    ...
    # Nếu có action macro thì không nên có response template và ngược lại (logic này nên ở tầng route/controller)
    # Ở DB chỉ lưu giá trị được truyền vào
    ...
    try:
        cur = conn.cursor()
        # Thêm response_template_ref vào INSERT
        sql = """
        ...
        params = (
            ...
            priority,
            json.dumps(action_json_for_db) if action_json_for_db else None, # Lưu action
        ...
        success = True
    # ... (xử lý lỗi như cũ) ...
    except psycopg2.Error as e: error_msg = f"Lỗi CSDL: {e}"; success = False; if conn: conn.rollback()
    except Exception as e: error_msg = f"Lỗi không xác định: {e}"; success = False; if conn: conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success, error_msg

# --- Sửa hàm update_transition ---
def update_transition(transition_id: int, current_stage_id: str, user_intent: str,
    ...
    """Cập nhật transition, xử lý cả language và control."""
    if not transition_id or not current_stage_id or not user_intent:
        return False, "ID, Current Stage và User Intent là bắt buộc."

    action_json_for_db = None
    if action_macro_code and action_macro_code.strip():
        # ... (logic xử lý params như cũ) ...
        params_dict = {}
        if action_params_str and action_params_str.strip():
             try:
                 params_dict = json.loads(action_params_str)
                 if not isinstance(params_dict, dict): raise json.JSONDecodeError("Params phải là object JSON.", action_params_str, 0)
             except json.JSONDecodeError: return False, "Action Params không phải JSON object hợp lệ."
        action_json_for_db = {"macro_code": action_macro_code.strip(), "params": params_dict}
    ...
    try:
        cur = conn.cursor()
        # Thêm response_template_ref vào UPDATE
        sql = """
...
# --- Sửa hàm get_transition_details ---
def get_transition_details(transition_id: int) -> dict | None:
    """Lấy chi tiết transition, bao gồm cả response_template_ref và điều kiện."""
    if not transition_id: return None
    details = None
    conn = get_db_connection()
    if not conn: return None
    ...
    try:
        ...
        sql = """
            SELECT t.transition_id, t.current_stage_id, t.user_intent, t.condition_type, t.condition_value,
                   t.next_stage_id, t.action_to_suggest, t.response_template_ref, t.priority, t.updated_at,
                   ss.strategy_id
            FROM stage_transitions t
        ...
        if row:
            details = dict(row)
            # Xử lý action_to_suggest để lấy code và params string (như cũ)
            ...
            if isinstance(action_data, dict):
                 ...
                 try: details['action_params_str'] = json.dumps(params_dict, indent=2, ensure_ascii=False)
                 except TypeError: details['action_params_str'] = '{}'
            # response_template_ref, condition_type, condition_value đã có sẵn
    # ... (xử lý lỗi như cũ) ...
    except psycopg2.Error as e: print(f"LỖI (database.py - get_transition_details): {e}"); details = None
    except Exception as e: print(f"LỖI (database.py - get_transition_details): {e}"); details = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return details

# --- Hàm get_strategy_action_sequence giữ nguyên ---
# Vì hàm compile_strategy_package sẽ lọc và xử lý dựa trên strategy_type
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
     """Lấy danh sách các transition thô (bao gồm điều kiện, action, template_ref) cho một strategy."""
     # ... (Code hàm này giữ nguyên như đã cung cấp trước đó) ...
     if not strategy_id: return None
     transitions_list = None
     conn = get_db_connection()
     ...
     try:
         cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
         # <<< Lấy tất cả các cột liên quan từ transition >>>
         sql = """
              SELECT
     ...
     # ... (xử lý lỗi như cũ) ...
     except psycopg2.Error as db_err: print(f"LỖI (db - get_strat_action_seq): {db_err}"); transitions_list = None
     except Exception as e: print(f"LỖI (db - get_strat_action_seq): {e}"); transitions_list = None
     finally:
         if cur: cur.close()
         if conn: conn.close()
     return transitions_list
