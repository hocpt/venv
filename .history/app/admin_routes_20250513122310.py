...
def view_node_management():
    ...
    return render_template('admin_node_management.html', 
                           ...
                           screenshots_subdir=current_app.config.get('SCREENSHOTS_SUBDIR_IN_STATIC', 'screenshots'))
...
def api_get_managed_nodes():
    logger = current_app.logger
    try:
        # ... (lấy app_name_filter, filter_status, page, per_page_val như cũ) ...
        ...
        # Gọi hàm graph_db để lấy dữ liệu thô (bao gồm app_name và screenshot_path)
        raw_nodes_page_data, total_items = graph_db.get_screen_nodes_for_management(
            app_name=app_name_for_db,
            filter_status=filter_status,
            page=page,
        ...
        # Xử lý dữ liệu thô để thêm screenshot_full_url
        processed_nodes_list = []
        if raw_nodes_page_data:
            for node_data_raw in raw_nodes_page_data:
                node_dict = dict(node_data_raw) # Chuyển Record sang dict nếu cần
                screenshot_filename = node_dict.get('screenshot_path')
                node_app_name = node_dict.get('app_name')
                
                if screenshot_filename and node_app_name:
                    try:
                        # Đảm bảo endpoint 'admin.serve_app_specific_screenshot' tồn tại và nhận app_name, filename
                        node_dict['screenshot_full_url'] = url_for(
                            'admin.serve_app_specific_screenshot', # Endpoint phục vụ ảnh
                            app_name=node_app_name, 
                            filename=screenshot_filename,
                            _external=False # Hoặc True nếu cần URL tuyệt đối
                        )
                    except Exception as url_exc:
                        logger.error(f"Lỗi tạo URL ảnh cho node {node_dict.get('screen_id')}: {url_exc}")
                        node_dict['screenshot_full_url'] = None
                else:
                    node_dict['screenshot_full_url'] = None
                processed_nodes_list.append(node_dict)
        
        # ... (tạo pagination_details như cũ, dùng total_items) ...
        total_pages_val = 0
        ...
        return jsonify({
            "nodes": processed_nodes_list, # << TRẢ VỀ LIST ĐÃ XỬ LÝ
            "pagination": {
                "page": page,
                "per_page": per_page_val,
                "total_items": current_total_items,
                "total_pages": total_pages_val,
                # ... (các trường has_prev, has_next, ...)
...
@admin_bp.route('/app_screenshots/<path:filename>') # << URL GIỐNG NHƯ BẠN CUNG CẤP (bỏ 1 dấu / thừa)
# @admin_required
def serve_app_screenshots(filename): # Chỉ cần filename
    logger = current_app.logger
    # base_path bây giờ là thư mục chứa tất cả ảnh (ví dụ: C:\Users\hocpt\venv\app_screenshots)
    base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH') 

    if not base_path:
        logger.error("SCREENSHOT_STORAGE_PATH is not configured in Flask app.")
        return "Screenshot storage path not configured on server.", 500

    # directory_path bây giờ chính là base_path
    directory_path = base_path

    logger.debug(f"Attempting to serve screenshot from directory: '{directory_path}', filename: '{filename}'")

    try:
        return send_from_directory(directory_path, filename)
    except FileNotFoundError:
        logger.warning(f"Screenshot file not found: {os.path.join(directory_path, filename)}")
