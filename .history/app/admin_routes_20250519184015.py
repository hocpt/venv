...
# hpt10/app/admin_routes.py

import math 
from flask import Blueprint, jsonify, request, current_app, url_for, flash
import os

# Đảm bảo admin_bp đã được định nghĩa ở đâu đó trong file này
# Ví dụ:
# admin_bp = Blueprint('admin', __name__, url_prefix='/admin', template_folder='../templates')

# Import các module cần thiết
from . import graph_db # Hoặc from app import graph_db
from . import database as db # db này là cho PostgreSQL (lấy distinct app names)

# Helper function để tạo URL ảnh, có thể đặt ở đầu file hoặc import
def _generate_screenshot_url_for_admin(logger, node_app_name, raw_screenshot_filename_from_neo4j, node_id_for_log="N/A"):
    if raw_screenshot_filename_from_neo4j and node_app_name:
        try:
            # Endpoint 'serve_screenshot_for_app' nhận app_name và filename (chỉ tên file)
            full_url = url_for(
                'serve_screenshot_for_app', # Tên endpoint của route trong app/__init__.py
                app_name=node_app_name,
                filename=raw_screenshot_filename_from_neo4j # CHỈ TÊN FILE
            )
        ...
        # Log chi tiết hơn lý do không tạo được URL
        ...
        if not node_app_name: # Điều này ít khi xảy ra nếu node có screenshot_path
             logger.warning(f"URL_HELPER: Node {node_id_for_log} không có app_name, không thể tạo URL ảnh.")
        return None
...
def view_node_management():
    ...
    # ... (code lấy filter, page, per_page như cũ) ...
    ...
    distinct_app_names_for_filter = [] # Khởi tạo

    try: # Lấy distinct app names cho filter
        # ... (code lấy distinct_app_names_for_filter từ PostgreSQL của bạn) ...
        # Ví dụ:
        # conn_pg = db.get_db_connection() # Giả sử db là module PostgreSQL của bạn
        # if conn_pg:
        #     try:
        ...
        #     finally:
        #         conn_pg.close()
        pass # Bỏ qua phần này nếu bạn đã có logic lấy distinct_app_names
    ...
    if raw_nodes_page_data is None:
        ...
        for node_data_from_graphdb in raw_nodes_page_data:
            node_to_render = dict(node_data_from_graphdb) 
            
            internal_neo4j_id_val = node_to_render.get('node_neo4j_internal_id')
            logger.debug(f"ADMIN_ROUTES: Processing node screen_id='{node_to_render.get('screen_id')}', internal_neo4j_id_val='{internal_neo4j_id_val}'")
            if not internal_neo4j_id_val:
                logger.error(f"ADMIN_ROUTES LỖI NGHIÊM TRỌNG: Node screen_id='{node_to_render.get('screen_id')}' có internal_neo4j_id_val rỗng/None sau khi lấy từ graph_db!")
            node_to_render['id_cho_data_attribute_html'] = internal_neo4j_id_val
            
            # ---- CHUYỂN ĐỔI DATETIME SANG ISO STRING ----
            ...
            # Lấy thông tin PIE từ PostgreSQL (nếu cần và chưa có trong node_to_render)
            # Ví dụ:
            # if node_to_render.get('status') == 'defined':
    ...
    logger.info(f"ADMIN_ROUTES: Rendering admin_node_management.html with {len(nodes_for_template)} nodes.")
...
def api_get_managed_nodes():
    logger = current_app.logger
    # ... (code lấy filter, page, per_page, app_name_for_db_query như phiên bản trước) ...
    ...
    if nodes_page_data is None:
        logger.error("API get_managed_nodes: graph_db.get_screen_nodes_for_management returned None.")
        return jsonify({"nodes": [], "pagination": {}, "error": "Lỗi tải dữ liệu Node."}), 500

    processed_nodes_list = []
    if nodes_page_data:
        for node_data_neo4j in nodes_page_data:
            processed_node = dict(node_data_neo4j)
    ...
    # ... (logic pagination và jsonify như phiên bản trước) ...
    total_pages_val = 0
