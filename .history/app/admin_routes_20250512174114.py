...
def view_screen_definitions():
    ...
    # current_app.logger.info(f"VIEW_SCREEN_DEFS: Lọc với app_name = '{selected_app_filter}' (repr: {repr(selected_app_filter)})") # In ra cả repr
        print(f"VIEW_SCREEN_DEFS: Lọc với app_name = '{selected_app_filter}' (repr: {repr(selected_app_filter)})") # Dùng print để chắc chắn
        definitions_list = db.get_screen_definitions_for_app(selected_app_filter, activity_name=None)
    definitions_list = []
    if definitions_list: # Chỉ log nếu list không rỗng
        current_app.logger.info(f"--- DEBUG: TRƯỚC KHI RENDER TEMPLATE CHO /admin/mapping/screen-definitions ---")
        current_app.logger.info(f"Số lượng definitions_list: {len(definitions_list)}")
        for i, definition_item in enumerate(definitions_list):
            desc_value = definition_item.get('description')
            desc_type = type(desc_value).__name__
            current_app.logger.info(f"  Definition {i}: ID={definition_item.get('definition_id')}, logical_name='{definition_item.get('logical_screen_name')}', description='{desc_value}' (Type: {desc_type})")

            # Kiểm tra cụ thể giá trị description
            if desc_value is None:
                current_app.logger.warning(f"    >> Definition {i} có description là None!")

            # Kiểm tra identifying_elements_json luôn, để chắc chắn
            pie_json_value = definition_item.get('identifying_elements_json')
            pie_json_type = type(pie_json_value).__name__
            pie_json_len = "N/A"
            if isinstance(pie_json_value, list):
                pie_json_len = len(pie_json_value)
            elif pie_json_value is None:
                pie_json_len = "is None"

            current_app.logger.info(f"    PIE JSON (Type: {pie_json_type}, Len: {pie_json_len}): {pie_json_value}")

    current_app.logger.info(f"Rendering admin_screen_definitions.html with {len(definitions_list)} definitions.")
