...
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
    ...
    if extracted_elements:
        ...
        for el_count, el in enumerate(extracted_elements):
            element_id_val = el.get('element_id') # Lấy ID đã được chuẩn hóa từ bước trước đó
            identifier_type_val = el.get('identifier_type')
            if element_id_val:
                element_entry = {
                    # === PHẢI CÓ KEY 'element_id' Ở ĐÂY ===
                    ...
                    'coordinate_x': None,
                    'coordinate_y': None,
                    # ... (các trường khác như classification, counts, is_clickable...)
