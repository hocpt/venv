...
def admin_screen_elements(screen_id):
    ...
    try:
        ...
        logger.debug(f"Dữ liệu Screen từ Neo4j: {screen_data_from_neo4j}")
        ...
        if detailed_ui_state and isinstance(detailed_ui_state, dict):
            ...
            try:
                original_width = int(fetched_width) if fetched_width is not None else None
            except (ValueError, TypeError):
                logger.warning(f"Giá trị width không hợp lệ: {fetched_width}")
                original_width = None
            try:
                original_height = int(fetched_height) if fetched_height is not None else None
            except (ValueError, TypeError):
                logger.warning(f"Giá trị height không hợp lệ: {fetched_height}")
                original_height = None
        else:
            logger.warning(f"Không tìm thấy detailed_ui_state hoặc không phải dict cho screen {screen_id}. Không thể lấy kích thước gốc.")
        # =======================================
