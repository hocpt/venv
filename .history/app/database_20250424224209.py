...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    """
    Thêm một template_ref mới vào response_templates (nếu chưa tồn tại)
    VÀ thêm variation đầu tiên vào template_variations.
    Đã sửa lỗi không tồn tại cột updated_at và last_used_at.
    """
    logger = current_app.logger if current_app else print
    if not template_ref or not first_variation_text:
        logger.error("Template Ref và First Variation Text không được rỗng.")
        return None
    ...
    try:
        cur = conn.cursor()
        logger.info(f"Attempting to add template '{template_ref}' and first variation within a transaction.")

        # === BƯỚC 1: THỬ INSERT vào response_templates (Đã bỏ updated_at) ===
        template_insert_failed = False
        try:
            sql_template = """
                INSERT INTO public.response_templates (template_ref, description, category)
                VALUES (%s, %s, %s);
            """
            cur.execute(sql_template, (template_ref.strip(), description, category if category else None))
            logger.info(f"Successfully inserted template_ref '{template_ref}'.")
        except psycopg2.errors.UniqueViolation:
            # Template ref đã tồn tại, đây không phải là lỗi, bỏ qua và tiếp tục
            logger.warning(f"Template ref '{template_ref}' already exists. Will proceed to add variation.")
            # Không cần rollback ở đây vì lệnh INSERT trước đó tự động bị hủy trong transaction
            pass # Bỏ qua lỗi này
        except psycopg2.Error as db_err_template:
            # Bắt các lỗi DB khác ở bước 1
            logger.error(f"DB Error inserting into response_templates for '{template_ref}': {db_err_template}", exc_info=True)
            template_insert_failed = True # Đánh dấu là bước 1 lỗi
            # Ném lại lỗi để except bên ngoài bắt và rollback
            raise db_err_template
        except Exception as e_template:
             logger.error(f"Unexpected Error inserting into response_templates for '{template_ref}': {e_template}", exc_info=True)
             template_insert_failed = True
             raise e_template # Ném lại lỗi

        # === BƯỚC 2: INSERT vào template_variations (Bỏ cột last_used_at) ===
        variation_insert_failed = False
        # Chỉ thực hiện nếu bước 1 không lỗi nghiêm trọng
        # (lỗi UniqueViolation đã được bỏ qua bởi 'pass')
        # if not template_insert_failed: <--- Dòng này không cần nữa vì lỗi khác đã raise và nhảy ra ngoài
        try:
            # <<< BỎ last_used_at KHỎI CÂU LỆNH SQL VÀ PARAMS >>>
            sql_variation = """
                INSERT INTO public.template_variations (template_ref, variation_text)
                VALUES (%s, %s);
            """
            # Chỉ cần truyền 2 tham số
            cur.execute(sql_variation, (template_ref.strip(), first_variation_text.strip()))
            logger.info(f"Successfully inserted first variation for '{template_ref}'.")

            # Nếu mọi thứ thành công đến đây, commit transaction
            conn.commit()
            logger.info(f"Transaction committed for adding template/variation '{template_ref}'.")
            returned_ref = template_ref # Gán giá trị trả về

        except psycopg2.Error as db_err_variation:
            # Lỗi ở bước 2 (Ví dụ: FK violation nếu bước 1 lỗi ngầm, hoặc variation trùng lặp nếu có UNIQUE)
            logger.error(f"DB Error inserting into template_variations for '{template_ref}': {db_err_variation}", exc_info=True)
            variation_insert_failed = True # Đánh dấu bước 2 lỗi
            raise db_err_variation # Ném lại lỗi để except bên ngoài bắt và rollback
        except Exception as e_variation:
            logger.error(f"Unexpected Error inserting into template_variations for '{template_ref}': {e_variation}", exc_info=True)
            variation_insert_failed = True
            raise e_variation

    except psycopg2.Error as db_err_outer: # Bắt lỗi từ Bước 1 hoặc Bước 2 được ném lại
         logger.error(f"LỖI (database.py - add_new_template): INSERT thất bại: {db_err_outer}", exc_info=False) # Log lỗi cuối cùng
         if conn: conn.rollback() # Rollback ở đây nếu lỗi bị bắt ở ngoài
         returned_ref = None
    except Exception as e_outer:
        logger.error(f"LỖI (database.py - add_new_template): Lỗi không xác định ngoài dự kiến: {e_outer}", exc_info=True)
        if conn: conn.rollback()
        returned_ref = None
    finally:
        if cur: cur.close()
