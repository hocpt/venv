...
def create_app(config_class=config.Config): # Mặc định dùng class Config từ config.py
    ...
    # --- Đăng ký tác vụ nền ---
        # Import hàm tác vụ SAU KHI đã có app context (hoặc đảm bảo import an toàn)
    try:
        from . import background_tasks

        # Lên lịch chạy hàm analyze_interactions_and_suggest mỗi giờ
        # ID 'suggestion_job' là duy nhất để quản lý job
        # replace_existing=True: Ghi đè job cũ nếu app khởi động lại
        if not scheduler.get_job('suggestion_job'): # Chỉ thêm job nếu chưa tồn tại
                scheduler.add_job(id='suggestion_job', func=background_tasks.analyze_interactions_and_suggest,
                                trigger='interval', hours=1, replace_existing=True)
                print("INFO: Đã đăng ký tác vụ nền 'suggestion_job' chạy mỗi giờ.")
        else:
                print("INFO: Tác vụ nền 'suggestion_job' đã tồn tại.")

    except ImportError:
        print("WARNING: Không tìm thấy module background_tasks. Bỏ qua đăng ký tác vụ nền.")
    except Exception as e:
            print(f"LỖI khi đăng ký tác vụ nền: {e}")
