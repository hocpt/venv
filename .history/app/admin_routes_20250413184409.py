...
def view_ai_simulations():
    ...
    if not db:
        ...
        try:
            ...
            if not strategies:
                flash("Không tìm thấy Strategies nào. Vui lòng thêm strategy trước.", "warning")
            if not accounts:
                flash("Không tìm thấy Accounts nào. Vui lòng thêm account (ví dụ: sim_account_01) để dùng cho việc ghi log.", "warning")
    ...
    return render_template('admin_ai_simulations.html',
                           ...
                           personas=personas,
                           strategies=strategies,
                           accounts=accounts) # <<< Truyền accounts vào template
