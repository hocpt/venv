...
@main_bp.route('/api/upload/screenshot', methods=['POST']) # Đảm bảo URL này client gọi đúng
def handle_file_upload_screenshot(): # Đổi tên hàm cho rõ ràng
    logger = current_app.logger
    
    if 'file' not in request.files:
        logger.warning("Upload API: No 'file' part in request.files")
    ...
    if not client_declared_filename:
        logger.warning("Upload API: Thiếu tham số 'filename' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'filename'"}), 400
    
    if not app_name_from_form:
        logger.warning("Upload API: Thiếu tham số 'app_name' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'app_name'"}), 400

    if uploaded_file.filename == '':
        logger.warning("Upload API: Không có file nào được chọn (filename rỗng từ request.files).")
        return jsonify({"success": False, "error": "Không có file nào được chọn"}), 400

    # An toàn hóa tên file LƯU TRÊN SERVER, sử dụng tên file client cung cấp làm cơ sở
    # Điều này đảm bảo tên file trên server khớp với screenshot_path trong Neo4j
    final_filename_on_server = secure_filename(os.path.basename(client_declared_filename))
    if not final_filename_on_server:
        logger.error(f"Upload API: Tên file '{client_declared_filename}' không hợp lệ sau khi xử lý.")
        return jsonify({"success": False, "error": "Tên file không hợp lệ."}), 400

    # An toàn hóa app_name để dùng làm tên thư mục
    safe_app_name_as_folder = secure_filename(app_name_from_form)
    if not safe_app_name_as_folder:
        logger.error(f"Upload API: Tên app '{app_name_from_form}' không hợp lệ cho việc tạo thư mục.")
        return jsonify({"success": False, "error": "Tên app không hợp lệ."}), 400

    storage_base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
    if not storage_base_path:
        logger.critical("UPLOAD API: SCREENSHOT_STORAGE_PATH không được cấu hình trong server.")
    ...
    try:
        if not os.path.exists(app_specific_storage_directory):
            os.makedirs(app_specific_storage_directory)
            logger.info(f"UPLOAD API: Đã tạo thư mục cho app '{safe_app_name_as_folder}': {app_specific_storage_directory}")
    except OSError as e:
        logger.error(f"UPLOAD API: Không thể tạo thư mục '{app_specific_storage_directory}': {e}", exc_info=True)
        return jsonify({"success": False, "error": "Lỗi tạo thư mục lưu trữ trên server."}), 500
    
    # Đường dẫn đầy đủ để lưu file ảnh: STORAGE_PATH / APP_NAME / FILENAME
    full_path_to_save_file = os.path.join(app_specific_storage_directory, final_filename_on_server)
    
    try:
        uploaded_file.save(full_path_to_save_file)
        logger.info(f"UPLOAD API: Đã lưu ảnh thành công: {full_path_to_save_file}")
        # Trả về path tương đối mà Neo4j sẽ lưu (chỉ tên file) và app_name
        return jsonify({
            "success": True, 
            "message": "Upload ảnh thành công.", 
            "app_name": safe_app_name_as_folder, # app_name đã được an toàn hóa
            "filename": final_filename_on_server  # tên file đã được an toàn hóa
        }), 201
    except Exception as e:
        logger.error(f"UPLOAD API: Lỗi khi lưu file ảnh '{final_filename_on_server}' vào '{app_specific_storage_directory}': {e}", exc_info=True)
        if os.path.exists(full_path_to_save_file): # Xóa file nếu lưu lỗi
