...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    ...
    Sử dụng transaction và xử lý lỗi UniqueViolation rõ ràng hơn.

    Returns:
        template_ref nếu thành công hoàn toàn, None nếu thất bại.
    """
    logger = current_app.logger if current_app else print
    ...
    if not conn: return None
    cur = None
    returned_ref = None # Biến để lưu ref trả về

    try:
        cur = conn.cursor()
        logger.info(f"Attempting to add template '{template_ref}' and first variation.")

        # === BƯỚC 1: INSERT vào response_templates ===
        template_exists = False
        try:
            sql_template = """
                INSERT INTO public.response_templates (template_ref, description, category, updated_at)
                VALUES (%s, %s, %s, NOW());
            """
        ...
        except psycopg2.errors.UniqueViolation: # <<< BẮT LỖI NẾU REF ĐÃ TỒN TẠI >>>
            conn.rollback() # Rollback lại transaction nhỏ của lệnh INSERT lỗi
            conn.autocommit = True # Tắt chế độ transaction tự động nếu đang bật
            logger.warning(f"Template ref '{template_ref}' already exists. Proceeding to add variation.")
            template_exists = True
            # Cần mở lại cursor vì rollback có thể đã đóng nó (hoặc reset transaction)
            cur = conn.cursor()
            ...
            conn.rollback()
            raise db_err_template # Ném lại lỗi để except bên ngoài bắt
        ...
        # === BƯỚC 2: INSERT vào template_variations ===
        # Chỉ thực hiện nếu bước 1 thành công hoặc template đã tồn tại
        try:
            sql_variation = """
                INSERT INTO public.template_variations (template_ref, variation_text, last_used_at)
                VALUES (%s, %s, NULL);
            """
            ...
            # Lỗi ở bước 2 (có thể do variation trùng lặp nếu có UNIQUE constraint khác, hoặc FK lỗi - dù không nên)
            logger.error(f"DB Error inserting into template_variations for '{template_ref}': {db_err_variation}", exc_info=True)
            conn.rollback() # Rollback toàn bộ transaction (bao gồm cả insert template nếu nó mới được thêm)
            raise db_err_variation # Ném lại lỗi
         ...
         # Không cần rollback lại vì đã rollback bên trong
         returned_ref = None
    except Exception as e_outer:
        logger.error(f"LỖI (database.py - add_new_template): Lỗi không xác định: {e_outer}", exc_info=True)
        # Không cần rollback lại vì đã rollback bên trong
        returned_ref = None
    finally:
        if cur: cur.close()
