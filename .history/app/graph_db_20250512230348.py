        ...
        with driver.session(database=db_name) as session:
            ...
            if total_items > 0:
                ...
                for record in data_result:
                    node_dict = dict(record) # Chuyển Record thành dict
                    # Chuyển đổi datetime sang ISO string
                    for dt_field in ['created_at', 'last_seen']:
                        if node_dict.get(dt_field) and isinstance(node_dict[dt_field], (datetime, neo4j.time.DateTime, neo4j.time.ZonedDateTime)):
                            try:
                                # Đối với neo4j.time.DateTime hoặc ZonedDateTime
                                if hasattr(node_dict[dt_field], 'to_native') and callable(node_dict[dt_field].to_native):
                                    native_dt = node_dict[dt_field].to_native()
                                    if native_dt.tzinfo is None: # Nếu là naive datetime, gán timezone UTC
                                        native_dt = native_dt.replace(tzinfo=timezone.utc)
                                    node_dict[dt_field] = native_dt.isoformat()
                                elif isinstance(node_dict[dt_field], datetime): # Đối với datetime của Python
                                    if node_dict[dt_field].tzinfo is None:
                                        node_dict[dt_field] = node_dict[dt_field].replace(tzinfo=timezone.utc)
                                    node_dict[dt_field] = node_dict[dt_field].isoformat()
                                else: # Fallback
                                    node_dict[dt_field] = str(node_dict[dt_field])
                            except Exception as e:
                                logger.warning(f"Could not convert datetime {node_dict[dt_field]} to ISO string for field {dt_field}: {e}")
                                node_dict[dt_field] = str(node_dict[dt_field]) # Fallback
