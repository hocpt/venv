...
def admin_screen_elements(screen_id):
    ...
    return render_template('admin_screen_elements.html',
                           ...
                           screenshot_url=screenshot_url) # URL đã được tạo bằng endpoint tùy chỉnh
...
def api_get_app_graph_data(): # <<< Tên hàm này
    """
    API Endpoint trả về dữ liệu nodes và edges cho Cytoscape,
    bao gồm cả screenshot_url cho nodes.
    Lấy app_name từ query parameter.
    """
    logger = current_app.logger if current_app else print
    # Lấy app_name từ query parameter (?app_name=...)
    app_name = request.args.get('app_name')

    logger.info(f"API: Yêu cầu dữ liệu đồ thị cho app: '{app_name}'")
    # Kiểm tra xem app_name có được cung cấp không
    if not app_name:
    ...
    try:
        ...
        # Hàm get_app_graph_data cần trả về dict có key 'nodes' và 'edges'
        # và trong mỗi node data, cần có 'screenshot_path' (là tên file)
        graph_data = graph_db.get_app_graph_data(app_name)

        # Xử lý trường hợp graph_db trả về None (lỗi)
        ...
        # Xử lý trường hợp không có node (vẫn trả về cấu trúc hợp lệ)
        nodes_list = graph_data.get("nodes", []) # Lấy danh sách node, mặc định là rỗng
        edges_list = graph_data.get("edges", []) # Lấy danh sách cạnh

        if not nodes_list:
             logger.info(f"API: Không tìm thấy node nào cho app '{app_name}'.")
             # Vẫn trả về cấu trúc hợp lệ nhưng node rỗng
             return jsonify({"nodes": [], "edges": edges_list})

        # === THÊM screenshot_url VÀO DỮ LIỆU NODE ===
        processed_nodes = []
        for node_element in nodes_list:
            # node_element có dạng {"data": {...}}
            node_data = node_element.get("data", {}) # Lấy dictionary data bên trong

            # Lấy tên file từ thuộc tính screenshot_path (đã được trả về từ get_app_graph_data)
            filename = node_data.get("screenshot_path") # <<< Lấy tên file

            if filename:
                try:
                    # Tạo URL dùng route tùy chỉnh 'serve_app_specific_screenshot'
                    # Endpoint này được định nghĩa trong app/__init__.py
                    # _external=False để tạo URL tương đối (ví dụ: /app_screenshots/...)
                    ...
                    # Log lỗi nếu không tạo được URL
                    logger.error(f"API: Lỗi tạo URL cho screenshot '{filename}' của node {node_data.get('id')}: {url_err}", exc_info=True)
                    node_data["screenshot_url"] = None # Đặt là None nếu lỗi
            else:
                # Nếu không có filename thì không có URL
                node_data["screenshot_url"] = None

            # Không cần trả về screenshot_path nữa vì đã có screenshot_url
            node_data.pop("screenshot_path", None)

            # Thêm node đã xử lý (với screenshot_url) vào danh sách mới
            processed_nodes.append({"data": node_data})

        # Cập nhật lại danh sách node trong graph_data
        graph_data["nodes"] = processed_nodes
        # =========================================

        logger.info(f"API: Trả về dữ liệu đồ thị thành công cho app '{app_name}'.")
        return jsonify(graph_data) # Trả về dữ liệu JSON hoàn chỉnh
