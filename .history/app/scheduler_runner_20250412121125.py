...
def run_scheduler():
    ...
    Tự đọc cấu hình DB từ biến môi trường.
    """
    global live_scheduler
    print("INFO: Initializing independent BackgroundScheduler...")

    # --- Tự đọc cấu hình CSDL từ biến môi trường ---
    db_config = {
    ...
    if not all(db_config.values()):
        print("CRITICAL ERROR (scheduler_runner): Missing Database configuration environment variables (DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD). Cannot start scheduler.")
        return

    db_uri = f"postgresql+psycopg2://{db_config['user']}:{db_config['password']}@{db_config['host']}:{db_config['port']}/{db_config['dbname']}"
    print(f"DEBUG (scheduler_runner): Database URI for JobStore: {db_uri.replace(db_config['password'],'***')}") # Che password

    # --- Cấu hình Jobstore và Executor ---
    jobstores = { 'default': SQLAlchemyJobStore(url=db_uri) }
    executors = { 'default': ThreadPoolExecutor(max_workers=10) } # Có thể lấy max_workers từ config nếu muốn
    ...
    try:
        ...
        executor_instance = executors.get('default')
        executor_type_name = type(executor_instance).__name__ if executor_instance else 'N/A'
        ...
        print(traceback.format_exc())
        live_scheduler = None # Đặt lại là None nếu lỗi
    finally:
