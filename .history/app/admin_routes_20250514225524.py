...
def api_get_managed_nodes():
    ...
    try:
        ...
        logger.debug(f"API get_managed_nodes: app_filter='{app_name_for_db}', status_filter='{filter_status}', page={page}")

        # nodes_page_data đã bao gồm logical_pie_name từ hàm graph_db
        ...
        if nodes_page_data is None:
            logger.error(f"API get_managed_nodes: graph_db.get_screen_nodes_for_management returned None.")
            return jsonify({"nodes": [], "pagination": {}, "error": "Lỗi tải dữ liệu Node."}), 500

        # Xử lý thêm screenshot_full_url cho mỗi node
        final_nodes_list_for_response = []
        if nodes_page_data:
            for node_dict in nodes_page_data: # node_dict đã là dictionary
                screenshot_filename = node_dict.get('screenshot_path')
                # node_app_name = node_dict.get('app_name') # Không cần app_name cho URL ảnh này nữa

                if screenshot_filename:
                    try:
                        # Endpoint 'serve_app_specific_screenshot' không có 'admin.' vì nó nằm ở root app
                        # hoặc trong blueprint gốc (ví dụ: 'main.serve_app_specific_screenshot')
                        # Hãy đảm bảo tên endpoint này là đúng với định nghĩa route của bạn.
                        # Nếu hàm serve_app_specific_screenshot nằm trong app/__init__.py và không có endpoint tường minh,
                        # thì tên endpoint chính là tên hàm.
                        node_dict['screenshot_full_url'] = url_for(
                            'serve_app_specific_screenshot', # TÊN ENDPOINT CỦA ROUTE PHỤC VỤ ẢNH
                            filename=screenshot_filename,
                            _external=False 
                        )
                    except Exception as url_exc:
                        logger.error(f"Lỗi tạo URL ảnh cho node {node_dict.get('screen_id')} với file {screenshot_filename}: {url_exc}")
                        node_dict['screenshot_full_url'] = None
                else:
                    node_dict['screenshot_full_url'] = None
                final_nodes_list_for_response.append(node_dict)
        ...
        return jsonify({
            "nodes": final_nodes_list_for_response, # << TRẢ VỀ LIST ĐÃ XỬ LÝ
            "pagination": {
                "page": page,
                "per_page": per_page_val,
