...
def view_node_management():
    ...
    # ... (code lấy filter, page, per_page, distinct_app_names như cũ) ...
    selected_app_filter = request.args.get('app_name_filter', '').strip()
    # Sửa lại filter_status: nếu không có, không lọc theo status (lấy tất cả status)
    # Hoặc bạn có thể đặt một giá trị mặc định cụ thể nếu muốn, ví dụ 'all_issues'
    filter_status_req = request.args.get('filter_status', 'all') # 'all' có thể là giá trị mới cho "Tất cả"
    ...
    # Chuyển filter_status_req thành giá trị mà graph_db.py hiểu (có thể là None nếu 'all')
    ...
    try:
        # ... (logic lấy distinct_app_names_for_filter từ PostgreSQL của bạn) ...
        # Ví dụ:
    ...
    # raw_nodes_page_data là list các dict, mỗi dict phải chứa 'neo4j_db_id'
    raw_nodes_page_data, total_items_from_db = graph_db.get_screen_nodes_for_management(
        app_name=app_name_query_param,
        filter_status=status_for_graphdb, # Truyền status đã xử lý
        page=page,
    ...
    if raw_nodes_page_data is None:
        ...
        for node_data_from_graphdb_item in raw_nodes_page_data:
            ...
            # Lấy giá trị ID Neo4j từ key đã định nghĩa trong graph_db.py (neo4j_db_id)
            ...
            # Lấy thông tin PIE từ PostgreSQL nếu node 'defined' và chưa có logical_pie_name
            # Điều này quan trọng để đảm bảo node 'defined' có thông tin PIE name
            node_status = node_to_render.get('status', '').lower()
            # Các trạng thái được coi là "defined"
            ...
            if node_status in defined_statuses and not node_to_render.get('logical_pie_name'):
                try:
                    # Sử dụng screen_id (là defined_screen_id cho các node này) để query PostgreSQL
        ...
        if total_items_from_db is not None and total_items_from_db > 0:
            ...
            if filter_status_req: extra_params_for_pagination['filter_status'] = filter_status_req # Giữ filter gốc cho link
    ...
    if not nodes_for_template and (selected_app_filter or filter_status_req != 'all'): # Sửa điều kiện flash
    ...
    return render_template('admin_node_management.html', 
                           ...
                           distinct_app_names=distinct_app_names_for_filter,
                           selected_app_filter=selected_app_filter,
                           current_filter_status=filter_status_req, # Truyền filter gốc cho select box
