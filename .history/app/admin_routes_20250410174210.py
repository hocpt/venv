...
@admin_bp.route('/')
def index():
    # Sửa lỗi hiển thị flash message ở trang index
    # Trang index nên render template riêng, không kế thừa từ base ở đây
    return render_template('admin_index_content.html', title="Admin Dashboard")
...
def view_history():
    try:
        ...
        # Chỉ giữ lại các filter có giá trị
        ...
        if page > total_pages and total_pages > 0:
             page = total_pages
             # Có thể cần gọi lại DB nếu page bị thay đổi, hoặc chấp nhận hiển thị trang cuối rỗng
             # Đơn giản là set lại page để hiển thị đúng thông tin phân trang
             pass # Giữ nguyên history_items của trang cuối (có thể rỗng)
        ...
        active_filters = {} # Đặt lại filters khi có lỗi
    ...
    return render_template('admin_history.html',
                           ...
                           filters=active_filters, # Truyền filters để điền lại form
                           possible_statuses=POSSIBLE_HISTORY_STATUS) # Truyền list status cho dropdown
