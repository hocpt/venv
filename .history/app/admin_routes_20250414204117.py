...
def view_ai_simulations():
    ...
    simulations_display_list = [] # <<< Danh sách thống nhất để hiển thị
    ...
    if not db:
        ...
        try:
            ...
            # === TẠO DANH SÁCH HIỂN THỊ MÔ PHỎNG ===
            server_tz = _get_configured_timezone()

            # 1. Lấy các Lệnh đang chờ hoặc lỗi từ scheduler_commands
            recent_commands = db.get_recent_simulation_commands(
                status_list=['pending', 'processing', 'error'], limit=20
            ) or []

            command_map = {cmd['command_id']: cmd for cmd in recent_commands} # Map để tra cứu nhanh

            # 2. Lấy các Job đã được lên lịch từ apscheduler_jobs
            live_run_times = _get_live_next_run_times() or {} # Dùng dict trả về từ hàm helper

            processed_command_ids_from_jobs = set()

            # 3. Xử lý các Job đã lên lịch (từ apscheduler_jobs)
            for job_id, next_run_timestamp in live_run_times.items():
                if job_id.startswith('sim_run_'):
                    sim_info = {'id': job_id, 'type': 'job', 'config_info': '(Ad-hoc/Unknown)', 'status_text': 'Unknown', 'created_at': None}
                    command_id_from_job = None
                    # Cố gắng trích xuất command_id từ job_id nếu có thể (tùy cấu trúc ID bạn đặt)
                    ...
                    # Lấy thông tin config từ command tương ứng (nếu tìm được)
                    if command_id_from_job and command_id_from_job in command_map:
                         original_command = command_map[command_id_from_job]
                         payload = original_command.get('payload', {})
                         sim_info['config_info'] = f"Cmd {command_id_from_job}: {payload.get('persona_a_id','?')} vs {payload.get('persona_b_id','?')} ({payload.get('strategy_id','?')})"
                         sim_info['created_at'] = original_command.get('created_at')
                         processed_command_ids_from_jobs.add(command_id_from_job) # Đánh dấu đã xử lý

                    # Xác định trạng thái và thời gian chạy
                    if next_run_timestamp is not None:
                        try:
                            utc_dt = datetime.fromtimestamp(next_run_timestamp, tz=timezone.utc)
                            local_dt = utc_dt.astimezone(server_tz)
                        ...
                        sim_info['status_text'] = 'Paused/Finished?' # Khó xác định chính xác chỉ từ đây

                    simulations_display_list.append(sim_info)

            # 4. Xử lý các Lệnh chưa được xử lý hoặc đang xử lý (từ scheduler_commands)
            for cmd_id, command_data in command_map.items():
                 # Chỉ thêm nếu lệnh này chưa được xử lý ở bước trên (tức là chưa có job tương ứng được tìm thấy)
                 # Hoặc nếu trạng thái là error
                 if cmd_id not in processed_command_ids_from_jobs or command_data.get('status') == 'error':
                    payload = command_data.get('payload', {})
                    status_text = command_data.get('status', 'unknown').capitalize()
                    if status_text == 'Pending': status_text = 'Pending Queue'
                    elif status_text == 'Processing': status_text = 'Processing Cmd'
                    elif status_text == 'Error': status_text = f"Error: {command_data.get('error_message', '')[:100]}"
                    ...
                    sim_info = {
                        ...
                        'next_run_time_str': '---' # Chưa được lên lịch
                    }
                    simulations_display_list.append(sim_info)

            # 5. Sắp xếp danh sách hiển thị (ví dụ: theo thời gian tạo lệnh)
