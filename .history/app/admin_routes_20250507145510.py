...
def admin_screen_elements(screen_id):
    ...
    try:
        ...
        if filename_from_neo4j:
            try:
                # Sử dụng route 'serve_project_screenshot' đã định nghĩa trong app/__init__.py
                screenshot_url = url_for('serve_project_screenshot', filename=filename_from_neo4j)
                logger.info(f"Đã tạo URL screenshot: {screenshot_url} từ tên file: {filename_from_neo4j}")
            except Exception as url_err:
                logger.error(f"Lỗi tạo URL tĩnh cho screenshot path '{filename_from_neo4j}': {url_err}")
                screenshot_url = None
        else:
            logger.info(f"Không tìm thấy screenshot_path cho màn hình {screen_id}.")
        # =======================================
