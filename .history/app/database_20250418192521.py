...
def delete_transition(transition_id: int) -> tuple[bool, str | None]: # <<< Sửa kiểu trả về
    """Xóa một transition khỏi stage_transitions.
       Trả về tuple (success: bool, error_message: str | None).
    """
    print(f"DEBUG (db.delete_transition): Attempting to delete transition ID={transition_id}") # Log khi bắt đầu
    ...
    try:
        ...
        conn.commit()
        affected_rows = cur.rowcount # Lấy số dòng bị ảnh hưởng
        success = affected_rows > 0 # True nếu có ít nhất 1 dòng bị xóa
        if not success:
             error_msg = f"Không tìm thấy transition_id {transition_id} để xóa." # <<< Gán lỗi nếu không tìm thấy
        ...
        error_msg = f"Lỗi CSDL khi xóa transition: {db_err}" # <<< Gán lỗi DB
        print(f"LỖI (database.py - delete_transition): {error_msg}\n{traceback.format_exc()}")
        if conn: conn.rollback()
        success = False # Đảm bảo success là False khi có lỗi
    except Exception as e:
        error_msg = f"Lỗi không xác định khi xóa transition: {e}" # <<< Gán lỗi chung
        print(f"LỖI (database.py - delete_transition): {error_msg}\n{traceback.format_exc()}")
        if conn: conn.rollback()
        success = False # Đảm bảo success là False khi có lỗi
    finally:
...
def add_new_transition(current_stage_id: str, user_intent: str,
                       ...
                       response_template_ref: str | None = None, # Tham số 5
                       action_macro_code: str | None = None, # Tham số 6
                       action_params_str: str | None = None, # Tham số 7
                       condition_type: str | None = None, # Tham số 8
                       condition_value: str | None = None  # Tham số 9
                       ) -> tuple[bool, str | None]:
    """Thêm transition mới, xử lý cả language và control."""
    if not current_stage_id or not user_intent:
        return False, "Current Stage và User Intent là bắt buộc."

    action_json_for_db = None
    # Xử lý action chỉ khi có macro_code được cung cấp
        ...
        if action_params_str and action_params_str.strip():
            try:
                ...
                if not isinstance(params_dict, dict):
                     raise json.JSONDecodeError("Params phải là object JSON.", action_params_str, 0)
            except json.JSONDecodeError:
                return False, "Action Params không phải JSON object hợp lệ."
    ...
    try:
        cur = conn.cursor()
        # Câu lệnh INSERT với đủ các cột
        sql = """
        ...
        params = (
            ...
            priority,
            # Chuyển dict action thành chuỗi JSON hoặc None
            json.dumps(action_json_for_db) if action_json_for_db else None,
            template_ref_to_save # Lưu template ref
        )
        cur.execute(sql, params)
        conn.commit()
        success = True
