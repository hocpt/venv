...
def get_all_macro_definitions(app_target: str | None = None, include_generic: bool = True) -> list[dict] | None:
    """Lấy danh sách định nghĩa macro. Nếu không có app_target, lấy tất cả."""
    macros_list = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        ...
        # --- START MODIFICATION ---
        # Chỉ áp dụng filter nếu app_target được cung cấp CỤ THỂ
        if app_target is not None:
            filter_targets = [app_target] # Bắt đầu với target cụ thể
            if include_generic:
                # Thêm các target dùng chung
            ...
            if target_conditions: # Chỉ thêm WHERE nếu có điều kiện target
                 where_clauses.append(f"({ ' OR '.join(target_conditions) })")
                 params.extend(valid_targets_in_params)

        # Nếu không có filter nào được thêm (ví dụ: app_target=None từ route view_macro_definitions)
        if where_clauses:
            sql = f"{base_sql} WHERE {' AND '.join(where_clauses)} ORDER BY app_target, macro_code;"
        else:
            # Lấy TẤT CẢ nếu không có filter app_target được chỉ định
            sql = f"{base_sql} ORDER BY app_target, macro_code;"
        # --- END MODIFICATION ---

        # Giữ lại các dòng print debug để kiểm tra
        print(f"DEBUG SQL (get_all_macros): {cur.mogrify(sql, tuple(params)).decode('utf-8', 'ignore')}")
        cur.execute(sql, tuple(params))
        rows = cur.fetchall()
        print(f"DEBUG DB (get_all_macros): Fetched {len(rows)} rows.") # Kiểm tra số dòng lấy được

        macros_list = [dict(row) for row in rows] if rows else []
    except Exception as e:
        print(f"ERROR (db - get_all_macro_definitions): {e}")
        traceback.print_exc() # In lỗi chi tiết
        macros_list = None # Trả về None khi lỗi
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return macros_list # Trả về list (có thể rỗng) hoặc None
