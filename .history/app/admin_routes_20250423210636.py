...
def add_strategy():
    ...
    # --- Xử lý POST request ---
    if request.method == 'POST':
        # ... (lấy dữ liệu form: strategy_id, name, description, initial_stage_id, strategy_type) ...
        ...
        # Validate dữ liệu
        errors = []
        # --- DÒNG KIỂM TRA NÀY PHẢI ĐƯỢC SỬA ĐÚNG ---
        # Nó KHÔNG được kiểm tra 'if not initial_stage_id' nữa
        if not strategy_id or not name or not strategy_type:
            # Thông báo lỗi này KHÔNG nên bao gồm "Initial Stage ID"
            errors.append("Strategy ID, Name, và Strategy Type (ẩn) là bắt buộc.")
        # --- KẾT THÚC SỬA DÒNG KIỂM TRA ---

        # Các kiểm tra khác (ví dụ: strategy_type hợp lệ) giữ nguyên
        if strategy_type not in ['language', 'control', 'mainloop']:
            errors.append("Strategy Type không hợp lệ.")

        # Nếu có lỗi validation
        if errors:
            for error in errors: flash(error, "warning")
            # ... (Lấy lại stages và render lại template như cũ) ...
            stages = db.get_all_stages() or [] # Cần lấy lại stages
            return render_template('admin_add_strategy.html',
                                   title=f"Thêm {strategy_type.capitalize()} Strategy Mới (Lỗi)",
                                   stages=stages,
        ...
        try:
            # <<< Truyền initial_stage_id đã xử lý (có thể là None) >>>
            success, error_msg = db.add_new_strategy(
                strategy_id=strategy_id,
                name=name,
                description=description or None,
                initial_stage_id=initial_stage_id if initial_stage_id else None, # <<< Chuyển "" thành None
                strategy_type=strategy_type
            )
            ...
            if success:
                ...
                # ... (Redirect như cũ) ...
                if strategy_type == 'control': redirect_url = url_for('admin.view_strategies_control')
                elif strategy_type == 'mainloop': redirect_url = url_for('admin.view_strategies_mainloop')
                ...
                stages = db.get_all_stages() or []
                return render_template('admin_add_strategy.html',
                                       title=f"Thêm {strategy_type.capitalize()} Strategy Mới (Lỗi DB)",
                                       stages=stages,
                                       strategy_type=strategy_type, # Dùng type từ form
                                       current_data=request.form)

        # ... (Khối except Exception giữ nguyên) ...
        except Exception as e:
            ...
            stages = db.get_all_stages() or []
            return render_template('admin_add_strategy.html',
                                   title=f"Thêm {strategy_type.capitalize()} Strategy Mới (Lỗi Exception)",
                                   stages=stages,
                                   strategy_type=strategy_type, # Dùng type từ form
                                   current_data=request.form)

    # --- Xử lý GET request (giữ nguyên như cũ) ---
    # ... (Lấy stages, xác định title, render template) ...
    strategy_type_from_url = request.args.get('type', 'language')
    if strategy_type_from_url not in ['language', 'control', 'mainloop']:
        strategy_type_from_url = 'language'
    title = f"Thêm {strategy_type_from_url.capitalize()} Strategy Mới"
    stages = []
