...
def view_scheduled_jobs():
    ...
    db_jobs_config = []
    pending_counts = {} # Vẫn giữ lại để đếm pending
    ...
    if not db:
        ...
        try:
            ...
            # Logic đếm pending cho suggestion_job (giữ nguyên)
    ...
    # <<< KHÔNG CÒN LẤY TRẠNG THÁI LIVE TỪ SCHEDULER >>>

    # Render template chỉ với cấu hình DB và pending counts
    return render_template('admin_scheduled_jobs.html',
                            title="Quản lý Tác vụ nền (Cấu hình)",
                            jobs=db_jobs_config,
                            pending_counts=pending_counts) # Chỉ truyền 2 biến này
