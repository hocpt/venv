...
def get_screen_nodes_for_management(app_name: str | None,
    ...
    driver = get_driver() 
    if not driver:
        logger.error("GRAPH_DB: get_screen_nodes_for_management - Neo4j driver not available.")
        return None, None

    raw_nodes_from_neo4j = []
    total_items = 0
    
    skip_val = (page - 1) * per_page

    where_clauses = []
    params_cypher = {"skip_val": skip_val, "limit_val": per_page}

    if app_name:
        where_clauses.append("s.app_name = $app_name_param")
        params_cypher["app_name_param"] = app_name
    
    # Xử lý filter_status một cách cẩn thận
    if filter_status and filter_status != 'all':
        actual_filter_status = filter_status # Giữ nguyên giá trị từ request
        if actual_filter_status == 'unknown': 
            # Nếu người dùng chọn 'unknown', bao gồm cả các biến thể như 'provisional_unknown'
            # và có thể cả các node chưa có status hoặc status là NULL (tùy logic của bạn)
            where_clauses.append("(s.status STARTS WITH 'unknown' OR s.status STARTS WITH 'provisional_unknown' OR s.status IS NULL)")
        elif actual_filter_status == 'defined':
             where_clauses.append("(s.status = 'defined' OR s.status = 'defined_from_unknown' OR s.status = 'merged_to_defined')") # Bao gồm các trạng thái defined
        else: # Các status cụ thể khác
            where_clauses.append("s.status = $status_param")
            params_cypher["status_param"] = actual_filter_status
    ...
    data_query = f"""
        ...
        {where_statement}
        RETURN
            elementId(s) AS neo4j_db_id, // << LUÔN LẤY ID NÀY
            ...
            s.element_count AS defined_element_count, 
            size([(s)-[:HAS_ELEMENT]->(e) | e]) as actual_element_count_rel,
    ...
    try:
        db_name_neo4j = current_app.config.get('NEO4J_DATABASE', 'neo4j')
        with driver.session(database=db_name_neo4j) as session:
            logger.debug(f"GRAPH_DB: Executing count query: {count_query} with params {params_cypher}")
            count_result = session.run(count_query, params_cypher)
            total_record = count_result.single()
            if total_record:
                total_items = total_record["total"]
            
            if total_items > 0:
                logger.debug(f"GRAPH_DB: Executing data query: {data_query} with params {params_cypher}")
                data_result = session.run(data_query, params_cypher)
                for record in data_result:
                    record_dict = dict(record) 
                    # Log quan trọng để kiểm tra giá trị neo4j_db_id cho MỌI node
                    logger.info(f"GRAPH_DB_FETCHED_NODE_DETAIL: screen_id='{record_dict.get('screen_id')}', status='{record_dict.get('status')}', neo4j_db_id='{record_dict.get('neo4j_db_id')}' (Type: {type(record_dict.get('neo4j_db_id'))})")
                    if not record_dict.get('neo4j_db_id'):
                         logger.error(f"GRAPH_DB LỖI NGHIÊM TRỌNG: Node {record_dict.get('screen_id')} (status: {record_dict.get('status')}) có neo4j_db_id là None hoặc rỗng từ Neo4j!")
                    raw_nodes_from_neo4j.append(record_dict)
        
        logger.debug(f"GRAPH_DB: get_screen_nodes_for_management - Returning {len(raw_nodes_from_neo4j)} raw nodes for page {page}. Total items: {total_items}")
        return raw_nodes_from_neo4j, total_items
            
    except Exception as e:
        logger.error(f"GRAPH_DB: Lỗi không mong muốn trong get_screen_nodes_for_management: {e}", exc_info=True)
        return None, None  
...
def merge_neo4j_screen_nodes_and_delete_source(source_screen_id: str, target_screen_id: str, app_name: str) -> tuple[bool, str | None]:
    logger = current_app.logger
    driver = get_driver()
    if not driver: return False, "Neo4j driver not available"
    if source_screen_id == target_screen_id: return False, "Source và Target screen ID không thể giống nhau."

    logger.info(f"Attempting to merge Neo4j node '{source_screen_id}' into '{target_screen_id}' for app '{app_name}'.")

    # Câu lệnh Cypher để merge nodes.
    # LƯU Ý: Đây là một câu lệnh phức tạp và cần được kiểm thử kỹ lưỡng.
    # Nó sử dụng apoc.refactor.mergeNodes nếu có, hoặc logic thủ công.
    # Cách tiếp cận thủ công (an toàn hơn nếu không có APOC hoặc muốn kiểm soát):
    # 1. Tìm source node và target node.
    # 2. Với mỗi cạnh INCOMING vào source, tạo một cạnh mới tương tự vào target (nếu chưa tồn tại).
    # 3. Với mỗi cạnh OUTGOING từ source, tạo một cạnh mới tương tự từ target (nếu chưa tồn tại).
    # 4. (Tùy chọn) Gộp thuộc tính.
    # 5. DETACH DELETE source node.

    # Sử dụng APOC (nếu đã cài đặt và cho phép) sẽ đơn giản hơn nhiều:
    # cypher_merge = """
    # MATCH (source:Screen {screen_id: $source_id, app_name: $app_name_param})
    # MATCH (target:Screen {screen_id: $target_id, app_name: $app_name_param})
    # CALL apoc.refactor.mergeNodes([source, target], {
    #   properties: 'overwrite', // Hoặc 'discard', 'combine'
    #   mergeRels: true
    # }) YIELD node
    # RETURN node.screen_id as final_id
    # """
    # params = {
    ...
    # Cách thủ công (ví dụ cơ bản, cần hoàn thiện nhiều hơn cho các loại thuộc tính và cạnh)
    # Bước này cần được thực hiện trong một transaction
    def transaction_merge_nodes(tx: Transaction, source_id, target_id, app):
        tx_logger = current_app.logger # Hoặc logger đã truyền vào

        # 0. Kiểm tra sự tồn tại của cả hai node
        res_source = tx.run("MATCH (s:Screen {screen_id: $id, app_name: $app}) RETURN s", id=source_id, app=app).single()
        res_target = tx.run("MATCH (t:Screen {screen_id: $id, app_name: $app}) RETURN t", id=target_id, app=app).single()

        if not res_source:
            raise Neo4jError(f"Source node '{source_id}' không tìm thấy cho app '{app}'.")
        if not res_target:
            raise Neo4jError(f"Target node '{target_id}' không tìm thấy cho app '{app}'.")

        tx_logger.info(f"  TX Merge: Source '{source_id}' và Target '{target_id}' tìm thấy.")

        # 1. Chuyển các cạnh INCOMING
        # MATCH (anyNode)-[r:TRANSITION]->(source:Screen {screen_id:$source_id, app_name:$app})
        # WITH anyNode, r, source
        # MATCH (target:Screen {screen_id:$target_id, app_name:$app})
        # MERGE (anyNode)-[new_r:TRANSITION]->(target) // Cần đảm bảo không tạo cạnh trùng nếu đã có
        # ON CREATE SET new_r = properties(r) // Copy thuộc tính
        # DELETE r
        # RETURN count(new_r) as incoming_moved

        # Cách an toàn hơn là tạo cạnh mới nếu chưa tồn tại, rồi xóa cạnh cũ
        # (Phần này cần code Cypher phức tạp hơn để xử lý đúng các loại cạnh và thuộc tính)

        # Tạm thời, đơn giản hóa bằng cách chỉ cập nhật status của source và liên kết nó
        # Đây KHÔNG phải là merge thực sự, mà là đánh dấu source đã được xử lý bởi target.
        # Để merge thực sự, bạn cần dùng APOC hoặc logic Cypher chuyển cạnh phức tạp.
        # Với mục tiêu hiện tại, chúng ta sẽ "resolve" node unknown này bằng cách
        # đánh dấu nó đã được xử lý và trỏ đến defined node.

        # Đơn giản hóa: Đổi status của source_screen_id và thêm quan hệ RESOLVED_TO
        # Node source sẽ không bị xóa ngay, mà có thể được dọn dẹp sau.
        update_source_cypher = """
        MATCH (source:Screen {screen_id: $source_id, app_name: $app_name_param})
        MATCH (target:Screen {screen_id: $target_id, app_name: $app_name_param})
        SET source.status = 'merged_to_defined', 
            source.merged_to_screen_id = $target_id,
            source.updated_at = datetime()
        MERGE (source)-[res:RESOLVED_TO]->(target)
        RETURN source.screen_id as updated_source_id
        """
        result = tx.run(update_source_cypher, source_id=source_id, target_id=target_id, app_name_param=app)
        record = result.single()
        if not record or not record["updated_source_id"]:
             raise Neo4jError(f"Không thể cập nhật trạng thái cho source node '{source_id}' khi merge.")
        tx_logger.info(f"  TX Merge: Node '{source_id}' được đánh dấu là merged_to_defined và trỏ đến '{target_id}'.")


    try:
        db_name = current_app.config.get('NEO4J_DATABASE', 'neo4j')
        with driver.session(database=db_name) as session:
            session.write_transaction(transaction_merge_nodes, source_screen_id, target_screen_id, app_name)
        logger.info(f"Đã xử lý merge logic cho '{source_screen_id}' -> '{target_screen_id}'.")
        return True, None
    except Neo4jError as db_err:
        logger.error(f"Lỗi Neo4j khi merge nodes: {db_err.message}", exc_info=False)
        return False, db_err.message
    except Exception as e:
        logger.error(f"Lỗi không mong muốn khi merge nodes: {e}", exc_info=True)
        return False, str(e)
def convert_unknown_to_defined_node(unknown_screen_id: str, app_name: str,
    new_defined_screen_id: str, new_status: str = 'defined',
    new_logical_name_for_neo4j: str | None = None) -> tuple[bool, str | None]:
    logger = current_app.logger
    driver = get_driver()
    if not driver: return False, "Neo4j driver not available"

        # 1. Kiểm tra xem new_defined_screen_id đã tồn tại cho một NODE KHÁC chưa
        #    (Node unknown hiện tại sẽ được đổi tên, nên không tự so sánh với chính nó)
    check_conflict_query = """
        MATCH (existing:Screen {screen_id: $new_id, app_name: $app})
        WHERE existing.screen_id <> $old_id // Chỉ xét node khác với node unknown hiện tại
        RETURN count(existing) as conflict_count
        """
    conflict_params = {"new_id": new_defined_screen_id, "app": app_name, "old_id": unknown_screen_id}
    
    db_name = current_app.config.get('NEO4J_DATABASE', 'neo4j')
    try:
        with driver.session(database=db_name) as session:
            conflict_result = session.run(check_conflict_query, conflict_params).single()
            if conflict_result and conflict_result["conflict_count"] > 0:
                msg = f"New Defined Screen ID '{new_defined_screen_id}' đã được sử dụng bởi một Node khác trong app '{app_name}'."
                logger.error(msg)
                return False, msg
            
            # 2. Nếu không xung đột, thực hiện cập nhật node unknown
            cypher_update = """
            MATCH (s:Screen {screen_id: $old_id_param, app_name: $app_name_param})
            WHERE s.status STARTS WITH 'provisional_unknown' OR s.status STARTS WITH 'unknown' // Chỉ đổi từ trạng thái unknown
            SET s.screen_id = $new_id_param, 
                s.status = $new_status_param,
                s.logical_pie_name_ref = $logical_name_param, // Lưu lại tên logic PIE
                s.updated_at = datetime()
            RETURN s.screen_id AS updated_id, s.status AS updated_status
            """
            update_params = {
                "old_id_param": unknown_screen_id,
                "app_name_param": app_name,
                "new_id_param": new_defined_screen_id,
                "new_status_param": new_status,
                "logical_name_param": new_logical_name_for_neo4j
            }
            logger.debug(f"Executing Neo4j update for unknown node: {cypher_update} with params {update_params}")
            result = session.run(cypher_update, update_params)
            record = result.single()
            
            if record and record["updated_id"] == new_defined_screen_id:
                logger.info(f"Node Neo4j '{unknown_screen_id}' đã được cập nhật thành screen_id '{new_defined_screen_id}' và status '{new_status}'.")
                # TODO (Nâng cao): Xử lý refactor các cạnh TRANSITION ở đây nếu cần.
                # Ví dụ: Tìm các cạnh cũ, tạo cạnh mới với node đã đổi ID, xóa cạnh cũ.
                # Hoặc sử dụng APOC nếu có.
                return True, None
            else:
                logger.warning(f"Không thể cập nhật Node Neo4j '{unknown_screen_id}' (có thể không ở trạng thái unknown, không tìm thấy, hoặc new_id bị trùng và đã bị chặn).")
                return False, f"Node '{unknown_screen_id}' không thể cập nhật (trạng thái không phù hợp hoặc không tìm thấy)."
    except Neo4jError as db_err:
        logger.error(f"Lỗi Neo4j khi convert unknown node '{unknown_screen_id}': {db_err.message}", exc_info=False)
        return False, db_err.message
    except Exception as e:
        logger.error(f"Lỗi không mong muốn khi convert unknown node '{unknown_screen_id}': {e}", exc_info=True)
        return False, str(e)
    
def convert_unknown_node_to_defined_and_update_id(
    tx, # Đối tượng transaction của Neo4j
    unknown_screen_id: str, 
    app_name: str, 
    new_defined_screen_id: str, 
    ...
    """
    Updates an 'unknown' Screen node in Neo4j to 'defined' status,
    changes its screen_id to the new_defined_screen_id, and optionally updates its logical_name.
    This function is intended to be run within a Neo4j transaction.

    IMPORTANT: This basic version does NOT handle merging/updating :TRANSITION relationships.
               It focuses on updating the node's properties. More complex logic
               would be needed to correctly re-link transitions if the screen_id
               is a primary key for relationships. A safer approach for relationships
               might involve creating a new node and migrating relationships, then deleting the old.
               However, for now, we update in-place assuming screen_id changes are manageable
               or that relationship integrity is handled elsewhere/later.
    Args:
        tx: Neo4j transaction object.
        unknown_screen_id: The current screen_id of the 'unknown' node.
        app_name: The application name.
        new_defined_screen_id: The new screen_id (defined PIE ID).
        new_status: The new status (e.g., 'defined').
        new_logical_name: Optional new logical name for the screen.
    Returns:
        Tuple (success: bool, error_message: str | None)
    """
    current_app.logger.info(f"Neo4j: Cố gắng chuyển đổi node '{unknown_screen_id}' (app: {app_name}) thành '{new_defined_screen_id}' với status '{new_status}'.")
    
    # Bước 1: Kiểm tra xem new_defined_screen_id đã tồn tại cho app_name này chưa (trừ chính node đang xét)
    # Điều này quan trọng nếu bạn thực sự đổi screen_id. Nếu chỉ thêm thuộc tính thì không cần.
    # Vì screen_id là định danh chính, việc đổi nó có thể gây ra xung đột.
    # Để an toàn, PIE definition phải là duy nhất, nên node trong Neo4j cũng nên phản ánh điều đó.
    
    # Query kiểm tra new_defined_screen_id
    # query_check_existing = """
    # MATCH (s:Screen {app_name: $app_name, screen_id: $new_screen_id})
    # WHERE s.screen_id <> $old_screen_id  // Loại trừ chính node đang đổi tên (nếu screen_id thực sự thay đổi)
    # RETURN s.screen_id AS screenId LIMIT 1
    # """
    # result_check = tx.run(query_check_existing, app_name=app_name, new_screen_id=new_defined_screen_id, old_screen_id=unknown_screen_id)
    # existing_node = result_check.single()
    # if existing_node:
    #     msg = f"Neo4j: Screen ID '{new_defined_screen_id}' đã tồn tại cho app '{app_name}' (là một node khác)."
    #     current_app.logger.warning(msg)
    #     return False, msg

    # Cập nhật node hiện tại
    # Lưu ý: Nếu screen_id là một phần của identity của node hoặc được dùng trong index UNIQUE,
    # việc đổi tên trực tiếp có thể phức tạp.
    # Cách tiếp cận an toàn hơn có thể là tạo node mới và di chuyển quan hệ, nhưng phức tạp hơn.
    # Giả định rằng chúng ta có thể cập nhật screen_id và các thuộc tính khác.
    
    set_clauses = ["s.status = $new_status", "s.screen_id = $new_screen_id", "s.updated_at = $updated_at"]
    params = {
        "old_screen_id": unknown_screen_id,
        "app_name": app_name,
        "new_screen_id": new_defined_screen_id,
        "new_status": new_status,
        "updated_at": datetime.now(timezone.utc) # Hoặc dùng datetime() của Neo4j
    }

    if new_logical_name:
        set_clauses.append("s.logical_name = $new_logical_name")
        params["new_logical_name"] = new_logical_name
    
    # Nếu có các thuộc tính khác của node "unknown" cần xóa (ví dụ: is_provisional_unknown), thêm vào REMOVE
    # remove_clauses = [] 
    # if remove_clauses: query_remove_part = "REMOVE " + ", ".join(remove_clauses) else: query_remove_part = ""

    query_update_node = f"""
        MATCH (s:Screen {{screen_id: $old_screen_id, app_name: $app_name}})
        SET {', '.join(set_clauses)}
        RETURN s.screen_id AS updatedScreenId, s.status AS updatedStatus
    """
    # query_update_node += query_remove_part # Thêm phần REMOVE nếu có

    try:
        current_app.logger.debug(f"Executing Neo4j update: {query_update_node} with params: {params}")
        result = tx.run(query_update_node, **params)
        summary = result.consume() # Lấy summary để kiểm tra counters

        if summary.counters.properties_set > 0 or summary.counters.nodes_created == 0 and summary.counters.nodes_deleted == 0:
            # Kiểm tra cụ thể hơn nếu screen_id thực sự đã đổi
            # verification_query = "MATCH (s:Screen {app_name: $app_name, screen_id: $new_screen_id}) RETURN s.screen_id"
            # verification_result = tx.run(verification_query, app_name=app_name, new_screen_id=new_defined_screen_id)
            # if verification_result.single():
            current_app.logger.info(f"Neo4j: Node '{unknown_screen_id}' đã được cập nhật thành '{new_defined_screen_id}', status '{new_status}'.")
            return True, None
            # else:
            #     msg = f"Neo4j: Cập nhật Node properties thành công, nhưng không xác minh được screen_id mới '{new_defined_screen_id}'."
            #     current_app.logger.warning(msg)
            #     return False, msg # Hoặc True nhưng có cảnh báo
        else:
            # Không có thuộc tính nào được set, có thể node không tìm thấy
            msg = f"Neo4j: Không tìm thấy Node với screen_id '{unknown_screen_id}' và app_name '{app_name}' để cập nhật, hoặc không có thay đổi nào được thực hiện."
            current_app.logger.warning(msg)
            return False, msg
            
    except Neo4jError as ne: # Bắt lỗi cụ thể của Neo4j driver
        current_app.logger.error(f"Lỗi Neo4j khi cập nhật node '{unknown_screen_id}': {ne}", exc_info=True)
        return False, f"Lỗi Neo4j: {ne}"
    except Exception as e:
        current_app.logger.error(f"Lỗi không mong muốn khi cập nhật node Neo4j '{unknown_screen_id}': {e}", exc_info=True)
        return False, f"Lỗi không mong muốn: {e}"


# Thêm một hàm wrapper để quản lý transaction nếu cần gọi từ route
def convert_unknown_to_defined_node_wrapper(
    unknown_screen_id: str, 
    app_name: str, 
    new_defined_screen_id: str, 
    new_status: str = 'defined', 
    ...
    try:
        driver = get_driver()
        if not driver:
            return False, "Không thể kết nối Neo4j."
        
        db_name_neo4j = current_app.config.get('NEO4J_DATABASE', 'neo4j')
        with driver.session(database=db_name_neo4j) as session:
            # Thực hiện trong một transaction
            success, error_msg = session.execute_write(
                convert_unknown_node_to_defined_and_update_id, # Hàm transaction của bạn
                unknown_screen_id,
                app_name,
                new_defined_screen_id,
                new_status,
                new_logical_name
            )
            return success, error_msg
    except Exception as e:
        current_app.logger.error(f"Lỗi trong convert_unknown_to_defined_node_wrapper: {e}", exc_info=True)
...
def update_transition_by_neo4j_id_tx(tx: Transaction, neo4j_edge_id: str, properties_to_set: dict) -> dict:
    ...
    Hàm này được thiết kế để chạy bên trong một Neo4j transaction (ví dụ: session.write_transaction).
    
    Args:
        tx: Đối tượng Neo4j Transaction.
        neo4j_edge_id: ID nội bộ của cạnh Neo4j (lấy từ elementId(r)).
        properties_to_set: Dictionary chứa các thuộc tính và giá trị mới để SET.
                           Lưu ý: các key trong dict này phải khớp với tên thuộc tính 
                           bạn muốn dùng trong Cypher (ví dụ: actionType thay vì action_type).
    ...
    """
    logger = current_app.logger # Hoặc logger của bạn
    logger.debug(f"GRAPH_DB: Bắt đầu update_transition_by_neo4j_id_tx cho edge ID: {neo4j_edge_id} với props: {properties_to_set}")

    # Xây dựng phần SET của câu lệnh Cypher một cách động để chỉ SET các thuộc tính được cung cấp
    # Hoặc dùng cách đơn giản hơn là SET r += $props nếu properties_to_set đã được chuẩn bị đúng tên key
    
    # Cách 1: SET r += $props (đơn giản nếu properties_to_set có tên key khớp với Neo4j)
    # Đảm bảo properties_to_set không chứa các key không mong muốn hoặc cần xử lý đặc biệt
    cypher_query = """
        MATCH ()-[r]->()
        WHERE elementId(r) = $edge_id
        SET r += $props_to_set 
        RETURN elementId(r) AS updated_edge_id, properties(r) as updated_props
    """
    params = {
        "edge_id": neo4j_edge_id,
        "props_to_set": properties_to_set # properties_to_set đã bao gồm updated_at từ route
    }

    # Cách 2: Xây dựng mệnh đề SET động (an toàn hơn nếu muốn kiểm soát chặt chẽ từng thuộc tính)
    # set_clauses = []
    # query_params = {"edge_id": neo4j_edge_id}
    # for key, value in properties_to_set.items():
    #     # Cần map key từ Python (ví dụ 'action_type') sang key của Neo4j (ví dụ 'actionType') nếu khác nhau
    #     # Ví dụ: neo4j_key = "actionType" if key == "action_type" else key
    #     # set_clauses.append(f"r.{neo4j_key} = ${key}") # Dùng key của Python làm placeholder
    #     set_clauses.append(f"r.{key} = ${key}") # Giả sử key trong properties_to_set đã đúng với Neo4j
    #     query_params[key] = value
    
    # if not set_clauses:
    #     logger.warning(f"GRAPH_DB: Không có thuộc tính nào để cập nhật cho edge ID {neo4j_edge_id}.")
    #     return {"success": False, "error": "Không có thuộc tính nào được cung cấp để cập nhật."}

    # cypher_query = f"""
    #     MATCH ()-[r]->()
    #     WHERE elementId(r) = $edge_id
    #     SET {', '.join(set_clauses)}
    #     RETURN elementId(r) AS updated_edge_id
    # """
    # params = query_params
    
    logger.debug(f"GRAPH_DB: Executing Cypher: {cypher_query} with params: {params}")
    
    try:
        result = tx.run(cypher_query, params)
        record = result.single()
        
        if record and record["updated_edge_id"]:
            logger.info(f"GRAPH_DB: Cập nhật thành công thuộc tính cho cạnh Neo4j ID: {record['updated_edge_id']}. Props mới: {record['updated_props']}")
            return {"success": True, "message": "Cập nhật transition thành công."}
        else:
            logger.warning(f"GRAPH_DB: Không tìm thấy cạnh với Neo4j ID '{neo4j_edge_id}' để cập nhật, hoặc không có gì thay đổi.")
            return {"success": False, "error": f"Không tìm thấy transition với ID '{neo4j_edge_id}' hoặc không có thuộc tính nào được cập nhật."}
