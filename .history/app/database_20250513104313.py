...
def get_screen_definitions_for_app(app_name: str, activity_name: str | None = None) -> list[dict] | None:
    conn = get_db_connection() 
    if not conn:
        # current_app.logger.error(f"get_screen_definitions_for_app: Không thể lấy kết nối DB cho app {app_name}")
        print(f"get_screen_definitions_for_app: Không thể lấy kết nối DB cho app {app_name}") 
        return None
    ...
    try:
        ...
        # Query 1: Lấy các định nghĩa có activity_name cụ thể (nếu activity_name được cung cấp)
        if activity_name: # CHỈ chạy nếu activity_name được truyền vào và không phải None
            sql_with_activity = """
            ...
            """
            # current_app.logger.debug(f"Querying screen_defs for {app_name} AND activity {activity_name}")
            cur.execute(sql_with_activity, (app_name, activity_name))
            rows_with_activity = cur.fetchall()
            for row in rows_with_activity:
                screen_defs.append(dict(row))

        # Query 2: Luôn lấy các định nghĩa có activity_name là NULL cho app_name đó
        # Điều này đảm bảo các định nghĩa chung cho app luôn được xem xét.
        sql_null_activity = """
            SELECT definition_id, app_name, activity_name, logical_screen_name, 
                   defined_screen_id, identifying_elements_json, description
            FROM screen_definitions 
            ...
            ORDER BY logical_screen_name;
        """
        # current_app.logger.debug(f"Querying screen_defs for {app_name} AND NULL activity")
        cur.execute(sql_null_activity, (app_name,))
        rows_null_activity = cur.fetchall()
        for row_na in rows_null_activity:
            # Tránh thêm trùng nếu một định nghĩa (rất hiếm) có thể khớp cả hai query
            # (ví dụ, nếu activity_name được truyền vào là NULL)
            # Cách đơn giản là kiểm tra definition_id
            if not any(d['definition_id'] == row_na['definition_id'] for d in screen_defs):
                screen_defs.append(dict(row_na))
        else: # Nếu activity_name là None
            sql = """
                SELECT definition_id, app_name, activity_name, logical_screen_name, 
                        defined_screen_id, identifying_elements_json, description, created_at, updated_at
                FROM screen_definitions 
                WHERE app_name = %s 
                ORDER BY activity_name NULLS FIRST, logical_screen_name; 
            """
            params_tuple = (app_name,) # Đảm bảo app_name là một tuple cho execute
            print(f"DEBUG DB (get_screen_definitions_for_app - app_name only): SQL: {sql.strip()}")
            print(f"DEBUG DB (get_screen_definitions_for_app - app_name only): PARAMS: {params_tuple}")
            cur.execute(sql, params_tuple)
        # current_app.logger.debug(f"Found {len(screen_defs)} definitions for app '{app_name}' (activity filter: '{activity_name}')")
        return screen_defs
    except Exception as e:
