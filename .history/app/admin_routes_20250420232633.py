...
def add_device():
    ...
    if request.method == 'POST':
        if not db:
            flash("Lỗi DB.", "error")
            return render_template('admin_add_device.html', title=title, current_data=request.form), 500
        ...
        if not device_id:
            flash("Device ID là bắt buộc.", "warning")
            return render_template('admin_add_device.html', title=title + " (Lỗi)", current_data=request.form), 400
...
def edit_device(device_id):
    """Hiển thị form và xử lý sửa thông tin thiết bị (tên, ghi chú, trạng thái)."""
    if not db:
        flash("Lỗi DB.", "error")
        return redirect(url_for('admin.view_devices'))
    ...
    # Các trạng thái Admin có thể đặt (tùy chọn)
    ...
    if request.method == 'POST':
        ...
        try:
            success, error_msg = db.update_device_admin(
                ...
                device_name=device_name or None,
                notes=notes or None,
                status=status if status else None # Chỉ cập nhật nếu admin chọn status mới
            )
            if success:
