        ...
        with driver.session() as session:
            ...
            edge_query = """
                ...
                RETURN a.screen_id AS source,
                       ...
                       r.action_details AS action,  // <<< Giờ đây 'r' đã được định nghĩa
                       r.status AS status,          // <<< Giờ đây 'r' đã được định nghĩa
                       elementId(r) AS neo4j_edge_id // <<< Giờ đây 'r' đã được định nghĩa
            """
            edges_result = session.run(edge_query, app_name=app_name) # <<< Lỗi xảy ra ở đây sẽ hết
            edge_counter = 0
            for record in edges_result:
               # ... (phần xử lý kết quả cạnh giữ nguyên như phiên bản đúng trước đó) ...
               edge_counter += 1
