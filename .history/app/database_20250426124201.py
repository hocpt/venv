...
def add_new_template(_template_ref, description, category, first_variation_text):
    ...
    vào bảng 'template_variations'.

    Sử dụng ON CONFLICT để xử lý trường hợp template_ref đã tồn tại (chỉ cập nhật).
    Loại bỏ hoàn toàn việc tham chiếu đến bảng 'response_templates'.
    """
    conn = None
    try:
        conn = get_db_connection()
        if conn is None:
            raise Exception("Không thể kết nối tới CSDL.") # Thêm kiểm tra kết nối

        with conn.cursor() as cur:
            # === Bước 1: INSERT hoặc UPDATE vào bảng 'templates' ===
            # Bảng này lưu thông tin chính của template_ref
            sql_upsert_template = """
                ...
                ON CONFLICT (template_ref) DO UPDATE SET
                    ...
                    updated_at = NOW();
            """
            # Đảm bảo category là None nếu rỗng
            ...
            log_message(f"Executed UPSERT for templates: {_template_ref}", level='debug')

            # === Bước 2: INSERT variation đầu tiên vào 'template_variations' ===
            # Bảng này lưu các nội dung cụ thể của template
            # Khóa ngoại đã được sửa để trỏ đúng vào public.templates
            sql_insert_variation = """
                INSERT INTO public.template_variations (template_ref, variation_text, created_at)
                VALUES (%s, %s, NOW())
                ON CONFLICT (template_ref, variation_text) DO NOTHING; -- Tránh insert trùng variation
            """
            cur.execute(sql_insert_variation, (_template_ref, first_variation_text))
            log_message(f"Executed INSERT for template_variations: {_template_ref}", level='debug')

            conn.commit()
            log_message(f"Template '{_template_ref}' và variation đầu tiên đã được thêm/cập nhật thành công.")
            return True, f"Template '{_template_ref}' added/updated successfully."

    except psycopg2.Error as e:
        log_message(f"LỖI (database.py - add_new_template): INSERT/UPSERT thất bại: {e}\nTraceback: {traceback.format_exc()}", level='error')
        if conn:
            conn.rollback()
        # Trả về lỗi cụ thể hơn nếu là lỗi khóa chính/duy nhất
        if e.pgcode == psycopg2.errors.UNIQUE_VIOLATION:
             return False, f"Database error: Template Ref '{_template_ref}' already exists or variation text is duplicated."
        elif e.pgcode == psycopg2.errors.FOREIGN_KEY_VIOLATION:
             # Lỗi này không nên xảy ra nữa nếu FK đã đúng
             return False, f"Database error: Foreign key violation. Check if template_ref exists in templates."
        else:
            return False, f"Database error adding template: {e}"
    except Exception as ex: # Bắt các lỗi khác (ví dụ lỗi kết nối)
        log_message(f"LỖI KHÔNG XÁC ĐỊNH (database.py - add_new_template): {ex}\nTraceback: {traceback.format_exc()}", level='critical')
        if conn:
            conn.rollback()
