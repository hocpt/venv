...
def add_scheduled_job():
    """Hiển thị form và xử lý thêm Scheduled Job mới."""
    title = "Thêm Scheduled Job Mới"
    cancel_url = url_for('admin.view_scheduled_jobs')

    # === DANH SÁCH CÁC HÀM TÁC VỤ HỢP LỆ ===
    # Định nghĩa một list các tuple: (Tên hiển thị, 'module:function')
    # Lấy thông tin từ file app/background_tasks.py của bạn
    available_tasks = [
    ('Chạy Engine Đề xuất (Suggestion Engine)', 'app.background_tasks:run_suggestion_engine_task'), # Đúng
    ('Chạy Mô phỏng Hội thoại (Simulation)', 'app.background_tasks:run_simulation_task'), # Đúng
    # Thêm các hàm khác nếu có...
    ]
    # Sắp xếp theo tên hiển thị cho dễ chọn
    available_tasks.sort()
    # =========================================

    # Các loại trigger (có thể giữ nguyên hoặc lấy từ config)
    valid_trigger_types = ['interval', 'date', 'cron']

    if request.method == 'POST':
        # ... (Lấy dữ liệu form: job_id, function_path, trigger_type, ...) ...
        job_id = request.form.get('job_id', '').strip()
        job_function_path = request.form.get('job_function_path') # <<< Lấy từ select
        ...
        if not job_function_path: errors.append("Function Path là bắt buộc.") # <<< Kiểm tra giá trị từ select
        if not trigger_type: errors.append("Trigger Type là bắt buộc.")
        if not trigger_args_str: errors.append("Trigger Args là bắt buộc.")
        # ... (Validate JSON trigger_args_str như cũ) ...
        ...
        if errors:
            for error in errors: flash(error, "warning")
            # Render lại form, truyền lại available_tasks
            return render_template('admin_add_scheduled_job.html',
                                   title=title + " (Lỗi)", cancel_url=cancel_url,
                                   available_tasks=available_tasks, # <<< Truyền list task
                                   valid_trigger_types=valid_trigger_types,
                                   current_data=request.form), 400

        # Gọi hàm DB add_job_config
        try:
            success, error_msg = db.add_job_config(
                job_id=job_id,
                function_path=job_function_path, # <<< Dùng giá trị từ select
                trigger_type=trigger_type,
                trigger_args_str=trigger_args_str, # Hàm DB sẽ parse lại
                is_enabled=is_enabled,
                description=description or None
            )
            if success:
                flash(f"Đã thêm job '{job_id}' thành công.", "success")
                # Quan trọng: Thông báo cho scheduler biết có job mới/thay đổi
                if db: db.add_scheduler_command('reload_jobs', {}) # Gửi lệnh reload
                return redirect(cancel_url)
            else:
                flash(f"Thêm job '{job_id}' thất bại: {error_msg}", "error")
        except Exception as e:
            current_app.logger.error(f"Lỗi khi thêm scheduled job: {e}", exc_info=True)
            flash(f"Lỗi không mong muốn khi thêm job: {e}", "error")

        # Render lại form nếu lỗi DB/Exception
        return render_template('admin_add_scheduled_job.html',
    ...
    # --- GET request ---
    # Render form, truyền available_tasks
    return render_template('admin_add_scheduled_job.html',
...
def add_simulation_config_view(): # <<< Sử dụng đúng tên hàm bạn đã cung cấp
    """Hiển thị form thêm và xử lý lưu cấu hình mô phỏng mới."""
    title = "Thêm Cấu hình Mô phỏng AI"
    cancel_url = url_for('admin.view_ai_simulations') # URL quay lại trang danh sách simulation

    # --- Lấy dữ liệu cho các Dropdown ---
    all_personas = []
    all_accounts = []
    all_language_strategies = [] # Chỉ lấy language strategies

    if not db:
        flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
    else:
        try:
            # 1. Lấy tất cả Personas
            personas_data = db.get_all_personas() # Có thể trả về list, None hoặc tuple?
            if personas_data is None: flash("Lỗi tải danh sách Personas.", "warning"); all_personas = []
            elif isinstance(personas_data, (list, tuple)): # Chấp nhận cả list và tuple
                # === DÙNG sorted() ĐỂ SẮP XẾP AN TOÀN ===
                try:
                    all_personas = sorted(personas_data, key=lambda x: x.get('name', '').lower())
                except Exception as sort_err:
                     current_app.logger.error(f"Error sorting personas: {sort_err}", exc_info=True)
                     all_personas = list(personas_data) # Chuyển thành list nếu sort lỗi
            else:
                 all_personas = [] # Reset nếu kiểu không đúng

            # 2. Lấy tất cả Accounts
            accounts_data = db.get_all_accounts()
            if accounts_data is None: flash("Lỗi tải danh sách Accounts.", "warning"); all_accounts = []
            elif isinstance(accounts_data, (list, tuple)):
                 # === DÙNG sorted() ĐỂ SẮP XẾP AN TOÀN ===
                 try:
                     all_accounts = sorted(accounts_data, key=lambda x: x.get('username', x.get('account_id', '')).lower())
                 except Exception as sort_err:
                      current_app.logger.error(f"Error sorting accounts: {sort_err}", exc_info=True)
                      all_accounts = list(accounts_data) # Chuyển thành list nếu sort lỗi
            else:
                  all_accounts = [] # Reset nếu kiểu không đúng

            # 3. Lấy tất cả Language Strategies và sắp xếp
            raw_lang_strategies = db.get_all_strategies(strategy_type_filter='language')
            if raw_lang_strategies is None:
                flash("Lỗi tải danh sách Language Strategies.", "error"); all_language_strategies = []
            elif isinstance(raw_lang_strategies, list):
                 try:
                     all_language_strategies = sorted(raw_lang_strategies, key=lambda x: x.get('strategy_id', ''))
                 except Exception as sort_err:
                      current_app.logger.error(f"Error sorting language strategies: {sort_err}", exc_info=True)
                      all_language_strategies = raw_lang_strategies
            else:
                 all_language_strategies = []

        except Exception as e:
            current_app.logger.error(f"Lỗi tải dữ liệu cho form add_simulation_config_view: {e}", exc_info=True)
            flash("Lỗi không mong muốn khi tải dữ liệu cho form.", "error")
            all_personas, all_accounts, all_language_strategies = [], [], [] # Reset về rỗng

    # --- Xử lý POST (Logic POST giữ nguyên như model_58) ---
    if request.method == 'POST':
        current_data = request.form.to_dict()
        try:
            # ... (Lấy dữ liệu form: config_name, persona_a_id, ...) ...
            ...
            strategy_id = request.form.get('strategy_id')
            iterations_str = request.form.get('iterations', '1').strip()
            ...
            if not persona_a_id: errors.append("Vui lòng chọn Persona A.")
            if persona_a_id and persona_a_id == persona_b_id: errors.append("Persona A và Persona B không được giống nhau.")
            if account_log_a_id and account_log_a_id == account_log_b_id: errors.append("Account Log cho A và B không được giống nhau.")
            if not strategy_id: errors.append("Vui lòng chọn Chiến lược Language.")
            iterations = 1
            try:
                iterations = int(iterations_str)
                if not (1 <= iterations <= 100): raise ValueError("Số lượt phải từ 1 đến 100.")
            except ValueError as e: errors.append(f"Số lần lặp không hợp lệ: {e}")

            if errors: # Nếu có lỗi validation
                for error in errors: flash(error, 'warning')
                # Render lại form, truyền các list ĐÃ SẮP XẾP
            ...
            # --- Gọi hàm DB để thêm mới (như cũ) ---
            success, error_msg = db.add_simulation_config(
                 config_name=config_name, description=description or None,
                 persona_a_id=persona_a_id, persona_b_id=persona_b_id if persona_b_id else None,
                 log_account_id_a=account_log_a_id if account_log_a_id else None,
                 log_account_id_b=account_log_b_id if account_log_b_id else None,
                 strategy_id=strategy_id, max_turns=iterations,
            ...
            if success:
                ...
                flash(f"Thêm cấu hình '{config_name}' thất bại! {error_msg or '(Lỗi CSDL)'}", 'error')
                # Render lại form với lỗi DB, truyền list ĐÃ SẮP XẾP
                return render_template('admin_add_simulation_config.html',
            ...
            current_app.logger.error(f"Lỗi nghiêm trọng khi thêm cấu hình mô phỏng: {e}", exc_info=True)
            flash(f"Đã xảy ra lỗi không mong muốn: {e}", "error")
            # Render lại form với lỗi Exception, truyền list ĐÃ SẮP XẾP
            return render_template('admin_add_simulation_config.html',
    ...
    # --- GET request ---
    # Render form, truyền các list ĐÃ SẮP XẾP cho dropdowns
    return render_template('admin_add_simulation_config.html',
                           title=title, cancel_url=cancel_url,
...
@admin_bp.route('/simulations/configs/add', methods=['GET', 'POST'])
def add_simulation_config():
    """Hiển thị form và xử lý thêm mới Simulation Config."""
    title = "Thêm Cấu hình Mô phỏng Mới"
    cancel_url = url_for('admin.view_ai_simulations') # URL quay lại trang danh sách simulation
    ...
    if not db:
        ...
        try:
            # 1. Lấy tất cả Personas (Giả sử có hàm db.get_all_personas)
            all_personas = db.get_all_personas() or []
            if all_personas is None: flash("Lỗi tải danh sách Personas.", "warning"); all_personas = []
            # Sắp xếp personas theo ID hoặc tên
            all_personas.sort(key=lambda x: x.get('persona_id', ''))

            # 2. Lấy tất cả Accounts (Giả sử có hàm db.get_all_accounts)
            # Hàm này cần trả về ít nhất account_id và username/display_name
            all_accounts = db.get_all_accounts() or []
            if all_accounts is None: flash("Lỗi tải danh sách Accounts.", "warning"); all_accounts = []
             # Sắp xếp accounts theo ID hoặc tên
            all_accounts.sort(key=lambda x: x.get('account_id', ''))

            # 3. Lấy tất cả Language Strategies và sắp xếp
            raw_lang_strategies = db.get_all_strategies(strategy_type_filter='language')
            if raw_lang_strategies is None:
                flash("Lỗi tải danh sách Language Strategies.", "error")
                all_language_strategies = []
            elif isinstance(raw_lang_strategies, list):
                 try:
                     all_language_strategies = sorted(raw_lang_strategies, key=lambda x: x.get('strategy_id', ''))
                     ...
                     current_app.logger.error(f"Error sorting language strategies: {sort_err}", exc_info=True)
                     all_language_strategies = raw_lang_strategies # Dùng list chưa sort nếu lỗi
            else:
                 all_language_strategies = []

        except Exception as e:
            current_app.logger.error(f"Lỗi tải dữ liệu cho form add_simulation_config: {e}", exc_info=True)
            flash("Lỗi không mong muốn khi tải dữ liệu cho form.", "error")
            all_personas, all_accounts, all_language_strategies = [], [], [] # Reset về rỗng
    ...
    if request.method == 'POST':
        ...
        # Validate
        if not name: errors.append("Tên cấu hình là bắt buộc.")
        if not persona_a_id: errors.append("Persona A là bắt buộc.")
        if not strategy_id: errors.append("Chiến lược Áp dụng là bắt buộc.")
        # Thêm validate cho persona_b_id, account logs nếu cần
        iterations = 1
        try:
            iterations = int(iterations_str)
            if iterations < 1: errors.append("Số lần lặp phải lớn hơn 0.")
        except ValueError: errors.append("Số lần lặp phải là số nguyên.")

        if persona_a_id and persona_a_id == persona_b_id:
             errors.append("Persona A và Persona B không được giống nhau.")
        if account_log_a_id and account_log_a_id == account_log_b_id:
             errors.append("Account Log cho A và B không được giống nhau (nếu cả hai được chọn).")


        if errors:
            for error in errors: flash(error, "warning")
            # Render lại form với lỗi, truyền lại các list đã lấy
            return render_template('admin_add_simulation_config.html',
                                   title=title + " (Lỗi)", cancel_url=cancel_url,
                                   personas=all_personas, # <<< Truyền list personas
                                   accounts=all_accounts, # <<< Truyền list accounts
                                   strategies=all_language_strategies, # <<< Truyền list strategies đã lọc và sort
                                   current_data=request.form), 400

        # Gọi hàm DB để thêm mới (Giả sử có hàm db.add_simulation_config)
        try:
            # Hàm DB cần nhận các ID đã chọn
            success, error_msg = db.add_simulation_config(
                name=name,
                description=description or None,
                persona_a_id=persona_a_id,
                persona_b_id=persona_b_id if persona_b_id else None, # Cho phép B là optional?
                account_log_a_id=account_log_a_id if account_log_a_id else None, # Log A optional?
                account_log_b_id=account_log_b_id if account_log_b_id else None, # Log B optional?
                strategy_id=strategy_id,
            ...
            if success:
                flash(f"Thêm cấu hình mô phỏng '{name}' thành công!", "success")
                return redirect(cancel_url) # Redirect về trang danh sách simulation
            else:
                flash(f"Thêm cấu hình mô phỏng thất bại: {error_msg or 'Lỗi không xác định.'}", "error")

        except Exception as e:
            current_app.logger.error(f"Lỗi khi thêm simulation config: {e}", exc_info=True)
            flash(f"Lỗi không mong muốn khi thêm cấu hình: {e}", "error")

        # Render lại form nếu lỗi DB/Exception, truyền lại các list
        return render_template('admin_add_simulation_config.html',
                               title=title + " (Lỗi DB/Exception)", cancel_url=cancel_url,
                               personas=all_personas, # <<< Truyền list personas
                               accounts=all_accounts, # <<< Truyền list accounts
                               strategies=all_language_strategies, # <<< Truyền list strategies
                               current_data=request.form)


    # --- GET request ---
    # Render form, truyền các list đã lấy cho dropdowns
    return render_template('admin_add_simulation_config.html',
                           title=title, cancel_url=cancel_url,
                           personas=all_personas, # <<< Truyền list personas
                           accounts=all_accounts, # <<< Truyền list accounts
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            return render_template('admin_add_transition_control.html',
                                   ...
                                   all_stages=all_control_stages,      # Dùng list control stages
...
def cancel_task_assignment(assignment_id):
    ...
    try:
        ...
        if success:
            ...
            flash(f"Hủy Task Assignment thất bại: {error_msg or 'ID không tồn tại hoặc lỗi CSDL.'}", 'error')
