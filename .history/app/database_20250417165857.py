...
# --- HÀM UPDATE STRATEGY ĐÃ SỬA (CHỈ NHẬN 4 THAM SỐ) ---
def update_strategy(strategy_id: str, name: str, description: str | None, initial_stage_id: str) -> tuple[bool, str | None]:
    ...
    Args:
        ...
        name: Tên mới.
        description: Mô tả mới (có thể None).
        initial_stage_id: Stage ID bắt đầu mới.

    Returns:
        Tuple (bool, str | None): (True nếu thành công, None) hoặc (False, thông báo lỗi).
    """
    print(f"DEBUG (db.update_strategy): Attempting to update strategy '{strategy_id}' (excluding type)") # Log debug
    ...
    try:
        ...
        print(f"DEBUG SQL (update_strategy): {cur.mogrify(sql, params).decode('utf-8','ignore')}") # Log SQL
        cur.execute(sql, params)
        conn.commit()
        ...
        if not success:
         ...
         # Chỉ có thể là lỗi UNIQUE của name nếu bạn có ràng buộc đó
