...
def admin_screen_elements(screen_id):
    ...
    if not screen_data:
        flash(f"Screen node with ID '{screen_id}' not found.", 'warning')
        # Chuyển hướng về trang nào đó, ví dụ trang mapping viewer hoặc index admin
        return redirect(url_for('admin.index')) # Hoặc trang khác

    # Truyền danh sách các loại classification hợp lệ vào template
    valid_classifications = ai_service.VALID_CLASSIFICATIONS

    # Lấy đường dẫn ảnh chụp màn hình nếu có (giả sử lưu trong thuộc tính 'screenshot_path')
