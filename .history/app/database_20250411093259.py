...
def add_suggestion(keywords: str, template_text: str, source_examples: dict) -> bool:
    """
    Lưu một đề xuất mới (keywords, template) vào bảng suggested_rules.

    Args:
        keywords: Chuỗi keywords do AI đề xuất.
        template_text: Chuỗi template text do AI đề xuất.
        source_examples: Dictionary chứa thông tin nguồn gốc (ví dụ: {'history_ids': [1, 2]}).
                         Sẽ được lưu vào cột JSONB.

    Returns:
        True nếu lưu thành công, False nếu thất bại.
    """
    if not keywords or not template_text:
        print("WARNING (add_suggestion): Keywords và Template Text không được rỗng.")
        return False
    if source_examples is None:
        source_examples = {} # Đảm bảo là dict rỗng nếu không có nguồn
    ...
    try:
        ...
        # Chuyển đổi dictionary source_examples thành chuỗi JSON
        # để lưu vào cột kiểu JSONB trong PostgreSQL
        try:
             source_examples_json = json.dumps(source_examples)
        except TypeError as json_err:
             print(f"LỖI (add_suggestion): Không thể chuyển đổi source_examples thành JSON: {json_err}")
             # Có thể gán giá trị mặc định hoặc báo lỗi tùy logic
             source_examples_json = json.dumps({'error': 'Serialization failed'})

        print(f"DEBUG (add_suggestion): Lưu đề xuất: keywords='{keywords[:50]}...', template='{template_text[:50]}...'")
        # suggestion_id, status, created_at sẽ dùng giá trị DEFAULT từ DB schema
        sql = """
            INSERT INTO suggested_rules (suggested_keywords, suggested_template_text, source_examples)
            VALUES (%s, %s, %s);
        """
        params = (keywords, template_text, source_examples_json)
        cur.execute(sql, params)
        conn.commit()
        success = True
