...
def view_rules():
    ...
    active_filters = {} # Dùng để giữ lại giá trị filter trên form
    ...
    if not db:
        ...
        try:
            ...
            # Tạo dict chứa các filter đang được áp dụng
            active_filters = {k.replace('filter_', ''): v for k, v in request.args.items() if v and k.startswith('filter_')}
            print(f"DEBUG (view_rules): Active Filters = {active_filters}")

            # Gọi hàm lấy luật đã lọc từ DB
            rules = db.get_filtered_rules(filters=active_filters)
            if rules is None:
                 flash("Lỗi khi tải danh sách luật từ CSDL.", "error"); rules = []
            print(f"DEBUG (view_rules): Fetched {len(rules)} rules after filtering.")

            # Lấy danh sách cho dropdown filter
            distinct_categories = db.get_distinct_rule_categories() or []
            distinct_template_refs = db.get_distinct_rule_template_refs() or []

            # --- Logic tìm kiếm keywords trùng lặp (giữ nguyên) ---
            if rules:
                ...
                duplicate_keywords = {kw_tuple for kw_tuple, count in keyword_counts.items() if count > 1}
                for rule in rules:
                    normalized_keywords = tuple(sorted([k.strip().lower() for k in rule.get('trigger_keywords', '').split(',') if k.strip()]))
                    if normalized_keywords in duplicate_keywords:
                        duplicate_keyword_rule_ids.add(rule.get('rule_id'))
                print(f"DEBUG (view_rules): Found {len(duplicate_keyword_rule_ids)} rules with duplicate keywords.")
            # --- Kết thúc logic tìm trùng lặp ---

        except Exception as e:
            print(f"Lỗi nghiêm trọng khi load rules: {e}")
            flash("Đã xảy ra lỗi không mong muốn khi tải danh sách luật.", "error")
            rules = []; duplicate_keyword_rule_ids = set(); distinct_categories = []; distinct_template_refs = []
    ...
    return render_template('admin_rules.html',
                           ...
                           distinct_categories=distinct_categories,      # <<< Cho filter
                           distinct_template_refs=distinct_template_refs, # <<< Cho filter
...
def view_templates():
    ...
    templates_data = []
    distinct_categories = []
    active_filters = {} # Lưu lại filter đang dùng để hiển thị trên form
    ...
    if not db:
        ...
        try:
            ...
            filter_category = request.args.get('filter_category', '').strip()
            active_filters = {k: v for k, v in request.args.items() if v} # Chỉ lấy filter có giá trị

            # Gọi hàm DB mới với các filter
            templates_data = db.get_filtered_templates_with_details(
            ...
            # Lấy danh sách category duy nhất cho dropdown lọc
    ...
    # Truyền dữ liệu đã lọc, các category, và filter đang dùng vào template
    return render_template('admin_templates.html',
                           title=title,
                           templates=templates_data, # <<< Danh sách đã lọc
                           distinct_categories=distinct_categories, # <<< Cho dropdown filter
...
def view_ai_simulations():
    """Hiển thị trang quản lý mô phỏng AI với thông tin config chi tiết hơn."""
    # ... (khởi tạo biến như cũ) ...
    title = "Quản lý Mô phỏng AI"
    personas, strategies, accounts, saved_configs = [], [], [], []
    simulations_display_list = []

    if not db: flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
    else:
        try:
            # Lấy dữ liệu cho dropdowns và cấu hình đã lưu
            personas = db.get_all_personas() or []
            strategies = db.get_all_strategies() or []
            accounts = db.get_all_accounts() or []
            saved_configs = db.get_all_simulation_configs() or []

            # === TẠO DANH SÁCH HIỂN THỊ CÁC LẦN CHẠY/LỆNH ===
            server_tz = _get_configured_timezone()

            # 1. Lấy các Lệnh gần đây (bao gồm cả done để lấy payload)
            recent_commands = db.get_recent_simulation_commands(
                status_list=['pending', 'processing', 'error', 'done'], # <<< Lấy cả 'done'
                command_type='run_simulation',
                limit=30 # Tăng nhẹ limit nếu cần
            ) or []
            # Tạo map để tra cứu command dễ dàng bằng command_id
            command_map = {cmd['command_id']: cmd for cmd in recent_commands}
            print(f"DEBUG: Found {len(recent_commands)} recent commands (incl. done).")

            # 2. Lấy các Job mô phỏng đang được lên lịch
            live_job_times = _get_live_next_run_times() or {}
            ...
            # 3. Tạo danh sách hiển thị - Ưu tiên thông tin từ Commands
            for cmd_id, command_data in command_map.items():
                payload = command_data.get('payload', {})
                status = command_data.get('status', 'unknown')
                status_text = status.capitalize()
                config_info_str = f"{payload.get('persona_a_id','?')} vs {payload.get('persona_b_id','?')} ({payload.get('strategy_id','?')})"

                sim_info = {
                    'id': f"cmd_{cmd_id}",
                    'type': 'command',
                    'command_id': cmd_id, # Thêm command_id gốc
                    'job_id': None, # Sẽ cập nhật nếu tìm thấy job live
                    'config_info': config_info_str,
                    'status_text': status_text,
                    'created_at': command_data.get('created_at'),
                    'next_run_time_str': '---',
                    'error_message': command_data.get('error_message') # Thêm error message
                }

                # Chuẩn hóa status text hiển thị
                if status == 'pending': status_text = 'Pending Queue'
                elif status == 'processing': status_text = 'Processing Cmd'
                elif status == 'done': status_text = 'Command Done' # Trạng thái lệnh là Done
                elif status == 'error': status_text = f"Command Error: {sim_info['error_message'][:100]}" if sim_info['error_message'] else 'Command Error'
                sim_info['status_text'] = status_text

                # Tìm xem có live job nào tương ứng với command này không
                # (Dựa vào quy tắc đặt tên job_id = f"sim_run_{command_id}_{uuid}")
                corresponding_job_id = None
                for live_job_id in live_sim_jobs_dict.keys():
                     parts = live_job_id.split('_')
                     if len(parts) >= 3 and parts[0] == 'sim' and parts[1] == 'run':
                          try:
                               if int(parts[2]) == cmd_id:
                                    corresponding_job_id = live_job_id
                                    break
                          except ValueError: pass

                if corresponding_job_id:
                    sim_info['job_id'] = corresponding_job_id
                    sim_info['type'] = 'job' # Ưu tiên hiển thị là job nếu tìm thấy
                    processed_command_ids.add(cmd_id) # Đánh dấu đã xử lý
                    next_run_timestamp = live_sim_jobs_dict[corresponding_job_id]
                    if next_run_timestamp is not None:
                        try:
                            utc_dt = datetime.fromtimestamp(next_run_timestamp, tz=timezone.utc)
                            local_dt = utc_dt.astimezone(server_tz)
                            sim_info['next_run_time_str'] = local_dt.strftime('%Y-%m-%d %H:%M:%S %z')
                            sim_info['status_text'] = 'Scheduled' # Cập nhật trạng thái thành Scheduled
                        except Exception: sim_info['next_run_time_str'] = 'Lỗi Format'
                    else:
                        # Job tồn tại nhưng next_run_time là None -> Paused? Hoặc vừa chạy xong?
                        sim_info['next_run_time_str'] = '---'
                        if status == 'done': # Nếu command đã done và job không có next run -> Coi như Completed
                             sim_info['status_text'] = 'Job Completed/Removed'
                        else:
                             sim_info['status_text'] = 'Job Paused?' # Ít khả năng xảy ra với job chạy 1 lần


                simulations_display_list.append(sim_info)

            # 4. (Tùy chọn) Thêm các live job không tìm thấy command tương ứng (có thể là job cũ)
            # for live_job_id, next_run_timestamp in live_sim_jobs_dict.items():
            #      parts = live_job_id.split('_'); command_id_from_job = None
            #      if len(parts) >= 3 and parts[0] == 'sim' and parts[1] == 'run':
            #           try: command_id_from_job = int(parts[2])
            #           except ValueError: pass
            #      if command_id_from_job not in processed_command_ids: # Chỉ thêm nếu chưa xử lý
            #           # ... tạo sim_info cho job không rõ nguồn gốc ...
            #           simulations_display_list.append(sim_info)


            # 5. Sắp xếp
            simulations_display_list.sort(key=lambda x: x.get('created_at') or datetime.now(timezone.utc), reverse=True)
