...
import threading # Thêm timedelta nếu chưa có
...
def load_scheduled_jobs(app):
        ...
        try:
            ...
            for job_config in job_configs:
                ...
                try:
                    ...
                    scheduler.add_job(
                        id='test_hello_job',
                        func=lambda: print(f"[TEST_JOB] Hello at {datetime.now()}"),
                        trigger='interval',
                        seconds=10,
    ...
    if job:
        print(f"✅ Job {job.id} sẽ chạy lúc: {job.next_run_time}")
    else:
        print("❌ Job không tồn tại trong scheduler.")    
...
def create_app(config_class=config.Config):
    ...
    try:
        ...
        with app.app_context():
            scheduler.start()
            print("✅ APScheduler started (final test)")
    ...
    start_scheduler_after_app_ready(app)
    return app

def test_job():
    from datetime import datetime
    print(f"[TEST_JOB] Hello at {datetime.now()}")
def start_scheduler_after_app_ready(app):
    with app.app_context():
        scheduler.start()
        print("✅ APScheduler started inside app context.")
