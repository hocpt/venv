...
def add_transition_control():
    """Hiển thị form và xử lý thêm Transition mới cho Control Strategy.
       Đã thêm sắp xếp stages ở backend.
    """
    # ... (Lấy strategy_id, current_stage_id_prefill như cũ) ...
    strategy_id = request.args.get('strategy_id') or request.form.get('strategy_id')
    ...
    try:
        ...
        if raw_strategy_stages is None:
            ...
            try:
             ...
             strategy_stages_for_dropdowns = [] # Gán list rỗng nếu kiểu trả về không đúng

        # === KẾT THÚC SẮP XẾP STAGES ===
        ...
        flash(f"Lỗi tải dữ liệu cho form: {e}", "error")
        strategy_stages_for_dropdowns = [] # Đảm bảo là list rỗng khi lỗi

    # Định nghĩa valid intents/types (như đã sửa ở model_39)
    valid_intents=['any', 'on_stage_entry', 'element_clicked', ...]
    valid_condition_types=['element_visible', 'variable_check', ...]

    title = f"Thêm Control Transition cho Strategy {strategy_id}"

    # --- Xử lý POST (logic validate và gọi DB giữ nguyên) ---
    if request.method == 'POST':
        # ... (Code xử lý POST giữ nguyên như ở model_39) ...
        # Lưu ý: Khi render lại form lỗi, vẫn truyền list đã sắp xếp
        ...
        # Gọi DB...
        try:
            success, error_msg = db.add_new_transition(...) # Gọi DB như cũ
            if success: return redirect(cancel_url)
            else:
                # ... (flash lỗi DB) ...
                return render_template('admin_add_transition_control.html',
    ...
    # --- GET request ---
    # Render template, truyền cùng list stages đã sắp xếp cho cả hai dropdown
    return render_template('admin_add_transition_control.html',
