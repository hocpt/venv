...
def get_all_macro_definitions(app_target: str | None = None, include_generic: bool = True) -> list[dict] | None:
    ...
    try:
        ...
        if include_generic:
            ...
            # Thêm '' và NULL nếu bạn dùng chúng thay vì 'system'/'generic'
            # filter_targets.extend(['system', 'generic', '', None])

        if filter_targets:
             # Xử lý trường hợp có target cụ thể và cả generic/system
             # Cần đảm bảo query đúng nếu app_target có thể là NULL hoặc ''
             target_conditions = []
             valid_targets_in_params = [] # Chỉ thêm vào params nếu không phải None hoặc ''
             for target in filter_targets:
                 if target is None:
                     target_conditions.append("app_target IS NULL")
                 elif target == '':
                      target_conditions.append("app_target = ''")
                 else:
                      target_conditions.append("app_target = %s")
                      valid_targets_in_params.append(target) # Thêm vào list để truyền cho execute
             where_clauses.append(f"({ ' OR '.join(target_conditions) })")
             params.extend(valid_targets_in_params) # Chỉ truyền các giá trị hợp lệ

        if where_clauses:
            sql = f"{base_sql} WHERE {' AND '.join(where_clauses)} ORDER BY app_target, macro_code;"
        else:
            sql = f"{base_sql} ORDER BY app_target, macro_code;"
        print(f"DEBUG SQL (get_all_macros): {cur.mogrify(sql, tuple(params)).decode('utf-8', 'ignore')}")
        # print(f"DEBUG SQL (get_all_macros): {cur.mogrify(sql, tuple(params)).decode('utf-8', 'ignore')}") # Bỏ comment để debug SQL
        cur.execute(sql, tuple(params))
        rows = cur.fetchall()
        print(f"DEBUG DB (get_all_macros): Fetched {len(rows)} rows.")
