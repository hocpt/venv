...
def get_screen_definitions_for_app(app_name: str, activity_name: str | None = None) -> list[dict] | None:
    conn = get_db_connection() # Lấy kết nối mới
    if not conn:
        # current_app.logger.error(f"get_screen_definitions_for_app: Không thể lấy kết nối DB cho app {app_name}")
        print(f"get_screen_definitions_for_app: Không thể lấy kết nối DB cho app {app_name}") # Fallback log
        return None
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # ... (logic query SQL của bạn ở đây, như đã cung cấp trước đó)
        # Query 1: Ưu tiên có activity_name
        if activity_name:
            sql = """
        ...
        # Query 2: Lấy các định nghĩa có activity_name là NULL cho app_name đó
        # (Nên chạy query này bất kể query 1 có kết quả hay không, để gộp cả hai loại)
        sql_null_activity = """
            SELECT definition_id, app_name, activity_name, logical_screen_name, 
                   defined_screen_id, identifying_elements_json, description
            FROM screen_definitions 
        ...
        for row_na in rows_null_activity:
            # Tránh thêm trùng nếu query 1 đã lấy được (ít khả năng nếu logic đúng)
            # Hoặc bạn có thể dùng UNION trong SQL nếu muốn
        ...
        print(f"Lỗi khi lấy screen_definitions cho app {app_name}: {e}") # Fallback log
        import traceback
        traceback.print_exc()
        return None # Trả về None khi có lỗi
    finally:
        if cur: cur.close()
        if conn: conn.close() # <<<< ĐÓNG KẾT NỐI TRONG FINALLY
