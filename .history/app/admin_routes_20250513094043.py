...
@admin_bp.route('/api/mapping/management/nodes', methods=['GET'])
# @admin_required
def api_get_managed_nodes():
    logger = current_app.logger
    try:
        app_name_filter = request.args.get('app_name_filter') # Sẽ là None nếu không có
        filter_status = request.args.get('filter_status', 'unknown') # Mặc định 'unknown'
        page = request.args.get('page', 1, type=int)
        if page < 1: page = 1
        # PER_PAGE_NODE_ISSUES nên được định nghĩa ở đầu file hoặc config
        per_page_val = current_app.config.get('PER_PAGE_NODE_ISSUES', 15)

        logger.debug(f"API get_managed_nodes: app_filter='{app_name_filter}', status_filter='{filter_status}', page={page}")

        nodes_page_data, total_items = graph_db.get_screen_nodes_for_management(
            app_name=app_name_filter if app_name_filter else None,
            filter_status=filter_status,
            page=page,
            per_page=per_page_val
        )

        if nodes_page_data is None: # Lỗi từ graph_db
            logger.error(f"API get_managed_nodes: graph_db.get_screen_nodes_for_management trả về None.")
            return jsonify({"error": "Lỗi tải dữ liệu Node từ cơ sở dữ liệu."}), 500

        total_pages_val = 0
        if total_items is not None and total_items > 0 and per_page_val > 0:
            total_pages_val = math.ceil(total_items / per_page_val)
        
        # Đảm bảo page không vượt quá total_pages (nếu có total_pages > 0)
        if total_pages_val > 0 and page > total_pages_val:
            logger.warning(f"API get_managed_nodes: page {page} vượt quá total_pages {total_pages_val}. Sẽ trả về trang cuối.")
            # Có thể quyết định trả lỗi hoặc trang cuối. Hiện tại cứ để server trả dữ liệu (sẽ là list rỗng nếu vượt quá).
            # Hoặc gọi lại graph_db.get_screen_nodes_for_management với page = total_pages_val
            # Tạm thời: nếu page quá lớn, nodes_page_data từ graph_db có thể là []
        ...
        return jsonify({
            ...
            "pagination": {
                ...
                "has_prev": page > 1 and total_pages_val > 0, # Chỉ có prev nếu không phải trang 1 và có trang
