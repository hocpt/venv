...
def get_all_template_refs() -> list[dict] | None:
    ...
    try:
        ...
        # === SỬA TÊN BẢNG Ở ĐÂY ===
        cur.execute("SELECT template_ref FROM public.templates ORDER BY template_ref;") -- <<< SỬA Ở ĐÂY
        # =========================
        rows = cur.fetchall()
        if rows:
...
# Trong file: hpt2/app/database.py
...
def get_filtered_rules(filters: dict = None, page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    ...
    # --- Xây dựng mệnh đề WHERE và params (giữ nguyên) ---
    where_clauses = []
    params = []
    if filters:
        ...
        # Thêm filter theo strategy_id nếu cần
    ...
    try:
        ...
        cur = conn.cursor()
        # === SỬA TÊN BẢNG Ở ĐÂY ===
        count_sql = f"SELECT COUNT(*) FROM public.rules {where_sql};" # <<< SỬA Ở ĐÂY
        # =========================
        cur.execute(count_sql, tuple(params))
        total_items = cur.fetchone()[0]
        cur.close()
        ...
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        offset = (page - 1) * per_page
        # === SỬA TÊN BẢNG VÀ THÊM strategy_id VÀO SELECT ===
        data_sql = f"""
            SELECT rule_id, strategy_id, trigger_keywords, category, response_template_ref, priority, notes
            FROM public.rules           -- <<< SỬA Ở ĐÂY
            {where_sql}
...
def update_rule(rule_id: int, strategy_id: str, keywords: str, category: str | None, template_ref: str | None, priority: int, notes: str | None) -> bool:
    """Cập nhật một luật trong bảng 'rules'."""
    if not rule_id or not strategy_id or not keywords: # Thêm kiểm tra strategy_id
        return False
    ...
    try:
        ...
        # === SỬA TÊN BẢNG VÀ THÊM strategy_id VÀO SET ===
        sql = """
            UPDATE public.rules           -- <<< SỬA Ở ĐÂY
            SET strategy_id = %s, trigger_keywords = %s, category = %s, response_template_ref = %s, priority = %s, notes = %s, updated_at = NOW()
            WHERE rule_id = %s;
        """
        params = (strategy_id, keywords, category, template_ref, priority, notes, rule_id)
        # ===============================================
        cur.execute(sql, params)
        conn.commit()
        success = cur.rowcount > 0
...
def delete_rule(rule_id: int) -> bool:
    ...
    try:
        ...
        # === SỬA TÊN BẢNG Ở ĐÂY ===
        sql = "DELETE FROM public.rules WHERE rule_id = %s;" # <<< SỬA Ở ĐÂY
        # =========================
...
def get_distinct_rule_categories() -> list[str] | None:
    ...
    try:
        cur = conn.cursor()
        # === SỬA TÊN BẢNG Ở ĐÂY ===
        sql = "SELECT DISTINCT category FROM public.rules WHERE category IS NOT NULL AND category <> '' ORDER BY category;" # <<< SỬA Ở ĐÂY
        # =========================
        cur.execute(sql)
        rows = cur.fetchall()
...
def get_distinct_rule_template_refs() -> list[str] | None:
    ...
    try:
        cur = conn.cursor()
        # === SỬA TÊN BẢNG Ở ĐÂY ===
        sql = "SELECT DISTINCT response_template_ref FROM public.rules WHERE response_template_ref IS NOT NULL ORDER BY response_template_ref;" # <<< SỬA Ở ĐÂY + THÊM IS NOT NULL
        # =========================
        cur.execute(sql)
        rows = cur.fetchall()
...
# Hàm get_all_rules không còn cần thiết vì get_filtered_rules đã bao hàm
# def get_all_rules() -> list[dict] | None:
#     """Lấy tất cả luật (gọi hàm lọc không có filter)."""
#     print("DEBUG (database.py - get_all_rules): Calling get_filtered_rules with no filters.")
#     # Gọi hàm lọc với page=1 và per_page rất lớn để lấy hết (không hiệu quả lắm)
#     rules, total = get_filtered_rules(filters=None, page=1, per_page=10000)
#     return rules
