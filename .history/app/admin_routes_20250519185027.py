...
def view_node_management():
    ...
    logger.info("VIEW: Accessing Node Management page - Initial Load")

    distinct_app_names_for_filter = [] 
    try:
        conn = db.get_db_connection() # Sử dụng module db (database.py) cho PostgreSQL
        if conn:
            try:
                # Giả sử bạn dùng DictCursor hoặc có cách lấy tên cột
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur: 
                    cur.execute("SELECT DISTINCT app_name FROM screen_definitions WHERE app_name IS NOT NULL ORDER BY app_name;")
                    distinct_app_names_for_filter = [row['app_name'] for row in cur.fetchall()]
            except Exception as e_db_apps:
                logger.error(f"VIEW: Lỗi khi lấy distinct app_names cho filter: {e_db_apps}", exc_info=True)
                flash("Lỗi tải danh sách ứng dụng cho bộ lọc.", "warning")
            finally:
                conn.close()
        else:
            logger.error("VIEW: Không thể kết nối PostgreSQL để lấy app_names cho filter.")
            flash("Lỗi kết nối cơ sở dữ liệu (PostgreSQL).", "error")
    except Exception as e_conn:
        logger.error(f"VIEW: Lỗi kết nối PostgreSQL ban đầu: {e_conn}", exc_info=True)
        flash("Lỗi kết nối cơ sở dữ liệu nghiêm trọng.", "error")
    ...
    logger.debug(f"VIEW: Filters - App: '{app_name_query_param}', Status: '{filter_status}', Page: {page}")
    ...
    if raw_nodes_page_data is None: # Lỗi từ graph_db
        ...
        logger.error("VIEW: graph_db.get_screen_nodes_for_management trả về None.")
    else:
        # **QUAN TRỌNG: Xử lý tạo screenshot_full_url ở đây cho lần render đầu**
        for node_neo4j_data in raw_nodes_page_data: # node_neo4j_data là dict
            node_processed_data = dict(node_neo4j_data) # Tạo bản sao để thêm key mới
            internal_id = node_neo4j_data.get('node_neo4j_id') # HOẶC 'id' nếu bạn dùng id(n)
            logger.debug(f"Processing node for template: screen_id='{node_neo4j_data.get('screen_id')}', neo4j_internal_id='{internal_id}'")

            if not internal_id: # Nếu ID này rỗng hoặc None
                logger.error(f"LỖI NGHIÊM TRỌNG: Node với screen_id '{node_neo4j_data.get('screen_id')}' không có ID nội bộ Neo4j!")
                # Bạn có thể gán một giá trị placeholder hoặc bỏ qua node này,
                # nhưng tốt nhất là tìm hiểu tại sao nó rỗng từ graph_db.py

            # Gán ID này vào dict sẽ truyền cho template dưới một key nhất quán, ví dụ 'node_db_id'
            node_processed_data['node_db_id_for_html'] = internal_id 
            # `screenshot_path` từ Neo4j là CHỈ TÊN FILE
            # `app_name` cũng từ Neo4j
            node_processed_data['screenshot_full_url'] = _generate_screenshot_url_for_admin(
                logger,
                node_processed_data.get('app_name'),
                node_processed_data.get('screenshot_path'), 
                node_processed_data.get('screen_id') # Để log nếu có lỗi tạo URL
            )
    ...
    logger.info(f"VIEW: Rendering admin_node_management.html with {len(nodes_for_template)} nodes.")
    return render_template('admin_node_management.html', 
                           title=title,
                           nodes=nodes_for_template, # Truyền danh sách đã có screenshot_full_url
                           distinct_app_names=distinct_app_names_for_filter,
                           selected_app_filter=selected_app_filter,
                           current_filter_status=filter_status,
                           pagination=pagination_info_for_template,
                           # config=current_app.config # Nếu template JS cần truy cập config trực tiếp
                          )
...
def api_get_managed_nodes():
    ...
    filter_status_from_req = request.args.get('filter_status', 'unknown') # Mặc định là unknown nếu không có
    page = request.args.get('page', 1, type=int)
    if page < 1: page = 1
    ...
    try:
        # Gọi hàm graph_db để lấy dữ liệu thô
        # raw_nodes_page_data là list các dict, mỗi dict chứa 'node_neo4j_internal_id', 'created_at', 'last_seen', etc.
        ...
        if raw_nodes_page_data is None: # Lỗi từ graph_db.py
            logger.error("API_GET_MANAGED_NODES: graph_db.get_screen_nodes_for_management trả về None.")
            # Trả về lỗi JSON thay vì để Flask tự sinh trang lỗi HTML
        ...
        if raw_nodes_page_data:
            for node_data_from_graphdb in raw_nodes_page_data:
                processed_node = dict(node_data_from_graphdb) # Tạo bản sao để sửa đổi

                # 1. Đảm bảo có ID Neo4j để dùng cho data attribute
                internal_id = processed_node.get('node_neo4j_internal_id')
                if not internal_id:
                    logger.warning(f"API_GET_MANAGED_NODES: Node screen_id='{processed_node.get('screen_id')}' thiếu 'node_neo4j_internal_id'. Sẽ dùng screen_id thay thế cho data attribute.")
                    # Sử dụng screen_id làm fallback nếu ID Neo4j thực sự bị thiếu, mặc dù không lý tưởng
                ...
                # 2. Chuyển đổi các trường datetime sang chuỗi ISO
                if 'created_at' in processed_node:
                    processed_node['created_at_iso'] = _convert_neo4j_datetime_to_iso(processed_node['created_at'])
                if 'last_seen' in processed_node:
                    processed_node['last_seen_iso'] = _convert_neo4j_datetime_to_iso(processed_node['last_seen'])
                
                # 3. Tạo URL ảnh (nếu có)
                processed_node['screenshot_full_url'] = _generate_screenshot_url_for_admin(
                    logger,
                    processed_node.get('app_name'),
                    processed_node.get('screenshot_path'), # Chỉ tên file từ Neo4j
                    processed_node.get('screen_id')
                )
                
                # 4. (Tùy chọn) Lấy thông tin PIE từ PostgreSQL nếu node 'defined'
                # Điều này có thể đã được thực hiện trong get_screen_nodes_for_management
                # Nếu chưa, bạn có thể thêm logic gọi db.get_screen_definition_by_defined_id ở đây
        ...
        pagination_details = {
            ...
            'has_prev': page > 1 and total_pages_val > 0, # Sửa điều kiện has_prev
