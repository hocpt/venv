...
# @admin_required # Tạm thời comment out để test dễ hơn
def admin_screen_elements(screen_id):
    logger = current_app.logger if current_app else print # Lấy logger
    logger.info(f"[Route] Bắt đầu tải trang phân loại phần tử cho screen_id: {screen_id}")

    # Khởi tạo giá trị mặc định
    screen_data = None          # Dữ liệu screen từ Neo4j
    elements_list = []          # List element cuối cùng để vẽ bảng
    elements_json_data = '[]'   # Chuỗi JSON cho Javascript
    screenshot_url = None       # URL ảnh chụp
    valid_classifications = []  # List các classification hợp lệ
    original_width = None       # Kích thước ảnh gốc
    ...
    if not graph_db or not db or not ai_service:
        ...
        # Cân nhắc redirect về trang trước hoặc trang lỗi
        return redirect(url_for('admin.index')) # Redirect về dashboard

    try:
        # --- Bước 1: Lấy thông tin Screen cơ bản từ Neo4j ---
        logger.debug(f"[Route] Truy vấn Neo4j để lấy thông tin screen: {screen_id}")
        # Cần đảm bảo hàm này trả về dict hoặc None
        screen_data_neo4j = graph_db.get_screen_with_elements(screen_id)

        if not screen_data_neo4j:
            flash(f"Không tìm thấy Screen node với ID '{screen_id}' trong Neo4j.", 'warning')
            # Redirect về trang mapping viewer nếu không tìm thấy screen
            # Cần lấy app_name từ đâu đó hoặc redirect về index
            # Tạm thời redirect về index nếu không có app_name
            return redirect(url_for('admin.admin_mapping_viewer'))

        # Gán screen_data để template sử dụng
        screen_data = screen_data_neo4j
        app_name = screen_data.get('app_name') # Lấy app_name từ dữ liệu Neo4j
        logger.debug(f"[Route] Lấy được thông tin Screen từ Neo4j. App Name: {app_name}")

        # --- Bước 2: Tạo URL Screenshot (nếu có) ---
        filename_from_neo4j = screen_data.get('screenshot_path')
        logger.debug(f"[Route] Screenshot path từ Neo4j: '{filename_from_neo4j}'")
        if filename_from_neo4j:
            try:
                # Tạo URL dùng endpoint tùy chỉnh
                screenshot_url = url_for('serve_app_specific_screenshot', filename=filename_from_neo4j, _external=False)
                logger.info(f"[Route] Đã tạo URL screenshot: '{screenshot_url}'")
            except Exception as url_err:
                logger.error(f"[Route] Lỗi tạo URL cho screenshot '{filename_from_neo4j}': {url_err}", exc_info=True)
                screenshot_url = None # Reset nếu lỗi
        else:
            logger.info(f"[Route] Không có screenshot_path cho màn hình {screen_id}.")

        # --- Bước 3: Lấy dữ liệu UI State chi tiết gần nhất từ PostgreSQL ---
        # Lưu ý: Hàm này có thể cần cải thiện để tìm đúng log cho screen_id này
        logger.debug(f"[Route] Truy vấn PostgreSQL để lấy UI state gần nhất...")
        detailed_ui_state = db.get_last_detailed_ui_state_for_screen(screen_id)

        if not detailed_ui_state:
            logger.warning(f"[Route] Không tìm thấy detailed_ui_state từ log cho screen {screen_id}.")
            # Vẫn tiếp tục để thử lấy classification, nhưng elements_list sẽ rỗng
            elements_from_log = []
        elif not isinstance(detailed_ui_state.get('elements'), list):
            logger.warning(f"[Route] detailed_ui_state tìm thấy nhưng không có key 'elements' hoặc không phải list.")
            elements_from_log = []
        else:
            elements_from_log = detailed_ui_state.get('elements', [])
            logger.info(f"[Route] Tìm thấy {len(elements_from_log)} elements từ detailed_ui_state trong log.")
            # (Tùy chọn) Lấy kích thước màn hình gốc từ state nếu có
            # if 'original_width' in detailed_ui_state: original_width = detailed_ui_state['original_width']
            # if 'original_height' in detailed_ui_state: original_height = detailed_ui_state['original_height']

        # --- Bước 4: Lấy thông tin Phân loại đã lưu từ PostgreSQL ---
        logger.debug(f"[Route] Truy vấn PostgreSQL để lấy classifications đã lưu...")
        saved_classifications = db.get_element_classifications_for_screen(screen_id)
        if saved_classifications is None:
            logger.error(f"[Route] Lỗi khi lấy classifications từ DB cho screen {screen_id}. Coi như không có.")
            saved_classifications = {} # Xử lý lỗi DB, coi như rỗng
        else:
            logger.info(f"[Route] Lấy được {len(saved_classifications)} classifications đã lưu từ DB.")

        # --- Bước 5: Lấy thông tin Trạng thái Khám phá (từ Neo4j Transitions) ---
        logger.debug(f"[Route] Truy vấn Neo4j để lấy outgoing transitions...")
        tried_element_ids = set()
        if app_name: # Cần app_name để query đúng
            outgoing_transitions = graph_db.get_outgoing_transitions(screen_id, app_name)
            if outgoing_transitions is None:
                logger.error(f"[Route] Lỗi khi lấy outgoing transitions từ Neo4j cho screen {screen_id}.")
                outgoing_transitions = [] # Xử lý lỗi, coi như rỗng
            else:
                logger.info(f"[Route] Lấy được {len(outgoing_transitions)} outgoing transitions từ Neo4j.")
                # Build set các element đã được tương tác (click/input)
                for trans_props in outgoing_transitions:
                    el_id = trans_props.get('element_id') # Dùng key chuẩn snake_case
                    action_type = trans_props.get('actionType') or trans_props.get('action_type') # Thử cả hai
                    macro_code = trans_props.get('macro_code')
                    # Xác định xem có phải hành động tương tác element không
                    is_interaction = (action_type in ['click', 'input']) or \
                                     (action_type == 'run_macro' and macro_code in ['UI_CLICK', 'UI_INPUT_TEXT'])
                    if el_id and is_interaction:
                        tried_element_ids.add(el_id)
                logger.debug(f"[Route] Xác định được {len(tried_element_ids)} elements đã được thử tương tác.")
        else:
            logger.warning(f"[Route] Không có app_name cho screen {screen_id}, không thể lấy outgoing transitions.")

        # --- Bước 6: Hợp nhất dữ liệu và Tạo elements_list cuối cùng ---
        logger.debug(f"[Route] Bắt đầu hợp nhất dữ liệu element...")
        # Ưu tiên dữ liệu từ log (elements_from_log) vì nó mới nhất từ client
        processed_elements = {} # Dùng dict để tránh trùng lặp element_id
        for el_log in elements_from_log:
            el_id = el_log.get('element_id')
            if not el_id:
                logger.warning(f"[Route] Bỏ qua element từ log vì thiếu element_id: {el_log}")
                continue

            # Lấy thông tin đã lưu (classification & override)
            element_saved_data = saved_classifications.get(el_id, {})
            classification = element_saved_data.get('classification', 'unclassified')
            manual_override = element_saved_data.get('override') # Có thể là True, False, None

            # Xác định trạng thái khám phá tự động
            auto_explored_status = el_id in tried_element_ids

            # Xác định trạng thái hiển thị cuối cùng
            display_explored_status = auto_explored_status # Mặc định là trạng thái tự động
            override_active = 'auto'
            if manual_override is True:
                display_explored_status = True
                override_active = 'force_explored'
            elif manual_override is False:
                display_explored_status = False
                override_active = 'force_unexplored'

            # Tạo entry hoàn chỉnh cho element này
            processed_elements[el_id] = {
                # Dữ liệu chính từ log
                'element_id': el_id,
                'identifier_type': el_log.get('identifier_type'),
                'element_type': el_log.get('element_type'),
                'text_content': el_log.get('text_content'),
                'coordinates': el_log.get('coordinates'), # Giữ nguyên dict {x, y} nếu có
                'bounds': el_log.get('bounds'),       # Giữ nguyên dict {top, left...} nếu có
                # Dữ liệu hợp nhất/tính toán
                'classification': classification,
                'display_explored_status': display_explored_status, # True/False cuối cùng
                'override_active': override_active, # 'auto', 'force_explored', 'force_unexplored'
                # (Tùy chọn) Thêm các trường khác nếu cần cho JS
                'attempt_count': 0, # Tạm thời chưa lấy count từ Neo4j
                'success_count': 0, # Tạm thời chưa lấy count từ Neo4j
            }

        # Chuyển dict thành list để truyền vào template
        elements_list = list(processed_elements.values())
        # (Tùy chọn) Sắp xếp lại elements_list nếu cần (ví dụ theo tọa độ y, x)
        # elements_list.sort(key=lambda x: (x.get('coordinates',{}).get('y', 0), x.get('coordinates',{}).get('x', 0)))

        logger.info(f"[Route] Hợp nhất xong, có {len(elements_list)} elements để hiển thị.")

        # --- Bước 7: Tạo chuỗi JSON elements_json_data ---
        if elements_list:
            try:
                # Chuyển đổi các kiểu không serialize được nếu có (hiện tại chưa có)
                elements_serializable = elements_list
                elements_json_data = json.dumps(elements_serializable)
                logger.debug(f"[Route] Tạo thành công elements_json_data (first 200 chars): {elements_json_data[:200]}...")
            except Exception as e_json:
                logger.error(f"[Route] Lỗi serialize elements_list thành JSON: {e_json}", exc_info=True)
                elements_json_data = '[]' # Đặt lại thành rỗng nếu lỗi
                flash("Lỗi xử lý dữ liệu element JSON.", "error")
        else:
            logger.warning(f"[Route] elements_list rỗng sau khi xử lý, elements_json_data sẽ là '[]'.")
            elements_json_data = '[]'

        # --- Bước 8: Lấy danh sách classification hợp lệ ---
        if hasattr(ai_service, 'VALID_CLASSIFICATIONS'):
            valid_classifications = ai_service.VALID_CLASSIFICATIONS
        else:
            logger.error("[Route] VALID_CLASSIFICATIONS không tìm thấy trong ai_service!")
            flash("Lỗi cấu hình: Thiếu danh sách phân loại hợp lệ.", "error")
            valid_classifications = ['unclassified', 'error'] # Cung cấp giá trị mặc định
        ...
        # Đặt lại các biến về trạng thái an toàn nếu có lỗi nghiêm trọng
        elements_list = []
        elements_json_data = '[]'
        # Không redirect ở đây, để template hiển thị thông báo lỗi

    # --- Bước 9: Render Template ---
    logger.info(f"[Route] Chuẩn bị render template admin_screen_elements.html")
    # Truyền tất cả các biến cần thiết
    return render_template('admin_screen_elements.html',
                           title=f"Phân loại - {screen_id[:15]}...", # Tiêu đề ngắn gọn
                           screen=screen_data, # Dữ liệu screen từ Neo4j
                           elements=elements_list, # List Python cho bảng HTML
                           valid_classifications=valid_classifications, # List cho dropdown
                           screenshot_url=screenshot_url, # URL ảnh (có thể None)
                           original_width=original_width, # Kích thước gốc (có thể None)
                           original_height=original_height,# Kích thước gốc (có thể None)
                           elements_json_data=elements_json_data) # Chuỗi JSON cho Javascript
...
# @login_required # Đảm bảo có xác thực nếu cần
def api_get_app_graph_data():
    logger = current_app.logger if current_app else print
    app_name = request.args.get('app_name')
    logger.info(f"API: Yêu cầu dữ liệu đồ thị cho app: '{app_name}'")

    if not app_name:
        logger.warning("API: Thiếu tham số 'app_name' trong query.")
        return jsonify({"error": "Thiếu tham số app_name"}), 400

    if not graph_db:
         logger.error("API: GraphDB module chưa sẵn sàng.")
         return jsonify({"error": "Lỗi cấu hình server (GraphDB)"}), 500

    try:
        # Bước 1: Gọi hàm DB để lấy dữ liệu thô (bao gồm screenshot_path)
        # Đảm bảo hàm get_app_graph_data trong graph_db.py đã được sửa để trả về screenshot_path
        graph_data = graph_db.get_app_graph_data(app_name)

        if graph_data is None:
             logger.error(f"API: graph_db.get_app_graph_data trả về None cho app '{app_name}'.")
             return jsonify({"error": "Không thể lấy dữ liệu đồ thị từ cơ sở dữ liệu."}), 500

        nodes_list = graph_data.get("nodes", [])
        edges_list = graph_data.get("edges", [])

        processed_nodes = []
        # === VÒNG LẶP QUAN TRỌNG ĐỂ TẠO URL ===
        for node_element in nodes_list:
            node_data = node_element.get("data", {})
            filename = node_data.get("screenshot_path") # Lấy tên file từ DB

            # === THÊM LOG DEBUG NGAY ĐÂY ===
            print(f"--- DEBUG NODE {node_data.get('id')} ---")
            print(f"  Raw screenshot_path from graph_db: {filename} (Type: {type(filename)})")
            # ==============================

            if filename and isinstance(filename, str): # Kiểm tra filename tồn tại và là string
                try:
                    # Tạo URL ảnh bằng endpoint tùy chỉnh
                    # _external=False để tạo URL tương đối /app_screenshots/...
                    generated_url = url_for('serve_app_specific_screenshot', filename=filename, _external=False)
                    node_data["screenshot_url"] = generated_url
                    # === THÊM LOG DEBUG KHI TẠO URL THÀNH CÔNG ===
                    print(f"  Generated screenshot_url: {generated_url}")
                    # ===========================================
                except Exception as url_err:
                    logger.error(f"API: Lỗi tạo URL cho screenshot '{filename}' của node {node_data.get('id')}: {url_err}", exc_info=False)
                    node_data["screenshot_url"] = None
            else:
                # === THÊM LOG DEBUG KHI FILENAME KHÔNG HỢP LỆ ===
                print(f"  Skipping URL generation (filename invalid or missing)")
                # ==========================================
                node_data["screenshot_url"] = None

            # Không cần trả về screenshot_path nữa
            node_data.pop("screenshot_path", None)

            processed_nodes.append({"data": node_data})
        # === KẾT THÚC VÒNG LẶP ===

        final_graph_data = {"nodes": processed_nodes, "edges": edges_list}

        logger.info(f"API: Trả về dữ liệu đồ thị thành công cho app '{app_name}'.")
        return jsonify(final_graph_data)

    except Exception as e:
        logger.error(f"API: Lỗi không mong muốn khi lấy dữ liệu đồ thị cho app '{app_name}': {e}", exc_info=True)
        return jsonify({"error": "Lỗi server nội bộ khi xử lý yêu cầu."}), 500

@admin_bp.route('/mapping/') # Route khi chưa chọn app nào
@admin_bp.route('/mapping/<path:app_name>') # Route khi đã chọn app_name
# @admin_required # Bỏ comment nếu bạn cần xác thực admin
def admin_mapping_viewer(app_name=None):
    """
    Hiển thị trang xem đồ thị mapping cho một app cụ thể hoặc trang chọn app.
    """
    logger = current_app.logger if current_app else print # Lấy logger
    logger.info(f"Accessing mapping viewer. Selected app: {app_name}")

    title = "Trình xem Bản đồ Ứng dụng" # Tiêu đề mặc định
    available_apps = [] # List chứa các app_name cho dropdown

    # Kiểm tra module graph_db có sẵn không
    if not graph_db:
        flash("Lỗi nghiêm trọng: Module GraphDB (Neo4j) chưa sẵn sàng.", "error")
        # Có thể redirect về index hoặc trang lỗi
        return redirect(url_for('admin.index'))

    # Luôn lấy danh sách các app_name đã có trong Neo4j để hiển thị dropdown
    try:
        # Gọi hàm trong graph_db.py để lấy các app_name duy nhất
        apps_from_db = graph_db.get_distinct_app_names()
        if apps_from_db is None:
            # Lỗi khi truy vấn DB
            flash("Không thể tải danh sách ứng dụng từ Neo4j.", "warning")
            available_apps = []
        else:
            available_apps = apps_from_db # Gán danh sách lấy được
            logger.debug(f"Found distinct app names: {available_apps}")

    except Exception as e:
         logger.error(f"Lỗi khi lấy distinct app names: {e}", exc_info=True)
         flash("Lỗi không mong muốn khi tải danh sách ứng dụng.", "error")
         available_apps = [] # Đảm bảo là list rỗng khi lỗi

    # Cập nhật tiêu đề nếu có app được chọn
    if app_name:
        title = f"Bản đồ cho: {app_name}"

    # Render template, truyền các biến cần thiết
    # Template sẽ tự xử lý việc hiển thị dropdown và khu vực đồ thị
    # dựa trên việc selected_app_name có giá trị hay không.
    return render_template(
        'admin_mapping_viewer.html', # <<< Đảm bảo render đúng template này
        title=title,
        selected_app_name=app_name, # Truyền app_name đang được chọn (có thể là None)
        available_apps=available_apps # Truyền danh sách app cho dropdown
    )
