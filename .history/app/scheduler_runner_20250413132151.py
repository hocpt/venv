...
def _process_pending_commands():
    ...
    try:
        ...
        for command in pending_commands:
            ...
            # Đánh dấu là đang xử lý trước
            updated = db.update_command_status(conn, command_id, 'processing')
            if not updated:
                print(f"WARNING (Command Processor): Could not mark command {command_id} as processing. Skipping.")
                continue # Bỏ qua nếu không cập nhật được status
            ...
            try:
                ...
                # Validate cơ bản (có thể thêm kiểm tra kỹ hơn)
                if not all([persona_a_id, persona_b_id, strategy_id]):
                    raise ValueError("Missing required parameters in payload (personas, strategy)")
                ...
                print(f"DEBUG (Command Processor): Scheduling job '{job_id}' to run simulation...")

                # Thêm job vào scheduler
                live_scheduler.add_job(
                    id=job_id,
                    func=run_ai_conversation_simulation, # Hàm tác vụ nền
                    args=job_args,
                    trigger='date',
                    run_date=run_time,
                    jobstore='default',
                    executor='processpool', # <<< Chạy mô phỏng trong processpool nếu nặng
                    # executor='default', # Hoặc dùng threadpool nếu nhẹ
                    replace_existing=False,
                    misfire_grace_time=120 # Cho phép trễ nhiều hơn chút
                )

                # Nếu thành công, cập nhật status thành 'done'
                ...
                # Nếu có lỗi khi parse hoặc thêm job
                ...
                # Không cần rollback vì lỗi xử lý 1 command không nên ảnh hưởng command khác
