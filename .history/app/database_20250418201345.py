...
def add_new_transition(current_stage_id: str, user_intent: str,
    ...
    loop_selector_json = None
    if loop_target_selector_str and loop_target_selector_str.strip():
        try:
             loop_selector_json = json.loads(loop_target_selector_str) # Chỉ parse để validate
    ...
    try:
        ...
        params = (
            ...
            priority,
            json.dumps(action_json_for_db) if action_json_for_db else None, # Lưu action JSON
            template_ref_to_save,
            # --- Giá trị cho các cột loop mới ---
            loop_type if loop_type and loop_type.strip() else None,
            loop_count, # Có thể là None
            loop_condition_type if loop_condition_type and loop_condition_type.strip() else None,
            loop_condition_value if loop_condition_value and loop_condition_value.strip() else None,
            json.dumps(loop_selector_json) if loop_selector_json else None, # Lưu loop selector JSON
            loop_variable_name if loop_variable_name and loop_variable_name.strip() else None
        )
        print(f"DEBUG SQL (add_new_transition): {cur.mogrify(sql, params).decode('utf-8','ignore')}") # Log SQL
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.Error as e:
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str,
    ...
    loop_selector_json = None
    if loop_target_selector_str and loop_target_selector_str.strip():
        try: loop_selector_json = json.loads(loop_target_selector_str)
        except json.JSONDecodeError as json_err: return False, f"Loop Target Selector JSON không hợp lệ: {json_err}"
    ...
    try:
        ...
        sql = """
            UPDATE public.stage_transitions SET
                ...
                next_stage_id = %s, priority = %s, action_to_suggest = %s, response_template_ref = %s,
                loop_type = %s, loop_count = %s, loop_condition_type = %s, loop_condition_value = %s,
                loop_target_selector = %s, loop_variable_name = %s
        ...
        print(f"DEBUG SQL (update_transition): {cur.mogrify(sql, params).decode('utf-8','ignore')}") # Log SQL
        cur.execute(sql, params)
        conn.commit()
        success = cur.rowcount > 0 # True nếu có dòng được cập nhật
        if not success:
...
def get_transition_details(transition_id: int) -> dict | None:
    ...
    try:
        ...
        if row:
            details = dict(row)
            # Xử lý trường hợp action_to_suggest là None trước khi truy cập key
            if isinstance(details.get('action_to_suggest'), dict):
                # Tạo các key riêng để dễ dùng trong template edit
                details['action_macro_code'] = details['action_to_suggest'].get('macro_code')
                # Format params thành chuỗi JSON đẹp cho textarea
                params_dict = details['action_to_suggest'].get('params', {})
                try:
                    details['action_params_str'] = json.dumps(params_dict, indent=2, ensure_ascii=False) if params_dict else '{}'
                except TypeError:
                    details['action_params_str'] = str(params_dict) # Fallback
            else:
                details['action_macro_code'] = None
                details['action_params_str'] = '{}' # Mặc định JSON rỗng

            # Format loop_target_selector thành chuỗi JSON đẹp
            if details.get('loop_target_selector') is not None:
                 try:
                      details['loop_target_selector_str'] = json.dumps(details['loop_target_selector'], indent=2, ensure_ascii=False)
                 except TypeError:
                      details['loop_target_selector_str'] = str(details['loop_target_selector'])
            else:
                 details['loop_target_selector_str'] = '' # Chuỗi rỗng cho textarea

            print(f"DEBUG DB (get_transition_details): Found details for {transition_id}: {details}")
        else:
...
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    ...
    try:
        ...
        # Lấy transitions mà current_stage_id thuộc strategy này
        # <<< THÊM CÁC CỘT loop_* VÀO SELECT >>>
        sql = """
            SELECT
                t.transition_id, t.current_stage_id, t.user_intent,
        ...
        # Log chi tiết hơn nếu cần debug dữ liệu loop_* trả về
        # print(f"DEBUG DB (get_strategy_action_sequence): Returning raw transitions: {transitions_list}")
