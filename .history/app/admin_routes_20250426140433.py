...
def add_simulation_config_view(): # <<< Đổi tên hàm ở đây
    ...
    # --- Lấy dữ liệu cho các Dropdown (Giữ nguyên logic lấy data) ---
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            strategy_id = request.form.get('strategy_id')
            max_turns_str = request.form.get('max_turns', '5').strip() # <<< Sửa lấy max_turns
            starting_prompt = request.form.get('starting_prompt', '').strip() # <<< Sửa lấy starting_prompt
            simulation_goal = request.form.get('simulation_goal', '').strip() # <<< Sửa lấy simulation_goal
            is_enabled = request.form.get('is_enabled') == 'on' # <<< Sửa lấy is_enabled

            # --- Validate (Giữ nguyên) ---
            errors = []
            if not config_name: errors.append("Tên cấu hình là bắt buộc.")
            if not persona_a_id: errors.append("Persona A là bắt buộc.")
            if not persona_b_id: errors.append("Persona B là bắt buộc.") # Nên bắt buộc
            if persona_a_id == persona_b_id: errors.append("Persona A và B phải khác nhau.")
            if not account_log_a_id: errors.append("Account Log A là bắt buộc.")
            if not account_log_b_id: errors.append("Account Log B là bắt buộc.")
            if account_log_a_id == account_log_b_id: errors.append("Account Log A và B phải khác nhau.")
            if not strategy_id: errors.append("Chiến lược là bắt buộc.")
            max_turns = 5
            try:
                max_turns = int(max_turns_str)
            ...
            # --- Gọi hàm DB (đã sửa để trả về tuple) ---
            ...
            # === SỬA LẠI: Dùng logger và hiển thị lại form ===
            logger.error(f"Lỗi nghiêm trọng khi thêm simulation config: {e}", exc_info=True) # Ghi log chi tiết
            flash(f"Lỗi không mong muốn khi thêm cấu hình: {e}", "error") # Flash lỗi chung
            # Hiển thị lại form với lỗi
            return render_template('admin_add_simulation_config.html',
                                   title=title + " (Lỗi Exception)", cancel_url=cancel_url,
                                   personas=all_personas, accounts=all_accounts, strategies=all_language_strategies,
                                   current_data=current_data) # Giữ lại dữ liệu form
