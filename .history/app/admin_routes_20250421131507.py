...
def edit_task_assignment(assignment_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            target_data_parsed = None # Để lưu trữ dict đã parse
            if target_data_str and target_data_str.strip() != '{}':
                try:
                    target_data_parsed = json.loads(target_data_str)
                    if not isinstance(target_data_parsed, dict): raise ValueError()
                    # Chỉ lưu nếu nó khác giá trị cũ (để tránh update không cần thiết)
                    # Hoặc luôn lưu nếu bạn muốn ghi đè hoàn toàn
                    # if target_data_parsed != assignment.get('target_data'):
                ...
                # Nếu người dùng nhập {} hoặc rỗng, coi như muốn xóa target data
                 if assignment.get('target_data') is not None: # Chỉ cập nhật nếu khác None
                      update_data['target_data'] = None

            # Notes (Luôn cập nhật nếu có thay đổi)
            # if notes != (assignment.get('notes') or ''): # Chỉ cập nhật nếu khác
            update_data['notes'] = notes if notes else None

            # Schedule Times (Validate format và logic)
            schedule_start_time = None
            if schedule_start_str:
                try: schedule_start_time = datetime.fromisoformat(schedule_start_str).astimezone(timezone.utc)
                except ValueError: errors.append("Định dạng thời gian bắt đầu không hợp lệ.")
            # Chỉ cập nhật nếu khác giá trị cũ (cần so sánh timezone-aware)
            # if schedule_start_time != assignment.get('schedule_start_time'):
            update_data['schedule_start_time'] = schedule_start_time # Cho phép đặt lại thành None
            ...
            # if schedule_end_time != assignment.get('schedule_end_time'):
            update_data['schedule_end_time'] = schedule_end_time # Cho phép đặt lại thành None

            if schedule_start_time and schedule_end_time and schedule_start_time >= schedule_end_time:
                 errors.append("Thời gian kết thúc phải sau thời gian bắt đầu.")

            # Status (Tạm thời không cho sửa)
            # if status and status not in editable_statuses: errors.append(...)
            ...
            if errors:
                ...
                return render_template('admin_edit_task_assignment.html',
                                       title=title + " (Lỗi)",
                                       assignment=assignment, # Dữ liệu gốc để hiển thị phần không đổi
                                       current_data=current_data), # Dữ liệu vừa nhập để fill lại form
                                       # valid_statuses=editable_statuses), # Nếu cho sửa status
                                       400

            # --- Nếu không có lỗi validation, gọi hàm DB để cập nhật ---
            # Chỉ update nếu có trường thay đổi thực sự (tùy chọn, hàm DB cũng kiểm tra rowcount)
            if update_data:
                 ...
                 if success:
                     ...
                     return render_template('admin_edit_task_assignment.html',
                                           ...
                                           current_data=current_data)
                                           # valid_statuses=editable_statuses)
            else:
    ...
    return render_template('admin_edit_task_assignment.html',
                           title=title,
                           assignment=assignment) # Truyền dữ liệu assignment gốc
                           # valid_statuses=editable_statuses)
