...
def view_scheduled_jobs():
    ...
    try:
        ...
        db_jobs_config = []
    print(f"DEBUG: Successfully accessed current_app.scheduler: {scheduler}")
    # 2. Lấy trạng thái jobs thực tế từ Scheduler đang chạy
    if scheduler: # Chỉ thực hiện nếu import scheduler thành công
        try:
            running_jobs = scheduler.get_jobs() # Lấy danh sách các job object đang chạy
...
def edit_scheduled_job(job_id):
    if request.method == 'POST':
        ...
        print(f"DEBUG: Scheduler object accessible in route: {scheduler}")
        if scheduler:
                     try:
                         all_live_jobs = scheduler.get_jobs()
        ...
        if scheduler:
            ...
            try:
                ...
                live_job = scheduler.get_job(job_id) # Lấy lại job sau khi reschedule
