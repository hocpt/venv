...
def _generate_screenshot_url_for_admin(logger, node_app_name, raw_screenshot_filename_from_neo4j, node_id_for_log="N/A"):
    """
    Tạo URL đầy đủ cho ảnh screenshot.
    Args:
        logger: Đối tượng logger.
        node_app_name: Tên ứng dụng của node.
        raw_screenshot_filename_from_neo4j: Tên file ảnh (chỉ tên file) từ Neo4j.
        node_id_for_log: Screen ID của node để ghi log.
    Returns:
    ...
    if raw_screenshot_filename_from_neo4j and node_app_name:
        ...
        if not node_app_name:
             logger.warning(f"URL_HELPER: Node {node_id_for_log} không có app_name, không thể tạo URL ảnh an toàn.")
        return None
...
def view_node_management():
    ...
    logger.info("VIEW: Accessing Node Management page.")

    distinct_app_names_for_filter = [] 
    try:
        # Lấy danh sách app_name từ PostgreSQL (bảng screen_definitions) để lọc
        # Hoặc bạn có thể lấy từ Neo4j nếu muốn hiển thị tất cả app_name có node
        conn = db.get_db_connection() 
        if conn:
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur: # Sử dụng psycopg2.extras.DictCursor nếu get_db_connection trả về cursor chuẩn
                    cur.execute("SELECT DISTINCT app_name FROM screen_definitions WHERE app_name IS NOT NULL ORDER BY app_name;")
                    distinct_app_names_for_filter = [row['app_name'] for row in cur.fetchall()]
    ...
    if raw_nodes_page_data is None:
        flash("Lỗi khi tải dữ liệu Node từ cơ sở dữ liệu đồ thị.", "error")
        ...
        for node_neo4j_data in raw_nodes_page_data: # node_neo4j_data là dict
            node_processed_data = dict(node_neo4j_data)
            
            node_processed_data['screenshot_full_url'] = _generate_screenshot_url_for_admin(
                logger,
                node_processed_data.get('app_name'),
                node_processed_data.get('screenshot_path'), # Chỉ tên file từ Neo4j
                node_processed_data.get('screen_id')
            )
        ...
        if total_items_from_db is not None and total_items_from_db > 0:
            ...
            # Đảm bảo page không vượt quá số trang thực tế (quan trọng khi filter thay đổi total_items)
            if page > total_pages_calc and total_pages_calc > 0: 
                page = total_pages_calc # Điều chỉnh lại page nếu nó vượt quá
    ...
    # Không cần flash "không có node unknown" vì JavaScript sẽ hiển thị nếu list rỗng
    ...
    return render_template('admin_node_management.html', 
                           ...
                           # config=current_app.config # Chỉ truyền nếu template JS thực sự cần đọc config trực tiếp
                          )
...
def api_get_managed_nodes():
    ...
    try:
        ...
        # Hàm này trong graph_db.py cần trả về các node đã có logical_pie_name nếu defined
        ...
        if nodes_page_data:
            for node_data_from_neo4j in nodes_page_data:
                ...
                # raw_screenshot_filename LÀ CHỈ TÊN FILE từ Neo4j (n.screenshot_path)
                raw_screenshot_filename = node_render_data.get('screenshot_path') 
                # node_app_name = node_render_data.get('app_name') # Vẫn cần app_name cho các logic khác, nhưng không dùng để tạo URL ảnh nữa
                ...
                if raw_screenshot_filename: # Chỉ cần tên file để tạo URL
                    try:
                        # filename TRUYỀN VÀO url_for LÀ CHỈ TÊN FILE
                        # Route /app_screenshots/<filename> sẽ xử lý phần còn lại
                        node_render_data['screenshot_full_url'] = url_for(
                            'serve_app_specific_screenshot', # Endpoint của route trong app/__init__.py
                            filename=raw_screenshot_filename  # CHỈ TRUYỀN TÊN FILE
                        )
                        logger.info(f"API: Generated screenshot_full_url for '{raw_screenshot_filename}': {node_render_data['screenshot_full_url']}")
                    except Exception as url_exc:
                        logger.error(f"API: Lỗi tạo URL ảnh cho file {raw_screenshot_filename}: {url_exc}", exc_info=True)
                        node_render_data['screenshot_full_url'] = None
                else:
                    node_render_data['screenshot_full_url'] = None
                    logger.info(f"Node {node_render_data.get('screen_id')} (App: {node_render_data.get('app_name')}) không có screenshot_path.")
