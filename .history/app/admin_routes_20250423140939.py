...
def view_api_documentation(): # Đổi tên hàm để phân biệt với hàm cũ
    """Hiển thị trang danh sách tài liệu API từ CSDL với phân trang và xem chi tiết."""
    title = "Tài liệu API Client (Database)"
    page = request.args.get('page', 1, type=int)
    if page < 1: page = 1
    # Lấy số lượng item mỗi trang từ config hoặc đặt giá trị mặc định
    per_page = current_app.config.get('ADMIN_ITEMS_PER_PAGE', 15)

    all_docs_details = [] # Danh sách chứa đầy đủ chi tiết cho trang hiện tại
    total_items = 0
    pagination_details = None # Dict thông tin phân trang

    if not db:
        flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
    else:
        try:
            # Gọi hàm DB để lấy cả chi tiết và tổng số
            ...
            if docs_result is not None and total_result is not None:
                all_docs_details = docs_result # List các dict chi tiết
                total_items = total_result

                # Tính toán phân trang (logic giống các trang list khác)
                if total_items > 0:
                    total_pages = math.ceil(total_items / per_page)
                    if page > total_pages and total_pages > 0: page = total_pages # Chỉnh lại page nếu vượt quá
                    pagination_details = {
                        'page': page, 'per_page': per_page, 'total_items': total_items,
                        'total_pages': total_pages, 'has_prev': page > 1, 'has_next': page < total_pages,
                        'prev_num': page - 1 if page > 1 else None,
            ...
            all_docs_details = [] # Đảm bảo là list rỗng khi lỗi

    # Render template list, truyền cả danh sách chi tiết và pagination
    return render_template('admin_api_docs_list.html',
                           title=title,
                           api_docs=all_docs_details,         # Dùng cho vòng lặp chính của bảng
                           all_docs_details=all_docs_details, # Dùng để nhúng vào JS cho modal
                           pagination_details=pagination_details) 

@admin_bp.route('/api-docs/add', methods=['GET', 'POST'])
def add_api_doc(): # <<< Tên hàm này khớp với endpoint 'admin.add_api_doc'
    """
    Route để hiển thị form thêm và xử lý thêm mới tài liệu API.
    (Hiện tại chỉ là khung sườn để giải quyết BuildError)
    """
    # title = "Thêm Tài liệu API Mới" # Sẽ dùng khi tạo template form

    if request.method == 'POST':
        # TODO: Xử lý dữ liệu form gửi lên ở đây
        # 1. Lấy dữ liệu từ request.form
        # 2. Validate dữ liệu
        # 3. Gọi hàm db.add_api_doc(...) (sẽ viết sau)
        # 4. Flash thông báo thành công/thất bại
        flash("Chức năng Thêm API Doc chưa được triển khai hoàn chỉnh.", "info")
        # Tạm thời redirect về trang danh sách
        return redirect(url_for('admin.view_api_documentation'))

    # GET request: Hiển thị form thêm mới
    # TODO: Tạo template 'admin_add_api_doc.html' và render ở đây
    # return render_template('admin_add_api_doc.html', title=title)

    # Tạm thời chỉ flash thông báo và redirect để giải quyết BuildError
    flash("Trang Thêm API Doc chưa được tạo, vui lòng quay lại sau.", "info")
    return redirect(url_for('admin.view_api_documentation'))

@admin_bp.route('/api-docs/<int:doc_id>/edit', methods=['GET', 'POST'])
def edit_api_doc(doc_id): # <<< Tên hàm khớp endpoint, nhận doc_id từ URL
    """
    Route để hiển thị form và xử lý sửa tài liệu API.
    (Hiện tại chỉ là khung sườn để giải quyết BuildError)
    """
    # title = f"Sửa Tài liệu API #{doc_id}" # Sẽ dùng khi tạo template form

    if request.method == 'POST':
        # TODO: Xử lý dữ liệu form gửi lên ở đây
        # 1. Lấy dữ liệu từ request.form
        # 2. Validate dữ liệu
        # 3. Gọi hàm db.update_api_doc(doc_id, ...) (sẽ viết sau)
        # 4. Flash thông báo thành công/thất bại
        flash(f"Chức năng Sửa API Doc (ID: {doc_id}) chưa được triển khai hoàn chỉnh.", "info")
        # Tạm thời redirect về trang danh sách
        return redirect(url_for('admin.view_api_documentation')) # Nhớ sửa tên route xem list nếu khác

    # GET request: Hiển thị form sửa
    # TODO:
    # 1. Gọi hàm db.get_api_doc_by_id(doc_id) (sẽ viết sau) để lấy chi tiết doc cần sửa
    # 2. Tạo template 'admin_edit_api_doc.html'
    # 3. Render template đó và truyền dữ liệu doc vào
    # return render_template('admin_edit_api_doc.html', title=title, doc=doc_details)

    # Tạm thời chỉ flash thông báo và redirect để giải quyết BuildError
    flash(f"Trang Sửa API Doc (ID: {doc_id}) chưa được tạo, vui lòng quay lại sau.", "info")
    return redirect(url_for('admin.view_api_documentation')) 
