...
def add_new_strategy(strategy_id: str, name: str, description: str | None, initial_stage_id: str, strategy_type: str) -> tuple[bool, str | None]: # <<< Đảm bảo có 5 tham số ở đây
    """Thêm một chiến lược mới, bao gồm cả strategy_type."""
    ...
    try:
        cur = conn.cursor()
        print(f"DEBUG: Thêm strategy mới: id='{strategy_id}', name='{name}', type='{strategy_type}'")
        # <<< Thêm cột strategy_type vào INSERT và updated_at >>>
        sql = """
            INSERT INTO strategies (strategy_id, name, description, initial_stage_id, strategy_type, updated_at)
            VALUES (%s, %s, %s, %s, %s, NOW());
        """
        params = (strategy_id, name, description, initial_stage_id, strategy_type) # <<< Thêm strategy_type vào params
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.IntegrityError as int_err:
         error_msg = f"Lỗi ràng buộc CSDL (ID hoặc Name đã tồn tại?): {int_err}"
