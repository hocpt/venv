...
def admin_screen_elements(screen_id):
    ...
    try:
        ...
        # === TẠO SCREENSHOT URL DÙNG url_for('static', ...) ===
        # screen_data.get('screenshot_path') giờ là 'screenshots/ten_file.png'
        path_from_neo4j = screen_data.get('screenshot_path')
        logger.debug(f"[admin_screen_elements] Lấy screenshot_path từ Neo4j: '{path_from_neo4j}'")

        if path_from_neo4j:
            try:
                # Sử dụng endpoint 'static' mặc định của Flask
                screenshot_url = url_for('static', filename=path_from_neo4j)
                logger.info(f"[admin_screen_elements] Đã tạo URL screenshot: '{screenshot_url}'")
            except Exception as url_err:
                logger.error(f"[admin_screen_elements] Lỗi khi tạo URL tĩnh cho screenshot path '{path_from_neo4j}': {url_err}", exc_info=True)
                screenshot_url = None
        else:
