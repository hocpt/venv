...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    """
    Thêm một template_ref mới (nếu chưa tồn tại) và variation đầu tiên.
    Đơn giản hóa xử lý transaction.
    """
    logger = current_app.logger if current_app else print
    _template_ref = template_ref.strip() # Dùng biến đã strip
    _variation_text = first_variation_text.strip()

    if not _template_ref or not _variation_text:
    ...
    try:
        cur = conn.cursor()
        logger.info(f"Attempting transaction for template '{_template_ref}'.")

        # === BƯỚC 1: THỬ INSERT hoặc BỎ QUA nếu đã tồn tại ===
        proceed_to_variation = False
        try:
            # Thử INSERT template ref
            sql_template = """
                INSERT INTO public.response_templates (template_ref, description, category)
                VALUES (%s, %s, %s);
            """
            cur.execute(sql_template, (_template_ref, description, category if category else None))
            logger.info(f"Step 1: INSERTED new template_ref '{_template_ref}'.")
            proceed_to_variation = True # Insert thành công, tiếp tục bước 2
        except psycopg2.errors.UniqueViolation:
            # Đã tồn tại, đây là trường hợp OK, chỉ cần ghi log và tiếp tục
            logger.warning(f"Step 1: Template ref '{_template_ref}' already exists (UniqueViolation caught). Proceeding.")
            conn.rollback() # Rollback transaction nhỏ của lệnh INSERT lỗi này
            cur = conn.cursor() # Mở lại cursor sau rollback để tiếp tục transaction lớn
            proceed_to_variation = True # Vẫn tiếp tục bước 2
        except psycopg2.Error as db_err_template:
            # Lỗi DB khác ở Bước 1 -> Không nên tiếp tục
            logger.error(f"Step 1: FAILED DB Error inserting template '{_template_ref}': {db_err_template}", exc_info=True)
            proceed_to_variation = False # Đánh dấu không tiếp tục
            # Không raise lỗi vội, để khối ngoài xử lý rollback
        except Exception as e_template:
             logger.error(f"Step 1: FAILED Unexpected Error inserting template '{_template_ref}': {e_template}", exc_info=True)
             proceed_to_variation = False
             # Không raise lỗi vội

        # === BƯỚC 2: INSERT variation (CHỈ KHI BƯỚC 1 OK HOẶC BÁO ĐÃ TỒN TẠI) ===
        variation_insert_success = False
        if proceed_to_variation:
            logger.info(f"Proceeding to Step 2: Insert variation for '{_template_ref}'.")
            try:
                sql_variation = """
                    INSERT INTO public.template_variations (template_ref, variation_text)
                    VALUES (%s, %s);
                """
                cur.execute(sql_variation, (_template_ref, _variation_text))
                logger.info(f"Step 2: Successfully inserted variation for '{_template_ref}'.")
                variation_insert_success = True
            except psycopg2.Error as db_err_variation:
                # Lỗi ở bước 2 - ĐÂY LÀ NƠI LỖI FK CỦA BẠN ĐANG XẢY RA
                logger.error(f"Step 2: FAILED DB Error inserting variation for '{_template_ref}': {db_err_variation}", exc_info=True)
                variation_insert_success = False # Đánh dấu lỗi
            except Exception as e_variation:
                logger.error(f"Step 2: FAILED Unexpected Error inserting variation for '{_template_ref}': {e_variation}", exc_info=True)
                variation_insert_success = False

        # === COMMIT HOẶC ROLLBACK ===
        if proceed_to_variation and variation_insert_success:
            conn.commit()
            logger.info(f"Transaction committed for '{_template_ref}'.")
            returned_ref = _template_ref # Trả về thành công
        else:
            logger.error(f"Rolling back transaction for '{_template_ref}' due to failure in Step 1 (serious error) or Step 2.")
            conn.rollback()
            returned_ref = None # Trả về thất bại

    except Exception as e_outer: # Bắt lỗi tổng quát (ví dụ lỗi tạo cursor)
        logger.error(f"LỖI (database.py - add_new_template): Lỗi không xác định ngoài dự kiến: {e_outer}", exc_info=True)
        if conn: conn.rollback() # Đảm bảo rollback
        returned_ref = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
