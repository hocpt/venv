...
def add_macro_definition(macro_code: str, description: str | None, app_target: str | None, params_schema_str: str | None, notes: str | None) -> tuple[bool, str | None]:
    ...
    if params_schema_str and params_schema_str.strip():
        try:
            params_schema_json = json.dumps(json.loads(params_schema_str)) # Validate và chuẩn hóa JSON
        except json.JSONDecodeError:
    ...
    try:
        ...
        """
        app_target_db = app_target if app_target and app_target.strip() else 'system' # Default 'system' nếu trống
...
def get_all_macro_definitions(app_target: str | None = None, include_generic: bool = True) -> list[dict] | None:
    """Lấy danh sách định nghĩa macro, tùy chọn lọc theo app và bao gồm generic/system."""
    macros_list = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        base_sql = "SELECT macro_code, description, app_target, notes FROM public.macro_definitions"
        where_clauses = []
        params = []

        filter_targets = []
        if app_target:
            filter_targets.append(app_target)
        if include_generic:
            filter_targets.extend(['system', 'generic'])

        if filter_targets:
             # Tạo placeholder cho mỗi target: (%s, %s, %s)
             placeholders = ', '.join(['%s'] * len(filter_targets))
             where_clauses.append(f"app_target IN ({placeholders})")
             params.extend(filter_targets)

        if where_clauses:
            sql = f"{base_sql} WHERE {' AND '.join(where_clauses)} ORDER BY app_target, macro_code;"
        else:
            sql = f"{base_sql} ORDER BY app_target, macro_code;"
...
def delete_macro_definition(macro_code: str) -> tuple[bool, str | None]:
    ...
    try:
        ...
        # Kiểm tra xem macro_code có đang được dùng trong stage_transitions không
        check_sql = """
...
def add_new_transition(current_stage_id: str, user_intent: str, condition_logic: str | None,
                       ...
                       action_params_str: str | None = None) -> tuple[bool, str | None]:
    """Thêm transition mới với action dạng macro. Trả về (success, error_message)."""
    if not current_stage_id or not user_intent: return False, "Current Stage và User Intent là bắt buộc."
    ...
    if action_macro_code and action_macro_code.strip():
        ...
        if action_params_str and action_params_str.strip():
            ...
            except json.JSONDecodeError: return False, "Action Params không phải là JSON hợp lệ."
        action_data = {"macro_code": action_macro_code.strip(), "params": params_dict}
        try: action_json_str = json.dumps(action_data)
        except TypeError: return False, "Không thể serialize Action Data thành JSON."
    # else: action_json_str là None
    ...
    try:
        ...
        """
        params = (current_stage_id, user_intent, condition_logic or None,
                  next_stage_id if next_stage_id and next_stage_id.strip() else None,
                  priority, action_json_str)
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.Error as e:
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str, condition_logic: str | None,
                      ...
                      action_macro_code: str | None = None,
                      action_params_str: str | None = None) -> tuple[bool, str | None]:
    """Cập nhật transition với action dạng macro. Trả về (success, error_message)."""
    if not transition_id or not current_stage_id or not user_intent: return False, "ID, Current Stage và User Intent là bắt buộc."
    ...
    if action_macro_code and action_macro_code.strip():
        ...
        action_data = {"macro_code": action_macro_code.strip(), "params": params_dict}
        try: action_json_str = json.dumps(action_data)
        except TypeError: return False, "Không thể serialize Action Data."
    # else: action_json_str là None
    ...
    try:
        ...
        params = (current_stage_id, user_intent, condition_logic or None,
                  next_stage_id if next_stage_id and next_stage_id.strip() else None,
                  priority, action_json_str, transition_id)
        cur.execute(sql, params)
        conn.commit()
        success = cur.rowcount > 0
        if not success:
...
def get_transition_details(transition_id: int) -> dict | None:
    """Lấy chi tiết transition, chuẩn bị action_params_str cho form."""
    # ... (Code đã cung cấp ở lần trước, đảm bảo trả về 'action_macro_code' và 'action_params_str') ...
    if not transition_id: return None
    details = None
    conn = get_db_connection()
    if not conn: return None
    ...
    try:
        ...
        sql = """
            SELECT t.*, ss.strategy_id
            FROM stage_transitions t
            LEFT JOIN strategy_stages ss ON t.current_stage_id = ss.stage_id -- Use LEFT JOIN in case stage was deleted
            WHERE t.transition_id = %s;
        """
        cur.execute(sql, (transition_id,))
        row = cur.fetchone()
...
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    """Lấy chuỗi các bước hành động đã được biên dịch sẵn cho chiến lược."""
    # Đây là hàm phức tạp, cần logic để xác định trình tự dựa trên transitions
    # Tạm thời trả về list các action từ transitions để compile_strategy_package dùng
    transitions = get_transitions_for_strategy(strategy_id)
    if transitions is None:
        return None

    action_sequence = []
    for t in transitions:
        action_json_db = t.get('action_to_suggest') # Đây là dict Python từ JSONB
        if isinstance(action_json_db, dict) and 'macro_code' in action_json_db:
            action_step = {
                "macro_code": action_json_db.get('macro_code'),
                "params": action_json_db.get('params', {}),
                # Thêm thông tin trigger để phone có thể tham khảo (tùy chọn)
                "trigger": {"stage": t.get('current_stage_id'), "intent": t.get('user_intent'), "priority": t.get('priority', 0)}
            }
            action_sequence.append(action_step)
    # TODO: Cần logic sắp xếp và xử lý điều kiện/nhánh phức tạp hơn ở đây
    # Tạm thời trả về list thô
    return action_sequence

def get_strategy_version(strategy_id: str) -> str | None:
    """Lấy phiên bản hiện tại của chiến lược (ví dụ đơn giản dùng timestamp)."""
    # Thực tế cần cơ chế versioning tốt hơn
    conn = get_db_connection()
    if not conn: return None
    cur = None
    version = None
    try:
        cur = conn.cursor()
        # Lấy timestamp cập nhật mới nhất của strategy hoặc stages/transitions liên quan
        # Ví dụ: Lấy updated_at của strategy
        sql = "SELECT updated_at FROM strategies WHERE strategy_id = %s;"
        cur.execute(sql, (strategy_id,))
        row = cur.fetchone()
        if row and row[0]:
            version = row[0].isoformat() # Dùng ISO format làm version
        else:
            # Hoặc lấy max(updated_at) từ stages/transitions
            version = datetime.datetime.now(datetime.timezone.utc).isoformat() # Fallback
...
def log_phone_action(session_id, timestamp, macro_code, params_json, execution_status, execution_error=None, current_stage=None, received_state_json=None):
     ...
     try:
          ...
          sql = """
               ...
               (session_id, "timestamp", received_state_json, goal_context, llm_prompt, llm_action_response, execution_status, execution_error, analyzed_state)
               VALUES (%s, %s, %s::jsonb, %s, %s, %s::jsonb, %s, %s, %s::jsonb);
               -- Cần điều chỉnh cột cho phù hợp với logic lưu trữ mới
               -- Ví dụ: Lưu macro_code và params thay vì llm_action_response?
          """
          # Chuyển đổi timestamp từ điện thoại (nếu có) sang kiểu Python datetime
          ts_obj = timestamp # Giả sử timestamp đã là datetime
          if isinstance(timestamp, str):
              try: ts_obj = datetime.datetime.fromisoformat(timestamp)
              except: ts_obj = datetime.datetime.now(datetime.timezone.utc) # Fallback

          # Chuẩn bị dữ liệu JSON để lưu
          state_json_str = json.dumps(received_state_json) if received_state_json else None
          # Lưu macro và params vào cột llm_action_response (hoặc cột riêng)
          action_info_json = json.dumps({"macro_code": macro_code, "params": params_json}) if macro_code else None

          params = (
               session_id,
               ts_obj,
               state_json_str,
               current_stage, # Lưu stage vào goal_context tạm thời?
               None, # Không có LLM prompt trực tiếp
               action_info_json, # Lưu action macro đã chạy
               execution_status,
               execution_error,
               None # Chưa có analyzed_state
          )
          cur.execute(sql, params)
          conn.commit()
          success = True
