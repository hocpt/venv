...
def add_strategy():
    # ... (logic GET và POST giữ nguyên như bước trước) ...
    stages = []
    ...
    # GET request
    # Có thể thêm tham số ?type=control hoặc ?type=language vào link "Thêm mới"
    # để chọn sẵn loại strategy trên form này
    default_type = request.args.get('type', 'language') # Mặc định là language nếu không có param
    return render_template('admin_add_strategy.html', title="Thêm Chiến lược Mới", stages=stages,
                           valid_types=['language', 'control'], default_type=default_type)

# --- Sửa Route Sửa Strategy để xử lý redirect dựa trên type ---
@admin_bp.route('/strategies/<strategy_id>/edit', methods=['GET', 'POST'])
def edit_strategy(strategy_id):
    # ... (logic GET và POST giữ nguyên như bước trước) ...
    strategy = db.get_strategy_details(strategy_id)
    ...
    return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                           strategy=strategy, stages=stages,
                           valid_types=['language', 'control'])

# ... (Các route khác, ví dụ delete_strategy cần quyết định redirect về đâu sau khi xóa) ...
...
def delete_strategy(strategy_id):
    ...
    strategy_type_redirect = strategy_details.get('strategy_type') if strategy_details else 'language' # Default redirect
...
def view_strategies_language():
    ...
    return render_template('admin_strategies_language.html',
                           title=title,
                           strategies=language_strategies)
