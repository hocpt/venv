...
def add_new_strategy(strategy_id: str, name: str, description: str | None, initial_stage_id: str, strategy_type: str) -> tuple[bool, str | None]: # <<< Thêm strategy_type
    ...
    try:
        ...
        # <<< Thêm cột strategy_type vào INSERT >>>
        sql = """
            INSERT INTO strategies (strategy_id, name, description, initial_stage_id, strategy_type, updated_at) -- Thêm updated_at
            VALUES (%s, %s, %s, %s, %s, NOW()); -- Thêm placeholder %s và NOW()
        """
...
def update_strategy(strategy_id: str, name: str, description: str | None, initial_stage_id: str) -> tuple[bool, str | None]:
    ...
    except psycopg2.IntegrityError as int_err: error_msg = f"Lỗi ràng buộc CSDL (Name đã tồn tại?): {int_err}"; if conn: conn.rollback()
    except psycopg2.Error as db_err: error_msg = f"Lỗi CSDL: {db_err}"; if conn: conn.rollback()
    except Exception as e: error_msg = f"Lỗi không xác định: {e}"; if conn: conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success, error_msg
