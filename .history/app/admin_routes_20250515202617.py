    ...
    try:
        ...
        if nodes_page_data:
            for node_data_from_neo4j in nodes_page_data:
                ...
                # node_app_name = node_render_data.get('app_name') # Không cần dùng để tạo URL nữa nếu cấu trúc file phẳng

                logger.debug(f"Node processing: ID={node_render_data.get('screen_id')}, RawScreenshotFile={raw_screenshot_filename}")
                ...
                if raw_screenshot_filename: # Chỉ cần tên file
                    try:
                        ...
                        node_render_data['screenshot_full_url'] = url_for(
                            ...
                            filename=raw_screenshot_filename, # TRUYỀN TRỰC TIẾP TÊN FILE
                            _external=False
                        )
                        logger.debug(f"Generated URL for {raw_screenshot_filename}: {node_render_data['screenshot_full_url']}")
