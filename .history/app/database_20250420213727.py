...
def get_all_devices_for_select() -> list[dict] | None:
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # Lấy ID và Name, sắp xếp theo tên
        cur.execute("SELECT device_id, device_name FROM public.devices ORDER BY device_name, device_id;")
        rows = cur.fetchall()
        devices = [dict(row) for row in rows] if rows else []
    except Exception as e:
        current_app.logger.error(f"Error getting all devices for select: {e}", exc_info=True)
        devices = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
