...
from flask_apscheduler import APScheduler # Giữ lại import này nếu cần kiểu dữ liệu
...
from apscheduler.job import Job # <<< Thêm import Job
...
def run_scheduler():
    ...
    db_config = _get_db_config() # Lấy cấu hình DB
    if not all(db_config.values()): # Kiểm tra thiếu config DB cơ bản
         print("CRITICAL ERROR (scheduler_runner): Missing database configuration in environment. Cannot start scheduler.")
         return

    # Cấu hình logger
    logging.basicConfig(level=logging.INFO, # Đặt INFO hoặc DEBUG tùy nhu cầu
                        format='%(asctime)s %(levelname)-8s %(name)-25s %(threadName)s : %(message)s')
    # Giảm log của apscheduler xuống WARNING để đỡ nhiễu, trừ khi bạn cần debug sâu
