...
def analyze_interactions_and_suggest(app: Flask):
    """
    Tác vụ nền được lên lịch chạy định kỳ.
    Cần đối tượng 'app' để tạo application context.
    """
    # <<< KHÔNG CẦN LẤY app = current_app._get_current_object() nữa >>>
    # app = current_app._get_current_object() # Dòng này bỏ đi
    if not app:
        print(f"CRITICAL ERROR (Task {JOB_ID}): Received invalid Flask app instance.")
        return

    # Sử dụng app_context từ đối tượng app được truyền vào
    with app.app_context():
        # <<< TOÀN BỘ LOGIC CŨ CỦA HÀM NẰM TRONG KHỐI with NÀY >>>
        # Đảm bảo không có lỗi tham chiếu nào đến biến 'app' bên ngoài khối with này nữa
        # (trừ khi bạn muốn truyền nó vào các hàm con)
