...
def get_all_macro_definitions(app_target: str | None = None, include_generic: bool = True) -> list[dict] | None: """Lấy danh sách định nghĩa macro, tùy chọn lọc theo app và bao gồm generic/system.""" macros_list = None conn = get_db_connection() if not conn: return None cur = None try: cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor) base_sql = "SELECT macro_code, description, app_target, notes FROM public.macro_definitions" where_clauses = [] params = [] filter_targets = [] if app_target: filter_targets.append(app_target) if include_generic: filter_targets.extend(['system', 'generic']) # Thêm '' và NULL nếu bạn dùng chúng thay vì 'system'/'generic' # filter_targets.extend(['system', 'generic', '', None]) if filter_targets: # Xử lý trường hợp có target cụ thể và cả generic/system # Cần đảm bảo query đúng nếu app_target có thể là NULL hoặc '' target_conditions = [] valid_targets_in_params = [] # Chỉ thêm vào params nếu không phải None hoặc '' for target in filter_targets: if target is None: target_conditions.append("app_target IS NULL") elif target == '': target_conditions.append("app_target = ''") else: target_conditions.append("app_target = %s") valid_targets_in_params.append(target) # Thêm vào list để truyền cho execute where_clauses.append(f"({ ' OR '.join(target_conditions) })") params.extend(valid_targets_in_params) # Chỉ truyền các giá trị hợp lệ if where_clauses: sql = f"{base_sql} WHERE {' AND '.join(where_clauses)} ORDER BY app_target, macro_code;" else: sql = f"{base_sql} ORDER BY app_target, macro_code;" # print(f"DEBUG SQL (get_all_macros): {cur.mogrify(sql, tuple(params)).decode('utf-8', 'ignore')}") # Bỏ comment để debug SQL cur.execute(sql, tuple(params)) rows = cur.fetchall() macros_list = [dict(row) for row in rows] if rows else [] except Exception as e: print(f"ERROR (db - get_all_macro_definitions): {e}"); macros_list = None finally: if cur: cur.close() if conn: conn.close() return macros_list
...
def add_new_transition(current_stage_id: str, user_intent: str, condition_logic: str | None,
    ...
    if action_macro_code and action_macro_code.strip():
        ...
        # Chỉ lưu dict vào action_json_for_db, DB sẽ tự chuyển sang JSONB
    ...
    try:
        ...
        sql = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s); -- Không cần ép kiểu ::jsonb nếu cột đã là jsonb
        """
        params = (current_stage_id, user_intent, condition_logic or None,
                  ...
                  priority,
                  # Truyền dict hoặc None, psycopg2 sẽ xử lý JSONB
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str, condition_logic: str | None,
    ...
    try:
        ...
        sql = """
            UPDATE stage_transitions
            SET current_stage_id = %s, user_intent = %s, condition_logic = %s, next_stage_id = %s,
                priority = %s, action_to_suggest = %s
            WHERE transition_id = %s;
        """
        params = (current_stage_id, user_intent, condition_logic or None,
                  next_stage_id if next_stage_id and next_stage_id.strip() else None,
...
def get_transition_details(transition_id: int) -> dict | None:
    ...
    # ... (Giữ nguyên code đã cung cấp ở lần trước, nó đã xử lý đúng) ...
    if not transition_id: return None
    details = None
    conn = get_db_connection()
    if not conn: return None
    ...
    try:
        ...
        sql = """
            SELECT t.*, ss.strategy_id
            FROM stage_transitions t
            LEFT JOIN strategy_stages ss ON t.current_stage_id = ss.stage_id
            WHERE t.transition_id = %s;
        """
        cur.execute(sql, (transition_id,))
        row = cur.fetchone()
...
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    """Lấy chuỗi các bước hành động (macro_code + params) cho strategy."""
    ...
    action_sequence = []
    for t in sorted(transitions, key=lambda x: (x.get('current_stage_id', ''), -x.get('priority', 0))): # Sắp xếp cơ bản
        action_json_db = t.get('action_to_suggest')
        if isinstance(action_json_db, dict) and 'macro_code' in action_json_db:
            action_step = {
                ...
                "next_stage_id": t.get('next_stage_id'), # Thêm next_stage để phone engine biết
                "condition": t.get('condition_logic') # Thêm condition để phone engine kiểm tra
            }
