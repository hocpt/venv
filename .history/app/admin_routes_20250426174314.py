...
def add_scheduled_job():
    ...
    available_tasks = list(AVAILABLE_SCHEDULED_TASKS.items()) # AVAILABLE_SCHEDULED_TASKS đã định nghĩa ở trên
    available_tasks.sort()
    valid_trigger_types = ['interval', 'date', 'cron']
    saved_simulation_configs = [] # <<< THÊM: List để chứa các config sim đã lưu
    try:
        # <<< THÊM: Lấy danh sách config simulation đã lưu >>>
        configs_from_db, _ = db.get_all_simulation_configs(page=1, per_page=10000) # Lấy hết
        if configs_from_db:
        ...
        saved_simulation_configs = [] # Đảm bảo là list rỗng
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            errors = []
            # ... (validate job_id, function_path, trigger_type, trigger_args) ...
            ...
            job_args_str_final = None # Biến lưu job_args cuối cùng để truyền vào DB

            # === KIỂM TRA FUNCTION PATH ĐỂ XỬ LÝ JOB ARGS ===
            if job_function_path == SIMULATION_FUNCTION_PATH:
                # Lấy config_id từ dropdown mới
                ...
                if not config_id_str:
                    ...
                    try:
                        ...
                        # Lấy chi tiết config từ DB
                        config_details = db.get_simulation_config(config_id)
                        if not config_details:
                            errors.append(f"Không tìm thấy chi tiết cho Cấu hình Mô phỏng ID {config_id}.")
                        else:
                            # === Tạo job_args từ config_details ===
                            job_args = {
                                ...
                                'sim_thread_id_base': f"scheduled_sim_{config_id}", # Tạo base ID riêng
                                'sim_goal': config_details.get('simulation_goal') or f"scheduled_run_{config_id}"
                            }
                            # Kiểm tra lại các trường bắt buộc trong job_args
                            if not all(job_args[k] for k in ['persona_a_id', 'persona_b_id', 'log_account_id_a', 'log_account_id_b', 'strategy_id']):
                                errors.append(f"Cấu hình Mô phỏng ID {config_id} thiếu thông tin cần thiết.")
                            else:
                                job_args_str_final = json.dumps(job_args) # Chuyển thành chuỗi JSON
                    except ValueError:
                ...
                # Lấy job_args từ textarea như cũ
                ...
                if job_args_str_textarea and job_args_str_textarea.strip() != '{}':
                    try:
                        ...
                        job_args_str_final = job_args_str_textarea # Giữ nguyên chuỗi nếu hợp lệ
                    except: errors.append("Job Args JSON không hợp lệ.")
                # Nếu textarea rỗng hoặc '{}', job_args_str_final sẽ là None
            # ==============================================

            if errors:
                for error in errors: flash(error, "warning")
                # <<< Truyền thêm saved_simulation_configs khi render lỗi >>>
            ...
            # === Gọi hàm DB add_job_config với job_args_str_final ===
            try:
                ...
                logger.error(f"Lỗi khi thêm scheduled job: {e}", exc_info=True)
