...
def get_assignment_logs(assignment_id: int, page: int = 1, per_page: int = 50) -> tuple[list[dict] | None, int | None]:
    ...
    try:
        ...
        if total_items > 0:
            ...
            data_sql = """
                SELECT log_id, "timestamp", current_stage, action_macro_code, params_json, execution_status, execution_error
            ...
            # Chuyển đổi params_json nếu cần (thường DictCursor tự làm)
            for log_entry in logs:
                if log_entry.get('params_json') and isinstance(log_entry['params_json'], str):
                    ...
                    except: log_entry['params_json'] = {} # Hoặc giữ nguyên string nếu parse lỗi
        ...
        log_func = current_app.logger.error if current_app else print
        log_func(f"DB Error fetching logs for assignment {assignment_id}: {db_err}", exc_info=True)
        logs, total_items = None, None
    except Exception as e:
        log_func = current_app.logger.error if current_app else print
        log_func(f"Unexpected Error fetching logs for assignment {assignment_id}: {e}", exc_info=True)
        logs, total_items = None, None
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def get_assignment_logs(assignment_id: int, page: int = 1, per_page: int = 50) -> tuple[list[dict] | None, int | None]:
    ...
    try:
        # Query đếm tổng số log cho assignment này
        cur = conn.cursor()
        # Đảm bảo tên cột assignment_id trong phone_action_log là đúng
        count_sql = "SELECT COUNT(*) FROM public.phone_action_log WHERE assignment_id = %s;"
        cur.execute(count_sql, (assignment_id,))
        total_items = cur.fetchone()[0]
        cur.close()
        ...
        if total_items > 0:
            cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
            offset = (page - 1) * per_page
            # Lấy các cột cần thiết từ phone_action_log
            # Sắp xếp theo timestamp giảm dần (mới nhất lên đầu)
            data_sql = """
                SELECT log_id, "timestamp", current_stage, action_macro_code, params_json, execution_status, execution_error
                FROM public.phone_action_log
            ...
            rows = cur.fetchall()
            logs = [dict(row) for row in rows] if rows else []
            # Chuyển đổi JSONB thành dict nếu driver chưa làm
            for log_entry in logs:
                if log_entry.get('params_json') and isinstance(log_entry['params_json'], str):
                    try: log_entry['params_json'] = json.loads(log_entry['params_json'])
                    except: log_entry['params_json'] = {} # Gán dict rỗng nếu parse lỗi
                elif not isinstance(log_entry.get('params_json'), dict):
                    log_entry['params_json'] = {} # Đảm bảo là dict

    except psycopg2.Error as db_err:
        # Kiểm tra xem có current_app context không trước khi dùng logger
        log_func = current_app.logger.error if current_app else print
        log_func(f"DB Error fetching logs for assignment {assignment_id}: {db_err}", exc_info=True)
        logs, total_items = None, None
    except Exception as e:
