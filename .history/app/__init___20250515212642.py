...
def create_app(config_class=config.Config):
    ...
    def serve_screenshot_for_app(filename): # Tên hàm này sẽ là tên endpoint
        logger = current_app.logger # Hoặc app.logger nếu app được truyền vào
        # SCREENSHOT_STORAGE_PATH là thư mục gốc: .../app/static/screenshots/
        ...
        logger.info(f"[Serve Screenshot] Request for App: '{app_name}', File: '{filename}'")
        
        # Tạo đường dẫn an toàn đến thư mục của app cụ thể
        # filename ở đây chỉ là tên file (ví dụ: image.png)
        # app_name là tên thư mục con (ví dụ: com.example.app)
        # send_from_directory sẽ tìm kiếm filename bên trong thư mục được chỉ định
        # Thư mục cần truyền vào send_from_directory là .../app/static/screenshots/APP_NAME/
        app_specific_screenshot_dir = os.path.join(screenshots_base_dir, app_name)
        
        # Kiểm tra an toàn cơ bản cho filename (dù send_from_directory cũng có kiểm tra)
        secure_filename = os.path.normpath(filename)
        if secure_filename.startswith("..") or os.path.isabs(secure_filename) or '/' in secure_filename or '\\' in secure_filename:
            logger.error(f"[Serve Screenshot] Invalid filename characters in '{filename}'")
            return "Filename không hợp lệ", 400

        logger.debug(f"[Serve Screenshot] Serving from directory: '{app_specific_screenshot_dir}', file: '{secure_filename}'")

        # Kiểm tra xem thư mục app có tồn tại không (tùy chọn, send_from_directory sẽ báo 404 nếu path sai)
        if not os.path.isdir(app_specific_screenshot_dir):
            logger.error(f"[Serve Screenshot] App specific directory not found: '{app_specific_screenshot_dir}'")
            return "Application resources not found", 404
            
        try:
            return send_from_directory(app_specific_screenshot_dir, secure_filename, as_attachment=False)
        except FileNotFoundError: # Bắt lỗi cụ thể hơn
            logger.error(f"[Serve Screenshot] 404 File Not Found: '{os.path.join(app_specific_screenshot_dir, secure_filename)}'")
            return "File ảnh không tìm thấy", 404
        except Exception as e:
            logger.error(f"[Serve Screenshot] Error sending file '{secure_filename}' from '{app_specific_screenshot_dir}': {e}", exc_info=True)
