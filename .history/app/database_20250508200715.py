...
def get_last_detailed_ui_state_for_screen(screen_id: str) -> dict | None:
    ...
    try:
        ...
        sql = """
            SELECT received_state_json
            FROM public.phone_action_log
            WHERE assignment_id = %s -- Lọc theo assignment_id liên quan đến screen_id
            AND received_state_json IS NOT NULL
            AND jsonb_typeof(received_state_json) = 'object'
            AND received_state_json::text <> '{}'::text
        ...
        if row and row['received_state_json']:
            ...
            if isinstance(row['received_state_json'], dict):
                ...
                try: last_state_dict = json.loads(row['received_state_json'])
                except json.JSONDecodeError: pass # Bỏ qua nếu parse lỗi

            # === KIỂM TRA CẤU TRÚC ===
            if isinstance(last_state_dict, dict) and \
                'elements' in last_state_dict and \
                isinstance(last_state_dict['elements'], list):
                logger.debug(f"Tìm thấy state hợp lệ chứa elements list.")
                return last_state_dict # Chỉ trả về nếu hợp lệ
            else:
                logger.warning(f"State gần nhất tìm thấy không hợp lệ hoặc thiếu key 'elements'. State: {str(row['received_state_json'])[:200]}...")
                return None # Trả về None nếu không hợp lệ
        else:
            logger.info(f"Không tìm thấy log nào có state gần đây.")
            return None
