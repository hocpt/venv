...
def admin_screen_elements(screen_id):
    ...
    original_width = None # << Thêm biến
    original_height = None # << Thêm biến
    if not graph_db or not db or not ai_service: # Kiểm tra các module
    ...
    try:
        ...
        original_width = screen_data.get('original_width')   # << Lấy kích thước gốc
        original_height = screen_data.get('original_height') 
        # === TẠO SCREENSHOT URL DÙNG ENDPOINT TÙY CHỈNH ===
        # screen_data.get('screenshot_path') giờ chỉ là tên file (ví dụ: 'abc.png')
        ...
        if filename_from_neo4j:
            try:
                # Sử dụng endpoint 'serve_app_specific_screenshot' đã định nghĩa trong app/__init__.py
                screenshot_url = url_for('serve_app_specific_screenshot', filename=filename_from_neo4j)
                logger.info(f"[admin_screen_elements] Đã tạo URL screenshot bằng 'serve_app_specific_screenshot': '{screenshot_url}'")
                elements_json_data = json.dumps(elements_list)
            except Exception as url_err:
                logger.error(f"[admin_screen_elements] Lỗi khi tạo URL cho screenshot path '{filename_from_neo4j}' bằng endpoint tùy chỉnh: {url_err}", exc_info=True)
                screenshot_url = None
        else:
            logger.info(f"[admin_screen_elements] Không tìm thấy screenshot_path cho màn hình {screen_id}.")
        # ===================================================

        # ... (Lấy transitions, state từ log, classifications từ PG, hợp nhất dữ liệu như cũ) ...
        # Đoạn code này cần được giữ lại và đảm bảo các biến như app_name, db, ai_service có sẵn
        tried_element_ids = set()
        if app_name: # Cần app_name để lấy outgoing_transitions
            outgoing_transitions = graph_db.get_outgoing_transitions(screen_id, app_name)
            if outgoing_transitions is None: outgoing_transitions = []
            ...
            logger.warning(f"app_name không có cho screen {screen_id}, không thể lấy outgoing transitions.")
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            if saved_data is None: saved_data = {} # Xử lý trường hợp lỗi DB
            for el_from_log in elements_list_from_log:
                ...
                if el_id:
                    ...
                    else: display_explored_status = auto_explored_status; override_active = 'auto'
    ...
    # Render template, truyền thêm screenshot_url
    return render_template('admin_screen_elements.html',
                           screen=screen_data,
                           elements=elements_list,
                           valid_classifications=valid_classifications,
                           screenshot_url=screenshot_url,original_width=original_width,original_height=original_height,elements_json_data=elements_json_data)
