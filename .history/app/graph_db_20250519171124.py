...
def get_screen_nodes_for_management(app_name: str | None, 
    ...
    """
    Lấy danh sách các Node :Screen từ Neo4j với tùy chọn lọc và phân trang.
    Nếu node đã 'defined', sẽ query PostgreSQL để lấy 'logical_pie_name'.
    """
    logger = current_app.logger
    driver = get_driver() # Hàm get_driver() của bạn để lấy Neo4j driver
    if not driver:
    ...
    # Xây dựng mệnh đề WHERE động cho Cypher
    where_clauses = []
    params_cypher = {"skip_val": skip, "limit_val": per_page}
    ...
    # Lấy các trường cần thiết, bao gồm cả screen_id và app_name để query PIE def sau
    data_query = f"""
        MATCH (s:Screen)
        {where_statement}
        RETURN 
            elementId(s) AS node_neo4j_id, 
    ...
    try:
        ...
        with driver.session(database=db_name_neo4j) as session:
            # Thực thi query đếm
            logger.debug(f"Executing Neo4j count query for node management: {count_query.strip()} with params {params_cypher}")
            ...
            if total_items > 0:
                logger.debug(f"Executing Neo4j data query for node management: {data_query.strip()} with params {params_cypher}")
                data_result = session.run(data_query, params_cypher)
                for record in data_result:
                    raw_nodes_from_neo4j.append(dict(record)) # Chuyển Record sang dict
                    logger.debug(f"GRAPH_DB raw record: screen_id='{record_dict.get('screen_id')}', node_neo4j_id='{record_dict.get('node_neo4j_id')}'")
                    raw_nodes_from_neo4j.append(record_dict)
        
        # Xử lý raw_nodes_from_neo4j để thêm thông tin PIE và chuyển đổi datetime
        processed_nodes_list = []
        if raw_nodes_from_neo4j:
            for node_data_neo4j in raw_nodes_from_neo4j:
                node_dict = dict(node_data_neo4j) # Đảm bảo là một dict copy
                
                # Lấy thông tin PIE Definition nếu node đã defined
                node_status = node_dict.get('status')
                # Các status cho biết node đã được "defined" bởi một PIE
                defined_statuses = ['defined', 'defined_from_unknown', 'merged_to_defined'] 

                if node_status in defined_statuses:
                    current_screen_id_for_pie = node_dict.get('screen_id') # Đây chính là defined_screen_id
                    current_app_name_for_pie = node_dict.get('app_name')
                    if current_screen_id_for_pie and current_app_name_for_pie:
                        try:
                            # Gọi hàm từ database.py (PostgreSQL)
                            pie_def_details = db_pg.get_screen_definition_by_defined_id(
                                current_screen_id_for_pie, 
                                current_app_name_for_pie
                            )
                            if pie_def_details:
                                node_dict['logical_pie_name'] = pie_def_details.get('logical_screen_name')
                                # node_dict['pie_definition_id_pg'] = pie_def_details.get('definition_id') # ID của bản ghi trong PostgreSQL
                            else:
                                # Node có status 'defined' nhưng không tìm thấy PIE def tương ứng -> Cảnh báo
                                logger.warning(f"Node '{current_screen_id_for_pie}' (app: {current_app_name_for_pie}) có status '{node_status}' nhưng không tìm thấy PIE Definition trong PostgreSQL.")
                                node_dict['logical_pie_name'] = "Lỗi: PIE Def không khớp"
                        except Exception as e_pie_lookup:
                            logger.error(f"Lỗi khi query PostgreSQL để lấy PIE def cho {current_app_name_for_pie}/{current_screen_id_for_pie}: {e_pie_lookup}", exc_info=True)
                            node_dict['logical_pie_name'] = "Lỗi DB (PIE Def)"
                else: # Nếu là unknown hoặc status khác
                    node_dict['logical_pie_name'] = None 

                # Xử lý chuyển đổi datetime object sang ISO string
                for dt_field in ['created_at', 'last_seen']:
                        ...
                        try:
                            if hasattr(raw_dt_val, 'to_native') and callable(raw_dt_val.to_native):
                                ...
                                if isinstance(native_dt, datetime):
                                    if native_dt.tzinfo is None: native_dt = native_dt.replace(tzinfo=timezone.utc)
                                    node_dict[dt_field] = native_dt.isoformat()
                                else: node_dict[dt_field] = str(native_dt) 
                            elif isinstance(raw_dt_val, datetime):
                                if raw_dt_val.tzinfo is None: raw_dt_val = raw_dt_val.replace(tzinfo=timezone.utc)
                            ...
                            logger.warning(f"Could not convert datetime field '{dt_field}' (value: {raw_dt_val}, type: {type(raw_dt_val)}) to ISO string for screen {node_dict.get('screen_id')}: {e_dt_convert}")
                            node_dict[dt_field] = str(raw_dt_val) # Giữ lại dạng string nếu lỗi
                
                processed_nodes_list.append(node_dict)
        
        logger.debug(f"get_screen_nodes_for_management: Fetched and processed {len(processed_nodes_list)} nodes for page {page}. Total items: {total_items}")
        return processed_nodes_list, total_items
            
    except ServiceUnavailable as su_err:
        logger.error(f"Neo4j ServiceUnavailable (get_screen_nodes_for_management): {su_err}", exc_info=True)
        return None, None
    except Neo4jError as neo_err: # Bắt các lỗi Neo4j khác
        logger.error(f"Neo4jError (get_screen_nodes_for_management): Code={neo_err.code}, Message={neo_err.message}", exc_info=True)
        return None, None
    except Exception as e:
        logger.error(f"Lỗi không mong muốn khi lấy screen nodes cho management: {e}", exc_info=True)
        return None, None
...
def convert_unknown_to_defined_node_wrapper(
    ...
    # finally: # Không close driver ở đây, driver nên là singleton
    #     if driver:
    #         driver.close() 
