...
def view_screen_definitions():
    ...
    # Lấy danh sách app_name cho dropdown lọc (tùy chọn, có thể lấy từ Neo4j hoặc bảng khác)
    # distinct_app_names = graph_db.get_distinct_app_names() or [] 
    # Tạm thời để trống, bạn có thể thêm sau
    distinct_app_names = [] 
    ...
    if selected_app_filter:
        # Gọi hàm db để lấy definitions đã lọc theo app (hoặc bạn có thể lấy tất cả rồi lọc ở Python nếu ít)
        # Tạm thời lấy tất cả nếu không có filter, hoặc bạn sửa get_screen_definitions_for_app để lấy hết nếu app_name rỗng
        definitions_list = db.get_screen_definitions_for_app(selected_app_filter)
        if definitions_list is None: # Lỗi DB
            flash("Lỗi khi tải danh sách định nghĩa màn hình.", "error")
            definitions_list = []
    elif not selected_app_filter: # Nếu không chọn filter, có thể lấy tất cả hoặc yêu cầu chọn
         # Để đơn giản, tạm thời không lấy gì nếu chưa chọn app
         pass # Hoặc: definitions_list = db.get_all_screen_definitions() # Cần viết hàm này

    # Lấy danh sách các app_name đã có trong bảng screen_definitions để tạo filter
    try:
        conn = db.get_db_connection()
        if conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute("SELECT DISTINCT app_name FROM screen_definitions ORDER BY app_name;")
                distinct_app_names = [row['app_name'] for row in cur.fetchall()]
            conn.close()
    except Exception as e:
        current_app.logger.error(f"Error fetching distinct app_names for screen_definitions filter: {e}")


    # TODO: Phân trang nếu danh sách definitions_list quá dài
    return render_template('admin_screen_definitions.html', 
                           title=title, 
                           definitions=definitions_list,
                           distinct_app_names=distinct_app_names,
...
def api_get_screen_definition_detail(def_id):
    ...
    if definition:
        ...
        if definition.get('created_at'): definition['created_at'] = definition['created_at'].isoformat()
        if definition.get('updated_at'): definition['updated_at'] = definition['updated_at'].isoformat()
...
@admin_bp.route('/mapping/screen-definitions')
# @admin_required # Bỏ comment nếu bạn đã triển khai xác thực admin
def view_screen_definitions():
    title = "Quản lý Định nghĩa Màn hình (PIE)"
    ...
    try:
        conn = db.get_db_connection() # Sử dụng hàm get_db_connection từ database.py
        if conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute("SELECT DISTINCT app_name FROM screen_definitions ORDER BY app_name;")
    ...
    definitions_list = []
    if selected_app_filter:
        # Gọi hàm db để lấy definitions đã lọc theo app
        # Hàm get_screen_definitions_for_app đã được thiết kế để xử lý việc này
        # (nó sẽ trả về cả những defs có activity_name và cả những defs có activity_name là NULL cho app đó)
        definitions_list = db.get_screen_definitions_for_app(selected_app_filter, activity_name=None) # Lấy tất cả cho app này
        if definitions_list is None: # Lỗi DB
            flash(f"Lỗi khi tải danh sách định nghĩa cho app '{selected_app_filter}'.", "error")
            definitions_list = []
    elif not selected_app_filter and distinct_app_names_for_filter: # Nếu chưa chọn filter nhưng có app để chọn
        flash("Vui lòng chọn một App Name từ bộ lọc để xem các định nghĩa.", "info")
    elif not distinct_app_names_for_filter: # Nếu không có app nào trong DB
        flash("Chưa có định nghĩa màn hình nào được tạo. Hãy bắt đầu bằng cách thêm mới.", "info")


    # Cân nhắc việc phân trang ở đây nếu definitions_list quá lớn
    # Tạm thời hiển thị tất cả nếu đã lọc theo app

    return render_template('admin_screen_definitions.html', 
                           title=title, 
                           definitions=definitions_list, # Danh sách đã lọc (hoặc rỗng)
                           distinct_app_names=distinct_app_names_for_filter, # Cho dropdown filter
                           selected_app_filter=selected_app_filter) # Để giữ lại giá trị filter