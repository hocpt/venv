...
# --- Các hàm truy vấn/tìm kiếm cơ bản (Bạn có thể thêm sau) ---
def find_screen_node(screen_id: str) -> dict | None:
    """Tìm và trả về thuộc tính của một Screen node bằng screenId."""
    cypher = "MATCH (s:Screen {screenId: $screenId}) RETURN s"
    params = {"screenId": screen_id}
    results = execute_read(cypher, params) # Dùng hàm helper execute_read
    if results:
        # execute_read trả về list các dict, mỗi dict chứa {'s': properties}
        return results[0].get('s')
    return None

def get_all_screen_ids() -> list[str] | None:
    """Lấy danh sách tất cả screenId hiện có."""
    cypher = "MATCH (s:Screen) RETURN s.screenId as screenId ORDER BY s.screenId"
    results = execute_read(cypher, params)
    if results:
        return [r.get('screenId') for r in results if r.get('screenId')]
    elif results == []: # Không có node nào
        return []
    else: # Lỗi đọc DB
        return None