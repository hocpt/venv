...
def load_scheduled_jobs(app):
    ...
    if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        try: # Thêm try...except ở đây cho chắc chắn
            scheduler.start()
...
def create_app(config_class=config.Config):
    ...
    # --- Khởi tạo Scheduler với App ---
    print("DEBUG: Initializing scheduler with app...")
    scheduler.init_app(app)
    print("DEBUG: Scheduler initialized.")

    # --- Bắt buộc Start Scheduler và Load Jobs (DEBUGGING) ---
    print("DEBUG: !!! Forcing scheduler start and job loading (DEBUGGING STEP) !!!")
    try:
        # Bắt đầu scheduler trước khi thêm job
        scheduler.start(paused=True) # Khởi động ở trạng thái tạm dừng để tránh race condition? Hoặc start bình thường. Thử start bình thường trước.
        scheduler.start()
    ...
    # --- Đăng ký Blueprint ---
    # Tạm thời vẫn đăng ký để các route hoạt động
