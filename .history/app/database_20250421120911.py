...
def get_all_task_assignments(filters: dict = None, page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    ...
    select_sql = """ SELECT ta.assignment_id, ta.status, ta.priority, ta.strategy_id, s.name as strategy_name, da.device_id, d.device_name, da.account_id, acc.username as account_username, acc.platform as account_platform, ta.target_data, -- <<< Lấy cả target_data (JSONB) ta.result_data, -- <<< THAY THẾ: Lấy result_data (JSONB) thay vì error_message ta.created_at, ta.assigned_at, ta.last_report_at, ta.completed_at -- Bỏ cột ta.error_message không tồn tại FROM public.task_assignments ta JOIN public.device_accounts da ON ta.device_account_id = da.device_account_id JOIN public.devices d ON da.device_id = d.device_id JOIN public.accounts acc ON da.account_id = acc.account_id LEFT JOIN public.strategies s ON ta.strategy_id = s.strategy_id """
