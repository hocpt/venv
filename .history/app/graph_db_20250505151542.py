...
# --- HÀM MERGE NODE SCREEN (Chuẩn hóa) ---
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
    ...
    Khởi tạo list elements nếu node mới được tạo.
    """
    ...
    # Chuẩn bị thuộc tính cơ bản
    current_time = datetime.now(timezone.utc)
    screen_props = {
        "screen_id": screen_id, # Dùng để MERGE
        "app_name": app_name,
        "activity_name": activity_name or 'UnknownActivity',
        "status": 'provisional', # Mặc định khi tạo/match lần đầu
        "last_analyzed_log_id": log_id,
        "last_seen": current_time, # Cập nhật thời gian thấy lần cuối
        # "structure_hash": screen_id, # Có thể dùng screen_id làm hash cấu trúc
        "element_count": len(extracted_elements) if extracted_elements else 0
    }

    # Cấu trúc elements chuẩn hóa (chỉ chứa các key cần lưu)
        ...
        for el in extracted_elements:
            if isinstance(el, dict) and el.get('element_id'):
                standardized_elements.append({
                    ...
                    'coordinates': el.get('coordinates'), # Thêm tọa độ
                     # Khởi tạo các trường trạng thái/phân loại
    ...
    try:
        with driver.session(database=current_app.config.get('NEO4J_DATABASE', 'neo4j')) as session:
            # Dùng MERGE dựa trên screen_id
            # ON CREATE: Đặt các giá trị ban đầu, bao gồm cả list elements mới
            # ON MATCH: Cập nhật các giá trị thay đổi (last_seen, log_id, activity, element_count, elements)
            # Việc cập nhật elements phức tạp, tạm thời ghi đè bằng list mới nhất mỗi lần thấy screen
            # (Cách tốt hơn là merge list trong Python rồi SET lại, nhưng phức tạp hơn)
            query = """
            ...
            ON CREATE SET
                ...
                s.elements = $elements_list, // Khởi tạo list elements
                s.element_count = $props.element_count,
                s.last_analyzed_log_id = $props.last_analyzed_log_id,
                s.last_seen = $props.last_seen,
                s.created_at = $props.last_seen // Dùng last_seen làm created_at
            ON MATCH SET
                s.app_name = $props.app_name, // Đảm bảo app_name đúng
                s.activity_name = $props.activity_name,
                // Không nên đổi status ở đây, cần logic riêng
                s.elements = $elements_list, // <<< TẠM THỜI GHI ĐÈ LIST ELEMENTS
                s.element_count = $props.element_count,
                s.last_analyzed_log_id = $props.last_analyzed_log_id,
                s.last_seen = $props.last_seen,
                s.updated_at = $props.last_seen // Dùng last_seen làm updated_at
            """
            session.run(query, props=screen_props, elements_list=standardized_elements)
            logger.debug(f"Merged Screen node: {screen_id}")
            return True
...
def get_app_graph_data(app_name: str) -> dict | None:
    ...
    định dạng cho Cytoscape.js. Đã sửa lỗi cú pháp Cypher cho edge_query.
    ...
    logger = current_app.logger if current_app else print
    if not app_name:
        logger.warning("get_app_graph_data called with empty app_name.")
        return None

    driver = get_driver()
    if not driver:
        logger.error("get_app_graph_data: Failed to get Neo4j driver.")
        return None
    ...
    try:
        with driver.session(database=current_app.config.get('NEO4J_DATABASE', 'neo4j')) as session:
            ...
            # Ưu tiên dùng screen_id làm định danh
            node_query = """
            ...
            for record in nodes_result:
                node_id = record["id"]
                if node_id is None:
                    # Log chi tiết hơn về node bị thiếu ID nếu có thể (ví dụ: elementId(n))
                    logger.warning(f"WARNING (get_app_graph_data): Found Screen node with missing 'screen_id' property for app {app_name}. Skipping.")
                    continue # Bỏ qua node lỗi này

                element_count = record["element_count"]
                if element_count is None:
                     logger.warning(f"WARNING (get_app_graph_data): Node {node_id} missing 'elements' property. Setting element_count to 0.")
                     element_count = 0

                node_data = {
                    "id": node_id,
                    "activity": record["activity"], # Có thể None
                    "status": record["status"],     # Có thể None
                    "element_count": element_count,
                    "label": node_id[:8] # Lấy 8 ký tự đầu làm nhãn
                }
                # Chỉ thêm vào nếu giá trị là hợp lệ (không None/rỗng)
                node_data_clean = {k: v for k, v in node_data.items() if v is not None and v != ''}
                cy_elements["nodes"].append({"data": node_data_clean})

            # 2. Lấy tất cả TRANSITION edges giữa các Screen nodes của app này
            # <<< SỬA LẠI QUERY EDGE Ở ĐÂY >>>
            edge_query = """
                ...
                RETURN a.screen_id AS source,
                       ...
                       r.action_details AS action,
                       r.status AS status,
                       elementId(r) AS neo4j_edge_id
            """
            logger.debug(f"Executing Edge Query for app '{app_name}':\n{edge_query}")
            edges_result = session.run(edge_query, app_name=app_name) # <<< Lỗi SyntaxError xảy ra ở đây nếu query sai
            edge_counter = 0
            for record in edges_result:
                # Kiểm tra xem source và target có tồn tại không (quan trọng)
                source_id = record["source"]
                target_id = record["target"]
                if not source_id or not target_id:
                     logger.warning(f"Skipping edge due to missing source ('{source_id}') or target ('{target_id}') screen_id.")
                ...
                if isinstance(action_details, dict):
                    clicked_element_id = action_details.get('element_id') or action_details.get('onElementId') or action_details.get('target', {}).get('resource_id')
                ...
                edge_data = {
                    ...
                    # Chỉ thêm element_id nếu nó thực sự tồn tại
                    **({"element_id": clicked_element_id} if clicked_element_id else {})
                }
                # Loại bỏ key có giá trị None/rỗng nếu muốn
                edge_data_clean = {k: v for k, v in edge_data.items() if v is not None and v != ''}
                cy_elements["edges"].append({"data": edge_data_clean})

    except neo4j.exceptions.CypherSyntaxError as syn_err: # Bắt lỗi cú pháp cụ thể
        logger.error(f"Neo4j Cypher Syntax Error (get_app_graph_data): {syn_err}")
        # In ra query gây lỗi để dễ debug
        if 'edge_query' in locals(): logger.error(f"Failed Edge Query was likely:\n{edge_query}")
        elif 'node_query' in locals(): logger.error(f"Failed Node Query was likely:\n{node_query}")
        return None # Trả về None khi lỗi cú pháp
    except ServiceUnavailable as e:
    ...
    logger.info(f"GraphDB: Found {len(cy_elements['nodes'])} nodes and {len(cy_elements['edges'])} edges for app '{app_name}'.")
    return cy_elements
