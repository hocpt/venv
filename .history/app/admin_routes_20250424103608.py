...
def view_strategy_definition_json(strategy_id):
    """
    Trả về định nghĩa thô của một strategy (details, stages, transitions)
    dưới dạng JSON, đã xử lý datetime.
    """
    logger = current_app.logger if current_app else print
    if not db:
        return jsonify({"error": "Database module not available"}), 500
    ...
    try:
        # 1. Lấy dữ liệu gốc từ DB (vẫn chứa datetime objects)
        ...
        # 2. Chuyển đổi datetime thành chuỗi ISO trong dữ liệu đã lấy
        ...
        # 3. Tạo đối tượng dictionary cuối cùng với dữ liệu đã chuyển đổi
        strategy_definition = {
        ...
        # 4. Trả về dưới dạng JSON (Dùng json.dumps để format đẹp)
