...
#@admin_required # <<< TẠM THỜI comment out dòng này để kiểm tra xem có phải lỗi do auth không
def api_update_task_status(assignment_id):
    ...
    try:
        ...
        if success:
             ...
             # Trả về lỗi 404 nếu không tìm thấy ID, 500 nếu lỗi DB khác
        ...
        # Bắt lỗi không mong muốn và trả về JSON error 500
        current_app.logger.error(f"Exception in api_update_task_status for ID {assignment_id}: {e}", exc_info=True)
        traceback.print_exc() # In traceback ra console/log server
