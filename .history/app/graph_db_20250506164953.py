...
def merge_transition(source_screen_id: str, target_screen_id: str, app_name: str,
    ...
    Sửa lỗi CypherTypeError do lưu Map vào thuộc tính relationship.
    """
    ...
    # --- 1. Chuẩn bị các thuộc tính để MERGE và SET ---
    # Thuộc tính dùng để xác định duy nhất loại transition (phải là primitive)
    merge_props = {
        'actionType': action_details.get('actionType'),
        # Ưu tiên lấy element_id từ các key có thể có
    ...
    # Loại bỏ key có giá trị None khỏi merge_props
    merge_props = {k: v for k, v in merge_props.items() if v is not None}

    # Kiểm tra các key bắt buộc cho MERGE
    if not merge_props.get('actionType'):
        logger.warning(f"Cannot MERGE transition: 'actionType' is missing in action_details: {action_details}")
        return False
    # Có thể nới lỏng nếu actionType là 'back' hoặc 'swipe' không cần element_id/identifier_type
    # if merge_props['actionType'] not in ['NAV_GO_BACK', 'UI_SWIPE_UP', 'UI_SWIPE_DOWN'] and \
    #    (not merge_props.get('element_id') or not merge_props.get('identifier_type')):
    #     logger.warning(f"Cannot MERGE transition: Missing element_id or identifier_type for actionType '{merge_props['actionType']}'. Details: {action_details}")
    #     return False


    # Thuộc tính khác cần lưu/cập nhật (phải là primitive hoặc list primitive)
    update_props = {
        'macro_code': action_details.get('macro_code'),
        # Lấy text từ các key có thể có
        'element_text': action_details.get('element_text') or action_details.get('onElementText'),
        # Lưu params dưới dạng chuỗi JSON (Neo4j có thể xử lý string)
        'params_json_str': json.dumps(action_details.get('params')) if action_details.get('params') else None,
        # Thêm các thuộc tính nguyên thủy khác từ action_details nếu cần
        # 'some_other_primitive': action_details.get('some_other_primitive')
    }
    # Loại bỏ key có giá trị None khỏi update_props
    ...
    # --- 2. Xây dựng câu lệnh Cypher ---
    # Tạo phần properties cho MERGE một cách động
    merge_pattern_props_list = []
    params = { # Khởi tạo params cơ bản
        "sourceId": source_screen_id,
        "targetId": target_screen_id,
        "appName": app_name,
        "updateProps": update_props, # Truyền dict các thuộc tính cần update
        "success": is_success,
        "logId": log_id,
        "now": current_time
    }
    ...
    for key, value in merge_props.items():
        param_name = f"p_{key}" # Tạo tên param duy nhất, ví dụ p_actionType
        merge_pattern_props_list.append(f"`{key}`: ${param_name}") # Dùng backtick nếu key có thể trùng keyword
        params[param_name] = value # Thêm vào dict params

    # Nối các phần tử lại thành chuỗi pattern properties: {`key1`: $p_key1, `key2`: $p_key2}
    merge_pattern_props_str = "{" + ", ".join(merge_pattern_props_list) + "}" if merge_pattern_props_list else ""

    # Tạo câu lệnh Cypher hoàn chỉnh
    # Sử dụng SET r += $updateProps để cập nhật nhiều thuộc tính cùng lúc
    cypher = f"""
    ...
    ON CREATE SET
        r += $updateProps, // Gán các thuộc tính từ updateProps
        ...
        r.created_at = $now // Thêm created_at
    ON MATCH SET
        r += $updateProps, // Cập nhật các thuộc tính từ updateProps
        ...
        r.updated_at = $now // Thêm updated_at
    RETURN r
    """
    ...
    success = execute_write(cypher, params) # execute_write đã có logging lỗi chi tiết
    if not success:
...
def get_app_graph_data(app_name: str) -> dict | None:
    ...
    Đã sửa để lấy đúng thuộc tính element_count.
    """
    # Sửa lại: Dùng logger đã khởi tạo hoặc logger của Flask app
    logger = current_app.logger if current_app else log
    ...
    node_ids_found = set() # Khởi tạo ở đây để dùng được trong cả khối try

    try:
        # Sử dụng database mặc định hoặc từ config
        ...
        with driver.session(database=db_name) as session:
            ...
            node_query = """
                ...
                RETURN n.screen_id AS id,
                       ...
                       n.status AS status,
                       // === SỬA Ở ĐÂY: Lấy trực tiếp thuộc tính n.element_count ===
            ...
            for record in nodes_result:
                node_id = record["id"]
                # Lấy element_count, mặc định là 0 nếu null hoặc không tồn tại
                ...
                node_data = {
                    ...
                    "element_count": element_count, # <<< Giờ sẽ lấy giá trị đúng
            ...
            logger.info(f"Processed {processed_nodes} nodes for app '{app_name}'. Found node IDs: {node_ids_found}")

            # 2. Lấy edges TRANSITION (Sử dụng coalesce cho action_details)
            edge_query = """
                ...
                RETURN a.screen_id AS source,
                       ...
                       coalesce(r.action_details, null) AS action, // Xử lý action_details có thể thiếu
                       r.status AS status,
            ...
            for record in edges_result:
                ...
                # Kiểm tra xem source và target có nằm trong danh sách node hợp lệ không
                ...
                # Xử lý action details (đã dùng coalesce nên action có thể là null)
                action_details = record['action'] # Có thể là None
                clicked_element_id = None
                action_type = None
                if isinstance(action_details, dict):
                    action_type = action_details.get("actionType")
                    # Lấy element_id từ các key có thể có
                    target_info = action_details.get('target', {}) if isinstance(action_details.get('target'), dict) else {}
                    clicked_element_id = action_details.get('element_id') or action_details.get('onElementId') or target_info.get('element_id') or target_info.get('resource_id')
                ...
                edge_data = {
                    ...
                    "target": target_id,
                    "status": record["status"],
                    # Chỉ thêm các key nếu có giá trị
        ...
        return None
    except neo4j.exceptions.ServiceUnavailable as e: # Sửa lại tên exception
