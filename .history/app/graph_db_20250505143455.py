    ...
    try:
        with driver.session() as session:
            ...
            for record in nodes_result:
                node_id = record["id"] # Lấy ID ra biến riêng
                if node_id is None: # <<< KIỂM TRA NONE >>>
                    print(f"WARNING (get_app_graph_data): Found Screen node with missing screen_id property for app {app_name}. Skipping this node.")
                    continue # Bỏ qua node này

                element_count = record["element_count"] # Lấy element_count
                if element_count is None: # <<< KIỂM TRA NONE >>>
                    # Có thể xảy ra nếu thuộc tính 'elements' không tồn tại trên node
                    print(f"WARNING (get_app_graph_data): Node {node_id} missing 'elements' property. Setting element_count to 0.")
                    element_count = 0
                ...
                node_data = {
                    ...
                    "element_count": element_count, # Dùng giá trị đã kiểm tra
                    # <<< SỬA LỖI SLICE: dùng node_id đã kiểm tra >>>
                    "label": node_id[:8] if node_id else 'NO_ID' # Nhãn hiển thị ngắn gọn
                }
            ...
            edge_query = """
                ...
                RETURN a.screen_id AS source, b.screen_id AS target, // <<< Dùng tên chuẩn screen_id
                       r.action_details AS action,
                       r.status AS status,
                       elementId(r) AS neo4j_edge_id // <<< Lấy elementId() của r
            """
            edges_result = session.run(edge_query, app_name=app_name) # Lỗi xảy ra ở đây do query sai
            edge_counter = 0
            for record in edges_result:
                ...
                # Lấy element_id từ action_details (ưu tiên resource_id nếu có)
                ...
                edge_data = {
                    "id": edge_id,
                    "source": record["source"], # <<< Đã lấy từ a.screen_id
                    "target": record["target"], # <<< Đã lấy từ b.screen_id
                    "status": record["status"],
                ...
                # edge_data = {k: v for k, v in edge_data.items() if v is not None} # Loại bỏ None nếu muốn
