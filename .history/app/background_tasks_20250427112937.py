...
def build_app_map_task(job_data: dict):
    # ... (code tạo app context, lấy logger giữ nguyên) ...
    with temp_app.app_context():
        log = current_app.logger
        # ... (code lấy device_id, account_id, processed_ui_state, previous_action) ...

        # === LẤY appName TỪ DỮ LIỆU UI ===
        app_name = processed_ui_state.get('package_name') # Dùng package_name làm appName
        if not app_name:
             # Có thể dùng fallback hoặc lấy từ account_id nếu cần
             log.error("Missing package_name in processed_ui_state. Cannot determine appName for map building.")
             return # Không thể tiếp tục nếu không biết app nào

        log.info(f"--- Starting build_app_map_task for App: {app_name} ---")

        # --- Logic Nhận diện/Tạo target_screen_id (Giữ nguyên hoặc cải tiến) ---
        ...
        if activity_name and elements:
             # ... (logic tạo hash/ID như cũ) ...
             structure_string = f"{activity_name}|{element_repr}" # Ví dụ
             target_screen_id = hashlib.sha256(structure_string.encode('utf-8')).hexdigest()[:16]
             log.debug(f"Generated target_screen_id: {target_screen_id} for app {app_name}")

        # --- Tạo/Cập nhật Node màn hình đích (Truyền thêm app_name) ---
        if target_screen_id:
            screen_props = {
                "activityName": activity_name,
                "structureHash": target_screen_id, # Tạm dùng hash làm định danh và hash cấu trúc
                "elementCount": len(elements),
                # Thêm các thuộc tính khác
            }
            # <<< Gọi hàm đã cập nhật với app_name >>>
            success_node = graph_db.create_or_update_screen_node(target_screen_id, app_name, screen_props)
            # ... (xử lý lỗi như cũ) ...

        # --- Tạo Quan hệ TRANSITION (Truyền thêm app_name) ---
        if previous_action and target_screen_id:
            source_screen_id = previous_action.get('source_screen_context', {}).get('screenId')
            action_details = previous_action.get('action_details')

            if source_screen_id and action_details:
                # <<< Gọi hàm đã cập nhật với app_name >>>
                success_rel = graph_db.create_or_update_transition_relationship(
                    source_screen_id=source_screen_id,
                    target_screen_id=target_screen_id,
                    app_name=app_name, # <<< Truyền app_name
                    action_data=action_details
                )
                # ... (xử lý lỗi như cũ) ...
            # ... (log warning nếu thiếu context như cũ) ...
