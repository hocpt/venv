...
def view_ai_simulations():
    ...
    - Danh sách các lần chạy/lệnh gần đây với config chi tiết
    """
    ...
    if not db: flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
        ...
        try:
            # 1. Lấy dữ liệu cho dropdowns và cấu hình đã lưu
            personas = db.get_all_personas() or []
            strategies = db.get_all_strategies() or []
            ...
            # Tạo map để tra cứu command nhanh bằng ID
            ...
            processed_command_ids = set() # Theo dõi command ID đã được liên kết với job live

            # 4. Tạo danh sách hiển thị - Ưu tiên xử lý các Job Live trước
            for job_id, next_run_timestamp in live_sim_jobs_dict.items():
                ...
                # Cố gắng trích xuất command_id từ job_id
                ...
                if command_id_from_job and command_id_from_job in command_map:
                    ...
                    # <<< Tạo chuỗi config chi tiết hơn >>>
                    config_info_str = f"A: {cfg_pa} <-> B: {cfg_pb}<br>" \
                                      f"<small>Strat: {cfg_stra} | Goal: {cfg_goal} | Turns: {cfg_turns}</small>"
                    sim_info['created_at'] = command_data.get('created_at') # Lấy thời gian tạo lệnh
                    processed_command_ids.add(command_id_from_job) # Đánh dấu đã xử lý

                sim_info['config_info'] = config_info_str # Cập nhật config_info

                # Format thời gian và status (như cũ)
                if next_run_timestamp is not None:
                    try:
                        utc_dt = datetime.fromtimestamp(next_run_timestamp, tz=timezone.utc)
                        local_dt = utc_dt.astimezone(server_tz)
            ...
            # 5. Xử lý các Command chưa được liên kết hoặc có trạng thái cuối cùng
            for cmd_id, command_data in command_map.items():
                if cmd_id not in processed_command_ids: # Chỉ xử lý nếu chưa được thêm bởi vòng lặp job live
                    payload = command_data.get('payload', {})
                    status = command_data.get('status', 'unknown')
                    # Xác định status_text
                    status_text = status.capitalize()
                    if status == 'pending': status_text = 'Pending Queue'
                    elif status == 'processing': status_text = 'Processing Cmd'
                    elif status == 'done': status_text = 'Done'
                    ...
                    sim_info = {
                        ...
                        'job_id': None, 'config_info': config_info_str, # <<< Dùng chuỗi mới
            ...
            # 6. Sắp xếp danh sách hiển thị cuối cùng
            def get_sort_key(item):
                 # Ưu tiên created_at nếu có, sau đó đến next_run_time nếu là job
                 ...
                 return datetime.now(timezone.utc) # Default về thời gian hiện tại nếu không có gì
            simulations_display_list.sort(key=get_sort_key, reverse=True) # Sắp xếp mới nhất trước
