    ...
    try:
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            if saved_classifications is None:
                logger.error(f"DB Error fetching saved classifications for screen {screen_id}.")
                saved_classifications = {} # Coi như rỗng nếu lỗi DB
            else:
                logger.debug(f"Saved classifications from DB: {saved_classifications}")

            # 4. Hợp nhất dữ liệu
            elements_list = []
            log_element_ids = set() # Để kiểm tra ID nào có trong log
            for el_from_log in elements_list_from_log:
                el_id = el_from_log.get('element_id')
                if el_id:
                    log_element_ids.add(el_id) # Thêm ID vào set
                    # Lấy classification đã lưu hoặc mặc định 'unclassified'
                    classification = saved_classifications.get(el_id, 'unclassified')
                    el_from_log['classification'] = classification
                    # Log việc gán classification
                    logger.debug(f"  Merging element ID '{el_id}': Assigned classification '{classification}' (Found in saved: {el_id in saved_classifications})")
                    # (Tùy chọn) Lấy các thông tin khác
                    elements_list.append(el_from_log)
                else:
                    logger.warning(f"  Skipping element from log due to missing element_id: {el_from_log}")

            # Log các classification đã lưu nhưng không tìm thấy element tương ứng trong log mới nhất
            saved_only_ids = set(saved_classifications.keys()) - log_element_ids
            if saved_only_ids:
                logger.warning(f"  Note: These saved classifications were not found in the latest log's elements: {saved_only_ids}")
