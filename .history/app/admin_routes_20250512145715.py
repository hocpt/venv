...
def view_screen_definitions():
    ...
    # Lấy distinct app names (hàm này nên tự quản lý connection)
    # Bạn có thể tạo một hàm mới trong database.py cho việc này
    # ví dụ: db.get_distinct_app_names_from_screen_definitions()
    try:
        # Giả sử bạn tạo hàm này trong database.py
        # distinct_app_names_for_filter = db.get_distinct_app_names_from_screen_definitions()
        # Hoặc giữ nguyên logic query trực tiếp nếu bạn chắc chắn về quản lý connection
        conn = db.get_db_connection()
        if conn:
            try: # Thêm try...finally để đảm bảo conn.close()
                ...
                conn.close() # Đóng kết nối ngay sau khi dùng xong
        else:
            flash("Không thể kết nối CSDL để lấy danh sách app.", "error")
            distinct_app_names_for_filter = [] # Đảm bảo là list
    ...
    if selected_app_filter:
        # Hàm này trong database.py sẽ tự lấy và đóng kết nối của nó
        ...
        if definitions_list is None: 
            ...
            definitions_list = []
    elif not selected_app_filter and distinct_app_names_for_filter: 
        flash("Vui lòng chọn một App Name từ bộ lọc để xem các định nghĩa.", "info")
    elif not distinct_app_names_for_filter: 
