...
Chắc chắn rồi, đây là toàn bộ code đã sửa đổi cho hàm add_strategy trong file app/admin_routes.py. Hàm này xử lý cả việc hiển thị form (GET) và nhận dữ liệu để thêm chiến lược mới (POST), đồng thời đảm bảo strategy_type được xử lý đúng và gọi hàm db.add_new_strategy với các tham số chính xác:

Python

# Dán đoạn code này vào file app/admin_routes.py,
# thay thế hoàn toàn hàm add_strategy cũ (nếu có)
# Đảm bảo các import cần thiết đã có ở đầu file admin_routes.py:
# from flask import Blueprint, render_template, request, redirect, url_for, flash
# from . import database as db # Hoặc cách import db của bạn
...
def add_strategy():
    ...
    strategy_type_from_url = request.args.get('type', 'language')
    if strategy_type_from_url not in ['language', 'control', 'mainloop']:
        strategy_type_from_url = 'language' # Đảm bảo giá trị hợp lệ cho GET

    title = f"Thêm {strategy_type_from_url.capitalize()} Strategy Mới"

    # Lấy danh sách stages cho dropdown (cần cho cả GET và POST lỗi)
    stages = []
    try:
        stages = db.get_all_stages() or []
        if stages is None: # Phân biệt lỗi DB và list rỗng
    ...
    if request.method == 'POST':
        ...
        strategy_type = request.form.get('strategy_type') # Lấy từ trường ẩn

        # Validate dữ liệu
        errors = []
        if not strategy_id: errors.append("Strategy ID là bắt buộc.")
        if not name: errors.append("Name là bắt buộc.")
        if not initial_stage_id: errors.append("Initial Stage ID là bắt buộc.")
        if not strategy_type: errors.append("Lỗi: Thiếu Strategy Type trong form.")
        elif strategy_type not in ['language', 'control', 'mainloop']: errors.append("Strategy Type không hợp lệ.")

        # Nếu có lỗi validation
        if errors:
            for error in errors: flash(error, "warning")
            # Render lại form với lỗi, truyền lại dữ liệu đã nhập và strategy_type ĐÚNG
            return render_template('admin_add_strategy.html',
                                   title=title + " (Lỗi)",
                                   stages=stages,
                                   strategy_type=strategy_type, # Dùng type từ form nếu có lỗi POST
                                   current_data=request.form), 400

        # Nếu không có lỗi validation, gọi hàm DB
        try:
            # Gọi hàm db.add_new_strategy đã sửa (KHÔNG có target_platform, execution_config)
            success, error_msg = db.add_new_strategy(
                strategy_id=strategy_id,
                name=name,
                description=description or None, # Chuyển chuỗi rỗng thành None
                initial_stage_id=initial_stage_id,
                strategy_type=strategy_type
            )

            if success:
                flash(f'Thêm chiến lược ({strategy_type}) "{name}" thành công!', 'success')
                # Redirect đến trang danh sách tương ứng
                if strategy_type == 'control':
                    redirect_url = url_for('admin.view_strategies_control')
                elif strategy_type == 'mainloop':
                    redirect_url = url_for('admin.view_strategies_mainloop')
                else: # Mặc định là language
                    redirect_url = url_for('admin.view_strategies_language')
                return redirect(redirect_url)
            else:
                # Lỗi từ DB (ví dụ: ID trùng)
                flash(f'Thêm chiến lược thất bại: {error_msg or "Lỗi không xác định."}', 'error')
                # Render lại form với lỗi DB
                return render_template('admin_add_strategy.html',
                                       title=title + " (Lỗi DB)",
                                       stages=stages,
                                       strategy_type=strategy_type, # Dùng type từ form
                                       current_data=request.form)
    ...
    # --- Xử lý GET request ---
    # Chỉ cần render template với dữ liệu cho dropdown và strategy_type từ URL
    return render_template('admin_add_strategy.html',
                           title=title,
                           stages=stages,
...
def view_api_documentation():
    title = "Quản lý Tài liệu API Client"
    page = request.args.get('page', 1, type=int)
    if page < 1: page = 1
    ...
    return render_template('admin_api_docs_list.html',
                           title=title,
                           api_docs=all_docs_details, # Dùng cho bảng list
                           all_docs_details=all_docs_details, # Dùng cho JS modal
                           pagination_details=pagination_details)
...
def view_strategies_mainloop():
    """Hiển thị danh sách các chiến lược loại 'mainloop'."""
    ...
    if not db:
        ...
        try:
            ...
            page = request.args.get('page', 1, type=int)
            if page < 1: page = 1
            # Dùng hằng số PER_PAGE chung hoặc tạo hằng số riêng nếu muốn
            per_page = current_app.config.get('ADMIN_ITEMS_PER_PAGE', 15)

            # Gọi hàm DB với bộ lọc type='mainloop'
            # Giả sử get_all_strategies trả về list (hoặc cần sửa để hỗ trợ pagination)
            # Nếu get_all_strategies chưa hỗ trợ pagination, bạn cần sửa nó hoặc tạo hàm mới
            # Tạm thời dùng get_all_strategies và bỏ qua pagination nếu nó chưa hỗ trợ
            # Ví dụ nếu get_all_strategies trả về tuple (list, total):
            # mainloop_strategies, total_items = db.get_all_strategies_paginated(strategy_type_filter='mainloop', page=page, per_page=per_page)

            # Dùng hàm get_all_strategies hiện có (chưa phân trang)
            all_strategies_filtered = db.get_all_strategies(strategy_type_filter='mainloop')
            if all_strategies_filtered is None:
                flash("Lỗi khi tải danh sách chiến lược Main Loop.", "error")
                mainloop_strategies = []
                # total_items = 0 # Nếu có phân trang
            else:
                 mainloop_strategies = all_strategies_filtered
                 # total_items = len(mainloop_strategies) # Nếu không có phân trang từ DB

                 # --- TÍNH TOÁN PAGINATION (NẾU CẦN) ---
                 # Đoạn này chỉ hoạt động đúng nếu bạn có total_items từ DB
                 # Nếu không, bạn cần lấy hết rồi tự phân trang ở Python hoặc sửa hàm DB
                 # Ví dụ tự phân trang (không hiệu quả với dữ liệu lớn):
                 # total_items = len(mainloop_strategies)
                 # offset = (page - 1) * per_page
                 # mainloop_strategies = mainloop_strategies[offset : offset + per_page]
                 # if total_items > 0:
                 #     total_pages = ceil(total_items / per_page)
                 #     if page > total_pages and total_pages > 0: page = total_pages
                 #     pagination = { 'page': page, 'per_page': per_page, ...} # Giống các route khác
                 # else: pagination = { ... } # Default pagination

                 # Tạm thời không dùng pagination cho route này nếu hàm DB chưa hỗ trợ
                 pagination = None
    ...
    # Render template MỚI (sẽ tạo ở bước sau)
    return render_template('admin_strategies_mainloop.html', # <<< Template mới
                           title=title,
                           strategies=mainloop_strategies,
                           pagination=pagination)
