...
def add_transition_control():
    ...
    all_macros = []
    all_templates = [] # Thêm list cho response templates
    ...
    try:
        ...
        # Lấy macro definitions (lọc nếu muốn)
        temp_macros, _ = db.get_all_macro_definitions(page=1, per_page=1000)
        all_macros = temp_macros or []

        # Lấy response templates
        # Giả sử bạn có hàm get_all_template_refs_with_details hoặc tương tự
        # Nếu chưa có, bạn cần tạo hàm này hoặc lấy hết template refs
        all_templates = db.get_all_template_refs_with_details() # Cần đảm bảo hàm này tồn tại
        ...
        flash(f"Lỗi tải dữ liệu cho form: {e}", "error")
        # strategy_stages_for_dropdowns, all_macros, all_templates sẽ là list rỗng

    # Các hằng số hoặc dữ liệu khác cho form (ví dụ)
    valid_intents=config.VALID_INTENTS_FOR_TRANSITION # Lấy từ config hoặc định nghĩa list cứng
    valid_condition_types=config.VALID_CONDITION_TYPES # Lấy từ config hoặc định nghĩa list cứng

    title = f"Thêm Control Transition cho Strategy {strategy_id}"

    # --- Xử lý POST (logic validate và gọi DB giữ nguyên như model_10) ---
    if request.method == 'POST':
        # ... (Lấy dữ liệu từ form: current_stage_id, user_intent, next_stage_id, etc...) ...
        # ... (Phần validate dữ liệu như cũ) ...
        # ... (Gọi hàm db.add_new_transition như cũ, bao gồm cả các trường loop, response_template_ref...) ...

        # --- Khi render lại form nếu lỗi, truyền đúng list stages ---
        if errors: # Nếu có lỗi validation
            for error in errors: flash(error, "warning")
            return render_template('admin_add_transition_control.html',
                                   ...
                                   current_data=request.form), 400
        # ... (Xử lý kết quả từ db.add_new_transition) ...
        try:
             # Gọi hàm DB add_new_transition (logic đầy đủ như model_10)
             success, error_msg = db.add_new_transition(...) # Điền đủ tham số
             if success:
...
def edit_transition_control(transition_id):
    ...
    transition = db.get_transition_details(transition_id) # Hàm này cần trả về cả strategy_id
    if not transition:
    ...
    strategy_stages_for_dropdowns = [] # <<< Dùng list này cho cả 2 dropdown
    all_macros = []
    all_templates = []
    cancel_url = url_for('admin.view_strategy_stages_control', strategy_id=strategy_id)
    ...
    valid_intents=config.VALID_INTENTS_FOR_TRANSITION # Lấy từ config
    valid_condition_types=config.VALID_CONDITION_TYPES # Lấy từ config

    # --- Xử lý POST (logic validate và gọi DB update_transition giữ nguyên như model_10) ---
    if request.method == 'POST':
        # ... (Lấy dữ liệu từ form: current_stage_id, user_intent, next_stage_id, etc...) ...
        # ... (Phần validate dữ liệu như cũ) ...
        # ... (Gọi hàm db.update_transition như cũ, bao gồm cả các trường loop, response_template_ref...) ...
        ...
        if errors: # Nếu có lỗi validation
            ...
            return render_template('admin_edit_transition_control.html',
                                   ...
                                   valid_intents=valid_intents, valid_condition_types=valid_condition_types,
                                   current_data=request.form), 400
        # ... (Xử lý kết quả từ db.update_transition) ...
        try:
            # Gọi hàm DB update_transition (logic đầy đủ như model_10)
            success, error_msg = db.update_transition(...) # Điền đủ tham số
            if success:
                ...
                # Render lại template mới: admin_edit_transition_control.html
                return render_template('admin_edit_transition_control.html',
    ...
    try:
        ...
        control_stages = db.get_stages_by_type('control') or []
        temp_macros, _ = db.get_all_macro_definitions(page=1, per_page=1000)
        all_macros = temp_macros or []
    except Exception as e:
