...
def load_scheduled_jobs_standalone(scheduler: BackgroundScheduler, db_config: dict):
    ...
    for job_config in job_configs:
        ...
        if job_args_dict is None:
            job_args_dict = {} # Đảm bảo là dict rỗng nếu null trong DB
        elif isinstance(job_args_dict, str): # Dự phòng nếu trả về string
            try:
                job_args_dict = json.loads(job_args_dict)
                if not isinstance(job_args_dict, dict): job_args_dict = {}
            except json.JSONDecodeError:
                print(f"WARNING (scheduler_runner): Invalid JSON in job_args for job '{job_id}'. Using empty args.")
                job_args_dict = {}
        elif not isinstance(job_args_dict, dict): # Trường hợp khác không phải dict
             print(f"WARNING (scheduler_runner): Invalid type for job_args for job '{job_id}'. Using empty args.")
             job_args_dict = {}
        ...
        try:
            ...
            trigger_obj = None
            # ... (code tạo trigger_obj dựa trên trigger_type và trigger_args_dict) ...
            # Ví dụ cho interval:
            if trigger_type == 'interval':
                trigger_obj = IntervalTrigger(
                    weeks=trigger_args_dict.get('weeks', 0),
                    days=trigger_args_dict.get('days', 0),
                    hours=trigger_args_dict.get('hours', 0),
                    minutes=trigger_args_dict.get('minutes', 0),
                    seconds=trigger_args_dict.get('seconds', 0),
                    start_date=trigger_args_dict.get('start_date'),
                    end_date=trigger_args_dict.get('end_date'),
                    timezone=SCHEDULER_TIMEZONE,
                    jitter=trigger_args_dict.get('jitter')
                )
            # ... (elif cho cron, date) ...
            else:
                 raise ValueError(f"Unsupported trigger type: {trigger_type}")

            # === SỬA LẠI scheduler.add_job: Thêm kwargs ===
            print(f"DEBUG (scheduler_runner): Adding job '{job_id}' with kwargs: {job_args_dict}") # Log kwargs
            scheduler.add_job(
                id=job_id,
                func=func,
                trigger=trigger_obj,
                kwargs=job_args_dict,  # <<< THÊM DÒNG NÀY ĐỂ TRUYỀN THAM SỐ
        ...
        except TypeError as trigger_err: # Bắt lỗi TypeError riêng
             print(f"ERROR (scheduler_runner): Invalid trigger/job args for job '{job_id}', type '{trigger_type}', trigger_args {trigger_args_dict}, job_args {job_args_dict}: {trigger_err}") # Log cả 2 loại args
