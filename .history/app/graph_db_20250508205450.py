...
def get_elements_for_screen(screen_id: str) -> list[dict] | None:
    """
    Lấy danh sách các phần tử UI (dạng list of dicts) được lưu trữ
    trong thuộc tính 'elements_data_json' của một Screen node trong Neo4j.

    Args:
        screen_id: ID của màn hình.

    Returns:
        List các dictionary phần tử, hoặc None nếu lỗi, hoặc list rỗng nếu không có dữ liệu.
    """
    logger = current_app.logger if current_app else log # Lấy logger
    if not screen_id:
    ...
    try:
        with driver.session(database=db_name) as session:
            # Chỉ lấy thuộc tính elements_data_json
            # Đảm bảo tên node và thuộc tính là đúng (vd: Screen, elements_data_json)
            query = "MATCH (s:Screen {screen_id: $screen_id}) RETURN s.elements_data_json AS elements_json"
            logger.debug(f"Executing Neo4j query for elements: {query} with screen_id={screen_id}")
            result = session.run(query, screen_id=screen_id)
            record = result.single() # Lấy một bản ghi duy nhất

            if record and record["elements_json"]:
                elements_json_str = record["elements_json"]
                try:
                    elements_list = json.loads(elements_json_str) # Parse chuỗi JSON thành list Python
                    if not isinstance(elements_list, list):
                        logger.error(f"Parsed elements_data_json cho screen {screen_id} không phải là list. Data: {elements_json_str}")
                        elements_list = [] # Trả về list rỗng nếu không phải list
                    else:
                        logger.info(f"Lấy và parse thành công {len(elements_list)} elements cho screen {screen_id} từ Neo4j.")
                        # === QUAN TRỌNG: Đổi tên key 'id' thành 'element_id' để khớp template ===
                        processed_list = []
                        for el_data in elements_list:
                            if 'id' in el_data:
                                el_data['element_id'] = el_data.pop('id')
                            # Đổi tên các key khác nếu cần (vd: type -> element_type, text -> text_content, ...)
                            if 'type' in el_data:
                                el_data['element_type'] = el_data.pop('type')
                            if 'text' in el_data:
                                el_data['text_content'] = el_data.pop('text')
                            if 'coords' in el_data:
                                el_data['coordinates'] = el_data.pop('coords')
                            # Giữ nguyên 'bounds'
                            processed_list.append(el_data)
                        elements_list = processed_list
                        # ===============================================================
                except json.JSONDecodeError as json_err:
                    logger.error(f"Lỗi parse elements_data_json cho screen {screen_id}: {json_err}. Data: {elements_json_str}")
                    elements_list = [] # Trả về list rỗng nếu lỗi JSON
            elif record:
                logger.warning(f"Tìm thấy Screen node {screen_id}, nhưng thiếu thuộc tính 'elements_data_json' hoặc là null.")
                elements_list = [] # Trả về list rỗng nếu thiếu thuộc tính
            else:
                logger.warning(f"Không tìm thấy Screen node {screen_id} trong Neo4j.")
                elements_list = [] # Trả về list rỗng nếu không tìm thấy screen

    except ServiceUnavailable as e:
        logger.error(f"Lỗi kết nối Neo4j (get_elements_for_screen): {e}")
        elements_list = None # Lỗi thì trả về None
    except Exception as e:
        logger.error(f"Lỗi Neo4j khi lấy elements cho screen {screen_id}: {e}", exc_info=True)
        elements_list = None # Lỗi thì trả về None

    # Trả về None nếu lỗi, list rỗng nếu không tìm thấy data/screen
