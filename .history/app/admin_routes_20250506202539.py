...
def admin_screen_elements(screen_id):
    ...
    try:
        ...
        relative_path = screen_data.get('screenshot_path')
        if relative_path:
            try:
                # Dùng url_for để tạo URL an toàn và đúng đắn
                screenshot_url = url_for('static', filename=relative_path, _external=False) # _external=False cho URL tương đối
                logger.info(f"Generated screenshot URL: {screenshot_url} from path: {relative_path}")
            except Exception as url_err:
                logger.error(f"Error generating static URL for screenshot path '{relative_path}': {url_err}")
                screenshot_url = None # Đặt lại thành None nếu lỗi
        else:
            logger.info(f"No screenshot_path found for screen {screen_id}.")
