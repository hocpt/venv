...
def add_macro_definition(macro_code: str, description: str | None, app_target: str | None, params_schema_str: str | None, notes: str | None) -> bool:
    """Thêm định nghĩa macro mới vào DB."""
    if not macro_code: return False
    ...
    if params_schema_str:
        try:
            ...
            print(f"WARN (add_macro_definition): Invalid JSON schema for {macro_code}. Saving as NULL.")
    ...
    try:
        ...
        app_target_db = app_target if app_target else 'system'
        params = (macro_code, description, app_target_db, params_schema_json, notes)
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.IntegrityError as e: # Bắt lỗi PK violation
        print(f"ERROR (db - add_macro_definition): Integrity Error (Code exists?): {e}")
        if conn: conn.rollback()
    except psycopg2.Error as e:
        print(f"ERROR (db - add_macro_definition): DB Error: {e}")
        if conn: conn.rollback()
    except Exception as e:
        print(f"ERROR (db - add_macro_definition): Unexpected error: {e}")
        if conn: conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def get_macro_definition(macro_code: str) -> dict | None:
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # params_schema lấy ra sẽ là dict Python nếu lưu là JSONB
        sql = """
        ...
        if row:
            ...
            if details.get('params_schema') is not None:
                try: details['params_schema_str'] = json.dumps(details['params_schema'], indent=2)
                except: details['params_schema_str'] = str(details['params_schema'])
            else: details['params_schema_str'] = ''
...
def get_all_macro_definitions(app_target: str | None = None) -> list[dict] | None:
    """Lấy danh sách tất cả định nghĩa macro, có thể lọc theo app_target."""
    macros_list = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        sql = "SELECT macro_code, description, app_target, notes FROM public.macro_definitions"
        params = []
        if app_target:
            sql += " WHERE app_target = %s OR app_target = 'system' OR app_target = 'generic'" # Lấy cả system/generic
            params.append(app_target)
        sql += " ORDER BY app_target, macro_code;"
        cur.execute(sql, tuple(params))
        rows = cur.fetchall()
        macros_list = [dict(row) for row in rows] if rows else []
    except Exception as e: print(f"ERROR (db - get_all_macro_definitions): {e}"); macros_list = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return macros_list

def update_macro_definition(macro_code: str, description: str | None, app_target: str | None, params_schema_str: str | None, notes: str | None) -> bool:
    """Cập nhật định nghĩa macro."""
    if not macro_code: return False
    params_schema_json = None
    if params_schema_str:
        try: params_schema_json = json.dumps(json.loads(params_schema_str))
        except json.JSONDecodeError: print(f"WARN (update_macro_definition): Invalid JSON schema for {macro_code}. Saving as NULL.")
    ...
    try:
        ...
        conn.commit()
        success = cur.rowcount > 0
    except psycopg2.Error as e: print(f"ERROR (db - update_macro_definition) Code {macro_code}: {e}"); conn.rollback()
    except Exception as e: print(f"ERROR (db - update_macro_definition) Code {macro_code}: {e}"); conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success

def delete_macro_definition(macro_code: str) -> bool:
    """Xóa định nghĩa macro."""
    # Lưu ý: Cần kiểm tra xem macro_code này có đang được dùng trong transitions không trước khi xóa? (Hoặc để DB báo lỗi FK nếu có)
    if not macro_code: return False
    conn = get_db_connection()
    if not conn: return False
    cur = None
    ...
    try:
        cur = conn.cursor()
        # TODO: Thêm logic kiểm tra ràng buộc khóa ngoại nếu cần thiết
        # Ví dụ: SELECT 1 FROM stage_transitions WHERE (action_to_suggest->>'macro_code') = %s LIMIT 1;
        # Nếu có kết quả thì không cho xóa.
        sql = "DELETE FROM public.macro_definitions WHERE macro_code = %s;"
        cur.execute(sql, (macro_code,))
        conn.commit()
        success = cur.rowcount > 0
    except psycopg2.Error as e: print(f"ERROR (db - delete_macro_definition) Code {macro_code}: {e}"); conn.rollback()
    except Exception as e: print(f"ERROR (db - delete_macro_definition) Code {macro_code}: {e}"); conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success

# --- Hàm lấy chuỗi hành động cho Strategy Package ---
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    """Lấy danh sách các bước hành động (macro_code + params) cho một strategy."""
    # Hàm này cần logic phức tạp hơn là chỉ query transitions.
    # Nó cần xác định trình tự các bước dựa trên transitions, stages, và có thể cả logic lặp/điều kiện.
    # Ban đầu, có thể làm đơn giản là lấy tất cả transitions của strategy và sắp xếp theo stage/priority.
    print(f"DEBUG (get_strategy_action_sequence): Getting actions for strategy {strategy_id}")
    transitions = get_transitions_for_strategy(strategy_id) # Dùng hàm đã có
    if transitions is None:
        return None

    action_sequence = []
    # TODO: Xây dựng logic sắp xếp/biên dịch transitions thành chuỗi hành động thực tế.
    # Tạm thời chỉ lấy ra phần action từ transitions
    for t in transitions:
        action_json = t.get('action_to_suggest')
        if action_json: # Chỉ lấy các transition có định nghĩa action
            if isinstance(action_json, str): # Parse nếu nó là string
                try: action_json = json.loads(action_json)
                except: action_json = None
            if isinstance(action_json, dict) and 'macro_code' in action_json:
                 # Thêm các thông tin ngữ cảnh nếu cần cho phone engine
                 action_step = {
                      "macro_code": action_json.get('macro_code'),
                      "params": action_json.get('params', {}),
                      "trigger": {"stage": t.get('current_stage_id'), "intent": t.get('user_intent')} # Ví dụ ngữ cảnh
                 }
                 action_sequence.append(action_step)

    print(f"DEBUG: Assembled action sequence with {len(action_sequence)} steps (simple extraction).")
    return action_sequence # Trả về list các dict {"macro_code": ..., "params": ...}

# --- Hàm lấy version (ví dụ đơn giản) ---
def get_strategy_version(strategy_id: str) -> str | None:
    """Lấy mã phiên bản (ví dụ: timestamp cập nhật cuối)."""
    # TODO: Implement versioning logic (e.g., using updated_at timestamp or a dedicated version table)
    return datetime.datetime.now(datetime.timezone.utc).isoformat() # Placeholder

# --- Sửa hàm add/update transition để lưu đúng định dạng action ---
def add_new_transition(current_stage_id: str, user_intent: str, condition_logic: str | None,
                       next_stage_id: str | None, # Bỏ action_to_suggest (text cũ)
                       response_template_ref: str | None, # Bỏ
                       priority: int = 0,
                       action_macro_code: str | None = None, # Thêm macro code
                       action_params_str: str | None = None) -> bool: # Thêm params JSON string
    """Thêm một transition mới, lưu action dạng macro_code + params."""
    if not current_stage_id or not user_intent: return False

    # Tạo JSON cho action_to_suggest
    action_json = None
    if action_macro_code:
        params_dict = {}
        if action_params_str:
            try: params_dict = json.loads(action_params_str)
            except json.JSONDecodeError: print(f"WARN (add_new_transition): Invalid params JSON. Saving empty params."); params_dict = {}
        action_data = {"macro_code": action_macro_code, "params": params_dict}
        try: action_json = json.dumps(action_data) # Lưu dạng chuỗi JSON
        except TypeError: print(f"ERROR (add_new_transition): Cannot serialize action data."); return False
    # else: action_json là None nếu không có macro code

    conn = get_db_connection()
    # ... (try/except/finally như cũ) ...
    try:
        cur = conn.cursor()
        # Bỏ action_to_suggest (text cũ), response_template_ref
        # Thêm cột action_to_suggest (kiểu JSONB)
        sql = """
            INSERT INTO stage_transitions (current_stage_id, user_intent, condition_logic,
        ...
        params = (current_stage_id, user_intent, condition_logic,
                  next_stage_id if next_stage_id else None,
                  priority,
                  action_json # Truyền chuỗi JSON hoặc None
                  )
        cur.execute(sql, params)
        conn.commit()
        success = True
    # ... (phần xử lý lỗi giữ nguyên) ...
    except psycopg2.Error as e: print(f"ERROR (db - add_new_transition): INSERT failed: {e}"); success = False; conn.rollback()
    except Exception as e: print(f"ERROR (db - add_new_transition): Unexpected error: {e}"); success = False; conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str, condition_logic: str | None,
                      ...
                      response_template_ref: str | None, # Bỏ
                      priority: int,
                      action_macro_code: str | None = None, # Thêm
                      action_params_str: str | None = None) -> bool: # Thêm
    """Cập nhật một transition với action dạng macro_code + params."""
    if not transition_id or not current_stage_id or not user_intent: return False
    ...
    if action_macro_code:
        ...
        if action_params_str:
            try: params_dict = json.loads(action_params_str)
            except json.JSONDecodeError: print(f"WARN (update_transition): Invalid params JSON."); params_dict = {}
        action_data = {"macro_code": action_macro_code, "params": params_dict}
        try: action_json = json.dumps(action_data)
        except TypeError: print(f"ERROR (update_transition): Cannot serialize action data."); return False
    ...
    try:
        ...
        sql = """
            UPDATE stage_transitions
            SET current_stage_id = %s, user_intent = %s, condition_logic = %s, next_stage_id = %s,
                priority = %s, action_to_suggest = %s::jsonb
            WHERE transition_id = %s;
        """
        params = (current_stage_id, user_intent, condition_logic,
                  next_stage_id if next_stage_id else None,
        ...
        success = cur.rowcount > 0
    # ... (phần xử lý lỗi giữ nguyên) ...
    except psycopg2.Error as e: print(f"ERROR (db - update_transition): UPDATE failed: {e}"); success = False; conn.rollback()
    except Exception as e: print(f"ERROR (db - update_transition): Unexpected error: {e}"); success = False; conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return success
...
# --- Hàm log action của phone (cần tạo) ---
def log_phone_action(session_id, timestamp, macro_code, params_json, execution_status, execution_error=None, current_stage=None, received_state_json=None):
     ...
     # TODO: Implement INSERT vào bảng phone_action_log
     # Chú ý xử lý timestamp (chuyển đổi timezone nếu cần) và JSON data
     pass # Placeholder

# --- Lấy chi tiết transition (cần sửa để trả về action_macro_code, action_params_str) ---
def get_transition_details(transition_id: int) -> dict | None:
    """Lấy chi tiết một transition, tách action_to_suggest thành code và params string."""
    if not transition_id: return None
    details = None
    conn = get_db_connection()
    if not conn: return None
    ...
    try:
        ...
        if row:
            ...
            action_data = details.get('action_to_suggest')
            details['action_macro_code'] = None
            details['action_params_str'] = '{}' # Mặc định chuỗi JSON rỗng
            if isinstance(action_data, dict):
                 details['action_macro_code'] = action_data.get('macro_code')
                 params_dict = action_data.get('params', {})
                 try: details['action_params_str'] = json.dumps(params_dict, indent=2) # Format đẹp cho form
                 except: details['action_params_str'] = '{}'

    # ... (phần xử lý lỗi giữ nguyên) ...
    except psycopg2.Error as e: print(f"LỖI (database.py - get_transition_details): Truy vấn thất bại: {e}")
    except Exception as e: print(f"LỖI (database.py - get_transition_details): Lỗi không xác định: {e}")
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return details