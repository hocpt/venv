...
def create_app(config_class=config.Config):
    ...
    if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        try:
            # <<< BẮT ĐẦU SỬA ĐỔI LOGGING CONFIG >>>
            log_level_name = app.config.get('SCHEDULER_LOG_LEVEL_NAME', 'DEBUG')
            log_level = getattr(logging, log_level_name.upper(), logging.DEBUG)

            # Tạo formatter chung
            log_formatter = logging.Formatter(
                '%(asctime)s %(levelname)-8s %(name)-25s %(threadName)s : %(message)s'
            )

            # Cấu hình StreamHandler (ghi ra console)
            stream_handler = logging.StreamHandler(sys.stdout)
            stream_handler.setFormatter(log_formatter)
            # Không setLevel ở đây, để logger tự quyết định

            # Cấu hình Root Logger (bao gồm cả Flask và các thư viện khác)
            root_logger = logging.getLogger()
            # Xóa handler cũ nếu có để tránh trùng lặp (Cẩn thận nếu Flask thêm handler riêng)
            # for handler in root_logger.handlers[:]: root_logger.removeHandler(handler)
            root_logger.addHandler(stream_handler)
            root_logger.setLevel(log_level) # Đặt level cho root

            # Cấu hình Logger riêng cho APScheduler
            apscheduler_logger = logging.getLogger('apscheduler')
            # Xóa handler cũ của apscheduler nếu có
            # for handler in apscheduler_logger.handlers[:]: apscheduler_logger.removeHandler(handler)
            apscheduler_logger.addHandler(stream_handler) # <<< Thêm handler tường minh
            apscheduler_logger.setLevel(log_level) # <<< Đặt level tường minh
            apscheduler_logger.propagate = False # <<< Ngăn không cho log lên root nữa để tránh trùng lặp
            ...
            # <<< KẾT THÚC SỬA ĐỔI LOGGING CONFIG >>>

            # Bắt đầu scheduler và load jobs (giữ nguyên code này)
            ...
            load_scheduled_jobs(app) # Vẫn giữ phiên bản load job date trigger debug
