...
def get_all_strategies(strategy_type_filter: str | None = None) -> list[dict] | None: # <<< Thêm bộ lọc
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        sql = "SELECT strategy_id, name, description, initial_stage_id, strategy_type, updated_at FROM strategies" # <<< Thêm strategy_type
        params = []
        # <<< Thêm logic lọc theo type >>>
