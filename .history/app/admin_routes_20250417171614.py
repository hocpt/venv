...
def add_stage():
    # Lấy strategy_id từ query param cho GET request và để biết ngữ cảnh
    strategy_id = request.args.get('strategy_id')
    if not strategy_id:
        flash("Cần cung cấp strategy_id để thêm stage.", "error")
        # Không biết redirect về đâu, tạm về control list
        return redirect(url_for('admin.view_strategies_control'))

    # Lấy thông tin strategy để biết type (cần cho redirect sau POST)
    # Đảm bảo db và hàm get_strategy_details hoạt động đúng
    ...
    if not strategy_details:
        flash(f"Không tìm thấy strategy ID {strategy_id} hoặc lỗi khi đọc.", "error")
        # Không biết type, tạm redirect về language list
        return redirect(url_for('admin.view_strategies_language'))

    # Xác định trang redirect đích dựa trên strategy type
    strategy_type = strategy_details.get('strategy_type', 'language') # Mặc định là language nếu lỗi
    redirect_endpoint = 'admin.view_strategy_stages_language' if strategy_type == 'language' else 'admin.view_strategy_stages_control'
    print(f"DEBUG (add_stage): Strategy Type = {strategy_type}, Redirect Endpoint = {redirect_endpoint}") # Print debug

    if request.method == 'POST':
        stage_id = request.form.get('stage_id')
        description = request.form.get('description')
        order_str = request.form.get('stage_order', '0')
        # Lấy lại strategy_id từ hidden input để đảm bảo đúng
        ...
        if not stage_id or not form_strategy_id:
             ...
             return render_template('admin_add_stage.html', title="Thêm Stage (Lỗi)",
                                    strategy_id=strategy_id, current_data=request.form), 400
        if form_strategy_id != strategy_id:
             flash("Lỗi không khớp Strategy ID trong form và URL.", "error")
             return redirect(url_for(redirect_endpoint, strategy_id=strategy_id)) # Redirect về trang detail đúng
        ...
        try:
            # Giả sử add_new_stage trả về tuple (success, error_msg)
            ...
            if success:
                ...
                # <<< SỬA LẠI REDIRECT Ở ĐÂY ĐỂ DÙNG redirect_endpoint >>>
                print(f"DEBUG (add_stage): Redirecting to '{redirect_endpoint}' with strategy_id '{form_strategy_id}'") # Print debug
                return redirect(url_for(redirect_endpoint, strategy_id=form_strategy_id))
                # <<< KẾT THÚC SỬA REDIRECT >>>
            else:
                ...
                return render_template('admin_add_stage.html', title="Thêm Stage (Lỗi DB)",
                                       strategy_id=strategy_id, current_data=request.form)
        except Exception as e:
    ...
    # Truyền strategy_id để dùng trong hidden input và link Hủy
