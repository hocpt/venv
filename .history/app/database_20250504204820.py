...
def get_valid_api_keys(provider: str) -> list[dict] | None:
    ...
    sql = """
        SELECT key_id, provider, key_name, encrypted_api_key, status, notes, created_at, updated_at
        ...
        if rows:
            for row in rows:
                ...
                if encrypted_key:
                    ...
                    logger.warning(f"API key ID {key_id} for provider '{provider}' has no encrypted_api_key value in DB. Skipping.")
                    continue
                ...
                key_data.pop('encrypted_api_key', None)
