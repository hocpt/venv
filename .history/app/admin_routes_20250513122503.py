...
@admin_bp.route('/serve_screenshot_for_node_management/<path:app_name>/<path:filename>', endpoint='serve_app_specific_screenshot')
# @admin_required
def serve_app_specific_screenshot(app_name, filename): # Tên hàm có thể khác, endpoint mới quan trọng
    logger = current_app.logger
    base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
    if not base_path:
    ...
    directory_path = os.path.join(base_path, app_name) # Ảnh nằm trong thư mục con app_name
