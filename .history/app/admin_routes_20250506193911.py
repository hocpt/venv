...
def admin_screen_elements(screen_id):
    """Hiển thị trang chi tiết các elements của một Screen node, thêm cờ is_explored."""
    logger = current_app.logger if current_app else print
    ...
    try:
        ...
        app_name = screen_data.get('app_name') # Lấy app_name từ screen_data

        # === THÊM BƯỚC: LẤY CÁC HÀNH ĐỘNG ĐÃ THỬ TỪ NEO4J ===
        tried_element_ids = set()
        if app_name: # Chỉ lấy nếu có app_name
            ...
            logger.debug(f"Found {len(tried_element_ids)} explored element IDs from transitions: {tried_element_ids}")
        else:
            logger.warning(f"Cannot fetch outgoing transitions because app_name is missing for screen {screen_id}.")
        # ======================================================

        # 2. Lấy UI state chi tiết từ log PostgreSQL
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            # 3. Lấy classifications đã lưu từ PostgreSQL
            ...
            # 4. Hợp nhất dữ liệu: Thêm classification VÀ cờ is_explored
            elements_list = []
            for el_from_log in elements_list_from_log:
                ...
                if el_id:
                    ...
                    # === GÁN CỜ is_explored ===
                    el_from_log['is_explored'] = el_id in tried_element_ids
                    # ==========================
    ...
    return render_template('admin_screen_elements.html',
                           screen=screen_data,
                           elements=elements_list, # <<< Đã chứa cờ is_explored
