...
def view_node_management():
    ...
    if raw_nodes_page_data is None:
            ...
            for node_neo4j_data in raw_nodes_page_data:
                ...
                # node_app_name = node_processed_data.get('app_name') # Không dùng để tạo URL ảnh nữa

                logger.debug(f"VIEW: Processing node for template: ID={node_processed_data.get('screen_id')}, RawFile={raw_screenshot_file}")
                ...
                if raw_screenshot_file:
                    try:
                        ...
                        logger.info(f"VIEW: Generated URL for {raw_screenshot_file}: {node_processed_data['screenshot_full_url']}")
                    except Exception as e:
                        logger.error(f"VIEW: Error generating URL for {raw_screenshot_file}: {e}", exc_info=True)
