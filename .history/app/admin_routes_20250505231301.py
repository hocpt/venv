...
#@admin_required # Bảo vệ route nếu cần
def admin_screen_elements(screen_id):
    """Hiển thị trang chi tiết các elements của một Screen node."""
    logger = current_app.logger if current_app else print
    logger.info(f"Loading elements page for screen_id: {screen_id}")

    screen_data = None
    elements_list = [] # Khởi tạo list rỗng
    screenshot_url = None # Khởi tạo screenshot_url

    if not graph_db or not db:
        flash("Lỗi nghiêm trọng: Database hoặc GraphDB module chưa sẵn sàng.", "error")
        return redirect(url_for('admin.index')) # Hoặc trang khác

    try:
        # 1. Lấy thông tin cơ bản của Screen từ Neo4j
        # Hàm này không còn trả về list elements chi tiết nữa
        screen_data = graph_db.get_screen_with_elements(screen_id)
        if not screen_data:
            flash(f"Screen node with ID '{screen_id}' not found in Neo4j.", 'warning')
            return redirect(url_for('admin.admin_mapping_viewer')) # Redirect về mapping viewer

        # 2. Lấy danh sách elements chi tiết từ log PostgreSQL gần nhất
        # Gọi hàm DB mới đã tạo ở Bước 1
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            elements_list = detailed_ui_state.get('elements')
            logger.info(f"Successfully fetched {len(elements_list)} elements from the latest log for screen {screen_id}.")
            # (Tùy chọn) Bổ sung thông tin classification từ Neo4j vào list này nếu cần
            # neo4j_elements_info = screen_data.get('elements', []) # Lấy list cũ từ Neo4j (nếu có)
            # if neo4j_elements_info:
            #     neo4j_map = {el.get('element_id'): el for el in neo4j_elements_info if el.get('element_id')}
            #     for i, el_from_log in enumerate(elements_list):
            #         el_id = el_from_log.get('element_id')
            #         if el_id in neo4j_map:
            #             # Lấy classification và các thông tin khác từ Neo4j gán vào đây
            #             el_from_log['classification'] = neo4j_map[el_id].get('classification', 'unclassified')
            #             el_from_log['attempt_count'] = neo4j_map[el_id].get('attempt_count', 0)
            #             # ... thêm các trường khác ...
            #         else:
            #              el_from_log['classification'] = 'unclassified' # Default nếu chưa có trong Neo4j
            #         elements_list[i] = el_from_log # Cập nhật lại list
        else:
            logger.warning(f"Could not find detailed elements list in logs for screen {screen_id}. Displaying empty list.")
            # elements_list sẽ là list rỗng đã khởi tạo

        # 3. Lấy danh sách các loại classification hợp lệ
        # Đảm bảo ai_service được import và có biến VALID_CLASSIFICATIONS
        valid_classifications = []
        if ai_service and hasattr(ai_service, 'VALID_CLASSIFICATIONS'):
        ...
        # 4. Lấy đường dẫn ảnh chụp màn hình (logic này giữ nguyên nếu bạn có)
        screenshot_url = url_for('static', filename=screen_data.get('screenshot_path')) if screen_data.get('screenshot_path') else None

    except Exception as e:
        logger.error(f"Error loading screen elements page for {screen_id}: {e}", exc_info=True)
        flash(f"Lỗi không mong muốn khi tải dữ liệu trang: {e}", "error")
        # Nếu lỗi nặng không lấy được screen_data thì redirect
        if screen_data is None:
             return redirect(url_for('admin.admin_mapping_viewer'))
        # Nếu lỗi chỉ ở phần lấy elements thì vẫn render trang với list rỗng
        elements_list = []


    # Render template, truyền cả screen_data và elements_list
    return render_template('admin_screen_elements.html',
                           screen=screen_data, # Thông tin cơ bản của screen
                           elements=elements_list, # Danh sách elements chi tiết từ log
...
@admin_bp.route('/api/screen/<path:screen_id>/suggest_classifications', methods=['GET'])
#@admin_required
def api_suggest_classifications(screen_id):
    """API Endpoint để lấy gợi ý phân loại từ AI."""
    screen_data = graph_db.get_screen_with_elements(screen_id)
    if not screen_data or not screen_data.get('elements'):
        return jsonify({"success": False, "error": "Screen or elements not found"}), 404

    elements_to_classify = [
        el for el in screen_data['elements']
        # Chỉ gửi những cái chưa phân loại hoặc có thể gửi tất cả để AI review
        if el.get('classification') == 'unclassified'
    ]

    if not elements_to_classify:
         return jsonify({"success": True, "suggestions": [], "message": "No unclassified elements found."})

    try:
        # Chỉ gửi các trường cần thiết cho AI
        elements_for_ai = [
            {"element_id": el.get("element_id"),
             "element_type": el.get("element_type"),
             "text_content": el.get("text_content")}
            for el in elements_to_classify
        ]
        suggestions = ai_service.suggest_element_classifications(elements_for_ai)
        # suggestions giờ là list các element với key 'suggested_classification'

        return jsonify({"success": True, "suggestions": suggestions})
    except Exception as e:
        print(f"Error getting AI classification suggestions: {e}")
        traceback.print_exc()
        return jsonify({"success": False, "error": "Failed to get AI suggestions."}), 500
...
@admin_bp.route('/api/map/graph_data/<path:app_name>')
# @admin_required # Tạm comment out để test
def api_get_app_graph_data(app_name):
    """API trả về dữ liệu nodes và edges cho Cytoscape.js."""
    current_app.logger.info(f"API Request: Get graph data for app: {app_name}") # Thêm log
    if not graph_db:
        current_app.logger.error("Graph database module not available in api_get_app_graph_data")
        return jsonify({"error": "Graph database module not available"}), 503
    try:
        graph_data = graph_db.get_app_graph_data(app_name) # Gọi hàm đã tạo ở Giai đoạn A
        if graph_data:
            # Trả về dữ liệu đã có cấu trúc nodes/edges
            return jsonify(graph_data)
        else:
            # Trả về cấu trúc rỗng nếu không có dữ liệu hoặc lỗi DB nhẹ
            current_app.logger.warning(f"No graph data found or DB error for app: {app_name}")
            return jsonify({"nodes": [], "edges": []})
    except Exception as e:
        current_app.logger.error(f"Error fetching graph data API for {app_name}: {e}", exc_info=True)
        return jsonify({"error": "Internal server error fetching graph data"}), 500
...
def admin_screen_elements(screen_id):
    ...
    elements_list = []
    screenshot_url = None
    valid_classifications = [] # Khởi tạo list rỗng

    if not graph_db or not db or not ai_service:
        flash("Lỗi nghiêm trọng: Database hoặc GraphDB hoặc AI module chưa sẵn sàng.", "error")
    ...
    try:
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            # 3. Lấy các classification đã lưu từ PostgreSQL
            saved_classifications = db.get_element_classifications_for_screen(screen_id)
            if saved_classifications is None:
                logger.warning(f"Could not fetch saved classifications for screen {screen_id}, defaulting to 'unclassified'.")
                saved_classifications = {} # Đặt là dict rỗng nếu lỗi

            # 4. Hợp nhất thông tin: Lấy element từ log và thêm classification từ DB
            elements_list = []
            for el_from_log in elements_list_from_log:
                el_id = el_from_log.get('element_id')
                if el_id:
                    # Lấy classification đã lưu hoặc dùng 'unclassified'
                    el_from_log['classification'] = saved_classifications.get(el_id, 'unclassified')
                    # (Tùy chọn) Lấy các thông tin khác như attempt_count nếu cần
                    elements_list.append(el_from_log)
                else:
                    logger.warning(f"Skipping element from log due to missing element_id: {el_from_log}")

            logger.info(f"Merged {len(elements_list)} elements from log with classifications from DB for screen {screen_id}.")

        else:
            logger.warning(f"Could not find detailed elements list in logs for screen {screen_id}.")

        # 5. Lấy danh sách các loại classification hợp lệ
        ...
        # 6. Lấy screenshot URL (giữ nguyên)
        screenshot_url = url_for('static', filename=screen_data.get('screenshot_path')) if screen_data.get('screenshot_path') else None

    except Exception as e:
        logger.error(f"Error loading screen elements page for {screen_id}: {e}", exc_info=True)
        flash(f"Lỗi không mong muốn khi tải dữ liệu trang: {e}", "error")
        if screen_data is None: return redirect(url_for('admin.admin_mapping_viewer'))
        elements_list = []

    # Render template
    return render_template('admin_screen_elements.html',
                            screen=screen_data,
                            elements=elements_list, # Danh sách đã hợp nhất classification
                            valid_classifications=valid_classifications,
                            screenshot_url=screenshot_url)
