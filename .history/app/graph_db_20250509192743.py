...
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
                 ...
                 screen_width: int | None = None, screen_height: int | None = None) -> bool:
    logger = current_app.logger if current_app else log
    logger.info(f"[merge_screen] Received for screen_id: {screen_id}, app: {app_name}")
    # ... (log screenshot_path, dimensions) ...

    if not screen_id or not app_name: # Các kiểm tra đầu vào khác
        logger.error("merge_screen: screen_id and app_name are required.")
        return False
    driver = get_driver()
    if not driver: logger.error("merge_screen: Neo4j driver not available."); return False

    current_time_utc = datetime.now(timezone.utc)

    # --- Chuẩn bị screen_props_to_set (chỉ thuộc tính Screen) ---
    screen_props = {
        "screen_id": screen_id, "app_name": app_name, "activity_name": activity_name or 'UnknownActivity',
        "status": 'provisional', "last_analyzed_log_id": log_id, "last_seen": current_time_utc,
        "element_count": len(extracted_elements), "screenshot_path": screenshot_path,
        "width": screen_width, "height": screen_height
    }
    screen_props_to_set = {k: v for k, v in screen_props.items() if v is not None and k != 'screen_id'}
    logger.debug(f"[merge_screen] Screen properties to set: {screen_props_to_set}")

    # --- Chuẩn bị standardized_elements (đảm bảo chỉ có kiểu nguyên thủy) ---
    standardized_elements = []
    # ... (Copy logic chuẩn hóa elements từ phiên bản trước vào đây) ...
    # --- (logic chuẩn hóa elements như cũ) ---
    if extracted_elements:
        for el_count, el in enumerate(extracted_elements):
            if isinstance(el, dict) and el.get('element_id'):
                element_entry = {
                    'element_id': el['element_id'], 'identifier_type': el.get('identifier_type'),
                    'element_type': el.get('element_type'), 'text_content': el.get('text_content'),
                    'coordinate_x': None, 'coordinate_y': None, 'classification': 'unclassified',
                    'attempt_count': el.get('attempt_count', 0), 'success_count': el.get('success_count', 0),
                    'is_clickable_observed': el.get('is_clickable_observed', False),
                    'is_editable_observed': el.get('is_editable_observed', False),
                    'last_seen_timestamp': current_time_utc.isoformat()
                }
                coords = el.get('coordinates');
                if isinstance(coords, dict) and 'x' in coords and 'y' in coords:
                    try: element_entry['coordinate_x'] = int(coords['x']); element_entry['coordinate_y'] = int(coords['y'])
                    except: pass
    ...
    logger.debug(f"[merge_screen] Standardized {len(standardized_elements)} elements.")
    if standardized_elements: logger.debug(f"[merge_screen] Sample standardized element 0: {standardized_elements[0]}")
    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            # === Hàm thực thi transaction với LOGGING CHI TIẾT ===
            def merge_screen_and_elements_tx(tx: Transaction, screen_id_param, screen_props_param, elements_param):
                log.info(f"--- Starting Neo4j Transaction for screen {screen_id_param} ---")
                # 1. Merge Screen Node
                screen_query = """
                MERGE (s:Screen {screen_id: $screenId})
                ON CREATE SET s = $propsToSet, s.screen_id = $screenId, s.created_at = datetime()
                ON MATCH SET s += $propsToSet, s.updated_at = datetime()
                RETURN s
                """
                screen_params = {"screenId": screen_id_param, "propsToSet": screen_props_param}
                # +++ LOGGING TRƯỚC KHI CHẠY SCREEN QUERY +++
                log.debug(f"--- TX LOG: Running Screen Query ---")
                log.debug(f"Screen Query: {screen_query}")
                log.debug(f"Screen Params: {screen_params}")
                # Kiểm tra kiểu dữ liệu trong screen_props_param
                for k, v in screen_props_param.items():
                    if not isinstance(v, (str, int, float, bool, datetime, type(None))): # Thêm datetime
                        log.warning(f"Potential non-primitive type in screen props: Key='{k}', Type='{type(v)}', Value='{v}'")
                # ++++++++++++++++++++++++++++++++++++++++++++
                screen_result = tx.run(screen_query, screen_params) # <<< CHẠY QUERY >>>
                screen_node_record = screen_result.single()
                if not screen_node_record:
                    raise Exception(f"Failed to merge Screen node with ID: {screen_id_param}")
                log.debug(f"--- TX LOG: Screen Query OK ---")

                # 2. Merge Element Nodes và Relationships
                log.debug(f"--- TX LOG: Starting Element/Relationship Merge Loop ({len(elements_param)} elements) ---")
                for i, element_data in enumerate(elements_param):
                    element_id_val = element_data.get('element_id')
                    if not element_id_val: continue

                    element_props_to_set = {k: v for k, v in element_data.items() if v is not None and k != 'element_id'}

                    element_query = """
                    MATCH (s:Screen {screen_id: $screenId})
                    MERGE (e:Element {screen_id: $screenId, element_id: $elementId})
                    ON CREATE SET e = $propsToSet, e.element_id = $elementId, e.screen_id = $screenId, e.created_at = datetime()
                    ON MATCH SET e += $propsToSet, e.updated_at = datetime()
                    MERGE (s)-[r:HAS_ELEMENT]->(e)
                    RETURN e.element_id
                    """
                    element_params = {
                        "screenId": screen_id_param,
                        "elementId": element_id_val,
                        "propsToSet": element_props_to_set
                    }
                    # +++ LOGGING TRƯỚC KHI CHẠY ELEMENT QUERY +++
                    log.debug(f"--- TX LOG: Running Element Query {i+1}/{len(elements_param)} ---")
                    log.debug(f"Element Query: {element_query}")
                    log.debug(f"Element Params: {element_params}")
                    # Kiểm tra kiểu dữ liệu trong element_props_to_set
                    invalid_types_found = False
                    for k, v in element_props_to_set.items():
                        # Chỉ cho phép các kiểu Neo4j hỗ trợ làm thuộc tính
                        if not isinstance(v, (str, int, float, bool, list, type(None))): # List được phép (nhưng phải là list nguyên thủy)
                           # KIỂM TRA LIST: Nếu là list, các phần tử bên trong phải là nguyên thủy
                           if isinstance(v, list):
                               for item in v:
                                   if not isinstance(item, (str, int, float, bool, type(None))):
                                       log.error(f"INVALID TYPE in element props: Key='{k}', List item Type='{type(item)}', Value='{item}'")
                                       invalid_types_found = True
                                       break # Chỉ cần tìm thấy 1 lỗi là đủ
                           else: # Nếu không phải list và cũng không phải nguyên thủy khác
                               log.error(f"INVALID TYPE in element props: Key='{k}', Type='{type(v)}', Value='{v}'")
                               invalid_types_found = True
                    if invalid_types_found:
                         # Có thể raise lỗi ở đây để dừng transaction hoặc chỉ log warning
                         log.error(f"!!! ABORTING ELEMENT MERGE due to invalid type in props for element {i+1} !!!")
                         # raise TypeError(f"Invalid property type found for element {element_id_val}") # Dừng hẳn
                         continue # Hoặc bỏ qua element này và tiếp tục

                    # ++++++++++++++++++++++++++++++++++++++++++++
                    tx.run(element_query, element_params).consume() # <<< CHẠY QUERY >>>
                    log.debug(f"--- TX LOG: Element Query {i+1} OK ---")
                # Kết thúc vòng lặp element
                log.info(f"--- Neo4j Transaction for screen {screen_id_param} completed ---")

            # Gọi hàm transaction
            session.write_transaction(
                merge_screen_and_elements_tx,
                screen_id, screen_props_to_set, standardized_elements
            )
            logger.info(f"Successfully merged Screen node '{screen_id}' and its elements.")
            return True

    except Exception as e:
        # Log lỗi chi tiết hơn
        if isinstance(e, neo4j.exceptions.CypherTypeError):
             logger.error(f"Neo4j CypherTypeError DETECTED (merge_screen for {screen_id}): {e.message} - Code: {e.code}", exc_info=False)
        else:
            logger.error(f"Neo4j Transaction Error (merge_screen for {screen_id}): {type(e).__name__} - {e}", exc_info=True)

        if "element" in str(e).lower() or isinstance(e, neo4j.exceptions.CypherTypeError):
             logger.error(f"Data that might have caused error in elements (first 5):")
...
# Trong app/graph_db.py

def get_elements_for_screen(screen_id: str) -> list[dict] | None:
    """
    Lấy danh sách các node Element liên kết với một Screen cụ thể.
    Đã sửa lỗi AttributeError cho neo4j.time.LocalDateTime.
    """
    logger = current_app.logger if current_app else log
    if not screen_id:
        logger.warning("get_elements_for_screen: screen_id là bắt buộc.")
        return []

    driver = get_driver()
    if not driver:
        logger.error("get_elements_for_screen: Không lấy được Neo4j driver.")
        return None
    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            """
            logger.debug(f"Executing Cypher to get elements for screen '{screen_id}': {query}")
            result = session.run(query, screen_id=screen_id)
            for record in result:
                props = record["element_properties"]
                if props:
                    processed_props = {}
                    for key, value in props.items():
                        # Kiểm tra các kiểu thời gian phổ biến của neo4j.time
                        # Bỏ neo4j.time.LocalDateTime nếu nó gây lỗi
                        is_neo4j_time_type = False
                        if hasattr(neo4j, 'time'): # Kiểm tra module time tồn tại
                            if hasattr(neo4j.time, 'DateTime') and isinstance(value, neo4j.time.DateTime):
                                is_neo4j_time_type = True
                            elif hasattr(neo4j.time, 'Date') and isinstance(value, neo4j.time.Date):
                                is_neo4j_time_type = True
                            elif hasattr(neo4j.time, 'Time') and isinstance(value, neo4j.time.Time):
                                is_neo4j_time_type = True
                            # Bạn có thể thêm các kiểu khác nếu cần và nếu chúng tồn tại
                            # Ví dụ: Duration, nhưng nó không phải là datetime để convert sang ISO

                        if isinstance(value, datetime) or is_neo4j_time_type:
                            try:
                                # Đối với datetime của Python
                                if isinstance(value, datetime):
                                    if value.tzinfo is None: # Nếu là naive datetime, gán timezone UTC
                                        value_to_format = value.replace(tzinfo=timezone.utc)
                                    else:
                                        value_to_format = value
                                    processed_props[key] = value_to_format.isoformat()
                                # Đối với các kiểu neo4j.time có to_native()
                                elif hasattr(value, 'to_native') and callable(value.to_native):
                                    native_datetime = value.to_native()
                                    if isinstance(native_datetime, datetime):
                                        if native_datetime.tzinfo is None:
                                            native_datetime = native_datetime.replace(tzinfo=timezone.utc)
                                        processed_props[key] = native_datetime.isoformat()
                                    else: # Nếu to_native() không trả về datetime (ví dụ: Date, Time)
                                        processed_props[key] = str(native_datetime) # Chuyển thành string
                                else: # Fallback nếu không có to_native() hoặc không phải datetime Python
                                    processed_props[key] = str(value)
                            except Exception as dt_err:
                                logger.warning(f"Could not convert datetime-like object {value} for key '{key}' to ISO string: {dt_err}")
                                processed_props[key] = str(value) # Fallback to string
                        elif isinstance(value, neo4j.spatial.Point):
                            point = value
                            processed_props[key] = {'x': point.x, 'y': point.y, 'srid': point.srid}
                        else:
                            processed_props[key] = value
                    elements_list.append(processed_props)
                else:
                    logger.warning(f"Tìm thấy Element node không có thuộc tính cho screen '{screen_id}'.")
            logger.debug(f"Lấy được {len(elements_list)} elements cho màn hình '{screen_id}'.")
            return elements_list
    except neo4j.exceptions.ServiceUnavailable as su_err:
        logger.error(f"Neo4j ServiceUnavailable (get_elements_for_screen for {screen_id}): {su_err}", exc_info=True)
        return None
    except AttributeError as ae: # Bắt cụ thể AttributeError nếu vẫn xảy ra
        logger.error(f"AttributeError trong get_elements_for_screen for {screen_id} (có thể liên quan đến neo4j.time): {ae}", exc_info=True)
        return None
    except Exception as e:
        logger.error(f"Lỗi Neo4j (get_elements_for_screen for {screen_id}): {type(e).__name__} - {e}", exc_info=True)
        return None
