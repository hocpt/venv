...
def add_stage():
    ...
    if not strategy_id:
        ...
        return redirect(url_for('admin.view_strategies_control')) # Tạm về control list

    strategy_details = None
    redirect_endpoint = 'admin.view_strategies_language' # Default redirect
    cancel_url = url_for('admin.view_strategies_language') # Default cancel URL
    ...
    if db:
        try:
            ...
            if strategy_details:
                ...
                cancel_url = url_for(redirect_endpoint, strategy_id=strategy_id) # URL cho nút Hủy
            else:
                 flash(f"Không tìm thấy strategy ID {strategy_id}.", "error")
                 # Vẫn dùng cancel_url default
        except Exception as e:
            print(f"Lỗi khi lấy strategy details trong add_stage: {e}")
            flash("Lỗi khi đọc thông tin strategy gốc.", "error")
            # Vẫn dùng cancel_url default

    if request.method == 'POST':
        # ... (Logic xử lý POST như đã cung cấp ở lần trước) ...
        ...
        if not stage_id or not form_strategy_id:
             flash("Stage ID và Strategy ID (ẩn) là bắt buộc.", "warning")
             # <<< Truyền cancel_url và strategy_id vào template khi lỗi POST >>>
             return render_template('admin_add_stage.html', title="Thêm Stage (Lỗi)",
                                    strategy_id=strategy_id, cancel_url=cancel_url,
                                    current_data=request.form), 400
        if form_strategy_id != strategy_id:
             flash("Lỗi không khớp Strategy ID trong form và URL.", "error")
             return redirect(url_for(redirect_endpoint, strategy_id=strategy_id))
        ...
        try:
            success, error_msg = db.add_new_stage(stage_id, form_strategy_id, description, order)
            if success:
                flash(f"Thêm stage '{stage_id}' thành công!", 'success')
                # <<< Redirect dùng redirect_endpoint đã xác định >>>
                return redirect(url_for(redirect_endpoint, strategy_id=form_strategy_id))
            else:
                flash(f"Thêm stage '{stage_id}' thất bại: {error_msg or '(ID đã tồn tại?)'}", 'error')
                return render_template('admin_add_stage.html', title="Thêm Stage (Lỗi DB)",
                                       strategy_id=strategy_id, cancel_url=cancel_url,
                                       current_data=request.form)
        except Exception as e:
    ...
    # <<< Truyền cancel_url vào template >>>
...
def edit_stage(stage_id):
    ...
    if not db:
        flash("Lỗi DB.", "error")
        # Không biết về trang nào, tạm về control list
        return redirect(url_for('admin.view_strategies_control'))

    # Lấy thông tin stage hiện tại cho cả GET và POST (nếu có lỗi)
    # Hàm get_stage_details cần được sửa để lấy cả identifying_elements
    stage = db.get_stage_details(stage_id)
    if not stage:
        flash(f"Không tìm thấy stage ID {stage_id}.", "error")
        return redirect(url_for('admin.view_strategies_control')) # Tạm

    # Lấy strategy_id và type để redirect về đúng trang
    strategy_id_redirect = stage.get('strategy_id')
    strategy_details = db.get_strategy_details(strategy_id_redirect) if strategy_id_redirect else None
    strategy_type = strategy_details.get('strategy_type', 'language') if strategy_details else 'language'
    redirect_endpoint = 'admin.view_strategy_stages_language' if strategy_type == 'language' else 'admin.view_strategy_stages_control'
    cancel_url = url_for(redirect_endpoint, strategy_id=strategy_id_redirect) if strategy_id_redirect else url_for('admin.view_strategies_language')
    ...
    if request.method == 'POST':
        description = request.form.get('description')
        order_str = request.form.get('stage_order', '0')
        identifying_elements_str = request.form.get('identifying_elements', '{}') # Lấy JSON string
        ...
        try:
            order = int(order_str)
             ...
             flash("Stage Order phải là số nguyên.", "warning")
             # Truyền lại dữ liệu lỗi và stage gốc
             return render_template('admin_edit_stage.html', title=title + " (Lỗi)",
                                    stage=stage, cancel_url=cancel_url, current_data=request.form), 400
        ...
        if identifying_elements_str and identifying_elements_str.strip() and identifying_elements_str != '{}':
            try:
                json.loads(identifying_elements_str) # Chỉ cần validate
            except json.JSONDecodeError:
                 flash("Identifying Elements không phải là JSON hợp lệ. Vui lòng nhập đúng định dạng hoặc để trống/ {}.", "warning")
                 return render_template('admin_edit_stage.html', title=title + " (Lỗi)",
                                        stage=stage, cancel_url=cancel_url, current_data=request.form), 400
        else:
             # Nếu người dùng nhập rỗng hoặc chỉ khoảng trắng, coi như JSON rỗng
             identifying_elements_str = '{}'


        try:
            # <<< Gọi hàm update_stage đã sửa để nhận identifying_elements_str >>>
            success, error_msg = db.update_stage(stage_id, description, order, identifying_elements_str)

            if success:
                flash(f"Cập nhật stage '{stage_id}' thành công!", 'success')
                return redirect(cancel_url) # Redirect về trang chi tiết strategy gốc
            else:
                flash(f"Cập nhật stage '{stage_id}' thất bại: {error_msg or 'Không có gì thay đổi?'}", 'error')
                # Render lại form với lỗi, giữ giá trị nhập lỗi
                return render_template('admin_edit_stage.html', title=title + " (Lỗi DB)",
                                       stage=stage, cancel_url=cancel_url, current_data=request.form)
        except Exception as e:
    ...
    # GET request
    # Truyền stage details và cancel_url vào template
...
def add_transition_control():
    """Thêm transition cho Control Strategy (dùng Macro Code)."""
    strategy_id = request.args.get('strategy_id')
    current_stage_id_prefill = request.args.get('current_stage_id')
    if not strategy_id: flash("Cần strategy_id.", "error"); return redirect(url_for('admin.view_strategies_control'))
    ...
    if request.method == 'POST':
        current_stage_id = request.form.get('current_stage_id')
        user_intent = request.form.get('user_intent') # Trigger signal for control
        ...
        # Validate (tương tự logic cũ nhưng tập trung vào macro/condition)
        if not current_stage_id or not user_intent:
             flash("Current Stage và User Intent/Trigger là bắt buộc.", "warning")
             return render_template('admin_add_transition_control.html', title="Thêm Control Transition (Lỗi)",
                                    strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                    strategy_stages=strategy_stages, all_stages=all_stages, all_macros=all_macros,
                                    valid_intents=VALID_INTENTS_FOR_TRANSITION, valid_condition_types=VALID_CONDITION_TYPES,
                                    current_data=request.form), 400
        try: priority = int(priority_str)
        except ValueError: flash("Priority phải là số nguyên.", "warning"); #... return render_template ...
        # Validate params JSON
        if action_params_str and action_params_str.strip() and action_params_str != '{}':
             try: json.loads(action_params_str)
             except json.JSONDecodeError: flash("Action Params JSON không hợp lệ.", "warning"); #... return render_template ...

        # Gọi hàm DB (truyền None cho response_template_ref)
        success, error_msg = db.add_new_transition(
            current_stage_id, user_intent, next_stage_id, priority, None, # response_template_ref = None
            action_macro_code if action_macro_code else None,
            action_params_str if (action_params_str and action_params_str.strip() and action_params_str != '{}') else None,
            condition_type, condition_value
        )

        if success:
            flash('Thêm control transition thành công!', 'success')
            # Redirect về trang chi tiết control
            ...
            # Render lại form control
            return render_template('admin_add_transition_control.html', title="Thêm Control Transition (Lỗi DB)",
                                   strategy_id=strategy_id, current_stage_id_prefill=current_stage_id,
                                   strategy_stages=strategy_stages, all_stages=all_stages, all_macros=all_macros,
                                   valid_intents=VALID_INTENTS_FOR_TRANSITION, valid_condition_types=VALID_CONDITION_TYPES,
                                   current_data=request.form)
    ...
    # GET request
    return render_template('admin_add_transition_control.html', title=f"Thêm Control Transition cho Strategy {strategy_id}",
...
def edit_transition_control(transition_id):
    """Sửa transition cho Control Strategy."""
    transition = db.get_transition_details(transition_id)
    if not transition: flash(f"Ko tìm thấy transition ID {transition_id}.", "error"); return redirect(url_for('admin.view_strategies_control'))
    strategy_id_redirect = transition.get('strategy_id')
    strategy_details = db.get_strategy_details(strategy_id_redirect)
    if not strategy_details or strategy_details.get('strategy_type') != 'control':
         flash("Transition này không thuộc về Control Strategy.", "error")
         return redirect(url_for('admin.view_strategies_control'))
    ...
    if request.method == 'POST':
        ...
        # Gọi hàm update DB (truyền None cho response_template_ref)
        success, error_msg = db.update_transition(
            transition_id, current_stage_id, user_intent, next_stage_id, priority, None, # response_template_ref = None
            action_macro_code if action_macro_code else None,
            action_params_str if (action_params_str and action_params_str.strip() and action_params_str != '{}') else None,
            condition_type, condition_value
        )
        ...
        if success:
            flash('Cập nhật control transition thành công!', 'success')
            return redirect(url_for('admin.view_strategy_stages_control', strategy_id=strategy_id_redirect))
        else:
            flash(f'Cập nhật control transition thất bại: {error_msg or "Lỗi."}', 'error')
            # Render lại form control edit
            return render_template('admin_edit_transition_control.html', title=f"Sửa Control Transition {transition_id} (Lỗi DB)",
                                   transition=transition, strategy_stages=strategy_stages, all_stages=all_stages, all_macros=all_macros,
                                   valid_intents=VALID_INTENTS_FOR_TRANSITION, valid_condition_types=VALID_CONDITION_TYPES,
                                   current_data=request.form)

    # GET request
    return render_template('admin_edit_transition_control.html', title=f"Sửa Control Transition {transition_id}",
                           transition=transition, # Đã chứa các trường macro/condition
                           strategy_stages=strategy_stages, all_stages=all_stages, all_macros=all_macros,
                           valid_intents=VALID_INTENTS_FOR_TRANSITION, valid_condition_types=VALID_CONDITION_TYPES)
...
def delete_transition(transition_id):
    ...
    # Lấy strategy_id và type TRƯỚC KHI XÓA để biết đường redirect
    transition_details = db.get_transition_details(transition_id)
    strategy_id_redirect = None
    strategy_type_redirect = 'language' # Default về language nếu không tìm thấy
    if transition_details:
         strategy_id_redirect = transition_details.get('strategy_id')
         # Lấy type của strategy cha
         strategy_info = db.get_strategy_details(strategy_id_redirect) if strategy_id_redirect else None
         if strategy_info: strategy_type_redirect = strategy_info.get('strategy_type', 'language')

    try:
        success = db.delete_transition(transition_id) # Gọi hàm xóa trong DB
        if success: flash(f"Đã xóa transition ID {transition_id}.", 'success')
        else: flash(f"Xóa transition ID {transition_id} thất bại (ID không tồn tại?).", 'error')
    except Exception as e:
        print(f"Lỗi nghiêm trọng khi xóa transition {transition_id}: {e}")
        flash(f"Lỗi không mong muốn khi xóa transition: {e}", "error")

    # Redirect về trang strategy detail tương ứng với type
    if strategy_id_redirect:
         if strategy_type_redirect == 'control':
              return redirect(url_for('admin.view_strategy_stages_control', strategy_id=strategy_id_redirect))
         else:
         ...
         # Nếu không tìm được strategy gốc, quay về trang strategy list mặc định (ví dụ: language)
         return redirect(url_for('admin.view_strategies_language'))
