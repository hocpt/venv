...
def check_defined_screen_id_exists(app_name: str, defined_screen_id: str) -> bool:
    """
    Checks if a defined_screen_id already exists for a given app_name.
    """
    conn = None
    try:
        conn = get_db_connection()
        if not conn:
            current_app.logger.error("Không thể lấy kết nối CSDL trong check_defined_screen_id_exists.")
            # Coi như tồn tại để tránh ghi đè nếu không check được
            return True # Hoặc ném Exception
        with conn.cursor() as cur:
            sql = "SELECT 1 FROM screen_definitions WHERE app_name = %s AND defined_screen_id = %s LIMIT 1;"
            cur.execute(sql, (app_name, defined_screen_id))
            return cur.fetchone() is not None
    except psycopg2.Error as e:
        current_app.logger.error(f"Lỗi CSDL khi kiểm tra screen_id '{defined_screen_id}' cho app '{app_name}': {e}", exc_info=True)
        return True # Giả định là tồn tại nếu có lỗi DB để tránh ghi đè
    except Exception as ex:
        current_app.logger.error(f"Lỗi không mong muốn khi kiểm tra screen_id: {ex}", exc_info=True)
        return True
    finally:
        if conn:
            conn.close()
...
def create_new_pie_definition_from_node(
    ...
    """
    Creates a new PIE definition in the screen_definitions table.
    Args:
        app_name: Name of the application.
        activity_name: Activity name (optional).
        logical_name: User-friendly logical name for the PIE.
        new_defined_screen_id: The new unique screen_id for this PIE for this app.
        conditions: List of condition objects for this PIE.
        description: Optional description for the PIE.
    Returns:
        A tuple (success: bool, error_message: str | None, new_pie_db_id: int | None).
    """
    conn = None
    try:
        # Kiểm tra xem new_defined_screen_id đã tồn tại cho app_name này chưa
        if check_defined_screen_id_exists(app_name, new_defined_screen_id):
            return False, f"Defined Screen ID '{new_defined_screen_id}' đã tồn tại cho ứng dụng '{app_name}'. Vui lòng chọn ID khác.", None

        conn = get_db_connection()
        if not conn:
            current_app.logger.error("Không thể lấy kết nối CSDL trong create_new_pie_definition_from_node.")
            return False, "Lỗi kết nối cơ sở dữ liệu.", None

        with conn.cursor() as cur:
            # Chuyển đổi conditions list thành JSON string
            conditions_json_to_store = json.dumps(conditions)

            sql = """
        INSERT INTO screen_definitions 
            (app_name, activity_name, logical_screen_name, defined_screen_id, 
             identifying_elements, description, created_at, updated_at) 
        VALUES (%s, %s, %s, %s, %s, %s, NOW(), NOW()); -- <<< DẤU CHẤM PHẨY Ở ĐÂY CÓ THỂ GÂY LỖI
        RETURNING definition_id; -- Hoặc tên cột ID của bạn, ví dụ 'id'
    """
            # Chú ý: `screen_id` trong bảng DB tương ứng với `new_defined_screen_id` của chúng ta
            cur.execute(sql, (
                app_name, activity_name, logical_name, new_defined_screen_id, 
                conditions_json_to_store, description
            ))
            new_id = cur.fetchone()
            if new_id and new_id[0] is not None:
                conn.commit()
                current_app.logger.info(f"Đã tạo PIE definition mới: app='{app_name}', screen_id='{new_defined_screen_id}', db_id={new_id[0]}.")
                return True, None, new_id[0]
            else:
                conn.rollback()
                current_app.logger.error(f"Không thể tạo PIE definition cho {app_name}/{new_defined_screen_id}, RETURNING id không trả về gì.")
                return False, "Không thể tạo PIE definition (lỗi không xác định khi ghi vào DB).", None

    except psycopg2.IntegrityError as ie: # Bắt lỗi cụ thể nếu có ràng buộc unique khác bị vi phạm
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi Integrity CSDL khi tạo PIE cho {app_name}/{new_defined_screen_id}: {ie}", exc_info=True)
        # Kiểm tra xem có phải lỗi do new_defined_screen_id đã tồn tại không (dù đã check ở trên, nhưng có thể có race condition)
        if "screen_definitions_app_name_screen_id_key" in str(ie).lower() or \
           (hasattr(ie, 'diag') and ie.diag and "screen_definitions_app_name_screen_id_key" in str(ie.diag.constraint_name).lower()):
            return False, f"Defined Screen ID '{new_defined_screen_id}' đã tồn tại cho ứng dụng '{app_name}'. Vui lòng chọn ID khác.", None
        return False, f"Lỗi ràng buộc cơ sở dữ liệu: {ie}", None
    except psycopg2.Error as e:
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi CSDL trong create_new_pie_definition_from_node cho {app_name}/{new_defined_screen_id}: {e}", exc_info=True)
        return False, f"Lỗi cơ sở dữ liệu khi tạo PIE: {e}", None
    except Exception as ex:
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi không mong muốn trong create_new_pie_definition_from_node cho {app_name}/{new_defined_screen_id}: {ex}", exc_info=True)
        return False, f"Lỗi máy chủ không mong muốn khi tạo PIE: {ex}", None
    finally:
        if conn:
            conn.close()
