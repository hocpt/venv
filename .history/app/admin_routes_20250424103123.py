...
def edit_strategy(strategy_id):
    ...
    if request.method == 'POST':
        # ... (Lấy dữ liệu form và gọi db.update_strategy như cũ) ...
        name = request.form.get('name')
        description = request.form.get('description')
        initial_stage_id = request.form.get('initial_stage_id')
        # strategy_type không lấy từ form vì không cho sửa

        # Validate (giữ nguyên)
        if not name or not initial_stage_id:
             flash("Name và Initial Stage ID là bắt buộc.", "warning")
             # Cần lấy lại stages và strategy để render lại form lỗi
             stages = db.get_all_stages() or []
             # strategy đã được lấy ở đầu hàm
             return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id} (Lỗi)",
                                    strategy=strategy, stages=stages,
                                    current_data=request.form), 400

        # Gọi hàm update DB (giữ nguyên)
        success, error_msg = db.update_strategy(strategy_id, name, description, initial_stage_id)

        # --- SỬA LẠI LOGIC REDIRECT Ở ĐÂY ---
        if success:
            flash('Cập nhật chiến lược thành công!', 'success')
            # Lấy lại strategy_type từ biến 'strategy' đã lấy ở đầu hàm lúc GET
            # hoặc nếu POST lỗi thì biến strategy này vẫn tồn tại
            strategy_type_redirect = strategy.get('strategy_type', 'language') # Default về language nếu không lấy được

            # Redirect đến trang danh sách tương ứng
            if strategy_type_redirect == 'control':
                 redirect_url = url_for('admin.view_strategies_control')
            elif strategy_type_redirect == 'mainloop': # <<< THÊM KIỂM TRA MAINLOOP
            ...
            print(f"DEBUG (edit_strategy): Redirecting to {redirect_url} based on type {strategy_type_redirect}") # Thêm log debug
            return redirect(redirect_url)
        else:
            # ... (Xử lý lỗi update và render lại form như cũ) ...
    ...
    # ... (Lấy strategy, stages và render template) ...
    stages = []
    try: stages = db.get_all_stages() or []
    except Exception: flash("Lỗi tải stages", "error")
    # strategy đã được lấy ở đầu hàm
...
def view_strategy_definition_json(strategy_id):
    """
    Trả về định nghĩa thô của một strategy (details, stages, transitions) dưới dạng JSON.
    """
    logger = current_app.logger if current_app else print
    if not db:
        return jsonify({"error": "Database module not available"}), 500
    ...
    try:
        # 1. Lấy thông tin chi tiết strategy
        strategy_details = db.get_strategy_details(strategy_id)
        if not strategy_details:
            return jsonify({"error": f"Strategy ID '{strategy_id}' not found."}), 404

        # 2. Lấy danh sách stages thuộc strategy này
        strategy_stages = db.get_stages_for_strategy(strategy_id)
        if strategy_stages is None: # Phân biệt lỗi DB và list rỗng
             logger.warning(f"Could not fetch stages for strategy '{strategy_id}' while generating JSON def.")
             strategy_stages = [] # Coi như list rỗng nếu lỗi

        # 3. Lấy danh sách transitions thô thuộc strategy này
        strategy_transitions = db.get_strategy_action_sequence(strategy_id)
        if strategy_transitions is None: # Phân biệt lỗi DB và list rỗng
             logger.warning(f"Could not fetch transitions for strategy '{strategy_id}' while generating JSON def.")
             strategy_transitions = [] # Coi như list rỗng nếu lỗi

        # 4. Tạo đối tượng dictionary chứa tất cả thông tin
        strategy_definition = {
        ...
        # Dùng indent=2 để format JSON cho dễ đọc trong trình duyệt
        response = jsonify(strategy_definition)
        response.headers['Content-Type'] = 'application/json; charset=utf-8' # Đảm bảo UTF-8
        # Gợi ý trình duyệt hiển thị đẹp hơn (tùy chọn)
        import json
        response.set_data(json.dumps(strategy_definition, indent=2, ensure_ascii=False))
        return response
