...
def update_task_mapping_status(assignment_id: int, new_status: str) -> tuple[bool, str | None]:
    """
    Cập nhật trạng thái mapping ('active' hoặc 'paused') cho một Task Assignment.

    Args:
        assignment_id: ID của Task Assignment cần cập nhật.
        new_status: Trạng thái mới ('active' hoặc 'paused').

    Returns:
        Tuple (bool, str | None): (True, None) nếu thành công, (False, error_message) nếu thất bại.
    """
    if new_status not in ['active', 'paused']:
        return False, f"Trạng thái '{new_status}' không hợp lệ. Chỉ chấp nhận 'active' hoặc 'paused'."
    if assignment_id is None:
        return False, "Cần cung cấp assignment_id."
    ...
    try:
        conn = get_db_connection() # Lấy kết nối CSDL
        if not conn:
            return False, "Không thể kết nối CSDL."
        ...
        if cur.rowcount > 0:
            conn.commit()
            success = True
            current_app.logger.info(f"Updated mapping_status to '{new_status}' for assignment_id: {assignment_id}")
        else:
            # Không có dòng nào được update, có thể do assignment_id không tồn tại
            error_msg = f"Không tìm thấy Task Assignment có ID {assignment_id} để cập nhật trạng thái."
            conn.rollback() # Rollback nếu không update được dòng nào

    except psycopg2.Error as db_err:
        error_msg = f"Lỗi CSDL khi cập nhật mapping_status: {db_err}"
        current_app.logger.error(error_msg, exc_info=True)
        if conn: conn.rollback()
    except Exception as e:
        error_msg = f"Lỗi không xác định khi cập nhật mapping_status: {e}"
        current_app.logger.error(error_msg, exc_info=True)
        if conn: conn.rollback()
    finally:
        if cur: cur.close()
