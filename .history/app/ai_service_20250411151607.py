...
# === Đặt hàm này vào backup/app/ai_service.py ===
...
def suggest_rule_from_interaction(interaction_data: dict, persona_id: str) -> tuple[str | None, str | None]:
    ...
    try:
        persona = db.get_persona_details(persona_id)
        template_content = db.get_prompt_template_by_task(task_type) # Hàm mới tạo
    except Exception as db_err:
        print(f"LỖI (suggest_rule): Không thể lấy Persona/Template từ DB: {db_err}")
        return None, None # Trả về theo signature của hàm

    # Xử lý nếu không tìm thấy persona hoặc template
    base_prompt_fallback = "You are an AI analyzing conversations to suggest rules."
    if not persona:
        print(f"WARNING (suggest_rule): Không tìm thấy Persona ID '{persona_id}'. Sử dụng base prompt mặc định.")
        persona = {'base_prompt': base_prompt_fallback, 'model_name': None, 'generation_config': None} # Tạo persona giả
    if not template_content:
        print(f"LỖI (suggest_rule): Không tìm thấy Prompt Template cho task_type '{task_type}'. Không thể tạo đề xuất.")
        return None, None # Cần template này
    ...
    try:
        ...
        # Tạo context, kết hợp base_prompt và dữ liệu interaction
        render_context = {
            "base_prompt": persona.get('base_prompt', base_prompt_fallback), # Dùng fallback nếu base_prompt rỗng
            **interaction_data # Thêm các key từ interaction_data
        }
        final_prompt = jinja_template.render(render_context)
        print(f"DEBUG (suggest_rule): Final Prompt (first 500 chars):\n{final_prompt[:500]}...")
        ...
        if response.parts:
            ...
            if ai_text:
                ...
                if keywords_match:
                    suggested_keywords = keywords_match.group(1).strip()
                    # <<< SỬA LẠI REGEX Ở ĐÂY >>>
                    # Xóa khoảng trắng, *, - ở đầu hoặc cuối chuỗi/dòng
                    suggested_keywords = re.sub(r'^[\s*-]+|[\s*-]+$', '', suggested_keywords, flags=re.MULTILINE).strip()
                    print(f"DEBUG (suggest_rule): Extracted & Cleaned Keywords: '{suggested_keywords}'")
                else:
                ...
                if template_match:
                    ...
                    if "cannot generalize" in suggested_template.lower():
                        ...
                        print("INFO (suggest_rule): AI indicated template cannot be generalized.")
                         ...
                         print(f"DEBUG (suggest_rule): Extracted & Cleaned Template Text: '{suggested_template[:200]}...'")
                else:
    ...
    print(f"INFO (suggest_rule): Suggestion generation completed with status: {status}")
    # Trả về keywords và template, status chỉ dùng để log/debug nội bộ
    return suggested_keywords, suggested_template
