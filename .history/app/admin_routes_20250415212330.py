...
def view_ai_simulations():
    ...
    if not db: flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
        ...
        try:
            ...
            # 3. Tạo danh sách hiển thị - Duyệt qua các Lệnh trước
            for cmd_id, command_data in command_map.items():
                payload = command_data.get('payload', {})
                status = command_data.get('status', 'unknown')

                # --- Tạo chuỗi config_info chi tiết ---
                cfg_pa = payload.get('persona_a_id','?')
                cfg_pb = payload.get('persona_b_id','?')
                cfg_stra = payload.get('strategy_id','?')
                cfg_turns = payload.get('max_turns','?')
                ...
                sim_info = {
                    ...
                    'command_id': cmd_id,
                    'job_id': None,
                    'config_info': config_info_str, # <<< Dùng chuỗi mới
                ...
                if status == 'pending': sim_info['status_text'] = 'Pending Queue'
                elif status == 'processing': sim_info['status_text'] = 'Processing Cmd'
                elif status == 'done': sim_info['status_text'] = 'Command Done'
                elif status == 'error': sim_info['status_text'] = f"Command Error: {sim_info['error_message'][:100]}" if sim_info['error_message'] else 'Command Error'

                # Tìm job live tương ứng (dựa vào cấu trúc job_id mới)
                corresponding_job_id = None
                for live_job_id in live_sim_jobs_dict.keys():
                     parts = live_job_id.split('_')
                     # job_id = f"sim_run_{command_id}_{uuid}"
                ...
                if corresponding_job_id:
                    sim_info['job_id'] = corresponding_job_id # Lưu job_id tìm được
                    sim_info['type'] = 'job' # Ưu tiên type là job
                    processed_command_ids.add(cmd_id) # Đánh dấu command này đã có job live
                    ...
                    if next_run_timestamp is not None:
                        try:
                            ...
                            sim_info['status_text'] = 'Scheduled' # Cập nhật trạng thái
                        except Exception: sim_info['next_run_time_str'] = 'Lỗi Format'
                    else:
                        sim_info['next_run_time_str'] = '---'
                        if status == 'done': sim_info['status_text'] = 'Job Completed/Removed?'
                        else: sim_info['status_text'] = 'Job Paused?'

                simulations_display_list.append(sim_info)

            # 4. Thêm các live job không tìm thấy command tương ứng (job cũ?)
            for live_job_id, next_run_timestamp in live_sim_jobs_dict.items():
                 ...
                 if command_id_from_job is None or command_id_from_job not in processed_command_ids:
                      # Tạo sim_info cho job không rõ nguồn gốc
                      sim_info = {'id': live_job_id, 'type': 'job', 'config_info': '(Live Job - Unknown Origin)', 'status_text': 'Scheduled', 'created_at': None, 'command_id': None, 'error_message': None}
                      if next_run_timestamp is not None:
                          try:
                              utc_dt = datetime.fromtimestamp(next_run_timestamp, tz=timezone.utc)
                              local_dt = utc_dt.astimezone(server_tz)
                              sim_info['next_run_time_str'] = local_dt.strftime('%Y-%m-%d %H:%M:%S %z')
                          except: sim_info['next_run_time_str'] = 'Lỗi Format'
                      else:
                          sim_info['next_run_time_str'] = '---'; sim_info['status_text'] = 'Paused?'
                      simulations_display_list.append(sim_info)
