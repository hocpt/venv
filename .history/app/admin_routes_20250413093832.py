...
def run_adhoc_simulation():
    """Xử lý yêu cầu chạy mô phỏng ad-hoc từ form."""
    # <<< BƯỚC 3: VÔ HIỆU HÓA KHỐI KIỂM TRA NÀY >>>
    # if not live_scheduler:
    #    flash("Lỗi: Scheduler không hoạt động. Không thể chạy tác vụ nền.", "error")
    ...
    try:
        ...
        strategy_id = request.form.get('strategy_id')
        max_turns_str = request.form.get('max_turns', '5') # Mặc định 5 nếu thiếu
        starting_prompt = request.form.get('starting_prompt', '').strip() # Có thể rỗng
        ...
        try:
            ...
            if not (1 <= max_turns <= 20): # Giới hạn hợp lý
        ...
        if errors:
            for error in errors:
                flash(error, 'warning')
            # Cần lấy lại personas và strategies để render lại form với lỗi
            personas = db.get_all_personas() or []
            strategies = db.get_all_strategies() or []
            return render_template('admin_ai_simulations.html',
                                   title="Chạy Mô phỏng (Lỗi)",
                                   personas=personas, strategies=strategies,
                                   current_data=request.form), 400 # Trả về form với dữ liệu cũ

        # --- Chuẩn bị thông tin để thêm job ---
        # Tạo job ID duy nhất để tránh xung đột nếu nhấn nút nhiều lần
        job_id = f"sim_run_{sim_thread_id_base}_{uuid.uuid4().hex[:8]}"
        # Tạo tuple các tham số cho hàm tác vụ nền
        job_args = (
            persona_a_id, persona_b_id, strategy_id, max_turns,
            starting_prompt if starting_prompt else None, # Truyền None nếu rỗng
            sim_account_id, sim_thread_id_base, sim_goal
        )
        # Lên lịch chạy ngay lập tức (hoặc sau 1 giây)
        run_time = datetime.now(pytz.timezone('Asia/Ho_Chi_Minh')) + timedelta(seconds=1) # Thêm timezone

        print(f"INFO: Scheduling adhoc simulation job '{job_id}' with args: {job_args}")

        # --- Thêm Job vào Scheduler ---
        live_scheduler.add_job(
            id=job_id,
            func=run_ai_conversation_simulation, # Truyền thẳng đối tượng hàm
            args=job_args,
            trigger='date',
            run_date=run_time,
            jobstore='default', # Chỉ định jobstore nếu cần
            executor='default', # Chọn executor phù hợp (default=threadpool)
            replace_existing=False, # Không ghi đè job khác
            misfire_grace_time=60 # Cho phép trễ 1 phút
        )

        flash(f"Đã yêu cầu chạy mô phỏng '{sim_goal}' giữa {persona_a_id} và {persona_b_id}. Tác vụ sẽ chạy nền (Job ID: {job_id[:15]}...).", 'success')

    except Exception as e:
        print(f"Lỗi nghiêm trọng khi kích hoạt mô phỏng: {e}")
        print(traceback.format_exc())
        flash(f"Đã xảy ra lỗi không mong muốn khi yêu cầu chạy mô phỏng: {e}", "error")

    return redirect(url_for('admin.view_ai_simulations')) # Luôn redirect về trang quản lý

@admin_bp.route('/ai-simulations/run-adhoc', methods=['POST'])
def run_adhoc_simulation():
    """Xử lý yêu cầu chạy mô phỏng ad-hoc bằng cách thêm lệnh vào DB queue."""
    # Bỏ kiểm tra `if not live_scheduler:` và `if not run_ai_conversation_simulation:` vì route này chỉ thêm vào DB

    try:
        # --- Lấy dữ liệu từ form (giữ nguyên) ---
        persona_a_id = request.form.get('persona_a_id')
        persona_b_id = request.form.get('persona_b_id')
        strategy_id = request.form.get('strategy_id')
        ...
        try:
            ...
            errors.append(f"Số lượt nói tối đa không hợp lệ: {e}")
        if errors:
            # ... (code flash lỗi và render lại form như cũ) ...
    ...
    # Bỏ except AttributeError vì không còn gọi live_scheduler trực tiếp
