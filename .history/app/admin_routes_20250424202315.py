...
def add_simulation_config_view(): # <<< Sử dụng đúng tên hàm bạn đã cung cấp
    ...
    if not db:
        ...
        try:
            # 1. Lấy tất cả Personas và sắp xếp
            all_personas = db.get_all_personas() or []
            if all_personas is None: flash("Lỗi tải danh sách Personas.", "warning"); all_personas = []
            else: all_personas.sort(key=lambda x: x.get('name', '').lower()) # Sắp xếp theo tên

            # 2. Lấy tất cả Accounts và sắp xếp
            all_accounts = db.get_all_accounts() or [] # Hàm này cần trả về account_id, username, platform
            if all_accounts is None: flash("Lỗi tải danh sách Accounts.", "warning"); all_accounts = []
            else: all_accounts.sort(key=lambda x: x.get('username', x.get('account_id', '')).lower()) # Sắp xếp theo username hoặc ID

            # 3. Lấy tất cả Language Strategies và sắp xếp theo ID
            ...
            if raw_lang_strategies is None:
                ...
                all_language_strategies = []
                 ...
                 try:
                     ...
                     current_app.logger.debug(f"Fetched and sorted {len(all_language_strategies)} language strategies.")
    ...
    if request.method == 'POST':
        ...
        try:
            config_name = request.form.get('config_name', '').strip() # Đổi tên biến để rõ ràng hơn
            description = request.form.get('description', '').strip()
            persona_a_id = request.form.get('persona_a_id')
            persona_b_id = request.form.get('persona_b_id')
            account_log_a_id = request.form.get('account_log_a_id')
            account_log_b_id = request.form.get('account_log_b_id')
            strategy_id = request.form.get('strategy_id') # Strategy ID đã chọn
            iterations_str = request.form.get('iterations', '1').strip()
            enabled = request.form.get('enabled') == 'on'
            starting_prompt = request.form.get('starting_prompt', '').strip() # Lấy thêm starting prompt
            simulation_goal = request.form.get('simulation_goal', '').strip() # Lấy thêm simulation goal

            # --- Validate dữ liệu ---
            errors = []
            if not config_name: errors.append("Tên cấu hình là bắt buộc.")
            if not persona_a_id: errors.append("Vui lòng chọn Persona A.")
            # Persona B có thể là optional hoặc không, tùy bạn quy định
            # if not persona_b_id: errors.append("Vui lòng chọn Persona B.")
            if persona_a_id and persona_a_id == persona_b_id:
                errors.append("Persona A và Persona B không được giống nhau.")
            # Account Logs có thể là optional
            ...
            try:
                ...
                # Nên giới hạn iterations hợp lý, ví dụ max 50 hoặc 100
                if not (1 <= iterations <= 100): raise ValueError("Số lượt phải từ 1 đến 100.")
            except ValueError as e:
                errors.append(f"Số lần lặp không hợp lệ: {e}")

            if errors: # Nếu có lỗi validation
                for error in errors: flash(error, 'warning')
                # Render lại form với lỗi, truyền lại các list dropdown và current_data
                return render_template('admin_add_simulation_config.html',
                                       title=title + " (Lỗi)", cancel_url=cancel_url,
                                       personas=all_personas,
                                       accounts=all_accounts,
                                       strategies=all_language_strategies,
                                       current_data=current_data), 400 # Truyền current_data

            # --- Gọi hàm DB để thêm mới (Giả sử có hàm db.add_simulation_config) ---
            # Bạn cần tạo hàm này trong database.py nếu chưa có
            success, error_msg = db.add_simulation_config(
                ...
                log_account_id_a=account_log_a_id if account_log_a_id else None, # Log A optional
                log_account_id_b=account_log_b_id if account_log_b_id else None, # Log B optional
                strategy_id=strategy_id,
                max_turns=iterations, # Đổi tên tham số thành max_turns nếu hàm DB dùng tên này
            ...
            if success:
                ...
                flash(f"Thêm cấu hình '{config_name}' thất bại! {error_msg or '(Tên có thể đã tồn tại hoặc lỗi CSDL)'}", 'error')
                # Render lại form với dữ liệu cũ và lỗi DB
                return render_template('admin_add_simulation_config.html',
                                       title=title + " (Lỗi DB)", cancel_url=cancel_url,
                                       personas=all_personas,
                                       accounts=all_accounts,
                                       strategies=all_language_strategies,
                                       current_data=current_data) # Truyền current_data
