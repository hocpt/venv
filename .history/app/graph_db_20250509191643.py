    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            for record in result:
                ...
                if props: # Đảm bảo props không None
                    # Chuyển đổi Point thành dict nếu Neo4j trả về kiểu Point cho tọa độ
                    if 'coordinates' in props and isinstance(props['coordinates'], neo4j.spatial.Point):
                        point = props['coordinates']
                        # Giả sử là 2D Point
                        props['coordinates'] = {'x': point.x, 'y': point.y, 'srid': point.srid}
                    # Tương tự cho bounds nếu nó là kiểu dữ liệu đặc biệt
                    elements_list.append(dict(props)) # Chuyển thành dict Python chuẩn
                else:
