...
from app import graph_db # Module tương tác Neo4j
from app import ai_service
from app import database as db # Import database (PostgreSQL)
# ... (các hằng số, blueprint, etc.) ...
...
@admin_bp.route('/api/screen_elements_for_mapping/<path:screen_id>', methods=['GET'])
# @admin_required (nếu cần)
def api_get_screen_elements_for_mapping(screen_id):
    logger = current_app.logger if current_app else print
    if not graph_db:
        logger.error("API (screen_elements_for_mapping): GraphDB module chưa sẵn sàng.")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (GraphDB)"}), 500
    if not screen_id:
        logger.warning("API (screen_elements_for_mapping): Thiếu screen_id.")
        return jsonify({"success": False, "error": "Thiếu screen_id"}), 400

    try:
        # Hàm này cần được tạo trong graph_db.py
        # Nó sẽ query tất cả node :Element liên kết với :Screen có screen_id này
        # và trả về list các dict element (mỗi dict có element_id, bounds, coords, type, text...)
        elements = graph_db.get_elements_for_screen(screen_id)

        if elements is None: # Phân biệt lỗi DB và không có element
            logger.error(f"API (screen_elements_for_mapping): Lỗi khi lấy elements cho screen '{screen_id}'.")
            return jsonify({"success": False, "error": "Không thể lấy dữ liệu elements từ CSDL."}), 500

        logger.info(f"API (screen_elements_for_mapping): Trả về {len(elements)} elements cho screen '{screen_id}'.")
        return jsonify({"success": True, "elements": elements})

    except Exception as e:
        logger.error(f"API (screen_elements_for_mapping): Lỗi không mong muốn cho screen '{screen_id}': {e}", exc_info=True)
...
def view_screen_definitions():
        ...
        if conn:
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                    cur.execute("SELECT DISTINCT app_name FROM screen_definitions ORDER BY app_name;")
                    distinct_app_names_for_filter = [row['app_name'] for row in cur.fetchall()]
            finally:
                conn.close()
        else:
            flash("Không thể kết nối đến CSDL để lấy danh sách app.", "error")
    except Exception as e:
        current_app.logger.error(f"Lỗi khi lấy distinct app_names cho screen_definitions filter: {e}", exc_info=True)
        flash(f"Lỗi tải danh sách app: {str(e)}", "error")

    selected_app_filter = request.args.get('app_name_filter', '') # Giá trị '' nghĩa là "-- Tất cả Apps --"
    page = request.args.get('page', 1, type=int)
    if page < 1: page = 1
    # PER_PAGE_DEFINITIONS nên được định nghĩa ở đầu file admin_routes.py hoặc lấy từ config
    PER_PAGE_DEFINITIONS = current_app.config.get('ADMIN_ITEMS_PER_PAGE_DEFS', 15) 
    ...
    if selected_app_filter: 
        current_app.logger.info(f"VIEW_SCREEN_DEFS: Lọc với app_name = '{selected_app_filter}'")
        # Khi lọc theo app cụ thể, hàm get_screen_definitions_for_app không có phân trang sẵn
        # Tạm thời lấy tất cả rồi phân trang ở Python, hoặc sửa hàm đó để hỗ trợ phân trang
        all_defs_for_app = db.get_screen_definitions_for_app(selected_app_filter, activity_name=None)
        
        if all_defs_for_app is None: 
            flash(f"Lỗi khi tải danh sách định nghĩa cho app '{selected_app_filter}'.", "error")
            definitions_list = []
        elif not all_defs_for_app:
             current_app.logger.info(f"VIEW_SCREEN_DEFS: Không tìm thấy định nghĩa nào trong DB cho app '{selected_app_filter}'.")
             definitions_list = [] # Để template hiển thị thông báo đúng
        else:
            # Phân trang thủ công cho trường hợp lọc theo app
            total_items_for_display = len(all_defs_for_app)
            if total_items_for_display > 0:
                start_index = (page - 1) * PER_PAGE_DEFINITIONS
                end_index = start_index + PER_PAGE_DEFINITIONS
                definitions_list = all_defs_for_app[start_index:end_index]
            else:
                definitions_list = []
            
    else: # selected_app_filter là rỗng, nghĩa là xem "Tất cả Apps"
        current_app.logger.info(f"VIEW_SCREEN_DEFS: Lấy tất cả định nghĩa, page {page}")
        defs_page, total_db_items = db.get_all_screen_definitions(page=page, per_page=PER_PAGE_DEFINITIONS)
        
        if defs_page is None: # Lỗi DB
            ...
            if total_items_for_display == 0 and not distinct_app_names_for_filter: # Không có app nào cả
                 flash("Chưa có định nghĩa màn hình nào được tạo. Hãy bắt đầu bằng cách thêm mới.", "info")


    # Tạo thông tin phân trang chung
    if total_items_for_display > 0:
        total_pages = math.ceil(total_items_for_display / PER_PAGE_DEFINITIONS)
        if page > total_pages and total_pages > 0: # Nếu trang yêu cầu vượt quá, đi đến trang cuối
            # Cần redirect để URL page khớp, hoặc chỉ hiển thị trang cuối nhưng URL không đổi
            # Tạm thời chỉ set page về total_pages để không lỗi hiển thị
            page = total_pages
            # Nếu phân trang thủ công (lọc theo app), cần lấy lại definitions_list cho trang cuối
            if selected_app_filter and all_defs_for_app:
                start_index = (page - 1) * PER_PAGE_DEFINITIONS
                end_index = start_index + PER_PAGE_DEFINITIONS
                definitions_list = all_defs_for_app[start_index:end_index]
        ...
        pagination_details = {
            ...
            # Thêm các query param khác để giữ lại khi chuyển trang (ví dụ: app_name_filter)
            'extra_params': {'app_name_filter': selected_app_filter} if selected_app_filter else {}
        }
    
    current_app.logger.debug(f"Rendering admin_screen_definitions.html with {len(definitions_list)} definitions for display.")
    return render_template('admin_screen_definitions.html', 
                           ...
                           pagination=pagination_details) # Truyền pagination
...
def view_node_management():
    ...
    # Lấy các app_name cho bộ lọc (tương tự như trang định nghĩa PIE)
    distinct_app_names_for_filter = [] 
    try:
        # distinct_app_names_for_filter = db.get_distinct_app_names_from_screen_definitions()
        # Hoặc lấy từ Neo4j nếu bạn có hàm đó
        conn = db.get_db_connection()
        if conn:
            try:
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                    cur.execute("SELECT DISTINCT app_name FROM screen_definitions ORDER BY app_name;") # Lấy từ screen_definitions để chỉ hiện app có PIE
                    distinct_app_names_for_filter = [row['app_name'] for row in cur.fetchall()]
            finally:
                conn.close()
    except Exception as e:
        current_app.logger.error(f"Lỗi khi lấy distinct app_names cho node management filter: {e}")


    selected_app_filter = request.args.get('app_name_filter', '')
    filter_status = request.args.get('filter_status', 'unknown') # Mặc định xem 'unknown'
    page = request.args.get('page', 1, type=int)
    PER_PAGE_NODE_ISSUES = 15 # Hoặc config

    nodes_data_list = []
    pagination_info = None

    # Chỉ gọi graph_db nếu app_name được chọn hoặc muốn xem tất cả (nếu filter_status không phụ thuộc app_name)
    # Hiện tại, get_screen_nodes_for_management nhận app_name có thể là None
    nodes_page, total_items = graph_db.get_screen_nodes_for_management(
        app_name=selected_app_filter if selected_app_filter else None, # Truyền None nếu không lọc app
        filter_status=filter_status,
        page=page,
        per_page=PER_PAGE_NODE_ISSUES
    )

    if nodes_page is None:
        flash("Lỗi khi tải dữ liệu Node từ Neo4j.", "error")
    else:
        nodes_data_list = nodes_page
        if total_items is not None and total_items > 0:
            # Tạo pagination_info (giống như trang định nghĩa PIE)
            total_pages = math.ceil(total_items / PER_PAGE_NODE_ISSUES)
            if page > total_pages and total_pages > 0: page = total_pages

            extra_params_for_pagination = {'app_name_filter': selected_app_filter, 'filter_status': filter_status}

            pagination_info = {
                'page': page, 'per_page': PER_PAGE_NODE_ISSUES, 
    ...
    if not nodes_data_list and selected_app_filter:
        flash(f"Không tìm thấy node nào khớp với bộ lọc cho app '{selected_app_filter}' và status '{filter_status}'.", "info")
    elif not nodes_data_list and not selected_app_filter and filter_status == 'unknown':
...
def api_delete_managed_node(screen_id):
    ...
    if success_neo4j:
        ...
        if screenshot_to_delete:
            from app.phone.utils import delete_screenshot_file_on_server # Cần import hàm này
            # Hàm delete_screenshot_file_on_server cần đường dẫn gốc SCREENSHOT_STORAGE_PATH từ config
            base_screenshot_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
            if base_screenshot_path:
                deleted_file, error_file = delete_screenshot_file_on_server(base_screenshot_path, app_name, screenshot_to_delete)
                if deleted_file:
                    logger.info(f"Đã xóa file screenshot: {screenshot_to_delete} cho app {app_name}")
                else:
...
@admin_bp.route('/api/mapping/management/nodes/<path:screen_id>/classify', methods=['POST'])
# @admin_required
def api_classify_managed_node(screen_id):
    ...
    data = request.get_json()
    app_name = data.get('app_name')
    node_classification = data.get('node_classification') # Có thể là chuỗi rỗng nếu muốn xóa phân loại

    if not app_name:
        return jsonify({"success": False, "error": "Thiếu tham số app_name."}), 400
    if node_classification is None: # Nếu client gửi null, coi như muốn xóa
        node_classification_to_set = None
    elif not isinstance(node_classification, str):
         return jsonify({"success": False, "error": "node_classification phải là chuỗi."}), 400
    else:
        node_classification_to_set = node_classification if node_classification.strip() else None


    logger.info(f"API: Yêu cầu phân loại Node '{screen_id}' của app '{app_name}' thành '{node_classification_to_set}'")
    success = graph_db.update_node_classification_in_neo4j(screen_id, app_name, node_classification_to_set)

    if success:
        return jsonify({"success": True, "message": f"Đã cập nhật phân loại cho Node {screen_id}."})
    else:
        # Hàm graph_db nên log lỗi cụ thể
        return jsonify({"success": False, "error": "Cập nhật phân loại Node thất bại."}), 500 

@admin_bp.route('/api/mapping/management/nodes', methods=['GET']) # Đảm bảo URL này là đúng
# @admin_required # Nếu bạn có xác thực
def api_get_managed_nodes():
    logger = current_app.logger
    try:
        # Lấy app_name từ query string, nó có thể là chuỗi rỗng nếu người dùng chọn "-- Tất cả Apps --"
        ...
        # Chuyển chuỗi rỗng app_name_filter thành None để hàm graph_db hiểu là không lọc theo app
        app_name_for_db = app_name_filter_from_req if app_name_filter_from_req else None

        logger.debug(f"API get_managed_nodes: app_filter_req='{app_name_filter_from_req}' (used as '{app_name_for_db}'), status_filter='{filter_status}', page={page}")

        nodes_page_data, total_items = graph_db.get_screen_nodes_for_management(
            app_name=app_name_for_db, # Truyền None nếu không lọc theo app
            filter_status=filter_status,
            page=page,
        ...
        if nodes_page_data is None: 
            logger.error(f"API get_managed_nodes: graph_db.get_screen_nodes_for_management trả về None.")
            return jsonify({"nodes": [], "pagination": {}, "error": "Lỗi tải dữ liệu Node từ cơ sở dữ liệu."}), 500

        total_pages_val = 0
        if total_items is not None and total_items > 0 and per_page_val > 0:
            total_pages_val = math.ceil(total_items / per_page_val)
        
        return jsonify({
            "nodes": nodes_page_data,
            "pagination": {
                "page": page,
                "per_page": per_page_val,
        ...
        return jsonify({"nodes": [], "pagination": {}, "error": "Lỗi server nội bộ."}), 500


@admin_bp.route('/serve_screenshot_for_node_management/<path:app_name>/<path:filename>')
# @admin_required
def serve_screenshot_for_node_management(app_name, filename):
    logger = current_app.logger
    base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
    if not base_path:
        logger.error("SCREENSHOT_STORAGE_PATH is not configured.")
        return "Screenshot path not configured", 500

    # Cấu trúc thư mục của bạn có thể là base_path/app_name/filename
    # Hoặc chỉ là base_path/filename nếu app_name đã có trong filename
    # Điều chỉnh cho phù hợp:
    directory_path = os.path.join(base_path, app_name) # Nếu app_name là thư mục con
    # directory_path = base_path # Nếu filename đã bao gồm app_name hoặc không phân chia theo app_name

    logger.debug(f"Attempting to serve screenshot: directory='{directory_path}', filename='{filename}'")
    try:
        return send_from_directory(directory_path, filename)
    except FileNotFoundError:
        logger.warning(f"Screenshot not found: {os.path.join(directory_path, filename)}")
...
# @admin_required
def api_merge_unknown_to_defined():
    logger = current_app.logger
    if not request.is_json:
        return jsonify({"success": False, "error": "Yêu cầu phải là JSON"}), 400
    ...
    if not all([unknown_screen_id, target_defined_screen_id, app_name]):
        logger.warning(f"API merge_unknown_to_defined: Thiếu tham số. Unknown: {unknown_screen_id}, TargetDefined: {target_defined_screen_id}, App: {app_name}")
        return jsonify({"success": False, "error": "Thiếu tham số bắt buộc (unknown_screen_id, target_defined_screen_id, app_name)."}), 400

    # --- Logic nghiệp vụ ---
    # 1. Kiểm tra xem target_defined_screen_id có thực sự là một PIE definition hợp lệ không (tùy chọn, nhưng nên có)
    pie_def = db.get_screen_definition_by_defined_id(target_defined_screen_id, app_name) # Cần tạo hàm này trong database.py
    if not pie_def:
        logger.warning(f"API merge_unknown_to_defined: Target Defined Screen ID '{target_defined_screen_id}' không phải là một định nghĩa PIE hợp lệ cho app '{app_name}'.")
        return jsonify({"success": False, "error": f"Target Defined Screen ID '{target_defined_screen_id}' không hợp lệ."}), 400

    # 2. Thực hiện việc merge trong Neo4j
    # Đây là phần phức tạp nhất, liên quan đến việc:
    #   - Tìm Node Unknown (source) và Node Defined (target)
    #   - Chuyển tất cả các cạnh TRANSITION vào/ra từ Node Unknown sang Node Defined.
    #   - (Tùy chọn) Merge các thuộc tính (screenshots, element counts, etc.) từ Unknown vào Defined.
    #   - Xóa Node Unknown.
    # Bạn sẽ cần một hàm mới trong graph_db.py, ví dụ:
    # success_neo4j, error_msg_neo4j = graph_db.merge_screen_nodes_in_neo4j(
    # source_screen_id=unknown_screen_id,
    # target_screen_id=target_defined_screen_id,
    # app_name=app_name
    # )
    
    # *** GIẢ LẬP LOGIC MERGE NEO4J - BẠN CẦN IMPLEMENT HÀM NÀY TRONG GRAPH_DB.PY ***
    # Ví dụ: chỉ cập nhật status của node unknown và thêm defined_as
    updated_node_props = {
        "status": "merged_to_defined", # Hoặc một status mới
        "merged_to_screen_id": target_defined_screen_id,
        "node_classification": pie_def.get("default_node_classification") # Nếu PIE def có gợi ý classification
    }
    success_neo4j_update, error_msg_neo4j_update = graph_db.update_screen_node_properties_by_id(
        screen_id_to_update=unknown_screen_id,
        app_name=app_name,
    ...
    # Hàm graph_db.update_screen_node_properties_by_id cần được tạo nếu chưa có.
    # Nó sẽ MATCH node theo unknown_screen_id và app_name, rồi SET các properties.

    if not success_neo4j_update:
        logger.error(f"API merge_unknown_to_defined: Không thể cập nhật/merge Node Neo4j '{unknown_screen_id}' -> '{target_defined_screen_id}': {error_msg_neo4j_update}")
        return jsonify({"success": False, "error": f"Lỗi cập nhật/merge Node Neo4j: {error_msg_neo4j_update}"}), 500
    
    # (Tùy chọn) Xóa Node Unknown sau khi đã chuyển hết thông tin/cạnh sang Node Defined.
    # Hoặc chỉ đánh dấu Node Unknown là đã merge.

    logger.info(f"API merge_unknown_to_defined: Node Unknown '{unknown_screen_id}' đã được merge/liên kết với Defined Screen ID '{target_defined_screen_id}'.")
    return jsonify({
        "success": True, 
        "message": f"Node Unknown '{unknown_screen_id}' đã được merge/liên kết với '{target_defined_screen_id}'."
    })
