...
def get_stage_details(stage_id: str) -> dict | None:
    """Lấy chi tiết một stage, bao gồm cả identifying_elements."""
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # <<< Lấy thêm identifying_elements >>>
        sql = """
            SELECT stage_id, strategy_id, description, stage_order, identifying_elements
            FROM public.strategy_stages WHERE stage_id = %s;
        """
        cur.execute(sql, (stage_id,))
        row = cur.fetchone()
        if row:
            details = dict(row)
            # Chuyển identifying_elements thành chuỗi JSON format đẹp cho textarea
            if details.get('identifying_elements') is not None:
                try: details['identifying_elements_str'] = json.dumps(details['identifying_elements'], indent=2, ensure_ascii=False)
                except: details['identifying_elements_str'] = str(details['identifying_elements'])
            else:
                 details['identifying_elements_str'] = '{}' # Default là JSON rỗng
    # ... (xử lý lỗi như cũ) ...
    except psycopg2.Error as db_err: print(f"LỖI (db - get_stage_details): {db_err}"); details=None
    except Exception as e: print(f"LỖI (db - get_stage_details): {e}"); details=None
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return details
...
def update_stage(stage_id: str, description: str | None, stage_order: int, identifying_elements_str: str | None = None) -> tuple[bool, str | None]:
    ...
    if not stage_id: return False, "Stage ID là bắt buộc."
    ...
    if identifying_elements_str and identifying_elements_str.strip() and identifying_elements_str != '{}':
        try:
            # Validate và đảm bảo là chuỗi JSON
            identifying_elements_json = json.dumps(json.loads(identifying_elements_str))
        except json.JSONDecodeError:
            return False, "Identifying Elements không phải là JSON hợp lệ."
    # Nếu chuỗi rỗng, trống, hoặc '{}', sẽ lưu NULL vào DB
         ...
         identifying_elements_json = None # Lưu NULL
    ...
    try:
        cur = conn.cursor()
        # <<< Thêm identifying_elements vào UPDATE >>>
        sql = """
        ...
        success = cur.rowcount > 0
    # ... (xử lý lỗi như cũ) ...
    except psycopg2.Error as db_err: error_msg = f"Lỗi CSDL: {db_err}"; success = False; if conn: conn.rollback()
