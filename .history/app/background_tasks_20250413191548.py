...
def run_ai_conversation_simulation(
    ...
    with temp_app.app_context():
        ...
        while turns_taken < max_turns * 2:
            ...
            print(f"DEBUG ({task_id_log}): Using Account ID for logging turn {turns_taken + 1} = {account_id_for_this_turn_log} (Speaker: {speaker_label})")
            ...
            if ai_status.startswith("success") and ai_reply:
                ...
                try:
                    history_id = db.log_interaction_received(
                        account_id=account_id_for_this_turn_log, app_name=SIM_APP_NAME, thread_id=SIM_THREAD_ID,
