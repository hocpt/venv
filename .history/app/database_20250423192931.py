...
def add_phone_action_logs(assignment_id: int, device_id: str, account_id: str,
                            logs: list[dict], # List các log entries thô từ client
                            structured_ui_state_json: str | None = None # <<< Tham số mới (chuỗi JSON đã xử lý)
                            ) -> bool:
    """
    Ghi nhiều bản ghi log hành động từ client vào CSDL.
    Gắn kèm trạng thái UI đã xử lý (nếu có) vào cột received_state_json
    cho TẤT CẢ các log entry trong batch này (cách làm đơn giản ban đầu).

    Args:
        assignment_id: ID của task assignment.
        device_id: ID thiết bị.
        account_id: ID tài khoản.
        logs: List các dictionary, mỗi dict là một log entry từ client.
              Mong đợi các key như 'timestamp', 'macro', 'params', 'status', 'error', 'stage'.
        structured_ui_state_json: (Tùy chọn) Chuỗi JSON chứa trạng thái UI đã được xử lý
                                   (mảng các element object) tại thời điểm gửi báo cáo.

    Returns:
        True nếu ghi log thành công (hoặc không có log để ghi), False nếu có lỗi CSDL.
    """
    logger = current_app.logger if current_app else print

    # Kiểm tra input cơ bản
    if not assignment_id:
        logger.warning(f"Input không hợp lệ cho add_phone_action_logs: Thiếu assignment_id")
        return False
    if not logs or not isinstance(logs, list):
        logger.info(f"Không có log entries nào để ghi cho assignment {assignment_id}.")
        return True # Không có log thì coi như thành công
    ...
    try:
        cur = conn.cursor()
        # Câu lệnh SQL INSERT bao gồm cột received_state_json
        # Lưu ý: Tên cột trong SQL ("timestamp", macro_code, params_json...) phải khớp với schema CSDL của bạn
        sql_template = """
            INSERT INTO public.phone_action_log
            (assignment_id, device_id, account_id, "timestamp", macro_code,
             params_json, execution_status, execution_error, current_stage,
             received_state_json) -- <<< Cột lưu state UI
            VALUES %s; -- Template cho execute_values
        """
        # Chuẩn bị danh sách các tuple dữ liệu để insert
        data_tuples = []
        for log_entry in logs:
            if not isinstance(log_entry, dict):
                logger.warning(f"Bỏ qua log entry không phải dict trong assignment {assignment_id}: {log_entry}")
                continue # Bỏ qua entry không hợp lệ

            # Xử lý timestamp (ưu tiên từ client, fallback về server time)
            ts_str = log_entry.get('timestamp')
            ts_obj = datetime.now(timezone.utc) # Default là thời gian server nhận được
            if isinstance(ts_str, str):
                try:
                    # Xử lý 'Z' timezone nếu client gửi dạng ISO 8601 UTC
                    ts_obj = datetime.fromisoformat(ts_str.replace('Z', '+00:00'))
                except ValueError:
                    logger.warning(f"Timestamp không hợp lệ từ client log: '{ts_str}'. Dùng thời gian server.")
                    pass # Giữ default nếu parse lỗi

            # Xử lý params (có thể là dict hoặc chuỗi JSON từ client) thành chuỗi JSON để lưu
            params_data = log_entry.get('params')
            params_json_str_to_save = None
            if params_data is not None:
                try:
                    # Luôn chuyển thành chuỗi JSON chuẩn trước khi lưu
                    params_json_str_to_save = json.dumps(params_data, ensure_ascii=False)
                except TypeError as json_err:
                     logger.warning(f"Không thể serialize params thành JSON cho log: {params_data}. Lỗi: {json_err}")
                     params_json_str_to_save = json.dumps({"raw_param_data": str(params_data)}) # Lưu dạng thô nếu lỗi

            # Tạo tuple cho dòng log này
            data_tuples.append((
                assignment_id,
                device_id,
                account_id,
                ts_obj, # Timestamp đã xử lý
                log_entry.get('macro'), # Mong đợi key 'macro' từ client
                params_json_str_to_save, # Chuỗi JSON hoặc None (sẽ ép kiểu jsonb trong DB nếu cột là jsonb)
                log_entry.get('status', 'unknown'), # Key 'status' từ client
                log_entry.get('error'), # Key 'error' từ client (có thể None)
                log_entry.get('stage'), # Key 'stage' từ client (có thể None)
                structured_ui_state_json # <<< Dùng state UI JSON đã xử lý (giống nhau cho cả batch)
            ))

        # Thực thi INSERT nhiều dòng nếu có dữ liệu
        if data_tuples:
            logger.debug(f"Chuẩn bị INSERT {len(data_tuples)} log entries cho assignment {assignment_id}.")
            # Dùng execute_values để tối ưu hiệu năng
            psycopg2.extras.execute_values(
                cur, sql_template, data_tuples,
                # Template chỉ định cách psycopg2 định dạng từng giá trị trong tuple
                # Nếu cột params_json và received_state_json là JSONB, không cần template đặc biệt
                # Nếu là TEXT, cũng không cần template đặc biệt vì đã chuyển thành string
                template=None,
                page_size=100 # page_size có thể điều chỉnh
            )
            conn.commit()
            success = True
            logger.info(f"Đã thêm thành công {len(data_tuples)} phone action logs cho assignment {assignment_id}.")
        else:
            logger.warning(f"Không có log entry hợp lệ nào được tạo ra để ghi cho assignment {assignment_id}.")
            success = True # Vẫn coi là thành công vì không có lỗi DB
...
def add_new_strategy(strategy_id: str, name: str, description: str | None,
                     initial_stage_id: str, strategy_type: str,
                     target_platform: str | None = None, # <<< Thêm target_platform
                     execution_config_str: str | None = None # <<< Thêm execution_config
                    ) -> tuple[bool, str | None]:
    """
    ...
    # <<< SỬA LẠI VALIDATION STRATEGY TYPE >>>
    valid_types = ['language', 'control', 'mainloop'] # Thêm 'mainloop' vào danh sách hợp lệ
    if strategy_type not in valid_types:
        return False, f"Strategy Type không hợp lệ (phải là một trong: {', '.join(valid_types)})."

    # Validate JSON execution_config nếu có
    execution_config_json = None
    if execution_config_str and execution_config_str.strip() and execution_config_str.strip() != '{}':
        try:
            parsed_config = json.loads(execution_config_str)
            if not isinstance(parsed_config, dict): raise ValueError("Execution Config phải là JSON object.")
            execution_config_json = json.dumps(parsed_config) # Lưu dạng chuẩn
        except (json.JSONDecodeError, ValueError) as json_err:
            return False, f"Execution Config JSON không hợp lệ: {json_err}"
    ...
    try:
        cur = conn.cursor()
        # <<< Thêm target_platform và execution_config vào INSERT >>>
        sql = """
            INSERT INTO public.strategies (
                strategy_id, name, description, initial_stage_id, strategy_type,
                target_platform, execution_config, updated_at
            ) VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb, NOW());
        """
        params = (
            strategy_id, name, description, initial_stage_id, strategy_type,
            target_platform if target_platform else None, # Lưu NULL nếu rỗng
            execution_config_json # Chuỗi JSON hoặc None (sẽ ép kiểu jsonb)
        )
    ...
    # ... (Phần except và finally giữ nguyên như trong file của bạn) ...
    except psycopg2.IntegrityError as int_err:
        error_msg = f"Lỗi ràng buộc CSDL (ID hoặc Name '{strategy_id}'/'{name}' đã tồn tại?): {int_err}"
