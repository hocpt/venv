...
def get_interaction_history(page: int = 1, per_page: int = 30, filters: dict = None) -> tuple[list[dict] | None, dict | None]:
    ...
    try:
        # Đếm tổng số dòng (chưa có filter)
        cur = conn.cursor()
        ...
        if total_items == 0:
            logger.debug("DEBUG (get_interaction_history): No interaction history entries found in total.")
            return [], {'page': page, 'per_page': per_page, 'total_items': 0, 'total_pages': 0} # Trả về list rỗng và pagination 0
        ...
        data_sql = """
            SELECT history_id, timestamp, account_id, strategy_id, stage_id, received_text,
                   sent_text, detected_user_intent, status, next_stage_id
        ...
        rows = cur.fetchall()
        # === THÊM LOG KẾT QUẢ FETCH ===
        logger.debug(f"DEBUG (get_interaction_history): Fetched {len(rows)} rows from DB for page {page}.")
        # ===========================
        ...
        # === THÊM LOG TRƯỚC KHI RETURN ===
        logger.debug(f"DEBUG (get_interaction_history): Returning {len(entries)} entries for page {page}.")
        logger.debug(f"DEBUG (get_interaction_history): Returning pagination data: {pagination_data}")
        # ==============================

    except psycopg2.Error as db_err:
        logger.error(f"LỖI (get_interaction_history): Truy vấn thất bại: {db_err}", exc_info=True)
        entries, pagination_data = None, None # Trả về None khi lỗi
    except Exception as e:
