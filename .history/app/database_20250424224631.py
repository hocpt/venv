...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    """
    Thêm một template_ref mới (nếu chưa tồn tại bằng ON CONFLICT)
    VÀ thêm variation đầu tiên vào template_variations trong cùng transaction.
    """
    logger = current_app.logger if current_app else print
    _template_ref = template_ref.strip()
    ...
    try:
        cur = conn.cursor()
        logger.info(f"Attempting to add template '{_template_ref}' and first variation.")

        # === BƯỚC 1: INSERT vào response_templates với ON CONFLICT DO NOTHING ===
        # Cách này sẽ thêm mới nếu chưa có, hoặc không làm gì nếu đã có, mà không báo lỗi UniqueViolation
        sql_template = """
        ...
        logger.info(f"Executed INSERT ... ON CONFLICT DO NOTHING for template_ref '{_template_ref}'.")
        # Ở bước này, template_ref 'welcome' chắc chắn tồn tại trong bảng (hoặc vừa được thêm, hoặc đã có từ trước)
        # trừ khi có lỗi DB khác xảy ra với lệnh INSERT này.

        # === BƯỚC 2: INSERT vào template_variations ===
        # Bây giờ việc INSERT này phải thành công về mặt FK, trừ khi có lỗi khác (vd: variation trùng lặp)
        sql_variation = """
        ...
        logger.info(f"Executed INSERT for variation of '{_template_ref}'.")

        # === COMMIT transaction ===
        # Nếu cả hai lệnh execute ở trên không gây ra exception, commit
        conn.commit()
        logger.info(f"Transaction committed for '{_template_ref}'.")
        returned_ref = _template_ref # Thành công

    except psycopg2.Error as db_err:
         # Bắt tất cả lỗi psycopg2 ở đây (bao gồm cả FK nếu vẫn xảy ra!)
         logger.error(f"LỖI (database.py - add_new_template): INSERT thất bại: {db_err}", exc_info=True) # Log cả traceback
         if conn: conn.rollback() # Rollback khi có lỗi
         returned_ref = None
    except Exception as e:
        logger.error(f"LỖI (database.py - add_new_template): Lỗi không xác định: {e}", exc_info=True)
        if conn: conn.rollback()
        returned_ref = None
    finally:
        if cur: cur.close()
