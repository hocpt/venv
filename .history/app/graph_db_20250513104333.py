...
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
                 ...
                 is_defined_by_pie: bool = False 
                 ) -> bool:
    logger = current_app.logger # Hoặc logging.getLogger(__name__)
    logger.info(f"[GraphDB merge_screen] ScreenID: {screen_id}, App: {app_name}, IsDefined: {is_defined_by_pie}")
    
    driver = get_driver() # Lấy driver
    if not driver:
    ...
    # Thuộc tính để SET khi node được TẠO MỚI
    # KHÔNG BAO GỒM screen_id ở đây
    props_for_create = {
        "app_name": app_name, # app_name vẫn cần thiết để đảm bảo tính duy nhất nếu screen_id không phải global unique
        ...
        "last_seen": current_time_utc_iso
        # created_at sẽ được SET bằng Cypher
    }
    props_for_create_clean = {k: v for k, v in props_for_create.items() if v is not None}
    
    # Thuộc tính để CẬP NHẬT nếu node ĐÃ TỒN TẠI
    # KHÔNG BAO GỒM screen_id ở đây
    props_for_match = { 
        "app_name": app_name, # Có thể cần cập nhật app_name nếu nó thay đổi cho cùng screen_id (ít khả năng)
        "activity_name": activity_name or 'UnknownActivity',
        "screenshot_path": screenshot_path, 
        "width": screen_width, 
        "height": screen_height,
    ...
    props_for_match_clean = {k: v for k, v in props_for_match.items() if v is not None}

    # ... (Phần chuẩn hóa standardized_elements_for_tx giữ nguyên như trước) ...
    standardized_elements_for_tx = []
    if extracted_elements:
        for el_data in extracted_elements:
            if isinstance(el_data, dict) and el_data.get('element_id'):
                coords = el_data.get('coordinates')
                coord_x_val = int(coords['x']) if isinstance(coords, dict) and coords.get('x') is not None else None
                coord_y_val = int(coords['y']) if isinstance(coords, dict) and coords.get('y') is not None else None
                
                bounds_val = el_data.get('bounds') 
                bounds_dict_for_neo4j = None
                if isinstance(bounds_val, dict):
                    bounds_dict_for_neo4j = {
                        k_b: int(v_b) if v_b is not None else None 
                        for k_b, v_b in bounds_val.items() 
                        if k_b in ['left', 'top', 'right', 'bottom']
                    }
                    bounds_dict_for_neo4j = {k:v for k,v in bounds_dict_for_neo4j.items() if v is not None}
                ...
                std_el = {
                    ...
                    'coordinate_x': coord_x_val,
                    'coordinate_y': coord_y_val,
                    'bounds': bounds_dict_for_neo4j,
                    'is_clickable_observed': el_data.get('is_clickable_observed', False),
                    'is_editable_observed': el_data.get('is_editable_observed', False),
                    'last_seen_timestamp': current_time_utc_iso 
                }
                standardized_elements_for_tx.append({k:v for k,v in std_el.items() if v is not None})
    # Kết thúc chuẩn hóa elements

    db_name_neo4j = current_app.config.get('NEO4J_DATABASE', 'neo4j')

    try:
        with driver.session(database=db_name_neo4j) as session:
            def merge_screen_and_elements_tx(tx: Transaction, screen_id_param, 
                                             props_create_param, props_match_param, 
                                             elements_param_list, is_defined_param):
                logger.debug(f"TX merge_screen: screen_id={screen_id_param}, is_defined={is_defined_param}")
                # logger.debug(f"TX props_create_param: {props_create_param}") # Chứa app_name
                # logger.debug(f"TX props_match_param: {props_match_param}") # Chứa app_name
                ...
                ON CREATE SET 
                    s = $propsForCreate, 
                    // s.screen_id = $screenIdVal, // KHÔNG CẦN NỮA, đã có trong MERGE key
                    s.created_at = datetime()
                ON MATCH SET 
                    s += $propsForMatch,
                    s.status = CASE 
                                   WHEN s.status IS NULL OR s.status = 'provisional_unknown' THEN 
                                       (CASE WHEN $isDefinedByPie THEN 'defined' ELSE 'provisional_unknown' END)
                                   ELSE s.status 
                               END,
                    s.updated_at = datetime()
                RETURN s.screen_id as merged_screen_id 
                """
                screen_run_params = {
                    "screenIdVal": screen_id_param, # $screenIdVal dùng để MERGE
                    "propsForCreate": props_create_param, # $propsForCreate không chứa screen_id
                    "propsForMatch": props_match_param,   # $propsForMatch không chứa screen_id
                    "isDefinedByPie": is_defined_param 
                }
                # logger.debug(f"Running Screen Query: {screen_query} \nParams: {json.dumps(screen_run_params, default=str)}")
                ...
                if not merged_id_record or not merged_id_record["merged_screen_id"]:
                    raise Exception(f"Thất bại khi MERGE Screen node với ID: {screen_id_param}")

                # Logic MERGE elements và HAS_ELEMENT giữ nguyên
                for el_data_item in elements_param_list:
                    el_id = el_data_item.get('element_id')
                    if not el_id: continue
                    
                    el_props_for_element = {k: v for k, v in el_data_item.items() if k not in ['element_id', 'screen_id_temp']}

                    element_query = """
                    MATCH (s:Screen {screen_id: $screenIdVal})
                    // screen_id trong Element node là để nhóm element theo màn hình, vẫn cần thiết
                    MERGE (e:Element {screen_id: $screenIdVal, element_id: $elementIdVal}) 
                    ON CREATE SET 
                        e = $props, 
                        // e.element_id = $elementIdVal, // Không cần, đã có trong MERGE key
                        // e.screen_id = $screenIdVal, // Không cần, đã có trong MERGE key
                    ...
                    """
                    tx.run(element_query, 
                           screenIdVal=screen_id_param, 
                           elementIdVal=el_id, 
                           props=el_props_for_element # $props cho element
                    ).consume()

            session.write_transaction(
                merge_screen_and_elements_tx,
                screen_id, 
                props_for_create_clean, # props_for_create_clean không chứa screen_id
                props_for_match_clean,  # props_for_match_clean không chứa screen_id
                standardized_elements_for_tx,
                is_defined_by_pie 
            )
            logger.info(f"GraphDB: Hoàn thành merge_screen cho {screen_id}")
            return True
    except Exception as e:
        logger.error(f"Lỗi Neo4j trong merge_screen cho {screen_id}: {e}", exc_info=True)
        return False
...
def get_elements_for_screen(screen_id: str) -> list[dict] | None:
    logger = current_app.logger # Hoặc log
    # ... (get driver, kiểm tra driver) ...
    elements_list = []
    try:
        db_name = current_app.config.get('NEO4J_DATABASE', 'neo4j')
        with driver.session(database=db_name) as session:
            ...
            """
            result = session.run(query, screen_id_param=screen_id)
            for record in result:
                props = record["element_properties"]
                if props:
                    processed_props = {}
                    for key, value in props.items():
                        # Xử lý datetime/neo4j.time sang ISO string
                        if isinstance(value, (datetime, neo4j.time.DateTime, neo4j.time.ZonedDateTime)):
                            try:
                                if hasattr(value, 'to_native') and callable(value.to_native):
                                    native_dt = value.to_native()
                                    if isinstance(native_dt, datetime):
                                        processed_props[key] = native_dt.isoformat()
                                    else: processed_props[key] = str(native_dt)
                                elif isinstance(value, datetime):
                                    processed_props[key] = value.isoformat()
                                else: processed_props[key] = str(value)
                            except Exception as dt_e:
                                logger.warning(f"Conversion error for {key}: {dt_e}")
                                processed_props[key] = str(value)
                        # Xử lý Neo4j Point (nếu bạn có lưu tọa độ dạng Point)
                        elif isinstance(value, neo4j.spatial.Point):
                            processed_props[key] = {'x': value.x, 'y': value.y, 'srid': value.srid}
                        else:
                            processed_props[key] = value

                    # Đảm bảo các trường tọa độ chính có mặt nếu chúng tồn tại dưới tên khác
                    if 'coordinate_x' not in processed_props and 'coordinates' in processed_props and isinstance(processed_props['coordinates'], dict):
                        processed_props['coordinate_x'] = processed_props['coordinates'].get('x')
                    if 'coordinate_y' not in processed_props and 'coordinates' in processed_props and isinstance(processed_props['coordinates'], dict):
                        processed_props['coordinate_y'] = processed_props['coordinates'].get('y')
