...
def api_get_managed_nodes():
    app_name = request.args.get('app_name_filter')
    status = request.args.get('filter_status', 'unknown')
    page = request.args.get('page', 1, type=int)
    per_page_val = 15 # Hoặc lấy từ config
    ...
    if nodes_page is None:
        return jsonify({"error": "Lỗi tải dữ liệu Node"}), 500

    # Tạo pagination data nếu cần trả về cho JS client-side pagination
    # Hoặc để server-side rendering lo việc này như hiện tại
    ...
    return jsonify({
        ...
        "page": page,
        "per_page": per_page_val,
        "total_pages": math.ceil(total_items / per_page_val) if total_items and total_items > 0 else 0
    })
