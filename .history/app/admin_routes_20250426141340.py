...
Chào bạn, tôi đã xem xét các vấn đề bạn nêu liên quan đến trang "Thêm/Sửa Cấu hình Mô phỏng AI". Chúng ta sẽ giải quyết từng điểm:

1. Vấn đề Thêm Config: Ô "Kích hoạt" được chọn nhưng lưu thành false (hiển thị "No")

Lỗi này xảy ra do cách code lấy giá trị từ checkbox "Kích hoạt" trong route add_simulation_config_view (file admin_routes.py) và/hoặc cách truyền giá trị đó vào hàm db.add_simulation_config.

Kiểm tra Template (admin_add_simulation_config.html):
Checkbox có name="is_enabled" và value="on" là đúng. Nó cũng checked mặc định.
Kiểm tra Route (admin_routes.py -> add_simulation_config_view):
Cách lấy giá trị checkbox đúng phải là:
Python

# Trong phần xử lý POST của route add_simulation_config_view
is_enabled = request.form.get('is_enabled') == 'on'
# Nếu checkbox được tick, request.form.get('is_enabled') sẽ là 'on', so sánh == 'on' cho kết quả True.
# Nếu checkbox không được tick, request.form.get('is_enabled') sẽ là None, so sánh == 'on' cho kết quả False.
Sau đó, giá trị boolean is_enabled này phải được truyền vào hàm DB:
Python

success, error_msg_db = db.add_simulation_config(
    # ... các tham số khác ...
    is_enabled=is_enabled # Truyền biến boolean
)
Kiểm tra Hàm DB (database.py -> add_simulation_config):
Hàm này cần nhận tham số is_enabled kiểu boolean và truyền đúng vào câu lệnh INSERT. Đoạn code tôi cung cấp ở phản hồi trước cho hàm này đã xử lý đúng.
Code sửa lỗi đề xuất (chỉ phần liên quan trong admin_routes.py):
