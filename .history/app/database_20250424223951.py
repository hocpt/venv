...
def add_new_template(template_ref: str, first_variation_text: str, description: str | None = None, category: str | None = None) -> str | None:
    ...
    Sử dụng transaction rõ ràng hơn.
    ...
    try:
        ...
        # === BƯỚC 1: THỬ INSERT vào response_templates ===
        ...
        try:
            ...
            # Không cần rollback ở đây vì lệnh INSERT trước đó tự động bị hủy
            pass # Bỏ qua lỗi này
            ...
            # Không ném lỗi vội, để khối finally xử lý rollback
        ...
        # === BƯỚC 2: INSERT vào template_variations (chỉ chạy nếu bước 1 không lỗi nghiêm trọng) ===
        variation_insert_failed = False
        if not template_insert_failed:
            try:
                sql_variation = """
                ...
                # Lỗi ở bước 2 (CÓ THỂ LÀ FK VIOLATION NẾU BƯỚC 1 LỖI NGẦM)
        ...
        if not template_insert_failed and not variation_insert_failed:
            # Chỉ commit nếu cả hai bước thành công (hoặc bước 1 báo đã tồn tại)
            conn.commit()
            ...
            logger.error(f"Rolling back transaction for template '{template_ref}' due to errors.")
            conn.rollback()
            returned_ref = None

    except Exception as e_outer: # Bắt lỗi tổng quát nếu có vấn đề với cursor hoặc connection
        logger.error(f"LỖI (database.py - add_new_template): Lỗi không xác định ngoài dự kiến: {e_outer}", exc_info=True)
        if conn: conn.rollback() # Đảm bảo rollback nếu lỗi ở đây
        returned_ref = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
