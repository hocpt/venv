...
def add_transition_control():
    # ... (lấy strategy_id, kiểm tra strategy type) ...

    # Lấy dữ liệu cho form
    strategy_stages = []
    all_stages = []
    all_macros = [] # <<< Khởi tạo list macro
    try:
        strategy_stages = db.get_stages_for_strategy(strategy_id) or []
        all_stages = db.get_all_stages() or []
        # <<< Gọi hàm lấy TẤT CẢ macro (không filter, không phân trang cho dropdown) >>>
        # Giả sử get_all_macro_definitions trả về list khi không phân trang
        temp_macros, _ = db.get_all_macro_definitions(page=1, per_page=1000) # Lấy nhiều để đủ cho dropdown
        all_macros = temp_macros or []
        # Hoặc nếu bạn có hàm riêng không phân trang:
        # all_macros = db.get_all_macro_definitions_no_paging() or []
        if all_macros is None: raise Exception("DB returned None for macros")
        print(f"DEBUG (add_transition_control): Fetched {len(all_macros)} macros for dropdown.") # Debug số lượng macro lấy được
    except Exception as e:
        flash(f"Lỗi tải dữ liệu cho form: {e}", "error")
        # Gán list rỗng để tránh lỗi template
        strategy_stages, all_stages, all_macros = [], [], []


    if request.method == 'POST':
        # ... (logic xử lý POST) ...
        # <<< Khi render lại form do lỗi POST, phải truyền lại all_macros >>>
        return render_template('admin_add_transition_control.html', ..., all_macros=all_macros, ...)

    # GET request
    # <<< Đảm bảo truyền all_macros vào template >>>
    return render_template('admin_add_transition_control.html', ..., all_macros=all_macros, ...)
