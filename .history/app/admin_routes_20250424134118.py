...
def add_transition_control():
    strategy_details = None
    strategy_stages = [] # Stages thuộc strategy hiện tại (dùng cho Current Stage)
    # all_stages = [] # <<< BỎ DÒNG NÀY (HOẶC KHÔNG DÙNG NỮA) >>>
    control_stages = [] # <<< BIẾN MỚI ĐỂ LƯU STAGES CÙNG LOẠI >>>
    all_macros = [] # Giữ nguyên hoặc lọc theo type control nếu muốn
    cancel_url = url_for('admin.view_strategies_control') # Fallback

    try:
        strategy_details = db.get_strategy_details(strategy_id)
        # ... (kiểm tra strategy_details như cũ) ...
        cancel_url = url_for('admin.view_strategy_stages_control', strategy_id=strategy_id)

        strategy_stages = db.get_stages_for_strategy(strategy_id) or []

        # === THAY ĐỔI CÁCH LẤY STAGES CHO DROPDOWN NEXT STAGE ===
        # Gọi hàm mới để chỉ lấy các stage loại 'control'
        control_stages = db.get_stages_by_type('control') or []
        if control_stages is None: # Xử lý lỗi nếu hàm mới trả về None
             flash("Lỗi khi tải danh sách Control Stages cho dropdown.", "error")
             control_stages = []
        # === KẾT THÚC THAY ĐỔI ===

        # Lấy macro definitions (giữ nguyên hoặc lọc)
        temp_macros, _ = db.get_all_macro_definitions(filters={'app_target': '__all__'}, page=1, per_page=1000) # Lấy nhiều macro
        all_macros = temp_macros or []
        # Optional: Lọc thêm all_macros để chỉ hiện macro liên quan đến control?
        # all_macros = [m for m in all_macros if m.get('app_target') != 'language_model' or m.get('app_target') in ['system', 'generic', None, '']]
    ...
    if request.method == 'POST':
        # ...
        # Khi render lại form nếu lỗi, truyền control_stages vào biến stages
        return render_template('admin_add_transition_control.html',
                               # ...
                               strategy_stages=strategy_stages, # Cho dropdown Current Stage
                               all_stages=control_stages,     # <<< SỬA Ở ĐÂY: Truyền control_stages vào biến all_stages của template
                               all_macros=all_macros,
                               # ...
                               )

    # --- GET request ---
    # Render template mới: admin_add_transition_control.html
    return render_template('admin_add_transition_control.html',
                           # ...
                           strategy_stages=strategy_stages, # Cho dropdown Current Stage
                           all_stages=control_stages,     # <<< SỬA Ở ĐÂY: Truyền control_stages vào biến all_stages của template
