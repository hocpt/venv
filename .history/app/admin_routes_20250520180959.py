...
import markdown # <<< THÊM IMPORT NÀY
...
def api_get_app_graph_data():
    logger = current_app.logger
    # app_name_from_request là app_name của toàn bộ đồ thị đang được yêu cầu
    app_name_from_request = request.args.get('app_name') 
    logger.info(f"API_MAPPING_DATA: Yêu cầu dữ liệu đồ thị cho app: '{app_name_from_request}'")

    if not app_name_from_request:
        logger.warning("API_MAPPING_DATA: Thiếu tham số 'app_name' trong query.")
        return jsonify({"error": "Thiếu tham số app_name"}), 400

    if not graph_db:
         logger.error("API_MAPPING_DATA: GraphDB module chưa sẵn sàng.")
         return jsonify({"error": "Lỗi cấu hình server (GraphDB)"}), 500

    nodes_list_for_cytoscape = []
    edges_list_for_cytoscape = []
    node_ids_found_in_query = set()

    try:
        driver = graph_db.get_driver()
        if not driver:
            logger.error("API_MAPPING_DATA: Neo4j driver không khả dụng.")
        ...
        with driver.session(database=db_name_neo4j) as session:
            node_query = """
                MATCH (n:Screen {app_name: $app_name_param_for_query}) // Sử dụng param mới
                WHERE n.screen_id IS NOT NULL
                RETURN n.screen_id AS id,
                       n.app_name AS node_app_name, // Lấy app_name của từng node
            ...
            for record in nodes_result:
                ...
                # QUAN TRỌNG: Sử dụng app_name của chính node đó (record["node_app_name"])
                # hoặc app_name_from_request nếu bạn chắc chắn tất cả node thuộc về app đó.
                # An toàn hơn là dùng record["node_app_name"] nếu có.
                # Nếu query của bạn đã có `MATCH (n:Screen {app_name: $app_name_param_for_query})`
                # thì record["node_app_name"] sẽ chính là app_name_from_request.
                app_name_for_screenshot = record.get("node_app_name", app_name_from_request)

                generated_screenshot_url = _generate_screenshot_url_for_admin(
                    logger,
                    app_name_for_screenshot, # Sử dụng app_name của node
                    screenshot_filename,
                    node_id # Để log nếu có lỗi
                )
                
                node_data = {
                    "id": node_id,
                    "app_name": app_name_for_screenshot, # Thêm app_name vào data của node nếu cần
                    "activity": record["activity"],
                    "status": record["status"],
                    "element_count": record.get("element_count") if record.get("element_count") is not None else 0,
                    "screenshot_url": generated_screenshot_url,
                    "original_width": record["width"],      
                    "original_height": record["height"],    
                    "label": node_id[:12] + '...' if node_id and len(node_id) > 12 else node_id
                }
            ...
            logger.info(f"API_MAPPING_DATA: Processed {len(nodes_list_for_cytoscape)} nodes for app '{app_name_from_request}'.")

            edge_query = """
                MATCH (a:Screen {app_name: $app_name_param_for_query})-[r:TRANSITION]->(b:Screen {app_name: $app_name_param_for_query})
            ...
            """
            logger.debug(f"API_MAPPING_DATA: Executing Edge Query for app '{app_name_from_request}'")
            edges_result = session.run(edge_query, app_name_param_for_query=app_name_from_request)
            ...
            logger.info(f"API_MAPPING_DATA: Processed {len(edges_list_for_cytoscape)} valid edges for app '{app_name_from_request}'.")

    except graph_db.ServiceUnavailable as su_err: 
        logger.error(f"API_MAPPING_DATA: Lỗi kết nối Neo4j cho app '{app_name_from_request}': {su_err}", exc_info=True)
        return jsonify({"error": "Lỗi dịch vụ cơ sở dữ liệu đồ thị."}), 503
    except Exception as e:
        logger.error(f"API_MAPPING_DATA: Lỗi không mong muốn cho app '{app_name_from_request}': {e}", exc_info=True)
        return jsonify({"error": f"Lỗi server nội bộ khi xử lý yêu cầu: {str(e)}"}), 500
...
def _convert_neo4j_datetime_to_iso(dt_obj):
    logger = current_app.logger # Sử dụng logger để debug
    if dt_obj is None:
        return None
    try:
        if hasattr(dt_obj, 'to_native'): 
        ...
        logger.warning(f"ADMIN_ROUTES: _convert_neo4j_datetime_to_iso nhận được kiểu không mong đợi: {type(dt_obj)}, giá trị: {str(dt_obj)[:100]}")
        return str(dt_obj) 
    except Exception as e:
        logger.error(f"ADMIN_ROUTES: Lỗi trong _convert_neo4j_datetime_to_iso: {e}", exc_info=True)
        return str(dt_obj) 

# Hàm helper này nên đã có ở đầu file admin_routes.py từ lần sửa trước
def _generate_screenshot_url_for_admin(logger, node_app_name, raw_screenshot_filename_from_neo4j, node_id_for_log="N/A"):
    if raw_screenshot_filename_from_neo4j and node_app_name:
        try:
            full_url = url_for(
                'serve_screenshot_for_app', 
                app_name=node_app_name,
                filename=raw_screenshot_filename_from_neo4j
            )
            # logger.debug(f"URL_HELPER: Generated URL for app '{node_app_name}', file '{raw_screenshot_filename_from_neo4j}': {full_url}")
            return full_url
        except Exception as e_url:
            logger.error(f"URL_HELPER: Lỗi tạo URL ảnh cho node {node_id_for_log}, app '{node_app_name}', file '{raw_screenshot_filename_from_neo4j}': {e_url}", exc_info=True)
            return None
    else:
        if not raw_screenshot_filename_from_neo4j:
            # logger.info(f"URL_HELPER: Node {node_id_for_log} (App: {node_app_name}) không có screenshot_path.")
            pass # Không cần log nhiều nếu đây là trường hợp phổ biến
        if not node_app_name and raw_screenshot_filename_from_neo4j: # Chỉ log nếu có filename nhưng thiếu app_name
             logger.warning(f"URL_HELPER: Node {node_id_for_log} có screenshot_path nhưng không có app_name, không thể tạo URL ảnh.")
        return None
...
def view_node_management():
    ...
    filter_status_req = request.args.get('filter_status', 'all') 
    ...
    status_for_graphdb = filter_status_req if filter_status_req != 'all' else None
    ...
    try:
        # Giả sử bạn có logic lấy distinct_app_names_for_filter từ PostgreSQL
        # Ví dụ:
        # conn_pg = db_postgres.get_db_connection() 
        # if conn_pg:
        #     try:
        #         with conn_pg.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur_pg:
        #             cur_pg.execute("SELECT DISTINCT app_name FROM screen_definitions WHERE app_name IS NOT NULL ORDER BY app_name;")
        #             distinct_app_names_for_filter = [row['app_name'] for row in cur_pg.fetchall()]
        #     finally:
        #         conn_pg.close()
        pass # Bỏ qua nếu bạn đã có logic này
    except Exception as e_pg_apps:
        logger.error(f"ADMIN_ROUTES: Lỗi lấy distinct app_names cho filter: {e_pg_apps}", exc_info=True)

    raw_nodes_page_data, total_items_from_db = graph_db.get_screen_nodes_for_management(
        app_name=app_name_query_param,
        filter_status=status_for_graphdb,
        page=page,
        per_page=per_page_nodes
    )

    if raw_nodes_page_data is None:
        flash("Lỗi khi tải dữ liệu Node từ cơ sở dữ liệu đồ thị (Neo4j).", "error")
        logger.error("ADMIN_ROUTES: view_node_management - graph_db.get_screen_nodes_for_management trả về None.")
    else:
        for node_data_from_graphdb_item in raw_nodes_page_data:
            node_to_render = dict(node_data_from_graphdb_item) 
            
            internal_neo4j_id_value = node_to_render.get('neo4j_db_id') 
            
            logger.info(f"ADMIN_ROUTES_NODE_TO_RENDER: screen_id='{node_to_render.get('screen_id')}', status='{node_to_render.get('status')}', neo4j_db_id_value='{internal_neo4j_id_value}' (Type: {type(internal_neo4j_id_value)})")
            
            if not internal_neo4j_id_value: 
                logger.error(f"ADMIN_ROUTES LỖI NGHIÊM TRỌNG: Node screen_id='{node_to_render.get('screen_id')}' (status: '{node_to_render.get('status')}') có neo4j_db_id_value rỗng/None sau khi lấy từ graph_db!")
            
            # SỬA LỖI KEY Ở ĐÂY: Bỏ các thẻ 
            node_to_render['neo4j_id_for_html'] = internal_neo4j_id_value 
            
            if 'created_at' in node_to_render:
                node_to_render['created_at_iso'] = _convert_neo4j_datetime_to_iso(node_to_render['created_at'])
            if 'last_seen' in node_to_render:
                node_to_render['last_seen_iso'] = _convert_neo4j_datetime_to_iso(node_to_render['last_seen'])
            
            node_to_render['screenshot_full_url'] = _generate_screenshot_url_for_admin(
                logger,
                node_to_render.get('app_name'),
                node_to_render.get('screenshot_path'), 
                node_to_render.get('screen_id')
            )
            
            node_status = node_to_render.get('status', '').lower()
            defined_statuses = ['defined', 'defined_from_unknown', 'merged_to_defined'] 
            ...
            if node_status in defined_statuses and not node_to_render.get('logical_pie_name'):
                try:
                    ...
                    if pie_def:
                        ...
                        logger.warning(f"ADMIN_ROUTES: Không tìm thấy PIE def trong PG cho defined node '{node_to_render.get('screen_id')}' (app: {node_to_render.get('app_name')}) dù status là '{node_status}'.")
                except Exception as e_pie: # Bắt lỗi cụ thể hơn nếu có thể
                    logger.warning(f"ADMIN_ROUTES: Lỗi khi lấy PIE name cho defined node {node_to_render.get('screen_id')}: {e_pie}")

            nodes_for_template.append(node_to_render)
        ...
        if total_items_from_db is not None and total_items_from_db > 0:
            ...
            if page > total_pages_calc and total_pages_calc > 0: 
                page = total_pages_calc 
            
            extra_params_for_pagination = {}
            if selected_app_filter: extra_params_for_pagination['app_name_filter'] = selected_app_filter
            if filter_status_req: extra_params_for_pagination['filter_status'] = filter_status_req
            ...
            pagination_info_for_template = {
                ...
                'has_prev': page > 1 and total_pages_calc > 0,
    ...
    if not nodes_for_template and (selected_app_filter or filter_status_req != 'all'):
        flash(f"Không tìm thấy node nào khớp với bộ lọc.", "info")
    
    logger.info(f"ADMIN_ROUTES: Rendering admin_node_management.html with {len(nodes_for_template)} nodes. Filter status req: '{filter_status_req}'")
    return render_template('admin_node_management.html', 
                           title=title,
                           nodes=nodes_for_template, 
                           distinct_app_names=distinct_app_names_for_filter,
                           selected_app_filter=selected_app_filter,
                           current_filter_status=filter_status_req,
                           pagination=pagination_info_for_template,
                          )



@admin_bp.route('/api/mapping/management/nodes/<path:screen_id>/delete', methods=['POST'], endpoint='api_delete_managed_node')
# @admin_required
def api_delete_managed_node(screen_id):
    logger = current_app.logger
    if not request.is_json:
        logger.warning("API delete_managed_node: Yêu cầu không phải JSON.")
        return jsonify({"success": False, "message": "Yêu cầu phải là JSON"}), 400

    data = request.get_json()
    app_name_from_request = data.get('app_name') # app_name này chỉ dùng để tìm node trong Neo4j

    if not app_name_from_request:
        logger.warning(f"API delete_managed_node: Thiếu app_name cho screen_id {screen_id}")
        return jsonify({"success": False, "message": "Thiếu tham số app_name."}), 400

    logger.info(f"API: Yêu cầu xóa Node '{screen_id}' của app '{app_name_from_request}'")

    # Bước 1: Lấy thông tin node từ Neo4j TRƯỚC KHI xóa để có app_name và screenshot_path (chỉ tên file)
    # Hàm get_screen_properties cần trả về dict chứa 'app_name' và 'screenshot_path'
    node_details_before_delete = graph_db.get_screen_properties(screen_id, app_name_from_request) 
    
    raw_screenshot_file_to_delete = None
    app_name_of_node_for_file_path = None # app_name thực sự của node để xây dựng đường dẫn file

    if node_details_before_delete:
        raw_screenshot_file_to_delete = node_details_before_delete.get('screenshot_path') # CHỈ TÊN FILE
        app_name_of_node_for_file_path = node_details_before_delete.get('app_name') # Lấy app_name từ Neo4j
        logger.debug(f"Thông tin node trước khi xóa: app='{app_name_of_node_for_file_path}', screenshot_file='{raw_screenshot_file_to_delete}'")
    else:
        logger.warning(f"Không tìm thấy thông tin node '{screen_id}' app '{app_name_from_request}' trong Neo4j để lấy thông tin ảnh.")
        # Nếu không tìm thấy node, có thể vẫn tiếp tục thử xóa nếu logic delete_screen_node_logic cho phép
        # Hoặc trả lỗi ở đây. Tạm thời vẫn để logic xóa chạy.

    # Bước 2: Xóa Node khỏi Neo4j (dùng app_name từ request để đảm bảo đúng node được nhắm tới)
    success_neo4j, error_msg_neo4j = graph_db.delete_screen_node_logic(screen_id, app_name_from_request)

    if success_neo4j:
        logger.info(f"Đã xóa Node '{screen_id}' (app: {app_name_from_request}) khỏi Neo4j thành công.")
        # Bước 3: Xóa file screenshot vật lý
        if raw_screenshot_file_to_delete and app_name_of_node_for_file_path:
            # app_name_of_node_for_file_path là tên thư mục con
            # raw_screenshot_file_to_delete là tên file bên trong thư mục đó
            ...
            if not storage_base_path:
                ...
                # Node Neo4j đã xóa, nhưng file ảnh có thể còn.
                # Trả về thành công vì phần chính (xóa node) đã xong.
                return jsonify({"success": True, "message": f"Đã xóa Node {screen_id}. Lỗi cấu hình đường dẫn lưu ảnh, không thể xóa file."})

            # Đường dẫn đầy đủ đến file ảnh cần xóa
            full_path_to_delete = os.path.join(storage_base_path, app_name_of_node_for_file_path, raw_screenshot_file_to_delete)
            logger.debug(f"Chuẩn bị xóa file ảnh tại: {full_path_to_delete}")
            
            try:
                if os.path.isfile(full_path_to_delete): # Kiểm tra xem có phải là file không
                    os.remove(full_path_to_delete)
                    logger.info(f"Đã xóa file ảnh vật lý: {full_path_to_delete}")
                else:
                    logger.warning(f"File ảnh không tồn tại để xóa (hoặc không phải file): {full_path_to_delete}")
            except Exception as e_file_delete:
                logger.error(f"Lỗi khi xóa file ảnh '{full_path_to_delete}': {e_file_delete}", exc_info=True)
                # Vẫn trả về thành công vì node Neo4j đã được xóa
                return jsonify({"success": True, "message": f"Đã xóa Node {screen_id}. Gặp lỗi khi xóa file ảnh: {e_file_delete}"})
        
        elif not raw_screenshot_file_to_delete:
            logger.info(f"Node '{screen_id}' không có thông tin screenshot_path, không có file ảnh để xóa.")
        elif not app_name_of_node_for_file_path:
             logger.warning(f"Node '{screen_id}' không có app_name từ Neo4j, không thể xác định đường dẫn file ảnh để xóa.")


        return jsonify({"success": True, "message": f"Đã xóa Node {screen_id} và các dữ liệu liên quan thành công."})
    else:
        logger.error(f"API delete_managed_node: Lỗi từ graph_db.delete_screen_node_logic cho {app_name_from_request}/{screen_id} - {error_msg_neo4j}")
        status_code = 404 if "không tìm thấy" in (error_msg_neo4j or "").lower() else 500
        return jsonify({"success": False, "message": error_msg_neo4j or "Xóa Node thất bại."}), status_code
...
def api_get_managed_nodes():
    ...
    page = request.args.get('page', 1, type=int)
    if page < 1: page = 1
    per_page_val = current_app.config.get('PER_PAGE_NODE_ISSUES', 15)
    
    app_name_for_db_query = app_name_filter_from_req if app_name_filter_from_req and app_name_filter_from_req.strip() else None

    logger.debug(f"API_GET_MANAGED_NODES: app_filter='{app_name_for_db_query}', status_filter='{filter_status_from_req}', page={page}")

    try:
        raw_nodes_page_data, total_items = graph_db.get_screen_nodes_for_management(
        ...
        if raw_nodes_page_data is None:
            logger.error("API_GET_MANAGED_NODES: graph_db.get_screen_nodes_for_management trả về None.")
            return jsonify({"nodes": [], "pagination": {}, "error": "Lỗi tải dữ liệu Node từ cơ sở dữ liệu đồ thị."}), 500

        processed_nodes_list = []
        if raw_nodes_page_data:
            for node_data_from_graphdb in raw_nodes_page_data:
                processed_node = dict(node_data_from_graphdb) 

                internal_id = processed_node.get('node_neo4j_internal_id')
                if not internal_id:
                    logger.warning(f"API_GET_MANAGED_NODES: Node screen_id='{processed_node.get('screen_id')}' thiếu 'node_neo4j_internal_id'.")
                    processed_node['id_cho_data_attribute_html'] = processed_node.get('screen_id', f"missing_neo4j_id_{processed_node.get('screen_id')}")
                else:
                    processed_node['id_cho_data_attribute_html'] = internal_id

                # ---- QUAN TRỌNG: Thay thế các trường datetime gốc bằng chuỗi ISO ----
                if 'created_at' in processed_node:
                    processed_node['created_at'] = _convert_neo4j_datetime_to_iso(processed_node['created_at'])
                if 'last_seen' in processed_node:
                    processed_node['last_seen'] = _convert_neo4j_datetime_to_iso(processed_node['last_seen'])
                # Thêm các trường datetime khác nếu có
                # Ví dụ: nếu có 'updated_at', 'first_seen', etc.
                # if 'updated_at' in processed_node:
                #     processed_node['updated_at'] = _convert_neo4j_datetime_to_iso(processed_node['updated_at'])
                # --------------------------------------------------------------------
                
                processed_node['screenshot_full_url'] = _generate_screenshot_url_for_admin(
                    logger,
                    processed_node.get('app_name'),
                    processed_node.get('screenshot_path'),
                    processed_node.get('screen_id')
                )
                
                # (Tùy chọn) Lấy PIE name nếu cần và chưa có
                # if processed_node.get('status') == 'defined' and not processed_node.get('logical_pie_name'):
                #     pie_def = db.get_screen_definition_by_defined_id(processed_node.get('screen_id'), processed_node.get('app_name'))
                #     if pie_def:
                #         processed_node['logical_pie_name'] = pie_def.get('logical_screen_name')
        ...
        pagination_details = {
            ...
            'has_prev': page > 1 and total_pages_val > 0,
            'has_next': page < total_pages_val,
            'prev_num': page - 1 if page > 1 else None,
            'next_num': page + 1 if page < total_pages_val else None
        }
        
        logger.debug(f"API_GET_MANAGED_NODES: Trả về {len(processed_nodes_list)} node(s) và pagination.")
        return jsonify({
            "nodes": processed_nodes_list, # Bây giờ các trường datetime đã là chuỗi ISO
            "pagination": pagination_details
        })

    except Exception as e:
        logger.error(f"API_GET_MANAGED_NODES: Lỗi không mong muốn: {e}", exc_info=True)
        return jsonify({"nodes": [], "pagination": {}, "error": f"Lỗi server nội bộ: {str(e)}"}), 500
...
def api_get_pie_definition_conditions():
    ...
    logger.debug(f"API_ROUTES: Yêu cầu GET /api/pie_definition_conditions với app_name='{app_name}', defined_screen_id='{defined_screen_id}'")

    if not defined_screen_id or not app_name:
        logger.warning("API_ROUTES: Thiếu tham số 'defined_screen_id' hoặc 'app_name' cho pie_definition_conditions.")
    ...
    try:
        ...
        if conditions is None: 
            # Điều này có nghĩa là PIE definition không được tìm thấy trong DB
            logger.warning(f"API_ROUTES: Không tìm thấy PIE Definition (hoặc lỗi DB) cho app '{app_name}' và screen_id '{defined_screen_id}'.")
            return jsonify({
                "success": False,
                "message": f"Không tìm thấy Định nghĩa PIE cho ứng dụng '{app_name}' và ID màn hình '{defined_screen_id}'."
            }), 404 # NOT FOUND
        
        # Nếu conditions là list rỗng [], nghĩa là PIE definition tồn tại nhưng không có điều kiện nào
        logger.debug(f"API_ROUTES: Trả về {len(conditions)} conditions cho PIE {app_name}/{defined_screen_id}.")
        return jsonify({"success": True, "conditions": conditions}), 200

    except Exception as e:
        logger.error(f"API_ROUTES: Lỗi trong api_get_pie_definition_conditions: {e}", exc_info=True)
        return jsonify({"success": False, "message": "Đã xảy ra lỗi máy chủ nội bộ khi lấy điều kiện PIE."}), 500
...
def api_define_new_pie_and_update_node():
    ...
    if not isinstance(selected_conditions, list) or not selected_conditions: # Phải là list và không rỗng
        ...
        current_app.logger.warning(f"API define_new_pie_with_conditions: {msg} Data: {data}")
        return jsonify({"success": False, "message": msg}), 400
    if db.check_defined_screen_id_exists(app_name, new_defined_screen_id): # Giả sử hàm này trả về True nếu tồn tại
        error_msg = f"Defined Screen ID '{new_defined_screen_id}' đã tồn tại cho ứng dụng '{app_name}'. Vui lòng chọn ID khác."
        current_app.logger.warning(f"API define_new_pie_with_conditions: {error_msg}")
        return jsonify({"success": False, "message": error_msg}), 409 # 409 Conflict
...
@admin_bp.route('/api/mapping/transition/update/<path:neo4j_edge_id>', methods=['POST'])
# @admin_required # Nếu bạn dùng xác thực
def api_update_transition_properties(neo4j_edge_id):
    logger = current_app.logger
    logger.info(f"API: Yêu cầu cập nhật cho Transition Neo4j ID: {neo4j_edge_id}")

    if not request.is_json:
        logger.warning("API Update Transition: Yêu cầu không phải JSON.")
        return jsonify({"success": False, "error": "Yêu cầu phải là JSON"}), 400

    updated_properties = request.get_json()
    if not updated_properties:
        logger.warning("API Update Transition: Không có dữ liệu thuộc tính được gửi.")
        return jsonify({"success": False, "error": "Không có dữ liệu thuộc tính để cập nhật."}), 400

    logger.debug(f"API Update Transition: Dữ liệu nhận được: {updated_properties}")

    # Validate dữ liệu (ví dụ cơ bản, bạn có thể mở rộng)
    allowed_action_types = ["click", "input", "swipe_up", "swipe_down", "swipe_left", "swipe_right", "nav_go_back", "start_app", "run_macro", "other", ""] # Thêm "" cho trường hợp không chọn
    allowed_statuses = ["provisional", "confirmed", "failed", "needs_review", "disabled", ""]

    if 'action_type' in updated_properties and updated_properties['action_type'] not in allowed_action_types:
        return jsonify({"success": False, "error": f"Loại hành động '{updated_properties['action_type']}' không hợp lệ."}), 400
    
    if 'status' in updated_properties and updated_properties['status'] not in allowed_statuses:
        return jsonify({"success": False, "error": f"Trạng thái '{updated_properties['status']}' không hợp lệ."}), 400

    if 'params_json_str' in updated_properties and updated_properties['params_json_str']:
        try:
            json.loads(updated_properties['params_json_str'])
        except json.JSONDecodeError:
            return jsonify({"success": False, "error": "Params JSON không hợp lệ."}), 400
    
    # Chuyển đổi attempt_count và success_count sang integer nếu có
    for count_field in ['attempt_count', 'success_count']:
        if count_field in updated_properties and updated_properties[count_field] is not None:
            try:
                updated_properties[count_field] = int(updated_properties[count_field])
            except ValueError:
                 return jsonify({"success": False, "error": f"Giá trị của '{count_field}' phải là số nguyên."}), 400


    if not graph_db:
        logger.error("API Update Transition: GraphDB module chưa sẵn sàng.")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (GraphDB)"}), 500

    try:
        # Gọi hàm trong graph_db.py để cập nhật
        # Hàm này cần được tạo và nhận transaction (tx) làm tham số đầu tiên
        
        # Lấy driver và session
        driver = graph_db.get_driver()
        if not driver:
            logger.error("API Update Transition: Không thể lấy Neo4j driver.")
            return jsonify({"success": False, "error": "Lỗi kết nối cơ sở dữ liệu đồ thị."}), 500
        
        db_name_neo4j = current_app.config.get('NEO4J_DATABASE', 'neo4j')

        with driver.session(database=db_name_neo4j) as session:
            # Sử dụng execute_write để đảm bảo nó chạy trong một transaction
            # update_transition_by_neo4j_id_tx là hàm sẽ được định nghĩa trong graph_db.py
            # Nó sẽ nhận (tx, neo4j_edge_id, properties_to_set)
            # và trả về True/False hoặc một dict kết quả.
            # Ở đây, chúng ta giả định nó trả về một dict có key 'success' và 'message'/'error'
            
            # Chuẩn bị properties để SET, loại bỏ các key không phải là thuộc tính của cạnh
            # hoặc những key mà bạn không muốn cập nhật trực tiếp bằng SET r += $props
            props_to_set_in_neo4j = {}
            for key, value in updated_properties.items():
                # Chỉ lấy các key mà bạn muốn lưu vào Neo4j và đã được validate
                # Ví dụ: actionType, element_id, status, params_json_str (đã là string), macro_code, etc.
                # Đảm bảo tên key khớp với thuộc tính trên cạnh Neo4j
                if key in ['action_type', 'element_id', 'identifier_type', 'element_text', 
                           'macro_code', 'params_json_str', 'status', 
                           'attempt_count', 'success_count']: # Thêm các trường số
                    # Đổi tên key nếu cần để khớp với Neo4j
                    if key == 'action_type': # Ví dụ: nếu trong Neo4j là actionType
                        props_to_set_in_neo4j['actionType'] = value 
                    elif key == 'params_json_str': # Neo4j lưu là params_json_str
                        props_to_set_in_neo4j['params_json_str'] = value
                    # Giữ nguyên tên cho các trường khác nếu khớp
                    else:
                        props_to_set_in_neo4j[key] = value
            
            # Thêm trường updated_at
            props_to_set_in_neo4j['updated_at'] = datetime.now(timezone.utc).isoformat()


            # Gọi hàm transaction
            # result_tx là dict trả về từ hàm transaction của bạn
            result_tx = session.write_transaction(
                graph_db.update_transition_by_neo4j_id_tx, # Hàm này cần được tạo trong graph_db.py
                neo4j_edge_id,
                props_to_set_in_neo4j # Chỉ truyền các properties đã được làm sạch và chuẩn bị
            )

        if result_tx and result_tx.get("success"):
            logger.info(f"API Update Transition: Cập nhật thành công cho ID {neo4j_edge_id}.")
            return jsonify({"success": True, "message": result_tx.get("message", "Cập nhật transition thành công.")})
        else:
            error_detail = result_tx.get("error") if result_tx else "Lỗi không xác định khi cập nhật trong CSDL."
            logger.error(f"API Update Transition: Cập nhật thất bại cho ID {neo4j_edge_id}. Lỗi: {error_detail}")
            return jsonify({"success": False, "error": error_detail}), 500 # Hoặc 404 nếu không tìm thấy

    except Exception as e:
        logger.error(f"API Update Transition: Lỗi không mong muốn cho ID {neo4j_edge_id}: {e}", exc_info=True)
...
def get_docs_sidebar_structure(docs_root_path, current_dir_path=''):
    ...
    Returns:
        ...
        Ví dụ: [{'name': 'Tên Hiển Thị', 'path': 'relative/path/to/file.md', 'type': 'file', 'children': None},
                 {'name': 'Tên Thư Mục', 'path': 'relative/path/to/dir', 'type': 'dir', 'children': [...]}]
    """
    items = []
    # Đường dẫn tuyệt đối đến thư mục hiện tại đang duyệt
    absolute_current_path = os.path.join(docs_root_path, current_dir_path)

    if not os.path.isdir(absolute_current_path):
        return []

    for entry_name in sorted(os.listdir(absolute_current_path)):
        if entry_name.startswith('.') or entry_name.startswith('_'):
            continue # Bỏ qua file/thư mục ẩn hoặc có tiền tố gạch dưới

        entry_path_absolute = os.path.join(absolute_current_path, entry_name)
        # Đường dẫn tương đối để dùng trong URL và hiển thị
    ...
    def sort_key(item):
        ...
        base_name_for_sort = os.path.basename(item['path']) # Dùng path để lấy số thứ tự gốc

        if item_name_lower == 'readme': return "000"
        if len(base_name_for_sort) > 2 and base_name_for_sort[2] == '_' and base_name_for_sort[:2].isdigit():
            return base_name_for_sort[:2] 
        return item['name'] # Sắp xếp theo tên hiển thị đã xử lý
...
# @admin_required # Nếu bạn có decorator này, hãy đảm bảo nó được import và hoạt động đúng
def project_documentation_viewer(filepath):
    logger = current_app.logger
    # Đường dẫn đến thư mục docs/ của bạn
    # Giả sử thư mục docs/ nằm cùng cấp với thư mục app/ (nơi chứa admin_routes.py)
    # current_app.root_path thường là thư mục gốc của ứng dụng (ví dụ: hpt11/)
    docs_base_dir = os.path.join(current_app.root_path, 'docs')
    
    # Chuẩn hóa filepath để tránh lỗi path traversal
    # và đảm bảo nó nằm trong docs_base_dir
    # Ví dụ: filepath có thể là "01_ARCHITECTURE/SYSTEM_ARCHITECTURE.md"
    # os.path.join sẽ xử lý đúng dấu / hoặc \ tùy hệ điều hành
    # os.path.normpath sẽ chuẩn hóa đường dẫn (ví dụ: loại bỏ ../)
    
    # Tạo đường dẫn tuyệt đối an toàn
    # Chúng ta không muốn người dùng có thể truyền vào ../../some_other_file
    safe_filepath = os.path.normpath(filepath).lstrip('.').lstrip('/') # Loại bỏ các ký tự nguy hiểm ở đầu
    requested_path_absolute = os.path.join(docs_base_dir, safe_filepath)

    # Kiểm tra kỹ hơn để đảm bảo requested_path_absolute thực sự nằm trong docs_base_dir
    # Chuyển cả hai về realpath để xử lý symlinks và chuẩn hóa
    real_docs_base_dir = os.path.realpath(docs_base_dir)
    real_requested_path = os.path.realpath(requested_path_absolute)

    if not real_requested_path.startswith(real_docs_base_dir):
        logger.warning(f"Documentation: Path traversal attempt or invalid path. Base: '{real_docs_base_dir}', Requested Filepath: '{filepath}', Resolved: '{real_requested_path}'")
        os.abort(404)

    content_html = None
    error_message = None
    page_title = "Tài liệu Dự án" # Tiêu đề mặc định

    # Tạo cấu trúc sidebar (có thể cache lại nếu cần cho hiệu suất)
    sidebar_structure = get_docs_sidebar_structure(docs_base_dir) # Gọi hàm mới
    ...
    if os.path.isfile(real_requested_path) and real_requested_path.endswith('.md'):
        try:
            ...
            # Cấu hình các extension cho Markdown
            # fenced_code: Cho các khối mã ```sql ... ```
            # tables: Cho bảng Markdown
            # toc: Tự động tạo mục lục (Table of Contents) từ các heading (ví dụ: [TOC])
            # codehilite: (Cần Pygments) Để tô màu cú pháp. 
            #     graceful_failure=True để không gây lỗi nếu Pygments không được cài hoặc ngôn ngữ không được hỗ trợ.
            # nl2br: Chuyển đổi newline thành <br> (hữu ích cho các đoạn văn bản thuần)
            md_extensions = [
                'fenced_code', 
                'tables', 
                'toc', 
                'codehilite', # (css_class='codehilite') # Thêm class CSS nếu muốn
                'nl2br',
                'extra' # Kích hoạt nhiều extension hữu ích khác như abbr, attr_list, def_list, footnotes, smarty
            ]
            # Cấu hình cho codehilite (nếu bạn muốn Pygments thêm class vào thẻ pre)
            # extension_configs = {
            #     'codehilite': {'css_class': 'codehilite-styles'} 
            # }
            content_html = markdown.markdown(md_content, extensions=md_extensions) #, extension_configs=extension_configs)
            
            # Lấy tên file làm tiêu đề trang
            page_title_base = os.path.splitext(os.path.basename(real_requested_path))[0]
            if len(page_title_base) > 2 and page_title_base[2] == '_' and page_title_base[:2].isdigit():
                page_title_base = page_title_base[3:]
            page_title = page_title_base.replace('_', ' ').title()

        except FileNotFoundError: # Mặc dù đã kiểm tra isfile, nhưng để an toàn
            error_message = f"Tài liệu '{filepath}' không tìm thấy."
            logger.warning(f"Documentation: File not found at {real_requested_path} (đã kiểm tra isfile).")
            os.abort(404)
        except Exception as e:
            error_message = f"Lỗi khi đọc hoặc chuyển đổi tài liệu '{filepath}': {e}"
            logger.error(f"Documentation: Error processing {real_requested_path}: {e}", exc_info=True)
            content_html = f"<p class='text-danger'>Lỗi xử lý file Markdown: {escape(str(e))}</p>"
    
    elif os.path.isdir(real_requested_path):
        # Nếu là thư mục, thử tìm README.md trong đó
        readme_in_dir_path = os.path.join(real_requested_path, 'README.md')
        if os.path.isfile(readme_in_dir_path):
            # Chuyển hướng đến file README.md của thư mục đó
            new_filepath = os.path.join(os.path.relpath(real_requested_path, real_docs_base_dir), 'README.md').replace(os.sep, '/')
            logger.info(f"Documentation: Directory access, redirecting to README: {new_filepath}")
            return redirect(url_for('.project_documentation_viewer', filepath=new_filepath))
        else:
            error_message = f"Thư mục '{filepath}' không chứa file README.md mặc định."
            content_html = f"<p class='text-info'>{error_message} Hãy chọn một file từ sidebar.</p>"
            page_title = os.path.basename(real_requested_path).replace('_', ' ').title()
            if len(page_title) > 2 and page_title[2] == ' ' and page_title[:2].isdigit(): # Đã có title() rồi
                page_title = page_title[3:]


    else: # Không phải file .md hợp lệ hoặc không phải thư mục
        error_message = f"Đường dẫn tài liệu '{filepath}' không hợp lệ hoặc không tồn tại."
        logger.warning(f"Documentation: Invalid path or not a .md file: {real_requested_path}")
        os.abort(404)
