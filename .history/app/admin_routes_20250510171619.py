...
from .graph_db import get_screen_with_elements, get_screen_node_by_id # Thêm get_screen_node_by_id nếu bạn dùng nó ở chỗ khác
...
#@login_required # Sử dụng decorator login của bạn
def admin_screen_elements(screen_id): # Tên hàm này có thể là `admin_screen_elements_view` hoặc tương tự trong code của bạn
    current_app.logger.info(f"Accessing admin elements page for screen_id: {screen_id}")

    # 1. Lấy dữ liệu Screen từ Neo4j
    screen_node_neo4j = get_screen_with_elements(screen_id) # Nên trả về Node object hoặc dict
    if not screen_node_neo4j:
        ...
        return redirect(url_for(f'{admin_bp.name}.admin_mapping_viewer', app_name='_default_app_')) # Cần app_name cho mapping viewer
    ...
    activity_name = screen_data_neo4j.get('activityName')
    ...
    neo4j_screenshot_path = screen_data_neo4j.get('screenshot_path') # Đây là tên file hoặc đường dẫn tương đối

    # 2. Xây dựng screenshot_url
    screenshot_url = None
    if neo4j_screenshot_path:
        # `neo4j_screenshot_path` có thể là "appName/filename.jpg" hoặc chỉ "filename.jpg"
        # Cần đảm bảo nó khớp với cách bạn xây dựng SCREENSHOT_URL_BASE và cách file được lưu
        # trong SCREENSHOT_STORAGE_PATH.
        # Ví dụ, nếu SCREENSHOT_URL_BASE là '/static/screenshots' và file được lưu trong 'app/static/screenshots/appName/filename.jpg'
        # và neo4j_screenshot_path lưu là 'appName/filename.jpg'
        screenshot_url = f"{current_app.config.get('SCREENSHOT_URL_BASE', '/static/screenshots')}/{neo4j_screenshot_path}"
        # Hoặc nếu neo4j_screenshot_path chỉ là tên file và bạn muốn thêm app_name:
        # screenshot_url = f"{current_app.config.get('SCREENSHOT_URL_BASE', '/static/screenshots')}/{app_name}/{neo4j_screenshot_path}"
    ...
    # Hàm này nên trả về chuỗi JSON của processed_ui_state gần nhất cho screen_id này
    detailed_ui_state_json_str = get_last_detailed_ui_state_for_screen(screen_id)
    
    processed_ui_state_for_analysis = {}
    if detailed_ui_state_json_str:
        try:
            processed_ui_state_for_analysis = json.loads(detailed_ui_state_json_str)
            current_app.logger.debug(f"Successfully loaded detailed UI state from PostgreSQL for screen {screen_id}")
        except json.JSONDecodeError as e:
            flash(f"Error decoding detailed UI state from PostgreSQL for screen {screen_id}: {e}", "warning")
            current_app.logger.error(f"JSONDecodeError for screen {screen_id}: {e}. Data: {detailed_ui_state_json_str[:200]}")
    else:
        current_app.logger.warning(f"No detailed UI state found in PostgreSQL for screen {screen_id}")
        flash(f"No detailed UI state (for element listing and hash analysis) found in PostgreSQL for screen {screen_id}", "info")

    # Trích xuất danh sách elements để hiển thị.
    # `processed_ui_state_for_analysis` nên chứa 'simplified_elements' hoặc 'elements'
    # theo như cách `determine_screen_id_from_state` và `merge_screen` sử dụng.
    ...
    if not elements_list_for_display and 'children' in processed_ui_state_for_analysis:
        # Nếu chỉ có cấu trúc cây, bạn cần hàm flatten_elements ở đây
        # elements_list_for_display = flatten_ui_tree_to_elements_list(processed_ui_state_for_analysis) # Ví dụ
        current_app.logger.warning(f"No 'simplified_elements' or 'elements' list in processed UI state for {screen_id}, tree flattening might be needed.")
        # For now, pass empty if not found directly:
    ...
    string_that_was_hashed = "Not available or could not be reconstructed."
    recalculated_screen_id_for_debug = None # Để so sánh với screen_id gốc

    if processed_ui_state_for_analysis and app_name: # Cần app_name để tính lại
        try:
            # Gọi hàm determine_screen_id_from_state đã được sửa đổi
            ...
            recalculated_screen_id_for_debug = recalculated_screen_id # Để debug xem có khớp không
            current_app.logger.debug(f"Hashing analysis for {screen_id} successful. Recalculated ID: {recalculated_screen_id_for_debug}")
        ...
        current_app.logger.warning(f"appName not found for screen {screen_id}, cannot perform hashing analysis.")
    elif not processed_ui_state_for_analysis:
        current_app.logger.warning(f"Processed UI state for analysis is empty for screen {screen_id}, cannot perform hashing analysis.")
        

    # 5. Lấy thông tin phân loại elements từ PostgreSQL (sẽ dùng ở Giai đoạn 2)
    element_classifications_raw = get_element_classifications_for_screen(screen_id) # Trả về list of dicts/rows
    element_classifications = {item['element_id']: item for item in element_classifications_raw} if element_classifications_raw else {}
    # Gắn thông tin phân loại vào elements_list_for_display
    for el in elements_list_for_display:
        el_xpath_id = el.get('xpath_id') # Hoặc định danh element bạn dùng
        if el_xpath_id and el_xpath_id in element_classifications:
            el['classification_info'] = element_classifications[el_xpath_id]
        else:
            el['classification_info'] = None


    # Tạo một set các (xpath_id, attribute_name) đã dùng để hash để kiểm tra nhanh trong template
    # Hoặc bạn có thể làm điều này trong template với Jinja filter
    # Ví dụ:
    contributing_identifiers_map = {} # {xpath_id: {attribute1: value1, attribute2: value2}}
    for feature in contributing_features_list:
        xpath = feature.get('xpath_id')
        attr = feature.get('attribute')
        if xpath and attr:
            if xpath not in contributing_identifiers_map:
                contributing_identifiers_map[xpath] = {}
            contributing_identifiers_map[xpath][attr] = feature.get('value')
    
    # Thêm cờ is_contributing_to_hash vào elements_list_for_display
    for el_display_item in elements_list_for_display:
        el_display_item['contributing_attributes'] = {} # Lưu các thuộc tính đã đóng góp vào hash của element này
        el_xpath = el_display_item.get('xpath_id')
        if el_xpath in contributing_identifiers_map:
            el_display_item['contributing_attributes'] = contributing_identifiers_map[el_xpath]
            # Đánh dấu chung nếu có bất kỳ thuộc tính nào đóng góp
            el_display_item['is_contributing_to_hash_overall'] = True if contributing_identifiers_map[el_xpath] else False
        else:
    ...
    return render_template('admin_screen_elements.html',
                           ...
                           screen_data_neo4j=screen_data_neo4j, # Node data từ Neo4j
                           original_screen_width=original_screen_width,
                           original_screen_height=original_screen_height,
                           screenshot_url=screenshot_url,
                           elements_to_display=elements_list_for_display, # Danh sách element để render bảng
                           # contributing_features_list=contributing_features_list, # Có thể không cần nếu đã xử lý ở trên
