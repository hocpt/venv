...
def create_app(config_class=config.Config):
    ...
    @app.route('/project_screenshots/<path:filename>')
    def serve_project_screenshot(filename):
        # Sử dụng logger của app hiện tại
        logger = app.logger
        screenshots_dir_from_config = app.config.get('UPLOAD_FOLDER')

        # === LOGGING CHI TIẾT ===
        logger.info(f"[serve_project_screenshot] Yêu cầu phục vụ file: '{filename}'")
        logger.debug(f"[serve_project_screenshot] Thư mục cấu hình UPLOAD_FOLDER: '{screenshots_dir_from_config}'")

        if not screenshots_dir_from_config:
            logger.error("[serve_project_screenshot] Lỗi: UPLOAD_FOLDER chưa được cấu hình trong app config.")
            return "Lỗi cấu hình server", 500

        # Kiểm tra xem file có tồn tại không
        file_path_to_check = os.path.join(screenshots_dir_from_config, filename)
        logger.debug(f"[serve_project_screenshot] Đường dẫn file đầy đủ để kiểm tra: '{file_path_to_check}'")

        if not os.path.exists(file_path_to_check):
            logger.error(f"[serve_project_screenshot] Lỗi 404: File không tồn tại tại đường dẫn: '{file_path_to_check}'")
            return "File ảnh không tìm thấy", 404
        # ========================

        try:
            logger.debug(f"[serve_project_screenshot] Đang gửi file '{filename}' từ thư mục '{screenshots_dir_from_config}'")
            return send_from_directory(screenshots_dir_from_config, filename)
        except Exception as e:
            logger.error(f"[serve_project_screenshot] Lỗi khi gửi file '{filename}': {e}", exc_info=True)
