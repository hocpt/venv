...
def create_app(config_class=config.Config):
    ...
    def serve_project_screenshot(filename):
        logger = app.logger # Hoặc app.logger
        screenshots_dir = app.config['UPLOAD_FOLDER']
        logger.debug(f"Serving from UPLOAD_FOLDER: {screenshots_dir}") # <<< THÊM LOG NÀY
        logger.debug(f"Attempting to serve file: {filename} from directory: {screenshots_dir}")
