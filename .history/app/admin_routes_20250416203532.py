...
def view_macro_definitions():
    """Hiển thị danh sách các Macro Code đã định nghĩa."""
    ...
    if not db:
        ...
        try:
            macros = db.get_all_macro_definitions() or []
            if macros is None:
                flash("Lỗi khi tải danh sách định nghĩa macro.", "error")
                macros = []
        except Exception as e:
            print(f"Lỗi load macro definitions: {e}")
            flash("Lỗi không mong muốn khi tải dữ liệu.", "error")
            macros = []
    return render_template('admin_macro_definitions.html', title=title, macros=macros)
...
def add_macro_definition_view():
    ...
    # Có thể lấy danh sách app target từ config hoặc DB nếu cần
...
@admin_bp.route('/macro-definitions/<macro_code>/edit', methods=['GET', 'POST'])
def edit_macro_definition_view(macro_code):
    ...
    if request.method == 'POST':
        ...
        if success:
            ...
            # Hiển thị lại form với lỗi, giữ lại dữ liệu vừa nhập
            current_data = request.form.copy() # Lấy dữ liệu từ form
            current_data['macro_code'] = macro_code # Thêm lại macro_code vì nó disabled
...
@admin_bp.route('/macro-definitions/<macro_code>/delete', methods=['POST'])
def delete_macro_definition_view(macro_code):
    ...
    if success:
        flash(f"Đã xóa định nghĩa macro '{macro_code}'.", 'success')
    else:
        flash(f"Xóa thất bại: {error_msg}", 'error')
...
def add_transition():
    ...
    strategy_stages = db.get_stages_for_strategy(strategy_id) or []
    all_stages = db.get_all_stages() or []
    # Lấy macro definitions thay vì templates
    all_macros = db.get_all_macro_definitions() or [] # <<< Lấy Macro Definitions
    ...
    if request.method == 'POST':
        ...
        next_stage_id = request.form.get('next_stage_id')
        priority_str = request.form.get('priority', '0')
        # Lấy macro code và params
        action_macro_code = request.form.get('action_macro_code') # <<< Lấy Macro Code
        action_params_str = request.form.get('action_params_str') # <<< Lấy Params JSON String
        ...
        # Gọi hàm DB đã sửa đổi
        success, error_msg = db.add_new_transition(current_stage_id, user_intent, condition_logic,
                                                  next_stage_id, priority,
                                                  action_macro_code, action_params_str) # <<< Truyền macro và params
...
def edit_transition(transition_id):
    ...
    transition = db.get_transition_details(transition_id) # Hàm này đã sửa để trả về macro_code/params_str
    if not transition:
        flash(f"Không tìm thấy transition ID {transition_id}.", "error")
        return redirect(url_for('admin.view_strategies'))

    strategy_id_redirect = transition.get('strategy_id')
    if not strategy_id_redirect: # Thêm kiểm tra phòng trường hợp strategy bị xóa
        flash(f"Không tìm thấy strategy gốc cho transition {transition_id}.", "error")
        return redirect(url_for('admin.view_strategies'))
    ...
    if request.method == 'POST':
        ...
        # Gọi hàm update DB đã sửa đổi
        success, error_msg = db.update_transition(transition_id, current_stage_id, user_intent, condition_logic,
                                                  next_stage_id, priority,
                                                  action_macro_code, action_params_str) # <<< Truyền macro và params
        ...
        if success:
            ...
            # Hiển thị lại form với lỗi, giữ dữ liệu nhập
    ...
    # GET request
    return render_template('admin_edit_transition.html', title=f"Sửa Transition {transition_id}",
                           transition=transition, # Chứa action_macro_code và action_params_str
                           strategy_stages=strategy_stages,
                           all_stages=all_stages,
                           all_macros=all_macros, # <<< Truyền macros
                           valid_intents=VALID_INTENTS_FOR_TRANSITION)
