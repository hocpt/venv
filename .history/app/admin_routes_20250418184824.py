...
def add_transition_control():
    ...
    if request.method == 'POST':
        ...
        params_dict = {} # Khởi tạo để dùng nếu validate thành công
        if action_params_str and action_params_str.strip() and action_params_str != '{}':
             try:
                  params_dict = json.loads(action_params_str) # Parse thử
        ...
        if errors:
            for error in errors: flash(error, "warning")
            # <<< SỬA RENDER_TEMPLATE: Dùng keyword arguments >>>
            return render_template('admin_add_transition_control.html',
                                   title=title + " (Lỗi)",
                                   strategy_id=strategy_id,
                                   current_stage_id_prefill=current_stage_id, # Dùng biến này nếu cần
                                   strategy_stages=strategy_stages,
                                   all_stages=all_stages,
                                   all_macros=all_macros,
                                   valid_intents=VALID_INTENTS_FOR_TRANSITION,
        ...
        try:
            success, error_msg = db.add_new_transition(
                ...
                # Chỉ truyền params string nếu nó hợp lệ và không rỗng
                action_params_str if (action_params_str and action_params_str.strip() and action_params_str != '{}' and not any('Action Params' in e for e in errors)) else None,
...
def delete_transition(transition_id):
    ...
    # --- Cố gắng lấy thông tin TRƯỚC KHI XÓA ---
    try:
        ...
        if transition_details:
            ...
            print(f"DEBUG (delete_transition): Found strategy_id_redirect: {strategy_id_redirect}")
            if strategy_id_redirect:
                 ...
                 if strategy_info:
                      strategy_type_redirect = strategy_info.get('strategy_type', 'unknown') # Lấy type, default là unknown
                      print(f"DEBUG (delete_transition): Determined strategy_type_redirect: {strategy_type_redirect}")
                      ...
                      if strategy_type_redirect == 'control':
                           ...
                           print(f"WARN (delete_transition): Unknown strategy type '{strategy_type_redirect}' found.")
                           redirect_endpoint = None # Không xác định được endpoint chi tiết
                 else:
                      print(f"WARN (delete_transition): Could not get strategy details for ID '{strategy_id_redirect}'.")
                      strategy_id_redirect = None # Không có info strategy thì không redirect về detail được
            else:
                 print(f"WARN (delete_transition): Transition {transition_id} has no associated strategy_id.")
                 strategy_id_redirect = None # Đặt lại strategy_id nếu nó rỗng trong details
        else:
            print(f"ERROR (delete_transition): Could not find transition details for ID {transition_id}.")
            strategy_id_redirect = None # Không tìm thấy transition

    except Exception as e_fetch:
        print(f"ERROR (delete_transition): Exception while fetching details before delete: {e_fetch}")
        print(traceback.format_exc())
        strategy_id_redirect = None # Đặt là None nếu có lỗi khi fetch

    # --- Thực hiện Xóa ---
    try:
        # Giả sử hàm delete_transition trả về tuple (success, error_msg)
        success, error_msg = db.delete_transition(transition_id)
        if success:
            ...
            flash(f"Xóa transition ID {transition_id} thất bại: {error_msg or 'ID không tồn tại?'}", 'error')
            print(f"ERROR (delete_transition): Failed to delete transition {transition_id}: {error_msg}")
        ...
        # Dù xóa lỗi, vẫn cố gắng redirect về nơi hợp lý dựa trên thông tin đã lấy được (nếu có)
    ...
    if strategy_id_redirect and redirect_endpoint:
        ...
        # Thay vì luôn về language list, chúng ta có thể về Dashboard hoặc trang list gần nhất có thể
        print(f"WARN (delete_transition): Fallback redirect needed. strategy_id={strategy_id_redirect}, type={strategy_type_redirect}, endpoint={redirect_endpoint}")
        flash("Đã xóa transition. Không thể quay lại trang chi tiết chiến lược.", "info")
        # Một fallback tốt hơn là về Dashboard admin
        return redirect(url_for('admin.index'))    
