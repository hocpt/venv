...
def view_scheduled_jobs():
    ...
    db_jobs_config = []
    pending_counts = {} # Lưu số lượng chờ xử lý cho suggestion_job
    ...
    if not db:
        ...
        try:
            ...
            # Logic đếm pending cho suggestion_job
    ...
    # 2. Không còn lấy trạng thái live từ scheduler nữa

    # 3. Render template chỉ với cấu hình DB và pending counts
    return render_template('admin_scheduled_jobs.html',
                            title="Quản lý Tác vụ nền (Cấu hình)",
                            jobs=db_jobs_config,
                            pending_counts=pending_counts) # Truyền số lượng chờ


@admin_bp.route('/scheduled-jobs/add', methods=['GET', 'POST'])
def add_scheduled_job():
    """Thêm cấu hình Job mới vào DB."""
    if not db: # Kiểm tra db trước khi dùng
    ...
    if request.method == 'POST':
        ...
        # ----- Validate Input -----
        error = False
        error_msg = [] # List để chứa các thông báo lỗi
        if not job_id: error_msg.append("Job ID là bắt buộc.")
        if not function_path: error_msg.append("Function Path là bắt buộc.")
        if not trigger_type: error_msg.append("Trigger Type là bắt buộc.")
        if trigger_type not in TRIGGER_TYPES: error_msg.append(f"Trigger Type '{trigger_type}' không hợp lệ.")

        trigger_args_dict = {}
        try:
            if not trigger_args_str: trigger_args_str = '{}' # Đảm bảo là JSON hợp lệ nếu rỗng
            trigger_args_dict = json.loads(trigger_args_str)
            if not isinstance(trigger_args_dict, dict): raise ValueError("phải là một JSON object.")
        except (json.JSONDecodeError, ValueError) as e:
             error_msg.append(f"Trigger Args không hợp lệ: {e}")

        # Chuyển đổi kiểu dữ liệu số (chỉ nếu JSON hợp lệ)
        if not any("Trigger Args" in msg for msg in error_msg):
            numeric_keys = ['weeks', 'days', 'hours', 'minutes', 'seconds', 'jitter', 'year', 'month', 'day', 'week', 'hour', 'minute', 'second']
            try:
                # Tạo bản copy để không sửa đổi dict gốc khi lặp
                args_to_check = trigger_args_dict.copy()
                for key in numeric_keys:
                    if key in args_to_check:
                        value = args_to_check[key] # Lấy giá trị
                        if isinstance(value, str): # Chỉ chuyển đổi nếu là chuỗi
                            value_str = value.strip()
                            if not value_str:
                                 # Nếu chuỗi rỗng, xóa key khỏi dict cuối cùng
                                 if key in trigger_args_dict: del trigger_args_dict[key]
                                 continue
                            # Thử chuyển đổi
                            if '.' in value_str: trigger_args_dict[key] = float(value_str)
                            else: trigger_args_dict[key] = int(value_str)
                        elif not isinstance(value, (int, float)): # Nếu không phải chuỗi và cũng không phải số
                             raise ValueError(f"giá trị cho '{key}' phải là số hoặc chuỗi số.")
            except (ValueError, TypeError) as e:
                error_msg.append(f"Giá trị số trong Trigger Args không hợp lệ: {e}")

        # Validate function path bằng cách thử import (chỉ nếu các lỗi trước chưa xảy ra)
        if not error_msg:
            try:
                 ...
                 error_msg.append(f"Function Path '{function_path}' không hợp lệ hoặc không tìm thấy: {import_err}")
        ...
        # ----- Lưu vào DB -----
        # Lưu trigger_args_str gốc (chuỗi JSON) người dùng nhập vào DB
        db_success, db_error = db.add_job_config(job_id, function_path, trigger_type, trigger_args_str, is_enabled, description)

        if db_success:
            flash(f"Thêm cấu hình tác vụ '{job_id}' thành công! Khởi động lại server để áp dụng lịch chạy.", 'success')
...
def edit_scheduled_job(job_id):
    """Sửa cấu hình một Job trong DB."""
    if not db:
         flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
         return redirect(url_for('admin.view_scheduled_jobs')) # Hoặc trang khác

    # Lấy dữ liệu job hiện tại từ DB cho GET và để dùng lại nếu POST lỗi
    ...
    if request.method == 'POST':
        ...
        # Validate JSON và chuyển đổi kiểu số
        trigger_args_dict = {}
        try:
            ...
            for key in numeric_keys:
                if key in args_to_check and isinstance(args_to_check[key], str):
                    ...
                    if not value_str:
                         del trigger_args_dict[key]; continue
                    if '.' in value_str: trigger_args_dict[key] = float(value_str)
                    else: trigger_args_dict[key] = int(value_str)
        ...
        if db_success:
             ...
             # Hiển thị lại form với lỗi nếu muốn
...
def toggle_scheduled_job(job_id):
    ...
    # <<< KHÔNG CÒN TƯƠNG TÁC VỚI SCHEDULER LIVE >>>
    return redirect(url_for('admin.view_scheduled_jobs'))


# Hàm chạy thủ công suggestion_job (Giữ nguyên)
...
def run_suggestion_job_now():
    ...
    try:
        ...
        limit = current_app.config.get('MANUAL_SUGGEST_LIMIT', 10)
        status_filter = current_app.config.get('STATUS_TO_ANALYZE_SUGGEST', ['success_ai'])
        print(f"DEBUG: (Manual Run) Lấy tối đa {limit} tương tác với status {status_filter} sau ID {last_processed_id}...")
        interactions = db.get_interactions_for_suggestion(min_history_id=last_processed_id, status_filter=status_filter, limit=limit)
        ...
        for interaction in interactions:
            ...
            if not interaction_data['received_text'] or not interaction_data['sent_text']:
                ...
                continue
            try:
                suggested_keywords, suggested_category, suggested_template_ref, suggested_template = ai_service.suggest_rule_from_interaction(interaction_data=interaction_data, persona_id=persona_id_for_suggestion)
            ...
            if suggested_keywords or suggested_template:
                ...
                try:
                    added = db.add_suggestion(keywords=suggested_keywords, category=suggested_category, template_ref=suggested_template_ref, template_text=suggested_template, source_examples=source_examples)
                    if added: suggestions_added += 1; print(f"INFO: (Manual Run) Đã lưu đề xuất từ interaction {history_id}.")
                    else: print(f"ERROR: (Manual Run) Lưu đề xuất từ interaction {history_id} thất bại.")
                except Exception as db_add_err:
                     print(f"ERROR: (Manual Run) Exception calling db.add_suggestion for ID {history_id}: {db_add_err}")
                     print(traceback.format_exc())
            else: print(f"DEBUG: (Manual Run) AI không tạo ra đề xuất hợp lệ cho interaction {history_id}.")
        if max_processed_id_in_batch > last_processed_id:
            ...
            if not update_success: error_message = "Lỗi khi cập nhật trạng thái last_processed_id sau khi chạy."; print(f"CRITICAL ERROR: (Manual Run) {error_message}")
        if error_message: flash(f"Chạy thủ công hoàn tất với lỗi: {error_message}", "error")
        else: flash(f"Chạy thủ công hoàn tất. Đã xử lý {processed_count}/{interactions_found_count} tương tác. Thêm {suggestions_added} đề xuất mới. ID xử lý cuối cùng được cập nhật thành {max_processed_id_in_batch}.", "success")
    except Exception as e:
         print(f"LỖI NGHIÊM TRỌNG trong khi chạy thủ công {job_id_to_run}: {e}")
         print(traceback.format_exc()); flash(f"Lỗi nghiêm trọng khi chạy thủ công: {e}", "error")
    return redirect(url_for('admin.view_scheduled_jobs'))
