...
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
    ...
    if not screen_id or not app_name: return False
    ...
    # --- Chuẩn bị standardized_elements ---
        ...
        for el in extracted_elements:
            if isinstance(el, dict) and el.get('element_id'):
                element_entry = {
                    ...
                    # 'coordinates': el.get('coordinates'), # <<< BỎ LƯU MAP TRỰC TIẾP
                    # <<< THAY THẾ BẰNG KIỂU Point hoặc x, y riêng >>>
                    'classification': 'unclassified',
                    'attempt_count': 0,
                    'success_count': 0,
                    'is_clickable_observed': False,
                    'is_editable_observed': False,
                    # <<< DÙNG datetime CÓ TIMEZONE >>>
                    'last_seen_timestamp': current_time_utc
                }
                # Xử lý coordinates
                coords = el.get('coordinates')
                if isinstance(coords, dict) and 'x' in coords and 'y' in coords:
                    # Lựa chọn 1: Dùng kiểu Point (khuyến nghị)
                    try:
                       # Neo4j thường dùng hệ tọa độ WGS-84 (longitude, latitude) hoặc Cartesian.
                       # Kiểm tra xem driver có hỗ trợ tạo Point 2D không gian thường không.
                       # Nếu dùng tọa độ màn hình pixel, lưu x, y riêng có thể dễ hơn.
                       # Tạm thời lưu x, y riêng:
                       element_entry['coordinate_x'] = coords['x']
                       element_entry['coordinate_y'] = coords['y']
                       # Hoặc thử tạo Point nếu driver hỗ trợ (cần xem docs driver):
                       # element_entry['location'] = Point((coords['x'], coords['y']))
                    except Exception as point_err:
                       logger.warning(f"Could not create Point for coordinates {coords}: {point_err}")
                       element_entry['coordinate_x'] = coords.get('x') # Lưu riêng nếu lỗi
                       element_entry['coordinate_y'] = coords.get('y')

                # Chỉ giữ lại key có giá trị (loại bỏ None)
    ...
    screen_props = {
        ...
        "last_seen": current_time_utc, # <<< Dùng datetime có timezone
        "element_count": len(standardized_elements) # Đếm list đã chuẩn hóa
    }
    ...
    try:
        with driver.session(database=current_app.config.get('NEO4J_DATABASE', 'neo4j')) as session:
            ...
            ON CREATE SET
                ...
                s.created_at = $props.last_seen // Dùng datetime có timezone
            ON MATCH SET
                s.app_name = $props.app_name,
                s.activity_name = $props.activity_name,
                s.elements = $elements_list, // Vẫn ghi đè tạm thời
                s.element_count = $props.element_count,
                s.last_analyzed_log_id = $props.last_analyzed_log_id,
                s.last_seen = $props.last_seen,
                s.updated_at = $props.last_seen // Dùng datetime có timezone
            """
            # Neo4j driver sẽ tự chuyển đổi datetime timezone-aware của Python thành DateTime của Neo4j
            session.run(query, props=screen_props, elements_list=standardized_elements)
            logger.debug(f"Merged Screen node: {screen_id}")
            return True
    except neo4j.exceptions.CypherTypeError as type_error: # Bắt lỗi cụ thể
         logger.error(f"Neo4j TypeError (merge_screen for {screen_id}): {type_error}")
         logger.error(f"Data attempted to save in elements_list: {standardized_elements}") # Log dữ liệu gây lỗi
         return False
