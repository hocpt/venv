...
def _process_pending_commands():
            ...
            try:
                ...
                # --- LỆNH CHẠY MÔ PHỎNG ---
                    ...
                    live_scheduler.add_job(id=job_id, func=run_sim_func, args=job_args, trigger='date', run_date=run_time,
                                           ...
                                           name=f"Sim: {persona_a_id} vs {persona_b_id}") # Thêm name cho dễ nhìn
                    db.update_command_status(conn, command_id, 'done')
    ...
    # <<< THAY ĐỔI KHỐI FINALLY NÀY >>>
        if conn: # Chỉ thực hiện nếu conn đã được gán giá trị (không phải None)
            try:
                # Kiểm tra xem kết nối có bị đóng chưa TRƯỚC KHI thao tác
                if not conn.closed:
                    # Nếu chưa đóng, thì mới reset autocommit và đóng nó
                    conn.autocommit = True # Trả về trạng thái autocommit mặc định
                    conn.close()
                    print("DEBUG (Command Processor): DB Connection closed successfully in finally.")
                else:
                    # Nếu đã đóng rồi thì chỉ cần log ra
                    print("DEBUG (Command Processor): DB Connection was already closed before finally block.")
            except psycopg2.Error as close_err: # Bắt lỗi có thể xảy ra khi close hoặc set autocommit
                print(f"ERROR (Command Processor): Error during connection cleanup: {close_err}")
            except Exception as general_close_err: # Bắt lỗi chung khác
                print(f"ERROR (Command Processor): Unexpected error during connection cleanup: {general_close_err}")
    # <<< KẾT THÚC THAY ĐỔI KHỐI FINALLY >>>
# --- HÀM GIÁM SÁT MỚI ---
