...
def get_all_task_assignments(filters: dict = None, page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    ...
    select_sql = """
            SELECT
                ...
                ta.result_data, -- <<< THAY THẾ: Lấy result_data (JSONB) thay vì error_message
                ta.created_at,
                ta.assigned_at,
                ta.last_report_at,
                ta.completed_at
    ...
    try:
        ...
        if total_items > 0:
            ...
            # Chuyển đổi JSONB thành dict nếu cần (thường DictCursor tự làm)
            for task in assignments:
                    if isinstance(task.get('target_data'), str):
                        try: task['target_data'] = json.loads(task['target_data'])
                        except: task['target_data'] = {}
                    if isinstance(task.get('result_data'), str):
