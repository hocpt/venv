...
def merge_screen(screen_id: str, app_name: str, activity_name: str | None,
                 extracted_elements: list, # log_id: int | None, # Bỏ log_id nếu không dùng trực tiếp
                 screenshot_path: str | None = None,
                 screen_width: int | None = None, screen_height: int | None = None,
                 is_defined_by_pie: bool = False # Thêm cờ này để biết status
                 ) -> bool:
    """
    ...
    logger.debug(f"[merge_screen] Received screenshot_path parameter: '{screenshot_path}' (Type: {type(screenshot_path)})")
    ...
    screen_status = 'defined' if is_defined_by_pie else 'provisional_unknown'
        ...
        for el_count, el in enumerate(extracted_elements):
            if isinstance(el, dict) and el.get('element_id'):
                element_entry = {
                    ...
                    'last_seen_timestamp': current_time_utc_iso # <<< LƯU DẠNG CHUỖI ISO >>>
                }
                # Xử lý coordinates
    ...
    params = {
        ...
        "lastSeen": current_time_utc,
        "elementCount": len(standardized_elements),
        "screenshotPath": screenshot_path # Truyền giá trị (có thể là None)
    }
    # === LOG 2: Kiểm tra screen_props trước khi lọc None ===
    logger.debug(f"[merge_screen] screen_props before cleaning None: {screen_props}")
    # ====================================================

    # Loại bỏ key có giá trị None khỏi props
    screen_props_clean = {k: v for k, v in screen_props.items() if v is not None}

    # === LOG 3: Kiểm tra screen_props_clean SAU KHI lọc None ===
    # Quan trọng: Xem screenshot_path có còn trong dict này không
    logger.debug(f"[merge_screen] screen_props_clean AFTER cleaning None: {screen_props_clean}")
    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            ON CREATE SET
                ...
                s.screenshot_path = $screenshotPath, // Gán tường minh
                s.created_at = $lastSeen
            ON MATCH SET
                s.app_name = $appName, // Cập nhật lại nếu thay đổi
                s.activity_name = $activityName,
                // s.status = $status, // Có thể không muốn ghi đè status khi MATCH
                s.last_analyzed_log_id = $lastAnalyzedLogId,
                s.last_seen = $lastSeen,
                s.element_count = $elementCount,
                s.screenshot_path = $screenshotPath, // Gán tường minh cả khi MATCH
                s.updated_at = $lastSeen
            """
            ...
            # === LOG 4: Log props truyền vào session.run ===
            logger.debug(f"[merge_screen] Executing MERGE query with parameter props: {screen_props_clean}")
            # ============================================
