...
@admin_bp.route('/api/mapping_data', methods=['GET']) # <<< Sử dụng route này
# @login_required # Đảm bảo có xác thực nếu cần
def api_get_app_graph_data(): # <<< Tên hàm này phải khớp với nơi gọi (nếu có)
    """
    ...
    try:
        ...
        if not graph_data.get("nodes"):
             ...
             return jsonify({"nodes": [], "edges": graph_data.get("edges", [])}) # Trả về node rỗng

        # === THÊM screenshot_url VÀO DỮ LIỆU NODE ===
        processed_nodes = []
        for node_element in graph_data.get("nodes", []):
            ...
            if filename:
                try:
                    ...
                    logger.error(f"API: Lỗi tạo URL cho screenshot '{filename}' của node {node_data.get('id')}: {url_err}")
                    node_data["screenshot_url"] = None # Đặt là None nếu lỗi
            else:
                node_data["screenshot_url"] = None # Không có path thì không có url
