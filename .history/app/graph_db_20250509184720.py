...
def get_elements_for_screen(screen_id: str) -> list[dict] | None:
    ...
    logger = current_app.logger if current_app else log
    if not screen_id:
        logger.warning("get_elements_for_screen: screen_id is required.")
    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            ORDER BY e.element_id // Hoặc một thứ tự khác nếu muốn
            """
            result = session.run(query, screen_id=screen_id)
            for record in result:
                props = record["element_properties"]
                # Xử lý lại bounds/coords nếu cần (ví dụ: Neo4j lưu point)
                # Hiện tại giả sử nó đã là dict {left,top,right,bottom} hoặc {x,y}
                elements_list.append(dict(props))
        logger.debug(f"Fetched {len(elements_list)} elements for screen '{screen_id}'.")
        return elements_list
    except Exception as e:
        logger.error(f"Neo4j Error (get_elements_for_screen for {screen_id}): {e}", exc_info=True)
        return None