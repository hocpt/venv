...
# @admin_required # Bỏ comment nếu cần bảo vệ API
def api_get_app_graph_data(app_name):
    ...
    try:
        ...
        if graph_data:
            # Không cần convert datetime ở đây vì hàm get_app_graph_data đã lấy dữ liệu đơn giản
            return jsonify(graph_data)
        else:
            # Trả về cấu trúc rỗng hợp lệ nếu không có dữ liệu hoặc lỗi DB
            return jsonify({"nodes": [], "edges": []})
    except Exception as e:
        current_app.logger.error(f"Error fetching graph data for {app_name}: {e}", exc_info=True)
