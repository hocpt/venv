...
def edit_scheduled_job(job_id):
    ...
    if request.method == 'POST':
        ...
        if error:
            # Truyền lại job_details (data cũ từ DB) và request.form (data mới bị lỗi)
        ...
        if not db_success:
             ...
             # Hiển thị lại form với dữ liệu người dùng vừa nhập bị lỗi
        ...
        if scheduler:
            try:
                print(f"Attempting to update job '{job_id}' in live scheduler with converted args: {trigger_args_dict}")

                # 1. Tạo đối tượng trigger mới với dict đã convert
                trigger_obj = None
                if trigger_type == 'interval':
                    trigger_obj = IntervalTrigger(**trigger_args_dict)
                elif trigger_type == 'cron':
                ...
                # 2. Gọi modify_job với trigger object
                scheduler.modify_job(job_id, trigger=trigger_obj)
                print(f"Modified job '{job_id}' trigger/args.")

                # 3. Pause/Resume dựa trên is_enabled
                live_job = scheduler.get_job(job_id)
                if live_job:
                    is_currently_paused = (live_job.next_run_time is None)
                    action_taken = ""
                    if is_enabled and is_currently_paused:
                         ...
                         action_taken = "Paused"
                    if action_taken:
                         print(f"Successfully {action_taken} job '{job_id}' enable status in live scheduler.")
                    else:
                         print(f"Job '{job_id}' state ({'enabled' if is_enabled else 'disabled'}) already matches scheduler state.")

                elif is_enabled: # Job được bật trong DB nhưng không có trong scheduler -> Thử thêm lại
                    print(f"WARNING: Job '{job_id}' not found in scheduler after DB update. Attempting to add it...")
                    try:
                        ...
                        if function_path:
                            ...
                            module = importlib.import_module(module_path)
                            func = getattr(module, func_name)
                            scheduler.add_job(id=job_id, func=func, trigger=trigger_obj, replace_existing=True)
                            print(f"Successfully ADDED job '{job_id}' to scheduler on edit enable.")
                            scheduler_message = f"Job '{job_id}' không tìm thấy trong scheduler, đã được thêm lại và kích hoạt."
                            flash(scheduler_message, "info") # Dùng info thay vì warning
                        else:
                             raise ValueError("Thiếu function_path để thêm lại job.")
                    except Exception as re_add_err:
                         scheduler_message = f"Lỗi khi cố gắng thêm lại job '{job_id}' vào scheduler sau khi sửa: {re_add_err}"
                         print(f"ERROR: {scheduler_message}")
                         # Flash lỗi này ở cuối vòng try chính
                else:
                     # Job bị tắt trong DB và cũng không có trong scheduler -> Ok
                     print(f"INFO: Job '{job_id}' disabled in DB and not found in scheduler. No scheduler action needed.")


            except (TypeError, ValueError) as trigger_err:
                 scheduler_message = f"Lưu DB thành công, nhưng Trigger Args/Type không hợp lệ khi cập nhật scheduler: {trigger_err}."
                 print(f"ERROR updating job '{job_id}' in scheduler: {scheduler_message}")
                 flash(scheduler_message, "warning")
            except JobLookupError:
                 scheduler_message = f"Lưu DB thành công, nhưng không tìm thấy job '{job_id}' trong scheduler đang chạy để cập nhật/pause/resume."
