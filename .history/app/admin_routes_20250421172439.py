...
def view_accounts():
    """
    Hiển thị trang quản lý danh sách các tài khoản (Accounts)
    với phân trang và chức năng tìm kiếm.
    Xử lý trường hợp hàm CSDL trả về None để tránh lỗi TypeError.
    """
    page = request.args.get('page', 1, type=int)
    # Đảm bảo page luôn >= 1
    if page < 1:
    ...
    accounts_list = []      # Giá trị mặc định nếu có lỗi hoặc không có dữ liệu
    pagination_obj = None   # Giá trị mặc định nếu có lỗi

    try:
        # Gọi hàm CSDL để lấy dữ liệu và phân trang
        # Truyền None nếu search_query rỗng sau khi strip()
        pagination_result = db.get_all_accounts_paginated(
            page=page,
            per_page=per_page,
            search_query=search_query if search_query else None
        )

        # Kiểm tra kết quả trả về từ hàm CSDL
        if pagination_result:
            # Nếu thành công, gán kết quả
            accounts_list = pagination_result.items
            pagination_obj = pagination_result

            # Xử lý trường hợp người dùng truy cập trang không tồn tại (ví dụ page=999)
            # khi pagination_result.items rỗng nhưng pagination_obj.pages > 0
            if not accounts_list and page > 1 and pagination_obj.pages > 0:
                 if page > pagination_obj.pages:
                    flash(f'Trang {page} không tồn tại. Hiển thị trang cuối ({pagination_obj.pages}).', 'warning')
                    # Chuyển hướng về trang cuối cùng có dữ liệu
                    return redirect(url_for('.view_accounts', page=pagination_obj.pages, search=search_query))
                 # else: trang hợp lệ nhưng không có KQ tìm kiếm -> hiển thị bình thường
        else:
            # Nếu hàm CSDL trả về None (do lỗi bên trong hàm đó, ví dụ CSDL lỗi)
            # Lỗi cụ thể đã được log bên trong hàm get_all_accounts_paginated
            flash('Không thể tải danh sách tài khoản do lỗi truy vấn CSDL. Vui lòng kiểm tra log server.', 'error')
            # Giữ giá trị mặc định: accounts_list = [], pagination_obj = None

    except Exception as e:
        # Bắt các lỗi không mong muốn khác có thể xảy ra (ngoài lỗi CSDL đã xử lý trong hàm get...)
        current_app.logger.error(f"Lỗi không xác định trong view_accounts: {e}", exc_info=True) # Log cả traceback
        flash('Đã xảy ra lỗi không mong muốn khi xử lý yêu cầu.', 'error')
        # Giữ giá trị mặc định: accounts_list = [], pagination_obj = None

    # Render template với dữ liệu đã được xử lý (hoặc giá trị mặc định nếu lỗi)
    return render_template('admin_accounts.html',
                           title="Quản lý Tài khoản",      # Truyền tiêu đề trang
                           accounts=accounts_list,         # Luôn là một list
                           pagination=pagination_obj,      # Là object pagination hoặc None (template cần xử lý được None)
...
def delete_stage(stage_id):
    ...
    # <<< THÊM: Lấy strategy_type để redirect đúng >>>
    strategy_type_redirect = 'unknown' # Default
    if strategy_id_redirect:
        strategy_info = db.get_strategy_details(strategy_id_redirect)
        if strategy_info:
            strategy_type_redirect = strategy_info.get('strategy_type', 'unknown')
    # <<< KẾT THÚC THÊM >>>

    try:
        success = db.delete_stage(stage_id) # Giả sử hàm này tồn tại
        if success:
    ...
    if strategy_id_redirect:
        # Xác định endpoint dựa trên type đã lấy được
        if strategy_type_redirect == 'control':
            ...
            # Fallback nếu type không xác định (về trang strategies chung - nếu có)
            flash("Không xác định được loại chiến lược để quay về trang chi tiết.", "warning")
            # Hoặc redirect về list tương ứng nếu biết type từ biến trước đó
            if strategy_type_redirect == 'control': # Dùng lại biến type đã lấy
                return redirect(url_for('admin.view_strategies_control'))
            else:
                return redirect(url_for('admin.view_strategies_language')) # Mặc định về language list

        # Redirect về trang chi tiết đúng
        return redirect(url_for(redirect_endpoint, strategy_id=strategy_id_redirect))
    else:
        # Fallback nếu không lấy được strategy_id (rất hiếm nếu dữ liệu đúng)
        flash("Không thể xác định chiến lược gốc để quay lại.", "warning")
        # Redirect về trang danh sách strategies chung (tạm về language)
        return redirect(url_for('admin.view_strategies_language'))
...
def add_transition_control():
    ...
    all_stages = []
    all_macros = []
    cancel_url = url_for('admin.view_strategies_control') # Fallback cancel URL
    ...
    if request.method == 'POST':
        ...
        current_stage_id = request.form.get('current_stage_id')
        user_intent = request.form.get('user_intent')
        condition_type = request.form.get('condition_type', '').strip() # Lấy và strip
        ...
        if loop_type == 'repeat_n':
            ...
            if not loop_condition_type: flash('Cần chọn Điều kiện Lặp khi chọn loại \'While\'.', 'danger')
        ...
        if errors:
            for error in errors: flash(error, "warning")
            # <<< Truyền đủ dữ liệu dropdown và current_data (bao gồm cả loop_*) >>>
        ...
        try:
            # <<< Truyền TẤT CẢ tham số, bao gồm cả loop_* đã xử lý >>>
            success, error_msg = db.add_new_transition(
                ...
                loop_count=loop_count, # Truyền giá trị int (hoặc None)
            ...
            if success:
                ...
                return redirect(cancel_url) # <<< Dùng cancel_url đã tính (trỏ về control detail)
            else:
                flash(f'Thêm control transition thất bại: {error_msg or "Lỗi."}', 'error')
                # <<< Truyền đủ dữ liệu dropdown và current_data >>>
                return render_template('admin_add_transition_control.html',
                                       title=title + " (Lỗi DB)",
                                       strategy_id=strategy_id,
                                       cancel_url=cancel_url,
    ...
    # --- GET request ---
    # <<< Truyền đủ dữ liệu dropdown và cancel_url >>>
    return render_template('admin_add_transition_control.html',
                           title=title,
                           strategy_id=strategy_id,
                           cancel_url=cancel_url,
...
def edit_transition_control(transition_id):
    ...
    try:
        ...
        print(f"ERROR (edit_transition_control): Lỗi tải dữ liệu cho form: {e}")
        flash(f"Lỗi tải dữ liệu cho form: {e}", "error")
    ...
    if request.method == 'POST':
        ...
        if errors:
            for error in errors: flash(error, "warning")
            # <<< Truyền đủ dropdowns, cancel_url và current_data >>>
        ...
        try:
            # <<< Truyền TẤT CẢ tham số, bao gồm loop_* đã xử lý >>>
            success, error_msg = db.update_transition(
                ...
                # --- Truyền các giá trị loop ---
                loop_type=loop_type if loop_type else None,
                loop_count=loop_count, # Truyền int hoặc None
                loop_condition_type=loop_condition_type if loop_condition_type else None,
                loop_condition_value=loop_condition_value if loop_condition_value else None,
            ...
            if success:
                ...
                return redirect(cancel_url) # <<< Dùng cancel_url đã tính
            else:
    ...
    # <<< Truyền đủ dropdowns và cancel_url >>>
    return render_template('admin_edit_transition_control.html',
                           title=title,
...
@admin_bp.route('/task-assignments')
def view_task_assignments():
    """Hiển thị danh sách Task Assignments với bộ lọc và phân trang."""
    title = "Quản lý Giao việc (Task Assignments)"
    assignments = []
    pagination = None
    ...
    if not db:
        ...
        try:
            ...
            filters = {
                ...
                'strategy_id': request.args.get('filter_strategy_id', '').strip(),
                'device_id': request.args.get('filter_device_id', '').strip(),
                'account_id': request.args.get('filter_account_id', '').strip(),
            }
            # Chỉ giữ lại filter có giá trị
            active_filters = {k: v for k, v in filters.items() if v}
            current_app.logger.debug(f"Task Assignment Filters: {active_filters}")

            # Lấy dữ liệu assignment đã lọc và phân trang
            assignments, total_items = db.get_all_task_assignments(
                filters=active_filters,
                page=page,
                per_page=PER_PAGE_ASSIGNMENTS
            )

            if assignments is None or total_items is None:
                 flash("Lỗi khi tải danh sách Task Assignments từ CSDL.", "error")
                 assignments = []; total_items = 0
            else:
                 # Tính toán phân trang
                 total_pages = 0
                 if total_items > 0: total_pages = ceil(total_items / PER_PAGE_ASSIGNMENTS)
                 if page > total_pages and total_pages > 0: page = total_pages
                 pagination = {
                     'page': page, 'per_page': PER_PAGE_ASSIGNMENTS, 'total_items': total_items,
                     'total_pages': total_pages, 'has_prev': page > 1, 'has_next': page < total_pages,
                     'prev_num': page - 1, 'next_num': page + 1,
                     'page_param': 'page' # Tên tham số page trong URL
                 }

            # Lấy dữ liệu cho dropdown bộ lọc
            available_strategies = db.get_all_strategies(strategy_type_filter='control') or []
            available_devices = db.get_all_devices_for_select() or [] # Cần hàm này trong db.py

        except Exception as e:
            current_app.logger.error(f"Lỗi nghiêm trọng load task assignments: {e}", exc_info=True)
            flash("Lỗi không mong muốn khi tải dữ liệu Task Assignments.", "error")
            assignments, pagination, available_strategies, available_devices = [], None, [], []
...
def edit_device(device_id):
    """Hiển thị form và xử lý sửa thông tin thiết bị."""
    # ... (code kiểm tra DB và lấy device details như cũ) ...
    ...
    editable_statuses = ['online', 'offline', 'disabled', 'error'] # Các status admin có thể chọn
    ...
    if request.method == 'POST':
        ...
        try:
            # <<< Truyền thêm os_info, macrodroid_version vào hàm DB >>>
            success, error_msg = db.update_device_admin(
                device_id=device_id,
                device_name=device_name or None,
                notes=notes or None,
                status=status if status else None, # Chỉ cập nhật nếu admin chọn
                os_info=os_info or None, # <<< Thêm
                macrodroid_version=macrodroid_version or None # <<< Thêm
            )
            if success:
                flash(f"Cập nhật thiết bị '{device_id}' thành công!", 'success')
                return redirect(url_for('admin.view_devices'))
            else:
                 flash(f"Cập nhật thiết bị thất bại: {error_msg or 'Không tìm thấy ID hoặc không có thay đổi.'}", 'error')
                 return render_template('admin_edit_device.html', title=title + " (Lỗi DB)",
                                        device=device, editable_statuses=editable_statuses,
                                        current_data=request.form)
    ...
    # GET Request
    return render_template('admin_edit_device.html', title=title, device=device, editable_statuses=editable_statuses)
...
# ... (Route cancel_task_assignment đã có) ...
...
@admin_bp.route('/task-assignments/<int:assignment_id>/logs')
def view_assignment_logs(assignment_id):
    """Hiển thị trang xem log chi tiết của một Task Assignment."""
    title = f"Log chi tiết cho Assignment ID {assignment_id}"
    assignment = None
    logs = []
    pagination = None
    ...
    try:
        # Lấy thông tin assignment để hiển thị context
        ...
        # Lấy dữ liệu log đã phân trang
        logs, total_items = db.get_assignment_logs(
            assignment_id=assignment_id,
            page=page,
            per_page=PER_PAGE_LOGS
        )

        if logs is None or total_items is None:
             flash(f"Lỗi khi tải log cho Assignment ID {assignment_id}.", "error")
             logs = []; total_items = 0
        else:
             # Tính toán phân trang
             total_pages = 0
             if total_items > 0: total_pages = ceil(total_items / PER_PAGE_LOGS)
             if page > total_pages and total_pages > 0: page = total_pages
             pagination = {
                 'page': page, 'per_page': PER_PAGE_LOGS, 'total_items': total_items,
                 'total_pages': total_pages, 'has_prev': page > 1, 'has_next': page < total_pages,
                 'prev_num': page - 1, 'next_num': page + 1,
                 'page_param': 'page'
             }
        ...
        current_app.logger.error(f"Lỗi nghiêm trọng load assignment logs {assignment_id}: {e}", exc_info=True)
        flash("Lỗi không mong muốn khi tải dữ liệu logs.", "error")
        logs, pagination = [], None
        # Nếu không lấy được assignment thì quay về list
        if assignment is None: return redirect(url_for('admin.view_task_assignments'))

    # Template này cần được tạo ở bước sau
    return render_template('admin_assignment_logs.html',
                           title=title,
                           assignment=assignment, # Truyền thông tin assignment
                           logs=logs, # Truyền list logs của trang hiện tại
                           pagination=pagination) 
