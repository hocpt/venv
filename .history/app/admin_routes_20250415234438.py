...
def view_ai_simulations():
    """Hiển thị trang quản lý mô phỏng AI với phân trang cho cấu hình đã lưu."""
    title = "Quản lý Mô phỏng AI"
    personas, strategies, accounts, saved_configs = [], [], [], []
    simulations_display_list = []
    saved_configs_pagination = None # <<< Pagination cho cấu hình đã lưu

    if not db: flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
    else:
        try:
            # Lấy page từ URL (dùng chung cho cả 2 list nếu muốn, hoặc dùng param khác)
            page = request.args.get('page', 1, type=int)
            if page < 1: page = 1
            print(f"DEBUG (view_ai_simulations): Requesting page {page}")

            # 1. Lấy dữ liệu cho dropdowns (như cũ)
            personas = db.get_all_personas() or []
            strategies = db.get_all_strategies() or []
            ...
            saved_configs, total_saved_configs = db.get_all_simulation_configs(
                page=page, per_page=PER_PAGE_SAVED_SIM_CONFIGS
            )
            if saved_configs is None or total_saved_configs is None:
                 ...
                 if total_saved_configs > 0:
                     ...
                     # Đảm bảo page hiện tại không vượt quá tổng số trang tính được
                     current_page_saved = page
                     if current_page_saved > total_pages_saved and total_pages_saved > 0: current_page_saved = total_pages_saved
                     saved_configs_pagination = {
                        'page': current_page_saved, 'per_page': PER_PAGE_SAVED_SIM_CONFIGS, 'total_items': total_saved_configs,
                        'total_pages': total_pages_saved, 'has_prev': current_page_saved > 1, 'has_next': current_page_saved < total_pages_saved,
                        'prev_num': current_page_saved - 1 if current_page_saved > 1 else None,
                        'next_num': current_page_saved + 1 if current_page_saved < total_pages_saved else None
                     }
                 else:
                     saved_configs_pagination = {'page': 1, 'per_page': PER_PAGE_SAVED_SIM_CONFIGS, 'total_items': 0, 'total_pages': 0, 'has_prev': False, 'has_next': False}
                 print(f"DEBUG (view_ai_simulations): Calculated Saved Configs pagination = {saved_configs_pagination}")


            # 3. Lấy danh sách Live/Pending Runs (giữ nguyên logic như cũ)
            # ... (Code lấy recent_commands, live_sim_jobs_dict, tạo simulations_display_list như cũ) ...
            server_tz = _get_configured_timezone()
            recent_commands = db.get_recent_simulation_commands(...) or []
            ...
            # ... (Vòng lặp xử lý live jobs và commands để tạo simulations_display_list) ...
