...
def add_simulation_config_view(): # <<< Sử dụng đúng tên hàm
    ...
    if not db:
        ...
        try:
            ...
            # <<< SỬA LẠI: Mở tuple trả về từ hàm DB >>>
            # Giả sử không cần phân trang cho dropdown này, lấy trang 1 và số lượng lớn
            personas_list_from_db, _ = db.get_all_personas(page=1, per_page=10000) # Lấy list và bỏ qua total_items
            if personas_list_from_db is None: flash("Lỗi tải danh sách Personas.", "warning"); all_personas = []
            elif isinstance(personas_list_from_db, list):
                try:
                    # Sắp xếp cái list đã lấy ra
                    all_personas = sorted(personas_list_from_db, key=lambda x: x.get('name', '').lower())
                except Exception as sort_err:
                     logger.error(f"Error sorting personas: {sort_err}", exc_info=True)
                     all_personas = personas_list_from_db # Dùng list chưa sort nếu lỗi
            else:
                 all_personas = []

            # 2. Lấy tất cả Accounts
            # <<< SỬA LẠI: Mở tuple trả về từ hàm DB >>>
            accounts_list_from_db, _ = db.get_all_accounts(page=1, per_page=10000) # Lấy list và bỏ qua total_items
            if accounts_list_from_db is None: flash("Lỗi tải danh sách Accounts.", "warning"); all_accounts = []
            elif isinstance(accounts_list_from_db, list):
                 try:
                     # Sắp xếp cái list đã lấy ra
                     all_accounts = sorted(accounts_list_from_db, key=lambda x: x.get('username', x.get('account_id', '')).lower())
                 except Exception as sort_err:
                      logger.error(f"Error sorting accounts: {sort_err}", exc_info=True)
                      all_accounts = accounts_list_from_db
            else:
                  all_accounts = []

            # 3. Lấy tất cả Language Strategies và sắp xếp (Cách này đã đúng vì get_all_strategies trả về list)
            raw_lang_strategies = db.get_all_strategies(strategy_type_filter='language')
            if raw_lang_strategies is None: flash("Lỗi tải danh sách Language Strategies.", "error"); all_language_strategies = []
            elif isinstance(raw_lang_strategies, list):
                 try: all_language_strategies = sorted(raw_lang_strategies, key=lambda x: x.get('strategy_id', ''))
                 except: all_language_strategies = raw_lang_strategies
            else: all_language_strategies = []

        except Exception as e:
            logger.error(f"Lỗi tải dữ liệu cho form add_simulation_config_view: {e}", exc_info=True)
            flash("Lỗi không mong muốn khi tải dữ liệu cho form.", "error")
            all_personas, all_accounts, all_language_strategies = [], [], [] # Reset về rỗng

    # --- Xử lý POST (Logic POST giữ nguyên như model_59) ---
    if request.method == 'POST':
        # ... (Toàn bộ logic xử lý POST như cũ) ...
        # Khi render lại form lỗi, truyền các list ĐÃ SẮP XẾP
        ...
        try:
            ...
            # ... (gọi hàm db.add_simulation_config) ...
