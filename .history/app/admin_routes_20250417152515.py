...
@admin_bp.route('/strategies/<strategy_id>/stages-control') # <<< Đổi URL
def view_strategy_stages_control(strategy_id): # <<< Đổi tên hàm
    """Hiển thị stages và control transitions cho một Control Strategy."""
    strategy = None
    strategy_stages_list = []
    transitions_list = []
    all_macros = [] # Cần cho nút Add Transition

    try:
        strategy = db.get_strategy_details(strategy_id)
        if strategy is None or strategy.get('strategy_type') != 'control': # <<< Kiểm tra type
            flash(f"Không tìm thấy Control Strategy ID {strategy_id}.", "warning")
            return redirect(url_for('admin.view_strategies_control')) # Redirect về list control

        strategy_stages_list = db.get_stages_for_strategy(strategy_id) or []
        # Hàm này lấy cả language và control transitions, template sẽ lọc hiển thị
        transitions_list = db.get_strategy_action_sequence(strategy_id) or [] # Dùng hàm lấy sequence để có action/condition
        all_macros = db.get_all_macro_definitions() or [] # Lấy macros cho nút Add

    except Exception as e:
        print(f"Lỗi tải stages/transitions cho control strategy {strategy_id}: {e}")
        flash("Lỗi tải chi tiết chiến lược điều khiển.", "error")

    # <<< Sửa tên template render >>>
    return render_template('admin_strategy_stages_control.html',
...
def view_strategy_stages_language(strategy_id):
    ...
    try:
        strategy = db.get_strategy_details(strategy_id)
        if strategy is None or strategy.get('strategy_type') != 'language': # <<< Kiểm tra type
            flash(f"Không tìm thấy Language Strategy ID {strategy_id}.", "warning")
...
def add_strategy():
    ...
    # ... (logic lấy stages như cũ) ...
    stages = []
    try: stages = db.get_all_stages() or []
    except Exception: flash("Lỗi tải stages", "error")

    if request.method == 'POST':
        # ... (logic lấy dữ liệu form và validate như cũ, bao gồm cả strategy_type) ...
        ...
        if not strategy_id or not name or not initial_stage_id or not strategy_type or strategy_type not in ['language', 'control']:
            flash("Dữ liệu nhập không hợp lệ.", "warning")
            return render_template('admin_add_strategy.html', title=title, stages=stages,
                                   valid_types=['language', 'control'], current_data=request.form), 400
        ...
        if success:
            ...
            return render_template('admin_add_strategy.html', title=title, stages=stages,
                                   valid_types=['language', 'control'], current_data=request.form)

    # GET request
    # Lấy type từ query param để chọn sẵn trên form
    default_type = request.args.get('type', 'language') # Mặc định là language nếu không có param type
    if default_type not in ['language', 'control']: default_type = 'language' # Đảm bảo giá trị hợp lệ

    return render_template('admin_add_strategy.html', title=title, stages=stages,
                           valid_types=['language', 'control'], default_type=default_type) # Truyền default_type

# --- Sửa Route Sửa Strategy ---
@admin_bp.route('/strategies/<strategy_id>/edit', methods=['GET', 'POST'])
def edit_strategy(strategy_id):
    # ... (logic GET lấy strategy details và stages như cũ) ...
    strategy = db.get_strategy_details(strategy_id) # Hàm này đã lấy cả strategy_type
    if not strategy: flash(f"Không tìm thấy strategy ID {strategy_id}.", "error"); return redirect(url_for('admin.view_strategies_language')) # Tạm về language list
    stages = []
    try: stages = db.get_all_stages() or []
    except Exception: flash("Lỗi tải stages", "error")

    # Xác định type để redirect sau này và có thể để check quyền (nếu cần)
    ...
    if request.method == 'POST':
        ...
        if not name or not initial_stage_id or not strategy_type or strategy_type not in ['language', 'control']:
             flash("Dữ liệu nhập không hợp lệ.", "warning")
             return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                                    strategy=strategy, stages=stages,
                                    valid_types=['language', 'control'], current_data=request.form), 400

        success, error_msg = db.update_strategy(strategy_id, name, description, initial_stage_id, strategy_type)
        if success:
            ...
            flash(f'Cập nhật chiến lược thất bại: {error_msg or "Lỗi không xác định."}', 'error')
            return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                                   strategy=strategy, stages=stages,
                                   valid_types=['language', 'control'], current_data=request.form)
    ...
    return render_template('admin_edit_strategy.html', title=f"Sửa Chiến lược {strategy_id}",
                           strategy=strategy, stages=stages,
                           valid_types=['language', 'control']) # Truyền loại hợp lệ
