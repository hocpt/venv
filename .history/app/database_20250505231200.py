...
def get_element_classifications_for_screen(screen_id: str) -> dict[str, str] | None:
    """
    Lấy tất cả các phân loại đã lưu cho một màn hình.

    Args:
        screen_id: ID của màn hình.

    Returns:
        Dictionary dạng {element_id: classification} hoặc None nếu lỗi.
        Trả về dict rỗng nếu không có phân loại nào được lưu cho màn hình này.
    """
    logger = current_app.logger if current_app else print
    if not screen_id: return {} # Trả về dict rỗng nếu không có screen_id

    classifications = None # Khởi tạo là None để phân biệt lỗi và không có dữ liệu
    conn = get_db_connection()
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        sql = """
            SELECT element_id, classification
            FROM public.element_classifications
            WHERE screen_id = %s;
        """
        cur.execute(sql, (screen_id,))
        rows = cur.fetchall()
        # Chuyển kết quả thành dict {element_id: classification}
        classifications = {row['element_id']: row['classification'] for row in rows} if rows else {}
        logger.debug(f"Fetched {len(classifications)} classifications for screen {screen_id}")

    except psycopg2.Error as db_err:
        logger.error(f"DB Error fetching classifications for screen {screen_id}: {db_err}", exc_info=True)
        classifications = None # Lỗi DB
    except Exception as e:
        logger.error(f"Unexpected error fetching classifications for screen {screen_id}: {e}", exc_info=True)
        classifications = None # Lỗi khác
    finally:
        if cur: cur.close()
        if conn: conn.close()
