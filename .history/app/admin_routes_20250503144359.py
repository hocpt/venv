...
def api_update_task_status(assignment_id):
    ...
    try:
        # !!! Sử dụng hàm cập nhật status trong database.py !!!
        # success = database.update_task_mapping_status(assignment_id, new_status)
        success = True # Placeholder, thay bằng lệnh gọi DB thật
        if success:
             return jsonify({"success": True, "message": f"Task assignment {assignment_id} status set to {new_status}"})
        else:
