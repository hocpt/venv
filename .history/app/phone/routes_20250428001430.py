...
def report_status():
    ...
    logger.info(f"Processing status report for Assignment: {assignment_id}, Device: {device_id}, Account: {account_id}")
    ...
    try:
        ...
        if phone_controller and hasattr(phone_controller, 'process_phone_report'):
            ...
            _, status_code = phone_controller.process_phone_report( # Không cần lưu response_body ở đây nữa
                device_id=device_id,
                account_id=account_id,
                assignment_id=assignment_id, # Truyền giá trị có thể là None
