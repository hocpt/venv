...
def process_raw_ui_state(reported_ui_state: dict | None) -> dict | None:
    ...
    # class_names không còn được dùng để tính min_len hoặc tạo ID chính
    ...
    min_len = 0
    # Tính min_len chỉ dựa trên các list thực sự được gửi và cần thiết cho việc lặp
    # Giả sử ids, texts, coords luôn phải có cùng độ dài nếu client gửi đúng
    ...
    if min_len == 0:
        logger.warning(f"  min_len is 0. No elements will be iterated and processed.")
    ...
    for i in range(min_len): 
        ...
        raw_text = texts[i]
        # raw_class_name sẽ không được dùng để tạo ID chính nữa

        if raw_resource_id and str(raw_resource_id).strip():
            element_id_val = str(raw_resource_id).strip()
            identifier_type_val = 'resource-id'
        elif raw_text and str(raw_text).strip(): 
            # Nếu không có resource-id, dùng text làm element_id
            # Lưu ý: Text có thể không duy nhất. Điều này có thể chấp nhận được nếu PIE của bạn
            # sẽ kết hợp text với các yếu tố khác (do admin định nghĩa) hoặc bạn chấp nhận rủi ro.
            element_id_val = str(raw_text).strip() 
            identifier_type_val = 'text' 
        # Bỏ qua việc tạo ID từ class_name hoặc index nếu không có resource-id hoặc text
        # Điều này có nghĩa là các element không có resource-id hoặc text sẽ không có 'element_id'
        # và có thể không được dùng để khớp PIE (tùy thuộc vào logic determine_screen_id_by_defined_pie)

        if element_id_val: # Chỉ xử lý nếu có ID (từ resource-id hoặc text)
            element_entry = {
                'element_id': element_id_val,
                'identifier_type': identifier_type_val,
                # 'element_type' vẫn có thể lấy từ client nếu client gửi (dù không dùng để tạo ID chính)
                'element_type': reported_ui_state.get('class_names', [])[i] if i < len(reported_ui_state.get('class_names', [])) and reported_ui_state.get('class_names') else None,
                'text_content': raw_text if raw_text and str(raw_text).strip() else None,
            }

            current_coord_str = coords_str_list[i]
            ...
            # Chỉ thêm nếu element_id_val thực sự được tạo ra
            processed_state["elements"].append(element_entry_clean)
            processed_elements_count +=1

    logger.info(f"process_raw_ui_state: Finished processing. Added {processed_elements_count} elements to processed_state.")
