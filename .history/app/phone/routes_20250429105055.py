...
def get_next_exploration_action():
    """
    Client gửi screenId hiện tại, server quyết định hành động tiếp theo để khám phá.
    """
    logger = current_app.logger if current_app else print
    data = request.get_json()

    if not data or not isinstance(data, dict):
        logger.error("Get Next Action: Invalid payload.")
        return jsonify({"status": "error", "message": "Invalid JSON payload."}), 400

    device_id = data.get('device_id')
    account_id = data.get('account_id')
    # Client gửi ID màn hình nó đang đứng (ID này do server trả về ở lần trước)
    current_screen_id = data.get('current_screen_id')

    if not device_id or not account_id or not current_screen_id:
        logger.error(f"Get Next Action: Missing device_id, account_id, or current_screen_id. Received: {data}")
        return jsonify({"status": "error", "message": "Missing 'device_id', 'account_id', or 'current_screen_id'."}), 400

    logger.info(f"Get Next Action request from Device: {device_id}, Account: {account_id}, Screen: {current_screen_id}")

    # --- Logic quyết định hành động tiếp theo ---
    next_action = None
    error_message = None
    confirmed_screen_id = current_screen_id # Tạm thời tin tưởng client, sẽ cải tiến sau

    try:
        # **TODO:** Triển khai logic phức tạp hơn ở đây, có thể gọi AI Planner
        # Tạm thời: Gọi một hàm helper trong controller để quyết định hành động đơn giản
        if phone_controller and hasattr(phone_controller, 'plan_simple_exploration_action'):
            next_action = phone_controller.plan_simple_exploration_action(
                app_name=data.get('app_name', db.get_app_name_from_account(account_id)), # <<< Cần lấy app_name
                current_screen_id=confirmed_screen_id
            )
        else:
            error_message = "Planning function not available."
            logger.error(error_message)

        if next_action:
            logger.info(f"Planned next action for screen {confirmed_screen_id}: {next_action.get('actionType')}")
            return jsonify({
                "status": "success",
                "confirmedCurrentScreenId": confirmed_screen_id, # <<< Trả về ID đã dùng để lên kế hoạch
                "nextAction": next_action
            }), 200
        else:
            # Không tìm thấy hành động nào để làm (hoặc lỗi planner)
            logger.warning(f"No next exploration action planned for screen {confirmed_screen_id}.")
            return jsonify({
                "status": "no_action",
                "confirmedCurrentScreenId": confirmed_screen_id,
                "message": "No further actions planned from this screen currently."
            }), 200 # Vẫn trả về 200 OK

    except Exception as e:
        logger.error(f"Error processing /get_next_exploration_action for screen {current_screen_id}: {e}", exc_info=True)
        return jsonify({"status": "error", "message": f"Internal server error planning next action: {e}"}), 500
