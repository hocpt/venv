...
def plan_simple_exploration_action(app_name: str, current_screen_id: str,
    ...
    không hiệu quả trên cùng một trạng thái màn hình.
    Chỉ xem xét click các element có resource_id.
    """
    logger = current_app.logger if current_app else print
    logger.info(f"Planning action for app '{app_name}', screen '{current_screen_id}'")
    if previous_action_context: logger.debug(f"Previous action context: {previous_action_context}")

    if not graph_db or not app_name or not current_screen_id:
        logger.error("Planner: Missing graph_db module, app_name, or screen_id.")
        return None
    ...
    try:
        # 1. Lấy thông tin màn hình và các transition đã thử
        ...
        if not screen_details:
            logger.warning(f"Planner: Could not find screen details in Neo4j for {current_screen_id}. Cannot plan.")
            return None
        if outgoing_transitions is None:
             logger.error(f"Planner: Failed to get outgoing transitions for {current_screen_id}. Aborting.")
             return None
        ...
        # 2. Kiểm tra xem có phải vừa bị lặp không và lấy thông tin action trước đó
        is_loop = False
        ...
        if previous_action_context:
            ...
            if source_context and action_details_from_prev:
                ...
                if previous_source_id == current_screen_id:
                    is_loop = True
                    previous_action_details = action_details_from_prev
                    # Nếu hành động trước là click, lấy target ID của nó
                    if previous_action_details.get('actionType') == 'click':
                        previous_target_id = previous_action_details.get('onElementId')
                    logger.info(f"Planner: Detected loop on screen {current_screen_id}. Previous action: {previous_action_details}. Previous target ID: {previous_target_id}")

        # 3. Tìm hành động click mới (ưu tiên element có resource_id)
        found_click_action = False
        if screen_elements:
            for element in screen_elements:
                element_id = element.get('resource_id')
                # Heuristic: Chỉ xem xét click nếu có resource_id
                if not element_id:
                    continue

                # Kiểm tra xem đã click vào element này từ screen này chưa
                click_tried = False
                for trans in outgoing_transitions:
                    # Chỉ kiểm tra transition click khớp với ID element này
                ...
                if not click_tried:
                    ...
                    # Nếu đang bị lặp (is_loop=True) VÀ hành động lặp trước đó là click
                    # VÀ element hiện tại chính là element gây lặp -> BỎ QUA
                    if is_loop and previous_action_details and \
                       previous_action_details.get('actionType') == 'click' and \
                       previous_target_id == element_id:
                        logger.warning(f"Planner: Skipping element ID {element_id} because clicking it JUST caused the current loop.")
                        continue # <<< Bỏ qua element này, xét element tiếp theo
                    # ===================================

                    # Nếu không bị lặp hoặc đây là element khác element gây lặp -> Đề xuất click
                    element_text = element.get('text')
                    element_class = element.get('class_name') # Vẫn lấy class nếu có trong state
                    ...
                    target_element_data = {
                        "resource_id": element_id, "text": element_text,
                        "class_name": element_class, "coordinates": element_coords
                    }
                    target_element_data = {k:v for k,v in target_element_data.items() if v is not None}

                    if target_element_data:
                        next_action = {
                            "actionType": "run_macro", "macro_code": "UI_CLICK",
                            "params": { "target": target_element_data }
                        }
                        if ADD_RANDOM_DELAY: next_action["random_delay_ms"] = { "min": MIN_DELAY_MS, "max": MAX_DELAY_MS }
                        if ADD_RANDOM_OFFSET and element_coords: next_action["random_offset_xy"] = { "x_max": MAX_OFFSET_X, "y_max": MAX_OFFSET_Y }
                        found_click_action = True
                        break # Dừng khi tìm thấy click mới
                    else:
                        logger.warning("Planner: Generated empty targetElement, skipping.")


        # 4. Nếu không tìm thấy click mới -> Xử lý fallback (tránh lặp)
        if not found_click_action:
            # Kiểm tra các hành động fallback đã thử TỪ node này chưa
            ...
            if not swipe_up_tried:
                logger.info(f"Planner: No new clicks found. Suggesting SWIPE_UP (untried).")
                next_action = { "actionType": "run_macro", "macro_code": "UI_SWIPE_UP", "params": {} }
                ...
                next_action = { "actionType": "run_macro", "macro_code": "NAV_GO_BACK", "params": {} }
            else:
                logger.warning(f"Planner: No new actions (clicks, swipe, back) found or applicable for screen {current_screen_id}. Returning no_action.")
                next_action = None # Trả về None -> route sẽ trả status: "no_action"

    except Exception as e:
        logger.error(f"Planner: Error during planning for screen {current_screen_id}: {e}", exc_info=True)
        next_action = None

    if next_action: logger.debug(f"Planner: Final planned nextAction: {next_action}")
    else: logger.warning("Planner: Returning None (no suitable action or error occurred).")
...
def handle_explore_step(device_id: str, account_id: str,
                        current_screen_id_from_client: str | None, # Client gửi lên
                        raw_ui_state: dict, # State UI thô mới nhất
                        previous_action: dict | None, # Context action trước đó
                        **kwargs # Các tham số tùy chọn khác
                        ) -> tuple[dict, int]:
    """
    Xử lý đồng bộ một bước khám phá: process state, log PG, update Neo4j, plan next action.
    Hàm này được gọi bởi route /phone/explore_step.
    """
    logger = current_app.logger if current_app else print
    # Giá trị mặc định cho response
    final_status = "error"
    final_message = "Unknown error occurred during explore step."
    confirmed_screen_id = None # ID server tính toán được cho state hiện tại
    next_action = None
    http_status_code = 500
    app_name = None # Khởi tạo app_name

    try:
        # --- Bước 1: Xử lý Raw UI State ---
        logger.debug(f"[Explore Step] Processing raw UI state for device {device_id}...")
        # Gọi hàm xử lý state (phiên bản mới nhất xử lý cả clickables nếu client gửi)
        processed_ui_state = process_raw_ui_state(raw_ui_state)
        if not processed_ui_state:
            # Log lỗi đã có trong process_raw_ui_state
            raise ValueError("Failed to process raw UI state. Check input data format.")

        # --- Bước 2: Xác định Target Screen ID (Dùng hàm helper thống nhất) ---
        logger.debug(f"[Explore Step] Determining target screen ID...")
        # Hàm này phải được định nghĩa trong cùng file hoặc import đúng
        target_screen_id = determine_screen_id_from_state(processed_ui_state)
        if not target_screen_id or target_screen_id.startswith("error_"):
            raise ValueError(f"Failed to determine a valid target screen ID (Result: {target_screen_id}). Ensure UI state has package_name.")
        confirmed_screen_id = target_screen_id # Gán ID vừa tính được
        logger.info(f"[Explore Step] Determined Target Screen ID: {confirmed_screen_id}")

        # --- Bước 3: Lấy App Name ---
        # Ưu tiên lấy từ state đã xử lý
        app_name = processed_ui_state.get('package_name')
        if not app_name:
            logger.warning("[Explore Step] Missing package_name in processed UI state, trying DB lookup...")
            if db and hasattr(db, 'get_app_name_from_account'):
                app_name = db.get_app_name_from_account(account_id)
            else:
                 logger.error("[Explore Step] Cannot lookup app_name: DB module or function missing.")
        if not app_name:
             # Nếu vẫn không có app_name thì không thể tiếp tục
             raise ValueError("Cannot determine app_name from UI state or database.")
        logger.debug(f"[Explore Step] Using App Name: {app_name}")

        # --- Bước 4: Ghi Log vào PostgreSQL (Chỉ ghi state, action đã được ghi ở bước trước) ---
        logger.debug(f"[Explore Step] Logging current state to PostgreSQL...")
        ui_state_for_log_saving = None
        try:
            ui_state_for_log_saving = json.dumps(processed_ui_state, ensure_ascii=False, default=str) # Thêm default=str phòng lỗi non-serializable
        except Exception as json_err:
            logger.error(f"Error serializing processed UI state for PG log: {json_err}", exc_info=True)
            ui_state_for_log_saving = json.dumps({"error": "Failed to serialize UI state", "details": str(json_err)})

        # Tạo một log entry "placeholder" chỉ để ghi state này
        # assignment_id là None cho mapping
        placeholder_log_entry = {
            "timestamp": processed_ui_state.get("timestamp", datetime.now(timezone.utc).isoformat()),
            "macro": "SYSTEM_EXPLORE_STATE_RECEIVED", # Mã đặc biệt
            "status": "info"
            # Có thể thêm current_screen_id_from_client nếu muốn log lại
        }
        if db and hasattr(db, 'add_phone_action_logs'):
            log_pg_success = db.add_phone_action_logs(
                assignment_id=None,
                device_id=device_id,
                account_id=account_id,
                logs=[placeholder_log_entry], # Chỉ ghi log này
                structured_ui_state_json=ui_state_for_log_saving # Gắn state vào log này
            )
            if not log_pg_success:
                logger.warning("[Explore Step] Failed to write current state log to PostgreSQL. Continuing...")
        else:
             logger.error("[Explore Step] Cannot log to PostgreSQL: DB module or function missing.")

        # --- Bước 5: Cập nhật Neo4j Đồng bộ ---
        logger.info(f"[Explore Step] Starting synchronous Neo4j update for screen: {confirmed_screen_id}...")
        neo4j_update_success = False
        node_success = False
        edge_success = True # Mặc định true nếu không có previous_action

        # 5a. Cập nhật Node Đích
        if graph_db and hasattr(graph_db, 'create_or_update_screen_node'):
            screen_props = {
                "activityName": processed_ui_state.get('activity_name'),
                "structureHash": confirmed_screen_id,
                "elementCount": len(processed_ui_state.get('elements', [])),
                "rawStateSample": ui_state_for_log_saving # Dùng lại chuỗi JSON đã tạo
            }
            node_success = graph_db.create_or_update_screen_node(confirmed_screen_id, app_name, screen_props)
            if not node_success:
                logger.error(f"[Explore Step] Neo4j Error: Failed to create/update target screen node {confirmed_screen_id}")
                # Không ném lỗi ngay, thử tạo cạnh nếu node nguồn có thể tồn tại
        else:
             logger.error("[Explore Step] Cannot update Neo4j node: graph_db module or function missing.")
             raise ConnectionError("Neo4j node update function unavailable.") # Lỗi nghiêm trọng

        # 5b. Cập nhật Cạnh Transition (Nếu có previous_action VÀ node đích được tạo/update)
        if node_success and previous_action and isinstance(previous_action, dict):
            ...
            if source_context and isinstance(source_context, dict) and action_details and isinstance(action_details, dict):
                ...
                if source_screen_id:
                    logger.info(f"[Explore Step] Updating Neo4j Transition: {source_screen_id} -> {confirmed_screen_id} via {action_details.get('actionType')}...")
                    if graph_db and hasattr(graph_db, 'create_or_update_transition_relationship'):
                        # Gọi hàm tạo/cập nhật cạnh (đã sửa logic MERGE)
                        edge_success = graph_db.create_or_update_transition_relationship(source_screen_id, confirmed_screen_id, app_name, action_details)
                        if not edge_success:
                            logger.error(f"[Explore Step] Neo4j Error: Failed to create/update transition edge from {source_screen_id} to {confirmed_screen_id}")
                            # Vẫn có thể tiếp tục gọi planner dù tạo cạnh lỗi
                    else:
                         logger.error("[Explore Step] Cannot update Neo4j edge: graph_db module or function missing.")
                         edge_success = False # Coi như lỗi nếu không có hàm
                else:
                    logger.warning("[Explore Step] previous_action missing source_screen_id. Cannot create transition.")
                    edge_success = False # Không thể tạo cạnh nếu thiếu nguồn
            else:
                 logger.warning("[Explore Step] previous_action has invalid structure. Cannot create transition.")
                 edge_success = False # Không thể tạo cạnh

        # Chỉ coi là thành công hoàn toàn nếu cả node và cạnh (nếu cần) đều OK
        neo4j_update_success = node_success and edge_success
        if neo4j_update_success:
             logger.info(f"[Explore Step] Synchronous Neo4j update completed successfully for screen {confirmed_screen_id}.")
        else:
             logger.warning(f"[Explore Step] Synchronous Neo4j update potentially incomplete for screen {confirmed_screen_id} (NodeOK: {node_success}, EdgeOK: {edge_success}).")
             # Vẫn tiếp tục gọi planner

        # --- Bước 6: Gọi Planner ---
        logger.debug(f"[Explore Step] Calling planner for confirmed screen: {confirmed_screen_id}...")
        # Hàm planner giờ nhận cả previous_action để tránh lặp
        next_action = plan_simple_exploration_action(
            app_name=app_name,
            current_screen_id=confirmed_screen_id,
            previous_action_context=previous_action # Truyền context vào
        )

        # --- Bước 7: Xác định Status và Message cuối cùng ---
        if next_action:
            final_status = "success"
            macro_code = next_action.get('macro_code', 'unknown action')
            final_message = f"Next action '{macro_code}' planned."
            http_status_code = 200
        else:
            final_status = "no_action"
            final_message = "No further actions planned from this screen currently, or planner error."
            http_status_code = 200

    except ValueError as ve: # Lỗi validate dữ liệu hoặc xử lý state/ID/app_name
        logger.error(f"[Explore Step] Value Error: {ve}", exc_info=False)
        final_status = "error"
        final_message = str(ve)
        http_status_code = 400
    except ConnectionError as ce: # Lỗi khi cập nhật Neo4j
        logger.error(f"[Explore Step] Neo4j Update Error: {ce}", exc_info=False)
        final_status = "error"
        final_message = f"Failed to update graph database: {ce}"
        http_status_code = 503 # Service unavailable
    except Exception as e: # Các lỗi không mong muốn khác
        logger.error(f"[Explore Step] Unexpected Error: {e}", exc_info=True)
        traceback.print_exc()
        final_status = "error"
        final_message = f"Internal server error during explore step processing: {type(e).__name__}"
        http_status_code = 500

    # --- Bước 8: Tạo và Trả về Response ---
    response_body = {
        "status": final_status,
        "message": final_message,
        "confirmedCurrentScreenId": confirmed_screen_id, # ID server tính được
        "nextAction": next_action # Object hành động hoặc None
    }
    logger.info(f"[Explore Step] Responding with status='{final_status}', screenId='{confirmed_screen_id}', nextMacro='{next_action.get('macro_code') if next_action else 'None'}'")
