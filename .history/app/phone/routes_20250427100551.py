...
def report_status():
    """
    Client gửi báo cáo tiến độ, log, UI state, và CONTEXT HÀNH ĐỘNG TRƯỚC ĐÓ.
    ...
    # --- Extract và Validate dữ liệu cơ bản (giữ nguyên logic cũ) ---
    device_id = data.get('device_id')
    account_id = data.get('account_id')
    assignment_id_str = data.get('assignment_id')
    status_report_payload = data.get('status_report')
    logs_payload = data.get('logs', []) # Lấy logs (có thể rỗng)

    # === TRÍCH XUẤT DỮ LIỆU MỚI CHO MAP BUILDING ===
    raw_ui_state = data.get('current_ui_state') # Dữ liệu UI thô từ client
    previous_action_context = data.get('previous_action') # CONTEXT HÀNH ĐỘNG TRƯỚC ĐÓ
    # =============================================
    ...
    if not assignment_id_str: errors.append("Missing 'assignment_id'.")
    else:
        try: assignment_id = int(assignment_id_str)
        except (ValueError, TypeError): errors.append("Invalid 'assignment_id' format.")
    if not status_report_payload or not isinstance(status_report_payload, dict): errors.append("Missing or invalid 'status_report'.")
    ...
    # === XỬ LÝ VÀ ĐƯA JOB XÂY DỰNG BẢN ĐỒ VÀO QUEUE ===
    ...
    if raw_ui_state and isinstance(raw_ui_state, dict):
        try:
            ...
            if not processed_ui_state_for_map:
             ...
             # Vẫn tiếp tục xử lý phần còn lại của report, nhưng không có map update

    # Chỉ đưa job vào queue nếu có dữ liệu UI đã xử lý thành công
    if processed_ui_state_for_map:
        map_job_payload = {
            'device_id': device_id,
            'account_id': account_id,
            'assignment_id': assignment_id, # Có thể cần assignment_id trong task
            'processed_ui_state': processed_ui_state_for_map,
            'previous_action': previous_action_context # Có thể là None ban đầu
        }
        try:
            # Dùng hàm add_scheduler_command để đưa vào queue
            # Cần định nghĩa một command_type mới, ví dụ 'build_map'
    ...
    # === XỬ LÝ PHẦN CÒN LẠI CỦA REPORT (Cập nhật status, ghi log PostgreSQL...) ===
    # Gọi Controller để xử lý logic cập nhật status, progress, log vào PostgreSQL
    # Hàm process_phone_report cần được cập nhật để nhận và lưu structured_ui_state_json vào phone_action_log
    try:
        # Truyền cả UI state đã xử lý (dạng dict) vào controller nếu muốn nó xử lý/lưu luôn
        response_body, status_code = phone_controller.process_phone_report(
            device_id=device_id,
            account_id=account_id,
            assignment_id=assignment_id,
            report_payload=data # Truyền cả payload gốc
            # Thêm tham số processed_ui_state nếu hàm controller cần:
            # processed_ui_state_for_log=processed_ui_state_for_map
        )
        # Trả về phản hồi từ controller
        return jsonify(response_body), status_code

    except Exception as e:
        current_app.logger.error(f"Error processing /report_status (main logic) for assignment {assignment_id}: {e}", exc_info=True)
