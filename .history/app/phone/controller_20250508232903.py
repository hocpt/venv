...
def handle_explore_step(device_id: str, account_id: str,
    ...
    """
    Xử lý một bước khám phá: nhận state, xử lý, cập nhật graph, lên kế hoạch hành động tiếp theo.
    Đã cập nhật để trích xuất và truyền screen_width/height vào graph_db.merge_screen.
    """
    logger = current_app.logger if current_app else print
    logger.info(f"[Explore Step Ctrl] Handling step for device: {device_id}, account: {account_id}")
    logger.debug(f"[Explore Step Ctrl] Received screenshot_filename: '{screenshot_filename}'")
    ...
    next_action = None
    # <<< KHỞI TẠO BIẾN KÍCH THƯỚC >>>
    original_screen_width = None
    original_screen_height = None

    try:
        # 1. Xử lý State
        logger.debug("[Explore Step Ctrl] Processing raw UI state...")
        processed_ui_state = process_raw_ui_state(raw_ui_state) # Hàm này nên giữ nguyên cấu trúc JSON gốc
        ...
        # 3. Lấy App Name, Activity Name VÀ KÍCH THƯỚC MÀN HÌNH
        ...
        # <<< TRÍCH XUẤT KÍCH THƯỚC TỪ PROCESSED_UI_STATE >>>
        # Giả sử client gửi key là 'screen_width' và 'screen_height' trong JSON gốc
        # Hoặc bạn có thể lấy từ key khác nếu client gửi tên khác (vd: 'width', 'height')
        fetched_width = processed_ui_state.get('screen_width')  # <<< THAY KEY NẾU CẦN
        fetched_height = processed_ui_state.get('screen_height') # <<< THAY KEY NẾU CẦN

        # Chuyển đổi thành số nguyên, nếu không hợp lệ thì để None
        try: original_screen_width = int(fetched_width) if fetched_width is not None else None
        except (ValueError, TypeError): original_screen_width = None
        try: original_screen_height = int(fetched_height) if fetched_height is not None else None
        except (ValueError, TypeError): original_screen_height = None

        logger.debug(f"[Explore Step Ctrl] App: {app_name}, Activity: {activity_name}, W: {original_screen_width}, H: {original_screen_height}")
        # ==================================================

        # Lấy screenshot_filename (logic giữ nguyên)
        path_to_store_in_neo4j = screenshot_filename
        logger.debug(f"[Explore Step Ctrl] Path to store in Neo4j: '{path_to_store_in_neo4j}'")

        # 4. Ghi Log PostgreSQL (logic giữ nguyên)
        try:
        ...
        # 5a. Merge Node Đích (TRUYỀN THÊM KÍCH THƯỚC)
        logger.debug(f"[Explore Step Ctrl] Calling graph_db.merge_screen with W={original_screen_width}, H={original_screen_height}")
        node_success = graph_db.merge_screen(
            screen_id=confirmed_target_screen_id,
            app_name=app_name,
            activity_name=activity_name,
            extracted_elements=extracted_elements_for_db,
            log_id=None, # Log ID của PostgreSQL, có thể lấy nếu cần
            screenshot_path=path_to_store_in_neo4j,
            screen_width=original_screen_width,      # <<< TRUYỀN WIDTH
            screen_height=original_screen_height     # <<< TRUYỀN HEIGHT
        )
        logger.debug(f"[Explore Step Ctrl] Target node merge result: {node_success}")

        # 5b. Merge Cạnh Transition (logic giữ nguyên)
        if not node_success:
            logger.error(f"[Explore Step Ctrl] Failed to merge target screen node {confirmed_target_screen_id}.")
            # Cân nhắc có nên dừng lại hay tiếp tục nếu merge node lỗi
        else:
            # Chỉ merge cạnh nếu merge node thành công VÀ có hành động trước đó
            edge_success = True # Mặc định là True nếu không cần merge cạnh
            if previous_action and isinstance(previous_action, dict):
                 ...
                 result_status_prev = previous_action.get('result_status', 'success') # Lấy status của hành động trước
                 # Chỉ merge nếu hành động trước thành công và đến từ màn hình khác
                 if source_context and action_details and result_status_prev == 'success':
                     source_screen_id_prev = source_context.get('screenId')
                     if source_screen_id_prev and source_screen_id_prev != confirmed_target_screen_id:
                          # Gọi hàm merge_transition (đảm bảo hàm này hoạt động đúng)
                          edge_success = graph_db.merge_transition(
                              source_screen_id=source_screen_id_prev,
                              target_screen_id=confirmed_target_screen_id,
                              app_name=app_name,
                              action_details=action_details, # Dict chứa chi tiết hành động
                              result_status=result_status_prev, # Status của hành động đó
                              log_id=None # ID log PostgreSQL nếu cần liên kết
                          )
                          ...
                          logger.info(f"[Explore Step Ctrl] Previous action resulted in the same screen ({confirmed_target_screen_id}), skipping edge merge.")
                     else: logger.warning("[Explore Step Ctrl] Missing previous source screen ID, cannot merge edge.")
                 elif result_status_prev != 'success':
                      logger.info(f"[Explore Step Ctrl] Previous action status was '{result_status_prev}', skipping edge merge.")
                 else: logger.warning("[Explore Step Ctrl] Missing source context or action details in previous action, cannot merge edge.")

            # Ghi nhận kết quả cập nhật Neo4j cuối cùng
            neo4j_update_success = node_success and edge_success
            if neo4j_update_success: logger.info(f"[Explore Step Ctrl] Neo4j update completed for target {confirmed_target_screen_id}.")
            else: logger.warning(f"[Explore Step Ctrl] Neo4j update potentially incomplete for target {confirmed_target_screen_id} (NodeOK:{node_success}, EdgeOK:{edge_success}).")

        # 6. Gọi Planner (logic giữ nguyên)
        logger.debug(f"[Explore Step Ctrl] Calling planner for screen: {confirmed_target_screen_id}...")
        # Đảm bảo hàm planner được import và gọi đúng
        next_action = plan_intelligent_exploration_action( # Hoặc plan_simple_exploration_action
            current_screen_id=confirmed_target_screen_id,
            app_name=app_name,
            processed_ui_state=processed_ui_state,
            previous_action=previous_action
        )
        logger.debug(f"[Explore Step Ctrl] Planner returned: {next_action}")

        # 7. Xác định Kết quả Cuối cùng (logic giữ nguyên)
        ...
        # Lỗi do dữ liệu đầu vào hoặc xử lý (vd: không lấy được ID, app_name)
