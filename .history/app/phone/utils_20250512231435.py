...
def delete_screenshot_file_on_server(base_storage_path: str, app_name: str, filename: str) -> tuple[bool, str | None]:
    logger = current_app.logger if current_app else print
    if not base_storage_path or not app_name or not filename:
        return False, "Thiếu đường dẫn gốc, app_name hoặc filename."
    try:
        # Đường dẫn đầy đủ đến file, ví dụ: /path/to/screenshots/com.app.name/image.png
        # Điều chỉnh nếu cấu trúc thư mục của bạn khác
        file_path = os.path.join(base_storage_path, app_name, filename)
        ...
        logger.error(f"Lỗi khi xóa file {filename} cho app {app_name}: {e}", exc_info=True)
        return False, str(e)
