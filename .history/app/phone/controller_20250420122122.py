        ...
        for transition in action_sequence_raw:
            ...
            if action_to_suggest_data is not None:
                if isinstance(action_to_suggest_data, dict):
                    ...
                    current_app.logger.error(f"Invalid data type for action_to_suggest in transition ID {transition_id}. Expected dict, got {type(action_to_suggest_data)}. Skipping.")
            ...
            if not action_holder or not action_holder.get('macro_code'):
                 is_valid_step = False
                 current_app.logger.warning(f"{step.get('type', 'Unknown type')} transition (ID: {transition_id}) missing macro_code. Skipping step.")
