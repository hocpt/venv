...
def handle_explore_step(device_id: str, account_id: str,
    ...
    next_action = None
    original_screen_width = None # <<< Khởi tạo
    original_screen_height = None # <<< Khởi tạo

    try:
        # 1. Xử lý State UI thô
        logger.debug("[Explore Step Ctrl] Processing raw UI state...")
        # Hàm này nên trả về dict Python đã chuẩn hóa, giữ nguyên cấu trúc gốc nếu có thể
        ...
        # 2. Xác định Screen ID Đích từ state đã xử lý
        ...
        # 3. Lấy App Name, Activity Name VÀ KÍCH THƯỚC MÀN HÌNH từ state đã xử lý
        ...
        # *** ĐẢM BẢO CLIENT GỬI CÁC KEY NÀY TRONG raw_ui_state ***
        fetched_width = processed_ui_state.get('screen_width')  # <<< Tên key phải khớp với client gửi
        fetched_height = processed_ui_state.get('screen_height') # <<< Tên key phải khớp với client gửi
        ...
        # Xử lý screenshot filename (logic giữ nguyên)
        ...
        # ... (code gọi db.add_phone_action_logs) ...
        try:
        ...
        # Lấy list elements đã được chuẩn hóa từ processed_ui_state
        extracted_elements_for_db = processed_ui_state.get('elements', [])

        # 5a. Merge Node Đích (TRUYỀN THÊM KÍCH THƯỚC ĐÃ TRÍCH XUẤT)
        ...
        node_success = graph_db.merge_screen(
            ...
            log_id=None, # Hoặc ID log PostgreSQL nếu bạn muốn liên kết
            screenshot_path=path_to_store_in_neo4j,
            screen_width=original_screen_width,      # <<< TRUYỀN WIDTH ĐÃ LẤY
            screen_height=original_screen_height     # <<< TRUYỀN HEIGHT ĐÃ LẤY
        )
