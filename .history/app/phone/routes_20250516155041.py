...
@phone_bp.route('/api/upload/screenshot', methods=['POST']) # Đảm bảo client gọi URL này
def upload_screenshot_with_server_uuid_name(): # Đổi tên hàm để phân biệt
    logger = current_app.logger
    logger.info("PHONE_UPLOAD_API_SERVER_UUID: Received new screenshot upload request.")
    
    logger.debug(f"PHONE_UPLOAD_API_SERVER_UUID: Request Headers: {request.headers}")
    logger.debug(f"PHONE_UPLOAD_API_SERVER_UUID: Request Form Data (request.form): {request.form.to_dict()}")
    logger.debug(f"PHONE_UPLOAD_API_SERVER_UUID: Request Files Keys (request.files.keys()): {list(request.files.keys())}")

    # 1. Lấy file từ request (client gửi với key 'screenshot_file')
    file_field_key_from_client = 'screenshot_file' 
    if file_field_key_from_client not in request.files:
        logger.warning(f"PHONE_UPLOAD_API_SERVER_UUID: No '{file_field_key_from_client}' part in request.files.")
    ...
    original_client_filename = uploaded_file_storage.filename # Ví dụ: "screenshot.png"

    # 2. Lấy app_name từ request.form (Client PHẢI gửi trường này)
    app_name_from_form = request.form.get('app_name')

    logger.debug(f"PHONE_UPLOAD_API_SERVER_UUID: Form data received - 'app_name': '{app_name_from_form}'")
    logger.debug(f"PHONE_UPLOAD_API_SERVER_UUID: Uploaded file object - original name on client's disk: '{original_client_filename}'")

    if not app_name_from_form:
        logger.warning("PHONE_UPLOAD_API_SERVER_UUID: Missing 'app_name' form parameter.")
        return jsonify({"success": False, "error": "Thiếu tham số 'app_name' trong dữ liệu form."}), 400
    
    if not original_client_filename: # Kiểm tra xem có file nào được chọn không
        logger.warning("PHONE_UPLOAD_API_SERVER_UUID: No file selected by client (original filename is empty).")
        return jsonify({"success": False, "error": "Không có file nào được chọn để upload."}), 400

    # (Tùy chọn) Kiểm tra phần mở rộng file
    if not allowed_file(original_client_filename):
        logger.warning(f"PHONE_UPLOAD_API_SERVER_UUID: File type not allowed for '{original_client_filename}'.")
        return jsonify({"success": False, "error": "Loại file không được phép."}), 400

    # 3. Tạo tên file duy nhất trên server bằng UUID
    file_extension = original_client_filename.rsplit('.', 1)[1].lower()
    unique_filename_for_server = f"{uuid.uuid4()}.{file_extension}" # Ví dụ: "47f7...e0.png"

    # An toàn hóa tên thư mục app_name
    safe_app_name_folder = secure_filename(app_name_from_form)
    if not safe_app_name_folder:
        logger.error(f"PHONE_UPLOAD_API_SERVER_UUID: Invalid app_name after sanitization: '{app_name_from_form}'")
        return jsonify({"success": False, "error": "Tên app không hợp lệ sau khi xử lý."}), 400
    ...
    if not storage_base_path:
        logger.critical("PHONE_UPLOAD_API_SERVER_UUID: SCREENSHOT_STORAGE_PATH is NOT CONFIGURED!")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (storage path missing)."}), 500
    
    logger.info(f"PHONE_UPLOAD_API_SERVER_UUID: Configured SCREENSHOT_STORAGE_PATH: '{storage_base_path}'")
    ...
    logger.info(f"PHONE_UPLOAD_API_SERVER_UUID: Target app-specific directory: '{app_specific_storage_directory}'")
    
    try:
        if not os.path.exists(app_specific_storage_directory):
            os.makedirs(app_specific_storage_directory, exist_ok=True)
            logger.info(f"PHONE_UPLOAD_API_SERVER_UUID: Created directory: {app_specific_storage_directory}")
    except OSError as e:
        logger.error(f"PHONE_UPLOAD_API_SERVER_UUID: Could not create directory '{app_specific_storage_directory}': {e}", exc_info=True)
        return jsonify({"success": False, "error": "Server error creating storage directory."}), 500
    
    # 6. Đường dẫn đầy đủ để lưu file ảnh (sử dụng unique_filename_for_server)
    full_path_to_save_file = os.path.join(app_specific_storage_directory, unique_filename_for_server) 
    logger.info(f"PHONE_UPLOAD_API_SERVER_UUID: Attempting to save uploaded file to: '{full_path_to_save_file}' (server-generated UUID name)")
    ...
    try:
        ...
        if os.path.exists(full_path_to_save_file) and os.path.getsize(full_path_to_save_file) > 0:
            logger.info(f"PHONE_UPLOAD_API_SERVER_UUID: SUCCESS - Image saved to: {full_path_to_save_file}")
            # TRẢ VỀ CHỈ TÊN FILE UUID ĐÃ TẠO CHO CLIENT
            return jsonify({
                "success": True, 
                "message": "Screenshot uploaded successfully.",
                "filename": unique_filename_for_server # <<< TRẢ VỀ TÊN FILE UUID NÀY
            }), 201
        else:
            logger.error(f"PHONE_UPLOAD_API_SERVER_UUID: FAILED - File not found or empty after save attempt at: {full_path_to_save_file}")
            return jsonify({"success": False, "error": "Server error: Could not confirm file save or file is empty."}), 500
    except Exception as e:
        logger.error(f"PHONE_UPLOAD_API_SERVER_UUID: EXCEPTION during save of file '{unique_filename_for_server}' to '{app_specific_storage_directory}': {e}", exc_info=True)
