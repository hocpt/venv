...
def handle_explore_step(device_id: str, account_id: str,
    ...
    try:
        ...
        # 2. Xác định Screen ID chuẩn hóa
        ...
        logger.info(f"[Explore Step Ctrl] Confirmed Target Screen ID: {confirmed_screen_id}")
        ...
        activity_name = processed_ui_state.get('activity_name') # Lấy activity name đã xử lý

        # 4. Ghi Log PostgreSQL (Giữ nguyên)
        # ... (logic ghi log exploration_logs) ...
        ...
        # 5. Cập nhật Neo4j Đồng bộ (Dùng hàm đã sửa và tên chuẩn)
        logger.info(f"[Explore Step Ctrl] Sync Neo4j update for screen: {confirmed_screen_id}...")
        extracted_elements_for_db = processed_ui_state.get('elements', []) # Lấy list elements đã chuẩn hóa
        # 5a. Merge Node Đích
        node_success = graph_db.merge_screen(
            screen_id=confirmed_screen_id,
            app_name=app_name,
            activity_name=activity_name,
            extracted_elements=extracted_elements_for_db, # Truyền list đã xử lý
            log_id=None # Log ID có thể không cần thiết ở đây nữa nếu log riêng
        )
        ...
        if node_success and previous_action and isinstance(previous_action, dict):
            ...
            result_status_prev = previous_action.get('result_status', 'unknown') # Lấy kết quả action trước
            if source_context and action_details:
                 source_screen_id_prev = source_context.get('screenId') # <<< Lấy ID nguồn từ context cũ
                 action_el_id = action_details.get('element_id') or action_details.get('onElementId') # Lấy ID element đã tương tác
                 action_id_type = action_details.get('identifier_type') # Lấy loại ID đã dùng
                 if source_screen_id_prev and action_el_id and action_id_type:
                      try:
                          # Gọi hàm cập nhật thống kê tương tác (đã tạo trong graph_db.py)
                          graph_db.update_element_interaction_stats(
                              screen_id=source_screen_id_prev, # Cập nhật trên node nguồn
                              element_id=action_el_id,
                              identifier_type=action_id_type,
                              success=(result_status_prev == 'success')
                          )
                      except Exception as e_stat: logger.error(f"Error updating element stats on source {source_screen_id_prev}: {e_stat}")

                 # 5c. Merge Cạnh Transition (nếu action trước thành công)
                 if result_status_prev == 'success' and source_screen_id_prev:
                     edge_success = graph_db.merge_transition(
                         source_screen_id=source_screen_id_prev, # <<< Dùng ID nguồn
                         target_screen_id=confirmed_screen_id,  # <<< ID đích hiện tại
                         app_name=app_name, # <<< Cần app_name
                         action_details=action_details, # <<< Chi tiết action đã thực hiện
                         result_status=result_status_prev, # Là 'success'
                         log_id=None # Log ID có thể không cần
                     )
                     if not edge_success: logger.error(f"Failed to merge transition {source_screen_id_prev} -> {confirmed_screen_id}")
                 else: edge_success = (result_status_prev != 'success') # Vẫn coi là OK nếu action trước không success

        neo4j_update_success = node_success and edge_success
        if neo4j_update_success: logger.info(f"[Explore Step Ctrl] Sync Neo4j update completed.")
        else: logger.warning(f"[Explore Step Ctrl] Sync Neo4j update potentially incomplete (NodeOK:{node_success}, EdgeOK:{edge_success}).")

        # 6. Gọi Planner (Dùng tên chuẩn)
        logger.debug(f"[Explore Step Ctrl] Calling planner for screen: {confirmed_screen_id}...")
        # Lấy context elements từ Neo4j cho AI Planner (dùng tên chuẩn)
        graph_context_elements = []
        if graph_db and hasattr(graph_db, 'get_screen_with_elements'):
            screen_details = graph_db.get_screen_with_elements(confirmed_screen_id) # <<< Dùng tên chuẩn screen_id
            if screen_details and 'elements' in screen_details: graph_context_elements = screen_details['elements']

        current_mapping_goal = None # Lấy mapping goal nếu cần
        # ... (logic lấy mapping goal) ...

        next_action = plan_exploration_action(
            current_screen_id=confirmed_screen_id, # <<< Dùng tên chuẩn
            ui_state=processed_ui_state,
            screen_elements=graph_context_elements,
            mapping_goal=current_mapping_goal
        )
        ...
        if next_action:
            response_body = {
                ...
                "confirmedCurrentScreenId": confirmed_screen_id, # <<< Dùng tên chuẩn
            ...
            response_body = {
                ...
                "confirmedCurrentScreenId": confirmed_screen_id,
                "nextAction": None
            }
            http_status_code = 200
    ...
    # Xử lý lỗi chung (giữ nguyên)
        ...
        response_body = {"status": "error", "message": str(ve), "confirmedCurrentScreenId": confirmed_screen_id, "nextAction": None}
        http_status_code = 400
    except Exception as e:
        logger.error(f"[Explore Step Ctrl] Unexpected Error (Device: {device_id}): {e}", exc_info=True)
        response_body = {"status": "error", "message": f"Internal server error: {type(e).__name__}", "confirmedCurrentScreenId": confirmed_screen_id, "nextAction": None}
        http_status_code = 500

    # 8. Trả về kết quả
    logger.info(f"[Explore Step Ctrl] Responding for {device_id}: Status={response_body['status']}, Screen={response_body['confirmedCurrentScreenId']}, NextAction={response_body['nextAction'] != None}")
...
def plan_sequential_click(current_screen_id: str, app_name: str,
    ...
    """
    Planner đơn giản: Click tuần tự element có ID (ưu tiên resource-id) chưa thử.
    Tránh lặp lại hành động vừa gây loop về màn hình hiện tại.
    """
    logger = current_app.logger if current_app else print
    ...
    if processed_ui_state and isinstance(processed_ui_state.get('elements'), list):
        # Lấy danh sách element đã được chuẩn hóa từ process_raw_ui_state
    ...
    if not elements_to_consider:
        ...
        # Fallback nếu không có element nào để xem xét
        return {"action": "back", "reason": "No elements found in processed state."}

    try:
        # Lấy các hành động đã thử từ màn hình này trong Neo4j
        ...
        for trans in outgoing_transitions:
             ...
             if isinstance(action, dict):
                  ...
                  if action_type == 'click':
                       # Ưu tiên lấy element_id chuẩn, fallback về các key cũ nếu cần
                       el_id = action.get('element_id') or action.get('onElementId') or action.get('target', {}).get('element_id') or action.get('target', {}).get('resource_id')
                       if el_id: tried_element_ids.add(el_id)
                  elif action_type == 'run_macro' and action.get('macro_code') == 'NAV_GO_BACK':
                       tried_back = True
                  # Có thể thêm kiểm tra swipe nếu bạn có action swipe
        ...
        if previous_action and isinstance(previous_action, dict):
             ...
             action_details = previous_action.get('action_details')
             if source_context and action_details:
                 previous_source_id = source_context.get('screen_id') # Dùng tên chuẩn
                 if previous_source_id == current_screen_id: # Kiểm tra xem có quay về màn hình cũ không
                      is_loop = True
                      if action_details.get('actionType') == 'click':
                          looping_element_id = action_details.get('element_id') or action_details.get('onElementId') # Lấy ID đã click
        ...
        # Duyệt qua danh sách elements theo thứ tự client gửi lên
        for element in elements_to_consider:
            element_id = element.get('element_id')
            id_type = element.get('identifier_type')

            # Chỉ xem xét element có ID và chưa được thử
            if element_id and id_type and (element_id not in tried_element_ids):
                # Kiểm tra xem có phải element vừa gây loop không
                if is_loop and looping_element_id == element_id:
                    logger.warning(f"[Sequential Planner] Skipping element '{element_id}' because it just caused a loop back to screen {current_screen_id}.")
                    continue # Bỏ qua element này

                # Ưu tiên resource-id hoặc content-desc hơn là text (ít tin cậy hơn)
                if id_type in ['resource-id', 'content-desc']:
                    logger.info(f"[Sequential Planner] Found UNTRIED element: id='{element_id}' (type='{id_type}'). Suggesting CLICK.")
                    next_action = {
                        "action": "click", # <<< Đổi actionType thành action cho nhất quán? Hoặc giữ nguyên tùy client
                        "element_id": element_id,
                        "identifier_type": id_type,
                        "reason": f"Clicking untried element ({id_type})"
                    }
                    # Bạn có thể thêm các thông tin khác như text, type vào đây nếu client cần
                    # next_action["text"] = element.get("text_content")
                    # next_action["element_type"] = element.get("element_type")
                    break # Tìm thấy hành động -> dừng duyệt

        # Nếu duyệt hết mà không tìm thấy hành động click mới
        if not next_action:
            logger.info(f"[Sequential Planner] No new elements with IDs to click on screen {current_screen_id}.")
            if not tried_back:
                logger.info("[Sequential Planner] Suggesting BACK action as fallback.")
                next_action = {"action": "back", "reason": "No new elements to click, trying back."}
            else:
                logger.warning(f"[Sequential Planner] All elements with IDs and BACK action seem explored for screen {current_screen_id}. Stuck.")
                next_action = {"action": "stuck", "reason": "Explored all elements with IDs and back action."}

    except Exception as e:
        logger.error(f"[Sequential Planner] Error during planning for screen {current_screen_id}: {e}", exc_info=True)
        next_action = {"action": "stuck", "reason": f"Planner error: {e}"} # Báo lỗi

    logger.debug(f"[Sequential Planner] Planned action: {next_action}")
    return next_action