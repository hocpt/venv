...
def explore_step_api():
    ...
    data = request.get_json()
    logger.debug(f"Received explore_step data: {data}") # Log toàn bộ data nhận được

    device_id = data.get('device_id')
    account_id = data.get('account_id')
    # === SỬA ĐỔI: Ưu tiên lấy 'raw_ui_state', nếu không có thì lấy 'current_ui_state' ===
    raw_ui_state = data.get('raw_ui_state')
    if raw_ui_state is None: # Nếu 'raw_ui_state' không có hoặc là null
        raw_ui_state = data.get('current_ui_state') # Thử lấy 'current_ui_state'
        if raw_ui_state:
            logger.info("Using 'current_ui_state' as raw_ui_state was not provided.")
    # =================================================================================
    previous_action = data.get('previous_action') # Có thể là None
    screenshot_filename = data.get('screenshot_filename') # Key này phải khớp với client gửi

    # Kiểm tra các tham số bắt buộc
    if not device_id or not raw_ui_state: # raw_ui_state giờ đã được kiểm tra từ cả hai key
        missing_params = []
        if not device_id: missing_params.append('device_id')
        if not raw_ui_state: missing_params.append('raw_ui_state (or current_ui_state)') # Cập nhật thông báo lỗi
    ...
    try:
        # Truyền screenshot_filename vào hàm controller
        response_data, status_code = controller.handle_explore_step(
            device_id=device_id,
            account_id=account_id,
            current_screen_id_from_client=None, # Controller sẽ tự xác định từ raw_ui_state
...
# @require_api_key # Thêm xác thực nếu cần
def upload_screenshot():
    ...
    if file and allowed_file(file.filename):
        ...
        upload_folder = current_app.config.get('UPLOAD_FOLDER') # Sẽ là htp6/screenshots/
        ...
        try:
            ...
            # === TRẢ VỀ CHỈ TÊN FILE ===
            relative_path_for_static = os.path.join('screenshots', unique_filename).replace('\\', '/')
            # ============================================================
            return jsonify({
                "success": True,
                "message": "File đã upload thành công",
                "static_path": relative_path_for_static # Trả về đường dẫn cho static
            }), 201
            # ===========================
        except Exception as e:
