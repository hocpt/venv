...
def handle_explore_step(device_id: str, account_id: str,
                        ...
                        screenshot_static_path: str | None = None, # <<< Đổi tên tham số
    ...
    try:
        ...
        # === XỬ LÝ SCREENSHOT_FILENAME ĐỂ LƯU VÀO NEO4J ===
        # Giá trị này đã là tên file, không cần parse URL nữa
        path_to_store_in_neo4j = screenshot_static_path
        if path_to_store_in_neo4j:
            logger.info(f"Sẽ sử dụng screenshot_static_path: '{path_to_store_in_neo4j}' để lưu vào Neo4j.")
        # =================================================

        # 4. Ghi Log PostgreSQL (Giữ nguyên logic cũ)
        # ... (code logging PostgreSQL) ...
        try:
        ...
        # 5a. Merge Node Đích
        # === LOGGING NGAY TRƯỚC KHI GỌI merge_screen ===
        logger.debug(f"[Explore Step Ctrl] Calling graph_db.merge_screen with:")
        logger.debug(f"  screen_id: {confirmed_target_screen_id}")
        logger.debug(f"  app_name: {app_name}")
        logger.debug(f"  activity_name: {activity_name}")
        # logger.debug(f"  extracted_elements: {extracted_elements_for_db[:2]}") # Log một phần elements
        logger.debug(f"  log_id: None")
        logger.debug(f"  screenshot_path (sẽ truyền vào merge_screen): '{path_to_store_in_neo4j}'") # <<< GIÁ TRỊ NÀY QUAN TRỌNG
        # ================================================
        node_success = graph_db.merge_screen(
            ...
            log_id=None,
            screenshot_path=path_to_store_in_neo4j # <<< TRUYỀN GIÁ TRỊ ĐÃ XỬ LÝ
        )
        ...
        if not node_success:
            ...
            # Xử lý lỗi nếu cần, ví dụ: không tạo cạnh nếu node đích lỗi
        else:
            # ... (logic merge cạnh giữ nguyên nếu có previous_action) ...
            edge_success = True # Mặc định
            if previous_action and isinstance(previous_action, dict):
                 ...
                 if source_context and action_details:
                     ...
                     # ... (logging lý do bỏ qua merge cạnh) ...
            # ... (kết luận neo4j_update_success) ...
