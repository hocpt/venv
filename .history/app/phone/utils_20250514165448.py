...
def process_raw_ui_state(reported_ui_state: dict | None) -> dict | None:
    ...
    # Khởi tạo processed_state trước
    processed_state = {
    ...
    # Lấy danh sách các thuộc tính thô của elements TỪ reported_ui_state
    ...
    # (Nếu có các list khác như clickable_list, editable_list, bounds_list, lấy chúng ở đây)
    # clickable_list = reported_ui_state.get('clickable', [])
    # editable_list = reported_ui_state.get('editable', [])
    # bounds_list = reported_ui_state.get('bounds', [])


    # --- DI CHUYỂN CÁC DÒNG LOG XUỐNG ĐÂY ---
    ...
    # --- KẾT THÚC DI CHUYỂN LOG ---

    # Kiểm tra kiểu dữ liệu cơ bản của các list (sau khi đã gán)
    if not all(isinstance(lst, list) for lst in [ids, texts, coords_str_list, class_names]):
        logger.warning("process_raw_ui_state: Một trong các list ids, texts, coords, class_names không phải là list sau khi lấy từ reported_ui_state.")
        # processed_state['elements'] đã là []
        return processed_state # Trả về state với elements rỗng

    min_len = 0
    # Chỉ tính min_len nếu tất cả các list chính (dùng để tạo element) đều là list và có phần tử
    # (hoặc ít nhất là không phải None, get với default là [] đã xử lý None)
    # Logic này đảm bảo chúng ta không bị lỗi nếu một list nào đó thiếu hẳn từ client và .get() trả về []
    # và len([]) là 0.
    if ids is not None and texts is not None and coords_str_list is not None and class_names is not None:
        min_len = min(len(ids), len(texts), len(coords_str_list), len(class_names))
    
    logger.debug(f"  Calculated min_len for element processing: {min_len}")

    if min_len == 0:
        logger.warning(f"  min_len is 0. No elements will be iterated and processed from these core lists.")
    
    processed_elements_count = 0
    for i in range(min_len): # Vòng lặp này sẽ không chạy nếu min_len = 0
        element_id_val = None
        identifier_type_val = None
        
        raw_resource_id = ids[i]
        raw_text = texts[i]
        raw_class_name = class_names[i] # class_names[i] sẽ không lỗi nếu min_len > 0
        ...
        elif raw_class_name and str(raw_class_name).strip():
        ...
        if element_id_val:
            ...
            current_coord_str = coords_str_list[i] if i < len(coords_str_list) else None
            coordinates = _parse_coordinates_safe(current_coord_str) # _parse_coordinates_safe đã xử lý None
            if coordinates:
                element_entry['coordinates'] = coordinates
            
            # Xử lý các thuộc tính khác nếu có (bounds, clickable, editable)
            # Ví dụ:
            # current_bounds_data = bounds_list[i] if i < len(bounds_list) else None
            # if current_bounds_data: element_entry['bounds'] = parse_and_validate_bounds(current_bounds_data) # Cần hàm riêng
