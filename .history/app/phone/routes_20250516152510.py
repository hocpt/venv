...
def upload_screenshot_from_client_corrected_name(): # Sử dụng tên hàm mới hoặc sửa hàm cũ
    logger = current_app.logger
    ...
    if file_field_key_from_client not in request.files:
        logger.warning(f"PHONE_UPLOAD_API: No '{file_field_key_from_client}' part in request.files. Available keys: {list(request.files.keys())}")
    ...
    # Client PHẢI gửi các trường này.
    ...
    logger.debug(f"PHONE_UPLOAD_API: Form data received - 'filename' for server: '{filename_to_use_on_server}', 'app_name': '{app_name_for_folder_structure}'")
    logger.debug(f"PHONE_UPLOAD_API: Uploaded file object - original name from client's disk: '{uploaded_file_storage.filename}' (this name is NOT used for saving)")

    if not filename_to_use_on_server: # Tên file này là bắt buộc
        logger.warning("PHONE_UPLOAD_API: Missing 'filename' form parameter. This is the intended filename for server storage and must match Neo4j's screenshot_path.")
    ...
    if uploaded_file_storage.filename == '': # Kiểm tra xem client có thực sự gửi file không
    ...
    # 3. An toàn hóa tên file (dựa trên filename_to_use_on_server) và tên thư mục app_name
    # KHÔNG SỬ DỤNG uploaded_file_storage.filename để quyết định tên file lưu trữ cuối cùng.
    ...
    if not final_safe_filename_to_save or not safe_app_name_folder:
        logger.error(f"PHONE_UPLOAD_API: Invalid filename or app_name after sanitization. Original form filename: '{filename_to_use_on_server}', app_name: '{app_name_for_folder_structure}'")
    ...
    # 6. Đường dẫn đầy đủ để lưu file ảnh
    full_path_to_save_file = os.path.join(app_specific_storage_directory, final_safe_filename_to_save) # Sử dụng final_safe_filename
    ...
    try:
        ...
        logger.error(f"PHONE_UPLOAD_API: EXCEPTION during save of file '{final_safe_filename_to_save}' to '{app_specific_storage_directory}': {e}", exc_info=True)
