...
@phone_bp.route('/_internal/accounts_for_device', methods=['GET'])
def internal_get_accounts_for_device():
    """
    API nội bộ (có thể cần bảo vệ) để lấy danh sách account cho một device.
    Dùng cho dropdown động trên form Add Task Assignment.
    """
    ...
    if accounts_list is None:
        # Lỗi DB hoặc không tìm thấy device
        return jsonify({"error": "Could not retrieve accounts for the device"}), 500
