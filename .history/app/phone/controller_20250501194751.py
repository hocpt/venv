...
def plan_simple_exploration_action(app_name: str, current_screen_id: str) -> dict | None:
    ...
    try:
        ...
        if not screen_details:
            ...
            return None # <<< Trả về None nếu không tìm thấy node

        # Lấy elements từ state đã lưu trong node
        ...
        for element in screen_elements:
            # Đảm bảo đọc đúng key 'clickable' mà process_raw_ui_state tạo ra
            ...
            # Kiểm tra xem đã có transition click nào khớp với element này chưa
            action_already_tried = False
            for trans in outgoing_transitions:
                # Chỉ quan tâm đến transition loại click để tránh trùng lặp click
                if trans.get('actionType') == 'click': # Giả sử transition lưu actionType là 'click'
                    # So sánh chặt chẽ hơn
            ...
            if not action_already_tried:
                ...
                # ===>>> THAY ĐỔI CẤU TRÚC RETURN CHO CLICK <<<===
                target_element_data = { # Tạo object target
                    "resource_id": element_id,
                    "text": element_text,
                    "class_name": element_class
                    # Thêm các thuộc tính khác nếu cần
                }
                # Loại bỏ key có giá trị None khỏi target_element_data
                target_element_data = {k:v for k,v in target_element_data.items() if v is not None}

                # Chỉ tạo action nếu target_element_data không rỗng
                if target_element_data:
                    next_action = {
                        "actionType": "run_macro", # <<< Loại action chuẩn
                        "macro_code": "UI_CLICK",  # <<< Macro code tương ứng
                        "params": {                # <<< Tham số cho macro
                            "target": target_element_data
                        }
                        # Có thể thêm random_delay_ms, random_offset_xy ở đây nếu muốn
                        # "random_delay_ms": {"min": 100, "max": 300}
                    }
                    found_click_action = True
                    break # Đã tìm thấy hành động click mới -> dừng tìm kiếm
                else:
                     logger.warning("Generated empty targetElement for a clickable element, skipping click action.")
                     # Tiếp tục vòng lặp để tìm element khác

        # Nếu không tìm thấy hành động click nào mới sau khi duyệt hết elements
        if not found_click_action:
             logger.info(f"No new clickable elements found for screen {current_screen_id}. Suggesting SWIPE_UP.")
             # ===>>> THAY ĐỔI CẤU TRÚC RETURN CHO SWIPE_UP <<<===
             next_action = {
                 "actionType": "run_macro",        # <<< Loại action chuẩn
                 "macro_code": "UI_SWIPE_UP",      # <<< Macro code tương ứng
                 "params": {}                      # <<< Params rỗng hoặc có thể thêm duration
                 # Có thể thêm random_delay_ms ở đây nếu muốn
             }
