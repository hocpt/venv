...
def get_next_exploration_action():
    ...
    device_id = data.get('device_id')
    account_id = data.get('account_id')
    # Client gửi ID màn hình nó đang đứng (ID này có thể là hash hoặc ID server trả về trước đó)
    ...
    if not device_id: errors.append("Missing 'device_id'.")
    if not account_id: errors.append("Missing 'account_id'.")
    if not current_screen_id_from_client: errors.append("Missing 'current_screen_id'.")
    ...
    # --- 2. Logic Chính (Gọi Planner) ---
    next_action = None
    error_message = None
    # TODO (Future Improvement): Implement logic to verify/confirm the actual current screen ID
    # based on the last reported UI state, instead of just trusting the client.
    # For now, we trust the client's ID.
    confirmed_current_screen_id = current_screen_id_from_client

    try:
        # Lấy app_name (cần thiết cho planner truy vấn Neo4j)
        app_name = None
        if db and hasattr(db, 'get_app_name_from_account'):
            app_name = db.get_app_name_from_account(account_id)
        else:
            logger.error("Get Next Action: Database module or 'get_app_name_from_account' function not available.")
            # Có thể thử lấy app_name từ request nếu client gửi, hoặc báo lỗi
            app_name = data.get('app_name') # Thử lấy từ request
            if not app_name:
                 return jsonify({"status": "error", "message": "Cannot determine app_name."}), 500 # Lỗi server

        # Kiểm tra sự tồn tại của phone_controller và hàm planner
        if not phone_controller or not hasattr(phone_controller, 'plan_simple_exploration_action'):
            ...
            logger.critical(error_message) # Dùng critical vì đây là lỗi cấu hình server
            return jsonify({"status": "error", "message": error_message}), 500

        # Gọi hàm planner trong controller
        ...
        next_action = phone_controller.plan_simple_exploration_action(
            app_name=app_name,
            current_screen_id=confirmed_current_screen_id
        )
        # Planner trả về dict hành động hoặc None

    except Exception as e:
        logger.error(f"Error calling planner for screen {confirmed_current_screen_id}: {e}", exc_info=True)
        print(traceback.format_exc()) # In traceback ra console để debug
        return jsonify({"status": "error", "message": f"Internal server error during planning: {e}"}), 500

    # --- 3. Xử lý Kết quả và Trả về Response ---
    if next_action:
        # Nếu planner trả về một hành động hợp lệ (là dict)
        ...
        response_data = {
            "status": "success",
            "confirmedCurrentScreenId": confirmed_current_screen_id, # Trả về ID đã dùng để lên kế hoạch
            "nextAction": next_action,
            "message": f"Next action '{action_type_log}' planned."
        }
        return jsonify(response_data), 200
    else:
        # Nếu planner trả về None (không tìm thấy hành động hoặc lỗi nội bộ planner)
        logger.warning(f"No next exploration action planned from screen {confirmed_current_screen_id}.")
        response_data = {
            "status": "no_action",
            "confirmedCurrentScreenId": confirmed_current_screen_id,
            "nextAction": None,
            "message": "No further actions planned from this screen currently, or planner encountered an issue."
        }
        return jsonify(response_data), 200
