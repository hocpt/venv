...
def handle_explore_step(device_id: str, account_id: str,
                        ...
                        screenshot_url: str | None = None, # <<< Nhận screenshot_url (ví dụ)
                        **kwargs) -> tuple[dict, int]:
    logger = current_app.logger if current_app else print
    logger.info(f"[Explore Step Ctrl] Handling step for device: {device_id}, account: {account_id}")
    # === THÊM LOGGING CHI TIẾT ===
    logger.debug(f"[Explore Step Ctrl] Received raw_ui_state keys: {list(raw_ui_state.keys()) if isinstance(raw_ui_state, dict) else 'Invalid Type'}")
    logger.debug(f"[Explore Step Ctrl] Received previous_action: {json.dumps(previous_action, indent=2) if previous_action else 'None'}")
    # ============================

    response_body = {}
    http_status_code = 500 # Mặc định lỗi server
    confirmed_target_screen_id = None # ID màn hình đích sau khi xử lý state
    app_name = None
    next_action = None

    try:
        # 1. Xử lý State -> Lấy state đã chuẩn hóa
        processed_ui_state = process_raw_ui_state(raw_ui_state)
        if not processed_ui_state: raise ValueError("Failed to process raw UI state.")
        ...
        if not app_name: raise ValueError("Cannot determine app_name.")
        activity_name = processed_ui_state.get('activity_name')

        # === XỬ LÝ SCREENSHOT URL THÀNH PATH ===
        relative_screenshot_path = None
        if screenshot_url:
            try:
                # Giả định URL có dạng http://.../static/screenshots/filename.png
                # Hoặc chỉ là đường dẫn tương đối /static/screenshots/filename.png
                parsed_url = urlparse(screenshot_url)
                path_parts = parsed_url.path.strip('/').split('/')
                # Tìm vị trí của 'static' và lấy phần còn lại
                if 'static' in path_parts:
                    static_index = path_parts.index('static')
                    relative_screenshot_path = '/'.join(path_parts[static_index+1:]) # Ví dụ: screenshots/filename.png
                    logger.info(f"Converted screenshot URL '{screenshot_url}' to relative path: '{relative_screenshot_path}'")
                else:
                    # Nếu không có 'static', thử giả định nó là đường dẫn tương đối đã đúng
                    # Cần kiểm tra kỹ định dạng URL client gửi về
                    relative_screenshot_path = parsed_url.path.strip('/')
                    logger.warning(f"Could not find 'static' in screenshot URL path '{parsed_url.path}'. Assuming it's already a relative path: '{relative_screenshot_path}'")

            except Exception as url_err:
                logger.error(f"Error processing screenshot URL '{screenshot_url}': {url_err}")
                relative_screenshot_path = None # Đặt lại thành None nếu lỗi

        # 4. Ghi Log PostgreSQL (Giữ nguyên logic cũ)
        logger.debug(f"[Explore Step Ctrl] Calling INTELLIGENT planner for screen: {confirmed_target_screen_id}...")
        try:
            next_action = plan_intelligent_exploration_action( # <<< GỌI HÀM MỚI
                current_screen_id=confirmed_target_screen_id,
                app_name=app_name,
                processed_ui_state=processed_ui_state, # Truyền state đã xử lý
                previous_action=previous_action        # Truyền action trước đó
            )
            logger.debug(f"[Explore Step Ctrl] Intelligent Planner returned: {next_action}")
        except Exception as planner_err:
                logger.error(f"[Explore Step Ctrl] Error calling intelligent planner: {planner_err}", exc_info=True)
                next_action = {"actionType": "wait", "duration": 30, "reason": "Planner error"}

        # 5. Cập nhật Neo4j Đồng bộ
        logger.info(f"[Explore Step Ctrl] Starting synchronous Neo4j update for screen: {confirmed_target_screen_id}...")
        extracted_elements_for_db = processed_ui_state.get('elements', [])

        # 5a. Merge Node Đích
        logger.debug(f"[Explore Step Ctrl] Merging target node: {confirmed_target_screen_id}...")
        node_success = graph_db.merge_screen(
            screen_id=confirmed_target_screen_id,
            app_name=app_name,
            activity_name=activity_name,
            extracted_elements=extracted_elements_for_db,
            log_id=None # Có thể lấy log_id từ bước 4 nếu cần
        )
        logger.debug(f"[Explore Step Ctrl] Target node merge result: {node_success}")
        if not node_success:
            # Có thể dừng lại hoặc chỉ cảnh báo nếu node đích không tạo/cập nhật được
            logger.error(f"[Explore Step Ctrl] Failed to merge target screen node {confirmed_target_screen_id}. Aborting Neo4j edge creation.")
            # raise ValueError(f"Failed to merge target screen node {confirmed_target_screen_id}") # Hoặc ném lỗi
        else:
            # Chỉ thực hiện bước 5b và 5c nếu node đích OK
            edge_success = True # Mặc định là true nếu không có previous_action
            if previous_action and isinstance(previous_action, dict):
                logger.debug("[Explore Step Ctrl] Processing previous_action for edge creation...")
                source_context = previous_action.get('source_screen_context')
                action_details = previous_action.get('action_details')
                # Giả sử client gửi kết quả thực thi của action trước đó
                result_status_prev = previous_action.get('result_status', 'success') # Mặc định là success nếu client không gửi

                # === THÊM LOGGING KIỂM TRA DỮ LIỆU PREVIOUS_ACTION ===
                logger.debug(f"[Explore Step Ctrl] Previous Action - Source Context: {source_context}")
                logger.debug(f"[Explore Step Ctrl] Previous Action - Action Details: {action_details}")
                logger.debug(f"[Explore Step Ctrl] Previous Action - Result Status: {result_status_prev}")
                # ===================================================
                ...
                if source_context and isinstance(source_context, dict) and action_details and isinstance(action_details, dict):
                    ...
                    # === THÊM LOGGING KIỂM TRA SOURCE ID ===
                    logger.debug(f"[Explore Step Ctrl] Previous Source Screen ID: {source_screen_id_prev}")
                    # =====================================

                    if source_screen_id_prev:
                        # 5b. Cập nhật Element Stats của hành động trước (trên node nguồn)
                        # (Logic này có thể giữ nguyên hoặc tạm bỏ qua nếu chưa cần)
                        action_el_id = action_details.get('element_id') or action_details.get('onElementId')
                        action_id_type = action_details.get('identifier_type')
                        if action_el_id and action_id_type:
                             logger.debug(f"[Explore Step Ctrl] Updating element stats on source {source_screen_id_prev} for element {action_el_id}...")
                             try:
                                 graph_db.update_element_interaction_stats(
                                     screen_id=source_screen_id_prev,
                                     element_id=action_el_id,
                                     identifier_type=action_id_type,
                                     success=(result_status_prev == 'success')
                                 )
                             except Exception as e_stat: logger.error(f"[Explore Step Ctrl] Error updating element stats: {e_stat}")

                        # 5c. Merge Cạnh Transition (Chỉ khi action trước thành công VÀ màn hình thay đổi)
                        if result_status_prev == 'success' and source_screen_id_prev != confirmed_target_screen_id:
                            logger.info(f"[Explore Step Ctrl] Conditions met for merging transition: {source_screen_id_prev} -> {confirmed_target_screen_id}")
                            # === THÊM LOGGING NGAY TRƯỚC KHI GỌI merge_transition ===
                            logger.debug(f"[Explore Step Ctrl] Calling graph_db.merge_transition with:")
                            logger.debug(f"  source_screen_id: {source_screen_id_prev}")
                            logger.debug(f"  target_screen_id: {confirmed_target_screen_id}")
                            logger.debug(f"  app_name: {app_name}")
                            logger.debug(f"  action_details: {json.dumps(action_details, indent=2)}")
                            logger.debug(f"  result_status: {result_status_prev}")
                            # =======================================================
                            ...
                            if not edge_success:
                                logger.error(f"[Explore Step Ctrl] graph_db.merge_transition returned False for {source_screen_id_prev} -> {confirmed_target_screen_id}")
                            else:
                                logger.info(f"[Explore Step Ctrl] graph_db.merge_transition returned True.")
                        elif result_status_prev != 'success':
                             logger.warning(f"[Explore Step Ctrl] Skipping transition merge because previous action status was '{result_status_prev}'.")
                        elif source_screen_id_prev == confirmed_target_screen_id:
                             logger.info(f"[Explore Step Ctrl] Skipping transition merge because source and target screen IDs are the same ('{source_screen_id_prev}'). Action might not have changed the screen.")
                        else:
                             logger.warning("[Explore Step Ctrl] Unexpected condition, skipping transition merge.")


                    else: # Thiếu source_screen_id_prev
                        logger.warning("[Explore Step Ctrl] Skipping transition merge: Missing source_screen_id in previous_action context.")
                        edge_success = False # Coi như lỗi nếu thiếu source ID
                else: # Thiếu source_context hoặc action_details
                    logger.warning("[Explore Step Ctrl] Skipping transition merge: Invalid or missing source_context/action_details in previous_action.")
                    edge_success = False # Coi như lỗi nếu thiếu context/details
            else: # Không có previous_action
                logger.info("[Explore Step Ctrl] No previous_action provided, skipping transition merge.")
                # edge_success vẫn là True

            neo4j_update_success = node_success and edge_success # Thành công nếu cả node và edge (nếu có) đều OK
            if neo4j_update_success:
                logger.info(f"[Explore Step Ctrl] Sync Neo4j update completed for target {confirmed_target_screen_id}.")
            else:
                logger.warning(f"[Explore Step Ctrl] Sync Neo4j update potentially incomplete for target {confirmed_target_screen_id} (NodeOK:{node_success}, EdgeOK:{edge_success}).")

        # 6. Gọi Planner (Dùng planner tuần tự đã sửa)
        logger.debug(f"[Explore Step Ctrl] Calling planner for screen: {confirmed_target_screen_id}...")
        try:
            # === Gọi planner tuần tự, truyền state đã xử lý và action trước đó ===
            next_action = plan_sequential_click(
                current_screen_id=confirmed_target_screen_id,
                app_name=app_name,
                processed_ui_state=processed_ui_state, # Truyền state đã xử lý
                previous_action=previous_action        # Truyền action trước đó
            ...
            logger.debug(f"[Explore Step Ctrl] Planner returned: {next_action}")
             ...
             logger.error(f"[Explore Step Ctrl] Error calling planner: {planner_err}", exc_info=True)
             # Có thể trả về lỗi hoặc fallback về 'wait'/'back'
             next_action = {"actionType": "wait", "duration": 30, "reason": "Planner error"}
        ...
        else: # Planner không trả về action nào (có thể là stuck hoặc lỗi nhẹ)
            response_body = {
                "status": "no_action", "message": "No further exploration action planned from this state.",
                "confirmedCurrentScreenId": confirmed_target_screen_id,
                "nextAction": None
            }
    ...
    # Xử lý lỗi chung
        ...
        http_status_code = 400 # Bad request do dữ liệu đầu vào
    except Exception as e:
        logger.error(f"[Explore Step Ctrl] Unexpected Error (Device: {device_id}): {e}", exc_info=True)
        # traceback.print_exc() # In traceback ra console nếu cần
