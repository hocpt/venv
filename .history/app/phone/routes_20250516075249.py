...
def upload_screenshot():
    logger = current_app.logger
    logger.info("Nhận yêu cầu upload screenshot.")

    if 'screenshot_file' not in request.files:
        logger.warning("Không có phần file tên 'screenshot_file' trong request.")
        return jsonify({"success": False, "error": "Không có phần file trong request"}), 400
    ...
    if file and allowed_file(file.filename):
        ...
        unique_filename = f"{uuid.uuid4()}.{file_extension}" # Chỉ tên file

        upload_folder = current_app.config.get('UPLOAD_FOLDER') # Sẽ là htp6/app/static/screenshots/
        if not upload_folder:
            logger.error("UPLOAD_FOLDER chưa được cấu hình.")
            return jsonify({"success": False, "error": "Lỗi cấu hình server (thư mục upload)"}), 500

        save_path = os.path.join(upload_folder, unique_filename)

        try:
            file.save(save_path)
            logger.info(f"Screenshot đã lưu thành công: {save_path}")

            # === TRẢ VỀ CHỈ TÊN FILE ===
            return jsonify({
                "success": True,
                "message": "File đã upload thành công",
                "filename": unique_filename # <<< Chỉ trả về tên file
            }), 201
            # ===========================
        except Exception as e:
            logger.error(f"Không thể lưu file đã upload vào {save_path}: {e}", exc_info=True)
            return jsonify({"success": False, "error": "Không thể lưu file trên server"}), 500
    else:
        logger.warning(f"Loại file không được phép: {file.filename}")
        return jsonify({"success": False, "error": "Loại file không được phép"}), 400

@phone_bp.route('/api/upload/screenshot', methods=['POST']) # Hoặc URL mà client của bạn gọi
def upload_screenshot_file_corrected():
