...
def process_raw_ui_state(reported_ui_state: dict | None) -> dict | None:
    ...
    logger.debug(f"process_raw_ui_state: Input reported_ui_state keys: {list(reported_ui_state.keys()) if reported_ui_state else 'None'}")

    # ... (lấy ids, texts, coords_str_list, class_names) ...
    logger.debug(f"  ids: {ids[:5]} (len: {len(ids if ids else [])})") # Log 5 phần tử đầu và độ dài
    logger.debug(f"  texts: {texts[:5]} (len: {len(texts if texts else [])})")
    logger.debug(f"  coords_str_list: {coords_str_list[:5]} (len: {len(coords_str_list if coords_str_list else [])})")
    logger.debug(f"  class_names: {class_names[:5]} (len: {len(class_names if class_names else [])})")
