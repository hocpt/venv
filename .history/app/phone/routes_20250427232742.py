...
def report_status():
    ...
    if not data or not isinstance(data, dict):
        current_app.logger.error("Report Status: Invalid payload - Not a JSON object.")
        return jsonify({"status": "error", "message": "Invalid JSON payload."}), 400

    # 2. Trích xuất Dữ liệu từ Payload
    device_id = data.get('device_id')
    account_id = data.get('account_id')
    assignment_id_str = data.get('assignment_id') # Lấy dạng chuỗi
    status_report_payload = data.get('status_report') # Dict chứa status, progress, error
    logs_payload = data.get('logs', []) # List các log entries thô
    raw_ui_state = data.get('current_ui_state') # Dict UI thô từ client
    previous_action_context = data.get('previous_action') # Dict context hành động trước đó
    ...
    if errors:
        ...
        current_app.logger.error(f"Report Status Validation Failed: {msg}. Payload: {data}")
    ...
    # 4. Xử lý UI State và Đưa Job Map Building vào Queue
    ...
    if raw_ui_state and isinstance(raw_ui_state, dict):
        current_app.logger.debug(f"Raw UI state received for assignment {assignment_id}.")
        try:
            ...
            if processed_ui_state_for_map:
                current_app.logger.debug(f"UI state processed successfully for assignment {assignment_id}.")
                ...
                try:
                    ui_state_for_log_saving = json.dumps(processed_ui_state_for_map, ensure_ascii=False)
                     ...
                     current_app.logger.error(f"Error serializing processed UI state to JSON for PG log (assignment {assignment_id}): {json_err}")
                ...
                try:
                    # Gọi hàm DB để thêm command vào queue
                    ...
                    if command_id:
                        ...
                        # Hàm add_scheduler_command trả về None nếu lỗi DB
                        current_app.logger.error(f"Failed to enqueue 'build_map' task (db.add_scheduler_command returned None) for assignment {assignment_id}.")
                except Exception as queue_err:
                    # Bắt lỗi khác có thể xảy ra khi gọi hàm DB
                    current_app.logger.error(f"Exception enqueuing 'build_map' task for assignment {assignment_id}: {queue_err}", exc_info=True)
                # --- Kết thúc đưa job vào queue ---

            else: # process_raw_ui_state trả về None
                 current_app.logger.warning(f"process_raw_ui_state returned None for assignment {assignment_id}. Map update skipped for this state.")

        except Exception as ui_proc_err:
             current_app.logger.error(f"Error during call to process_raw_ui_state for assignment {assignment_id}: {ui_proc_err}", exc_info=True)
             # Không enqueue job map nếu xử lý UI lỗi

    else:
        current_app.logger.debug(f"No 'current_ui_state' provided in report for assignment {assignment_id}.")

    # 5. Xử lý Phần Còn lại của Báo cáo (Cập nhật status PG, ghi log PG...)
    try:
        # **QUAN TRỌNG:** Bạn cần đảm bảo hàm phone_controller.process_phone_report
        # và hàm db.add_phone_action_logs đã được sửa để nhận và lưu tham số
        # structured_ui_state_json (là chuỗi JSON hoặc None).
        response_body, status_code = phone_controller.process_phone_report(
            device_id=device_id,
            account_id=account_id,
            assignment_id=assignment_id,
            report_payload=data, # Truyền cả payload gốc để hàm controller lấy status_report, logs...
            structured_ui_state_json=ui_state_for_log_saving # <<< Truyền chuỗi JSON state đã xử lý
        )
        # Trả về phản hồi từ controller
        current_app.logger.info(f"Report processed for assignment {assignment_id}. Returning status {status_code}.")
        return jsonify({"status": "success", "message": "Report received and queued for map update."}), 200

    except Exception as e:
        # Lỗi không mong muốn xảy ra trong controller hoặc DB calls (PostgreSQL)
        current_app.logger.error(f"Error calling process_phone_report for assignment {assignment_id}: {e}", exc_info=True)
        # print(traceback.format_exc()) # In traceback ra console nếu cần
