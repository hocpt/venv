...
def handle_explore_step(device_id: str,
                        account_id: str,
    ...
    Luôn trả về một tuple: (dictionary_response_body, http_status_code).
    """
    logger = current_app.logger if current_app else print
    logger.info(f"[Explore Step] Handling step for device: {device_id}, account: {account_id}")

    # Giá trị mặc định cho response body và status code
    response_body = {}
    http_status_code = 500 # Mặc định là lỗi server
    confirmed_screen_id = None # ID server tính được
    next_action = None
    app_name = None
    ...
    try:
        ...
        # Gọi hàm xử lý state (cần đảm bảo hàm này tồn tại và hoạt động)
        ...
        # --- Bước 2: Xác định Target Screen ID ---
        ...
        confirmed_screen_id = target_screen_id
        logger.info(f"[Explore Step] Determined Target Screen ID: {confirmed_screen_id}")

        # --- Bước 3: Lấy App Name ---
        app_name = processed_ui_state.get('package_name')
        if not app_name:
            # Cố gắng lấy từ DB nếu cần (cần hàm get_app_name_from_account trong db)
            # logger.warning("[Explore Step] Missing package_name in state, trying DB lookup...")
            # if db and hasattr(db, 'get_app_name_from_account'): app_name = db.get_app_name_from_account(account_id)
            if not app_name: raise ValueError("Cannot determine app_name.")
        logger.debug(f"[Explore Step] Using App Name: {app_name}")

        # --- Bước 4: Ghi Log vào PostgreSQL ---
        ...
        if db and hasattr(db, 'add_phone_action_logs'):
            log_pg_success = db.add_phone_action_logs(
                assignment_id=None, device_id=device_id, account_id=account_id,
                logs=[placeholder_log_entry], structured_ui_state_json=ui_state_for_log_saving
            )
            if not log_pg_success: logger.warning("[Explore Step] Failed to write current state log to PostgreSQL.")
        else: logger.error("[Explore Step] Cannot log to PostgreSQL: DB module or function missing.")


        # --- Bước 5: Cập nhật Neo4j Đồng bộ ---
        logger.info(f"[Explore Step] Starting sync Neo4j update for screen: {confirmed_screen_id}...")
        ...
        if graph_db and hasattr(graph_db, 'create_or_update_screen_node'):
            screen_props = {
                ...
                "structureHash": confirmed_screen_id, # Dùng ID làm hash hoặc tạo hash riêng
                "elementCount": len(processed_ui_state.get('elements', [])),
                "rawStateSample": ui_state_for_log_saving # Lưu state đã xử lý
            }
            # Lưu ý: Hàm create_or_update_screen_node cần được tạo/sửa trong graph_db.py
            ...
            if not node_success:
             ...
             # Có thể ném lỗi ở đây nếu cập nhật Neo4j là bắt buộc
             # raise ConnectionError("Neo4j node update function unavailable.")

        # 5b. Cập nhật Cạnh Transition (Nếu có previous_action và node đích OK)
            ...
            if source_context and isinstance(source_context, dict) and action_details and isinstance(action_details, dict):
                ...
                if source_screen_id:
                    ...
                    if graph_db and hasattr(graph_db, 'create_or_update_transition_relationship'):
                         # Hàm này cũng cần được tạo/sửa trong graph_db.py
                        ...
                        if not edge_success: logger.error(f"[Explore Step] Neo4j Error: Failed edge update {source_screen_id} -> {confirmed_screen_id}")
                    else:
                         logger.error("[Explore Step] Cannot update Neo4j edge: graph_db module or function missing.")
                         edge_success = False
                else: edge_success = False; logger.warning("[Explore Step] Missing source_screen_id in previous_action.")
            else: edge_success = False; logger.warning("[Explore Step] Invalid previous_action structure.")

        neo4j_update_success = node_success and edge_success
        if neo4j_update_success: logger.info(f"[Explore Step] Sync Neo4j update completed for screen {confirmed_screen_id}.")
        else: logger.warning(f"[Explore Step] Sync Neo4j update potentially incomplete (NodeOK:{node_success}, EdgeOK:{edge_success}).")


        # --- Bước 6: Gọi Planner ---
        logger.debug(f"[Explore Step] Calling planner for confirmed screen: {confirmed_screen_id}...")
        # Lấy context bổ sung cho AI Planner (nếu dùng AI planner nâng cấp)
        graph_context_elements = []
        if graph_db and hasattr(graph_db, 'get_screen_with_elements'):
            screen_details_for_ai = graph_db.get_screen_with_elements(confirmed_screen_id)
            if screen_details_for_ai and 'elements' in screen_details_for_ai:
                graph_context_elements = screen_details_for_ai['elements']

        # Lấy mapping goal (cần logic lấy goal cho device/account này)
        current_mapping_goal = None
        # if db and hasattr(db, 'get_active_task_for_device'):
        #     active_task = db.get_active_task_for_device(device_id)
        #     if active_task: current_mapping_goal = active_task.get('mapping_goal')

        # Gọi hàm planner (có thể là AI hoặc planner đơn giản)
        next_action = plan_exploration_action( # Đảm bảo hàm planner này được import đúng
            current_screen_id=confirmed_screen_id,
            ui_state=processed_ui_state, # Có thể truyền state đã xử lý
            screen_elements=graph_context_elements, # Truyền context từ Neo4j
            mapping_goal=current_mapping_goal, # Truyền mục tiêu
            # previous_action_context=previous_action # Truyền cả context action trước nếu planner cần
        )

        # --- Bước 7: Xác định Status và Message cuối cùng ---
        if next_action:
            final_status = "success"
            final_message = "Next action planned."
            http_status_code = 200
        else:
            final_status = "no_action"
            final_message = "No further exploration actions planned from this screen."
            http_status_code = 200 # Vẫn là 200 OK, chỉ là không có action

    # --- Xử lý lỗi chung ---
    except ValueError as ve: # Lỗi dữ liệu đầu vào hoặc xử lý state/ID
        logger.error(f"[Explore Step] Value Error (Device: {device_id}): {ve}", exc_info=False)
        final_status = "error"; final_message = str(ve); http_status_code = 400
    except ConnectionError as ce: # Lỗi kết nối hoặc cập nhật Neo4j (nếu raise lỗi)
        logger.error(f"[Explore Step] Neo4j Update Error (Device: {device_id}): {ce}", exc_info=False)
        final_status = "error"; final_message = f"Failed to update graph database: {ce}"; http_status_code = 503
    except Exception as e: # Các lỗi không mong muốn khác
        logger.error(f"[Explore Step] Unexpected Error (Device: {device_id}): {e}", exc_info=True)
        traceback.print_exc()
        final_status = "error"; final_message = f"Internal server error: {type(e).__name__}"; http_status_code = 500

    # --- Bước 8: Tạo và Trả về Dictionary Response Body và Status Code ---
    response_body = {
        "status": final_status,
        "message": final_message,
        "confirmedCurrentScreenId": confirmed_screen_id, # Luôn trả về ID server xác nhận
        "nextAction": next_action # Object hành động hoặc None
    }
    logger.info(f"[Explore Step] Responding for Device {device_id}: Status='{final_status}', ScreenId='{confirmed_screen_id}', NextAction='{next_action.get('actionType') if isinstance(next_action, dict) else 'None'}'")

    # Luôn trả về tuple (dict, int)
