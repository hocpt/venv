...
def handle_explore_step(device_id: str, account_id: str,
    ...
    try:
        ...
        # 2. Xác định Screen ID chuẩn hóa cho màn hình ĐÍCH (hiện tại)
        ...
        if not target_screen_id or target_screen_id.startswith("error_"):
            raise ValueError(f"Failed to determine valid target screen ID (Result: {target_screen_id}).")
        confirmed_target_screen_id = target_screen_id
        logger.info(f"[Explore Step Ctrl] Confirmed Target Screen ID: {confirmed_target_screen_id}")

        # 3. Lấy App Name chuẩn hóa
...
def plan_intelligent_exploration_action(
    ...
    if outgoing_transitions is None:
        ...
        outgoing_transitions = [] # Coi như rỗng nếu lỗi DB

    # Tạo set các element_id đã được thử click/input từ màn hình này
    tried_element_ids = set()
    tried_back = False
    tried_swipe_up = False # (Thêm nếu cần)
    for trans in outgoing_transitions:
        action_type = trans.get('actionType')
        el_id = trans.get('element_id') # Thuộc tính đã chuẩn hóa trong merge_transition
        if action_type in ['click', 'input', 'run_macro'] and el_id:
            tried_element_ids.add(el_id)
        elif action_type == 'back' or trans.get('macro_code') == 'NAV_GO_BACK':
            tried_back = True
        elif action_type == 'swipe_up' or trans.get('macro_code') == 'UI_SWIPE_UP':
             tried_swipe_up = True
        # Thêm các loại action khác nếu cần (swipe_down, etc.)
