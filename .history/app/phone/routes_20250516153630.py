...
def upload_screenshot_from_client_corrected_filename(): # Sử dụng tên hàm mới hoặc sửa hàm cũ
    logger = current_app.logger
    logger.info("PHONE_UPLOAD_API_V4: Received new screenshot upload request.") # V4 để theo dõi phiên bản
    
    logger.debug(f"PHONE_UPLOAD_API_V4: Request Headers: {request.headers}")
    logger.debug(f"PHONE_UPLOAD_API_V4: Request Form Data (request.form): {request.form.to_dict()}")
    logger.debug(f"PHONE_UPLOAD_API_V4: Request Files Keys (request.files.keys()): {list(request.files.keys())}")

    # 1. Lấy file từ request (client gửi với key 'screenshot_file' theo log của bạn)
    file_field_key_from_client = 'screenshot_file' 
    if file_field_key_from_client not in request.files:
        logger.warning(f"PHONE_UPLOAD_API_V4: No '{file_field_key_from_client}' part in request.files. Available keys: {list(request.files.keys())}")
        return jsonify({"success": False, "error": f"Không tìm thấy phần file với tên '{file_field_key_from_client}'."}), 400
    
    uploaded_file_storage = request.files[file_field_key_from_client]

    # 2. Lấy filename và app_name từ request.form
    # filename_to_use_on_server là tên file đã được thống nhất trước đó (ví dụ: UUID.png)
    # và phải khớp với n.screenshot_path trong Neo4j
    filename_to_use_on_server = request.form.get('filename') 
    app_name_for_folder_structure = request.form.get('app_name')

    logger.debug(f"PHONE_UPLOAD_API_V4: Form data received - 'filename' (for server storage): '{filename_to_use_on_server}', 'app_name': '{app_name_for_folder_structure}'")
    # uploaded_file_storage.filename là tên file gốc từ máy client (ví dụ: screenshot.png)
    logger.debug(f"PHONE_UPLOAD_API_V4: Uploaded file object - original name on client's disk: '{uploaded_file_storage.filename}' (this name is NOT used for final saving unless it accidentally matches 'filename' form param)")

    if not filename_to_use_on_server: # Tên file này là bắt buộc
        logger.warning("PHONE_UPLOAD_API_V4: Missing 'filename' form parameter. This is the intended filename for server storage and must match Neo4j's screenshot_path.")
        return jsonify({"success": False, "error": "Thiếu tham số 'filename' trong dữ liệu form (tên file để lưu trên server)."}), 400
    
    if not app_name_for_folder_structure:
        logger.warning("PHONE_UPLOAD_API_V4: Missing 'app_name' form parameter. This is required for directory structure.")
        return jsonify({"success": False, "error": "Thiếu tham số 'app_name' trong dữ liệu form."}), 400
    
    # Kiểm tra xem client có thực sự gửi file không (dựa vào tên file gốc từ client)
    if uploaded_file_storage.filename == '': 
        logger.warning("PHONE_UPLOAD_API_V4: No file selected by client (uploaded_file_storage.filename is empty).")
        return jsonify({"success": False, "error": "Không có file nào được chọn để upload."}), 400

    # 3. An toàn hóa tên file (dựa trên filename_to_use_on_server từ form) và tên thư mục app_name
    # QUAN TRỌNG: Tên file cuối cùng để lưu trên server PHẢI LÀ filename_to_use_on_server
    final_safe_filename_to_save = secure_filename(os.path.basename(filename_to_use_on_server))
    safe_app_name_folder = secure_filename(app_name_for_folder_structure)

    if not final_safe_filename_to_save or not safe_app_name_folder:
        logger.error(f"PHONE_UPLOAD_API_V4: Invalid filename or app_name after sanitization. Intended server filename: '{filename_to_use_on_server}', app_name: '{app_name_for_folder_structure}'")
        return jsonify({"success": False, "error": "Tên file hoặc app_name không hợp lệ sau khi xử lý."}), 400

    # 4. Lấy đường dẫn lưu trữ gốc từ config
    storage_base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
    if not storage_base_path:
        logger.critical("PHONE_UPLOAD_API_V4: SCREENSHOT_STORAGE_PATH is NOT CONFIGURED!")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (storage path missing)."}), 500
    
    logger.info(f"PHONE_UPLOAD_API_V4: Configured SCREENSHOT_STORAGE_PATH: '{storage_base_path}'")

    # 5. Tạo đường dẫn đến thư mục con của app và tạo thư mục nếu chưa có
    app_specific_storage_directory = os.path.join(storage_base_path, safe_app_name_folder)
    logger.info(f"PHONE_UPLOAD_API_V4: Target app-specific directory: '{app_specific_storage_directory}'")
    
    try:
        if not os.path.exists(app_specific_storage_directory):
            os.makedirs(app_specific_storage_directory, exist_ok=True)
            logger.info(f"PHONE_UPLOAD_API_V4: Created directory: {app_specific_storage_directory}")
    except OSError as e:
        logger.error(f"PHONE_UPLOAD_API_V4: Could not create directory '{app_specific_storage_directory}': {e}", exc_info=True)
        return jsonify({"success": False, "error": "Server error creating storage directory."}), 500
    
    # 6. Đường dẫn đầy đủ để lưu file ảnh (sử dụng final_safe_filename_to_save)
    full_path_to_save_file = os.path.join(app_specific_storage_directory, final_safe_filename_to_save) 
    logger.info(f"PHONE_UPLOAD_API_V4: Attempting to save uploaded file to: '{full_path_to_save_file}' (using filename from form parameter)")
    ...
    try:
        ...
        if os.path.exists(full_path_to_save_file) and os.path.getsize(full_path_to_save_file) > 0:
            logger.info(f"PHONE_UPLOAD_API_V4: SUCCESS - Image saved to: {full_path_to_save_file}")
            return jsonify({
                "success": True, 
                "message": "Screenshot uploaded successfully.", 
            }), 201
        else:
            logger.error(f"PHONE_UPLOAD_API_V4: FAILED - File not found or empty after save attempt at: {full_path_to_save_file}")
            return jsonify({"success": False, "error": "Server error: Could not confirm file save or file is empty."}), 500
    except Exception as e:
        logger.error(f"PHONE_UPLOAD_API_V4: EXCEPTION during save of file '{final_safe_filename_to_save}' to '{app_specific_storage_directory}': {e}", exc_info=True)
