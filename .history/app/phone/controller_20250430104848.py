...
def process_raw_ui_state(ui_state_data: dict) -> dict | None:
    """
    Xử lý dữ liệu trạng thái UI thô từ client và chuyển đổi thành JSON cấu trúc.
    *** Phiên bản này giả định client đã lọc "Only Clickable" bằng AutoInput,
    nên tất cả element nhận được đều được đánh dấu clickable=True. ***

    Args:
        ui_state_data: Dictionary chứa các keys như 'timestamp', 'package_name',
                       'ids' (list), 'texts' (list), 'coords' (list), [có thể thêm 'class_names' nếu client gửi].

    Returns:
        Một dictionary đại diện cho trạng thái UI có cấu trúc, hoặc None nếu lỗi.
    """
    ...
    # Trích xuất dữ liệu (thêm các mảng khác nếu client gửi, ví dụ: class_names)
    ...
    # Ví dụ lấy thêm class_names nếu client gửi:
    class_names = ui_state_data.get('class_names', []) # Mảng class name tương ứng

    # Kiểm tra kiểu dữ liệu
    if not isinstance(ids, list) or not isinstance(texts, list) or \
       not isinstance(coords, list) or not isinstance(class_names, list): # Thêm class_names
         logger.error("LỖI (process_raw_ui_state): Dữ liệu ids, texts, coords, hoặc class_names không phải là list.")
         return None

    # Kiểm tra độ dài (cần khớp với tất cả các mảng bạn dùng)
    list_lengths = [len(ids), len(texts), len(coords), len(class_names)] # Thêm len(class_names)
    if len(set(list_lengths)) > 1: # Nếu có nhiều hơn 1 độ dài khác nhau
        expected_len = min(list_lengths) # Lấy độ dài ngắn nhất
        logger.warning(f"CẢNH BÁO (process_raw_ui_state): Độ dài các mảng không khớp: {list_lengths}. Sử dụng độ dài ngắn nhất: {expected_len}.")
    elif list_lengths:
        expected_len = list_lengths[0]
    else:
        expected_len = 0

    # Xây dựng danh sách elements
    structured_elements = []
    for i in range(expected_len):
        element = {
            "index": i, # Có thể bắt đầu từ 0 hoặc 1 tùy ý
            "resource_id": ids[i] if ids[i] is not None else None,
            "text": texts[i] if texts[i] is not None else None,
            "coordinates": _parse_coordinates(coords[i]), # Dùng hàm phụ trợ
            "class_name": class_names[i] if i < len(class_names) and class_names[i] else None, # Thêm class_name
            # ===>>> THÊM TRƯỜNG CLICKABLE <<<===
            "clickable": True # Mặc định là True vì client đã lọc "Only Clickable"
            # ===================================
            # Thêm các trường khác nếu client gửi (vd: content_description)
        }
    ...
    logger.info(f"INFO (process_raw_ui_state): Đã xử lý thành công dữ liệu UI thô. Tìm thấy {len(structured_elements)} elements (tất cả được coi là clickable).")
...
def plan_simple_exploration_action(app_name: str, current_screen_id: str) -> dict | None:
    ...
    try:
        ...
        if not screen_elements:
             logger.warning(f"No elements found in stored state for screen {current_screen_id}. Cannot plan click actions.")
        ...
        for element in screen_elements:
            ...
            if not action_already_tried:
                logger.info(f"Found unexplored clickable element: id='{element_id}', text='{element_text}'")
                next_action = {
                    "actionType": "click",
                ...
                if not next_action["targetElement"]:
                    logger.warning("Generated empty targetElement, skipping click action.")
                    next_action = None
        ...
        if not next_action:
             logger.info(f"No new clickable elements found for screen {current_screen_id}. Suggesting SWIPE_UP.")
