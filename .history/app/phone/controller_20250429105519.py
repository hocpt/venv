...
def process_phone_report(report_payload: dict, structured_ui_state_json: str | None = None) -> tuple[dict, int]:
    ...
    if logs and isinstance(logs, list) and len(logs) > 0:
        ...
        try:
            # **QUAN TRỌNG**: Hàm db.add_phone_action_logs cần được sửa
            # để nhận và lưu tham số structured_ui_state_json
            log_success = db.add_phone_action_logs(
                assignment_id=assignment_id, # Truyền assignment_id (có thể None)
                device_id=device_id,
                account_id=account_id,
                logs=logs,
                structured_ui_state_json=structured_ui_state_json # <<< Truyền chuỗi JSON state
            )
            if not log_success:
                logger.warning(f"Call to db.add_phone_action_logs returned False for assignment {assignment_id}.")
                # Quyết định xem có nên coi đây là lỗi nghiêm trọng không
            ...
            log_success = False # Đánh dấu lỗi nếu có exception
    else:
        logger.debug(f"No log entries provided in report for assignment {assignment_id}.")
