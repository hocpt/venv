...
def process_phone_report(report_data: dict) -> tuple[bool, str | None]:
    ...
    raw_ui_state = report_data.get('current_ui_state') # <<< Lấy dữ liệu UI thô (dict chứa các mảng)
    ...
    error_message = status_report.get('error_message') # Có thể là None
    progress_data = status_report.get('progress') # Dict chứa tiến độ, ví dụ: {'followers_gained': 1}
    final_result_data = status_report.get('result') # Dict chứa kết quả cuối cùng

    logger.info(f"Processing report for assignment {assignment_id}, device {device_id}, account {account_id}. Status: {current_status}")

    # === Bước 1: Xử lý Trạng thái UI thô (nếu client gửi lên) ===
    ...
    if raw_ui_state and isinstance(raw_ui_state, dict):
        ...
        # Gọi hàm xử lý đã tạo trước đó để chuyển đổi mảng thô -> cấu trúc JSON
        structured_state = process_raw_ui_state(raw_ui_state)
        if structured_state:
            try:
                # Chuyển dictionary Python thành chuỗi JSON để lưu vào CSDL
                structured_ui_state_json = json.dumps(structured_state, ensure_ascii=False) # ensure_ascii=False để giữ tiếng Việt
            except Exception as json_err:
                 logger.error(f"Lỗi chuyển đổi structured UI state thành JSON cho assignment {assignment_id}: {json_err}")
                 # Lưu thông báo lỗi vào DB thay vì state thật? Hoặc bỏ qua? Tạm thời lưu lỗi.
    ...
    # === Bước 2: Ghi Logs Hành động vào CSDL ===
    ...
    if logs and isinstance(logs, list) and len(logs) > 0:
        ...
        if not log_success:
            ...
            # Tùy thuộc vào mức độ quan trọng của log, có thể quyết định dừng lại ở đây hoặc không
            # return False, "Lỗi khi ghi log hành động."
    else:
        logger.debug(f"Không có logs nào trong báo cáo cho assignment {assignment_id}.")

    # === Bước 3: Cập nhật Trạng thái và Tiến độ của Assignment ===
    update_data = {}
    now_utc = datetime.now(timezone.utc) # Dùng UTC cho nhất quán
    update_data['last_report_at'] = now_utc # Luôn cập nhật thời gian báo cáo cuối

    # Cập nhật thời gian bắt đầu/kết thúc dựa trên status
    current_assignment_status = db.get_task_assignment_status(assignment_id) # Lấy status hiện tại từ DB
    if current_status == 'running' and current_assignment_status != 'running':
        # Chỉ cập nhật started_at nếu trạng thái trước đó chưa phải là running
        ...
        # Chỉ cập nhật completed_at nếu trạng thái trước đó chưa phải là trạng thái kết thúc
        update_data['completed_at'] = now_utc
        if final_result_data: # Lưu kết quả cuối cùng nếu client gửi lên
    ...
    if not status_update_success:
         # Lỗi này khá nghiêm trọng vì không cập nhật được trạng thái chính
         return False, f"Không thể cập nhật trạng thái cho assignment {assignment_id}."

    # Cập nhật tiến độ trong target_data (nếu client gửi progress)
    progress_update_success = True # Mặc định thành công nếu không có progress
    if progress_data and isinstance(progress_data, dict):
        ...
        if not progress_update_success:
             ...
             # Có thể không cần báo lỗi nghiêm trọng cho việc này

    # === Bước 4: Trả về Kết quả ===
    # Chỉ coi là hoàn toàn thành công nếu status và log (nếu có) đều được cập nhật
    final_success = status_update_success and log_success # Có thể bỏ qua progress_update_success nếu không quá quan trọng
    ...
    # Server có thể trả về thông tin bổ sung cho client nếu cần (ví dụ: yêu cầu dừng lại ngay lập tức)
    # response_payload = {"status": "success", "message": final_msg}
    # if some_condition:
    #     response_payload['action_required'] = 'stop_immediately'
