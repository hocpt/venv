    ...
    # --- THÊM LOG Ở ĐÂY ---
        ...
        for transition in action_sequence_raw:
            ...
            if transition_id is None:
                ...
                step_counter += 1 # Vẫn tăng counter để tránh lỗi ở lần lặp sau nếu dùng fallback này
                continue # Bỏ qua transition bị lỗi ID

            step = {
                'step_id': transition_id, # <<< Gán trực tiếp transition_id làm step_id
                'trigger': {'current_stage': transition.get('current_stage_id'), 'user_intent': transition.get('user_intent')},
                'next_stage_id': transition.get('next_stage_id')
            }
