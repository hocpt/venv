...
def process_phone_report(report_payload: dict, structured_ui_state_json: str | None = None) -> tuple[dict, int]:
    ...
    Hàm này giờ nhận report_payload (dict) và structured_ui_state_json (str | None).

    Args:
        report_payload: Dictionary chứa dữ liệu từ JSON request body của client.
                     Mong đợi có các key: 'device_id', 'account_id', 'assignment_id' (optional),
                     'status_report' (dict), 'logs' (list).
    ...
    logs = report_payload.get('logs', []) # Luôn là list
    logger.info(f"Controller processing report for Assignment: {assignment_id}...")
    ...
    if assignment_id_str is not None:
        try:
            ...
            # Không báo lỗi trả về client, nhưng assignment_id sẽ là None

    # --- Trích xuất thông tin từ status_report ---
    # Cung cấp giá trị mặc định an toàn
    current_status_from_client = status_report.get('current_status', 'unknown')
    error_message_from_client = status_report.get('error_message') # Có thể None
    progress_data = status_report.get('progress') # Có thể None hoặc dict
    final_result_data = status_report.get('result') # Có thể None hoặc dict
    ...
    # --- Bước 1: Ghi Logs Hành động vào CSDL PostgreSQL ---
    # Luôn cố gắng ghi log, kể cả khi không có assignment_id (ghi nhận hoạt động mapping)
    ...
    if (logs and isinstance(logs, list) and len(logs) > 0) or structured_ui_state_json:
            logger.debug(f"Attempting to log actions (count: {len(logs)}) and/or UI state (present: {structured_ui_state_json is not None}).")
            try:
                # Hàm db.add_phone_action_logs sẽ cần xử lý trường hợp logs=[] nhưng structured_ui_state_json có giá trị
                # <<< LOGGING POINT 2: Thêm log ở đây để xem có vào được không >>>
                logger.debug(f"Calling db.add_phone_action_logs for assignment {assignment_id}...")
    ...
    status_update_success = True # Mặc định thành công nếu không có assignment_id
    progress_update_success = True

    if assignment_id is not None:
        update_data = {} # Dict chứa các trường cần cập nhật cho assignment
        now_utc = datetime.now(timezone.utc)
        update_data['last_report_at'] = now_utc # Luôn cập nhật thời gian báo cáo cuối

        try:
            # Lấy trạng thái hiện tại của assignment trong DB để tránh ghi đè timestamp không cần thiết
            current_db_status = db.get_task_assignment_status(assignment_id)

            # Xác định xem có cần cập nhật started_at hoặc completed_at không
            ...
            if is_starting:
                ...
                logger.debug(f"Setting started_at for assignment {assignment_id}")
            if is_finishing:
                update_data['completed_at'] = now_utc
                logger.debug(f"Setting completed_at for assignment {assignment_id}")
                # Chỉ cập nhật result_data khi kết thúc
                if final_result_data and isinstance(final_result_data, dict):
                    update_data['result_data'] = final_result_data # Hàm DB sẽ tự xử lý JSONB
                elif error_message_from_client: # Nếu không có result nhưng có lỗi từ client
                    update_data['result_data'] = {'error': error_message_from_client}


            # Gọi hàm DB để cập nhật status và các timestamp/result
            status_update_success = db.update_assignment_status(
                assignment_id=assignment_id,
                new_status=current_status_from_client,
                **update_data # Truyền các trường cần cập nhật khác
            )
            if not status_update_success:
                 logger.error(f"Failed to update status for assignment {assignment_id} to '{current_status_from_client}'.")

            # Cập nhật progress (target_data) nếu có
            if progress_data and isinstance(progress_data, dict):
                # Hàm update_assignment_progress cần xử lý việc merge hoặc cập nhật JSONB
                progress_update_success = db.update_assignment_progress(assignment_id, progress_data)
                if not progress_update_success:
                     logger.warning(f"Failed to update progress (target_data) for assignment {assignment_id}.")

        except Exception as update_err:
             logger.error(f"Exception during assignment status/progress update for {assignment_id}: {update_err}", exc_info=True)
             status_update_success = False # Đánh dấu lỗi
    ...
    if final_success:
        ...
        # Quyết định thông báo lỗi chi tiết hơn nếu cần
        error_detail = "Failed to process report components (check server logs)."
        if not log_success: error_detail = "Failed to save action logs."
        ...
        # Có thể vẫn trả về 200 nếu chỉ là lỗi phụ, hoặc 500 nếu lỗi nghiêm trọng
        status_code = 500 if not status_update_success else 200 # Ví dụ: Coi lỗi update status là nghiêm trọng

    # Trả về tuple (dict, int)
...
def plan_simple_exploration_action(app_name: str, current_screen_id: str) -> dict | None:
    ...
    Logic: Tìm element có thể click đầu tiên CHƯA có transition tương ứng đi ra.
    """
    logger = current_app.logger if current_app else print
    log.info(f"Planning simple action for app '{app_name}', screen '{current_screen_id}'")

    if not graph_db or not app_name or not current_screen_id:
        log.error("plan_simple_exploration_action: Missing graph_db module, app_name, or screen_id.")
        return None

    next_action = None
    try:
        # 1. Lấy thông tin chi tiết màn hình hiện tại TỪ NEO4J
        screen_details = graph_db.get_screen_properties(current_screen_id, app_name)
        if not screen_details:
            log.warning(f"Could not find screen details in Neo4j for {current_screen_id} / {app_name}. Cannot plan.")
            return None # Không tìm thấy node màn hình

        # Lấy danh sách elements từ state đã lưu trong node
        processed_ui_state = screen_details.get('processed_ui_state') # Đây là dict đã parse
        screen_elements = processed_ui_state.get('elements', []) if processed_ui_state else []
        if not screen_elements:
             log.warning(f"No elements found in stored state for screen {current_screen_id}. Cannot plan click actions.")
             # Có thể đề xuất hành động khác như SWIPE hoặc BACK ở đây
             # return {"actionType": "swipe_up", "targetElement": None}

        # 2. Lấy các hành động đã thử từ màn hình này (các transition đi ra) TỪ NEO4J
        outgoing_transitions = graph_db.get_outgoing_transitions(current_screen_id, app_name)
        if outgoing_transitions is None: # Lỗi DB
             log.error(f"Failed to get outgoing transitions for {current_screen_id}. Aborting planning.")
             return None
        log.debug(f"Found {len(outgoing_transitions)} existing outgoing transitions from {current_screen_id}")

        # 3. Tìm hành động tiếp theo (Logic như cũ nhưng dùng dữ liệu thật)
        for element in screen_elements:
            # **Quan trọng:** Cần xác định element có 'clickable' hay không.
            # Logic này phụ thuộc vào dữ liệu bạn lấy từ AutoInput và lưu trong 'elements'.
            # Giả sử element là dict có key 'clickable' (boolean)
            is_clickable = element.get('clickable', False) # Cần đảm bảo client gửi thông tin này
            element_id = element.get('resource_id')
            element_text = element.get('text')
            element_class = element.get('class_name') # Lấy thêm class nếu có

            if not is_clickable: continue

            # Kiểm tra xem đã có transition nào khớp với việc click element này chưa
            action_already_tried = False
            for trans in outgoing_transitions:
                # So sánh chặt chẽ hơn, ưu tiên ID
                if trans.get('actionType') == 'click':
                    if element_id and trans.get('onElementId') == element_id:
                        action_already_tried = True; break
                         ...
                         # Nếu không có ID, so sánh text VÀ class (nếu có) để chính xác hơn
                         action_already_tried = True; break
                    # Có thể thêm các điều kiện so sánh khác (vd: content-desc)

            if not action_already_tried:
                log.info(f"Found unexplored clickable element: id='{element_id}', text='{element_text}'")
                next_action = {
                    "actionType": "click",
                    "targetElement": { # Bao gồm các thông tin client cần để tìm element
                        "resource_id": element_id,
                        "text": element_text,
                        "class_name": element_class # Thêm class
                        # Thêm các thuộc tính khác client dùng để tìm kiếm
                    }
                }
                # Loại bỏ key có giá trị None khỏi targetElement
                next_action["targetElement"] = {k:v for k,v in next_action["targetElement"].items() if v is not None}
                # Đảm bảo targetElement không rỗng
                if not next_action["targetElement"]:
                    log.warning("Generated empty targetElement, skipping click action.")
                    next_action = None
                    continue # Thử element khác

                break # Đã tìm thấy hành động -> dừng tìm kiếm

        # Nếu không tìm thấy hành động click nào mới
        if not next_action:
             log.info(f"No new clickable elements found for screen {current_screen_id}. Suggesting SWIPE_UP.")
             next_action = {"actionType": "swipe_up", "targetElement": None} # Ví dụ

    except Exception as e:
        log.error(f"Error during simple planning for screen {current_screen_id}: {e}", exc_info=True)
        next_action = None # Trả về None nếu có lỗi
