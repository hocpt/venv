...
# Hàm _parse_coordinates giữ nguyên
def _parse_coordinates(coord_str: str | None) -> dict | None:
    if not coord_str or ',' not in coord_str: return None
    try:
        x, y = map(int, coord_str.split(','))
        return {"x": x, "y": y}
    except (ValueError, TypeError) as e:
...
def plan_simple_exploration_action(app_name: str, current_screen_id: str) -> dict | None:
    """
    Quyết định hành động khám phá đơn giản tiếp theo từ một màn hình.
    Logic: Tìm element clickable chưa có transition click đi ra.
    Trả về cấu trúc nextAction chuẩn hóa với actionType='run_macro',
    bao gồm coordinates và randomness tùy chọn.
    """
    logger = current_app.logger if current_app else print
    logger.info(f"Planning simple action for app '{app_name}', screen '{current_screen_id}'")

    if not graph_db or not app_name or not current_screen_id:
        logger.error("plan_simple_exploration_action: Missing graph_db module, app_name, or screen_id.")
        return None

    next_action = None
    # --- Cấu hình Randomness (Có thể đặt trong config nếu muốn) ---
    ADD_RANDOM_DELAY = True # Bật/tắt thêm delay
    MIN_DELAY_MS = 150
    MAX_DELAY_MS = 600
    ADD_RANDOM_OFFSET = True # Bật/tắt thêm offset
    MAX_OFFSET_X = 4
    MAX_OFFSET_Y = 4
    # ---------------------------------------------------------

    try:
        # 1. Lấy thông tin màn hình từ Neo4j
        screen_details = graph_db.get_screen_properties(current_screen_id, app_name)
        if not screen_details:
            logger.warning(f"Could not find screen details in Neo4j for {current_screen_id} / {app_name}. Cannot plan.")
            return None
        ...
        if not screen_elements:
             logger.warning(f"No elements found in stored state for screen {current_screen_id}. Planning fallback.")
             # Sẽ fallback về swipe_up bên dưới

        # 2. Lấy các transition đã thử từ màn hình này
        outgoing_transitions = graph_db.get_outgoing_transitions(current_screen_id, app_name)
        if outgoing_transitions is None:
             logger.error(f"Failed to get outgoing transitions for {current_screen_id}. Aborting planning.")
             return None
        logger.debug(f"Found {len(outgoing_transitions)} existing outgoing transitions from {current_screen_id}")

        # 3. Tìm hành động click mới
        found_click_action = False
        for element in screen_elements:
            is_clickable = element.get('clickable', False)
            if not is_clickable: continue

            element_id = element.get('resource_id')
            element_text = element.get('text')
            element_class = element.get('class_name')
            element_coords = element.get('coordinates') # <<< Lấy tọa độ

            # Kiểm tra đã thử chưa
            action_already_tried = False
            for trans in outgoing_transitions:
                if trans.get('actionType') == 'click': # Chỉ kiểm tra với action click
                    if element_id and trans.get('onElementId') == element_id:
                        action_already_tried = True; break
                    elif element_text and trans.get('onElementText') == element_text and not element_id and trans.get('onElementClass') == element_class:
                         action_already_tried = True; break
            if action_already_tried: continue # Bỏ qua nếu đã thử

            # ===>>> TẠO TARGET ELEMENT DATA (Bổ sung coordinates) <<<===
            target_element_data = {
                "resource_id": element_id,
                "text": element_text,
                "class_name": element_class,
                "coordinates": element_coords # <<< Thêm tọa độ vào đây
            }
            ...
            if target_element_data:
                logger.info(f"Found unexplored clickable element: {target_element_data}")
                # ===>>> TẠO NEXT ACTION (Bổ sung Randomness) <<<===
                next_action = {
                    "actionType": "run_macro",
                    "macro_code": "UI_CLICK",
                    "params": {
                        "target": target_element_data # <<< Target giờ đã có coordinates
                    }
                }
                # Thêm randomness nếu được bật
                if ADD_RANDOM_DELAY:
                    next_action["random_delay_ms"] = {
                        "min": MIN_DELAY_MS,
                        "max": MAX_DELAY_MS
                    }
                # Chỉ thêm offset nếu target có tọa độ
                if ADD_RANDOM_OFFSET and element_coords:
                    next_action["random_offset_xy"] = {
                        "x_max": MAX_OFFSET_X,
                        "y_max": MAX_OFFSET_Y
                    }
                # ============================================
                ...
                logger.warning("Generated empty targetElement for a clickable element, skipping.")

        # Nếu không tìm thấy click action
        if not found_click_action:
             logger.info(f"No new clickable elements found. Suggesting SWIPE_UP.")
             # ===>>> TẠO NEXT ACTION SWIPE_UP (Bổ sung Randomness) <<<===
             next_action = {
                 "actionType": "run_macro",
                 "macro_code": "UI_SWIPE_UP",
                 "params": {}
             }
             if ADD_RANDOM_DELAY:
                  next_action["random_delay_ms"] = {
                      "min": int(MIN_DELAY_MS / 2),
                      "max": int(MAX_DELAY_MS / 2)
                  }
             # ============================================

    except Exception as e:
        logger.error(f"Error during simple planning for screen {current_screen_id}: {e}", exc_info=True)
        next_action = None

    if next_action: logger.debug(f"Final planned nextAction: {next_action}")
    else: logger.warning("Planner returning None (no action or error).")
...
def handle_explore_step(device_id: str, account_id: str,
                        current_screen_id_from_client: str | None,
                        raw_ui_state: dict,
                        previous_action: dict | None,
                        # Thêm các tham số khác nếu route truyền vào
                        ) -> tuple[dict, int]:
    """
    ...
    confirmed_screen_id = None # ID được xác nhận/tính toán
    next_action = None
    http_status_code = 500

    try:
        # --- Bước 1: Xử lý Raw UI State ---
        logger.debug(f"[Explore Step] Processing raw UI state...")
        processed_ui_state = process_raw_ui_state(raw_ui_state)
        if not processed_ui_state:
            raise ValueError("Failed to process raw UI state.") # Lỗi này nên dừng lại
        ...
        if not target_screen_id or target_screen_id.startswith("error_"):
            raise ValueError(f"Failed to determine a valid target screen ID (Result: {target_screen_id}).")
        confirmed_screen_id = target_screen_id # Đây chính là ID server xác nhận cho state này
        logger.info(f"[Explore Step] Determined Target Screen ID: {confirmed_screen_id}")

        # --- Bước 3: Lấy App Name ---
        app_name = processed_ui_state.get('package_name')
        if not app_name:
            # Thử lấy từ DB account (cần đảm bảo hàm get_app_name_from_account đáng tin cậy)
            logger.warning("[Explore Step] Missing package_name in UI state, trying DB lookup...")
            if db and hasattr(db, 'get_app_name_from_account'):
                app_name = db.get_app_name_from_account(account_id)
        if not app_name:
             raise ValueError("Cannot determine app_name from UI state or database.") # Lỗi nghiêm trọng

        # --- Bước 4: Ghi Log vào PostgreSQL (bao gồm state đã xử lý) ---
        logger.debug(f"[Explore Step] Logging action/state to PostgreSQL...")
        ui_state_for_log_saving = None
        try: ui_state_for_log_saving = json.dumps(processed_ui_state, ensure_ascii=False)
        except Exception as json_err: logger.error(f"Error serializing processed UI state for PG log: {json_err}")

        # Tạo một log entry "ảo" để ghi state, hoặc dùng log từ client nếu có
        # Giả sử chúng ta tạo 1 entry để ghi nhận việc xử lý state này
        placeholder_log_entry = {
            "timestamp": processed_ui_state.get("timestamp", datetime.now(timezone.utc).isoformat()),
            "macro": "SYSTEM_PROCESS_STATE", # Mã đặc biệt
            "status": "success"
            # Thêm các trường khác nếu muốn
        }
        # Hàm add_phone_action_logs cần xử lý assignment_id=None
        log_pg_success = db.add_phone_action_logs(
            assignment_id=None, # Mapping không thuộc assignment cụ thể
            device_id=device_id,
            account_id=account_id,
            logs=[placeholder_log_entry], # Chỉ ghi log placeholder này
            structured_ui_state_json=ui_state_for_log_saving
        )
        if not log_pg_success:
            logger.warning("[Explore Step] Failed to write state log to PostgreSQL. Continuing...")
            # Có thể không cần dừng lại nếu chỉ là lỗi ghi log

        # --- Bước 5: Cập nhật Neo4j Đồng bộ ---
        # 5a. Cập nhật Node Đích
        logger.debug(f"[Explore Step] Updating Neo4j Screen Node: {confirmed_screen_id}...")
        screen_props = { # Chuẩn bị props cho node
            ...
            "rawStateSample": ui_state_for_log_saving # Lưu state đã xử lý
        }
        if not graph_db.create_or_update_screen_node(confirmed_screen_id, app_name, screen_props):
             raise ConnectionError(f"Failed to create/update target screen node {confirmed_screen_id} in Neo4j.") # Lỗi kết nối Neo4j

        # 5b. Cập nhật Cạnh Transition (Nếu có previous_action)
        if previous_action and isinstance(previous_action, dict):
            ...
            if source_context and action_details:
                ...
                if source_screen_id:
                    logger.debug(f"[Explore Step] Updating Neo4j Transition: {source_screen_id} -> {confirmed_screen_id} via {action_details.get('actionType')}...")
                    if not graph_db.create_or_update_transition_relationship(source_screen_id, confirmed_screen_id, app_name, action_details):
                         raise ConnectionError(f"Failed to create/update transition edge from {source_screen_id} to {confirmed_screen_id} in Neo4j.")
                else: logger.warning("[Explore Step] previous_action missing source_screen_id.")
            else: logger.warning("[Explore Step] previous_action has invalid structure.")

        # --- Bước 6: Gọi Planner (Sau khi Neo4j đã cập nhật) ---
        logger.debug(f"[Explore Step] Calling planner for confirmed screen: {confirmed_screen_id}...")
        next_action = plan_simple_exploration_action(app_name, confirmed_screen_id) # Hàm planner như cũ
        ...
        if next_action:
            ...
            http_status_code = 200 # Vẫn là 200 OK

    except ValueError as ve: # Lỗi validate dữ liệu hoặc xử lý state/ID
        logger.error(f"[Explore Step] Value Error: {ve}", exc_info=True)
        final_status = "error"
        final_message = str(ve)
        http_status_code = 400 # Bad request nếu lỗi từ input/logic
    except ConnectionError as ce: # Lỗi khi cập nhật Neo4j
        logger.error(f"[Explore Step] Neo4j Update Error: {ce}", exc_info=True)
        final_status = "error"
        final_message = f"Failed to update graph database: {ce}"
        http_status_code = 503 # Service unavailable (Neo4j error)
    except Exception as e: # Các lỗi không mong muốn khác
        logger.error(f"[Explore Step] Unexpected Error: {e}", exc_info=True)
        traceback.print_exc() # In traceback ra console để debug
        final_status = "error"
        final_message = "Internal server error during explore step processing."
        http_status_code = 500
    ...
    response_body = {
        ...
        "confirmedCurrentScreenId": confirmed_screen_id, # Luôn trả về ID đã xác nhận (có thể None nếu lỗi sớm)
        "nextAction": next_action # Là object hoặc None
    }
    logger.info(f"[Explore Step] Responding with status '{final_status}', screenId '{confirmed_screen_id}', action '{next_action.get('macro_code') if next_action else 'None'}'")
