...
def plan_simple_exploration_action(app_name: str, current_screen_id: str) -> dict | None:
    ...
    Trả về cấu trúc nextAction chuẩn hóa, BỔ SUNG coordinates và randomness.
    """
    logger = current_app.logger if current_app else print
    ...
    try:
        ...
        for element in screen_elements:
            ...
            # Kiểm tra đã thử chưa (logic như cũ)
        ...
        if not found_click_action:
             ...
             next_action = {
                 ...
                 "params": {} # Có thể thêm duration nếu macro hỗ trợ
             }
             # Chỉ thêm delay cho swipe
             if ADD_RANDOM_DELAY:
                  next_action["random_delay_ms"] = {
                      "min": int(MIN_DELAY_MS / 2), # Có thể delay ít hơn cho swipe
    ...
    # Log hành động cuối cùng trước khi trả về
    if next_action:
        logger.debug(f"Final planned nextAction: {next_action}")
    else:
        logger.warning("Planner returning None (no action or error).")
