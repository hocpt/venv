...
def plan_simple_exploration_action(app_name: str, current_screen_id: str,
                                   previous_action_context: dict | None) -> dict | None: # <<< Thêm tham số
    """
    Quyết định hành động khám phá tiếp theo, cố gắng tránh lặp lại hành động
    ...
    try:
        ...
        # 2. Kiểm tra xem có phải vừa bị lặp không
        is_loop = False
        ...
        if previous_action_context:
            ...
            previous_action_details = previous_action_context.get('action_details')
            if source_context:
                previous_source_id = source_context.get('screenId')
                if previous_source_id == current_screen_id:
                    is_loop = True
                    logger.info(f"Planner: Detected potential loop on screen {current_screen_id} from previous action.")

        # 3. Tìm hành động click mới (ưu tiên element có resource_id)
        found_click_action = False
        if screen_elements: # Chỉ tìm nếu có elements
            for element in screen_elements:
                ...
                if not click_tried:
                    # Nếu đang bị lặp VÀ hành động lặp trước đó là click chính element này -> Bỏ qua
                    if is_loop and previous_action_details and \
                       previous_action_details.get('actionType') == 'click' and \
                       previous_action_details.get('onElementId') == element_id:
                        logger.debug(f"Planner: Skipping element ID {element_id} because clicking it caused the current loop.")
                        continue # Bỏ qua, tìm element click khác

                    # Đề xuất click element này
                    element_text = element.get('text')
                    element_class = element.get('class_name')
                    element_coords = element.get('coordinates')
                    logger.info(f"Planner: Found unexplored clickable element with ID: {element_id}")

                    target_element_data = {"resource_id": element_id, "text": element_text, "class_name": element_class, "coordinates": element_coords}
                    ...
                    else: logger.warning("Planner: Generated empty targetElement, skipping.")
        ...
        if not found_click_action:
            ...
            if not swipe_up_tried:
                ...
                # Có thể thêm delay cho Back nếu muốn
            else:
                logger.warning(f"Planner: No new actions (clicks, swipe, back) found for screen {current_screen_id}. Returning no_action.")
