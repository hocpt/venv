...
def plan_simple_exploration_action(app_name: str, current_screen_id: str,
    ...
    try:
        ...
        # --- Kiểm tra lặp và LẤY ĐÚNG previous_target_id ---
        is_loop = False
        ...
        if previous_action_context:
            ...
            action_details_from_prev = previous_action_context.get('action_details') # Đây là dict chứa actionType, onElementId...
            if source_context and action_details_from_prev:
                ...
                if previous_source_id == current_screen_id:
                    ...
                    # <<< SỬA DÒNG NÀY >>>
                    # Lấy trực tiếp 'onElementId' từ action_details của hành động trước
                    if previous_action_details.get('actionType') == 'click':
                         previous_target_id = previous_action_details.get('onElementId') # <<< Lấy đúng key
                    # <<< KẾT THÚC SỬA >>>
                    logger.info(f"Planner: Detected loop on screen {current_screen_id}. Previous action: {previous_action_details}. Previous target ID: {previous_target_id}") # Log này giờ sẽ có ID

        # --- Tìm hành động click mới (logic còn lại giữ nguyên) ---
        found_click_action = False
        if screen_elements:
            for element in screen_elements:
                element_id = element.get('resource_id')
                if not element_id: continue # Heuristic: Chỉ click element có ID

                # Kiểm tra click_tried như cũ
                click_tried = False
                for trans in outgoing_transitions:
                    if trans.get('actionType') == 'click' and trans.get('onElementId') == element_id:
                        click_tried = True; break

                if not click_tried:
                    # ===>>> KIỂM TRA TRÁNH LẶP (Giờ sẽ hoạt động đúng vì previous_target_id có giá trị) <<<===
                    ...
                    # Đề xuất click element này (logic tạo next_action như cũ)
                    # ... (lấy text, class, coords) ...
                    logger.info(f"Planner: Found new unexplored clickable element: id='{element_id}'...") # Log gọn hơn
                    target_element_data = {"resource_id": element_id, ...} # Thêm text, class, coords
                    target_element_data = {k:v for k,v in target_element_data.items() if v is not None}
                    if target_element_data:
                        next_action = { "actionType": "run_macro", "macro_code": "UI_CLICK", "params": { "target": target_element_data }}
                        # ... (thêm randomness) ...
                        found_click_action = True
                        break

        # --- Xử lý Fallback (logic như cũ) ---
        if not found_click_action:
            # ... (kiểm tra swipe_up_tried, go_back_tried -> đề xuất swipe/back hoặc None) ...
            ...
            if not swipe_up_tried:
                logger.info(f"Planner: Suggesting SWIPE_UP (untried).")
                next_action = { "actionType": "run_macro", "macro_code": "UI_SWIPE_UP", "params": {} }
                # ... (thêm randomness) ...
            elif not go_back_tried:
                logger.info(f"Planner: Suggesting NAV_GO_BACK (untried).")
                next_action = { "actionType": "run_macro", "macro_code": "NAV_GO_BACK", "params": {} }
            else:
                logger.warning(f"Planner: No new actions found for screen {current_screen_id}. Returning no_action.")
                next_action = None

    # ... (except và return như cũ) ...
    except Exception as e:
