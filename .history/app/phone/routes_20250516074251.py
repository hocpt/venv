...
def upload_screenshot():
    ...
    if 'file' not in request.files:
        logger.warning("UPLOAD API (FINAL): No 'file' part in request.files")
        return jsonify({"success": False, "error": "Không có phần file nào trong request"}), 400
    
    uploaded_file_storage = request.files['file']
    
    # Đây là tên file mà client đã báo trước đó (ví dụ: uuid.png)
    # và cũng là tên file sẽ được lưu trên server.
    ...
    if not client_declared_filename:
        logger.warning("UPLOAD API (FINAL): Thiếu 'filename' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'filename'"}), 400
    
    if not app_name_from_form:
        logger.warning("UPLOAD API (FINAL): Thiếu 'app_name' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'app_name'"}), 400

    if uploaded_file_storage.filename == '':
        logger.warning("UPLOAD API (FINAL): Không có file nào được chọn (filename rỗng từ request.files).")
        return jsonify({"success": False, "error": "Không có file nào được chọn"}), 400

    # An toàn hóa tên file và tên thư mục app_name
    ...
    if not final_filename_to_save_on_server or not safe_app_name_as_folder:
        logger.error(f"UPLOAD API (FINAL): Tên file '{client_declared_filename}' hoặc app_name '{app_name_from_form}' không hợp lệ sau khi xử lý.")
        return jsonify({"success": False, "error": "Tên file hoặc app_name không hợp lệ."}), 400
    ...
    if not storage_base_path:
        logger.critical("UPLOAD API (FINAL): SCREENSHOT_STORAGE_PATH không được cấu hình trong server!")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (storage path)."}), 500
    
    logger.info(f"UPLOAD API (FINAL): Base storage path from config: '{storage_base_path}'")
    ...
    logger.info(f"UPLOAD API (FINAL): App specific storage directory target: '{app_specific_storage_directory}'")
    ...
    try:
        ...
        if not os.path.exists(app_specific_storage_directory):
            ...
            logger.info(f"UPLOAD API (FINAL): Đã tạo thư mục: {app_specific_storage_directory}")
    except OSError as e:
        logger.error(f"UPLOAD API (FINAL): Không thể tạo thư mục '{app_specific_storage_directory}': {e}", exc_info=True)
    ...
    logger.info(f"UPLOAD API (FINAL): Attempting to save file to: '{full_path_to_save_file}'")
    
    try:
        uploaded_file_storage.save(full_path_to_save_file)
        logger.info(f"UPLOAD API (FINAL): SUCCESS - Saved image to: {full_path_to_save_file}")
        return jsonify({
            "success": True, 
        ...
        logger.error(f"UPLOAD API (FINAL): FAILED to save file '{final_filename_to_save_on_server}' to '{app_specific_storage_directory}': {e}", exc_info=True)
