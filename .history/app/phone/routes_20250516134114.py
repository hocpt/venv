...
@phone_bp.route('/api/upload/screenshot', methods=['POST']) # Đảm bảo URL này client gọi
def upload_screenshot_from_client(): # Đây là hàm bạn đang gặp lỗi
    ...
    logger.debug(f"PHONE_UPLOAD_API: Request Form Data: {request.form.to_dict()}")
    logger.debug(f"PHONE_UPLOAD_API: Request Files Keys: {list(request.files.keys())}")

    # --- SỬ DỤNG ĐÚNG TÊN TRƯỜNG MÀ CLIENT GỬI ---
    # Dựa trên "anh.txt" của bạn, client sử dụng trường "screenshot_file"
    expected_file_field_name = 'screenshot_file' 
                                   # Nếu client thực sự gửi là 'file', thì đổi lại thành 'file'

    if expected_file_field_name not in request.files:
        logger.warning(f"PHONE_UPLOAD_API: No '{expected_file_field_name}' part in request.files. Available file keys in request: {list(request.files.keys())}")
        return jsonify({"success": False, "error": f"Không tìm thấy phần file với tên '{expected_file_field_name}' trong request."}), 400
    
    uploaded_file_storage = request.files[expected_file_field_name] # SỬ DỤNG ĐÚNG TÊN TRƯỜNG
    
    # Tên file mà client đã thông báo trước đó (ví dụ: uuid.png)
    # Đây sẽ là tên file cuối cùng được lưu trên server.
    client_declared_filename = request.form.get('filename') 
    app_name_from_client = request.form.get('app_name')

    logger.debug(f"PHONE_UPLOAD_API: Form data - filename: '{client_declared_filename}', app_name: '{app_name_from_client}'")
    # uploaded_file_storage.filename là tên file gốc từ máy client, không nhất thiết phải dùng để lưu
    logger.debug(f"PHONE_UPLOAD_API: Uploaded file object - original name from client's disk: '{uploaded_file_storage.filename}'")


    if not client_declared_filename:
        logger.warning("PHONE_UPLOAD_API: Missing 'filename' form parameter (expected name for server storage).")
        return jsonify({"success": False, "error": "Missing 'filename' parameter in form data"}), 400
    
    if not app_name_from_client:
        logger.warning("PHONE_UPLOAD_API: Missing 'app_name' form parameter.")
        return jsonify({"success": False, "error": "Missing 'app_name' parameter in form data"}), 400

    # Kiểm tra xem có file thực sự được upload không, dựa vào tên file mà client gửi trong form 'filename'
    # uploaded_file_storage.filename có thể rỗng nếu không có file được chọn, nhưng form vẫn được gửi
    if uploaded_file_storage.filename == '' and not client_declared_filename:
        logger.warning("PHONE_UPLOAD_API: No file selected or no 'filename' provided by client.")
        return jsonify({"success": False, "error": "No file selected or invalid filename"}), 400

    # An toàn hóa tên file (dựa trên client_declared_filename) và tên thư mục app_name
    final_filename_to_save_on_server = secure_filename(os.path.basename(client_declared_filename))
    safe_app_name_as_folder = secure_filename(app_name_from_client)

    if not final_filename_to_save_on_server or not safe_app_name_as_folder:
        logger.error(f"PHONE_UPLOAD_API: Invalid filename or app_name after sanitization. Original fn: '{client_declared_filename}', app: '{app_name_from_client}'")
    ...
    if not storage_base_path:
        ...
        return jsonify({"success": False, "error": "Server configuration error (storage path missing)."}), 500
    ...
    app_specific_storage_directory = os.path.join(storage_base_path, safe_app_name_as_folder)
    ...
    full_path_to_save_file = os.path.join(app_specific_storage_directory, final_filename_to_save_on_server)
    ...
    try:
        ...
        if os.path.exists(full_path_to_save_file) and os.path.getsize(full_path_to_save_file) > 0:
            ...
            return jsonify({
                ...
                "debug_saved_path": full_path_to_save_file 
            }), 201
        else:
        ...
        logger.error(f"PHONE_UPLOAD_API: EXCEPTION during save of file '{final_filename_to_save_on_server}' to '{app_specific_storage_directory}': {e}", exc_info=True)
    ...
    uploaded_file = request.files['screenshot_file']
    ...
    logger.info("UPLOAD_API: Received new upload request.")

    if 'file' not in request.files:
        logger.warning("UPLOAD_API: No 'file' part in request.files.")
        return jsonify({"success": False, "error": "Không có phần file nào trong request"}), 400
    ...
    # Đây sẽ là tên file cuối cùng được lưu trên server.
    client_declared_filename = request.form.get('filename') 
    app_name_from_form = request.form.get('app_name')

    logger.debug(f"UPLOAD_API: Received form data - filename: '{client_declared_filename}', app_name: '{app_name_from_form}'")
    logger.debug(f"UPLOAD_API: Received file object - original filename: '{uploaded_file_storage.filename}'")


    if not client_declared_filename:
        logger.warning("UPLOAD_API: Thiếu tham số 'filename' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'filename'"}), 400
    
    if not app_name_from_form:
        logger.warning("UPLOAD_API: Thiếu tham số 'app_name' trong form data.")
        return jsonify({"success": False, "error": "Thiếu tham số 'app_name'"}), 400

    if uploaded_file_storage.filename == '' and not client_declared_filename: # Nếu cả hai đều rỗng
        logger.warning("UPLOAD_API: Không có file nào được chọn và không có filename được cung cấp.")
        return jsonify({"success": False, "error": "Không có file nào được chọn hoặc tên file không hợp lệ"}), 400

    # Ưu tiên tên file từ form 'filename' vì nó phải khớp với Neo4j
    final_filename_to_save_on_server = secure_filename(os.path.basename(client_declared_filename))
    safe_app_name_as_folder = secure_filename(app_name_from_form) # An toàn hóa tên thư mục

    if not final_filename_to_save_on_server or not safe_app_name_as_folder:
        logger.error(f"UPLOAD_API: Tên file '{client_declared_filename}' hoặc app_name '{app_name_from_form}' không hợp lệ sau khi secure_filename.")
        return jsonify({"success": False, "error": "Tên file hoặc app_name không hợp lệ sau khi xử lý."}), 400

    # Lấy đường dẫn lưu trữ gốc từ config
    storage_base_path = current_app.config.get('SCREENSHOT_STORAGE_PATH')
    if not storage_base_path:
        logger.critical("UPLOAD_API: SCREENSHOT_STORAGE_PATH không được cấu hình trong server!")
        return jsonify({"success": False, "error": "Lỗi cấu hình server (storage path)."}), 500
    
    logger.info(f"UPLOAD_API: Configured SCREENSHOT_STORAGE_PATH: '{storage_base_path}'")

    # Đường dẫn đến thư mục con của app: STORAGE_PATH / APP_NAME /
    app_specific_storage_directory = os.path.join(storage_base_path, safe_app_name_as_folder)
    logger.info(f"UPLOAD_API: Target app specific storage directory: '{app_specific_storage_directory}'")
    
    try:
        # Tạo thư mục con cho app_name nếu chưa tồn tại
        if not os.path.exists(app_specific_storage_directory):
            os.makedirs(app_specific_storage_directory, exist_ok=True)
            logger.info(f"UPLOAD_API: Đã tạo thư mục: {app_specific_storage_directory}")
    except OSError as e:
        logger.error(f"UPLOAD_API: Không thể tạo thư mục '{app_specific_storage_directory}': {e}", exc_info=True)
        return jsonify({"success": False, "error": f"Lỗi tạo thư mục lưu trữ trên server."}), 500
    
    # Đường dẫn đầy đủ để lưu file ảnh: STORAGE_PATH / APP_NAME / FILENAME
    full_path_to_save_file = os.path.join(app_specific_storage_directory, final_filename_to_save_on_server)
    logger.info(f"UPLOAD_API: Attempting to save file to: '{full_path_to_save_file}'")
    
    try:
        uploaded_file_storage.save(full_path_to_save_file)
        # Kiểm tra lại file đã được lưu chưa
        if os.path.exists(full_path_to_save_file):
            logger.info(f"UPLOAD_API: SUCCESS - Saved image to: {full_path_to_save_file}")
            return jsonify({
                "success": True, 
                "message": "Upload ảnh thành công.", 
                "saved_server_path_debug": full_path_to_save_file 
            }), 201
        else:
            logger.error(f"UPLOAD_API: FAILED - File not found after save attempt at: {full_path_to_save_file}")
            return jsonify({"success": False, "error": "Lỗi server: Không thể xác nhận file đã được lưu."}), 500
    except Exception as e:
        logger.error(f"UPLOAD_API: FAILED to save file '{final_filename_to_save_on_server}' to '{app_specific_storage_directory}': {e}", exc_info=True)
        if os.path.exists(full_path_to_save_file):
            try: os.remove(full_path_to_save_file) 
            except: pass
        return jsonify({"success": False, "error": "Lỗi server khi lưu file ảnh."}), 500