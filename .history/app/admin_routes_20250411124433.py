...
# Thử import lại scheduler trực tiếp từ __init__.py cùng cấp
try:
    from . import scheduler
except ImportError:
     print("WARNING (admin_routes): Không thể import scheduler từ '.' (__init__.py). Thao tác live với scheduler sẽ thất bại.")
     scheduler = None
# ... (import db, ...) ...
...
def view_scheduled_jobs():
    """Hiển thị danh sách các Job đã cấu hình trong DB và trạng thái live (nếu có)."""
    db_jobs_config = [] # Danh sách cấu hình từ DB
    live_jobs_status = {} # Dictionary lưu trạng thái job đang chạy (key là job_id)
    # <<< THÊM LOG DEBUG Ở ĐÂY >>>
    print("\nDEBUG: Inside view_scheduled_jobs")
    print(f"DEBUG: Type of current_app: {type(current_app)}")
    print(f"DEBUG: Attributes on current_app (use dir()): {dir(current_app)}")
    # Kiểm tra xem 'extensions' có tồn tại và scheduler có trong đó không
    extensions = getattr(current_app, 'extensions', None)
    print(f"DEBUG: current_app.extensions: {extensions}")
    if extensions and isinstance(extensions, dict):
         print(f"DEBUG: 'scheduler' key in current_app.extensions: {extensions.get('scheduler')}")
    # <<< KẾT THÚC LOG DEBUG >>>
    
    # 1. Lấy cấu hình jobs từ Database
    try:
        db_jobs_config = db.get_all_job_configs() # Hàm này lấy từ bảng scheduled_jobs
        if db_jobs_config is None: # Phân biệt lỗi DB (None) và không có job (list rỗng)
             flash("Lỗi khi tải danh sách cấu hình jobs từ Database.", "error")
             db_jobs_config = [] # Đặt lại là list rỗng để template không lỗi
    except Exception as e:
        print(f"Lỗi nghiêm trọng khi load job configs từ DB: {e}")
        flash("Lỗi không mong muốn khi tải cấu hình jobs từ DB.", "error")
        db_jobs_config = []
    live_scheduler = current_app.scheduler
    print(f"DEBUG: Successfully accessed current_app.scheduler: {live_scheduler}")
    # 2. Lấy trạng thái jobs thực tế từ Scheduler đang chạy
    if live_scheduler: # Chỉ thực hiện nếu import scheduler thành công
        try:
            running_jobs = live_scheduler.get_jobs() # Lấy danh sách các job object đang chạy
            for job in running_jobs:
                # Lưu thời gian chạy tiếp theo (next_run_time là None nếu job đang bị pause)
            ...
            print(f"DEBUG: Lấy được trạng thái của {len(live_jobs_status)} jobs từ scheduler.")
             ...
             # Lỗi này không nên làm dừng trang, chỉ báo warning
    ...
    # 3. Render template, truyền cả cấu hình từ DB và trạng thái live
    return render_template('admin_scheduled_jobs.html',
                            title="Quản lý Tác vụ nền",
                            jobs=db_jobs_config, # Danh sách cấu hình từ DB
                            live_jobs_status=live_jobs_status) # Dict trạng thái từ scheduler

@admin_bp.route('/scheduled-jobs/add', methods=['GET', 'POST'])
def add_scheduled_job():
    if request.method == 'POST':
        # ... (Lấy dữ liệu form, validate JSON cơ bản như cũ) ...
        ...
        if not job_id or not function_path or not trigger_type:
             # ... (flash lỗi, return template như cũ) ...
        ...
        # Validate function path (giữ nguyên)
        func = None
        try:
             module_path, func_name = function_path.rsplit('.', 1)
             module = importlib.import_module(module_path)
             func = getattr(module, func_name)
             if not callable(func): raise AttributeError(...)
        except (ValueError, ImportError, AttributeError) as import_err:
             # ... (flash lỗi function path, return template như cũ) ...
             return render_template('admin_add_scheduled_job.html', ...), 400
        ...
        live_scheduler = current_app.scheduler
        if is_enabled and live_scheduler:
            # <<< BỌC try...except quanh add_job để bắt lỗi trigger args >>>
            try:
            ...
            except (TypeError, ValueError) as trigger_err: # Bắt lỗi sai tham số trigger
            ...
            except Exception as e: # Các lỗi khác của scheduler
        ...
        if not scheduler_error and db_success: # Chỉ flash success nếu cả DB và scheduler (nếu có) đều ổn
    ...
    if request.method == 'POST':
        ...
        if not trigger_type:
            ...
            return render_template('admin_edit_scheduled_job.html',
                       title=f"Sửa Tác vụ '{job_id}'", # Đảm bảo có title=
                       job=job_details,               # Đảm bảo có job=
        ...
        if not db_success:
             # ... (flash lỗi DB, return template như cũ) ...
             return render_template('admin_edit_scheduled_job.html', ...)

        # --- Cập nhật Scheduler đang chạy ---
        scheduler_error = None
        live_scheduler = current_app.scheduler
        print(f"\nDEBUG (edit_scheduled_job): Checking scheduler before interaction for job '{job_id}'...")
        print(f"DEBUG: Scheduler object accessible in route: {live_scheduler}")
        if live_scheduler:
                     try:
                         all_live_jobs = live_scheduler.get_jobs()
                         print(f"DEBUG: Jobs currently in scheduler instance: {[job.id for job in all_live_jobs]}")
                     except Exception as e:
                         print(f"DEBUG: Error getting jobs from scheduler in route: {e}")
        else:
                    print("DEBUG: current_app.scheduler is None or not available.")
        
        if live_scheduler:
             # <<< BỌC try...except quanh reschedule_job và pause/resume >>>
            try:
                all_live_jobs = live_scheduler.get_jobs()
                print(f"Attempting to update job '{job_id}' in live scheduler...")
                # 1. Reschedule (thử áp dụng trigger mới)
                live_scheduler.modify_job(
                    job_id, # <--- job_id đứng đầu, không có tên keyword
                    trigger=trigger_type,
                    **trigger_args_dict # Các thay đổi khác giữ dạng kwargs
                )
                print(f"Modified job '{job_id}' trigger/args.")
                # 2. Pause/Resume dựa trên is_enabled
                live_job = live_scheduler.get_job(job_id) # Lấy lại job sau khi reschedule
                is_currently_paused = (live_job.next_run_time is None) if live_job else True
                print(f"DEBUG: Jobs currently in scheduler instance: {[job.id for job in all_live_jobs]}")
                if is_enabled and is_currently_paused:
                     print(f"Resuming job '{job_id}'...")
                     live_scheduler.resume_job(job_id)
                elif not is_enabled and not is_currently_paused:
                     print(f"Pausing job '{job_id}'...")
                     live_scheduler.pause_job(job_id)
                print(f"Successfully updated job '{job_id}' in live scheduler.")

            except (TypeError, ValueError) as trigger_err: # Bắt lỗi sai tham số trigger khi reschedule
                 ...
                 flash(scheduler_error, "warning")
            except JobLookupError: # Bắt lỗi nếu job không tồn tại trong scheduler
                 ...
                 flash(scheduler_error, "warning")
                 # Nếu không tìm thấy, có thể thử add lại?
                 # try: scheduler.add_job(...) except ...
...
def delete_scheduled_job(job_id):
    ...
    if live_scheduler:
        try:
            ...
            live_scheduler.remove_job(job_id, missing_ok=True) # missing_ok=True để không lỗi nếu job đã bị xóa hoặc chưa tồn tại
...
def toggle_scheduled_job(job_id):
    ...
    if scheduler: # Kiểm tra xem import có thành công không
            try:
                print(f"DEBUG (toggle_scheduled_job): Checking scheduler: {scheduler}")
                all_live_jobs = scheduler.get_jobs() # <<< Dùng scheduler
                print(f"DEBUG: Live jobs: {[j.id for j in all_live_jobs]}")
    
                live_job = scheduler.get_job(job_id) # <<< Dùng scheduler
                if not live_job:
                     raise JobLookupError(job_id)
    
                if new_enabled_state:
                    print(f"Resuming job '{job_id}'...")
                    scheduler.resume_job(job_id) # <<< Dùng scheduler
                    flash(f"Đã kích hoạt (resume) job '{job_id}'.", 'success')
                else:
                    print(f"Pausing job '{job_id}'...")
                    scheduler.pause_job(job_id) # <<< Dùng scheduler
                    flash(f"Đã tạm dừng (pause) job '{job_id}'.", 'info')
    
            except JobLookupError:
                 # ... (Xử lý lỗi JobLookupError như cũ) ...
                 scheduler_error = f"Lưu trạng thái DB thành công, nhưng không tìm thấy job '{job_id}' trong scheduler để bật/tắt."
                 print(f"WARNING: {scheduler_error}")
                 flash(scheduler_error, "warning")
            except Exception as e:
                 # ... (Xử lý lỗi Exception như cũ) ...
