...
def get_pie_conditions_from_db(app_name: str, defined_screen_id: str) -> list | None:
    """
    Fetches the 'conditions' for a given PIE definition.
    Returns a list of conditions if found, an empty list if PIE exists but has no conditions,
    or None if the PIE definition itself is not found.
    """
    pie_definition = get_pie_definition_by_screen_id(app_name, defined_screen_id)
    if pie_definition:
        conditions = pie_definition.get('conditions')
        if conditions is None: # PIE exists, but conditions field is NULL
            return []
        if isinstance(conditions, str): # If stored as a JSON string (less likely with JSONB)
            try:
                return json.loads(conditions)
            except json.JSONDecodeError:
                current_app.logger.error(f"Invalid JSON in conditions for {app_name} - {defined_screen_id}")
                return [] # Or handle error differently
        return conditions # Assumed to be a list of dicts if JSONB and psycopg2 handles it
    return None # PIE definition not found

def update_pie_conditions_in_db(app_name: str, defined_screen_id: str, new_conditions_list: list) -> tuple[bool, str | None]:
    """
    Updates the 'conditions' field for a specific PIE definition.
    Args:
        app_name: The name of the application.
        defined_screen_id: The defined_screen_id of the PIE to update.
        new_conditions_list: The new list of condition objects.
                             This list will be stored as JSONB.
    Returns:
        A tuple (success: bool, error_message: str | None).
    """
    conn = None
    try:
        conn = get_db_connection() # Sử dụng hàm kết nối hiện có của bạn
        if not conn:
            current_app.logger.error("Không thể lấy kết nối CSDL trong update_pie_conditions_in_db.")
            return False, "Lỗi kết nối cơ sở dữ liệu."

        with conn.cursor() as cur:
            # Chuyển đổi new_conditions_list thành JSON string để lưu vào cột JSONB
            # psycopg2 thường có thể xử lý Python list/dict trực tiếp cho cột JSONB,
            # nhưng để chắc chắn, bạn có thể chuyển thành JSON string.
            # Nếu psycopg2 của bạn hỗ trợ trực tiếp, bạn có thể truyền new_conditions_list.
            conditions_json_to_store = json.dumps(new_conditions_list)

            sql = """
                UPDATE screen_definitions
                SET conditions = %s, updated_at = CURRENT_TIMESTAMP
                WHERE app_name = %s AND screen_id = %s;
            """
            cur.execute(sql, (conditions_json_to_store, app_name, defined_screen_id))
            
            if cur.rowcount == 0:
                # Không có dòng nào được cập nhật, nghĩa là PIE không tồn tại
                conn.rollback() # Không cần thiết nếu không có thay đổi, nhưng để an toàn
                current_app.logger.warning(f"Không tìm thấy PIE để cập nhật conditions: app='{app_name}', screen_id='{defined_screen_id}'.")
                return False, f"Không tìm thấy Định nghĩa PIE với app_name '{app_name}' và screen_id '{defined_screen_id}'."
            
            conn.commit()
            current_app.logger.info(f"Đã cập nhật conditions cho PIE: app='{app_name}', screen_id='{defined_screen_id}'.")
            return True, None

    except psycopg2.Error as e:
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi CSDL trong update_pie_conditions_in_db cho {app_name}/{defined_screen_id}: {e}", exc_info=True)
        return False, f"Lỗi cơ sở dữ liệu: {e}"
    except json.JSONDecodeError as je: # Nếu bạn dùng json.dumps và có lỗi (ít khả năng với list of dicts)
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi JSON khi chuẩn bị conditions cho {app_name}/{defined_screen_id}: {je}", exc_info=True)
        return False, f"Lỗi định dạng dữ liệu điều kiện: {je}"
    except Exception as ex:
        if conn:
            conn.rollback()
        current_app.logger.error(f"Lỗi không mong muốn trong update_pie_conditions_in_db cho {app_name}/{defined_screen_id}: {ex}", exc_info=True)
        return False, f"Lỗi máy chủ không mong muốn: {ex}"
    finally:
        if conn:
            conn.close() # Đóng kết nối nếu hàm này tự quản lý
...
def check_defined_screen_id_exists(app_name: str, defined_screen_id: str) -> bool:
    ...
    Hàm này đã được sửa để dùng đúng tên cột 'defined_screen_id'.
    """
    ...
    exists = False
    try:
        conn = get_db_connection() # Hàm helper của bạn để lấy connection
        if not conn:
            # logger.error("DB_CHECK_PIE_ID: Không thể kết nối CSDL.")
            return False # Hoặc ném Exception tùy theo cách xử lý lỗi của bạn

        cur = conn.cursor()
        # Sử dụng đúng tên cột 'defined_screen_id'
        ...
        # Tùy thuộc vào yêu cầu, bạn có thể ném lỗi ở đây hoặc trả về False/True một cách thận trọng
        return False # Giả sử không tồn tại nếu có lỗi để tránh ghi đè không mong muốn, hoặc ném lỗi
    except Exception as e:
...
def create_new_pie_definition_from_node(
    ...
    logical_name: str, # Đây là tên logic người dùng nhập, sẽ được lưu vào cột logical_screen_name
    new_defined_screen_id: str, # Đây là ID định danh người dùng nhập, lưu vào defined_screen_id
    conditions: list[dict], # Danh sách các điều kiện từ frontend
    description: str | None
) -> tuple[bool, str | None, int | None]:
    """
    Tạo một định nghĩa PIE (Screen Definition) mới trong cơ sở dữ liệu PostgreSQL.
    Bao gồm cả việc lưu các điều kiện nhận diện (identifying_elements).

    Args:
        app_name: Tên ứng dụng.
        activity_name: Tên activity (có thể None).
        logical_name: Tên logic cho màn hình (ví dụ: "Trang chủ", "Hồ sơ người dùng").
        new_defined_screen_id: ID duy nhất do người dùng định nghĩa cho màn hình này trong app.
        conditions: List các dictionary chứa các điều kiện PIE.
        description: Mô tả cho PIE (có thể None).

    Returns:
        Tuple (success: bool, error_message: str | None, new_definition_id: int | None)
    """
    logger = current_app.logger
    conn = None
    cur = None
    ...
    # Sử dụng hàm check_defined_screen_id_exists đã sửa lỗi tên cột
    ...
    try:
        ...
        # Chuyển đổi list conditions thành chuỗi JSON để lưu vào CSDL
        # Cột identifying_elements trong DB nên có kiểu JSON hoặc JSONB
        ...
        # Câu lệnh SQL INSERT - Đã sửa tên cột và loại bỏ dấu ; thừa
        # Giả sử cột primary key của bạn là 'definition_id' và nó tự tăng.
        # Giả sử tên cột cho "Tên Logic Màn hình" là 'logical_screen_name'.
        # Giả sử tên cột cho "ID Định danh" là 'defined_screen_id'.
        # Giả sử tên cột cho "Điều kiện nhận diện" là 'identifying_elements'.
        sql = """
            INSERT INTO screen_definitions 
                (app_name, activity_name, logical_screen_name, defined_screen_id, 
                 identifying_elements, description, created_at, updated_at) 
            VALUES (%s, %s, %s, %s, %s, %s, NOW(), NOW())
            RETURNING definition_id; 
        """
        ...
        params = (
            app_name,
            activity_name, # Có thể là None
            logical_name,  # Giá trị từ tham số logical_name
            new_defined_screen_id, # Giá trị từ tham số new_defined_screen_id
            identifying_elements_json_str, # Chuỗi JSON của conditions, có thể là None nếu conditions rỗng/lỗi
            description    # Có thể là None
        )
        ...
        if result_row and result_row[0] is not None:
            ...
            conn.rollback() # Quan trọng: rollback nếu không lấy được ID
        ...
        # Bắt lỗi vi phạm ràng buộc UNIQUE (ví dụ: (app_name, defined_screen_id) phải là duy nhất)
        conn.rollback()
        ...
        # Trả về thông báo lỗi thân thiện hơn, có thể bao gồm một phần của db_err.pgerror nếu an toàn
        # Ví dụ: error_msg = f"Lỗi cơ sở dữ liệu khi tạo PIE: {db_err.diag.message_primary if hasattr(db_err, 'diag') else str(db_err)}"
        error_msg = f"Lỗi cơ sở dữ liệu khi tạo PIE: {str(db_err)}" # Giữ thông báo lỗi gốc từ psycopg2
        return False, error_msg, None
    except Exception as e:
        if conn:
            conn.rollback()
