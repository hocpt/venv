...
def add_scheduled_job():
    ...
    if request.method == 'POST':
        ...
        # <<< THÊM BƯỚC CHUYỂN ĐỔI KIỂU DỮ LIỆU SỐ >>>
        ...
        for key in numeric_keys:
            if key in trigger_args_dict and isinstance(trigger_args_dict[key], str):
                ...
                if not value_str: # Nếu là chuỗi rỗng thì bỏ qua hoặc coi là None/0 tùy logic
                    # del trigger_args_dict[key] # Hoặc gán None/0
                    continue
                try:
                    # Thử chuyển thành int trước, nếu không được thì float
                    if '.' in value_str:
                        trigger_args_dict[key] = float(value_str)
                    else:
                        trigger_args_dict[key] = int(value_str)
                except ValueError:
                    flash(f"Giá trị cho '{key}' ('{value_str}') trong Trigger Args phải là một số hợp lệ.", "warning")
                    return render_template('admin_add_scheduled_job.html', ...), 400
        ...
        try:
             ...
             if not callable(func):
                  raise AttributeError("Function path không trỏ đến một hàm.")
        ...
        if is_enabled and scheduler:
            try:
                ...
                scheduler.add_job(id=job_id, func=func, trigger=trigger_obj, replace_existing=True) # <<< Dùng trigger_obj
                print(f"Successfully added job '{job_id}' to live scheduler.")
            except (TypeError, ValueError) as trigger_err: # Bắt lỗi sai tham số trigger TỪ APSCHEDULER
                 scheduler_error = f"Lưu DB thành công, nhưng Trigger Args không hợp lệ cho Trigger Type '{trigger_type}': {trigger_err}. Job sẽ không chạy cho đến khi được sửa."
             ...
             flash("Đã lưu cấu hình job vào DB, nhưng không thể thêm vào scheduler (scheduler chưa được khởi tạo?).", "warning")

        if not scheduler_error and db_success:
...
def edit_scheduled_job(job_id):
    ...
    # Lấy dữ liệu job hiện tại từ DB để hiển thị form GET
    # và để dùng lại nếu POST bị lỗi validation
    ...
    if request.method == 'POST':
            # ... (Lấy dữ liệu form, validate JSON như cũ) ...
            trigger_type = request.form.get('trigger_type', '').strip()
            trigger_args_str = request.form.get('trigger_args', '{}').strip()
            is_enabled_str = request.form.get('is_enabled')
            is_enabled = True if is_enabled_str == 'on' else False
            ...
            # <<< THÊM BƯỚC CHUYỂN ĐỔI KIỂU DỮ LIỆU SỐ (GIỐNG HET NHƯ TRONG add_scheduled_job) >>>
            numeric_keys = ['weeks', 'days', 'hours', 'minutes', 'seconds', 'jitter', 'year', 'month', 'day', 'week', 'hour', 'minute', 'second']
            for key in numeric_keys:
                if key in trigger_args_dict and isinstance(trigger_args_dict[key], str):
                    value_str = trigger_args_dict[key].strip()
                    ...
                    try:
                        ...
                        flash(f"Giá trị cho '{key}' ('{value_str}') trong Trigger Args phải là một số hợp lệ.", "warning")
                        # Truyền lại job_details và current_data
                        return render_template('admin_edit_scheduled_job.html', ... current_data=request.form), 400
        # <<< KẾT THÚC BƯỚC CHUYỂN ĐỔI >>>
            ...
            scheduler_error = None
            if scheduler:
                # <<< BỌC TOÀN BỘ KHỐI TƯƠNG TÁC SCHEDULER TRONG TRY...EXCEPT >>>
                try:
                    print(f"Attempting to update job '{job_id}' in live scheduler...")
    
                    # <<< 1. TẠO ĐỐI TƯỢNG TRIGGER MỚI >>>
                    trigger_obj = None
                    if trigger_type == 'interval':
                        # Chuyển đổi các giá trị số nếu cần (APScheduler thường làm được việc này)
                        trigger_obj = IntervalTrigger(**trigger_args_dict)
                    elif trigger_type == 'cron':
                         # CronTrigger nhận cả chuỗi hoặc số cho các trường như hour, minute...
                        trigger_obj = CronTrigger(**trigger_args_dict)
                    elif trigger_type == 'date':
                        # DateTrigger cần datetime hoặc chuỗi ISO
                        trigger_obj = DateTrigger(**trigger_args_dict)
                    else:
                         # Trường hợp không nên xảy ra do đã validate trigger_type
                         raise ValueError(f"Loại trigger không được hỗ trợ: {trigger_type}")
    
                    # <<< 2. GỌI modify_job VỚI TRIGGER OBJECT >>>
                    scheduler.modify_job(
                        job_id,
                        trigger=trigger_obj # <<< Truyền đối tượng trigger đã tạo >>>
                        # Không truyền **trigger_args_dict ở đây nữa
                    )
                    print(f"Modified job '{job_id}' trigger/args.")
    
                    # 3. Pause/Resume dựa trên is_enabled (Giữ nguyên logic này)
                    ...
                    if live_job:
                        ...
                        print(f"Successfully updated job '{job_id}' enable status in live scheduler.")
                    elif is_enabled:
                        # Job được bật trong DB nhưng không có trong scheduler -> Thử thêm lại?
                        # (Logic thêm lại job như trước - cần import func...)
                        print(f"WARNING: Job '{job_id}' not found in scheduler after DB update. Attempting to add it...")
                        try:
                            job_details = db.get_job_config_details(job_id) # Lấy lại details vừa lưu
                            ...
                            else: print(f"ERROR: Cannot re-add job '{job_id}', missing function path.")
                        except Exception as re_add_err:
                             print(f"ERROR: Failed to re-add job '{job_id}' to scheduler after edit: {re_add_err}")
                             scheduler_error = f"Lỗi khi cố gắng thêm lại job '{job_id}' vào scheduler sau khi sửa."
                             # Flash lỗi này ở cuối
    
                # <<< BẮT LỖI TỪ VIỆC TẠO TRIGGER HOẶC MODIFY/PAUSE/RESUME >>>
                except (TypeError, ValueError) as trigger_err: # Bắt lỗi sai tham số trigger từ APScheduler
                                 scheduler_error = f"Lưu DB thành công, nhưng Trigger Args/Type không hợp lệ khi cập nhật scheduler: {trigger_err}."
                                 print(f"ERROR updating job '{job_id}' in scheduler: {scheduler_error}")
                                 flash(scheduler_error, "warning")
                     # Flash lỗi này ở cuối
                except JobLookupError:
                     scheduler_error = f"Lưu DB thành công, nhưng không tìm thấy job '{job_id}' trong scheduler đang chạy để cập nhật/pause/resume."
                 ...
                 scheduler_error = "Đã cập nhật cấu hình job trong DB, nhưng không thể cập nhật scheduler (chưa khởi tạo?)."
    
            # Flash thông báo cuối cùng
            if scheduler_error:
                flash(scheduler_error, "warning")
            elif db_success: # Chỉ flash success nếu không có lỗi scheduler nào
                flash(f"Cập nhật tác vụ '{job_id}' thành công!", 'success')
            # else trường hợp db_success là False đã được xử lý ở trên
    
            return redirect(url_for('admin.view_scheduled_jobs'))
        

    # GET request (job_details đã được lấy ở đầu)
