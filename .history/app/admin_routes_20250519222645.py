    ...
    try:
        ...
        with driver.session(database=db_name_neo4j) as session:
            ...
            for record in nodes_result:
                ...
                current_node_app_name = record.get("app_name_from_node") # Giả sử query Neo4j trả về app_name của Node là app_name_from_node
                                                                         # Hoặc bạn đang dùng app_name là tham số của API này
                ...
                if screenshot_filename:
                    try:
                        ...
                        generated_screenshot_url = url_for(
                            'serve_screenshot_for_app',
                            app_name=current_node_app_name,
...
def _convert_neo4j_datetime_to_iso(dt_obj):
    """Chuyển đổi đối tượng neo4j.time.DateTime hoặc Python datetime thành chuỗi ISO."""
    if dt_obj is None:
        return None
    ...
    # Fallback cho các trường hợp khác, hoặc nếu bạn muốn xử lý kiểu dữ liệu không mong muốn
    logger = current_app.logger
