...
def admin_screen_elements(screen_id):
    ...
    elements_list = []          # List element cuối cùng để vẽ bảng HTML
    ...
    try:
        # --- Bước 1: Lấy thông tin Screen từ Neo4j (bao gồm elements_data_json) ---
        logger.debug(f"[Route] Truy vấn Neo4j để lấy thông tin screen: {screen_id}")
        # Dùng hàm đã sửa/đổi tên để lấy cả elements_data_json
        screen_data_neo4j = graph_db.get_screen_details_with_elements_json(screen_id)

        if not screen_data_neo4j:
            flash(f"Không tìm thấy Screen node với ID '{screen_id}' trong Neo4j.", 'warning')
            return redirect(url_for('admin.admin_mapping_viewer')) # Redirect về trang map

        screen_data = screen_data_neo4j # Gán dữ liệu để dùng trong template
        app_name = screen_data.get('app_name')
        logger.debug(f"[Route] Lấy được thông tin Screen từ Neo4j. App Name: {app_name}")

        # --- Bước 2: Tạo URL Screenshot ---
        ...
        # === Bước 3: Lấy và Parse elements_list TỪ Neo4j ===
        elements_data_json_from_neo4j = screen_data.get('elements_data_json')
        elements_from_neo4j = [] # Khởi tạo list rỗng
        if elements_data_json_from_neo4j:
            try:
                elements_from_neo4j = json.loads(elements_data_json_from_neo4j)
                if not isinstance(elements_from_neo4j, list):
                    logger.warning("[Route] elements_data_json từ Neo4j không phải là list.")
                    elements_from_neo4j = []
                else:
                    logger.info(f"[Route] Parse được {len(elements_from_neo4j)} elements từ thuộc tính node Neo4j.")
            except json.JSONDecodeError as json_err:
                logger.error(f"[Route] Lỗi parse elements_data_json từ Neo4j: {json_err}")
                elements_from_neo4j = []
        else:
            logger.warning(f"[Route] Thuộc tính 'elements_data_json' không có hoặc rỗng trong node Neo4j {screen_id}.")

        # --- Bước 4: Lấy classifications từ PostgreSQL ---
        ...
        if saved_classifications is None:
            logger.error(f"[Route] Lỗi khi lấy classifications từ DB cho screen {screen_id}.")
            saved_classifications = {}
        else:
            logger.info(f"[Route] Lấy được {len(saved_classifications)} classifications đã lưu từ DB.")

        # --- Bước 5: Lấy trạng thái explored từ Neo4j Transitions ---
        ...
        if app_name:
            ...
            if outgoing_transitions is None:
                logger.error(f"[Route] Lỗi khi lấy outgoing transitions từ Neo4j.")
                outgoing_transitions = []
            else:
                logger.info(f"[Route] Lấy được {len(outgoing_transitions)} outgoing transitions từ Neo4j.")
                # ... (logic build tried_element_ids như cũ) ...
            ...
            logger.warning(f"[Route] Không có app_name cho screen {screen_id}, không thể lấy transitions.")

        # --- Bước 6: Hợp nhất dữ liệu và tạo final_elements_list, processed_elements_for_json ---
        logger.debug(f"[Route] Bắt đầu hợp nhất dữ liệu element...")
        final_elements_list = [] # Dữ liệu đầy đủ cho bảng HTML
        processed_elements_for_json = [] # Dữ liệu tối giản cho Javascript overlay

        # Lặp qua dữ liệu element lấy TỪ NEO4J (không cần lấy từ log PG nữa)
        for el_base in elements_from_neo4j:
            el_id = el_base.get('element_id')
            if not el_id: continue # Bỏ qua nếu thiếu ID

            element_saved_data = saved_classifications.get(el_id, {})
            classification = element_saved_data.get('classification', 'unclassified')
            manual_override = element_saved_data.get('override')
            ...
            # Tạo entry đầy đủ cho bảng HTML
            element_for_table = {**el_base, **{
                'classification': classification,
                'display_explored_status': display_explored_status,
                'override_active': override_active,
                # Lấy thêm count từ neo4j element properties nếu có
                'attempt_count': el_base.get('attempt_count', 0),
                'success_count': el_base.get('success_count', 0)
            }}
            final_elements_list.append(element_for_table)

            # Tạo entry tối giản cho JSON (chỉ cần id, type, text, coords, bounds)
            element_for_json = {
                'id': el_id,
                'type': el_base.get('element_type'),
                'text': el_base.get('text_content'),
                'coords': el_base.get('coordinates'),
                'bounds': el_base.get('bounds')
            }
            processed_elements_for_json.append({k:v for k,v in element_for_json.items() if v is not None})

        elements_list = final_elements_list # Gán list cuối cùng cho template bảng
        logger.info(f"[Route] Hợp nhất xong, có {len(elements_list)} elements để hiển thị.")

        # --- Bước 7: Tạo chuỗi JSON elements_json_data từ processed_elements_for_json ---
        if processed_elements_for_json:
            try:
                elements_json_data = json.dumps(processed_elements_for_json)
                logger.debug(f"[Route] Tạo thành công elements_json_data (length: {len(elements_json_data)})")
            except Exception as e_json:
                logger.error(f"[Route] Lỗi serialize processed_elements_for_json: {e_json}", exc_info=True)
                elements_json_data = '[]'
                flash("Lỗi xử lý dữ liệu JSON cho Javascript.", "error")
        else:
            elements_json_data = '[]'
            logger.warning("[Route] processed_elements_for_json rỗng.")

        # --- Bước 8: Lấy valid_classifications (như cũ) ---
        ...
        # (Tùy chọn) Lấy kích thước màn hình gốc nếu được lưu ở đâu đó
        # original_width = screen_data.get('original_width')
        # original_height = screen_data.get('original_height')
    ...
    return render_template(
        ...
        elements=elements_list, # List Python cho bảng
        valid_classifications=valid_classifications,
        screenshot_url=screenshot_url,
        original_width=original_width, # Truyền kích thước gốc
        original_height=original_height, # Truyền kích thước gốc
        elements_json_data=elements_json_data # Chuỗi JSON cho script
    )
