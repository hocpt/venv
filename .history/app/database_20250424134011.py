...
def get_stages_by_type(strategy_type: str) -> list[dict] | None:
    ...
    Dùng để lọc dropdown "Next Stage" trong form Add/Edit Transition.

    Args:
        strategy_type: Loại strategy cần lọc ('control', 'language', 'mainloop').

    Returns:
        List các dictionary chứa stage_id và description, hoặc None nếu lỗi.
    """
    logger = current_app.logger if current_app else print
    if not strategy_type or strategy_type not in ['language', 'control', 'mainloop']:
        ...
        return None # Hoặc trả về list rỗng tùy logic
    ...
    try:
        ...
        sql = """
            ...
            ORDER BY ss.strategy_id, ss.stage_id; -- Sắp xếp để dễ nhìn trong dropdown
        """
