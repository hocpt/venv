...
def get_all_rules() -> list[dict] | None:
    ...
    try:
        ...
        if cur: cur.close()
        if conn: conn.close()
    return get_filtered_rules(filters=None) # Trả về list (có thể rỗng) hoặc None nếu lỗi DB
...
def get_all_simulation_configs() -> list[dict] | None:
    """Lấy danh sách tất cả các cấu hình mô phỏng đã lưu."""
    configs = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # Lấy các cột cần thiết để hiển thị danh sách
        sql = """
        ...
        print(f"ERROR (db - get_all_simulation_configs): DB Error: {e}")
    except Exception as e:
        print(f"ERROR (db - get_all_simulation_configs): Unexpected error: {e}")
    finally:
        if cur: cur.close()
        if conn: conn.close()
    # Trả về list (có thể rỗng) hoặc None nếu lỗi
    return configs if configs is not None else None
...
def get_filtered_templates_with_details(filter_ref: str | None = None, filter_category: str | None = None) -> list[dict] | None:
    """Lấy danh sách template refs cùng details và variation count, có bộ lọc."""
    templates_data = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        ...
        # Thêm điều kiện lọc
        if filter_ref:
            # Tìm kiếm không phân biệt hoa thường, chứa chuỗi filter_ref
            where_clauses.append("rt.template_ref ILIKE %s")
            params.append(f"%{filter_ref}%")
        if filter_category:
            # Tìm kiếm chính xác category
        ...
        print(f"DEBUG (db - get_filtered_templates): SQL: {base_sql}, PARAMS: {params}")
        cur.execute(base_sql, tuple(params)) # <<< Truyền params vào execute
        rows = cur.fetchall()
        templates_data = [dict(row) for row in rows] if rows else []

    except psycopg2.Error as db_err:
        print(f"LỖI (db - get_filtered_templates): Truy vấn thất bại: {db_err}")
        templates_data = None # Lỗi thì trả về None
    except Exception as e:
        ...
        if cur: cur.close()
        if conn: conn.close()
    return templates_data # Trả về list (có thể rỗng) hoặc None
...
def get_filtered_rules(filters: dict = None) -> list[dict] | None:
    """Lấy danh sách luật đã lọc từ simple_rules."""
    rules_list = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    try:
        ...
        base_sql += " ORDER BY rule_id;" # Sắp xếp theo ID

        print(f"DEBUG SQL (get_filtered_rules): {cur.mogrify(base_sql, tuple(params)).decode('utf-8', 'ignore')}")
        # print(f"DEBUG PARAMS (get_filtered_rules): {params}")

        cur.execute(base_sql, tuple(params))
        rows = cur.fetchall()
        rules_list = [dict(row) for row in rows] if rows else []
        print(f"DEBUG (get_filtered_rules): Fetched {len(rules_list)} rules.")
