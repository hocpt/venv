...
def add_strategy():
    ...
    # --- Xử lý POST request ---
    if request.method == 'POST':
        # ... (lấy strategy_id, name, description, strategy_type như cũ) ...
        strategy_id = request.form.get('strategy_id', '').strip()
        name = request.form.get('name', '').strip()
        description = request.form.get('description', '').strip()
        initial_stage_id = request.form.get('initial_stage_id') # Vẫn lấy giá trị (có thể là "" hoặc ID)
        strategy_type = request.form.get('strategy_type')

        # Validate dữ liệu
        errors = []
        # <<< Bỏ initial_stage_id khỏi kiểm tra bắt buộc này >>>
        if not strategy_id or not name or not strategy_type:
            errors.append("Strategy ID, Name, và Strategy Type (ẩn) là bắt buộc.")
        # <<< Kiểm tra strategy_type vẫn giữ nguyên >>>
        ...
        if errors:
            ...
            return render_template('admin_add_strategy.html',
                                   title=f"Thêm {strategy_type.capitalize()} Strategy Mới (Lỗi)", # Hiển thị type đúng
                                   stages=stages,
