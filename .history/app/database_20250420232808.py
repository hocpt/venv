...
def update_strategy(strategy_id: str, name: str, description: str | None, initial_stage_id: str) -> tuple[bool, str | None]:
    """
    Cập nhật tên, mô tả, stage bắt đầu cho một chiến lược.
    KHÔNG cập nhật strategy_type.
    """
    print(f"DEBUG (db.update_strategy): Attempting to update strategy '{strategy_id}' (excluding type)")

    # <<< Kiểm tra các tham số bắt buộc (không còn strategy_type) >>>
    if not strategy_id: return False, "Strategy ID là bắt buộc."
    if not name: return False, "Tên chiến lược (Name) là bắt buộc."
    if not initial_stage_id: return False, "Initial Stage ID là bắt buộc."
    ...
    try:
        cur = conn.cursor()
        # <<< Câu lệnh UPDATE không còn SET strategy_type >>>
        sql = """
            UPDATE public.strategies
            SET name = %s, description = %s, initial_stage_id = %s, updated_at = NOW()
            WHERE strategy_id = %s;
        """
        # <<< Tuple params chỉ còn 4 giá trị >>>
        params = (name, description, initial_stage_id, strategy_id)

        print(f"DEBUG SQL (update_strategy): {cur.mogrify(sql, params).decode('utf-8','ignore')}")
        cur.execute(sql, params)
        conn.commit()
        success = cur.rowcount > 0
        if not success:
             error_msg = f"Không tìm thấy strategy_id '{strategy_id}' để cập nhật hoặc không có gì thay đổi."
             print(f"WARNING (db.update_strategy): {error_msg}")

    except psycopg2.IntegrityError as int_err:
         error_msg = f"Lỗi ràng buộc CSDL (Tên '{name}' đã tồn tại?): {int_err}"
         print(f"ERROR (db.update_strategy): {error_msg}\n{traceback.format_exc()}")
         if conn: conn.rollback()
    except psycopg2.Error as db_err:
        error_msg = f"Lỗi CSDL khi cập nhật strategy: {db_err}"
        print(f"ERROR (db.update_strategy): {error_msg}\n{traceback.format_exc()}")
        if conn: conn.rollback()
    except Exception as e:
        ...
        if cur: cur.close()
        if conn: conn.close()
        # print("DEBUG (db.update_strategy): Database connection closed.")
...
def add_new_transition(current_stage_id: str, user_intent: str,
                       ...
                       response_template_ref: str | None = None, # Dùng cho Language
                       action_macro_code: str | None = None, # Dùng cho Control
                       action_params_str: str | None = None, # Dùng cho Control
                       condition_type: str | None = None,    # Dùng cho Control
                       condition_value: str | None = None,   # Dùng cho Control
                       # --- THAM SỐ LOOP MỚI ---
                       loop_type: str | None = None,
                       loop_count: int | None = None,
                       loop_condition_type: str | None = None,
                       loop_condition_value: str | None = None,
                       loop_target_selector_str: str | None = None, # Nhận chuỗi JSON
                       loop_variable_name: str | None = None
                       ) -> tuple[bool, str | None]:
    """Thêm transition mới, xử lý cả language, control, và loop."""
    print(f"DEBUG (db.add_new_transition): current='{current_stage_id}', intent='{user_intent}', loop='{loop_type}'")
    if not current_stage_id or not user_intent:
        return False, "Current Stage và User Intent là bắt buộc."

    # --- Xử lý action_to_suggest (cho Control) ---
    action_json_for_db = None
    if action_macro_code and action_macro_code.strip():
        params_dict = {}
        # Chỉ parse params nếu action_params_str không rỗng và khác '{}'
        if action_params_str and action_params_str.strip() and action_params_str != '{}':
            try:
                params_dict = json.loads(action_params_str)
                if not isinstance(params_dict, dict): raise ValueError("Params phải là JSON object.")
    ...
    # --- Xử lý loop_target_selector (cho Control - ForEach) ---
    loop_selector_json_to_save = None # Sẽ lưu NULL hoặc JSON
    if loop_target_selector_str and loop_target_selector_str.strip() and loop_target_selector_str != '{}':
        try:
             # Validate JSON
             loaded_json = json.loads(loop_target_selector_str)
             loop_selector_json_to_save = json.dumps(loaded_json) # Lưu lại dạng chuỗi JSON chuẩn
        except json.JSONDecodeError as json_err:
             return False, f"Loop Target Selector không phải JSON hợp lệ: {json_err}"


    template_ref_to_save = response_template_ref if response_template_ref and response_template_ref.strip() else None
    ...
    try:
        cur = conn.cursor()
        # <<< THÊM CÁC CỘT loop_* VÀO INSERT >>>
        sql = """
            INSERT INTO public.stage_transitions (
                current_stage_id, user_intent, condition_type, condition_value,
                next_stage_id, priority, action_to_suggest, response_template_ref,
                loop_type, loop_count, loop_condition_type, loop_condition_value,
                loop_target_selector, loop_variable_name
            ) VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb, %s, %s, %s, %s, %s, %s::jsonb, %s); -- Ép kiểu JSONB
        """
        params = (
            current_stage_id, user_intent,
            condition_type if condition_type else None,
            condition_value if condition_value else None,
            next_stage_id if next_stage_id else None,
            priority,
            json.dumps(action_json_for_db) if action_json_for_db else None, # Lưu action JSON hoặc NULL
            template_ref_to_save,
            # --- Giá trị cho các cột loop mới ---
            loop_type if loop_type else None,
            loop_count, # Giá trị INTEGER hoặc None
            loop_condition_type if loop_condition_type else None,
            loop_condition_value if loop_condition_value else None,
            loop_selector_json_to_save, # Chuỗi JSON hoặc None
            loop_variable_name if loop_variable_name else None
        )
        print(f"DEBUG SQL (add_new_transition): PARAMS = {params}") # Log Params để kiểm tra
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.Error as e:
        error_msg = f"Lỗi CSDL khi thêm transition: {e}"
...
def update_transition(transition_id: int, current_stage_id: str, user_intent: str,
                      ...
                      response_template_ref: str | None, # Dùng cho Language
                      action_macro_code: str | None, # Dùng cho Control
                      action_params_str: str | None, # Dùng cho Control
                      condition_type: str | None,    # Dùng cho Control
                      condition_value: str | None,   # Dùng cho Control
                      # --- THAM SỐ LOOP MỚI ---
                      loop_type: str | None = None,
                      loop_count: int | None = None,
                      loop_condition_type: str | None = None,
    ...
    """Cập nhật một transition, bao gồm cả các trường loop."""
    print(f"DEBUG (db.update_transition): Updating transition ID={transition_id}, loop='{loop_type}'")
    if not transition_id or not current_stage_id or not user_intent:
        return False, "ID, Current Stage và User Intent là bắt buộc."

    # --- Xử lý action_to_suggest (tương tự như add) ---
    action_json_for_db = None
    if action_macro_code and action_macro_code.strip():
        params_dict = {}
        if action_params_str and action_params_str.strip() and action_params_str != '{}':
    ...
    # --- Xử lý loop_target_selector (tương tự như add) ---
    loop_selector_json_to_save = None
    if loop_target_selector_str and loop_target_selector_str.strip() and loop_target_selector_str != '{}':
        try:
            loaded_json = json.loads(loop_target_selector_str)
            loop_selector_json_to_save = json.dumps(loaded_json)
        except json.JSONDecodeError as json_err:
    ...
    try:
        cur = conn.cursor()
        # <<< THÊM CÁC CỘT loop_* VÀO UPDATE >>>
        sql = """
            UPDATE public.stage_transitions SET
                current_stage_id = %s, user_intent = %s, condition_type = %s, condition_value = %s,
                next_stage_id = %s, priority = %s, action_to_suggest = %s::jsonb, response_template_ref = %s,
                loop_type = %s, loop_count = %s, loop_condition_type = %s, loop_condition_value = %s,
                loop_target_selector = %s::jsonb, loop_variable_name = %s
            WHERE transition_id = %s; -- Ép kiểu JSONB
        """
        params = (
            current_stage_id, user_intent,
            condition_type if condition_type else None,
        ...
        print(f"DEBUG SQL (update_transition): PARAMS = {params}") # Log Params
        cur.execute(sql, params)
        conn.commit()
        success = cur.rowcount > 0 # True nếu có dòng được cập nhật
        if not success:
...
def get_transition_details(transition_id: int) -> dict | None:
    ...
    try:
        ...
        sql = """
            ...
            LEFT JOIN
                public.strategy_stages ss ON t.current_stage_id = ss.stage_id
            WHERE
                t.transition_id = %s;
        """
        print(f"DEBUG SQL (get_transition_details): {cur.mogrify(sql, (transition_id,)).decode('utf-8','ignore')}")
        cur.execute(sql, (transition_id,))
        row = cur.fetchone()
        if row:
            details = dict(row)
            # Xử lý action_to_suggest và loop_target_selector để hiển thị trên form edit
            if isinstance(details.get('action_to_suggest'), dict):
                details['action_macro_code'] = details['action_to_suggest'].get('macro_code')
                params_dict = details['action_to_suggest'].get('params', {})
                try: details['action_params_str'] = json.dumps(params_dict, indent=2, ensure_ascii=False) if params_dict else '{}'
                except TypeError: details['action_params_str'] = str(params_dict)
            else:
                details['action_macro_code'] = None
                details['action_params_str'] = '{}'

            if details.get('loop_target_selector') is not None:
                 try: details['loop_target_selector_str'] = json.dumps(details['loop_target_selector'], indent=2, ensure_ascii=False)
                 except TypeError: details['loop_target_selector_str'] = str(details['loop_target_selector'])
            else: details['loop_target_selector_str'] = '' # Trả về chuỗi rỗng nếu NULL

            print(f"DEBUG DB (get_transition_details): Found details for {transition_id} (incl. loop): {details}")
        else:
            print(f"WARN (db.get_transition_details): Không tìm thấy transition ID {transition_id}.")
    except psycopg2.Error as db_err:
        print(f"LỖI (database.py - get_transition_details): Truy vấn thất bại cho ID {transition_id}: {db_err}")
        print(traceback.format_exc())
        details = None # Trả về None nếu lỗi DB
    except Exception as e:
        print(f"LỖI (database.py - get_transition_details): Lỗi không xác định cho ID {transition_id}: {e}")
        print(traceback.format_exc())
        details = None # Trả về None nếu lỗi khác
    finally:
        if cur: cur.close()
        if conn: conn.close()
    return details
...
def get_strategy_action_sequence(strategy_id: str) -> list[dict] | None:
    ...
    bao gồm cả các trường loop để biên dịch action_sequence.
    """
    if not strategy_id: return None
    transitions_list = None
    conn = get_db_connection()
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        print(f"DEBUG (database.py): Truy vấn transitions thô (incl. loop) cho strategy_id='{strategy_id}'...")
        # <<< THÊM CÁC CỘT loop_* VÀO SELECT >>>
        sql = """
            SELECT
                t.transition_id, t.current_stage_id, t.user_intent,
            ...
            WHERE
                ss.strategy_id = %s
            ORDER BY
                t.current_stage_id, t.priority DESC, t.user_intent; -- Sắp xếp để xử lý biên dịch
        """
        cur.execute(sql, (strategy_id,))
        rows = cur.fetchall()
        # Chuyển JSONB thành dict nếu cần (psycopg2 thường tự làm với DictCursor)
        transitions_list = [dict(row) for row in rows] if rows else []
        print(f"DEBUG: Found {len(transitions_list)} raw transitions (incl. loop) for strategy {strategy_id}")

    except psycopg2.Error as db_err:
        print(f"LỖI (db - get_strategy_action_sequence): Truy vấn thất bại: {db_err}")
        print(traceback.format_exc())
        transitions_list = None
    except Exception as e:
        print(f"LỖI (db - get_strategy_action_sequence): Lỗi không xác định: {e}")
        print(traceback.format_exc())
        transitions_list = None
    finally:
        if cur: cur.close()
...
def add_phone_action_logs(assignment_id: int, device_id: str, account_id: str, logs: list[dict]) -> bool:
    """
    Ghi nhiều bản ghi log hành động từ client vào CSDL.
    """
    if not assignment_id or not logs: return False
    conn = get_db_connection()
    if not conn: return False
    cur = None
    success = False
    try:
        ...
        sql = """
            INSERT INTO public.phone_action_log
            (assignment_id, device_id, account_id, "timestamp", macro_code, params_json, execution_status, execution_error, current_stage, received_state_json)
            VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s, %s, %s, %s::jsonb);
        """
        # Chuẩn bị list các tuple dữ liệu để insert nhiều dòng hiệu quả
        data_tuples = []
        for log_entry in logs:
            if not isinstance(log_entry, dict): continue # Bỏ qua log không phải dict

            ts_str = log_entry.get('timestamp')
            ts_obj = datetime.now(timezone.utc) # Default
            if isinstance(ts_str, str):
                try: ts_obj = datetime.fromisoformat(ts_str.replace('Z', '+00:00'))
                except ValueError: pass # Giữ default nếu parse lỗi

            params_data = log_entry.get('params')
            params_json_str = json.dumps(params_data) if isinstance(params_data, dict) else None

            state_data = log_entry.get('received_state') # Giả sử client gửi key này
            state_json_str = json.dumps(state_data) if isinstance(state_data, dict) else None
            ...
            data_tuples.append((
                ...
                account_id,
                ts_obj,
                log_entry.get('macro'), # Giả sử client gửi key 'macro'
                params_json_str,
                log_entry.get('status', 'unknown'), # Giả sử client gửi key 'status'
                log_entry.get('error'), # Có thể là None
                log_entry.get('stage'), # Giả sử client gửi key 'stage'
                state_json_str
            ))

        if data_tuples:
            # Dùng execute_values để tối ưu insert nhiều dòng
            psycopg2.extras.execute_values(
                cur, sql, data_tuples,
                template=None, page_size=100 # Điều chỉnh page_size nếu cần
            )
            conn.commit()
            success = True
            current_app.logger.debug(f"Added {len(data_tuples)} phone action logs for assignment {assignment_id}.")
        else:
            current_app.logger.warning(f"No valid log entries found in the report for assignment {assignment_id}.")
            success = True # Coi như thành công vì không có gì để ghi

    except psycopg2.Error as db_err:
        current_app.logger.error(f"DB Error adding phone action logs for {assignment_id}: {db_err}", exc_info=True)
        if conn: conn.rollback()
    except Exception as e:
        current_app.logger.error(f"Unexpected Error adding phone action logs for {assignment_id}: {e}", exc_info=True)
        if conn: conn.rollback()
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def get_all_task_assignments(filters: dict = None, page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    ...
    select_sql = """
        SELECT
            ...
            ta.priority,
            ta.strategy_id,
            s.name as strategy_name, -- Lấy tên strategy
            da.device_id,
            d.device_name, -- Lấy tên device
            da.account_id,
            acc.username as account_username, -- Lấy username account
            acc.platform as account_platform, -- Lấy platform account
            ta.target_data ->> 'current_count' as current_progress, -- Ví dụ lấy tiến độ
            ta.target_data ->> 'target_count' as target_goal,     -- Ví dụ lấy mục tiêu
            ta.created_at,
            ta.assigned_at,
            ta.last_report_at,
            ta.completed_at,
            ta.error_message -- Thêm cột error_message từ bảng scheduler_commands nếu bạn join tới đó, hoặc lưu trong result_data
        FROM public.task_assignments ta
        JOIN public.device_accounts da ON ta.device_account_id = da.device_account_id
        ...
        -- LEFT JOIN scheduler_commands sc ON ... (Nếu bạn muốn join để lấy error_message từ command)
    """
    ...
    if filters:
        ...
        if filters.get('strategy_id'):
            where_clauses.append("ta.strategy_id = %s")
            params.append(filters['strategy_id'])
        if filters.get('device_id'):
            where_clauses.append("da.device_id = %s")
            params.append(filters['device_id'])
        if filters.get('account_id'):
            where_clauses.append("da.account_id = %s")
            params.append(filters['account_id'])
        # Thêm các filter khác nếu cần (vd: theo ngày tháng created_at)
    ...
    try:
        ...
        # print(f"DEBUG Count SQL (assignments): {cur.mogrify(count_sql, tuple(params)).decode('utf-8','ignore')}")
        cur.execute(count_sql, tuple(params))
        total_items = cur.fetchone()[0]
        cur.close()

        # --- Query lấy dữ liệu trang ---
        assignments = [] # Khởi tạo list rỗng
        if total_items > 0:
            cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
            offset = (page - 1) * per_page
            data_sql = f"""
                {select_sql}
                {where_sql}
                ORDER BY ta.created_at DESC -- Sắp xếp theo thời gian tạo mới nhất
                LIMIT %s OFFSET %s;
            """
            data_params = params + [per_page, offset]
            # print(f"DEBUG Data SQL (assignments): {cur.mogrify(data_sql, tuple(data_params)).decode('utf-8','ignore')}")
            cur.execute(data_sql, tuple(data_params))
            rows = cur.fetchall()
...
# --- Hàm lấy dữ liệu cho Dropdowns trong Form ---
def get_all_devices(page: int = 1, per_page: int = 30) -> tuple[list[dict] | None, int | None]:
    ...
    devices = None
    total_items = None
    conn = get_db_connection() # Đảm bảo hàm get_db_connection tồn tại và hoạt động
    if not conn: return None, None
    cur = None
    try:
        # Query đếm tổng số
        ...
        # Không cần else ở đây vì đã khởi tạo list rỗng

    except psycopg2.Error as db_err:
        # Sử dụng current_app.logger nếu hàm này được gọi trong ngữ cảnh Flask
        # Nếu không, dùng print
...
def get_accounts_for_device_select(device_id: str) -> list[dict] | None:
    """Lấy danh sách tài khoản liên kết với một device (ID, Username, Platform) cho dropdown động."""
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # Join với bảng accounts để lấy username, platform
        sql = """
        ...
        current_app.logger.error(f"Error getting accounts for device {device_id}: {e}", exc_info=True)
        accounts = None
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def add_device(device_id: str, device_name: str | None, notes: str | None) -> tuple[bool, str | None]:
    ...
    conn = get_db_connection() # Đảm bảo hàm này tồn tại
    if not conn: return False, "Không thể kết nối CSDL."
    cur = None
    success = False
    error_msg = None
    try:
        cur = conn.cursor()
        # Chỉ insert các trường do Admin nhập, status có thể mặc định 'offline'
        ...
        # Sử dụng current_app logger nếu có, nếu không thì print
        log_func = current_app.logger.error if current_app else print
...
# --- Đảm bảo các hàm này cũng có trong file database.py ---
...
def update_device_admin(device_id: str, device_name: str | None, notes: str | None, status: str | None) -> tuple[bool, str | None]:
    ...
    try:
        cur = conn.cursor()
        sql = """
            UPDATE public.devices SET device_name = %s, notes = %s, status = COALESCE(%s, status)
            WHERE device_id = %s;
        """
        ...
        conn.commit()
        success = cur.rowcount > 0
        if not success: error_msg = f"Không tìm thấy Device ID '{device_id}' để cập nhật."
    except psycopg2.Error as e:
