...
def delete_transition(transition_id):
    ...
    if not db:
         # Xử lý trường hợp db chưa được import hoặc khởi tạo đúng
         flash("Lỗi nghiêm trọng: Database module chưa sẵn sàng.", "error")
         # Fallback về trang dashboard chính
         return redirect(url_for('admin.index'))

    try:
        # Hàm get_transition_details cần trả về dict có chứa 'strategy_id'
        ...
        if transition_details:
            ...
            if strategy_id_redirect:
                 ...
                 # Hàm get_strategy_details cần trả về dict có 'strategy_type'
                 ...
                 if strategy_info:
                      ...
                      print(f"WARN (delete_transition): Could not find strategy details for ID '{strategy_id_redirect}'. Cannot redirect to detail page.")
            ...
            # Lỗi này được báo trong log của bạn ở lần trước
            print(f"ERROR (delete_transition): Could not find transition details for ID {transition_id}. Cannot determine redirect target.")
            strategy_id_redirect = None # Không tìm thấy transition thì không thể redirect về detail
    ...
    try:
        # <<< Đảm bảo db.delete_transition trả về tuple (bool, str|None) >>>
        ...
        if delete_success:
            ...
            # <<< Lỗi TypeError xảy ra ở đây nếu db.delete_transition chỉ trả về bool >>>

    except TypeError as type_err: # Bắt lỗi TypeError cụ thể
        print(f"Lỗi TypeError khi gọi db.delete_transition({transition_id}): {type_err}")
        print(traceback.format_exc())
        flash(f"Lỗi hệ thống: Hàm xóa transition trả về sai định dạng. Vui lòng kiểm tra hàm delete_transition trong database.py.", "error")
        # Vẫn cố gắng redirect về nơi hợp lý
    ...
    if strategy_id_redirect and redirect_endpoint:
        ...
        # Thông báo flash này có thể đã xuất hiện ở lần trước
        flash("Đã xóa transition. Không thể xác định chính xác trang chi tiết để quay lại.", "info")
        # <<< Fallback về trang danh sách phù hợp hoặc Dashboard >>>
        if strategy_type_redirect == 'control':
             ...
             return redirect(url_for('admin.view_strategies_language'))
        else: # Nếu type là unknown hoặc có lỗi nghiêm trọng khi fetch
            print("INFO (delete_transition): Fallback redirecting to Admin Index due to unknown type/error.")
            return redirect(url_for('admin.index'))
# --- Route Xóa Transition (Có thể dùng chung) ---
# app/admin_routes.py
# ... (imports và các route khác) ...
