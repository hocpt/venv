...
def get_screen_nodes_for_management(app_name: str | None,
    ...
    if filter_status and filter_status != 'all':
        ...
        if actual_filter_status == 'unknown':
            where_clauses.append("(s.status STARTS WITH 'unknown' OR s.status STARTS WITH 'provisional_unknown')")
        else:
    ...
    # QUAN TRỌNG: Sửa đổi data_query để lấy elementId(s)
    data_query = f"""
        MATCH (s:Screen)
        {where_statement}
        RETURN
            elementId(s) AS node_neo4j_internal_id, // <<< LẤY ID NỘI BỘ NEO4J
        ...
        with driver.session(database=db_name_neo4j) as session:
            ...
            if total_items > 0:
                ...
                for record in data_result:
                    record_dict = dict(record) # Chuyển record thành dictionary
                    # Log để kiểm tra giá trị node_neo4j_internal_id
                    logger.debug(f"GRAPH_DB: Fetched raw record: screen_id='{record_dict.get('screen_id')}', node_neo4j_internal_id='{record_dict.get('node_neo4j_internal_id')}'")
                    if not record_dict.get('node_neo4j_internal_id'):
                         logger.error(f"GRAPH_DB LỖI NGHIÊM TRỌNG: Node {record_dict.get('screen_id')} có node_neo4j_internal_id là None hoặc rỗng từ Neo4j!")
                    raw_nodes_from_neo4j.append(record_dict)
        
        # Xử lý raw_nodes_from_neo4j để thêm thông tin PIE và chuyển đổi datetime (như code hiện tại của bạn)
        processed_nodes_list = []
        if raw_nodes_from_neo4j:
            for node_data_neo4j_item in raw_nodes_from_neo4j:
                node_dict_for_processing = dict(node_data_neo4j_item)
                
                # ... (logic lấy PIE definition, chuyển đổi datetime của bạn ở đây) ...
                # Đảm bảo 'node_neo4j_internal_id' vẫn còn trong node_dict_for_processing
                if 'node_neo4j_internal_id' not in node_dict_for_processing or not node_dict_for_processing['node_neo4j_internal_id']:
                    logger.error(f"GRAPH_DB LỖI SAU PROCESSING: Node {node_dict_for_processing.get('screen_id')} thiếu node_neo4j_internal_id!")

                processed_nodes_list.append(node_dict_for_processing)
        
        logger.debug(f"GRAPH_DB: get_screen_nodes_for_management - Returning {len(processed_nodes_list)} processed nodes for page {page}. Total items: {total_items}")
