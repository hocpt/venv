...
def add_stage():
    ...
    if not strategy_id:
        ...
        # <<< SỬA REDIRECT NÀY >>>
        return redirect(url_for('admin.index')) # Quay về Dashboard nếu thiếu ID
    ...
    if db:
        try:
            ...
            # <<< SỬA REDIRECT NÀY >>>
            return redirect(url_for('admin.index')) # Quay về Dashboard nếu lỗi DB

    if not strategy_details:
        flash(f"Không tìm thấy strategy ID {strategy_id} hoặc lỗi khi đọc.", "error")
        # <<< SỬA REDIRECT NÀY >>>
        return redirect(url_for('admin.index')) # Quay về Dashboard nếu không tìm thấy strategy

    # Xác định trang redirect đích sau khi POST thành công (giữ nguyên)
    ...
    if request.method == 'POST':
        # ... (Logic xử lý POST giữ nguyên như cũ, redirect sau success dùng redirect_endpoint) ...
        # Ví dụ:
        # if success:
        #     return redirect(url_for(redirect_endpoint, strategy_id=form_strategy_id))
        # else:
        #     return render_template('admin_add_stage.html', ..., cancel_url=cancel_url)
        # --- Bỏ qua phần logic POST ở đây để giữ ngắn gọn ---
        pass # Thay bằng logic POST của bạn

    # GET request (truyền cancel_url đúng)
...
def edit_stage(stage_id):
    ...
    cancel_url = url_for('admin.view_strategies_language') # Default Cancel URL (sẽ được ghi đè nếu thành công)
    ...
    try:
        ...
        if strategy_id_redirect:
            ...
            # Stage không có strategy_id? (Lỗi dữ liệu) - Đặt cancel về list chung
             ...
             cancel_url = url_for('admin.view_strategies_language') # Tạm về language

    except Exception as e:
        print(f"Lỗi nghiêm trọng khi lấy thông tin stage/strategy: {e}")
        flash("Lỗi khi đọc thông tin stage/strategy gốc.", "error")
        # cancel_url sẽ là default đã đặt ở trên
        # Có thể redirect về index nếu lỗi nghiêm trọng
    ...
    if request.method == 'POST':
        ...
        if identifying_elements_str and identifying_elements_str.strip() and identifying_elements_str != '{}':
            try:
                # Chỉ validate, hàm DB sẽ parse lại
             ...
             validated_identifying_elements_str = '{}' # Lưu '{}' nếu rỗng

        # Gọi hàm DB để cập nhật
        try:
            success, error_msg = db.update_stage(stage_id, description, order, validated_identifying_elements_str) # Truyền chuỗi đã validate (hoặc '{}')
