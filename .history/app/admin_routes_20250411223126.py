...
try:
    ...
    from . import scheduler # Import scheduler trực tiếp từ __init__.py cùng cấp
    from . import ai_service # Import ai_service nếu cần
except ImportError:
    print("ERROR (admin_routes): Could not import db or scheduler from current package.")
    # Gán giá trị None để các route không bị lỗi NameError ngay lập tức
    # nhưng các chức năng liên quan sẽ không hoạt động và báo lỗi sau đó.
    db = None
    scheduler = None
    ai_service = None
...
# === Sửa lại hàm view_scheduled_jobs trong backup/app/admin_routes.py ===
# ... (imports như cũ) ...

@admin_bp.route('/scheduled-jobs')
def view_scheduled_jobs():
    """Hiển thị danh sách Jobs, trạng thái live và số lượng pending (nếu có)."""
    db_jobs_config = []
    live_jobs_status = {}
    pending_counts = {} # <<< Dict mới để lưu số lượng pending

    # 1. Lấy cấu hình jobs từ Database
    if db:
        try:
                ...
                if job_conf.get('job_id') == 'suggestion_job':
                    try:
                        ...
                        if count is not None:
                             pending_counts['suggestion_job'] = count
                        else:
                             pending_counts['suggestion_job'] = 'Lỗi đếm'
                    except Exception as count_err:
                         print(f"Lỗi khi đếm pending items cho suggestion_job: {count_err}")
                         pending_counts['suggestion_job'] = 'Lỗi đếm'
                    break # Chỉ cần tìm suggestion_job một lần
            # <<< KẾT THÚC LOGIC ĐẾM PENDING >>>

        except Exception as e:
            # ... (xử lý lỗi load job configs như cũ) ...
            pass # Giữ nguyên xử lý lỗi cũ
    else:
        flash("Lỗi kết nối database module.", "error")

    # 2. Lấy trạng thái jobs thực tế từ Scheduler (giữ nguyên)
    # ... (code lấy live_jobs_status như cũ) ...
    if scheduler:
        try:
            running_jobs = scheduler.get_jobs()
            for job in running_jobs:
                live_jobs_status[job.id] = {'next_run': job.next_run_time}
            print(f"DEBUG: Lấy được trạng thái của {len(live_jobs_status)} jobs từ scheduler.")
        except Exception as e:
             print(f"Lỗi khi lấy trạng thái jobs từ scheduler: {e}")
             flash("Không thể lấy trạng thái job đang chạy từ scheduler.", "warning")
    else:
        # ... (flash warning scheduler is None như cũ) ...
        pass

    print(f"DEBUG: Checking request.endpoint before rendering...")
    print(f"DEBUG: request.endpoint value: {request.endpoint}")
    print(f"DEBUG: request.endpoint type: {type(request.endpoint)}")
    # 3. Render template, truyền thêm pending_counts
    return render_template('admin_scheduled_jobs.html',
                            title="Quản lý Tác vụ nền",
                            jobs=db_jobs_config,
                            live_jobs_status=live_jobs_status,
                            pending_counts=pending_counts) # <<< TRUYỀN BIẾN MỚI

# ... (Các route khác) ...
# === Đặt code này vào phần QUẢN LÝ TÁC VỤ NỀN trong admin_routes.py ===
...
def add_scheduled_job():
    ...
    if request.method == 'POST':
        ...
        if trigger_type not in TRIGGER_TYPES:
             flash(f"Trigger Type '{trigger_type}' không hợp lệ.", "warning")
             error = True

        trigger_args_dict = {}
        try:
            # Đảm bảo chuỗi rỗng cũng được coi là JSON object rỗng
            if not trigger_args_str:
                trigger_args_str = '{}'
            trigger_args_dict = json.loads(trigger_args_str)
            if not isinstance(trigger_args_dict, dict):
                 raise ValueError("Trigger Args phải là một JSON object (ví dụ: {} hoặc {\"minutes\": 5}).")
        except (json.JSONDecodeError, ValueError) as e:
            flash(f"Trigger Args không hợp lệ: {e}", "warning")
            error = True

        # Chuyển đổi kiểu dữ liệu số cho các key thời gian
        if not error: # Chỉ chuyển đổi nếu JSON hợp lệ
            numeric_keys = ['weeks', 'days', 'hours', 'minutes', 'seconds', 'jitter', 'year', 'month', 'day', 'week', 'hour', 'minute', 'second']
            try:
                for key in numeric_keys:
                    if key in trigger_args_dict and isinstance(trigger_args_dict[key], str):
                        value_str = trigger_args_dict[key].strip()
                        if not value_str:
                            # Xử lý chuỗi rỗng (có thể bỏ qua key hoặc gán giá trị mặc định phù hợp?)
                            # Tạm thời bỏ qua key này nếu giá trị là chuỗi rỗng
                             del trigger_args_dict[key]
                             continue
                        # Thử chuyển đổi
                        if '.' in value_str: trigger_args_dict[key] = float(value_str)
                        else: trigger_args_dict[key] = int(value_str)
            except (ValueError, TypeError):
                flash(f"Giá trị số trong Trigger Args không hợp lệ (ví dụ: '{key}'='{value_str}').", "warning")
                error = True

        # Validate function path bằng cách thử import
        func = None
        if not error:
            try:
                 ...
                 flash(f"Function Path '{function_path}' không hợp lệ hoặc không tìm thấy: {import_err}", "warning")
                 error = True

        # Nếu có lỗi validation thì render lại form
        if error:
        ...
        # ----- Lưu vào DB -----
        # Lưu trigger_args_str gốc mà người dùng nhập
        db_success, db_error = db.add_job_config(job_id, function_path, trigger_type, trigger_args_str, is_enabled, description)

        if not db_success:
            flash(f"Lỗi lưu vào DB: {db_error or 'Lỗi không xác định từ DB.'}", "error")
            # Hiển thị lại form với dữ liệu người dùng đã nhập
        ...
        scheduler_message = None # Lưu thông báo từ scheduler
        if is_enabled and scheduler:
            try:
                print(f"Attempting to add job '{job_id}' to live scheduler with converted args: {trigger_args_dict}")
                # Tạo trigger object với dict đã được chuyển đổi kiểu
                trigger_obj = None
                if trigger_type == 'interval':
                    trigger_obj = IntervalTrigger(**trigger_args_dict)
                elif trigger_type == 'cron':
                    trigger_obj = CronTrigger(**trigger_args_dict)
                elif trigger_type == 'date':
                    trigger_obj = DateTrigger(**trigger_args_dict)
                else:
                     # Về lý thuyết không xảy ra do đã validate ở trên
                     raise ValueError(f"Loại trigger không được hỗ trợ: {trigger_type}")

                scheduler.add_job(
                    id=job_id, func=func, trigger=trigger_obj,
                    replace_existing=True # Ghi đè nếu job đã có trong scheduler (ví dụ do restart)
                )
                print(f"Successfully added job '{job_id}' to live scheduler.")
                scheduler_message = f"Job '{job_id}' đã được thêm và kích hoạt trong scheduler."
                flash(scheduler_message, "info") # Thông báo thêm thành công vào scheduler

            except (TypeError, ValueError) as trigger_err:
                 scheduler_message = f"Lưu DB thành công, nhưng Trigger Args không hợp lệ cho Trigger Type '{trigger_type}' khi thêm vào scheduler: {trigger_err}. Job sẽ không chạy cho đến khi được sửa."
                 print(f"ERROR adding job '{job_id}' to scheduler: {scheduler_message}")
                 flash(scheduler_message, "warning")
            except Exception as e:
                scheduler_message = f"Lưu DB thành công nhưng không thể thêm job vào scheduler đang chạy: {e}"
                print(f"ERROR adding job '{job_id}' to scheduler: {scheduler_message}")
                flash(scheduler_message, "warning")
        elif is_enabled and not scheduler:
             scheduler_message = "Đã lưu cấu hình job vào DB, nhưng không thể thêm vào scheduler (chưa được khởi tạo?). Job sẽ không chạy."
             flash(scheduler_message, "warning")


        if not scheduler_message and db_success: # Chỉ flash success tổng nếu DB thành công và không có lỗi/warning từ scheduler
             flash(f"Thêm cấu hình tác vụ '{job_id}' thành công!", 'success')
...
def edit_scheduled_job(job_id):
    ...
    # Lấy dữ liệu job hiện tại từ DB cho GET request và để dùng lại nếu POST lỗi
    ...
    if request.method == 'POST':
            ...
            if not trigger_type or trigger_type not in TRIGGER_TYPES:
                 flash("Trigger Type không hợp lệ.", "warning")
                 error = True
            ...
            # Nếu có lỗi validation thì render lại form edit
            if error:
                # job_details đã được lấy ở đầu hàm edit_scheduled_job
            ...
            # --- Cập nhật DB ---
            # Luôn lưu trigger_args_str gốc vào DB
            ...
            # --- Cập nhật Scheduler đang chạy (Chiến lược: Xóa và Thêm lại) ---
            scheduler_message = None
            if scheduler:
                try:
                    print(f"Attempting to update job '{job_id}' in live scheduler (using remove & add)...")
    
                    # --- Lấy đối tượng hàm (cần cho add_job) ---
                    func = None
                    function_path = job_details.get('job_function_path') # Lấy path từ details đã load
                    if not function_path:
                         # Ném lỗi nếu không có path, vì không thể thêm lại job
                         raise ValueError("Function path is missing in job details, cannot re-add job.")
                    try:
                         module_path, func_name = function_path.rsplit('.', 1)
                         module = importlib.import_module(module_path)
                         func = getattr(module, func_name)
                         ...
                         # Ném lại lỗi cụ thể hơn để khối except bên ngoài bắt được
                         raise ValueError(f"Function Path '{function_path}' không hợp lệ hoặc không tìm thấy: {import_err}") from import_err
    
                    # --- Tạo đối tượng trigger mới với dict đã convert ---
                    trigger_obj = None
                    if trigger_type == 'interval':
                        trigger_obj = IntervalTrigger(**trigger_args_dict)
                    elif trigger_type == 'cron':
                    ...
                    # --- Xóa job hiện có (nếu tồn tại) khỏi scheduler ---
                    print(f"Removing job '{job_id}' from scheduler (if exists)...")
                    
                    try:
                        scheduler.remove_job(job_id)
                        print(f"Removed job '{job_id}'.")
                    except JobLookupError:
                        # Bắt lỗi nếu job không tồn tại để xóa -> Không sao cả, tiếp tục bình thường
                        print(f"Job '{job_id}' not found in scheduler, nothing to remove.")
                        pass
                    
                    # --- Thêm lại job với cấu hình mới (chỉ khi được enable) ---
                    action_taken_msg = f"Job '{job_id}' removed from scheduler (disabled)."
                    if is_enabled:
                        print(f"Adding job '{job_id}' back with new trigger...")
                        scheduler.add_job(
                            id=job_id,
                            func=func,
                            trigger=trigger_obj,
                            replace_existing=True # Nên dùng để ghi đè nếu remove_job vì lý do nào đó thất bại
                        )
                        action_taken_msg = f"Job '{job_id}' re-added to scheduler with new configuration."
                        print(f"Successfully {action_taken_msg}")
                    else:
                         print(action_taken_msg)
    
                    # Nếu không có lỗi nào xảy ra trong khối try này
                    scheduler_message = f"Cập nhật scheduler thành công ({action_taken_msg})"
                    # Không flash ở đây, để flash tổng cuối cùng
                    print(f"Successfully processed scheduler update for job '{job_id}'.")
                    # Clear scheduler_error nếu không có lỗi nào khác xảy ra trong khối try này
                    scheduler_error = None
                except (TypeError, ValueError) as trigger_or_import_err:
                     # Lỗi xảy ra khi tạo trigger hoặc import hàm
                     scheduler_message = f"Lưu DB thành công, nhưng có lỗi với cấu hình Trigger hoặc Function Path: {trigger_or_import_err}. Job có thể chưa được cập nhật trong scheduler."
                     print(f"ERROR updating job '{job_id}' in scheduler: {scheduler_message}")
                     flash(scheduler_message, "warning") # Báo lỗi nhưng vẫn redirect
                except JobLookupError: # Lỗi này không nên xảy ra với remove_job(missing_ok=True)
                     scheduler_message = f"Lưu DB thành công, nhưng không tìm thấy job '{job_id}' trong scheduler (lỗi không mong muốn)."
                     print(f"WARNING: {scheduler_message}")
                     flash(scheduler_message, "warning")
                except Exception as e:
                     scheduler_message = f"Lưu DB thành công, nhưng lỗi khi cập nhật scheduler: {e}"
                     print(f"ERROR updating job '{job_id}' in scheduler: {scheduler_message}")
                     flash(scheduler_message, "warning")
            else: # Trường hợp scheduler is None
                 scheduler_message = "Đã cập nhật cấu hình job trong DB, nhưng không thể cập nhật scheduler (chưa được khởi tạo?)."
                 flash(scheduler_message, "warning")
    
            # Chỉ flash success tổng nếu DB thành công và không có lỗi/warning nghiêm trọng từ scheduler
            if not scheduler_message or "Successfully" in scheduler_message:
                 flash(f"Cập nhật tác vụ '{job_id}' thành công!", 'success')
            # Else: các flash warning/error đã được gọi bên trong khối except của scheduler
...
# Hàm toggle_scheduled_job và delete_scheduled_job giữ nguyên như phiên bản trước
# (Đảm bảo chúng cũng dùng biến `scheduler` đã import thay vì `live_scheduler`)

@admin_bp.route('/scheduled-jobs/<job_id>/delete', methods=['POST'])
def delete_scheduled_job(job_id):
    """Xóa cấu hình Job khỏi DB và Scheduler."""
    scheduler_error = None
    # --- Xóa khỏi Scheduler trước ---
    if scheduler:
        try:
            ...
            print(f"Successfully removed job '{job_id}' from scheduler (if it existed).")
             ...
             # Không flash ngay, đợi kết quả DB
    else:
         scheduler_error = "Không thể xóa job khỏi scheduler (scheduler chưa được khởi tạo?)."
    ...
    if db_success:
        # Kết hợp thông báo
        final_message = f"Đã xóa cấu hình job '{job_id}' khỏi DB."
        if scheduler_error:
            final_message += f" Cảnh báo Scheduler: {scheduler_error}"
            flash(final_message, 'warning') # Coi là warning nếu xóa khỏi scheduler lỗi
        else:
            flash(final_message, 'success')
    else:
        # Lỗi xóa DB là nghiêm trọng hơn
        final_message = f"Lỗi xóa cấu hình job '{job_id}' khỏi DB: {db_error or 'Unknown DB error'}."
        if scheduler_error:
             final_message += f" Lỗi Scheduler: {scheduler_error}"
...
def toggle_scheduled_job(job_id):
    ...
    scheduler_error = None
    action_performed = None # Để flash message chính xác hơn
    if scheduler:
        try:
            live_job = scheduler.get_job(job_id)
            # Chỉ thực hiện pause/resume nếu job đang tồn tại trong scheduler
            if live_job:
                if new_enabled_state:
                    print(f"Resuming job '{job_id}'...")
                    scheduler.resume_job(job_id)
                    action_performed = 'resumed'
                else:
                    print(f"Pausing job '{job_id}'...")
                    scheduler.pause_job(job_id)
                    action_performed = 'paused'
            elif new_enabled_state:
                 # Job được bật trong DB nhưng không có trong scheduler -> Thử thêm lại?
                 print(f"WARNING: Job '{job_id}' enabled in DB but not found in scheduler. Attempting to add.")
                 try:
                     function_path = job_details.get('job_function_path')
                     trigger_type = job_details.get('trigger_type')
                     trigger_args_dict = job_details.get('trigger_args', {}) # Là dict từ DB
                     if function_path and trigger_type and trigger_args_dict is not None:
                         module_path, func_name = function_path.rsplit('.', 1)
                         module = importlib.import_module(module_path)
                         func = getattr(module, func_name)
                         scheduler.add_job(id=job_id, func=func, trigger=trigger_type, replace_existing=True, **trigger_args_dict)
                         action_performed = 'added'
                         print(f"Successfully ADDED job '{job_id}' to scheduler on toggle enable.")
                     else:
                          raise ValueError("Thiếu thông tin function_path/trigger_type/trigger_args để thêm lại job.")
                 except Exception as re_add_err:
                     scheduler_error = f"Lỗi khi cố gắng thêm lại job '{job_id}' vào scheduler: {re_add_err}"
                     print(f"ERROR: {scheduler_error}")
                     # Không flash ở đây, sẽ flash ở cuối
            else:
                 # Job bị tắt trong DB và cũng không có trong scheduler -> Không cần làm gì
                 print(f"INFO: Job '{job_id}' disabled in DB and not found in scheduler. No action needed.")
                 action_performed = 'not_found_disabled'


        except JobLookupError: # Trường hợp này ít xảy ra hơn do đã có get_job ở trên
             scheduler_error = f"Lưu trạng thái DB thành công, nhưng không tìm thấy job '{job_id}' trong scheduler để bật/tắt."
             print(f"WARNING: {scheduler_error}")
        except Exception as e:
             scheduler_error = f"Lỗi khi bật/tắt job '{job_id}' trong scheduler: {e}"
             print(f"ERROR toggling job '{job_id}' in scheduler: {scheduler_error}")

        # Flash message dựa trên kết quả
        if scheduler_error:
             flash(f"Đã cập nhật trạng thái DB cho '{job_id}', nhưng lỗi Scheduler: {scheduler_error}", "warning")
        elif action_performed == 'resumed':
             flash(f"Đã kích hoạt (resume) job '{job_id}'.", 'success')
        elif action_performed == 'paused':
              flash(f"Đã tạm dừng (pause) job '{job_id}'.", 'info')
        elif action_performed == 'added':
              flash(f"Job '{job_id}' không có trong scheduler, đã thêm và kích hoạt.", 'success')
        elif action_performed == 'not_found_disabled':
               flash(f"Đã tắt job '{job_id}' trong DB (Job cũng không có trong scheduler).", 'info')
        # else: trường hợp scheduler=None hoặc db update lỗi (đã xử lý)

    else: # Trường hợp scheduler is None
        flash("Đã cập nhật trạng thái job trong DB, nhưng không thể tương tác với scheduler (import lỗi?).", "warning")


    return redirect(url_for('admin.view_scheduled_jobs')) 


# === Đặt hàm này vào file backup/app/admin_routes.py ===
# (Thường nằm trong phần QUẢN LÝ TÁC VỤ NỀN)

@admin_bp.route('/scheduled-jobs/suggestion_job/run-now', methods=['POST'])
def run_suggestion_job_now():
    """
    Thực thi logic của tác vụ nền 'suggestion_job' một lần ngay lập tức
    khi người dùng nhấn nút 'Chạy Ngay' trong giao diện Admin.
    """
    job_id_to_run = 'suggestion_job' # ID của job chúng ta muốn mô phỏng
    print(f"\n--- Bắt đầu chạy thủ công tác vụ: {job_id_to_run} --- ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    print(f"DEBUG: (Manual Run) Sử dụng Persona ID for suggestion: {persona_id_for_suggestion}")
    ...
    processed_count = 0
    error_message = None # Lưu trữ thông báo lỗi nếu có

    try:
        # 1. Lấy trạng thái xử lý cuối cùng (ID của interaction cuối) từ DB
        last_processed_id = db.get_task_state(job_id_to_run) or 0
        max_processed_id_in_batch = last_processed_id # Bắt đầu bằng ID cũ
        print(f"DEBUG: (Manual Run) Lấy last_processed_id = {last_processed_id}")

        # 2. Lấy các tương tác MỚI HƠN ID đó để phân tích
        # Lấy cấu hình từ app config hoặc dùng giá trị mặc định
        status_filter = current_app.config.get('STATUS_TO_ANALYZE_SUGGEST', ['success_ai'])
        limit = current_app.config.get('MANUAL_SUGGEST_LIMIT', 10) # Giới hạn số lượng xử lý mỗi lần chạy tay
        print(f"DEBUG: (Manual Run) Lấy tối đa {limit} tương tác với status {status_filter} sau ID {last_processed_id}...")
        ...
        if interactions is None:
             error_message = "Lỗi khi lấy tương tác từ DB để phân tích."
             raise Exception(error_message) # Nhảy tới khối except cuối

        interactions_found_count = len(interactions)
        if not interactions:
             flash(f"Không tìm thấy tương tác mới nào (sau ID {last_processed_id}) có status '{','.join(status_filter)}' để phân tích.", "info")
             # Không có lỗi, chỉ là không có dữ liệu mới, redirect về
             return redirect(url_for('admin.view_scheduled_jobs'))

        print(f"INFO: (Manual Run) Tìm thấy {interactions_found_count} tương tác để phân tích.")
        ...
        for interaction in interactions:
            ...
            if history_id is None:
                print("WARNING: (Manual Run) Bỏ qua bản ghi interaction không có history_id.")
            ...
            print(f"DEBUG: (Manual Run) Phân tích interaction ID: {history_id}")
            ...
            if not interaction_data['received_text'] or not interaction_data['sent_text']:
                print(f"WARNING: (Manual Run) Bỏ qua interaction {history_id} do thiếu received_text hoặc sent_text.")
                continue # Bỏ qua interaction này, max_id đã được cập nhật

            # Gọi hàm AI service để lấy đề xuất (nhận 4 giá trị)
            try:
                # <<< NHẬN ĐỦ 4 GIÁ TRỊ TRẢ VỀ >>>
                suggested_keywords, suggested_category, suggested_template_ref, suggested_template = ai_service.suggest_rule_from_interaction(
                 ...
                 print(f"ERROR: (Manual Run) Exception calling ai_service.suggest_rule_from_interaction for ID {history_id}: {ai_call_err}")
                 print(traceback.format_exc())
                 continue # Bỏ qua interaction này nếu AI lỗi, xử lý cái tiếp theo

            # 4. Lưu đề xuất hợp lệ vào database
            # Chỉ lưu nếu có keywords hoặc template text được trả về
            if suggested_keywords or suggested_template:
                print(f"INFO: (Manual Run) AI đề xuất cho ID {history_id}: kw='{str(suggested_keywords)[:50]}...', cat='{suggested_category}', ref='{suggested_template_ref}', tpl='{str(suggested_template)[:50]}...'")
                source_examples = {'history_ids': [history_id], 'run_type': 'manual'}
                ...
                try:
                    ...
                    if added:
                        suggestions_added += 1
                        print(f"INFO: (Manual Run) Đã lưu đề xuất từ interaction {history_id}.")
                    else:
                        # Lỗi logic bên trong add_suggestion hoặc lỗi DB không bắt được Exception
                        print(f"ERROR: (Manual Run) Lưu đề xuất từ interaction {history_id} thất bại (db.add_suggestion trả về False).")
                except Exception as db_add_err:
                     print(f"ERROR: (Manual Run) Exception calling db.add_suggestion for ID {history_id}: {db_add_err}")
                     print(traceback.format_exc())
                     # Có thể quyết định dừng hoặc tiếp tục xử lý các interaction khác

            else:
                 print(f"DEBUG: (Manual Run) AI không tạo ra đề xuất hợp lệ (thiếu cả keywords và template) cho interaction {history_id}.")
        ...
        if max_processed_id_in_batch > last_processed_id:
            print(f"DEBUG: (Manual Run) Cập nhật last_processed_id từ {last_processed_id} thành {max_processed_id_in_batch} cho task {job_id_to_run}")
            update_success = db.update_task_state(job_id_to_run, max_processed_id_in_batch)
            if not update_success:
                # Lỗi nghiêm trọng: Lần chạy sau (dù thủ công hay tự động) sẽ xử lý lại batch này!
                error_message = "Lỗi khi cập nhật trạng thái last_processed_id sau khi chạy."
                print(f"CRITICAL ERROR: (Manual Run) {error_message}")
                # Không raise lỗi ở đây, chỉ flash ở cuối

        # Thông báo kết quả cuối cùng
        if error_message:
             flash(f"Chạy thủ công hoàn tất với lỗi: {error_message}", "error")
        else:
             flash(f"Chạy thủ công hoàn tất. Đã xử lý {processed_count}/{interactions_found_count} tương tác tìm thấy. Thêm được {suggestions_added} đề xuất mới. ID xử lý cuối cùng được cập nhật thành {max_processed_id_in_batch}.", "success")

    except Exception as e:
         # Bắt các lỗi không mong muốn xảy ra trong quá trình chạy thủ công
         print(f"LỖI NGHIÊM TRỌNG trong khi chạy thủ công {job_id_to_run}: {e}")
         print(traceback.format_exc())
         flash(f"Lỗi nghiêm trọng khi chạy thủ công: {e}", "error")

    # Luôn redirect về trang quản lý jobs sau khi chạy xong hoặc gặp lỗi
    return redirect(url_for('admin.view_scheduled_jobs'))
