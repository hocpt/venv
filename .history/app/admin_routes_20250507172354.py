...
def admin_screen_elements(screen_id):
    """Hiển thị trang chi tiết các elements, xử lý manual_explored_override."""
    logger = current_app.logger if current_app else print
    ...
    try:
        ...
        logger.debug(f"Screen data from Neo4j: {screen_data}")
        app_name = screen_data.get('app_name')
        # === TẠO SCREENSHOT URL TỪ PATH ===
        # === TẠO SCREENSHOT URL DÙNG ROUTE MỚI ===
        # screen_data.get('screenshot_path') giờ đây chỉ là tên file
        # === TẠO SCREENSHOT URL DÙNG ROUTE MỚI ===
        filename_from_neo4j = screen_data.get('screenshot_path') # Đây là tên file
        logger.debug(f"[admin_screen_elements] Lấy screenshot_path từ Neo4j: '{filename_from_neo4j}'") # LOG 1

        if filename_from_neo4j:
            try:
                # === LOG TRƯỚC KHI GỌI url_for ===
                logger.debug(f"[admin_screen_elements] Chuẩn bị gọi url_for('serve_project_screenshot', filename='{filename_from_neo4j}')")
                # ================================
                screenshot_url = url_for('serve_project_screenshot', filename=filename_from_neo4j)
                # === LOG SAU KHI GỌI url_for ===
                logger.info(f"[admin_screen_elements] Đã tạo URL screenshot: '{screenshot_url}'") # LOG 2
                # ===============================
            except Exception as url_err:
                logger.error(f"[admin_screen_elements] Lỗi khi tạo URL tĩnh cho screenshot path '{filename_from_neo4j}': {url_err}", exc_info=True)
                screenshot_url = None
        else:
            logger.info(f"[admin_screen_elements] Không tìm thấy screenshot_path cho màn hình {screen_id}.")
        # =======================================
        # =======================================
        # 2. Lấy các hành động đã thử từ Neo4j (trạng thái khám phá tự động)
        ...
        if app_name:
            ...
            if outgoing_transitions is None:
                logger.error(f"Failed to fetch outgoing transitions for screen {screen_id}. Cannot determine explored status.")
            ...
            logger.debug(f"Auto-detected explored element IDs from transitions: {tried_element_ids}")
        else:
            logger.warning(f"Cannot fetch outgoing transitions because app_name is missing for screen {screen_id}.")

        # 3. Lấy UI state chi tiết từ log PostgreSQL
        detailed_ui_state = db.get_last_detailed_ui_state_for_screen(screen_id)
        logger.debug(f"Detailed UI state from log: {'Found' if detailed_ui_state else 'Not Found'}")
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            logger.debug(f"Found {len(elements_list_from_log)} elements in log.")

            # 4. Lấy classifications và override status đã lưu từ PostgreSQL
            saved_data = db.get_element_classifications_for_screen(screen_id)
            if saved_data is None:
                logger.error(f"DB Error fetching saved classifications/overrides for screen {screen_id}.")
                saved_data = {} # Coi như rỗng nếu lỗi DB
            else:
                logger.debug(f"Saved classifications/overrides from DB: {saved_data}")

            # 5. Hợp nhất dữ liệu: Thêm classification và xác định trạng thái khám phá cuối cùng
            elements_list = []
            for el_from_log in elements_list_from_log:
                el_id = el_from_log.get('element_id')
                if el_id:
                    element_saved_data = saved_data.get(el_id, {}) # Lấy dict data đã lưu cho element này
                    classification = element_saved_data.get('classification', 'unclassified')
                    manual_override = element_saved_data.get('override') # Có thể là True, False, hoặc None
                    ...
                    else: # manual_override is None (hoặc không có trong DB)
                        display_explored_status = auto_explored_status
                        override_active = 'auto' # Không có ghi đè
                    # ============================================

                    el_from_log['classification'] = classification
                    el_from_log['display_explored_status'] = display_explored_status # Trạng thái cuối cùng để hiển thị
                    el_from_log['override_active'] = override_active # Cho biết loại override nào đang active

                    logger.debug(f"  Element ID '{el_id}': Class='{classification}', Override='{manual_override}', AutoExplored={auto_explored_status} => DisplayExplored={display_explored_status}")
                    elements_list.append(el_from_log)
                else:
                    logger.warning(f"Skipping element from log due to missing element_id: {el_from_log}")

            logger.info(f"Final merged elements list size: {len(elements_list)}")

        else:
            logger.warning(f"Could not find detailed elements list in logs for screen {screen_id}.")

        # 6. Lấy valid classifications
    ...
    return render_template('admin_screen_elements.html',
                           screen=screen_data,
                           elements=elements_list, # <<< Đã chứa display_explored_status và override_active
                           valid_classifications=valid_classifications,
                           screenshot_url=screenshot_url)
# ... (Các API và route khác) ...
