...
# @admin_bp.route('/api/map/graph_data/<path:app_name>', methods=['GET']) # Ví dụ route
@admin_bp.route('/api/mapping_data', methods=['GET']) # <<< HOẶC route này nếu bạn dùng query param
# @login_required # Đảm bảo có xác thực nếu cần
def api_get_app_graph_data(): # <<< Tên hàm có thể khác (ví dụ: api_get_map_graph_data_for_app)
    """
    API Endpoint trả về dữ liệu nodes và edges cho Cytoscape,
    bao gồm cả screenshot_url cho nodes.
    """
    logger = current_app.logger if current_app else print
    # Lấy app_name từ query parameter (nếu route là /api/mapping_data)
    app_name = request.args.get('app_name')
    # Hoặc nếu app_name là path variable: app_name = app_name (tham số hàm)
    ...
    try:
        # Gọi hàm graph_db để lấy dữ liệu thô (đã sửa để trả về screenshot_path là tên file)
        ...
        if not graph_data.get("nodes"):
             ...
             # Vẫn trả về cấu trúc hợp lệ nhưng rỗng
        ...
        for node_element in graph_data.get("nodes", []):
            ...
            filename = node_data.get("screenshot_path") # Lấy tên file từ graph_db

            if filename:
                try:
                    # Tạo URL dùng route tùy chỉnh đã tạo trong app/__init__.py
            ...
            processed_nodes.append({"data": node_data}) # Thêm node đã xử lý vào list mới

        graph_data["nodes"] = processed_nodes # Cập nhật lại danh sách node
        # =========================================

        logger.info(f"API: Trả về dữ liệu đồ thị thành công cho app '{app_name}'.")
        return jsonify(graph_data) # Trả về dữ liệu đã cập nhật
