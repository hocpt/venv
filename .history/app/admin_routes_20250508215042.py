...
#@admin_required # Bạn có thể bỏ comment dòng này nếu cần xác thực admin
def admin_screen_elements(screen_id):
    """
    Hiển thị trang chi tiết các phần tử của một màn hình (Screen).
    Lấy dữ liệu Screen từ Neo4j và thông tin phân loại từ PostgreSQL.
    """
    logger = current_app.logger if current_app else print
    logger.info(f"Đang tải trang phần tử cho screen_id: {screen_id}")

    screen_data = None        # Dữ liệu cơ bản của Screen từ Neo4j
    elements_list = []        # Danh sách elements đã được xử lý để hiển thị
    screenshot_url = None     # URL để hiển thị ảnh chụp màn hình
    valid_classifications = [] # Danh sách các loại phân loại hợp lệ (từ ai_service)

    # Kiểm tra các module cần thiết đã được import và khởi tạo chưa
    if not graph_db or not db or not ai_service:
        flash("Lỗi nghiêm trọng: Database, GraphDB hoặc AI module chưa sẵn sàng.", "error")
        return redirect(url_for('admin.index')) # Redirect về trang admin index nếu có lỗi nghiêm trọng

    try:
        # 1. Lấy thông tin Screen từ Neo4j (bao gồm cả elements nếu hàm DB trả về)
        # Giả sử graph_db.get_screen_with_elements trả về dict chứa thuộc tính node Screen
        # và có thể đã bao gồm list 'elements' lấy từ Neo4j nếu được thiết kế vậy.
        # Hoặc nó chỉ trả về thuộc tính Screen, và chúng ta lấy elements từ log PG sau.
        # **Quan trọng:** Cần xem lại hàm graph_db.get_screen_with_elements trả về gì.
        # Giả định hiện tại: Nó trả về thuộc tính của node Screen.
        ...
        if not screen_data_from_neo4j:
            ...
            # Có thể redirect về trang mapping viewer nếu màn hình không tồn tại
            return redirect(url_for('admin.admin_mapping_viewer')) # Hoặc 'admin.index'

        # Lưu trữ dữ liệu screen chính
        screen_data = screen_data_from_neo4j
        app_name = screen_data.get('app_name') # Lấy app_name từ dữ liệu screen
        logger.debug(f"Dữ liệu Screen từ Neo4j: {screen_data}")

        # 2. Tạo URL cho ảnh chụp màn hình
        filename_from_neo4j = screen_data.get('screenshot_path') # Lấy tên file từ Neo4j
        logger.debug(f"[admin_screen_elements] Lấy screenshot_path (tên file) từ Neo4j: '{filename_from_neo4j}'")

        if filename_from_neo4j:
            try:
                # Sử dụng endpoint 'serve_app_specific_screenshot' đã định nghĩa
                ...
                logger.error(f"[admin_screen_elements] Lỗi khi tạo URL cho screenshot path '{filename_from_neo4j}': {url_err}", exc_info=True)
                screenshot_url = None # Đặt là None nếu lỗi
        else:
            logger.info(f"[admin_screen_elements] Không tìm thấy screenshot_path cho màn hình {screen_id}.")

        # 3. Lấy trạng thái khám phá tự động từ transitions (Neo4j)
        tried_element_ids = set()
        if app_name: # Cần app_name để lọc transitions
            outgoing_transitions = graph_db.get_outgoing_transitions(screen_id, app_name)
            if outgoing_transitions is None:
                logger.warning(f"graph_db.get_outgoing_transitions trả về None cho screen {screen_id}, app {app_name}.")
                outgoing_transitions = []
            elif not isinstance(outgoing_transitions, list):
                 logger.error(f"Lỗi kiểu dữ liệu trả về từ get_outgoing_transitions: {type(outgoing_transitions)}")
                 outgoing_transitions = []

            # Duyệt qua các transition đi ra để xác định element nào đã được tương tác
            for trans_props in outgoing_transitions:
                 # Đảm bảo trans_props là dict
                 if isinstance(trans_props, dict):
                     el_id = trans_props.get('element_id')
                     action_type = trans_props.get('actionType') # Hoặc action_type nếu tên thuộc tính khác
                     macro_code = trans_props.get('macro_code')
                     # Xác định các hành động tương tác với element
                     is_interaction = (action_type in ['click', 'input']) or \
                                      (action_type == 'run_macro' and macro_code in ['UI_CLICK', 'UI_INPUT_TEXT', 'UI_TAP_XY']) # Thêm các macro tương tác nếu có
                     if el_id and is_interaction:
                          tried_element_ids.add(el_id)
                 else:
                      logger.warning(f"Dữ liệu transition không hợp lệ: {trans_props}")

        else:
            logger.warning(f"app_name không có cho screen {screen_id}, không thể lấy trạng thái khám phá tự động.")

        # 4. Lấy chi tiết elements từ log trạng thái UI gần nhất (PostgreSQL)
        # Hàm này nên trả về dict chứa cấu trúc UI, bao gồm list 'elements'
        ...
        if detailed_ui_state and isinstance(detailed_ui_state.get('elements'), list):
            ...
            logger.debug(f"Tìm thấy {len(elements_list_from_log)} elements trong detailed_ui_state log cho screen {screen_id}.")

            # 5. Lấy thông tin phân loại và override đã lưu từ PostgreSQL
            saved_data = db.get_element_classifications_for_screen(screen_id)
            if saved_data is None:
                 logger.error(f"Lỗi khi lấy saved classification data từ DB cho screen {screen_id}.")
                 saved_data = {} # Xử lý trường hợp lỗi DB, coi như chưa có gì được lưu

            # 6. Hợp nhất dữ liệu: Lấy element từ log, bổ sung classification/override/trạng thái explored
            for el_from_log in elements_list_from_log:
                el_id = el_from_log.get('element_id')
                if el_id:
                    element_saved_data = saved_data.get(el_id, {}) # Lấy data đã lưu cho element này

                    # Lấy classification từ DB, nếu không có thì mặc định là 'unclassified'
                    classification = element_saved_data.get('classification', 'unclassified')

                    # Lấy trạng thái ghi đè thủ công (manual_explored_override) từ DB
                    manual_override = element_saved_data.get('manual_explored_override') # Giá trị có thể là True, False, hoặc None

                    # Xác định trạng thái khám phá tự động
                    auto_explored_status = el_id in tried_element_ids

                    # Xác định trạng thái hiển thị cuối cùng và trạng thái active của nút override
                    override_active = 'auto' # Trạng thái active của nút (auto, force_explored, force_unexplored)
                    display_explored_status = auto_explored_status # Trạng thái cuối cùng hiển thị (True/False)
                    ...
                    # else: override_active là 'auto', display_explored_status giữ nguyên giá trị auto_explored_status

                    # Bổ sung các thông tin đã xử lý vào dict element
                    el_from_log['classification'] = classification
                    el_from_log['display_explored_status'] = display_explored_status
                    el_from_log['override_active'] = override_active # Để highlight nút đúng

                    # Thêm element đã xử lý vào danh sách cuối cùng
                    elements_list.append(el_from_log)
                else:
                    logger.warning(f"Bỏ qua element thiếu element_id trong detailed_ui_state log: {el_from_log}")
        else:
            logger.warning(f"Không tìm thấy detailed_ui_state hoặc 'elements' không phải list cho screen {screen_id}.")

        # 7. Lấy danh sách các loại phân loại hợp lệ từ module AI
        if hasattr(ai_service, 'VALID_CLASSIFICATIONS'):
            ...
            # valid_classifications sẽ là list rỗng
        ...
        # Nếu lỗi xảy ra trước khi lấy được screen_data, redirect về trang mapping
        if screen_data is None:
             return redirect(url_for('admin.admin_mapping_viewer'))
        # Nếu lỗi xảy ra sau khi đã có screen_data, vẫn render trang nhưng elements_list rỗng
        elements_list = []

    # Render template, truyền tất cả dữ liệu cần thiết
    return render_template('admin_screen_elements.html',
                           screen=screen_data,                 # Dữ liệu Screen node từ Neo4j
                           elements=elements_list,             # List elements đã xử lý để hiển thị
                           valid_classifications=valid_classifications, # List phân loại hợp lệ
...
def api_get_app_graph_data():
    ...
    try:
        ...
        edges_list = graph_data.get("edges", []) # Giữ nguyên edges

        # === THÊM LOGIC TẠO SCREENSHOT URL CHO TỪNG NODE ===
        processed_nodes = []
        for node_element in nodes_list:
            ...
            if filename:
                try:
                    ...
                    node_data["screenshot_url"] = url_for('serve_app_specific_screenshot', filename=filename, _external=False)
                    logger.debug(f"API: Đã tạo screenshot_url '{node_data['screenshot_url']}' cho node {node_data.get('id')}")
                except Exception as url_err:
                    logger.error(f"API: Lỗi tạo URL cho screenshot '{filename}' của node {node_data.get('id')}: {url_err}", exc_info=False) # Bỏ exc_info cho gọn log
        ...
        # Tạo dict kết quả cuối cùng với nodes đã xử lý
        ...
        return jsonify(final_graph_data) # Trả về dữ liệu đã có screenshot_url
