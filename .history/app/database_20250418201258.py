...
def get_transition_details(transition_id: int) -> dict | None:
    """Lấy chi tiết một transition bằng transition_id, bao gồm strategy_id gốc."""
    ...
    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # <<< SỬA SQL: Liệt kê các cột có thật trong bảng stage_transitions (t) >>>
        sql = """
            SELECT
                t.transition_id, t.current_stage_id, t.user_intent,
                t.condition_type, t.condition_value, -- Thêm các cột điều kiện
                t.next_stage_id, t.action_to_suggest, t.response_template_ref, t.priority,
                ss.strategy_id -- Lấy strategy_id từ bảng stages
            FROM
                public.stage_transitions t
            LEFT JOIN -- Dùng LEFT JOIN để vẫn lấy được transition nếu stage không tồn tại (dù ít xảy ra)
        ...
        print(traceback.format_exc()) # In traceback để xem dòng lỗi SQL
    except Exception as e:
