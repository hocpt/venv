...
def add_transition_control():
    ...
    strategy_id = request.args.get('strategy_id') or request.form.get('strategy_id')
    current_stage_id_prefill = request.args.get('current_stage_id') # Dùng cho GET
    ...
    strategy_details = None
    strategy_stages_for_dropdowns = [] # Sẽ chứa stages của strategy này
    all_control_stages = [] # Sẽ chứa stages của tất cả control strategies
    all_macros = []
    all_templates = []
    ...
    try:
        ...
        # Lấy stages của strategy hiện tại (cho dropdown Current Stage)
        ...
        # Lấy TẤT CẢ stages loại 'control' (cho dropdown Next Stage)
        all_control_stages = db.get_stages_by_type('control') or [] # Gọi hàm đã tạo
        if all_control_stages is None:
            flash("Lỗi khi tải danh sách Control Stages cho dropdown.", "error")
            all_control_stages = []
        ...
        all_templates = db.get_all_template_refs_with_details() # Đảm bảo hàm này tồn tại và hoạt động
        ...
        try:
            ...
            if success:
                ...
                return render_template('admin_add_transition_control.html',
                                       ...
                                       current_stage_id_prefill=current_stage_id, # <<< Truyền current_stage_id
                                       strategy_stages=strategy_stages_for_dropdowns,
                                       all_stages=all_control_stages,
                                       all_macros=all_macros, all_templates=all_templates,
                                       valid_intents=valid_intents, valid_condition_types=valid_condition_types,
                                       current_data=request.form)
        except Exception as e:
    ...
    return render_template('admin_add_transition_control.html',
                           ...
                           strategy_stages=strategy_stages_for_dropdowns, # List stage của strategy này
                           all_stages=all_control_stages,      # List stage của type 'control'
                           all_macros=all_macros, all_templates=all_templates,
                           valid_intents=valid_intents, valid_condition_types=valid_condition_types)
...
def edit_transition_control(transition_id):
    ...
    if request.method == 'POST':
        ...
        if current_app.logger.errors: # Nếu có lỗi validation
            for error in current_app.logger.errors: flash(error, "warning")
