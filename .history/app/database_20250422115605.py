...
def add_phone_action_logs(assignment_id: int, device_id: str, account_id: str,
    ...
    if not assignment_id or not logs or not isinstance(logs, list):
        logger.warning(f"Input không hợp lệ cho add_phone_action_logs (assignment_id={assignment_id}, #logs={len(logs) if isinstance(logs, list) else 0})")
        # Nếu không có logs thì coi như thành công, không cần ghi gì
        return True if not logs else False
    ...
    try:
        ...
        # Lưu ý: Tên cột trong SQL phải khớp với schema CSDL của bạn
        sql_template = """
            INSERT INTO public.phone_action_log
        ...
        for log_entry in logs:
            ...
            # Xử lý params (có thể là dict hoặc chuỗi JSON từ client)
            params_data = log_entry.get('params')
            params_json_str_to_save = None
            if isinstance(params_data, dict):
                try:
                    # Chuyển dict thành chuỗi JSON để lưu vào cột JSONB
                 ...
                 # Nếu client gửi sẵn string JSON, kiểm tra lại xem có hợp lệ không
                 try:
                      json.loads(params_data) # Chỉ để validate
                      params_json_str_to_save = params_data
                 except json.JSONDecodeError:
                      logger.warning(f"Params string từ client không phải JSON hợp lệ: {params_data}")
                      params_json_str_to_save = json.dumps({"raw_param_string": params_data}) # Lưu dạng thô nếu lỗi
            ...
            data_tuples.append((
                ...
                params_json_str_to_save, # Chuỗi JSON hoặc None (sẽ ép kiểu jsonb)
                ...
                structured_ui_state_json # <<< Dùng state UI đã xử lý cho TẤT CẢ log trong batch này
            ))
        ...
        if data_tuples:
            ...
            psycopg2.extras.execute_values(
                ...
                template=None, page_size=100 # page_size có thể điều chỉnh
            )
            conn.commit()
            success = True
