...
import json
import importlib
import math # Cần cho phân trang History (nếu dùng ceil)
from flask import (
    Blueprint, render_template, request, redirect, url_for, flash, current_app
)
from apscheduler.jobstores.base import JobLookupError # Để bắt lỗi job không tìm thấy
from datetime import datetime
...
def edit_scheduled_job(job_id):
    ...
    if request.method == 'POST':
        ...
        if not trigger_type or trigger_type not in TRIGGER_TYPES:
             flash("Trigger Type không hợp lệ.", "warning")
             # Truyền lại job_details cũ và dữ liệu form sai vào template
        ...
        if not db_success:
             ...
             # Vẫn dùng job_details cũ để hiển thị lại form
             return render_template('admin_edit_scheduled_job.html', title=f"Sửa Tác vụ '{job_id}'",
        ...
        if scheduler:
            try:
                ...
                # 1. Modify job (thay đổi trigger và args)
                # Truyền job_id positionally
                scheduler.modify_job(job_id, trigger=trigger_type, **trigger_args_dict)
                print(f"Modified job '{job_id}' trigger/args.")

                # 2. Pause/Resume dựa trên is_enabled
                live_job = scheduler.get_job(job_id)
                if live_job: # Chỉ pause/resume nếu job thực sự tồn tại trong scheduler
                    ...
                    # Nếu job không có trong scheduler (ví dụ chưa từng được add thành công hoặc đã bị xóa)
                    # nhưng is_enabled=True, có thể thử add lại?
                    if is_enabled:
                         print(f"WARNING: Job '{job_id}' not found in scheduler after DB update. Attempting to add it...")
                         try:
                             # Cần import lại function
                             function_path = job_details.get('job_function_path') # Lấy path từ data cũ
                             if function_path:
                                 module_path, func_name = function_path.rsplit('.', 1)
                                 module = importlib.import_module(module_path)
                                 func = getattr(module, func_name)
                                 scheduler.add_job(id=job_id, func=func, trigger=trigger_type, replace_existing=True, **trigger_args_dict)
                                 print(f"Successfully ADDED job '{job_id}' to scheduler after edit.")
                             else:
                                  print(f"ERROR: Cannot re-add job '{job_id}', missing function path in details.")
                 ...
                 scheduler_error = f"Lưu DB thành công, nhưng Trigger Args không hợp lệ cho Trigger Type '{trigger_type}' khi cập nhật scheduler: {trigger_err}. Job có thể không chạy đúng lịch mới."
                 print(f"ERROR updating job '{job_id}' in scheduler: {scheduler_error}")
                 flash(scheduler_error, "warning")
            except JobLookupError:
                 # Lỗi này ít xảy ra hơn nếu logic kiểm tra live_job ở trên hoạt động
             ...
             flash("Đã cập nhật cấu hình job trong DB, nhưng không thể cập nhật scheduler (scheduler chưa được khởi tạo?).", "warning")
