...
from .graph_db import get_screen_with_elements # Thêm get_screen_node_by_id nếu bạn dùng nó ở chỗ khác
from .database import get_last_detailed_ui_state_for_screen, get_element_classifications_for_screen
from .phone.utils import determine_screen_id_from_state
PER_PAGE_RULES = 30
...
@admin_bp.route('/screen/<string:screen_id>/elements', methods=['GET'])
#@login_required
def admin_screen_elements(screen_id): # Tên hàm này có thể là `admin_screen_elements_view`
    current_app.logger.info(f"Accessing admin elements page for screen_id: {screen_id}")

    # 1. Lấy dữ liệu Screen từ Neo4j sử dụng get_screen_with_elements
    screen_node_neo4j = get_screen_with_elements(screen_id)
    if not screen_node_neo4j:
        flash(f'Screen node {screen_id} not found in Neo4j.', 'danger')
        # Điều chỉnh url_for cho đúng với tên blueprint và tên route của trang mapping viewer
        # Giả sử tên route là 'admin_mapping_viewer' và cần app_name
        default_app_for_redirect = current_app.config.get("DEFAULT_APP_FOR_MAPPING", "_unknown_app_") # Lấy từ config hoặc đặt mặc định
        return redirect(url_for(f'{admin_bp.name}.admin_mapping_viewer', app_name=default_app_for_redirect))

    # Chuyển Neo4j Node object thành dict nếu nó chưa phải là dict
    screen_data_neo4j = dict(screen_node_neo4j) if not isinstance(screen_node_neo4j, dict) else screen_node_neo4j
    
    app_name = screen_data_neo4j.get('appName')
    # activity_name = screen_data_neo4j.get('activityName') # Có thể lấy nếu cần hiển thị
    original_screen_width = screen_data_neo4j.get('width')
    original_screen_height = screen_data_neo4j.get('height')
    actual_stored_hash = screen_data_neo4j.get('structureHash')
    neo4j_screenshot_path = screen_data_neo4j.get('screenshot_path')

    # 2. Xây dựng screenshot_url
    screenshot_url = None
    if neo4j_screenshot_path:
        screenshot_url_base = current_app.config.get('SCREENSHOT_URL_BASE', '/static/screenshots')
        # Logic xây dựng URL dựa trên việc neo4j_screenshot_path có bao gồm app_name hay không
        # và cấu trúc thư mục SCREENSHOT_STORAGE_PATH của bạn.
        # Ví dụ: nếu neo4j_screenshot_path đã là "appName/filename.jpg":
        if app_name and neo4j_screenshot_path.startswith(f"{app_name}/"):
            screenshot_url = f"{screenshot_url_base}/{neo4j_screenshot_path}"
        elif app_name: # Nếu neo4j_screenshot_path chỉ là "filename.jpg"
            screenshot_url = f"{screenshot_url_base}/{app_name}/{neo4j_screenshot_path}"
        else: # Trường hợp không có app_name rõ ràng hoặc cấu trúc khác
            screenshot_url = f"{screenshot_url_base}/{neo4j_screenshot_path}"
        current_app.logger.debug(f"Constructed screenshot URL for screen {screen_id}: {screenshot_url}")
    else:
        current_app.logger.warning(f"No screenshot_path found in Neo4j for screen {screen_id}")


    # 3. Lấy danh sách elements chi tiết từ log PostgreSQL
    detailed_ui_state_from_db = get_last_detailed_ui_state_for_screen(screen_id)
    current_app.logger.debug(f"Type from get_last_detailed_ui_state_for_screen: {type(detailed_ui_state_from_db)}")
    current_app.logger.debug(f"Data from get_last_detailed_ui_state_for_screen (first 200): {str(detailed_ui_state_from_db)[:200]}")

    processed_ui_state_for_analysis = {}
    if detailed_ui_state_from_db:
        if isinstance(detailed_ui_state_from_db, str): # Nếu là chuỗi JSON
            try:
                processed_ui_state_for_analysis = json.loads(detailed_ui_state_from_db)
                current_app.logger.debug(f"Successfully json.loads detailed UI state for screen {screen_id}")
            except json.JSONDecodeError as e:
                flash(f"Error decoding detailed UI state JSON string from PostgreSQL for screen {screen_id}: {e}", "warning")
                current_app.logger.error(f"JSONDecodeError for screen {screen_id}: {e}. Data: {detailed_ui_state_from_db[:200]}")
        elif isinstance(detailed_ui_state_from_db, dict): # Nếu đã là dict
            processed_ui_state_for_analysis = detailed_ui_state_from_db
            current_app.logger.debug(f"Detailed UI state from PostgreSQL was already a dict for screen {screen_id}")
        else:
            flash(f"Unexpected data type ({type(detailed_ui_state_from_db)}) for detailed UI state from PostgreSQL.", "danger")
            current_app.logger.error(f"Unexpected data type for detailed UI state: {type(detailed_ui_state_from_db)} for screen {screen_id}")
    else:
        current_app.logger.warning(f"No detailed UI state found in PostgreSQL for screen {screen_id}")
        flash(f"No detailed UI state found in PostgreSQL for screen {screen_id}. Element list and hash analysis might be incomplete.", "info")

    # Trích xuất elements_list_for_display từ processed_ui_state_for_analysis
    # Ưu tiên 'simplified_elements' nếu có, sau đó đến 'elements'
    elements_list_for_display = processed_ui_state_for_analysis.get('simplified_elements', 
                                                                processed_ui_state_for_analysis.get('elements', []))
    if not elements_list_for_display and 'children' in processed_ui_state_for_analysis:
        # Nếu đây là cấu trúc cây, bạn cần một hàm để làm phẳng nó.
        # Ví dụ: elements_list_for_display = flatten_ui_tree_to_elements_list(processed_ui_state_for_analysis)
        current_app.logger.warning(f"No 'simplified_elements' or 'elements' list in processed UI state for {screen_id}. If it's a tree, flattening is needed.")
        # Tạm thời để rỗng nếu không có dữ liệu phẳng trực tiếp
        elements_list_for_display = []

    # 4. Phân tích lại `processed_ui_state_for_analysis` để lấy `contributing_features` và `string_hashed`
    contributing_features_list = []
    string_that_was_hashed = "Not available."
    recalculated_screen_id_for_debug = None
    actual_stored_hash = screen_data_neo4j.get('structureHash') # Lấy từ screen_data_neo4j
    app_name = screen_data_neo4j.get('appName') # Lấy từ screen_data_neo4j

    # Chỉ gọi determine_screen_id_from_state nếu processed_ui_state_for_analysis là một dict hợp lệ và có app_name
    if isinstance(processed_ui_state_for_analysis, dict) and processed_ui_state_for_analysis and app_name:
        try:
            # Đảm bảo processed_ui_state_for_analysis là dict ở đây
            recalculated_screen_id, _, temp_contrib_features, _, temp_str_to_hash = \
                determine_screen_id_from_state(processed_ui_state_for_analysis, app_name)
            # ... (gán giá trị như cũ) ...
            contributing_features_list = temp_contrib_features
            string_that_was_hashed = temp_str_to_hash
            recalculated_screen_id_for_debug = recalculated_screen_id
        except Exception as e:
            # ... (xử lý lỗi như cũ) ...
            current_app.logger.error(f"Error during hashing analysis for screen {screen_id}: {e}", exc_info=True)
            flash(f"Could not fully analyze hashing components for screen {screen_id}. Error: {e}", "warning")
    elif not app_name:
        string_that_was_hashed = "appName not found in Neo4j data, cannot perform hashing analysis."
        current_app.logger.warning(f"appName not found for screen {screen_id} in Neo4j data.")
    elif not isinstance(processed_ui_state_for_analysis, dict) or not processed_ui_state_for_analysis :
        string_that_was_hashed = "Processed UI state for analysis is not a valid dictionary or is empty."
        current_app.logger.warning(f"Processed UI state for analysis is invalid or empty for screen {screen_id}.")
        
    # 5. Lấy và xử lý thông tin phân loại elements từ PostgreSQL
    element_classifications_raw = get_element_classifications_for_screen(screen_id)
    element_classifications_map = {item['element_id']: item for item in element_classifications_raw} if element_classifications_raw else {}
    
    # Gắn thông tin phân loại và thông tin đóng góp hash vào elements_list_for_display
    contributing_attributes_map = {} # {xpath_id: {attribute1: value1, ...}}
    for feature in contributing_features_list:
        xpath = feature.get('xpath_id')
        attr = feature.get('attribute')
        if xpath and attr:
            if xpath not in contributing_attributes_map:
                contributing_attributes_map[xpath] = {}
            contributing_attributes_map[xpath][attr] = feature.get('value')

    for el_display_item in elements_list_for_display:
        el_xpath_id = el_display_item.get('xpath_id') # Đảm bảo element có 'xpath_id'
        
        # Gán thông tin phân loại
        if el_xpath_id and el_xpath_id in element_classifications_map:
            el_display_item['classification_info'] = element_classifications_map[el_xpath_id]
        else:
            el_display_item['classification_info'] = None # Hoặc một dict rỗng

        # Gán thông tin đóng góp vào hash
        el_display_item['contributing_attributes'] = {}
        el_display_item['is_contributing_to_hash_overall'] = False
        if el_xpath_id and el_xpath_id in contributing_attributes_map:
            el_display_item['contributing_attributes'] = contributing_attributes_map[el_xpath_id]
            if contributing_attributes_map[el_xpath_id]: # Nếu dict không rỗng
                el_display_item['is_contributing_to_hash_overall'] = True


    return render_template('admin_screen_elements.html',
                           screen_id_prop=screen_id,
                           screen_data_neo4j=screen_data_neo4j,
                           original_screen_width=original_screen_width,
                           original_screen_height=original_screen_height,
                           screenshot_url=screenshot_url,
                           elements_to_display=elements_list_for_display,
                           # contributing_features_list=contributing_features_list, # Không cần truyền riêng nếu đã xử lý trong elements_to_display
                           string_hashed=string_that_was_hashed,
                           actual_hash=actual_stored_hash,
                           recalculated_screen_id_for_debug=recalculated_screen_id_for_debug,
                           title=f"Elements for Screen {screen_id}")
