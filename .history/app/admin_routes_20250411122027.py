...
def toggle_scheduled_job(job_id):
    ...
    live_scheduler = current_app.scheduler
    scheduler_error = None
    print(f"\nDEBUG (toggle_scheduled_job): Checking scheduler before interaction for job '{job_id}'...")
    print(f"DEBUG: Scheduler object accessible in route: {live_scheduler}")
    if live_scheduler:
            try:
                all_live_jobs = live_scheduler.get_jobs()
    ...
    if live_scheduler:
            try:
                live_job = live_scheduler.get_job(job_id) # <<< Dùng live_scheduler
                if not live_job:
                     raise JobLookupError(job_id)
    
                if new_enabled_state:
                    print(f"Resuming job '{job_id}'...")
                    live_scheduler.resume_job(job_id) # <<< Dùng live_scheduler
                    flash(f"Đã kích hoạt (resume) job '{job_id}'.", 'success')
                else:
                    print(f"Pausing job '{job_id}'...")
                    live_scheduler.pause_job(job_id) # <<< Dùng live_scheduler
            ...
            flash("Đã cập nhật trạng thái job trong DB, nhưng không thể tương tác với scheduler (chưa khởi tạo đúng cách?).", "warning")
