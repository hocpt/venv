...
def get_interactions_for_suggestion(min_timestamp, status_filter: list = None, limit: int = 100) -> list[dict] | None:
    ...
    (Cần hoàn thiện logic, ví dụ: chỉ lấy những cái chưa được xử lý)
    """
    interactions = None
    conn = get_db_connection()
    if not conn: return None
    cur = None
    params = [min_timestamp, limit]
    status_where = ""
    if status_filter:
         # Tạo placeholder (%s, %s, ...) cho danh sách status
         status_placeholders = ', '.join(['%s'] * len(status_filter))
         status_where = f"AND status IN ({status_placeholders})"
         params = status_filter + params # Thêm status vào đầu list params

    try:
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        # Chỉ lấy các cột cần thiết cho việc tạo đề xuất
        sql = f"""
            SELECT history_id, received_text, sent_text, detected_user_intent, stage_id, strategy_id
            FROM interaction_history
            WHERE timestamp >= %s {status_where} AND sent_text IS NOT NULL AND received_text IS NOT NULL
            -- TODO: Thêm điều kiện để không lấy lại những cái đã xử lý
            ORDER BY timestamp ASC -- Xử lý từ cũ đến mới
            LIMIT %s;
        """
...
def add_suggestion(keywords: str, template_text: str, source_examples: dict) -> bool:
    """Lưu một đề xuất mới vào bảng suggested_rules."""
    if not keywords or not template_text: return False
    conn = get_db_connection()
    if not conn: return False
    cur = None
    success = False
    try:
        cur = conn.cursor()
        # Cần import json để chuyển dict thành chuỗi JSON cho cột jsonb
        import json
        source_examples_json = json.dumps(source_examples)

        sql = """
            INSERT INTO suggested_rules (suggested_keywords, suggested_template_text, source_examples, status)
            VALUES (%s, %s, %s, %s);
        """
        # Status mặc định là 'pending' đã được định nghĩa trong DB schema
        params = (keywords, template_text, source_examples_json, 'pending')
        cur.execute(sql, params)
        conn.commit()
        success = True
    except psycopg2.Error as db_err:
