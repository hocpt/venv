...
def get_elements_for_screen(screen_id: str) -> list[dict] | None:
    ...
    Mỗi element trong list trả về là một dictionary các thuộc tính của node :Element.
    """
    logger = current_app.logger if current_app else log # Sử dụng logger
    if not screen_id:
        logger.warning("get_elements_for_screen: screen_id là bắt buộc.")
        return [] # Trả về list rỗng nếu không có screen_id

    driver = get_driver()
    if not driver:
        logger.error("get_elements_for_screen: Không lấy được Neo4j driver.")
        return None # Trả về None nếu lỗi driver
    ...
    try:
        ...
        with driver.session(database=db_name) as session:
            ...
            ORDER BY e.element_id // Hoặc một thứ tự khác nếu muốn, ví dụ: e.neo4j_internal_id
            """
            ...
            for record in result:
                ...
                if props:
                    # --- CHUYỂN ĐỔI DATETIME TRƯỚC KHI APPEND ---
                    processed_props = {}
                    for key, value in props.items():
                        # Kiểm tra nếu giá trị là đối tượng DateTime của Neo4j
                        # hoặc DateTime của Python (nếu driver đã tự chuyển đổi)
                        if isinstance(value, (datetime, neo4j.time.DateTime, neo4j.time.Date, neo4j.time.Time, neo4j.time.LocalDateTime)):
                            try:
                                # Chuyển đổi thành chuỗi ISO 8601 UTC
                                # Nếu là naive datetime, giả định là UTC hoặc chuyển về UTC
                                if isinstance(value, datetime) and value.tzinfo is None:
                                    value = value.replace(tzinfo=timezone.utc)
                                processed_props[key] = value.isoformat()
                            except AttributeError: # Nếu không có isoformat (vd: Date, Time của Neo4j)
                                processed_props[key] = str(value)
                            except Exception as dt_err:
                                logger.warning(f"Could not convert datetime object {value} for key '{key}' to ISO string: {dt_err}")
                                processed_props[key] = str(value) # Fallback to string
                        elif isinstance(value, neo4j.spatial.Point): # Xử lý Point nếu có
                            point = value
        ...
        logger.error(f"Lỗi Neo4j (get_elements_for_screen for {screen_id}): {e}", exc_info=True)
        return None
