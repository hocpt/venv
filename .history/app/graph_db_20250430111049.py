...
def create_or_update_transition_relationship(source_screen_id: str, target_screen_id: str, app_name: str, action_data: dict):
    ...
    SỬA LỖI: Sửa cú pháp MERGE và cách truyền tham số.
    """
    ...
    # --- Xử lý identity_props (Giữ nguyên) ---
    identity_props = {
    ...
    # <<< SỬA LỖI LOGIC: Đảm bảo có ít nhất actionType >>>
    if not identity_props_clean.get('actionType'):
         log.warning(f"Cannot MERGE transition from {source_screen_id} to {target_screen_id} for app {app_name} because 'actionType' is missing in action_data: {action_data}")
         return False
    ...
    for key in identity_props_clean: update_props.pop(key, None)
    update_props_clean = {k: v for k, v in update_props.items() if v is not None}

    # === SỬA CÂU LỆNH CYPHER VÀ PARAMS ===
    # 1. Sửa Cypher: Liệt kê tường minh các thuộc tính trong MERGE
    #    Dùng các tên tham số mới (vd: $p_actionType) để tránh trùng với params khác
    cypher = """
    ...
    MERGE (source)-[r:TRANSITION {
        ...
        onElementId: $p_onElementId,
        onElementText: $p_onElementText,
        onElementClass: $p_onElementClass
    }]->(target)
    ...
    # 2. Sửa Params: Tạo dict params hoàn chỉnh, unpack identity_props_clean vào
    params = {
        "sourceId": source_screen_id,
        "targetId": target_screen_id,
        "appName": app_name,
        "updatePropsClean": update_props_clean,
        # Thêm từng thuộc tính định danh làm tham số riêng, xử lý None
        "p_actionType": identity_props_clean.get('actionType'), # Bắt buộc phải có
        "p_onElementId": identity_props_clean.get('onElementId'), # Có thể None
        "p_onElementText": identity_props_clean.get('onElementText'), # Có thể None
        "p_onElementClass": identity_props_clean.get('onElementClass') # Có thể None
    }
    # ======================================

    # Gọi execute_write (đã sửa ở lượt trước để log lỗi tốt hơn)
    success = execute_write(cypher, params)
    if not success:
