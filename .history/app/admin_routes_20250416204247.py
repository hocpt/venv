...
def view_macro_definitions():
    ...
    if not db: flash("Lỗi DB.", "error")
        ...
        try:
            macros = db.get_all_macro_definitions()
            if macros is None: flash("Lỗi tải list macro.", "error"); macros = []
        except Exception as e: print(f"Lỗi load macro definitions: {e}"); flash("Lỗi server.", "error"); macros = []
