...
export function displayNodeDetails(nodeData) {
    if (!panelTextContentDiv && !getAndCheckDetailsPanelDOMElements()) { // Thử lấy lại nếu chưa có
        console.error("DETAILS_PANEL: Không thể hiển thị chi tiết Node, thiếu DOM elements chính.");
        return;
    }
    // Kiểm tra lại các elements cụ thể cần dùng
    if (!panelTextContentDiv || !panelActionsAreaDiv || !panelScreenshotAreaDiv || !panelScreenshotImage || !panelScreenshotContainer) {
        console.error("DETAILS_PANEL: Vẫn thiếu DOM elements cần thiết cho displayNodeDetails.");
        return;
    }
        ...
        const onImageLoadSuccess = () => {
            ...
            function checkSizeAndFetchElements() {
                ...
                if (panelScreenshotImage.clientWidth > 0 && panelScreenshotImage.clientHeight > 0) {
                    ...
                    const screenIdToFetch = nodeData.id; // nodeData.id là screen_id của node đang được click
