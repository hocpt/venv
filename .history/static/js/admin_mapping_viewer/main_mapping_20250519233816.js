...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Lấy các DOM elements chung (luôn tồn tại trên trang, không phụ thuộc vào selected_app_name)
    ...
    const refreshButton = document.getElementById(APP_CONFIG.DOM_ELEMENT_IDS.refreshGraphButton); // ID này cần khớp với HTML và config
    const editModalElement = document.getElementById(APP_CONFIG.DOM_ELEMENT_IDS.editTransitionModal);


    if (editModalElement) { // Chỉ khởi tạo nếu modal thực sự tồn tại
    ...
    if (APP_CONFIG.APP_NAME_FROM_FLASK) {
        ...
        // Chỉ gọi init nếu các thành phần cần thiết đã được tìm thấy
        ...
        // Đảm bảo loading indicator bị ẩn nếu không có app
