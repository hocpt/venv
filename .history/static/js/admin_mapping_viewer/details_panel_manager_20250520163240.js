...
import { escapeHtml, sendApiRequest as sendApiRequestFromUtils } from './utils_mapping.js';
import { openEditTransitionModal } from './modal_edit_transition.js';

// Khai báo các biến DOM element ở scope module
let panelTextContentDiv, panelActionsAreaDiv, panelScreenshotAreaDiv,
    panelScreenshotContainer, panelScreenshotImage;

/**
 * Lấy và kiểm tra các DOM elements cần thiết cho panel chi tiết.
 * Được gọi bởi initDetailsPanelManager và có thể được gọi lại nếu elements là null.
 * @returns {boolean} True nếu tất cả các elements chính được tìm thấy, ngược lại là false.
 */
function getAndCheckDetailsPanelDOMElements() {
    const IDS = APP_CONFIG.DOM_ELEMENT_IDS;
    let allCoreElementsFound = true;

    panelTextContentDiv = document.getElementById(IDS.detailsPanelTextContent);
    if (!panelTextContentDiv) { console.warn(`DETAILS_PANEL: Element '${IDS.detailsPanelTextContent}' not found.`); allCoreElementsFound = false; }

    panelActionsAreaDiv = document.getElementById(IDS.detailsPanelActionsArea);
    if (!panelActionsAreaDiv) { console.warn(`DETAILS_PANEL: Element '${IDS.detailsPanelActionsArea}' not found.`); allCoreElementsFound = false; }

    panelScreenshotAreaDiv = document.getElementById(IDS.detailsPanelScreenshotArea);
    if (!panelScreenshotAreaDiv) { console.warn(`DETAILS_PANEL: Element '${IDS.detailsPanelScreenshotArea}' not found.`); allCoreElementsFound = false; }

    panelScreenshotContainer = document.getElementById(IDS.detailsPanelScreenshotContainer);
    if (!panelScreenshotContainer) { console.warn(`DETAILS_PANEL: Element '${IDS.detailsPanelScreenshotContainer}' not found.`); allCoreElementsFound = false; }

    panelScreenshotImage = document.getElementById(IDS.detailsPanelScreenshotImage);
    if (!panelScreenshotImage) { console.warn(`DETAILS_PANEL: Element '${IDS.detailsPanelScreenshotImage}' not found.`); allCoreElementsFound = false; }

    return allCoreElementsFound;
}

export function initDetailsPanelManager() {
    console.log("DETAILS_PANEL: initDetailsPanelManager called.");
    if (!getAndCheckDetailsPanelDOMElements()) {
        console.error("DETAILS_PANEL: Một hoặc nhiều DOM elements của panel chi tiết không tìm thấy khi init. Chức năng có thể bị ảnh hưởng nghiêm trọng nếu các elements này không được render sau đó.");
    } else {
        console.log("DETAILS_PANEL: All required DOM elements for details panel seem to be found at init.");
    }
}

export function showDefaultDetailsMessage() {
    if (!panelTextContentDiv && !getAndCheckDetailsPanelDOMElements()) {
        console.warn("DETAILS_PANEL: showDefaultDetailsMessage - Không thể reset panel, elements không tìm thấy.");
        return;
    }
    ...
    if (panelScreenshotImage) { panelScreenshotImage.src = ''; panelScreenshotImage.style.display = 'none'; }
    if (panelScreenshotContainer) panelScreenshotContainer.innerHTML = ''; // Xóa hết overlays và ảnh cũ
}
...
export function displayEdgeDetails(edgeData) {
    if (!panelTextContentDiv && !getAndCheckDetailsPanelDOMElements()) {
        console.error("DETAILS_PANEL: displayEdgeDetails - Không thể hiển thị chi tiết Edge, thiếu DOM elements chính.");
        return;
    }
    if (!panelTextContentDiv || !panelActionsAreaDiv || !panelScreenshotAreaDiv) {
        console.error("DETAILS_PANEL: displayEdgeDetails - Vẫn thiếu một số DOM elements cần thiết.");
        return;
    }
    console.log("DETAILS_PANEL: Displaying Edge Details:", JSON.parse(JSON.stringify(edgeData)));

    panelScreenshotAreaDiv.style.display = 'none';
    panelActionsAreaDiv.innerHTML = '';

    // ... (code hiển thị thông tin text của Edge như cũ) ...
    let edgeDetailsHtml = `<h5>Chi tiết Cạnh (Transition)</h5><ul class="list-group list-group-flush"> <li class="list-group-item"><strong>ID (Cytoscape):</strong> <code>${escapeHtml(edgeData.id)}</code></li>`; if (edgeData.neo4j_edge_id) edgeDetailsHtml += `<li class="list-group-item"><strong>ID (Neo4j):</strong> <code>${escapeHtml(edgeData.neo4j_edge_id)}</code></li>`; edgeDetailsHtml += `<li class="list-group-item"><strong>Nguồn:</strong> <code>${escapeHtml(edgeData.source)}</code></li> <li class="list-group-item"><strong>Đích:</strong> <code>${escapeHtml(edgeData.target)}</code></li> <li class="list-group-item"><strong>Loại H.Động:</strong> ${escapeHtml(edgeData.action_type || 'N/A')}</li> <li class="list-group-item"><strong>Macro Code:</strong> <code>${escapeHtml(edgeData.macro_code || 'N/A')}</code></li> <li class="list-group-item"><strong>Element ID (tương tác):</strong> <code>${escapeHtml(edgeData.element_id || 'N/A')}</code></li> <li class="list-group-item"><strong>Loại ID (element):</strong> ${escapeHtml(edgeData.identifier_type || 'N/A')}</li> <li class="list-group-item"><strong>Element Text:</strong> ${escapeHtml(edgeData.element_text || '--')}</li> <li class="list-group-item"><strong>Trạng thái Cạnh:</strong> <span class="badge bg-secondary">${escapeHtml(edgeData.status || 'N/A')}</span></li> <li class="list-group-item"><strong>Lần thử:</strong> ${edgeData.attempt_count !== undefined ? edgeData.attempt_count : 'N/A'}</li> <li class="list-group-item"><strong>Thành công:</strong> ${edgeData.success_count !== undefined ? edgeData.success_count : 'N/A'}</li>`; if (edgeData.params_json) { try { const paramsObj = JSON.parse(edgeData.params_json); const formattedParams = JSON.stringify(paramsObj, null, 2); edgeDetailsHtml += `<li class="list-group-item"><strong>Params (JSON):</strong> <pre><code style="white-space: pre-wrap; word-break: break-all;">${escapeHtml(formattedParams)}</code></pre></li>`; } catch (e) { edgeDetailsHtml += `<li class="list-group-item"><strong>Params (Raw):</strong> <pre><code>${escapeHtml(edgeData.params_json)}</code></pre></li>`; } } else { edgeDetailsHtml += `<li class="list-group-item"><strong>Params:</strong> N/A</li>`; } edgeDetailsHtml += `</ul>`;
    ...
    if (edgeData.neo4j_edge_id) {
        ...
        editButton.addEventListener('click', function () {
            ...
            if (typeof openEditTransitionModal === 'function') {
                openEditTransitionModal(edgeData);
            } else {
                console.error("DETAILS_PANEL: Hàm openEditTransitionModal chưa được định nghĩa.");
...
function drawScreenOverlays(imgElement, elementsData, nodeOriginalWidth, nodeOriginalHeight) {
    ...
    // Chỉ xóa các overlay cũ, không xóa thẻ img hoặc loading text
    panelScreenshotContainer.querySelectorAll('.element-overlay').forEach(el => el.remove());
    // Xóa các thông báo lỗi/trạng thái cũ liên quan đến overlay
    panelScreenshotContainer.querySelectorAll('.overlay-dimension-error-dsp, .text-warning, .text-muted.small.fst-italic.mt-1').forEach(el => {
        if (!el.classList.contains('loading-image-text')) { // Giữ lại loading text nếu có
            el.remove();
        }
    });
    ...
    if (!elementsData || !Array.isArray(elementsData) || elementsData.length === 0) { // Kiểm tra là array
        ...
        noElementsMsg.textContent = '(Không có element nào được trả về từ API để vẽ trên ảnh.)';
    ...
    elementsData.forEach((elData, index) => {
        ...
        // Sử dụng element_id nếu có, fallback về resource_id, hoặc tạo ID tạm thời cho log
    ...
    if (drawnCount === 0 && elementsData.length > 0) {
        console.warn("DETAILS_PANEL (drawScreenOverlays): Có dữ liệu elements nhưng không vẽ được overlay nào. Kiểm tra logic tính toán tọa độ/kích thước và dữ liệu element.");
        const noOverlayMsg = document.createElement('p');
        noOverlayMsg.className = 'text-warning small fst-italic mt-1';
        noOverlayMsg.textContent = '(Có elements nhưng không thể vẽ overlay. Kiểm tra console log để biết chi tiết.)';
