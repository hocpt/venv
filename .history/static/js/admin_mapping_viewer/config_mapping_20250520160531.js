...
export let APP_CONFIG = {
    ...
    DOM_ELEMENT_IDS: {
        ...
        editTransitionStatusSelect: 'editTransitionStatusSelect', // Đổi tên key để nhất quán (ví dụ)
...
export function initializeAppConfig(pageConfig) {
    ...
    if (!pageConfig) {
        ...
        console.log("CONFIG_MAPPING: Default values will be used for API_BASE_URLS.SCREEN_ELEMENTS:", APP_CONFIG.API_BASE_URLS.SCREEN_ELEMENTS);
        console.log("CONFIG_MAPPING: Default values will be used for DOM_ELEMENT_IDS.editTransitionModal:", APP_CONFIG.DOM_ELEMENT_IDS.editTransitionModal);
        return;
    }
    ...
    if (pageConfig.apiBaseUrls) {
        console.log("CONFIG_MAPPING: Overwriting API_BASE_URLS from pageConfig. Received pageConfig.apiBaseUrls.SCREEN_ELEMENTS:", pageConfig.apiBaseUrls.screenElements);
        APP_CONFIG.API_BASE_URLS = { ...APP_CONFIG.API_BASE_URLS, ...pageConfig.apiBaseUrls };
    } else {
        console.warn("CONFIG_MAPPING: pageConfig.apiBaseUrls không được cung cấp. Using defaults for API_BASE_URLS.");
    }

    if (pageConfig.domElementIds && typeof pageConfig.domElementIds === 'object') {
        console.log("CONFIG_MAPPING: Overwriting DOM_ELEMENT_IDS from pageConfig. Received pageConfig.domElementIds:", JSON.parse(JSON.stringify(pageConfig.domElementIds)));
        APP_CONFIG.DOM_ELEMENT_IDS = { ...APP_CONFIG.DOM_ELEMENT_IDS, ...pageConfig.domElementIds };
    } else {
        console.warn("CONFIG_MAPPING: pageConfig.domElementIds không phải là object hoặc không được cung cấp. Using defaults for DOM_ELEMENT_IDS.");
    }
