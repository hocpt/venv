...
export function initCytoscapeManager(appNameToLoad) {
    ...
    console.log(`CYTOSCAPE_MANAGER: Attempting to get graphContainer with ID '${graphContainerId}'. Found:`, graphContainerElement ? 'YES' : 'NO');
    console.log(`CYTOSCAPE_MANAGER: Attempting to get loadingIndicator with ID '${loadingIndicatorId}'. Found:`, loadingIndicatorElement ? 'YES' : 'NO');

    if (!graphContainerElement) {
        console.error(`CYTOSCAPE_MANAGER: CRITICAL - Graph container ('${graphContainerId}') KHÔNG TÌM THẤY trong DOM. Đồ thị sẽ không hoạt động.`);
        return false;
    }
    // loadingIndicator là tùy chọn, nhưng nếu có ID mà không tìm thấy thì cũng cảnh báo
    if (loadingIndicatorId && !loadingIndicatorElement) {
        console.warn(`CYTOSCAPE_MANAGER: Loading indicator ('${loadingIndicatorId}') KHÔNG TÌM THẤY, nhưng ID đã được định nghĩa.`);
    }
