...
import { sendApiRequest, escapeHtml } from './utils_mapping.js';
import { displayNodeDetails, displayEdgeDetails, showDefaultDetailsMessage } from './details_panel_manager.js';

let cy = null;
// Không khai báo graphContainerElement và loadingIndicatorElement ở đây nữa,
// chúng sẽ là biến cục bộ trong initCytoscapeManager và được truyền đi nếu cần.
...
export function initCytoscapeManager(appNameToLoad) {
    ...
    // Lấy DOM elements và gán vào biến cục bộ
    ...
    if (currentAppName) {
        // Truyền các DOM element đã lấy được vào fetchAndRenderGraph
...
export async function fetchAndRenderGraph(appName, graphContainer, loadingIndicator) {
    if (!graphContainer) { // Kiểm tra tham số truyền vào
        console.error("CYTOSCAPE_MANAGER: fetchAndRenderGraph - Graph container là null.");
        const graphContainerById = document.getElementById(APP_CONFIG.DOM_ELEMENT_IDS.graphContainer);
        if (graphContainerById) { // Thử lấy lại một lần nữa nếu bị null
            graphContainerById.innerHTML = '<p class="text-danger text-center mt-5">Lỗi khởi tạo Cytoscape (container không được truyền đúng).</p>';
        }
        return;
    }
    // loadingIndicator có thể null, xử lý nhẹ nhàng hơn
    if (loadingIndicator) {
    ...
    try {
        ...
        attachCytoscapeEventListeners(); // Hàm này sẽ sử dụng biến 'cy' toàn cục của module
