...
export function initCytoscapeManager(appNameToLoad) {
    ...
    graphContainer = document.getElementById(graphContainerId);
    loadingIndicator = document.getElementById(loadingIndicatorId);

    console.log(`CYTOSCAPE_MANAGER: Attempting to get graphContainer with ID '${graphContainerId}'. Found:`, graphContainer !== null);
    console.log(`CYTOSCAPE_MANAGER: Attempting to get loadingIndicator with ID '${loadingIndicatorId}'. Found:`, loadingIndicator !== null);

    if (!graphContainer || !loadingIndicator) {
        console.error("CYTOSCAPE_MANAGER: Graph container hoặc loading indicator không tìm thấy. Kiểm tra ID trong HTML và config.");
        // Hiển thị lỗi trên UI nếu có thể, ví dụ, nếu có một div chung cho lỗi
        const bodyForError = document.body; // Hoặc một container lỗi cụ thể hơn
        if (bodyForError && !document.getElementById('cytoscapeInitErrorMsg')) {
            const errorDiv = document.createElement('div');
            errorDiv.id = 'cytoscapeInitErrorMsg';
            errorDiv.className = 'alert alert-danger';
            errorDiv.textContent = `Lỗi khởi tạo Cytoscape: Không tìm thấy container đồ thị ('${graphContainerId}') hoặc chỉ báo tải ('${loadingIndicatorId}').`;
            bodyForError.insertBefore(errorDiv, bodyForError.firstChild);
        }
        return; // Không tiếp tục nếu thiếu container
    }
    ...
    if (currentAppName) {
        ...
        // Trường hợp này đã được xử lý ở main_mapping.js
        // nhưng để an toàn, chúng ta cũng có thể đặt thông báo ở đây
        loadingIndicator.style.display = 'none';
        graphContainer.innerHTML = '<p class="text-center text-muted mt-5">Vui lòng chọn một ứng dụng để hiển thị bản đồ.</p>';
