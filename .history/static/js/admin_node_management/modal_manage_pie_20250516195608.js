...
function handleElementSelectionFromVisualizer(elementOriginalIndex) {
    ...
    if (!rawElementsDataForModal || rawElementsDataForModal.length === 0) {
        ...
        console.warn("[MODAL_PIE] rawElementsDataForModal is empty or not yet populated. Cannot select element.");

        return;

    }

    if (elementOriginalIndex < 0 || elementOriginalIndex >= rawElementsDataForModal.length) {

        console.warn("[MODAL_PIE] Invalid elementOriginalIndex:", elementOriginalIndex);
    ...
    if (existingConditionIndex > -1) { // Nếu đã tồn tại -> bỏ chọn (xóa khỏi conditions)
        ...
        // Gợi ý attribute và value dựa trên thuộc tính của element
        ...
        if (elementData.resource_id) {
        ...
        } else if (elementData.description) { // content-description
            ...
            console.warn("[MODAL_PIE] Element không có thuộc tính ưu tiên (resource_id, text, description, class_name) để tự động tạo điều kiện:", elementData);

            // Vẫn thêm condition rỗng để người dùng tự điền

            newCondition.attribute = 'text'; // Hoặc để trống và yêu cầu người dùng chọn
    ...
    console.log("[MODAL_PIE] currentSelectedPieConditions after update:", JSON.parse(JSON.stringify(currentSelectedPieConditions)));
...
// Hàm này sẽ được gọi sau khi modal đã "shown" và ảnh đã "onload"
...
async function processImageAndElementsWhenReady() {
    ...
    if (!screenshotImg || !imageContainer || !elementTextListDiv || !currentManagingNodeData) {
        ...
        console.error("[MODAL_MANAGE_PIE] processImageAndElementsWhenReady: Thiếu các đối tượng DOM hoặc dữ liệu cần thiết.");

        return;

    }

    console.log("[MODAL_MANAGE_PIE] processImageAndElementsWhenReady: Bắt đầu xử lý ảnh và elements.");

    console.log(` Image natural dimensions: ${screenshotImg.naturalWidth}x${screenshotImg.naturalHeight}`);

    console.log(` Image client dimensions: ${screenshotImg.clientWidth}x${screenshotImg.clientHeight}`);


    if (screenshotImg.naturalWidth === 0 || screenshotImg.naturalHeight === 0) {

        console.error("[MODAL_MANAGE_PIE] Ảnh không hợp lệ (kích thước gốc bằng 0).");
    ...
    // Nếu clientWidth/Height vẫn là 0, thử đợi một chút nữa bằng requestAnimationFrame

    if (screenshotImg.clientWidth === 0 || screenshotImg.clientHeight === 0) {

        console.warn("[MODAL_MANAGE_PIE] clientWidth/Height vẫn là 0. Thử đợi 1 frame.");

        await new Promise(resolve => requestAnimationFrame(resolve)); // Đợi 1 frame

        console.log(`[MODAL_MANAGE_PIE] Sau 1 frame, client dimensions: ${screenshotImg.clientWidth}x${screenshotImg.clientHeight}`);

    }



    // Nếu vẫn là 0, có thể có vấn đề CSS nghiêm trọng hoặc ảnh bị ẩn

    if (screenshotImg.clientWidth === 0 || screenshotImg.clientHeight === 0) {

        console.error("[MODAL_MANAGE_PIE] clientWidth/Height vẫn là 0 sau khi đợi. Không thể vẽ overlays.");
        ...
        if (imageContainer && !imageContainer.querySelector('.dimension-error-message')) {
            ...
            errorMsgEl.textContent = 'Lỗi: Không thể xác định kích thước ảnh để hiển thị elements. CSS của modal có thể đang ẩn ảnh.';
    ...
    // Kích thước đã hợp lệ, tiến hành tải elements và vẽ
    ...
    try {
        ...
        if (elementsJsonResponse.success && elementsJsonResponse.elements) {
            ...
            if (rawElementsDataForModal.length > 0 && nodeOriginalWidth && nodeOriginalHeight) {
                ...
                drawInteractiveOverlays( // utils.js
                    ...
                    // Đây là selectionHandler được truyền vào drawInteractiveOverlays:

                    (elementIndexFromOverlay) => {

                        console.log("[MODAL_PIE] Overlay click handler in processImageAndElementsWhenReady received index:", elementIndexFromOverlay);

                        handleElementSelectionFromVisualizer(elementIndexFromOverlay);
        ...
        console.error("[MODAL_MANAGE_PIE] Lỗi tải elements hoặc vẽ overlays:", error);
...
async function openModalWithData(nodeData) {
    ...
    if (imageContainer) {
        ...
        imageContainer.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.ELEMENT_OVERLAY_INTERACTIVE + ', p.text-danger, p.text-warning, p.text-muted, .overlay-dimension-error, .dimension-error-message').forEach(el => el.remove());
    ...
    const tryProcessImage = () => {
        ...
        if (imageLoaded && modalFullyShown) {

            console.log("[MODAL_MANAGE_PIE] Cả ảnh và modal đã sẵn sàng. Bắt đầu processImageAndElementsWhenReady.");

            processImageAndElementsWhenReady();

        } else {

            console.log(`[MODAL_MANAGE_PIE] Chờ... imageLoaded: ${imageLoaded}, modalFullyShown: ${modalFullyShown}`);
    ...
    if (modalEl) {
        ...
        const modalShownHandler = () => {
            ...
            console.log("[MODAL_MANAGE_PIE] Sự kiện 'shown.bs.modal' được kích hoạt.");
    ...
    if (screenshotUrlToLoad && screenshotImg) {
        ...
        screenshotImg.onload = () => {
            ...
            console.log("[MODAL_MANAGE_PIE] Sự kiện 'onload' của ảnh được kích hoạt:", screenshotImg.src);
        ...
        screenshotImg.onerror = () => {
            ...
            console.error("[MODAL_MANAGE_PIE] Lỗi tải ảnh:", screenshotImg.src);
        ...
        modalFullyShown = true; // Nếu không có ảnh, coi như modal đã sẵn sàng cho các bước khác (nếu có)
...
export function initManagePieModal() {
    ...
    if (!managePieModalInstance) {
        ...
        managePieModalInstance = new bootstrap.Modal(modalEl, {
            ...
            // keyboard: false // Giữ lại nếu bạn đã set data-bs-keyboard="false" trên HTML
        ...
        mainActionBtn.addEventListener('click', async function () {
            ...
            if (currentManagingNodeData.nodeStatus === 'defined') {
                ...
                if (finalConditionsToSave.length === 0) {
                    ...
                    if (errorMessagesSpan) errorMessagesSpan.textContent = 'Vui lòng chọn ít nhất một điều kiện.';
