    ...
    function attachTableTriggers() {
        ...
        nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER).forEach(trigger => {
            ...
            newTrigger.addEventListener('click', function () {
                const targetButton = event.target.closest('.define-new-pie-metadata-trigger');
                const row = this.closest('tr');
                const rowElement = targetButton.closest('tr');
                if (!row) return;
                const neo4jIdFromDataset = rowElement.dataset.nodeNeo4jId;

                // ---- DEBUG POINT QUAN TRỌNG ----
                console.log("[TABLE_HANDLER] Click 'Tạo PIE Mới'. Đọc từ dataset:");
                console.log("[TABLE_HANDLER] data-node-neo4j-id:", neo4jIdFromDataset, "(Kiểu:", typeof neo4jIdFromDataset, ")");
                console.log("[TABLE_HANDLER] data-current-screen-id:", row.dataset.currentScreenId);
                if (neo4jIdFromDataset === undefined || neo4jIdFromDataset === null || neo4jIdFromDataset.trim() === "") {
                    console.error("[TABLE_HANDLER] LỖI: nodeNeo4jId đọc từ dataset bị rỗng hoặc không xác định!");
                    alert("Lỗi: Không thể lấy ID của Node Neo4j từ bảng.");
                    return;
                }
                const nodeData = {
                ...
                if (window.openManagePieConditionsModalGlobal) {
                    ...
                    console.error("Hàm openManagePieConditionsModalGlobal chưa được định nghĩa (cần import từ modal_manage_pie.js và gán vào window trong main.js hoặc modal_manage_pie.js).");
