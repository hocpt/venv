            ...
            nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER).forEach(trigger => {
                ...
                newTrigger.addEventListener('click', function () { // 'this' ở đây là newTrigger (ảnh hoặc span N/A)
                    ...
                    if (row) {
                        ...
                        console.log("[TABLE_HANDLER] Clicked trigger. data-node-neo4j-id from row:", neo4jId, "(Type:", typeof neo4jId, ")");

                        if (neo4jId === undefined || neo4jId === null || String(neo4jId).trim() === "") {
                            console.error("[TABLE_HANDLER] LỖI QUAN TRỌNG: nodeNeo4jId đọc từ dataset bị rỗng hoặc không xác định!");
                            alert("Lỗi dữ liệu: Không thể lấy ID Neo4j của Node.");
                            return;
                        }
                        // ... tiếp tục xử lý nodeData ...
                    }
