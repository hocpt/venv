...
// Biến global để dễ truy cập từ các module khác nếu không dùng import/export trực tiếp mọi lúc
// Hoặc bạn có thể export từng hằng số nếu dùng ES Modules

// Flask's url_for generates these URLs. They are passed from the template.
// In a real scenario, these would be initialized in main.js after DOM is loaded,
// reading from data-* attributes or a global JS object set by the template.
// For this example, we'll assume these are correctly populated by the time other scripts run.

const APP_CONFIG = {
    // URL API từ Flask template (sẽ được gán giá trị thực trong main.js hoặc khi template render)
    API_MANAGED_NODES_URL: '',
    API_SCREEN_ELEMENTS_BASE_URL: '', // Placeholder: SCREEN_ID_PLACEHOLDER
    API_CLASSIFY_NODE_BASE_URL: '',   // Placeholder: SCREEN_ID_PLACEHOLDER
    API_DELETE_NODE_BASE_URL: '',     // Placeholder: SCREEN_ID_PLACEHOLDER
    API_GET_PIE_CONDITIONS_URL: '',
    API_UPDATE_PIE_CONDITIONS_BASE_URL: '', // Sẽ là /admin/api/pie_definition
    API_DEFINE_NEW_PIE_WITH_CONDITIONS_URL: '',
    SCREENSHOTS_BASE_URL: '', // Ví dụ: /static/screenshots/
    ADMIN_SCREEN_ELEMENTS_URL_BASE: '', // Placeholder: SCREEN_ID_PLACEHOLDER
    CSRF_TOKEN: '',
    ...
    // Các hằng số cho tên class CSS hoặc ID (nếu cần)
    CSS_CLASSES: {
        MANAGE_PIE_TRIGGER: 'manage-pie-trigger',
        DEFINE_NEW_PIE_TRIGGER: 'define-new-pie-metadata-trigger', // Đã đổi tên trong HTML
    ...
    ELEMENT_IDS: {
        ...
        // Modal định nghĩa metadata PIE mới
...
// Hàm để lấy giá trị config URLs từ global scope (được nhúng từ template)
// Sẽ được gọi trong main.js
function initializeAppConfig(templateConfig) {
    if (templateConfig) {
        APP_CONFIG.API_MANAGED_NODES_URL = templateConfig.API_MANAGED_NODES_URL || '';
        APP_CONFIG.API_SCREEN_ELEMENTS_BASE_URL = templateConfig.API_SCREEN_ELEMENTS_BASE_URL || '';
        APP_CONFIG.API_CLASSIFY_NODE_BASE_URL = templateConfig.API_CLASSIFY_NODE_BASE_URL || '';
        APP_CONFIG.API_DELETE_NODE_BASE_URL = templateConfig.API_DELETE_NODE_BASE_URL || '';
        APP_CONFIG.API_GET_PIE_CONDITIONS_URL = templateConfig.API_GET_PIE_CONDITIONS_URL || '';
        APP_CONFIG.API_UPDATE_PIE_CONDITIONS_BASE_URL = templateConfig.API_UPDATE_PIE_CONDITIONS_BASE_URL || '';
        APP_CONFIG.API_DEFINE_NEW_PIE_WITH_CONDITIONS_URL = templateConfig.API_DEFINE_NEW_PIE_WITH_CONDITIONS_URL || '';
        APP_CONFIG.SCREENSHOTS_BASE_URL = templateConfig.SCREENSHOTS_BASE_URL || '';
        APP_CONFIG.ADMIN_SCREEN_ELEMENTS_URL_BASE = templateConfig.ADMIN_SCREEN_ELEMENTS_URL_BASE || '';
        APP_CONFIG.CSRF_TOKEN = templateConfig.CSRF_TOKEN || '';
    }
}