...
async function onModalShownAndImageReady() {
    console.log("Modal 'shown.bs.modal' event triggered AND image should be ready.");
    console.log(`Image client dimensions on 'shown.bs.modal': ${screenshotImg.clientWidth}x${screenshotImg.clientHeight}`);
    console.log(`Image natural dimensions on 'shown.bs.modal': ${screenshotImg.naturalWidth}x${screenshotImg.naturalHeight}`);
    ...
    if (screenshotImg.naturalWidth === 0 || screenshotImg.naturalHeight === 0) {
        console.error("Modal image naturalWidth/Height is 0. Image might be invalid even on 'shown'.");
    ...
    // Kiểm tra lại clientWidth/Height một lần nữa sau khi modal 'shown'
    if (screenshotImg.clientWidth > 0 && screenshotImg.clientHeight > 0) {
        try {
            ...
            if (elementsJsonResponse.success && elementsJsonResponse.elements) {
                ...
                if (rawElementsDataForModal.length > 0 && nodeWidth && nodeHeight) {
                    drawInteractiveOverlays( // Gọi hàm từ utils.js
                        screenshotImg, rawElementsDataForModal, nodeWidth, nodeHeight,
                        imageContainer,
                        // currentSelectedPieConditions, // Không cần truyền vào đây nữa, drawOverlays không cần biết selection
                        (elDataFromOverlay) => { // Sửa callback cho drawInteractiveOverlays
            ...
            console.error("Lỗi tải elements cho modal (trong onModalShown):", error);
        ...
        updateVisualizerSelections(); // Gọi sau khi drawOverlays và renderElementTextList
    } else {
        console.error("Modal image clientWidth/Height is still 0 even on 'shown.bs.modal'. Cannot draw overlays.");
        if (imageContainer && !imageContainer.querySelector('.overlay-dimension-error')) {
            const errorMsgEl = document.createElement('p');
...
async function openModalWithData(nodeData) {
    ...
    if (screenshotUrlToLoad && screenshotImg) {
        ...
        // Không đặt display:block ở đây, để 'shown.bs.modal' xử lý

        // Xóa listener cũ của 'shown.bs.modal' nếu có và gắn listener mới
        // Điều này đảm bảo onModalShownAndImageReady chỉ chạy cho lần mở modal hiện tại.
        if (modalEl) {
            modalEl.removeEventListener('shown.bs.modal', onModalShownAndImageReady); // Xóa listener cũ
            modalEl.addEventListener('shown.bs.modal', onModalShownAndImageReady, { once: true }); // Gắn mới, chỉ chạy 1 lần
        }
        ...
        screenshotImg.onload = () => {
            ...
            // Không gọi checkDimensionsAndDraw ở đây nữa, để 'shown.bs.modal' xử lý
        };
...
export function initManagePieModal() {
    ...
    if (mainActionBtn) {
        mainActionBtn.addEventListener('click', async function () {
            // ... (Logic nút chính giữ nguyên như phiên bản trước) ...
            this.disabled = true;
