    ...
    function attachTableTriggers() {
        ...
        nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER).forEach(trigger => {
            ...
            newTrigger.addEventListener('click', function () {
                ...
                if (!row) return;
                const nodeData = {
                    nodeNeo4jId: rowElement.dataset.nodeNeo4jId, // Đọc từ data-node-neo4j-id
                    currentScreenId: rowElement.dataset.currentScreenId,
                    appName: rowElement.dataset.appName,
                    nodeStatus: rowElement.dataset.nodeStatus,
                    screenshotFullUrl: rowElement.dataset.screenshotFullUrl,
                    screenshotFilename: rowElement.dataset.screenshotFilename,
                    activityName: rowElement.dataset.activityName,
                    pieLogicalName: rowElement.dataset.pieLogicalName,
                    width: parseInt(rowElement.dataset.width) || null, // Chuyển sang số
                    height: parseInt(rowElement.dataset.height) || null // Chuyển sang số
                };
                console.log("[TableHandler/Main] nodeData prepared for modal:", JSON.stringify(nodeData)); // DEBUG
