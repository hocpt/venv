...
function handleElementSelectionFromVisualizer(elementOriginalIndex) {
    ...
    if (rawElementsDataForModal === null || rawElementsDataForModal === undefined || rawElementsDataForModal.length === 0 || elementOriginalIndex < 0 || elementOriginalIndex >= rawElementsDataForModal.length) {
        console.warn("[MODAL_PIE] Invalid elementOriginalIndex or rawElementsDataForModal is empty/null.",
            `Index: ${elementOriginalIndex}`,
            `rawElementsDataForModal length: ${rawElementsDataForModal ? rawElementsDataForModal.length : 'null/undefined'}`);
        return;
    }
    ...
    const existingConditionIndex = currentSelectedPieConditions.findIndex(
        cond => cond.internal_element_index === elementOriginalIndex && !cond.is_manual // Chỉ toggle nếu không phải điều kiện thủ công
    );
    ...
    if (existingConditionIndex > -1) {
        ...
        if (elementData.resource_id && typeof elementData.resource_id === 'string' && elementData.resource_id.trim() !== '') {
            ...
            // Fallback: Lấy thuộc tính đầu tiên trong APP_CONFIG hoặc 'text'
            if (APP_CONFIG.ELEMENT_ATTRIBUTES_FOR_PIE && APP_CONFIG.ELEMENT_ATTRIBUTES_FOR_PIE.length > 0) {
                ...
                // Thử tìm giá trị cho thuộc tính mặc định này từ elementData
                if (elementData.hasOwnProperty(defaultAttribute)) {
                    defaultValue = elementData[defaultAttribute] || '';
                } else {
                    defaultValue = ''; // Để trống nếu thuộc tính đó không có trong elementData
                }
            } else {
                defaultAttribute = 'text';
                defaultValue = elementData.text_content || ''; // Thử lấy text nếu có
            }
            displayIdentifier = `Elem Index ${elementOriginalIndex} (Cần cấu hình lại thuộc tính nếu muốn)`;
        }
        ...
        const newCondition = {
            ...
            comparison: 'EQUALS',
            value: defaultValue,
            is_manual: false // Đánh dấu đây không phải điều kiện thủ công
        };
