...
// Giả định hàm openManagePieConditionsModalGlobal được export từ modal_manage_pie.js
// và gán vào window object trong modal_manage_pie.js hoặc main.js
...
export function initTableHandler() {
    ...
    function buildApiUrl(page = 1, appName = null, statusFilter = null) {
        ...
        // Đảm bảo APP_CONFIG.API_MANAGED_NODES_URL đã được khởi tạo đúng trong main.js
    ...
    function renderNodeRow(node) {
        ...
        row.dataset.screenshotFullUrl = node.screenshot_full_url || ''; // URL đầy đủ từ API
        ...
        if (node.screenshot_full_url) { // Sử dụng giá trị biến node.screenshot_full_url
        ...
        row.innerHTML = `
            ...
            <td><a href="${screenElementsUrl}" target="_blank" title="Xem elements của ${node.screen_id}"><code class="small">${(node.screen_id || 'N/A').substring(0, 20)}</code></a></td>
    ...
    function renderNodePagination(paginationData, currentPageFilters) {
        ...
        // Gắn lại sự kiện cho các link phân trang mới được render
        paginationContainer.querySelectorAll('.page-link').forEach(link => {
            if (link.closest('.page-item.disabled')) return;
            // Xóa listener cũ (nếu có) trước khi thêm mới để tránh duplicate
    ...
    function attachTableTriggers() {
        ...
        // Sử dụng event delegation cho các trigger để không cần gắn lại sau mỗi lần render
        // Tuy nhiên, để đơn giản và nhất quán với cách bạn đã có, chúng ta sẽ gắn lại sau mỗi lần render
        // Xóa listener cũ trước khi thêm mới là một cách, hoặc cloneNode(true) rồi replace
        ...
        nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER).forEach(trigger => {
            ...
            // Xóa listener cũ bằng cách clone và replace (nếu trigger là button/input)
            // Với img, gán trực tiếp onclick có thể dễ hơn nếu không muốn clone
            trigger.onclick = () => {
                if (window.openManagePieConditionsModalGlobal) { // Gọi hàm global từ modal_manage_pie.js
        ...
        nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.DEFINE_NEW_PIE_TRIGGER).forEach(button => {
            ...
            const nodeDataForPieDef = {
                nodeNeo4jId: row.dataset.nodeNeo4jId, currentScreenId: row.dataset.currentScreenId,
                appName: row.dataset.appName, nodeStatus: 'unknown', // Luôn là unknown cho luồng này
                screenshotFilename: row.dataset.screenshotFilename,
                screenshotFullUrl: row.dataset.screenshotFullUrl,
                activityName: row.dataset.activityName,
                width: parseInt(row.dataset.width) || null, height: parseInt(row.dataset.height) || null
            ...
            button.onclick = () => {
                if (window.openManagePieConditionsModalGlobal) { // Vẫn mở modal chọn conditions trước
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
            ...
            tooltipTriggerList.forEach(function (tooltipTriggerEl) { // Dùng forEach thay map
                var existingTooltip = bootstrap.Tooltip.getInstance(tooltipTriggerEl);
                if (existingTooltip) { existingTooltip.dispose(); } // Xóa tooltip cũ nếu có
                new bootstrap.Tooltip(tooltipTriggerEl); // Tạo tooltip mới
            });
        }
    }
    ...
    async function fetchAndRenderTableNodes(page = 1, appName = null, statusFilter = null) {
        ...
        try {
            const data = await sendApiRequest(apiUrl, 'GET'); // sendApiRequest từ utils.js
        ...
        if (paginationContainer && paginationContainer.innerHTML.trim() !== '') {
            paginationContainer.querySelectorAll('.page-link').forEach(link => {
                ...
                newLink.addEventListener('click', function (e) {
                    ...
                    // Lấy filter hiện tại từ select boxes khi click pagination
