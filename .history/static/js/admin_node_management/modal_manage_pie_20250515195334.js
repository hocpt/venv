...
export async function openManagePieConditionsModal(nodeData) {
    ...
    const screenshotUrlFromData = currentManagingNodeData.screenshotFullUrl; // Sử dụng URL đã được backend tạo

    if (screenshotUrlFromData && screenshotImg) {
        screenshotImg.src = screenshotUrlFromData; // ĐẶT SRC TRỰC TIẾP
        screenshotImg.onload = async () => {
            // Logic tải elements và vẽ overlays chỉ chạy KHI ảnh đã tải thành công
            try {
                ...
                if (elementsJsonResponse.success && elementsJsonResponse.elements) {
                    ...
                    // Gọi hàm vẽ overlay, truyền kích thước gốc của node
                    drawInteractiveOverlays(
                        screenshotImg,
                        rawElementsDataForModal,
                        currentManagingNodeData.width, // width gốc
                        currentManagingNodeData.height, // height gốc
                        imageContainer, // container của ảnh trong modal
                        currentSelectedPieConditions, // để highlight nếu cần
                        handleElementSelectionFromVisualizer // callback khi click
                    );
                    renderElementTextList(rawElementsDataForModal); // Render danh sách text elements
                } else {
        ...
        screenshotImg.onerror = () => {
            ...
            if (imageContainer) imageContainer.innerHTML = `<p class="text-danger p-2">Lỗi tải ảnh. URL: ${screenshotImg.src || 'Không có URL'}</p>`;
