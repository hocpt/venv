...
// Giả định các hàm mở modal được gán vào window object từ các module tương ứng
// ví dụ: window.openManagePieConditionsModalGlobal
...
export function initTableHandler() {
    ...
    function renderNodeRow(node) {
        ...
        // --- SỬA LỖI RENDER ẢNH ---
        // Sử dụng template literals (dấu `) để dễ dàng chèn biến
        ...
        if (node.screenshot_full_url) {
            // Sử dụng giá trị của biến node.screenshot_full_url
            imgHtml = `<img src="${node.screenshot_full_url}" alt="ss_${node.screen_id || 'unknown'}" class="node-thumbnail ${APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER}" title="Quản lý PIE Conditions">`;
        }
    ...
    function renderNodePagination(paginationData, currentPageFilters) {
        // ... (Nội dung hàm renderNodePagination như đã cung cấp ở các phản hồi trước)
    ...
    async function handleDeleteNode(screenId, appName) {
        // ... (Nội dung hàm handleDeleteNode như đã cung cấp)
        if (!confirm(`Bạn có chắc chắn muốn xóa Node '${screenId}' của app '${appName}' không?`)) {
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                throw new Error(result.error || result.message || "Xóa Node thất bại.");
            }
        } catch (error) {
    ...
    async function handleNodeClassificationChange(event) {
        // ... (Nội dung hàm handleNodeClassificationChange như đã cung cấp)
        ...
        try {
            ...
            if (statusSpan) {
                if (result.success) {
                    ...
                    throw new Error(result.error || result.message || "Lỗi cập nhật.");
                }
    ...
    function attachTableTriggers() {
        ...
        nodesTableBody.querySelectorAll('.' + APP_CONFIG.CSS_CLASSES.MANAGE_PIE_TRIGGER).forEach(trigger => {
            ...
            // Gán sự kiện trực tiếp, hoặc cloneNode và addEventListener nếu muốn xóa listener cũ triệt để hơn
            trigger.onclick = () => {
    ...
    if (nodesTableBody && nodesTableBody.children.length > 0 &&
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
            var serverTooltips = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]')) // Lấy từ toàn bộ document
