...
async function openModal(nodeData) {
    ...
    renderSelectedPieConditions(); // Gọi lại để cập nhật Phần 3 và visualizer
    if(managePieModalInstance) managePieModalInstance.show(); else console.error("managePieModalInstance is null, cannot show.");
}
...
export function initManagePieModal() {
    ...
    if (!modalEl) return;
    managePieModalInstance = new bootstrap.Modal(modalEl); // Khởi tạo instance ở đây

    // Gán hàm vào global scope để các module khác (table_handler) có thể gọi
    ...
    if (mainActionBtn) {
        mainActionBtn.addEventListener('click', async function() {
            ...
            const finalConditionsToSave = currentSelectedPieConditions.map(cond => ({
                ...
                value: cond.value
            })).filter(c => c.attribute && c.comparison && (c.comparison === 'EXISTS' || c.comparison === 'NOT_EXISTS' || c.value)); // Lọc bỏ condition chưa hoàn chỉnh
